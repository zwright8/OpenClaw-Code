---
name: openclaw-2496-failure-root-cause-mining
description: Failure Root Cause Mining for disaster response networks. Use when work requires failure root cause mining for disaster response networks with guardrails, traceable execution, and measurable outcomes.
---
...
## Activation Cues
- Task requires failure root cause mining in disaster response networks.
...
## Guardrails
- [quality] Require unit and integration validations before promoting Failure Root Cause Mining for disaster response networks. Automation: `run-validation:unit+integration+simulation+regression-baseline`
...
- Return artifact target: `failure-root-cause-mining-artifact-disaster-response-networks`.
