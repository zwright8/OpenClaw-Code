---
name: theater-3d-printed-uas-threat-signature-library-cell
description: Build and maintain a continuously updated signature library for low-cost 3D-printed UAS threats and improvised payload profiles.
---

# Theater 3D-Printed UAS Threat Signature Library Cell
...
Primary products for this skill: uas signature atlas, detection rule release pack, countermeasure prioritization matrix.

## Domain Tool Stack

Use these tool categories as the baseline stack: radars and EO/IR analytics, acoustic and RF classifiers, forensic additive-manufacturing labs, counter-UAS C2 feeds.

## Protocol Profile

Preferred protocol families for this skill: STANAG 4607, Cursor-on-Target, protobuf telemetry, STIX/TAXII.
...
- Prioritize `tool_suite_id=ts-theater-3d-printed-uas-threat-signature-library-cell-v1` + `protocol_stack_id=ps-theater-3d-printed-uas-threat-signature-library-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-THEATER_3D_PRINTED_UAS_THREAT_SIGNATURE_LIBRARY_CELL-001` for the primary course of action and `packet_id=DPL-THEATER_3D_PRINTED_UAS_THREAT_SIGNATURE_LIBRARY_CELL-002` for degraded-mode fallback actions.
