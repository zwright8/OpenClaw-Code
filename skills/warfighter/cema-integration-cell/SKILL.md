...
- Add `tool_suite_id=ts-theater-digital-terrain-fabric-spoof-route-approval-v1` + `protocol_stack_id=ps-theater-digital-terrain-fabric-spoof-route-approval-stack-v1` when CEMA posture depends on detecting geospatial tampering that can misroute friendly maneuver under cyber-electromagnetic pressure.
- Add `tool_suite_id=ts-expeditionary-autonomous-sustainment-routing-drift-governance-v1` + `protocol_stack_id=ps-expeditionary-autonomous-sustainment-routing-drift-governance-stack-v1` when cyber or EW disruption degrades autonomous route trust, telemetry confidence, or convoy release timing.
- Add `tool_suite_id=ts-joint-contested-cellular-timing-holdover-first-responder-priority-v1` + `protocol_stack_id=ps-joint-contested-cellular-timing-holdover-first-responder-priority-stack-v1` when CEMA recommendations depend on telecom timing integrity, backhaul failover, or public-safety traffic prioritization.
- Add `tool_suite_id=ts-joint-cislunar-timing-trust-collision-mitigation-v1` + `protocol_stack_id=ps-joint-cislunar-timing-trust-collision-mitigation-stack-v1` when space-timing trust and custody assurance affect cross-domain synchronization under contested electromagnetic conditions.
- Add `packet_id=DPL-THEATER-DIGITAL-TERRAIN-SPOOF-ROUTE-APPROVAL-001`, `packet_id=DPL-EXPEDITIONARY-AUTONOMOUS-SUSTAINMENT-ROUTING-DRIFT-001`, `packet_id=DPL-JOINT-CELLULAR-TIMING-HOLDOVER-FIRST-RESPONDER-001`, and `packet_id=DPL-JOINT-CISLUNAR-TIMING-COLLISION-MITIGATION-001` for branches that alter CEMA release posture, timing integrity, or command-network continuity assumptions.

## Domain Toolchain Override (2026-03-14, Expansion Wave LIV Addendum)

- Add `tool_suite_id=ts-theater-emissions-decoy-robotic-control-spectrum-safety-v1` + `protocol_stack_id=ps-theater-emissions-decoy-robotic-control-spectrum-safety-stack-v1` when CEMA recommendations depend on robotic decoy emissions, blue-force sensor deconfliction, or verified human override.
