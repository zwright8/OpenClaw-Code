---
name: coalition-jadc2-message-priority-and-releasability-bridge-cell
description: Coordinate coalition JADC2 message priority, releasability, and acknowledgment bridging when combined operations depend on trusted cross-caveat data flow under time pressure.
---
...
1. Frame the mission problem using these core inputs: message classes, coalition caveats, transport paths, timing thresholds, and commander release authorities.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current bridging plan.
3. Build primary and alternate options with explicit tradeoffs in speed, releasability, fidelity, and acknowledgment assurance.
...
Primary products for this skill: coalition message priority ladder, releasability bridge matrix, acknowledgment integrity board.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: message priority router, releasability rules ledger, coalition acknowledgment tracker, schema translation workbench.
...
- Prefer these protocol families for this skill: Link 16 J-series, USMTF, NATO APP-11/ADatP-3 aligned exchange, API/JSON, NIEM.
...
- Do not fabricate sources, approvals, or coalition authorities.
...
- Primary: `tool_suite_id=ts-coalition-jadc2-message-priority-releasability-bridge-v1` with `protocol_stack_id=ps-coalition-jadc2-message-priority-releasability-bridge-stack-v1`.
- Alternate: `tool_suite_id=ts-joint-c2-fusion-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-COALITION-JADC2-PRIORITY-BRIDGE-001` for branches that materially alter coalition release speed, message ordering, or acknowledgment integrity.
