---
name: joint-high-altitude-nuclear-effects-continuity-cell
description: Plan mission continuity under high-altitude nuclear effects impacting space, electromagnetic, and critical infrastructure dependencies.
---
...
Primary products for this skill: high-altitude nuclear effects continuity board, mission dependency degradation map, reconstitution branch packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: EMP consequence models, NC3 resilience dashboards, mission dependency graph tools, restoration sequencing planners.

## Protocol Profile

Preferred protocol families for this skill: USMTF, STANAG-aligned CBRN exchange, OGC WMS/WFS/WMTS, API/JSON.
...
- Prioritize `tool_suite_id=ts-joint-high-altitude-nuclear-effects-continuity-cell-v1` + `protocol_stack_id=ps-joint-high-altitude-nuclear-effects-continuity-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-JOINT_HIGH_ALTITUDE_NUCLEAR_EFFECTS_CONTINUITY_CELL-001` for the primary course of action and `packet_id=DPL-JOINT_HIGH_ALTITUDE_NUCLEAR_EFFECTS_CONTINUITY_CELL-002` for degraded-mode fallback actions.
