---
name: coalition-expeditionary-data-center-emissions-and-signature-control-cell
description: Control expeditionary data-center emissions and signatures to reduce detectability while preserving coalition compute continuity. Use when survivability depends on electromagnetic and thermal discipline.
---
...
Primary products for this skill: expeditionary data-center signature posture brief, emissions-control enforcement board, compute failover sequence.
...
- Default to `tool_suite_id=ts-expeditionary-datacenter-signature-control-v1` and `protocol_stack_id=ps-expeditionary-datacenter-signature-control-stack-v1` for mission-critical recommendations in this domain.
- Include `interop_standard_set`, `endpoint_security_profile`, `primary_exchange_path`, and `degraded_exchange_method` fields in each tool invocation packet.
- If tool freshness, authority, or cross-check confidence is insufficient, mark outputs as `provisional`, require human approval, and publish revalidation suspense.
