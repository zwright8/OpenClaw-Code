---
name: theater-fuel-pier-hose-farm-and-bulk-transfer-bypass-restoration-cell
description: Support U.S. warfighter planning for fuel-pier, hose-farm, and bulk-transfer bypass restoration when contested ports and shore infrastructure threaten theater sustainment.
---
...
1. Frame the mission problem using these core inputs: fuel demand, pier and hose-farm status, maritime threat, transfer equipment readiness, and port constraints.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current bypass-restoration plan.
3. Build primary and alternate options with explicit tradeoffs in throughput, survivability, contamination risk, and repair burden.
...
Primary products for this skill: bulk fuel bypass ladder, hose-farm restoration sequence, contested port fuel throughput board.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: pier transfer status board, hose-farm integrity ledger, bulk transfer bypass planner, fuel quality and contamination tracker.
...
- Prefer these protocol families for this skill: AIS/NMEA, OGC, signed fuel-quality manifests, API/JSON, USMTF.
...
- Primary: `tool_suite_id=ts-theater-fuel-pier-hose-farm-bulk-transfer-bypass-restoration-v1` with `protocol_stack_id=ps-theater-fuel-pier-hose-farm-bulk-transfer-bypass-restoration-stack-v1`.
- Alternate: `tool_suite_id=ts-logistics-distribution-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-FUEL-PIER-BYPASS-001` for branches that materially alter fuel throughput, contamination posture, or port restoration priority.
