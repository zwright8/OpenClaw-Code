---
name: theater-joint-fires-legal-review-acceleration-cell
description: Accelerate lawful fires review workflows while preserving LOAC evidence, proportionality analysis, and command accountability.
---

# Theater Joint Fires Legal Review Acceleration Cell
...
Primary products for this skill: target legal review board packet, proportionality evidence chain, dynamic no-strike update bundle.

## Domain Tool Stack

Use these tool categories as the baseline stack: targeting workbench, geospatial collateral damage estimators, legal workflow case systems, evidence integrity vaults.

## Protocol Profile

Preferred protocol families for this skill: USMTF, OGC GeoJSON/WMS, STANAG 4559, CMS signatures.
...
- Prioritize `tool_suite_id=ts-theater-joint-fires-legal-review-acceleration-cell-v1` + `protocol_stack_id=ps-theater-joint-fires-legal-review-acceleration-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-THEATER_JOINT_FIRES_LEGAL_REVIEW_ACCELERATION_CELL-001` for the primary course of action and `packet_id=DPL-THEATER_JOINT_FIRES_LEGAL_REVIEW_ACCELERATION_CELL-002` for degraded-mode fallback actions.
