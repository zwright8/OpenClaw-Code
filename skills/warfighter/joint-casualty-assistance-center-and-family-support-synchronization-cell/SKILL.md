...
- Add `toolchain_id=TC-SURVIVOR-297`, `tool_suite_id=ts-joint-survivor-benefits-casualty-assistance-records-expedite-v1`, and `protocol_stack_id=ps-joint-survivor-benefits-casualty-assistance-records-expedite-stack-v1` when family-support actions depend on beneficiary verification, survivor-benefits initiation, or records-expedite legitimacy.
- Add `toolchain_id=TC-ARCMSG-295`, `tool_suite_id=ts-joint-emergency-leave-american-red-cross-message-command-approval-v1`, and `protocol_stack_id=ps-joint-emergency-leave-american-red-cross-message-command-approval-stack-v1` when casualty-support timing depends on trusted emergency-leave validation for immediate family travel or bedside presence.
- Add `packet_id=DPL-SURVIVOR-BENEFITS-CASUALTY-001` and `packet_id=DPL-EMERGENCY-LEAVE-ARC-001` for branches that materially alter survivor-support confidence, next-of-kin travel timing, or records-linked entitlement posture.
