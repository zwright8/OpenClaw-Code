---
name: joint-commercial-space-ssa-data-fusion-and-tasking-cell
description: Fuse commercial and military SSA data for resilient tasking and warning during contested space operations.
---
...
Primary products for this skill: SSA confidence-ranked fusion brief, sensor tasking matrix, conjunction and threat warning packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: commercial SSA broker feeds, military SDA catalogs, orbital tasking planner, warning dissemination board.

## Protocol Profile

Preferred protocol families for this skill: CCSDS, OGC WMS/WFS/WMTS, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-joint-commercial-space-ssa-data-fusion-and-tasking-cell-v1` + `protocol_stack_id=ps-joint-commercial-space-ssa-data-fusion-and-tasking-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-JOINT_COMMERCIAL_SPACE_SSA_DATA_FUSION_AND_TASKING_CELL-001` for the primary course of action and `packet_id=DPL-JOINT_COMMERCIAL_SPACE_SSA_DATA_FUSION_AND_TASKING_CELL-002` for degraded-mode fallback actions.
