---
name: joint-precision-aerial-delivery-and-dropzone-assurance-cell
description: Support U.S. warfighter planning and operational decision support for Joint Precision Aerial Delivery And Dropzone Assurance Cell scenarios across joint and coalition mission environments. Use when planning, synchronizing, or adapting operations that require cross-domain coordination under uncertainty.
---

# Joint Precision Aerial Delivery And Dropzone Assurance Cell
...
Primary products for this skill: dropzone readiness board, delivery risk matrix, recovery synchronization plan.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: air mission planning tools, weather and winds decision systems, ground recovery coordination systems, logistics COP.
...
- Prefer these protocol families for this skill: USMTF, AIXM/FIXM, CoT.
...
- Add `tool_suite_id=ts-joint-aerial-delivery-rigging-parachute-inspection-v1` + `protocol_stack_id=ps-joint-aerial-delivery-rigging-parachute-inspection-stack-v1` when mission viability depends on trusted parachute serviceability, rigging certification, or load-release discipline.
- Add `tool_suite_id=ts-theater-heavy-drop-platform-rigging-retrograde-recovery-v1` + `protocol_stack_id=ps-theater-heavy-drop-platform-rigging-retrograde-recovery-stack-v1` when heavy-drop platform integrity, extraction-system health, or recovery-force timing determines usable delivery throughput.
- Add `packet_id=DPL-AERIAL-DELIVERY-RIGGING-001` and `packet_id=DPL-HEAVY-DROP-PLATFORM-RIGGING-001` for branches that materially alter release authority, delivered-load confidence, or retrograde recovery timing.
