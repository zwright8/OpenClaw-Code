...
- Add `tool_suite_id=ts-strategic-tritium-reservoir-surveillance-maintenance-priority-v1` + `protocol_stack_id=ps-strategic-tritium-reservoir-surveillance-maintenance-priority-stack-v1` when commander risk posture depends on tritium surveillance backlogs, secure transport windows, or maintenance surety bottlenecks.
- Add `tool_suite_id=ts-homeland-chemical-plume-shelter-installation-access-control-v1` + `protocol_stack_id=ps-homeland-chemical-plume-shelter-installation-access-control-stack-v1` when risk acceptance depends on toxic-plume shelter timing, installation access restrictions, or civil-military protective actions.
- Add `tool_suite_id=ts-theater-mobile-hardened-shelter-nc3-relay-antenna-reconstitution-v1` + `protocol_stack_id=ps-theater-mobile-hardened-shelter-nc3-relay-antenna-reconstitution-stack-v1` when risk posture depends on displacing and reconnecting theater command nodes after strike or sabotage.
- Add `tool_suite_id=ts-theater-mission-device-credential-revocation-key-reissue-v1` + `protocol_stack_id=ps-theater-mission-device-credential-revocation-key-reissue-stack-v1` when commander confidence depends on rapidly burning compromised device credentials without losing mission-essential access.
- Add `tool_suite_id=ts-strategic-quantum-network-key-custody-and-compromise-response-cell-v1` + `protocol_stack_id=ps-strategic-quantum-network-key-custody-and-compromise-response-cell-stack-v1` when commander risk posture depends on strategic cryptographic custody, compromise confidence, or rollback-safe continuity.
- Add `tool_suite_id=ts-joint-gray-zone-commercial-fleet-shadowing-attribution-cell-v1` + `protocol_stack_id=ps-joint-gray-zone-commercial-fleet-shadowing-attribution-cell-stack-v1` when mission risk depends on gray-zone maritime coercion against commercial lift, sealift routing, or escalation-aware attribution.
- Add `tool_suite_id=ts-theater-hydrogen-fuel-cell-microgrid-safety-and-emissions-control-cell-v1` + `protocol_stack_id=ps-theater-hydrogen-fuel-cell-microgrid-safety-and-emissions-control-cell-stack-v1` when commander risk acceptance depends on hydrogen power-node safety, leak containment, or emissions-aware continuity.
- Add `packet_id=DPL-TRITIUM-RESERVOIR-MAINTENANCE-001`, `packet_id=DPL-CHEMICAL-PLUME-SHELTER-ACCESS-001`, `packet_id=DPL-MOBILE-NC3-RELAY-SHELTER-001`, `packet_id=DPL-MISSION-DEVICE-CREDENTIAL-REISSUE-001`, `packet_id=DPL-QUANTUM-NETWORK-KEY-CUSTODY-001`, `packet_id=DPL-GRAYZONE-FLEET-SHADOWING-001`, and `packet_id=DPL-HYDROGEN-MICROGRID-SAFETY-001` for branches that materially change GO/NO-GO posture, escalation confidence, or continuity trust.

## Domain Toolchain Override (2026-03-14, Expansion Wave LIV Addendum)

- Add `tool_suite_id=ts-joint-submarine-escape-rescue-system-integration-v1` + `protocol_stack_id=ps-joint-submarine-escape-rescue-system-integration-stack-v1` when commander risk acceptance depends on trapped-submariner recovery timelines, chamber capacity, or coalition rescue compatibility.
- Add `tool_suite_id=ts-joint-orbital-custody-loss-commercial-ssa-retask-v1` + `protocol_stack_id=ps-joint-orbital-custody-loss-commercial-ssa-retask-stack-v1` when mission risk posture depends on regained orbital custody, conjunction confidence, or maneuver-release timing.
- Add `tool_suite_id=ts-theater-air-gapped-credential-reissue-pki-recovery-v1` + `protocol_stack_id=ps-theater-air-gapped-credential-reissue-pki-recovery-stack-v1` when commander confidence depends on restoring trusted access inside disconnected enclaves after credential compromise.
