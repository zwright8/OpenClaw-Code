---
name: theater-air-defense-emitter-decoy-logistics-cell
description: Plan theater-scale air defense emitter and decoy logistics to preserve survivability under ISR and fires pressure.
---
...
Primary products for this skill: emitter-decoy allocation board, survivability sustainment branch plan, reload/reposition timeline.

## Domain Tool Stack

Use these tool categories as the baseline stack: air defense COP, decoy inventory optimizer, contested movement planning tools, adversary ISR pattern analytics.

## Protocol Profile

Preferred protocol families for this skill: Link 16 J-series, VMF, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-theater-air-defense-emitter-decoy-logistics-cell-v1` + `protocol_stack_id=ps-theater-air-defense-emitter-decoy-logistics-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-THEATER_AIR_DEFENSE_EMITTER_DECOY_LOGISTICS_CELL-001` for the primary course of action and `packet_id=DPL-THEATER_AIR_DEFENSE_EMITTER_DECOY_LOGISTICS_CELL-002` for degraded-mode fallback actions.
