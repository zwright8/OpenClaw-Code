---
name: joint-cyber-electromagnetic-spectrum-control-and-mission-reroute-cell
description: Coordinate joint cyber-electromagnetic control actions and mission rerouting under contested data links and degraded communications.
---
...
Primary products for this skill: joint cyber-EM control matrix, mission reroute execution board, authority-gated reroute packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: cyber mission assurance boards, electromagnetic order-of-battle fusion tools, mission-thread reroute orchestrators.

## Protocol Profile

Preferred protocol families for this skill: USMTF, Link 16 J-series, STIX/TAXII, VMF, API/JSON.
...
- Prioritize `tool_suite_id=ts-joint-cyber-em-spectrum-mission-reroute-v1` + `protocol_stack_id=ps-joint-cyber-em-spectrum-mission-reroute-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-JOINT-CYBER-EM-REROUTE-001` for the primary course of action and `packet_id=DPL-JOINT-CYBER-EM-REROUTE-002` for degraded-mode fallback actions.
