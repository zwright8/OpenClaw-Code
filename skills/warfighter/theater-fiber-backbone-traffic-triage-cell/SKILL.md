---
name: theater-fiber-backbone-traffic-triage-cell
description: Triage theater backbone outages and prioritize mission-critical traffic during fiber disruption. Use when commanders must preserve command, fires, medical, and sustainment data paths under cable loss.
---
...
Primary products for this skill: traffic triage playbook, reroute priority matrix, restoration sequencing packet.
...
- Prioritize `tool_suite_id=ts-fiber-backbone-traffic-triage-v1` + `protocol_stack_id=ps-fiber-backbone-traffic-triage-stack-v1` for this mission set.
- Include `packet_id=DPL-THEATER-FIBER-BACKBONE-TRAFFIC-TRIAGE-001` for high-consequence recommendations and branch decisions.
- Bind recommendations to `toolchain_profile_id=theater-fiber-backbone-traffic-triage-v1` in joint operations profile selection.
