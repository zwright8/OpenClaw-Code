---
name: joint-precision-navigation-spoofing-adjudication-cell
description: Adjudicate suspected PNT spoofing and coordinate resilient navigation branches across joint forces. Use when timing or geolocation trust degrades mission execution.
---

# Joint Precision Navigation Spoofing Adjudication Cell
...
1. Frame the mission problem using these core inputs: PNT integrity alarms, inertial and terrain cross-check confidence, mission timing constraints, and platform exposure map.
...
Primary products for this skill: spoofing confidence ladder, alternate navigation branch plan, timing integrity escalation matrix.
...
## Domain Toolchain Override (2026-03-09, PNT Spoofing Trust Recovery)

- Prioritize tool_suite_id=ts-precision-navigation-spoofing-adjudication-v1 with protocol_stack_id=ps-precision-navigation-spoofing-adjudication-stack-v1 when geolocation trust degrades across multiple platforms.
- Use packet DPL-PRECISION-NAVIGATION-SPOOFING-ADJUDICATION-001 to bind confidence bands, fallback navigation modes, and commander release checkpoints.
- If dual-source timing integrity cannot be maintained, downgrade to advisory-only pathing guidance and require explicit command approval for high-risk maneuvers.
