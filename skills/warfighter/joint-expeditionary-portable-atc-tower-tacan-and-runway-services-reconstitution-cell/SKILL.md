---
name: joint-expeditionary-portable-atc-tower-tacan-and-runway-services-reconstitution-cell
description: Support U.S. warfighter planning for expeditionary portable ATC tower, TACAN, and runway services reconstitution when austere or battle-damaged airfields must resume flight operations fast.
---
...
1. Frame the mission problem using these core inputs: sortie demand, runway status, ATC/TACAN equipment state, weather, and airspace constraints.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current reconstitution plan.
3. Build primary and alternate options with explicit tradeoffs in sortie tempo, safety margin, sustainment burden, and exposure risk.
...
Primary products for this skill: airfield services restoration ladder, portable ATC and TACAN emplacement plan, sortie release constraints matrix.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: portable tower readiness board, TACAN calibration queue, runway services status tracker, austere airfield airspace overlay.
...
- Prefer these protocol families for this skill: AIXM/FIXM, VMF, USMTF, API/JSON, signed airfield equipment manifests.
...
- Primary: `tool_suite_id=ts-joint-expeditionary-portable-atc-tower-tacan-runway-services-v1` with `protocol_stack_id=ps-joint-expeditionary-portable-atc-tower-tacan-runway-services-stack-v1`.
- Alternate: `tool_suite_id=ts-airfield-recovery-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-PORTABLE-ATC-TACAN-001` for branches that materially alter sortie release timing, airfield serviceability, or risk acceptance.
