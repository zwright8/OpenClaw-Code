---
name: theater-telemetry-denied-missile-launch-characterization-cell
description: Characterize missile launches under telemetry denial to support warning, attribution, and response options. Use when adversary emissions control or data loss obscures launch type, trajectory, or intent.
---
...
1. Frame the mission problem using these core inputs: early warning sensor detections, radar/IR track fragments, adversary order-of-battle context, geospatial constraints, and confidence thresholds.
...
Primary products for this skill: launch characterization confidence report, warning-to-response option matrix, attribution uncertainty ledger.
...
- Primary: `tool_suite_id=ts-telemetry-denied-missile-characterization-v1` with `protocol_stack_id=ps-joint-tactical-link-stack-v1`.
- Alternate: `tool_suite_id=ts-strategic-deterrence-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
