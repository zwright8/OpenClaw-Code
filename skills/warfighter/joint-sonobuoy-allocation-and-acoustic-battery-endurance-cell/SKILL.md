---
name: joint-sonobuoy-allocation-and-acoustic-battery-endurance-cell
description: Support U.S. warfighter planning for sonobuoy allocation, acoustic battery endurance, and coverage persistence when ASW operations must hold contact under limited stores.
---
...
1. Frame the mission problem using these core inputs: threat submarine picture, buoy inventories, aircraft and ship availability, acoustic conditions, and search priorities.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current buoy-allocation plan.
3. Build primary and alternate options with explicit tradeoffs in coverage persistence, battery life, sortie burden, and contact risk.
...
Primary products for this skill: sonobuoy expenditure ladder, acoustic endurance forecast, contact-persistence coverage plan.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: sonobuoy load planner, acoustic battery endurance board, pattern coverage analyzer, ASW contact confidence tracker.
...
- Prefer these protocol families for this skill: Link 16 J-series, USMTF, OGC, API/JSON, signed mission-load manifests.
...
- Primary: `tool_suite_id=ts-joint-sonobuoy-allocation-acoustic-battery-endurance-v1` with `protocol_stack_id=ps-joint-sonobuoy-allocation-acoustic-battery-endurance-stack-v1`.
- Alternate: `tool_suite_id=ts-maritime-undersea-v1` with `protocol_stack_id=ps-joint-tactical-link-stack-v1`.
- Add `packet_id=DPL-SONOBUOY-ENDURANCE-001` for branches that materially alter buoy expenditure, barrier persistence, or ASW release confidence.
