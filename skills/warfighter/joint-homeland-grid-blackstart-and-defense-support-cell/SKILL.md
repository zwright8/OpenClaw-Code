---
name: joint-homeland-grid-blackstart-and-defense-support-cell
description: Coordinate homeland grid blackstart and military defense support in contested outages. Use when balancing life-safety loads, mission-essential power restoration, and civil authority synchronization under cyber/kinetic disruption.
---
...
1. Frame the mission problem using these core inputs: grid damage state, generation start capability, critical-load tiers, cyber/physical threat indicators, civil authority constraints.
...
Primary products for this skill: blackstart synchronization matrix, mission-load restoration ladder, DSCA authority escalation log.
...
- Add `tool_suite_id=ts-homeland-grid-blackstart-defense-support-v1` + `protocol_stack_id=ps-homeland-grid-blackstart-defense-support-stack-v1` when this domain is mission-critical in current planning.
- Add `packet_id=DPL-HOMELAND-GRID-BLACKSTART-DEFENSE-001` for branches that can materially alter mission posture or civil-protection outcomes.
