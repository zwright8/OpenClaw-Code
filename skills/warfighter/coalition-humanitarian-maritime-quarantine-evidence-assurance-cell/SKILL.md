---
name: coalition-humanitarian-maritime-quarantine-evidence-assurance-cell
description: Synchronize coalition humanitarian maritime quarantine operations with evidence-grade auditability and throughput continuity.
---
...
Primary products for this skill: quarantine enforcement matrix, humanitarian throughput risk board, evidentiary audit packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: maritime COP, quarantine compliance ledgers, humanitarian flow analytics, legal evidence chain tooling.

## Protocol Profile

Preferred protocol families for this skill: NIMS/ICS, NIEM, AIS/NMEA, API/JSON.
...
- Prioritize `tool_suite_id=ts-coalition-humanitarian-maritime-quarantine-evidence-assurance-cell-v1` + `protocol_stack_id=ps-coalition-humanitarian-maritime-quarantine-evidence-assurance-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-COALITION_HUMANITARIAN_MARITIME_QUARANTINE_EVIDENCE_ASSURANCE_CELL-001` for the primary course of action and `packet_id=DPL-COALITION_HUMANITARIAN_MARITIME_QUARANTINE_EVIDENCE_ASSURANCE_CELL-002` for degraded-mode fallback actions.
