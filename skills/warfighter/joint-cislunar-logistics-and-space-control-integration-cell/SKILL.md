---
name: joint-cislunar-logistics-and-space-control-integration-cell
description: Integrate cislunar sustainment routing, space-control risk, and mission continuity for U.S. joint force operations beyond GEO.
---

# Joint Cislunar Logistics And Space Control Integration Cell
...
Primary products for this skill: cislunar route assurance matrix, launch-to-orbit sustainment branch map, command authority decision ledger.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: SDA catalogs, cislunar transfer-window planners, SATCOM mission-network resilience monitors.

## Protocol Profile

Preferred protocol families for this skill: USMTF, CCSDS message standards, API/JSON.

## Domain Toolchain Defaults

- Primary: `tool_suite_id=ts-cislunar-logistics-control-v1`.
- Alternate: `tool_suite_id=ts-space-satcom-v1`.
...
- Prefer `tool_suite_id=ts-cislunar-logistics-control-v1` for primary decision support and bind to mission packet `DPL-CISLUNAR-LOG-001`.
- Use `tool_suite_id=ts-space-satcom-v1` as required cross-check for divergence or trust-score decay.
- Include `packet_id=DPL-CISLUNAR-LOG-001`, `protocol_profile`, `ack_chain_status`, and `trust_score` in all commander-facing outputs.
