---
name: strategic-space-isr-collection-priority-arbitration-cell
description: Arbitrate strategic space ISR collection priorities across competing combatant command demands and time-sensitive mission windows.
---
...
- Use packet template `DPL-SPACE-ISR-PRIORITY-ARB-001` from `../_shared/references/domain-tool-packet-library.md`.
- Bind tool and protocol choices to `ts-space-isr-priority-arbitration-v1` and `ps-space-isr-priority-arbitration-stack-v1` from `../_shared/references/warfighter-external-tool-and-protocol-catalog.md`.
- Align execution profile with `space-isr-priority-arbitration-v1` from `../_shared/references/joint-operations-external-toolchain-profiles.md`.
...
- Primary: `tool_suite_id=ts-space-isr-priority-arbitration-v1` with `protocol_stack_id=ps-space-isr-priority-arbitration-stack-v1`.
...
- Default packet ID: `DPL-SPACE-ISR-PRIORITY-ARB-001`.
