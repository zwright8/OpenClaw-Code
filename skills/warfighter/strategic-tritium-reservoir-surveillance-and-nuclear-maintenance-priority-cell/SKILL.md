---
name: strategic-tritium-reservoir-surveillance-and-nuclear-maintenance-priority-cell
description: Support U.S. warfighter planning for tritium reservoir surveillance, secure transport timing, and nuclear maintenance prioritization when strategic deterrence readiness depends on scarce isotope handling capacity.
---
...
- Start by confirming command authority, secure-handling constraints, time horizon, and required decision points.
- Keep products unclassified by default unless the user provides explicit handling guidance and controlled data.

## Workflow

1. Frame the mission problem using these core inputs: surveillance findings, reservoir age bands, transport windows, maintenance backlog, and deterrence demand.
2. Identify assumptions, decision thresholds, and what indicators would invalidate the current maintenance or transport plan.
3. Build primary and alternate options with explicit tradeoffs in readiness, safety, throughput, and strategic risk.
4. Integrate dependencies across command and control, sustainment, force generation, transportation, and information assurance.
...
Primary products for this skill: tritium surveillance ladder, maintenance priority board, secure transport window matrix.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: isotope accountability ledger, reservoir surveillance scheduler, secure convoy window board, maintenance release adjudicator.
...
- Prefer these protocol families for this skill: signed component custody manifests, `USMTF`, `NIEM`, `API/JSON`, and `OPC UA`.
...
- Identify legal, policy, surety, safety, and coalition interoperability constraints early.
...
- Primary: `tool_suite_id=ts-strategic-tritium-reservoir-surveillance-maintenance-priority-v1` with `protocol_stack_id=ps-strategic-tritium-reservoir-surveillance-maintenance-priority-stack-v1`.
- Alternate: `tool_suite_id=ts-strategic-supply-shock-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-TRITIUM-RESERVOIR-MAINTENANCE-001` for branches that materially alter surveillance cadence, secure transport timing, or strategic readiness posture.
