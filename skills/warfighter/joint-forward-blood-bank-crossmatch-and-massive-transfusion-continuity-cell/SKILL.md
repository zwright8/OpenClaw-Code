---
name: joint-forward-blood-bank-crossmatch-and-massive-transfusion-continuity-cell
description: Support U.S. warfighter planning for forward blood-bank crossmatch, cold-chain integrity, and massive transfusion continuity when casualty surges threaten lifesaving sustainment.
---
...
1. Frame the mission problem using these core inputs: casualty load, blood product inventory, crossmatch capability, transport constraints, and clinical priorities.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current transfusion support plan.
3. Build primary and alternate options with explicit tradeoffs in survival benefit, cold-chain burden, transport exposure, and resupply timing.
...
Primary products for this skill: blood availability ladder, crossmatch and walking-blood-bank branch plan, massive transfusion risk matrix.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: blood product inventory ledger, crossmatch queue manager, transfusion trigger dashboard, cold-chain movement board.
...
- Prefer these protocol families for this skill: HL7/FHIR, USMTF, API/JSON, signed cold-chain manifests.
...
- Primary: `tool_suite_id=ts-joint-forward-blood-bank-crossmatch-massive-transfusion-v1` with `protocol_stack_id=ps-joint-forward-blood-bank-crossmatch-massive-transfusion-stack-v1`.
- Alternate: `tool_suite_id=ts-medical-force-health-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-FORWARD-BLOOD-BANK-CROSSMATCH-001` for branches that materially alter transfusion release, cold-chain use, or casualty survival priorities.
