---
name: coalition-cross-border-power-grid-load-shed-deconfliction-cell
description: Deconflict coalition cross-border load shedding and military mission priorities during grid instability or attack.
---
...
Primary products for this skill: load-shed deconfliction matrix, mission-priority power allocation board, coalition escalation packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: grid telemetry exchanges, interconnection balancing dashboards, mission-power dependency maps, coalition adjudication workflows.

## Protocol Profile

Preferred protocol families for this skill: NATO APP-11/ADatP-3, IEC CIM, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-coalition-cross-border-power-grid-load-shed-deconfliction-cell-v1` + `protocol_stack_id=ps-coalition-cross-border-power-grid-load-shed-deconfliction-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-COALITION_CROSS_BORDER_POWER_GRID_LOAD_SHED_DECONFLICTION_CELL-001` for the primary course of action and `packet_id=DPL-COALITION_CROSS_BORDER_POWER_GRID_LOAD_SHED_DECONFLICTION_CELL-002` for degraded-mode fallback actions.
