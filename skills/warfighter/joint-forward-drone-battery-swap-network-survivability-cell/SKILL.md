---
name: joint-forward-drone-battery-swap-network-survivability-cell
description: Optimize survivability and throughput of forward drone battery-swap networks under kinetic and cyber pressure.
---

# Joint Forward Drone Battery Swap Network Survivability Cell
...
1. Frame the mission problem using these core inputs: battery inventory state, swap-node uptime, sortie demand, and threat-to-node overlays.
...
Primary products for this skill: battery-swap network survivability map, node hardening priorities, sortie continuity branch plan.
...
## Domain Toolchain Override (2026-03-09, Forward Drone Battery-Swap Network Survivability)

- Prioritize `tool_suite_id=ts-forward-drone-battery-swap-survivability-v1` with `protocol_stack_id=ps-forward-drone-battery-swap-survivability-stack-v1` for this mission set when indicators exceed baseline risk thresholds.
- Use packet `DPL-FORWARD-DRONE-BATTERY-SWAP-SURVIVABILITY-001` to bind machine-ingestible tasking fields, commander decision prompts, and degraded-mode controls.
