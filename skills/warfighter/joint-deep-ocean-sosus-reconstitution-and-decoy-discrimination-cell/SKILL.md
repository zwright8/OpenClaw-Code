---
name: joint-deep-ocean-sosus-reconstitution-and-decoy-discrimination-cell
description: Reconstitute deep-ocean acoustic surveillance capacity and discriminate adversary decoy signatures for joint maritime undersea defense.
---
...
Primary products for this skill: SOSUS node restoration plan, decoy discrimination confidence ledger, undersea cueing authority packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: undersea acoustic fusion engines, seabed node integrity monitors, maritime decoy-discrimination analytics.

## Protocol Profile

Preferred protocol families for this skill: USMTF, STANAG, OGC, AIS/NMEA, API/JSON.
...
- Prioritize `tool_suite_id=ts-joint-deep-ocean-sosus-reconstitution-decoy-v1` + `protocol_stack_id=ps-joint-deep-ocean-sosus-reconstitution-decoy-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-JOINT-DEEP-OCEAN-SOSUS-001` for the primary course of action and `packet_id=DPL-JOINT-DEEP-OCEAN-SOSUS-002` for degraded-mode fallback actions.
