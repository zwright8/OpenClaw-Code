---
name: joint-deepfake-orders-authentication-and-countersigning-cell
description: Authenticate contested orders, detect synthetic command traffic, and enforce countersign workflows. Use when adversaries may spoof command directives across joint communications paths.
---
...
Primary products for this skill: synthetic-order authenticity brief, countersign branch matrix, command-channel trust ledger.

## Domain Tool Stack

Use these tool categories as the baseline stack: message-integrity validators, voice/video forensic analyzers, signing-key custody services, acknowledgment-chain ledgers.

## Protocol Profile

Preferred protocol families for this skill: USMTF, ACP 127, STIX/TAXII, JOSE/JWS, DISA PKI profiles.

## Domain Toolchain Defaults

- Primary: `tool_suite_id=ts-deepfake-orders-auth-countersign-v1` with `protocol_stack_id=ps-deepfake-orders-auth-countersign-stack-v1`.
- Alternate: `tool_suite_id=ts-physical-message-assurance-v1` with `protocol_stack_id=ps-physical-message-assurance-stack-v1`.
...
- Default packet IDs: DPL-DEEPFAKE-ORDERS-AUTH-COUNTERSIGN-001, DPL-PHYSICAL-MESSAGE-ASSURANCE-001.
