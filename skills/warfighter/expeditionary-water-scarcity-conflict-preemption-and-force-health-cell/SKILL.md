---
name: expeditionary-water-scarcity-conflict-preemption-and-force-health-cell
description: Anticipate water scarcity conflict triggers while preserving force health and civilian stabilization objectives.
---
...
Primary products for this skill: water conflict trigger ledger, force-health mitigation plan, water allocation branch order.

## Domain Tool Stack

Use these tool categories as the baseline stack: hydrology and demand forecasting, preventive medicine surveillance, water-production telemetry, civil-affairs engagement board.

## Protocol Profile

Preferred protocol families for this skill: HL7/FHIR, NIMS/ICS, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-expeditionary-water-scarcity-conflict-preemption-and-force-health-cell-v1` + `protocol_stack_id=ps-expeditionary-water-scarcity-conflict-preemption-and-force-health-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-EXPEDITIONARY_WATER_SCARCITY_CONFLICT_PREEMPTION_AND_FORCE_HEALTH_CELL-001` for the primary course of action and `packet_id=DPL-EXPEDITIONARY_WATER_SCARCITY_CONFLICT_PREEMPTION_AND_FORCE_HEALTH_CELL-002` for degraded-mode fallback actions.
