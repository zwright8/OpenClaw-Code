---
name: tactical-mission-order-watermark-and-authenticity-cell
description: Authenticate tactical mission orders with robust watermarking and anti-spoof controls in degraded and adversarial information environments.
---
...
Primary products for this skill: mission-order authenticity scorecard, watermark verification SOP, spoof-denial branch trigger packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: order dissemination controls, cryptographic authenticity services, watermark forensic analyzers, command acknowledgment ledgers.

## Protocol Profile

Preferred protocol families for this skill: VMF, STIX/TAXII, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-tactical-mission-order-watermark-and-authenticity-cell-v1` + `protocol_stack_id=ps-tactical-mission-order-watermark-and-authenticity-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-TACTICAL_MISSION_ORDER_WATERMARK_AND_AUTHENTICITY_CELL-001` for the primary course of action and `packet_id=DPL-TACTICAL_MISSION_ORDER_WATERMARK_AND_AUTHENTICITY_CELL-002` for degraded-mode fallback actions.
