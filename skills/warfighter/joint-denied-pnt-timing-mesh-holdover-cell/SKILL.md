---
name: joint-denied-pnt-timing-mesh-holdover-cell
description: Maintain mission timing coherence during GNSS denial and spoofing. Use when distributed kill chains require holdover timing and cross-domain synchronization.
---

# Joint Denied PNT Timing Mesh Holdover Cell
...
1. Frame the mission problem using these core inputs: timing source health, holdover device inventory, mesh synchronization state, and mission timing tolerances.
...
Primary products for this skill: timing holdover survivability plan, synchronization drift watchlist, resynchronization branch triggers.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: timing anomaly detectors, anti-spoof validation services, distributed mesh synchronization managers.

## Protocol Profile

Preferred protocol families for this skill: USMTF, API/JSON, Link 16 timing profiles, signed timing-state manifests.

## Domain Toolchain Defaults

- Primary:   `tool_suite_id=ts-denied-pnt-timing-holdover-v1` with `protocol_stack_id=ps-denied-pnt-timing-holdover-stack-v1`.
- Alternate:   `tool_suite_id=ts-anti-jam-gps-epoch-recovery-v1` with `protocol_stack_id=ps-anti-jam-gps-epoch-stack-v1`.
...
- Default packet IDs: DPL-DENIED-PNT-TIMING-HOLDOVER-001, DPL-ANTI-JAM-GPS-EPOCH-RECOVERY-001.
