---
name: coalition-joint-fires-ai-no-strike-boundary-assurance-cell
description: Assure coalition AI-assisted fires recommendations remain inside no-strike and restricted-target boundaries. Use when dynamic targeting is accelerated by automation and requires legal-safe geofence enforcement.
---
...
Primary products for this skill: AI fires boundary assurance board, no-strike compliance exception log, coalition legal escalation matrix.
...
- Add `tool_suite_id=ts-coalition-ai-fires-no-strike-assurance-v1` + `protocol_stack_id=ps-coalition-ai-fires-no-strike-assurance-stack-v1` when producing critical recommendations for this mission domain.
