---
name: coalition-denied-identity-proofing-and-insider-risk-expulsion-cell
description: Coordinate coalition identity proofing in denied environments and execute insider-risk isolation and expulsion workflows with legal controls.
---
...
Primary products for this skill: coalition identity assurance board, insider-risk isolation ladder, legal authority handoff packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: federated identity proofing suites, coalition watchlist and biometrics reconciliation tools, insider-risk forensics workflows.

## Protocol Profile

Preferred protocol families for this skill: NIEM, CJIS, NATO APP-11/ADatP-3 aligned, STIX/TAXII, API/JSON.
...
- Prioritize `tool_suite_id=ts-coalition-denied-identity-insider-expulsion-v1` + `protocol_stack_id=ps-coalition-denied-identity-insider-expulsion-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-COALITION-IDENTITY-INSIDER-001` for the primary course of action and `packet_id=DPL-COALITION-IDENTITY-INSIDER-002` for degraded-mode fallback actions.
