---
name: homeland-port-ransomware-manifest-recovery-and-customs-triage-cell
description: Restore homeland port operations after ransomware by recovering cargo manifests and triaging customs throughput.
---
...
Primary products: manifest recovery confidence map, customs triage queue, port mission-flow restoration packet.
...
- Primary: `tool_suite_id=ts-homeland-port-ransomware-manifest-recovery-customs-triage-v1` with `protocol_stack_id=ps-homeland-port-ransomware-manifest-recovery-customs-triage-stack-v1`.
...
- Default packet IDs: `DPL-HOMELAND-PORT-RANSOMWARE-MANIFEST-RECOVERY-CUSTOMS-TRIAGE-001`.
