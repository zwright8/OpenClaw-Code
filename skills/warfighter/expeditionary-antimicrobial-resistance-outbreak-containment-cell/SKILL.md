---
name: expeditionary-antimicrobial-resistance-outbreak-containment-cell
description: Contain antimicrobial-resistant outbreaks in expeditionary formations while sustaining combat effectiveness and casualty throughput.
---
...
Primary products for this skill: AMR outbreak containment plan, force-health risk ladder, medevac isolation branch packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: force-health surveillance platforms, lab result fusion tools, isolation logistics planners, medevac coordination systems.

## Protocol Profile

Preferred protocol families for this skill: HL7/FHIR, EDXL, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-expeditionary-antimicrobial-resistance-outbreak-containment-cell-v1` + `protocol_stack_id=ps-expeditionary-antimicrobial-resistance-outbreak-containment-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-EXPEDITIONARY_ANTIMICROBIAL_RESISTANCE_OUTBREAK_CONTAINMENT_CELL-001` for the primary course of action and `packet_id=DPL-EXPEDITIONARY_ANTIMICROBIAL_RESISTANCE_OUTBREAK_CONTAINMENT_CELL-002` for degraded-mode fallback actions.
