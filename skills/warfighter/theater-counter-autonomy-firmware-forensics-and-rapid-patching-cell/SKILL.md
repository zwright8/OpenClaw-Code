---
name: theater-counter-autonomy-firmware-forensics-and-rapid-patching-cell
description: Hunt, triage, and patch adversary-influenced firmware compromise across deployed autonomous systems without breaking mission tempo.
---

# Theater Counter-Autonomy Firmware Forensics And Rapid Patching Cell
...
Primary products for this skill: firmware compromise triage board, patch rollout authority matrix, autonomy safe-state transition packet.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: firmware SBOM analyzers, binary forensics sandboxes, autonomous fleet health dashboards.
...
- Primary: `tool_suite_id=ts-counter-autonomy-firmware-v1`.
- Alternate: `tool_suite_id=ts-cyber-defense-v1`.
...
- Prefer `tool_suite_id=ts-counter-autonomy-firmware-v1` for primary decision support and bind to mission packet `DPL-COUNTER-AUTON-FW-001`.
- Use `tool_suite_id=ts-cyber-defense-v1` as required cross-check for divergence or trust-score decay.
- Include `packet_id=DPL-COUNTER-AUTON-FW-001`, `protocol_profile`, `ack_chain_status`, and `trust_score` in all commander-facing outputs.
