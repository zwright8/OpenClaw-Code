...
- Add `tool_suite_id=ts-coalition-maritime-insurance-sanctions-and-convoy-underwriting-cell-v1` + `protocol_stack_id=ps-coalition-maritime-insurance-sanctions-and-convoy-underwriting-cell-stack-v1` when coalition interoperability depends on insurer posture, sanctions compliance, or convoy liability alignment across partners.
- Add `tool_suite_id=ts-coalition-cross-border-battery-recycling-and-critical-mineral-recovery-cell-v1` + `protocol_stack_id=ps-coalition-cross-border-battery-recycling-and-critical-mineral-recovery-cell-stack-v1` when coalition sustainment or industrial interoperability depends on cross-border recovery throughput, quality trust, or mineral release caveats.
- Add `tool_suite_id=ts-joint-gray-zone-commercial-fleet-shadowing-attribution-cell-v1` + `protocol_stack_id=ps-joint-gray-zone-commercial-fleet-shadowing-attribution-cell-stack-v1` when coalition maritime information-sharing depends on shared attribution confidence for coercive fleet shadowing and proxy presence.
- Add `packet_id=DPL-MARITIME-INSURANCE-UNDERWRITE-001`, `packet_id=DPL-BATTERY-RECYCLING-MINERAL-RECOVERY-001`, and `packet_id=DPL-GRAYZONE-FLEET-SHADOWING-001` for branches that materially alter coalition caveat handling, escort release, or shared maritime awareness.

## Domain Toolchain Override (2026-03-14, Expansion Wave LIV Addendum)

- Add `tool_suite_id=ts-coalition-host-nation-water-rights-military-demand-deconfliction-v1` + `protocol_stack_id=ps-coalition-host-nation-water-rights-military-demand-deconfliction-stack-v1` when coalition interoperability depends on host-nation water rights, allocation legitimacy, or emergency rationing transparency.
