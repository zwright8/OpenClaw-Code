---
name: civil-alert-authentication-and-rumor-control-cell
description: Authenticate civil warning traffic and suppress adversary rumor campaigns. Use when commanders must maintain trust in emergency messaging during kinetic and information conflict.
---
...
Primary products for this skill: alert authenticity verification packet, rumor disruption plan, civil warning confidence board.
...
- Prioritize `tool_suite_id=ts-civil-alert-authenticity-v1` + `protocol_stack_id=ps-civil-alert-authenticity-stack-v1` for this mission set.
- Include `packet_id=DPL-CIVIL-ALERT-AUTHENTICITY-001` for high-consequence recommendations and branch decisions.
- Bind recommendations to `toolchain_profile_id=civil-alert-authentication-rumor-control-v1` in joint operations profile selection.
