---
name: strategic-battery-supply-chain-thermal-runaway-preemption-cell
description: Preempt thermal-runaway risk across strategic battery supply chains supporting military mobility, storage, and basing.
---
...
Primary products for this skill: battery thermal-risk prioritization matrix, supply-chain preemption order, force-protection mitigation packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: battery telemetry analytics, supplier provenance ledgers, logistics routing optimizers, safety compliance orchestrators.
...
- Prioritize `tool_suite_id=ts-strategic-battery-supply-chain-thermal-runaway-preemption-cell-v1` + `protocol_stack_id=ps-strategic-battery-supply-chain-thermal-runaway-preemption-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-STRATEGIC_BATTERY_SUPPLY_CHAIN_THERMAL_RUNAWAY_PREEMPTION_CELL-001` for the primary course of action and `packet_id=DPL-STRATEGIC_BATTERY_SUPPLY_CHAIN_THERMAL_RUNAWAY_PREEMPTION_CELL-002` for degraded-mode fallback actions.
