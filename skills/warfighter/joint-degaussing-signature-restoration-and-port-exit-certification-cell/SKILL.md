---
name: joint-degaussing-signature-restoration-and-port-exit-certification-cell
description: Support U.S. warfighter planning for degaussing, magnetic-signature restoration, and port-exit certification when naval units must sortie under mine and signature threat.
---
...
1. Frame the mission problem using these core inputs: ship availability, degaussing system health, signature measurements, port threat, and sortie schedule.
2. Identify assumptions, decision thresholds, and what reporting or indicators would invalidate the current signature-restoration plan.
3. Build primary and alternate options with explicit tradeoffs in sortie speed, survivability, maintenance burden, and magnetic signature risk.
...
Primary products for this skill: signature restoration ladder, degaussing maintenance and test plan, port-exit certification matrix.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: degaussing coil health board, magnetic signature range tracker, port-exit certification queue, maintenance parts priority ledger.
...
- Prefer these protocol families for this skill: AIS/NMEA, OGC, signed maintenance manifests, API/JSON, USMTF.
...
- Primary: `tool_suite_id=ts-joint-degaussing-signature-restoration-port-exit-certification-v1` with `protocol_stack_id=ps-joint-degaussing-signature-restoration-port-exit-certification-stack-v1`.
- Alternate: `tool_suite_id=ts-spectrum-governance-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-DEGAUSSING-PORT-EXIT-001` for branches that materially alter naval sortie timing, mine-risk posture, or port-exit certification confidence.
