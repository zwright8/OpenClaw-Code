---
name: theater-denied-terrain-heavy-lift-autonomy-safety-cell
description: Manage safety and authority controls for autonomous heavy-lift operations in denied terrain under contested comms and ISR.
---
...
Primary products for this skill: heavy-lift autonomy safety case, denied-terrain lift corridor board, authority-gated execution packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: autonomy convoy controllers, terrain route risk analytics, lift load certification tools, mission assurance safety monitors.

## Protocol Profile

Preferred protocol families for this skill: VMF, CoT, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-theater-denied-terrain-heavy-lift-autonomy-safety-cell-v1` + `protocol_stack_id=ps-theater-denied-terrain-heavy-lift-autonomy-safety-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-THEATER_DENIED_TERRAIN_HEAVY_LIFT_AUTONOMY_SAFETY_CELL-001` for the primary course of action and `packet_id=DPL-THEATER_DENIED_TERRAIN_HEAVY_LIFT_AUTONOMY_SAFETY_CELL-002` for degraded-mode fallback actions.
