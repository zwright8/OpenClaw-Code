---
name: homeland-port-rail-hazmat-sabotage-cascade-response-cell
description: Coordinate military support to civil authorities for cascading port-rail hazardous-material sabotage incidents.
---
...
Primary products for this skill: hazmat cascade incident map, port-rail restoration sequence, military support request task board.

## Domain Tool Stack

Use these tool categories as the baseline stack: rail and port incident command systems, hazmat plume modelers, critical-route movement control boards, utility restoration dashboards.

## Protocol Profile

Preferred protocol families for this skill: NIMS/ICS, EDXL-DE/CAP, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-homeland-port-rail-hazmat-sabotage-cascade-response-cell-v1` + `protocol_stack_id=ps-homeland-port-rail-hazmat-sabotage-cascade-response-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-HOMELAND_PORT_RAIL_HAZMAT_SABOTAGE_CASCADE_RESPONSE_CELL-001` for the primary course of action and `packet_id=DPL-HOMELAND_PORT_RAIL_HAZMAT_SABOTAGE_CASCADE_RESPONSE_CELL-002` for degraded-mode fallback actions.
