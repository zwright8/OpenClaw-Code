...
- Add `toolchain_id=TC-AWARDS-347`, `tool_suite_id=ts-joint-awards-decorations-valor-board-record-protection-v1`, and `protocol_stack_id=ps-joint-awards-decorations-valor-board-record-protection-stack-v1` when personnel-record trust depends on award-order evidence, citation integrity, or decoration routing before a board or assignment action.
- Add `toolchain_id=TC-FINALOUT-348`, `tool_suite_id=ts-joint-final-out-processing-cif-ocie-medical-dental-separation-clearance-v1`, and `protocol_stack_id=ps-joint-final-out-processing-cif-ocie-medical-dental-separation-clearance-stack-v1` when DD214 production, release timing, or benefits handoff depends on final-out, accountability, or separation-clearance legitimacy.
- Add `packet_id=DPL-AWARDS-DECORATIONS-BOARD-001` and `packet_id=DPL-FINAL-OUT-CIF-SEPARATION-001` for branches that materially alter record-reconstruction priority, DD214 timing, or transition-benefit trust.

## Domain Toolchain Override (2026-04-11, Expansion Wave LXXXVI Addendum)

- Add `toolchain_id=TC-UIFGOMOR-351`, `tool_suite_id=ts-joint-unfavorable-information-file-gomor-board-record-rebuttal-continuity-v1`, and `protocol_stack_id=ps-joint-unfavorable-information-file-gomor-board-record-rebuttal-continuity-stack-v1` when adverse paperwork, UIF or GOMOR drift, or rebuttal evidence gaps corrupt personnel-record trust.
- Add `toolchain_id=TC-DRBBCMR-353`, `tool_suite_id=ts-joint-discharge-review-board-bcmr-character-of-service-va-eligibility-bridge-v1`, and `protocol_stack_id=ps-joint-discharge-review-board-bcmr-character-of-service-va-eligibility-bridge-stack-v1` when DD214 repair, record reconstruction, or discharge redress determines whether benefits and transition records remain authoritative.
- Add `packet_id=DPL-UIF-GOMOR-BOARD-001` and `packet_id=DPL-DRB-BCMR-CHARACTER-VA-001` for branches that materially alter record-reconstruction priority, DD214 trust, or transition legitimacy.

## Domain Toolchain Override (2026-04-11, Expansion Wave LXXXVII Addendum)

- Add `toolchain_id=TC-IMR-332`, `tool_suite_id=ts-joint-individual-medical-readiness-immunization-pha-dental-continuity-v1`, and `protocol_stack_id=ps-joint-individual-medical-readiness-immunization-pha-dental-continuity-stack-v1` when authoritative service or readiness records are needed to reconcile IMR status, immunization evidence, periodic health assessments, or dental deployability.
- Add `toolchain_id=TC-TAPBRIDGE-334`, `tool_suite_id=ts-joint-transition-assistance-vre-dol-federal-hiring-bridge-v1`, and `protocol_stack_id=ps-joint-transition-assistance-vre-dol-federal-hiring-bridge-stack-v1` when DD214 timing, separation records, or service certification must be trusted to preserve TAP milestones, VR&E continuity, or federal-hiring legitimacy.
- Add `toolchain_id=TC-COOLMAP-337`, `tool_suite_id=ts-joint-cool-usmap-credentialing-pathway-continuity-v1`, and `protocol_stack_id=ps-joint-cool-usmap-credentialing-pathway-continuity-stack-v1` when training evidence, service dates, or personnel-record integrity are needed to preserve COOL vouchers, USMAP apprenticeship hours, or credential-path continuity.
- Add `packet_id=DPL-IMR-PHA-DENTAL-001`, `packet_id=DPL-TAP-VRE-FEDHIRE-001`, and `packet_id=DPL-COOL-USMAP-CREDPATH-001` for branches that materially alter record-reconstruction priority, transition legitimacy, or credential-path trust.

## Domain Toolchain Override (2026-04-11, Expansion Wave LXXXVIII Addendum)

- Add `toolchain_id=TC-RESRET-355`, `tool_suite_id=ts-reserve-component-retirement-points-sanctuary-nonregular-retirement-continuity-v1`, and `protocol_stack_id=ps-reserve-component-retirement-points-sanctuary-nonregular-retirement-continuity-stack-v1` when service dates, orders, or record integrity must be trusted to preserve Reserve retirement credit or sanctuary calculations.
- Add `toolchain_id=TC-GITEB-358`, `tool_suite_id=ts-joint-post-9-11-gi-bill-transferability-dependent-education-yellow-ribbon-continuity-v1`, and `protocol_stack_id=ps-joint-post-9-11-gi-bill-transferability-dependent-education-yellow-ribbon-continuity-stack-v1` when personnel-record integrity or obligation evidence is required to preserve GI Bill transferability and dependent education continuity.
- Add `packet_id=DPL-RESERVE-RETIREMENT-SANCTUARY-001` and `packet_id=DPL-GI-BILL-TEB-YELLOW-RIBBON-001` for branches that materially alter record-reconstruction priority, benefits legitimacy, or family-education trust.
