---
name: joint-arctic-subsea-sensor-reseed-and-deception-cell
description: Coordinate Arctic subsea sensor reseeding, deception seeding, and warning continuity for U.S. and coalition maritime defense.
---
...
Primary products for this skill: subsea sensor reseed plan, deception pattern matrix, Arctic warning continuity packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: undersea surveillance orchestration, acoustic propagation modeling, deception pattern library, maritime warning dashboards.

## Protocol Profile

Preferred protocol families for this skill: USMTF, OGC WMS/WFS/WMTS, AIS/NMEA, API/JSON.
...
- Prioritize `tool_suite_id=ts-joint-arctic-subsea-sensor-reseed-and-deception-cell-v1` + `protocol_stack_id=ps-joint-arctic-subsea-sensor-reseed-and-deception-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-JOINT_ARCTIC_SUBSEA_SENSOR_RESEED_AND_DECEPTION_CELL-001` for the primary course of action and `packet_id=DPL-JOINT_ARCTIC_SUBSEA_SENSOR_RESEED_AND_DECEPTION_CELL-002` for degraded-mode fallback actions.
