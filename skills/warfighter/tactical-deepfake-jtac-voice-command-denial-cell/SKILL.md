---
name: tactical-deepfake-jtac-voice-command-denial-cell
description: Detect and deny deepfake voice command attempts against JTAC and fires coordination channels.
---
...
Primary products for this skill: voice spoof confidence board, command-channel trust posture, immediate authentication drill packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: voice authenticity forensics, tactical radio authentication services, JTAC command logs, fires deconfliction audit board.

## Protocol Profile

Preferred protocol families for this skill: VMF, Link 16 J-series, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-tactical-deepfake-jtac-voice-command-denial-cell-v1` + `protocol_stack_id=ps-tactical-deepfake-jtac-voice-command-denial-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-TACTICAL_DEEPFAKE_JTAC_VOICE_COMMAND_DENIAL_CELL-001` for the primary course of action and `packet_id=DPL-TACTICAL_DEEPFAKE_JTAC_VOICE_COMMAND_DENIAL_CELL-002` for degraded-mode fallback actions.
