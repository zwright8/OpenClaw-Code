---
name: theater-large-language-model-operations-security-and-prompt-injection-defense-cell
description: Harden theater mission AI and large-language-model workflows against prompt injection, data exfiltration, and adversarial manipulation.
---
...
Primary products for this skill: LLM mission-security posture dashboard, prompt-injection containment branches, model operations release authority packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: model gateway policy engines, prompt-injection detectors, retrieval provenance auditors, runtime policy enforcement tools.
...
- Prioritize `tool_suite_id=ts-theater-llm-opsec-prompt-injection-defense-v1` + `protocol_stack_id=ps-theater-llm-opsec-prompt-injection-defense-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-THEATER-LLM-OPSEC-INJECTION-001` for the primary course of action and `packet_id=DPL-THEATER-LLM-OPSEC-INJECTION-002` for degraded-mode fallback actions.
