...
description: Support arctic denied-communications planning for polar-orbit handovers. Use when high-latitude mission traffic must transition across orbital assets with minimal timing loss and command latency.
---
...
- Primary: `tool_suite_id=ts-polar-orbit-handover-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Alternate: `tool_suite_id=ts-lunar-cislunar-relay-v1` with `protocol_stack_id=ps-joint-tactical-link-stack-v1`.
...
- Preferred protocol families: CCSDS, USMTF, API/JSON.
