---
name: homeland-defense-telecom-911-priority-restoration-cell
description: Coordinate military support to restore telecom and 911 priority services during homeland attacks or cascading infrastructure failure.
---
...
Primary products for this skill: priority-service restoration sequence, DSCA telecom support matrix, emergency comms assurance packet.

## Domain Tool Stack

Use these tool categories as the baseline stack: telecom outage analytics, emergency dispatch status dashboards, priority service restoration orchestration, infrastructure dependency mapping.

## Protocol Profile

Preferred protocol families for this skill: NIMS/ICS, CAP/EDXL, USMTF, API/JSON.
...
- Prioritize `tool_suite_id=ts-homeland-defense-telecom-911-priority-restoration-cell-v1` + `protocol_stack_id=ps-homeland-defense-telecom-911-priority-restoration-cell-stack-v1` when this mission set is the critical path for commander decision timelines.
- Add `packet_id=DPL-HOMELAND_DEFENSE_TELECOM_911_PRIORITY_RESTORATION_CELL-001` for the primary course of action and `packet_id=DPL-HOMELAND_DEFENSE_TELECOM_911_PRIORITY_RESTORATION_CELL-002` for degraded-mode fallback actions.
