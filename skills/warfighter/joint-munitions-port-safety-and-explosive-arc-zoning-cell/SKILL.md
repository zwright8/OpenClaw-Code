---
name: joint-munitions-port-safety-and-explosive-arc-zoning-cell
description: Plan munitions port operations with blast arc zoning, explosive safety compatibility, and throughput protection. Use when ports handle mixed munition classes under contested loading timelines.
---
...
1. Frame the mission problem using these core inputs: munition compatibility groups, port layout and blast arcs, vessel loading schedules, safety waivers, and force-protection posture.
...
Primary products for this skill: explosive safety zoning map, munitions throughput risk matrix, pier loading control order.
...
- Primary: `tool_suite_id=ts-munitions-port-safety-v1` with `protocol_stack_id=ps-harbor-safety-logistics-stack-v1`.
