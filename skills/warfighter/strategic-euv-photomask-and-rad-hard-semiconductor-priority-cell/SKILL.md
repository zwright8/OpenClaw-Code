---
name: strategic-euv-photomask-and-rad-hard-semiconductor-priority-cell
description: Support U.S. warfighter planning for EUV photomask, specialty gas, and rad-hard semiconductor prioritization when missile, space, or NC3 programs depend on scarce fabrication steps and secure lot release.
---
...
- Start by confirming command authority, industrial-security constraints, time horizon, and required decision points.
- Keep products unclassified by default unless the user provides explicit handling guidance and controlled data.

## Workflow

1. Frame the mission problem using these core inputs: fab bottlenecks, photomask availability, gas purity, rad-hard yield, and program demand.
2. Identify assumptions, decision thresholds, and what indicators would invalidate the current release or allocation plan.
3. Build primary and alternate options with explicit tradeoffs in throughput, pedigree confidence, deterrence impact, and industrial fragility.
4. Integrate dependencies across industrial mobilization, supply assurance, strategic programs, cyber protection, and transportation.
...
Primary products for this skill: fab bottleneck ladder, rad-hard lot priority board, photomask and gas constraint matrix.

## Domain Tool Stack

Use these tool categories as the default stack for this skill: photomask library queue, EUV gas purity ledger, rad-hard yield tracker, secure lot release board.
...
- Prefer these protocol families for this skill: signed lot manifests, `USMTF`, `API/JSON`, `OPC UA`, and `NIEM`.
...
- Identify legal, policy, industrial-security, cyber, and coalition interoperability constraints early.
...
- Primary: `tool_suite_id=ts-strategic-euv-photomask-rad-hard-semiconductor-priority-v1` with `protocol_stack_id=ps-strategic-euv-photomask-rad-hard-semiconductor-priority-stack-v1`.
- Alternate: `tool_suite_id=ts-strategic-supply-shock-v1` with `protocol_stack_id=ps-cop-event-sharing-stack-v1`.
- Add `packet_id=DPL-EUV-PHOTOMASK-RADHARD-001` for branches that materially alter fab allocation, lot release, or strategic modernization posture.
