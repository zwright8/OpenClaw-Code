---
name: strategic-military-gps-m-code-keyfill-denial-recovery-cell
description: Orchestrate recovery from M-code keyfill denial, cryptographic distribution disruption, and timing trust degradation across joint force posture. Use when assured PNT trust is at strategic risk.
---

# Strategic Military GPS M-Code Keyfill Denial Recovery Cell
...
1. Frame the mission problem using these core inputs: keyfill distribution status, cryptographic trust indicators, platform time-sync drift, and mission dependency tiers.
...
Primary products for this skill: keyfill recovery branch map, timing trust restoration ladder, mission-priority PNT fallback matrix.

## External Tools and Protocol Integration

- Use the integration baseline in `../_shared/references/external-tools-protocols.md`.
- Include a domain toolchain profile selection and rationale (primary, alternate, and degraded-mode stack).
- Prioritize these tools or tool families for this domain: key management infrastructure, secure PNT monitoring tools, cryptographic audit and distribution systems.
...
## Domain Toolchain Override (2026-03-09, M-Code Keyfill Denial Recovery)

- Prioritize tool_suite_id=ts-gps-mcode-keyfill-denial-recovery-v1 with protocol_stack_id=ps-gps-mcode-keyfill-denial-recovery-stack-v1 when keyfill latency or trust-chain failures threaten assured PNT posture.
- Use packet DPL-GPS-MCODE-KEYFILL-DENIAL-RECOVERY-001 to bind key redistribution sequencing, fallback timing controls, and commander decision checkpoints.
- If cryptographic acknowledgment chain or key provenance validation is incomplete, downgrade to advisory-only timing posture changes and require command approval before mission-impacting key actions.
