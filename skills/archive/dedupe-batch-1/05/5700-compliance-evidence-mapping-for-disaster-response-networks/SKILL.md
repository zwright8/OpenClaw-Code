...
## When To Use
Use this skill only when the request explicitly needs `Compliance evidence mapping for disaster response networks` in disaster response networks and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Compliance evidence mapping for disaster response networks** outcomes (not generic brainstorming).
...
1. **Incident stabilization in disaster response networks" capability in production for disaster response networks workflows**
   - Input: noisy upstream payload requiring compliance evidence mapping for disaster response networks normalization/assessment.
