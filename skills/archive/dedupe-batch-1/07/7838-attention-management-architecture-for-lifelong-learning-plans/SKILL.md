...
This skill hardens a generated capability for production execution so lifelong learning plans workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `Attention Management Architecture for lifelong learning plans` in lifelong learning plans and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Attention Management Architecture for lifelong learning plans** outcomes (not generic brainstorming).
...
1. **Incident stabilization in lifelong learning plans" capability for lifelong learning plans**
   - Input: noisy upstream payload requiring attention management architecture for lifelong learning plans normalization/assessment.
