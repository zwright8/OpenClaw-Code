---
name: figma-migration-assistant
description: Use when tasks require figma migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Figma Migration Assistant
...
| Primary artifact | `figma-migration-cutover-plan` |
| Routing tag | `figma:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when state changes are hard to undo once they begin. This specific skill turns Figma Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from figma..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| figma-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| figma-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Figma Migration Assistant before any Figma rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from figma. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `figma-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] figma-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:migration-assistant` consumers with approval and credential context attached.
