...
description: Use when tasks require youtube migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# YouTube Migration Assistant
...
| Primary artifact | `youtube-migration-cutover-plan` |
| Routing tag | `youtube:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when state changes are hard to undo once they begin. This specific skill turns YouTube Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from youtube..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| youtube-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| youtube-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for YouTube Migration Assistant before any YouTube rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from youtube. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `youtube-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] youtube-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:migration-assistant` consumers with approval and credential context attached.
