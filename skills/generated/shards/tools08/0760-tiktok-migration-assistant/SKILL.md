---
name: tiktok-migration-assistant
description: Use when tasks require tiktok migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Migration Assistant
...
| Primary artifact | `tiktok-migration-cutover-plan` |
| Routing tag | `tiktok:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when state changes are hard to undo once they begin. This specific skill turns TikTok Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from tiktok..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| tiktok-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| tiktok-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for TikTok Migration Assistant before any TikTok rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from tiktok. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `tiktok-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:migration-assistant` consumers with approval and credential context attached.
