---
name: youtube-sync-orchestrator
description: Use when tasks require youtube sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `youtube-reconciliation-bundle` |
| Routing tag | `youtube:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for youtube records..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| youtube-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| youtube-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for YouTube Sync Orchestrator, then checkpoint the last known good cursor on both sides of YouTube.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for youtube records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `youtube-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] youtube-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:sync-orchestrator` consumers with approval and credential context attached.
