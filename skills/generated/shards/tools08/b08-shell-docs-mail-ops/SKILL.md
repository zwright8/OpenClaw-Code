---
name: b08-shell-docs-mail-ops
description: Use when orchestrating shell-centric workflows that combine document conversion, notifications, source-control ergonomics, package checks, and mail/server administration.
---

# Shell, docs, notifications, and mail operations

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-shell-docs-mail-ops` |
| Tool count | `26` |
| Inventory rank span | `3502-3544` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for shell, docs, notifications, mail operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `runit` | `plakar` | no | no |
| bounded mutation / transformation | `plakar` | `runit` | yes | yes |
| validation & artifact checks | `runit` | `plakar` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `runit` | Collection of tools for managing UNIX services | local-installable |
| `plakar` | Create backups with compression, encryption and deduplication | local-installable |
| `ydiff` | View colored diff with side by side and auto pager support | local-installable |
| `pdf2image` | Convert PDFs to images | local-installable |
| `gitup` | Update multiple git repositories at once | local-installable |
| `apprise` | Send notifications from the command-line to popular notification services | local-installable |
| `cargo-llvm-cov` | Cargo subcommand to easily use LLVM source-based code coverage | local-installable |
| `mad` | MPEG audio decoder | local-installable |
| `ford` | Automatic documentation generator for modern Fortran programs | local-installable |
| `clisp` | GNU CLISP, a Common Lisp implementation | local-installable |
| `mob` | Tool for smooth Git handover in mob programming sessions | local-installable |
| `walk` | Terminal navigator | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `runit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `plakar` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ydiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdf2image` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gitup` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apprise` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-llvm-cov` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mad` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ford` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clisp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mob` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `walk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pugixml` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `click` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `mcp-publisher` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dovecot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tccutil` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-outdated` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cariddi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rpds-py` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lcdf-typetools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xeyes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `arxiv_latex_cleaner` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nixfmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `easyrpg-player` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `click`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `runit` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `plakar` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `ydiff` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `pdf2image` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `gitup` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `apprise` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a shell, docs, notifications, mail operations request, run dry-run first, then produce a validated artifact bundle.
