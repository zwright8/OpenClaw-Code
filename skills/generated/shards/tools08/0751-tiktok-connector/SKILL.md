...
description: Use when tasks require tiktok connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `tiktok-connection-trace-bundle` |
| Routing tag | `tiktok:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Connector into a deterministic, auth-checked workflow for unified read/write connector for tiktok apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| tiktok-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| tiktok-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for TikTok Connector, then confirm the smallest read-only probe that proves access to TikTok.
...
3. Specify the request and response contract for unified read/write connector for tiktok apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `tiktok-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:connector` consumers with approval and credential context attached.
