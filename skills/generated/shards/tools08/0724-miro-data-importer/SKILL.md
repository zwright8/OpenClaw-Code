---
name: miro-data-importer
description: Use when tasks require miro data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `miro-import-ledger` |
| Routing tag | `miro:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when state changes are hard to undo once they begin. This specific skill turns Miro Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into miro..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-import-ledger | structured-artifact | yes | next workflow or operator |
| miro-import-ledger-scorecard | scorecard | yes | reviewer |
| miro-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Miro Data Importer before any Miro write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into miro. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `miro-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] miro-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:data-importer` consumers with approval and credential context attached.
