---
name: canva-migration-assistant
description: Use when tasks require canva migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Canva Migration Assistant
...
| Primary artifact | `canva-migration-cutover-plan` |
| Routing tag | `canva:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when state changes are hard to undo once they begin. This specific skill turns Canva Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from canva..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| canva-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| canva-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Canva Migration Assistant before any Canva rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from canva. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `canva-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] canva-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:migration-assistant` consumers with approval and credential context attached.
