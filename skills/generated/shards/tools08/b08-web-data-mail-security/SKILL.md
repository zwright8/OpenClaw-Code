...
| Field | Value |
|---|---|
| Skill pack | `b08-web-data-mail-security` |
| Tool count | `26` |
| Inventory rank span | `3717-3748` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for web data platforms, mail auth, security hygiene tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `mariadb-connector-odbc` | `memtester` | no | no |
| bounded mutation / transformation | `memtester` | `mariadb-connector-odbc` | yes | yes |
| validation & artifact checks | `mariadb-connector-odbc` | `memtester` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `mariadb-connector-odbc` | Database driver using the industry standard ODBC API | local-installable |
| `memtester` | Utility for testing the memory subsystem | local-installable |
| `gatsby-cli` | Gatsby command-line interface | local-installable |
| `ccat` | Like cat but displays content with syntax highlighting | local-installable |
| `mapproxy` | Accelerating web map proxy | local-installable |
| `mp4v2` | Read, create, and modify MP4 files | local-installable |
| `ghalint` | GitHub Actions linter | local-installable |
| `gnu-apl` | GNU implementation of the programming language APL | local-installable |
| `parsedmarc` | DMARC report analyzer and visualizer | local-installable |
| `orc` | Oil Runtime Compiler (ORC) | local-installable |
| `hmmer` | Build profile HMMs and scan against sequence databases | local-installable |
| `dufs` | Static file server | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `mariadb-connector-odbc` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `memtester` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gatsby-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ccat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mapproxy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mp4v2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ghalint` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gnu-apl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `parsedmarc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `orc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hmmer` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dufs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `webkitgtk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scdl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mariadb@11.8` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-gdbm@3.12` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pianobar` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `klee` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `findomain` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcp-atlassian` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `surfer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `macpine` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shadowenv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zopfli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rkhunter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `ghalint`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `mariadb-connector-odbc` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `memtester` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `gatsby-cli` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `ccat` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `mapproxy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `mp4v2` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a web data platforms, mail auth, security hygiene request, run dry-run first, then produce a validated artifact bundle.
