---
name: linkedin-workflow-automator
description: Use when tasks require linkedin workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Workflow Automator
...
| Primary artifact | `linkedin-workflow-runbook` |
| Routing tag | `linkedin:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using linkedin..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-workflow-runbook | structured-artifact | yes | next workflow or operator |
| linkedin-workflow-runbook-scorecard | scorecard | yes | reviewer |
| linkedin-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for LinkedIn Workflow Automator, then checkpoint where LinkedIn state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using linkedin. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `linkedin-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:workflow-automator` consumers with approval and credential context attached.
