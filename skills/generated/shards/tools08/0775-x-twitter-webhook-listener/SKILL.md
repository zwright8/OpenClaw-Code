---
name: x-twitter-webhook-listener
description: Use when tasks require x twitter webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Webhook Listener
...
| Primary artifact | `x-twitter-event-intake-log` |
| Routing tag | `x-twitter:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns X Twitter Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes x twitter webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `X_TWITTER_WEBHOOK_SECRET`, `X_TWITTER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| x-twitter-event-intake-log | structured-artifact | yes | next workflow or operator |
| x-twitter-event-intake-log-scorecard | scorecard | yes | reviewer |
| x-twitter-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for X Twitter Webhook Listener, then register the signing-secret policy for X Twitter.
...
3. Implement captures, verifies, and routes x twitter webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `x-twitter-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:webhook-listener` consumers with approval and credential context attached.
