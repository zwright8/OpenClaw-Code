---
name: miro-webhook-listener
description: Use when tasks require miro webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Miro Webhook Listener
...
| Primary artifact | `miro-event-intake-log` |
| Routing tag | `miro:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Miro Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes miro webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `MIRO_CLIENT_ID`, `MIRO_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `MIRO_TOKEN`, `MIRO_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `MIRO_WEBHOOK_SECRET`, `MIRO_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| miro-event-intake-log | structured-artifact | yes | next workflow or operator |
| miro-event-intake-log-scorecard | scorecard | yes | reviewer |
| miro-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Miro Webhook Listener, then register the signing-secret policy for Miro.
...
3. Implement captures, verifies, and routes miro webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `miro-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] miro-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:webhook-listener` consumers with approval and credential context attached.
