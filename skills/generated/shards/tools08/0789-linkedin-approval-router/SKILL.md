---
name: linkedin-approval-router
description: Use when tasks require linkedin approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Approval Router
...
| Primary artifact | `linkedin-approval-decision-packet` |
| Routing tag | `linkedin:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Approval Router into a deterministic, auth-checked workflow for routes linkedin-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| linkedin-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| linkedin-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for LinkedIn Approval Router, then capture the manual fallback path for LinkedIn.
...
3. Implement routes linkedin-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `linkedin-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:approval-router` consumers with approval and credential context attached.
