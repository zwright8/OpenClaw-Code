---
name: figma-report-builder
description: Use when tasks require figma report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-report-bundle` |
| Routing tag | `figma:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from figma activity..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-report-bundle | structured-artifact | yes | next workflow or operator |
| figma-report-bundle-scorecard | scorecard | yes | reviewer |
| figma-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Figma Report Builder, then confirm which Figma entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from figma activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `figma-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] figma-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:report-builder` consumers with approval and credential context attached.
