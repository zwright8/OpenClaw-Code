---
name: loom-data-importer
description: Use when tasks require loom data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `loom-import-ledger` |
| Routing tag | `loom:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when state changes are hard to undo once they begin. This specific skill turns Loom Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into loom..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-import-ledger | structured-artifact | yes | next workflow or operator |
| loom-import-ledger-scorecard | scorecard | yes | reviewer |
| loom-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Loom Data Importer before any Loom write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into loom. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `loom-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] loom-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:data-importer` consumers with approval and credential context attached.
