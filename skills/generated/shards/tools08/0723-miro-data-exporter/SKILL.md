---
name: miro-data-exporter
description: Use when tasks require miro data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `miro-export-package` |
| Routing tag | `miro:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Data Exporter into a deterministic, auth-checked workflow for exports miro entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-export-package | structured-artifact | yes | next workflow or operator |
| miro-export-package-scorecard | scorecard | yes | reviewer |
| miro-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Miro Data Exporter, then constrain the export scope for Miro before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports miro entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `miro-export-package` so downstream owners do not need to rediscover context.
...
- [ ] miro-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:data-exporter` consumers with approval and credential context attached.
