---
name: canva-webhook-listener
description: Use when tasks require canva webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Canva Webhook Listener
...
| Primary artifact | `canva-event-intake-log` |
| Routing tag | `canva:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Canva Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes canva webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `CANVA_CLIENT_ID`, `CANVA_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `CANVA_TOKEN`, `CANVA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CANVA_WEBHOOK_SECRET`, `CANVA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| canva-event-intake-log | structured-artifact | yes | next workflow or operator |
| canva-event-intake-log-scorecard | scorecard | yes | reviewer |
| canva-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Canva Webhook Listener, then register the signing-secret policy for Canva.
...
3. Implement captures, verifies, and routes canva webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `canva-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] canva-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:webhook-listener` consumers with approval and credential context attached.
