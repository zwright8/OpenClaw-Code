...
description: Use when tasks require instagram connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-connection-trace-bundle` |
| Routing tag | `instagram:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Connector into a deterministic, auth-checked workflow for unified read/write connector for instagram apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| instagram-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| instagram-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Instagram Connector, then confirm the smallest read-only probe that proves access to Instagram.
...
3. Specify the request and response contract for unified read/write connector for instagram apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `instagram-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] instagram-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:connector` consumers with approval and credential context attached.
