...
| Field | Value |
|---|---|
| Skill pack | `b08-analytics-security-genomics` |
| Tool count | `26` |
| Inventory rank span | `3880-3911` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for analytics platforms, security posture, genomics pipelines tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `druid` | `synergy-core` | no | no |
| bounded mutation / transformation | `synergy-core` | `druid` | yes | yes |
| validation & artifact checks | `druid` | `synergy-core` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `druid` | High-performance, column-oriented, distributed data store | local-installable |
| `synergy-core` | Synergy, the keyboard and mouse sharing tool | local-installable |
| `spacer` | Small command-line utility for adding spacers to command output | local-installable |
| `vunnel` | Tool for collecting vulnerability data from various sources | local-installable |
| `mimalloc` | Compact general purpose allocator | local-installable |
| `tarantool` | In-memory database and Lua application server | local-installable |
| `scoutsuite` | Open source multi-cloud security-auditing tool | local-installable |
| `argc` | Easily create and use cli based on bash script | local-installable |
| `httpstat` | Curl statistics made simple | local-installable |
| `vcftools` | Tools for working with VCF files | local-installable |
| `proctools` | OpenBSD and Darwin versions of pgrep, pkill, and pfind | local-installable |
| `wildfly-as` | Managed application runtime for building applications | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `druid` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `synergy-core` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spacer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vunnel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mimalloc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tarantool` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scoutsuite` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `argc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `httpstat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vcftools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `proctools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wildfly-as` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openkim-models` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mdfried` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `genometools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sqlpage` | QUIC, SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cc65` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `datamash` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `llvm@12` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libre` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `tintin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `evans` | gRPC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bosh-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `playwright-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mongo-cxx-driver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mlpack` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
...
1. `druid` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `synergy-core` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `spacer` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `vunnel` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `mimalloc` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `tarantool` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a analytics platforms, security posture, genomics pipelines request, run dry-run first, then produce a validated artifact bundle.
