...
description: Use when tasks require canva connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `canva-connection-trace-bundle` |
| Routing tag | `canva:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Connector into a deterministic, auth-checked workflow for unified read/write connector for canva apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| canva-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| canva-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Canva Connector, then confirm the smallest read-only probe that proves access to Canva.
...
3. Specify the request and response contract for unified read/write connector for canva apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `canva-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] canva-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:connector` consumers with approval and credential context attached.
