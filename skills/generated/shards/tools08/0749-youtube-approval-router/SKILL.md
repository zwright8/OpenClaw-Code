---
name: youtube-approval-router
description: Use when tasks require youtube approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# YouTube Approval Router
...
| Primary artifact | `youtube-approval-decision-packet` |
| Routing tag | `youtube:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Approval Router into a deterministic, auth-checked workflow for routes youtube-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| youtube-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| youtube-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for YouTube Approval Router, then capture the manual fallback path for YouTube.
...
3. Implement routes youtube-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `youtube-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] youtube-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:approval-router` consumers with approval and credential context attached.
