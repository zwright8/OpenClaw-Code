---
name: x-twitter-data-exporter
description: Use when tasks require x twitter data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Data Exporter
...
| Primary artifact | `x-twitter-export-package` |
| Routing tag | `x-twitter:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns X Twitter Data Exporter into a deterministic, auth-checked workflow for exports x twitter entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-export-package | structured-artifact | yes | next workflow or operator |
| x-twitter-export-package-scorecard | scorecard | yes | reviewer |
| x-twitter-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for X Twitter Data Exporter, then constrain the export scope for X Twitter before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports x twitter entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `x-twitter-export-package` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:data-exporter` consumers with approval and credential context attached.
