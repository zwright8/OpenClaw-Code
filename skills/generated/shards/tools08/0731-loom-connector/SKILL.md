---
name: loom-connector
description: Use when tasks require loom connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `loom-connection-trace-bundle` |
| Routing tag | `loom:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Connector into a deterministic, auth-checked workflow for unified read/write connector for loom apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| loom-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| loom-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Loom Connector, then confirm the smallest read-only probe that proves access to Loom.
...
3. Specify the request and response contract for unified read/write connector for loom apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `loom-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] loom-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:connector` consumers with approval and credential context attached.
