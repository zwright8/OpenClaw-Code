---
name: linkedin-webhook-listener
description: Use when tasks require linkedin webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Webhook Listener
...
| Primary artifact | `linkedin-event-intake-log` |
| Routing tag | `linkedin:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns LinkedIn Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes linkedin webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `LINKEDIN_CLIENT_ID`, `LINKEDIN_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `LINKEDIN_TOKEN`, `LINKEDIN_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `LINKEDIN_WEBHOOK_SECRET`, `LINKEDIN_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| linkedin-event-intake-log | structured-artifact | yes | next workflow or operator |
| linkedin-event-intake-log-scorecard | scorecard | yes | reviewer |
| linkedin-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for LinkedIn Webhook Listener, then register the signing-secret policy for LinkedIn.
...
3. Implement captures, verifies, and routes linkedin webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `linkedin-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:webhook-listener` consumers with approval and credential context attached.
