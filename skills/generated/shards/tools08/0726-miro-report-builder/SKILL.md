---
name: miro-report-builder
description: Use when tasks require miro report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `miro-report-bundle` |
| Routing tag | `miro:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from miro activity..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-report-bundle | structured-artifact | yes | next workflow or operator |
| miro-report-bundle-scorecard | scorecard | yes | reviewer |
| miro-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Miro Report Builder, then confirm which Miro entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from miro activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `miro-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] miro-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:report-builder` consumers with approval and credential context attached.
