...
description: Use when tasks require instagram webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-event-intake-log` |
| Routing tag | `instagram:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Instagram Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes instagram webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `INSTAGRAM_CLIENT_ID`, `INSTAGRAM_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `INSTAGRAM_TOKEN`, `INSTAGRAM_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `INSTAGRAM_WEBHOOK_SECRET`, `INSTAGRAM_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| instagram-event-intake-log | structured-artifact | yes | next workflow or operator |
| instagram-event-intake-log-scorecard | scorecard | yes | reviewer |
| instagram-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Instagram Webhook Listener, then register the signing-secret policy for Instagram.
...
3. Implement captures, verifies, and routes instagram webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `instagram-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] instagram-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:webhook-listener` consumers with approval and credential context attached.
