---
name: x-twitter-workflow-automator
description: Use when tasks require x twitter workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Workflow Automator
...
| Primary artifact | `x-twitter-workflow-runbook` |
| Routing tag | `x-twitter:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns X Twitter Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using x twitter..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-workflow-runbook | structured-artifact | yes | next workflow or operator |
| x-twitter-workflow-runbook-scorecard | scorecard | yes | reviewer |
| x-twitter-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for X Twitter Workflow Automator, then checkpoint where X Twitter state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using x twitter. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `x-twitter-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:workflow-automator` consumers with approval and credential context attached.
