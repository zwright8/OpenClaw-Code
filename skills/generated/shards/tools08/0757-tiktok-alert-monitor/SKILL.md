---
name: tiktok-alert-monitor
description: Use when tasks require tiktok alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Alert Monitor
...
| Primary artifact | `tiktok-alert-state-bundle` |
| Routing tag | `tiktok:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Alert Monitor into a deterministic, auth-checked workflow for monitors key tiktok signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| tiktok-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| tiktok-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for TikTok Alert Monitor, then lock the reviewed threshold set for TikTok.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key tiktok signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `tiktok-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:alert-monitor` consumers with approval and credential context attached.
