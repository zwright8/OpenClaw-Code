...
description: Use when tasks require linkedin connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `linkedin-connection-trace-bundle` |
| Routing tag | `linkedin:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Connector into a deterministic, auth-checked workflow for unified read/write connector for linkedin apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| linkedin-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| linkedin-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for LinkedIn Connector, then confirm the smallest read-only probe that proves access to LinkedIn.
...
3. Specify the request and response contract for unified read/write connector for linkedin apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `linkedin-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:connector` consumers with approval and credential context attached.
