---
name: loom-approval-router
description: Use when tasks require loom approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Loom Approval Router
...
| Primary artifact | `loom-approval-decision-packet` |
| Routing tag | `loom:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Approval Router into a deterministic, auth-checked workflow for routes loom-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| loom-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| loom-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Loom Approval Router, then capture the manual fallback path for Loom.
...
3. Implement routes loom-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `loom-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] loom-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:approval-router` consumers with approval and credential context attached.
