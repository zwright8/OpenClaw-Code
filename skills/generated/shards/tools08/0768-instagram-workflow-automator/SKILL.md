---
name: instagram-workflow-automator
description: Use when tasks require instagram workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Instagram Workflow Automator
...
| Primary artifact | `instagram-workflow-runbook` |
| Routing tag | `instagram:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using instagram..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-workflow-runbook | structured-artifact | yes | next workflow or operator |
| instagram-workflow-runbook-scorecard | scorecard | yes | reviewer |
| instagram-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Instagram Workflow Automator, then checkpoint where Instagram state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using instagram. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `instagram-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] instagram-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:workflow-automator` consumers with approval and credential context attached.
