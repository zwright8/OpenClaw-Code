---
name: linkedin-report-builder
description: Use when tasks require linkedin report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Report Builder
...
| Primary artifact | `linkedin-report-bundle` |
| Routing tag | `linkedin:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from linkedin activity..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-report-bundle | structured-artifact | yes | next workflow or operator |
| linkedin-report-bundle-scorecard | scorecard | yes | reviewer |
| linkedin-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for LinkedIn Report Builder, then confirm which LinkedIn entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from linkedin activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `linkedin-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:report-builder` consumers with approval and credential context attached.
