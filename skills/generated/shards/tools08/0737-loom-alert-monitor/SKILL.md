---
name: loom-alert-monitor
description: Use when tasks require loom alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `loom-alert-state-bundle` |
| Routing tag | `loom:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Alert Monitor into a deterministic, auth-checked workflow for monitors key loom signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| loom-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| loom-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Loom Alert Monitor, then lock the reviewed threshold set for Loom.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key loom signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `loom-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] loom-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:alert-monitor` consumers with approval and credential context attached.
