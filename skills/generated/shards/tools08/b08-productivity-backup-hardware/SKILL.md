...
| Field | Value |
|---|---|
| Skill pack | `b08-productivity-backup-hardware` |
| Tool count | `26` |
| Inventory rank span | `3814-3844` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for productivity orchestration, backups, hardware-aware tooling tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `kargo` | `mcat` | no | no |
| bounded mutation / transformation | `mcat` | `kargo` | yes | yes |
| validation & artifact checks | `kargo` | `mcat` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `kargo` | Multi-Stage GitOps Continuous Promotion | local-installable |
| `mcat` | Terminal image, video, directory, and Markdown viewer | local-installable |
| `threadweaver` | Helper for multithreaded programming | local-installable |
| `organize-tool` | File management automation tool | local-installable |
| `calcurse` | Text-based personal organizer | local-installable |
| `noti` | Trigger notifications when a process completes | local-installable |
| `pcb2gcode` | Command-line tool for isolation, routing and drilling of PCBs | local-installable |
| `mcp-grafana` | MCP server for Grafana | local-installable |
| `gifify` | Turn movies into GIFs | local-installable |
| `vbindiff` | Visual Binary Diff | local-installable |
| `po4a` | Documentation translation maintenance tool | local-installable |
| `apache-opennlp` | Machine learning toolkit for processing natural language text | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `kargo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `threadweaver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `organize-tool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `calcurse` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `noti` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pcb2gcode` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcp-grafana` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gifify` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vbindiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `po4a` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apache-opennlp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `codebook-lsp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `youplot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `argp-standalone` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `todoist-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `watson` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `brpc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `percona-xtrabackup@8.0` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `htmldoc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `arrayfire` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pixz` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lm-sensors` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `crystalline` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `git-interactive-rebase-tool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pianod` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `kargo` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `mcat` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `threadweaver` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `organize-tool` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `calcurse` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `noti` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a productivity orchestration, backups, hardware-aware tooling request, run dry-run first, then produce a validated artifact bundle.
