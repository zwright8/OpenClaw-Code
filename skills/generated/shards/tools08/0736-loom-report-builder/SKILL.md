---
name: loom-report-builder
description: Use when tasks require loom report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `loom-report-bundle` |
| Routing tag | `loom:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from loom activity..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-report-bundle | structured-artifact | yes | next workflow or operator |
| loom-report-bundle-scorecard | scorecard | yes | reviewer |
| loom-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Loom Report Builder, then confirm which Loom entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from loom activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `loom-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] loom-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:report-builder` consumers with approval and credential context attached.
