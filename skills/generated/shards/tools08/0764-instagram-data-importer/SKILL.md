---
name: instagram-data-importer
description: Use when tasks require instagram data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-import-ledger` |
| Routing tag | `instagram:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when state changes are hard to undo once they begin. This specific skill turns Instagram Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into instagram..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-import-ledger | structured-artifact | yes | next workflow or operator |
| instagram-import-ledger-scorecard | scorecard | yes | reviewer |
| instagram-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Instagram Data Importer before any Instagram write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into instagram. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `instagram-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] instagram-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:data-importer` consumers with approval and credential context attached.
