---
name: miro-migration-assistant
description: Use when tasks require miro migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Miro Migration Assistant
...
| Primary artifact | `miro-migration-cutover-plan` |
| Routing tag | `miro:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when state changes are hard to undo once they begin. This specific skill turns Miro Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from miro..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| miro-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| miro-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Miro Migration Assistant before any Miro rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from miro. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `miro-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] miro-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:migration-assistant` consumers with approval and credential context attached.
