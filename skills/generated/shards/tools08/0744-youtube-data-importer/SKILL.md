---
name: youtube-data-importer
description: Use when tasks require youtube data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# YouTube Data Importer
...
| Primary artifact | `youtube-import-ledger` |
| Routing tag | `youtube:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when state changes are hard to undo once they begin. This specific skill turns YouTube Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into youtube..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-import-ledger | structured-artifact | yes | next workflow or operator |
| youtube-import-ledger-scorecard | scorecard | yes | reviewer |
| youtube-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for YouTube Data Importer before any YouTube write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into youtube. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `youtube-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] youtube-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:data-importer` consumers with approval and credential context attached.
