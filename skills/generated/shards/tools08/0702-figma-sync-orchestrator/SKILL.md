---
name: figma-sync-orchestrator
description: Use when tasks require figma sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-reconciliation-bundle` |
| Routing tag | `figma:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for figma records..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| figma-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| figma-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Figma Sync Orchestrator, then checkpoint the last known good cursor on both sides of Figma.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for figma records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `figma-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] figma-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:sync-orchestrator` consumers with approval and credential context attached.
