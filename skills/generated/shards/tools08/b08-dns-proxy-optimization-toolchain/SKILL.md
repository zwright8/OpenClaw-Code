...
| Field | Value |
|---|---|
| Skill pack | `b08-dns-proxy-optimization-toolchain` |
| Tool count | `26` |
| Inventory rank span | `3847-3879` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for dns, proxy traversal, optimization, toolchain assembly tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `gkrellm` | `eralchemy` | no | no |
| bounded mutation / transformation | `eralchemy` | `gkrellm` | yes | yes |
| validation & artifact checks | `gkrellm` | `eralchemy` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `gkrellm` | Extensible GTK system monitoring application | local-installable |
| `eralchemy` | Simple entity relation (ER) diagrams generation | local-installable |
| `aqtinstall` | Another unofficial Qt installer | local-installable |
| `cntlm` | NTLM authentication proxy with tunneling | local-installable |
| `zsync` | File transfer program | local-installable |
| `auto-editor` | Effort free video editing! | local-installable |
| `libp11` | PKCS#11 wrapper library in C | local-installable |
| `highs` | Linear optimization software | local-installable |
| `knot-resolver` | Minimalistic, caching, DNSSEC-validating DNS resolver | local-installable |
| `glyph` | Converts images/video to ASCII art | local-installable |
| `dmd` | Digital Mars D compiler | local-installable |
| `minizinc` | Medium-level constraint modeling language | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `gkrellm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `eralchemy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aqtinstall` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cntlm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zsync` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `auto-editor` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libp11` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `highs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `knot-resolver` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `glyph` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dmd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `minizinc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pake` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `redress` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `primecount` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `ptpython` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zint` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `chatblade` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtmultimedia` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `i686-elf-grub` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `todo-txt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dnsrobocert` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ddclient` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `corkscrew` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `joe` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libdnet` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
...
1. `gkrellm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `eralchemy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `aqtinstall` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `cntlm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `zsync` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `auto-editor` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a dns, proxy traversal, optimization, toolchain assembly request, run dry-run first, then produce a validated artifact bundle.
