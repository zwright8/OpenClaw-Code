---
name: linkedin-migration-assistant
description: Use when tasks require linkedin migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Migration Assistant
...
| Primary artifact | `linkedin-migration-cutover-plan` |
| Routing tag | `linkedin:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when state changes are hard to undo once they begin. This specific skill turns LinkedIn Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from linkedin..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| linkedin-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| linkedin-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for LinkedIn Migration Assistant before any LinkedIn rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from linkedin. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `linkedin-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:migration-assistant` consumers with approval and credential context attached.
