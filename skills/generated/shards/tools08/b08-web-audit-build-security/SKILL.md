---
name: b08-web-audit-build-security
description: Use when running web quality checks, dependency and cloud security audits, frontend/backend scaffolding, and secure transfer/authentication workflows.
---

# Web quality, cloud audit, and build security

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-web-audit-build-security` |
| Tool count | `26` |
| Inventory rank span | `3545-3579` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for web quality, cloud audit, build security tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `lighthouse` | `safety` | no | no |
| bounded mutation / transformation | `safety` | `lighthouse` | yes | yes |
| validation & artifact checks | `lighthouse` | `safety` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `lighthouse` | Rust Ethereum 2.0 Client | local-installable |
| `safety` | Checks Python dependencies for known vulnerabilities and suggests remediations | local-installable |
| `pyqt-builder` | Tool to build PyQt | local-installable |
| `cloudsplaining` | AWS IAM Security Assessment tool | local-installable |
| `gci` | Control Golang package import order and make it always deterministic | local-installable |
| `sftpgo` | Fully featured SFTP server with optional HTTP/S, FTP/S and WebDAV support | local-installable |
| `x86_64-elf-gdb` | GNU debugger for x86_64-elf cross development | local-installable |
| `claws-mail` | User-friendly, lightweight, and fast email client | local-installable |
| `dooit` | TUI todo manager | local-installable |
| `jhipster` | Generate, develop and deploy Spring Boot + Angular/React applications | local-installable |
| `templ` | Language for writing HTML user interfaces in Go | local-installable |
| `f2` | Command-line batch renaming tool | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `lighthouse` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `safety` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pyqt-builder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudsplaining` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gci` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sftpgo` | SFTP/FTP/WebDAV, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `x86_64-elf-gdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `claws-mail` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dooit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jhipster` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `templ` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `f2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `i2p` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ykpers` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `spectral-cli` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `detox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opencore-amr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dub` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `berkeley-db@5` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osrm-backend` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pyscn` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pfetch` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bower` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `maigret` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `blake3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vineyard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `cloudsplaining`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `lighthouse` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `safety` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read/query command sequence, capture outputs and exit code in handoff packet.
3. `pyqt-builder` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `cloudsplaining` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `gci` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `sftpgo` (SFTP/FTP/WebDAV, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a web quality, cloud audit, build security request, run dry-run first, then produce a validated artifact bundle.
