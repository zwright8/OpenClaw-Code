---
name: loom-webhook-listener
description: Use when tasks require loom webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Loom Webhook Listener
...
| Primary artifact | `loom-event-intake-log` |
| Routing tag | `loom:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Loom Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes loom webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `LOOM_CLIENT_ID`, `LOOM_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `LOOM_TOKEN`, `LOOM_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `LOOM_WEBHOOK_SECRET`, `LOOM_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| loom-event-intake-log | structured-artifact | yes | next workflow or operator |
| loom-event-intake-log-scorecard | scorecard | yes | reviewer |
| loom-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Loom Webhook Listener, then register the signing-secret policy for Loom.
...
3. Implement captures, verifies, and routes loom webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `loom-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] loom-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:webhook-listener` consumers with approval and credential context attached.
