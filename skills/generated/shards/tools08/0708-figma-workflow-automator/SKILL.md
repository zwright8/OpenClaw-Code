---
name: figma-workflow-automator
description: Use when tasks require figma workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Figma Workflow Automator
...
| Primary artifact | `figma-workflow-runbook` |
| Routing tag | `figma:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using figma..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-workflow-runbook | structured-artifact | yes | next workflow or operator |
| figma-workflow-runbook-scorecard | scorecard | yes | reviewer |
| figma-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Figma Workflow Automator, then checkpoint where Figma state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using figma. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `figma-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] figma-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:workflow-automator` consumers with approval and credential context attached.
