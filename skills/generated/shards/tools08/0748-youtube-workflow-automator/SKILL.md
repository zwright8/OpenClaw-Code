---
name: youtube-workflow-automator
description: Use when tasks require youtube workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# YouTube Workflow Automator
...
| Primary artifact | `youtube-workflow-runbook` |
| Routing tag | `youtube:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using youtube..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-workflow-runbook | structured-artifact | yes | next workflow or operator |
| youtube-workflow-runbook-scorecard | scorecard | yes | reviewer |
| youtube-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for YouTube Workflow Automator, then checkpoint where YouTube state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using youtube. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `youtube-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] youtube-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:workflow-automator` consumers with approval and credential context attached.
