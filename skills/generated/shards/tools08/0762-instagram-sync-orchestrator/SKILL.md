---
name: instagram-sync-orchestrator
description: Use when tasks require instagram sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-reconciliation-bundle` |
| Routing tag | `instagram:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for instagram records..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| instagram-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| instagram-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Instagram Sync Orchestrator, then checkpoint the last known good cursor on both sides of Instagram.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for instagram records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `instagram-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] instagram-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:sync-orchestrator` consumers with approval and credential context attached.
