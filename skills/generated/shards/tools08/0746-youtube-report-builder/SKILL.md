---
name: youtube-report-builder
description: Use when tasks require youtube report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `youtube-report-bundle` |
| Routing tag | `youtube:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from youtube activity..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-report-bundle | structured-artifact | yes | next workflow or operator |
| youtube-report-bundle-scorecard | scorecard | yes | reviewer |
| youtube-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for YouTube Report Builder, then confirm which YouTube entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from youtube activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `youtube-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] youtube-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:report-builder` consumers with approval and credential context attached.
