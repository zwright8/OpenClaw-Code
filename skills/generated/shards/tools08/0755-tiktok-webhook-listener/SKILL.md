---
name: tiktok-webhook-listener
description: Use when tasks require tiktok webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `tiktok-event-intake-log` |
| Routing tag | `tiktok:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns TikTok Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes tiktok webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `TIKTOK_CLIENT_ID`, `TIKTOK_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `TIKTOK_TOKEN`, `TIKTOK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `TIKTOK_WEBHOOK_SECRET`, `TIKTOK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| tiktok-event-intake-log | structured-artifact | yes | next workflow or operator |
| tiktok-event-intake-log-scorecard | scorecard | yes | reviewer |
| tiktok-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for TikTok Webhook Listener, then register the signing-secret policy for TikTok.
...
3. Implement captures, verifies, and routes tiktok webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `tiktok-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:webhook-listener` consumers with approval and credential context attached.
