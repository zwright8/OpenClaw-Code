---
name: linkedin-alert-monitor
description: Use when tasks require linkedin alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Alert Monitor
...
| Primary artifact | `linkedin-alert-state-bundle` |
| Routing tag | `linkedin:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Alert Monitor into a deterministic, auth-checked workflow for monitors key linkedin signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| linkedin-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| linkedin-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for LinkedIn Alert Monitor, then lock the reviewed threshold set for LinkedIn.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key linkedin signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `linkedin-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:alert-monitor` consumers with approval and credential context attached.
