---
name: canva-data-importer
description: Use when tasks require canva data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `canva-import-ledger` |
| Routing tag | `canva:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when state changes are hard to undo once they begin. This specific skill turns Canva Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into canva..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-import-ledger | structured-artifact | yes | next workflow or operator |
| canva-import-ledger-scorecard | scorecard | yes | reviewer |
| canva-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Canva Data Importer before any Canva write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into canva. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `canva-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] canva-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:data-importer` consumers with approval and credential context attached.
