---
name: b08-storage-sql-sdr-cloud
description: Use when mounting remote filesystems, querying distributed SQL engines, managing mail and log-analysis workflows, inspecting RF captures, and applying cloud policy enforcement.
---

# Storage overlays, SQL analytics, SDR inspection, and cloud governance

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-storage-sql-sdr-cloud` |
| Tool count | `26` |
| Inventory rank span | `3912-3941` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for storage overlays, sql analytics, sdr inspection, cloud governance tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `sshfs` | `prestodb` | no | no |
| bounded mutation / transformation | `prestodb` | `sshfs` | yes | yes |
| validation & artifact checks | `sshfs` | `prestodb` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `sshfs` | File system client based on SSH File Transfer Protocol | local-installable |
| `prestodb` | Distributed SQL query engine for big data | local-installable |
| `electric` | Real-time sync for Postgres | local-installable |
| `ffmpeg2theora` | Convert video files to Ogg Theora format | local-installable |
| `gmailctl` | Declarative configuration for Gmail filters | local-installable |
| `libint` | Library for computing electron repulsion integrals efficiently | local-installable |
| `revive` | Fast, configurable, extensible, flexible, and beautiful linter for Go | local-installable |
| `rtk` | CLI proxy to minimize LLM token consumption | local-installable |
| `widelands` | Free real-time strategy game like Settlers II | local-installable |
| `neko` | High-level, dynamically typed programming language | local-installable |
| `skillshare` | Sync skills across AI CLI tools | local-installable |
| `fatal` | Facebook Template Library | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `sshfs` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `prestodb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `electric` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ffmpeg2theora` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gmailctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libint` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `revive` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rtk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `widelands` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `neko` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `skillshare` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fatal` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `angle-grinder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sigrok-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtwebsockets` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `exempi` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `imgdiet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcaca` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `inspectrum` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sipcalc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lemon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `smug` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `git-octopus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c7n` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `librasterlite2` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
...
1. `sshfs` (SSH, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `prestodb` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read/query command sequence, capture outputs and exit code in handoff packet.
3. `electric` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `ffmpeg2theora` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `gmailctl` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libint` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a storage overlays, sql analytics, sdr inspection, cloud governance request, run dry-run first, then produce a validated artifact bundle.
