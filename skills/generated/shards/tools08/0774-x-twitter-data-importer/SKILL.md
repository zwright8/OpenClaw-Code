---
name: x-twitter-data-importer
description: Use when tasks require x twitter data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Data Importer
...
| Primary artifact | `x-twitter-import-ledger` |
| Routing tag | `x-twitter:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when state changes are hard to undo once they begin. This specific skill turns X Twitter Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into x twitter..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-import-ledger | structured-artifact | yes | next workflow or operator |
| x-twitter-import-ledger-scorecard | scorecard | yes | reviewer |
| x-twitter-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for X Twitter Data Importer before any X Twitter write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into x twitter. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `x-twitter-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:data-importer` consumers with approval and credential context attached.
