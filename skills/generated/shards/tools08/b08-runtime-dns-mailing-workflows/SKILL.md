...
| Field | Value |
|---|---|
| Skill pack | `b08-runtime-dns-mailing-workflows` |
| Tool count | `26` |
| Inventory rank span | `3676-3716` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for runtime stacks, dns diagnostics, mail/media workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `uvwasi` | `solc-select` | no | no |
| bounded mutation / transformation | `solc-select` | `uvwasi` | yes | yes |
| validation & artifact checks | `uvwasi` | `solc-select` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `uvwasi` | WASI syscall API built atop libuv | local-installed |
| `solc-select` | Manage multiple Solidity compiler versions | local-installable |
| `shairport-sync` | AirTunes emulator that adds multi-room capability | local-installable |
| `keystone` | Assembler framework: Core + bindings | local-installable |
| `wasi-runtimes` | Compiler-RT and libc++ runtimes for WASI | local-installable |
| `pgbadger` | Log analyzer for PostgreSQL | local-installable |
| `dmg2img` | Utilities for converting macOS DMG images | local-installable |
| `kiota` | OpenAPI based HTTP Client code generator | local-installable |
| `libsass` | C implementation of a Sass compiler | local-installable |
| `ktop` | Top-like tool for your Kubernetes clusters | local-installable |
| `gtkmm` | C++ interfaces for GTK+ and GNOME | local-installable |
| `icecast` | Streaming MP3 audio server | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `uvwasi` | CLI/local file I/O | None or local runtime | no | no | No API key required for local/offline use. |
| `solc-select` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shairport-sync` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `keystone` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wasi-runtimes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgbadger` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dmg2img` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kiota` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsass` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ktop` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gtkmm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `icecast` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xsane` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dfc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fetchmail` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dnsviz` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `evernote2md` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mako` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osm-gps-map` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lzfse` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `cargo-instruments` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `quickjs` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `llvm@21` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxt` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `sassc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cadaver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `ktop`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. Select a minimal tool from this pack to solve a runtime stacks, dns diagnostics, mail/media workflows request, run dry-run first, then produce a validated artifact bundle.
