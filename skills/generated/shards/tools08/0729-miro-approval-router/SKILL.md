---
name: miro-approval-router
description: Use when tasks require miro approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Miro Approval Router
...
| Primary artifact | `miro-approval-decision-packet` |
| Routing tag | `miro:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Approval Router into a deterministic, auth-checked workflow for routes miro-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| miro-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| miro-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Miro Approval Router, then capture the manual fallback path for Miro.
...
3. Implement routes miro-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `miro-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] miro-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:approval-router` consumers with approval and credential context attached.
