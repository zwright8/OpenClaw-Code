---
name: b08-service-mesh-monitoring-media
description: Use when operating mesh and cloud-edge services, collecting system telemetry, handling embedded firmware flows, and comparing API/media/document deltas.
---

# Service mesh, monitoring, embedded, and media stacks

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-service-mesh-monitoring-media` |
| Tool count | `26` |
| Inventory rank span | `3646-3675` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for service mesh, monitoring, embedded, media stacks tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `linkerd` | `gupnp` | no | no |
| bounded mutation / transformation | `gupnp` | `linkerd` | yes | yes |
| validation & artifact checks | `linkerd` | `gupnp` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `linkerd` | Command-line utility to interact with linkerd | local-installable |
| `gupnp` | Framework for creating UPnP devices and control points | local-installable |
| `rcm` | RC file (dotfile) management | local-installable |
| `ggc` | Modern Git CLI | local-installable |
| `vorbis-tools` | Ogg Vorbis CODEC tools | local-installable |
| `tmux-mem-cpu-load` | CPU, RAM memory, and load monitor for use with tmux | local-installable |
| `gerbv` | Gerber (RS-274X) viewer | local-installable |
| `nbdime` | Jupyter Notebook Diff and Merge tools | local-installable |
| `libsigrok` | Drivers for logic analyzers and other supported devices | local-installable |
| `nom` | RSS reader for the terminal | local-installable |
| `kerl` | Easy building and installing of Erlang/OTP instances | local-installable |
| `collectd` | Statistics collection and monitoring daemon | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `linkerd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gupnp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rcm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ggc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vorbis-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tmux-mem-cpu-load` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gerbv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nbdime` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsigrok` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nom` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kerl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `collectd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `eigenpy` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `espflash` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `erlang@26` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `loc` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pan` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spaceman-diff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `malcontent` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `azion` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtsvg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `speex` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spectra` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `openapi-diff` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dyld-headers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chardet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `linkerd` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `gupnp` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `rcm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `ggc` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `vorbis-tools` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `tmux-mem-cpu-load` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a service mesh, monitoring, embedded, media stacks request, run dry-run first, then produce a validated artifact bundle.
