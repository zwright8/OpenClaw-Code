---
name: b08-sql-format-release-desktop
description: Use when formatting SQL/JS/config assets, managing release automation, packaging desktop multimedia components, and handling mail/testing sidecars.
---

# SQL/editor formatting, release engineering, and desktop media plumbing

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-sql-format-release-desktop` |
| Tool count | `26` |
| Inventory rank span | `3942-3972` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for sql/editor formatting, release engineering, desktop media plumbing tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `sql-language-server` | `grok` | no | no |
| bounded mutation / transformation | `grok` | `sql-language-server` | yes | yes |
| validation & artifact checks | `sql-language-server` | `grok` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `sql-language-server` | Language Server for SQL | local-installable |
| `grok` | DRY and RAD for regular expressions and then some | local-installable |
| `js-beautify` | JavaScript, CSS and HTML unobfuscator and beautifier | local-installable |
| `libwmf` | Library for converting WMF (Window Metafile Format) files | local-installable |
| `spice-server` | Implements the server side of the SPICE protocol | local-installable |
| `tre` | Lightweight, POSIX-compliant regular expression (regex) library | local-installable |
| `djview4` | Viewer for the DjVu image format | local-installable |
| `libsm` | X.Org: X Session Management Library | local-installable |
| `libxpm` | X.Org: X Pixmap (XPM) image file format library | local-installable |
| `qtdeclarative` | QML, Qt Quick and several related modules | local-installable |
| `gengetopt` | Generate C code to parse command-line arguments via getopt_long | local-installable |
| `ldid-procursus` | Put real or fake signatures in a Mach-O binary | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `sql-language-server` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `grok` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `js-beautify` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libwmf` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `spice-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tre` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `djview4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `libxpm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `qtdeclarative` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gengetopt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ldid-procursus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bilix` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `alda` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `inih` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `amp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `eiffelstudio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mkvtomp4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `quazip` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bluetoothconnector` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `orc-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `release-it` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spack` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `duc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtshadertools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mailcatcher` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `sql-language-server` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `grok` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `js-beautify` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `libwmf` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `spice-server` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `tre` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a sql/editor formatting, release engineering, desktop media plumbing request, run dry-run first, then produce a validated artifact bundle.
