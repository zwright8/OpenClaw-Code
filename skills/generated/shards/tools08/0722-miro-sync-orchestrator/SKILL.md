---
name: miro-sync-orchestrator
description: Use when tasks require miro sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Miro Sync Orchestrator
...
| Primary artifact | `miro-reconciliation-bundle` |
| Routing tag | `miro:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for miro records..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| miro-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| miro-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Miro Sync Orchestrator, then checkpoint the last known good cursor on both sides of Miro.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for miro records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `miro-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] miro-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:sync-orchestrator` consumers with approval and credential context attached.
