---
name: figma-data-importer
description: Use when tasks require figma data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-import-ledger` |
| Routing tag | `figma:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when state changes are hard to undo once they begin. This specific skill turns Figma Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into figma..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-import-ledger | structured-artifact | yes | next workflow or operator |
| figma-import-ledger-scorecard | scorecard | yes | reviewer |
| figma-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Figma Data Importer before any Figma write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into figma. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `figma-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] figma-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:data-importer` consumers with approval and credential context attached.
