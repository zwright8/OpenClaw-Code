---
name: miro-alert-monitor
description: Use when tasks require miro alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `miro-alert-state-bundle` |
| Routing tag | `miro:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Alert Monitor into a deterministic, auth-checked workflow for monitors key miro signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| miro-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| miro-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Miro Alert Monitor, then lock the reviewed threshold set for Miro.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key miro signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `miro-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] miro-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:alert-monitor` consumers with approval and credential context attached.
