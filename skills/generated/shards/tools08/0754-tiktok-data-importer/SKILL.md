---
name: tiktok-data-importer
description: Use when tasks require tiktok data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `tiktok-import-ledger` |
| Routing tag | `tiktok:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when state changes are hard to undo once they begin. This specific skill turns TikTok Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into tiktok..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-import-ledger | structured-artifact | yes | next workflow or operator |
| tiktok-import-ledger-scorecard | scorecard | yes | reviewer |
| tiktok-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for TikTok Data Importer before any TikTok write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into tiktok. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `tiktok-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:data-importer` consumers with approval and credential context attached.
