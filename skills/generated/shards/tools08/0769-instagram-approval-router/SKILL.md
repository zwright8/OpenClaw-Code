---
name: instagram-approval-router
description: Use when tasks require instagram approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Instagram Approval Router
...
| Primary artifact | `instagram-approval-decision-packet` |
| Routing tag | `instagram:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Approval Router into a deterministic, auth-checked workflow for routes instagram-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| instagram-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| instagram-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Instagram Approval Router, then capture the manual fallback path for Instagram.
...
3. Implement routes instagram-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `instagram-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] instagram-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:approval-router` consumers with approval and credential context attached.
