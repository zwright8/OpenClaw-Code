...
description: Use when tasks require youtube connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `youtube-connection-trace-bundle` |
| Routing tag | `youtube:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Connector into a deterministic, auth-checked workflow for unified read/write connector for youtube apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| youtube-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| youtube-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for YouTube Connector, then confirm the smallest read-only probe that proves access to YouTube.
...
3. Specify the request and response contract for unified read/write connector for youtube apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `youtube-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] youtube-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:connector` consumers with approval and credential context attached.
