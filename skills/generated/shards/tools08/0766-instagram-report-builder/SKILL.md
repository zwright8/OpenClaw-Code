---
name: instagram-report-builder
description: Use when tasks require instagram report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-report-bundle` |
| Routing tag | `instagram:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from instagram activity..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-report-bundle | structured-artifact | yes | next workflow or operator |
| instagram-report-bundle-scorecard | scorecard | yes | reviewer |
| instagram-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Instagram Report Builder, then confirm which Instagram entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from instagram activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `instagram-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] instagram-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:report-builder` consumers with approval and credential context attached.
