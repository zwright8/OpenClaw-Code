...
description: Use when tasks require youtube webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `youtube-event-intake-log` |
| Routing tag | `youtube:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns YouTube Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes youtube webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `YOUTUBE_WEBHOOK_SECRET`, `YOUTUBE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| youtube-event-intake-log | structured-artifact | yes | next workflow or operator |
| youtube-event-intake-log-scorecard | scorecard | yes | reviewer |
| youtube-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for YouTube Webhook Listener, then register the signing-secret policy for YouTube.
...
3. Implement captures, verifies, and routes youtube webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `youtube-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] youtube-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:webhook-listener` consumers with approval and credential context attached.
