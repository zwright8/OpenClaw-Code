...
| Field | Value |
|---|---|
| Skill pack | `b08-db-messaging-build-release` |
| Tool count | `26` |
| Inventory rank span | `3780-3813` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for database partitioning, messaging, build/release automation tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `glaze` | `libxdmcp` | no | no |
| bounded mutation / transformation | `libxdmcp` | `glaze` | yes | yes |
| validation & artifact checks | `glaze` | `libxdmcp` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `glaze` | Extremely fast, in-memory JSON and interface library for modern C++ | local-installable |
| `libxdmcp` | X.Org: X Display Manager Control Protocol library | local-installable |
| `mongoose` | Web server build on top of Libmongoose embedded library | local-installable |
| `clip` | Create high-quality charts from the command-line | local-installable |
| `frotz` | Infocom-style interactive fiction player | local-installable |
| `pg_partman` | Partition management extension for PostgreSQL | local-installable |
| `azure-dev` | Developer CLI that provides commands for working with Azure resources | local-installable |
| `spirv-cross` | Performing reflection and disassembling SPIR-V | local-installable |
| `cataclysm` | Fork/variant of Cataclysm Roguelike | local-installable |
| `s-search` | Web search from the terminal | local-installable |
| `stormy` | Minimal, customizable and neofetch-like weather CLI based on rainy | local-installable |
| `lld@19` | LLVM Project Linker | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `glaze` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `libxdmcp` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `mongoose` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `clip` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `frotz` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pg_partman` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `azure-dev` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `spirv-cross` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cataclysm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `s-search` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `stormy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lld@19` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pulsarctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgweb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `toxiproxy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `netatalk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `archey4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apt-dater` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ansible-builder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-c-io` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `bazel-diff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sqlparse` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ejabberd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `docker-squash` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `fnox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bump-my-version` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-c-io`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `azure-dev`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `docker-squash`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `glaze` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `libxdmcp` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `mongoose` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `clip` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `frotz` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `pg_partman` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a database partitioning, messaging, build/release automation request, run dry-run first, then produce a validated artifact bundle.
