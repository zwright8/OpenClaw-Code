...
description: Use when tasks require figma connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-connection-trace-bundle` |
| Routing tag | `figma:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Connector into a deterministic, auth-checked workflow for unified read/write connector for figma apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| figma-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| figma-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Figma Connector, then confirm the smallest read-only probe that proves access to Figma.
...
3. Specify the request and response contract for unified read/write connector for figma apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `figma-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] figma-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:connector` consumers with approval and credential context attached.
