---
name: tiktok-workflow-automator
description: Use when tasks require tiktok workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Workflow Automator
...
| Primary artifact | `tiktok-workflow-runbook` |
| Routing tag | `tiktok:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using tiktok..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-workflow-runbook | structured-artifact | yes | next workflow or operator |
| tiktok-workflow-runbook-scorecard | scorecard | yes | reviewer |
| tiktok-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for TikTok Workflow Automator, then checkpoint where TikTok state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using tiktok. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `tiktok-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:workflow-automator` consumers with approval and credential context attached.
