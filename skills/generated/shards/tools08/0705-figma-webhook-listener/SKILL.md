---
name: figma-webhook-listener
description: Use when tasks require figma webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-event-intake-log` |
| Routing tag | `figma:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Figma Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes figma webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `FIGMA_CLIENT_ID`, `FIGMA_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `FIGMA_TOKEN`, `FIGMA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `FIGMA_WEBHOOK_SECRET`, `FIGMA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| figma-event-intake-log | structured-artifact | yes | next workflow or operator |
| figma-event-intake-log-scorecard | scorecard | yes | reviewer |
| figma-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Figma Webhook Listener, then register the signing-secret policy for Figma.
...
3. Implement captures, verifies, and routes figma webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `figma-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] figma-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:webhook-listener` consumers with approval and credential context attached.
