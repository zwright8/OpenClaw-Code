---
name: x-twitter-report-builder
description: Use when tasks require x twitter report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Report Builder
...
| Primary artifact | `x-twitter-report-bundle` |
| Routing tag | `x-twitter:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns X Twitter Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from x twitter activity..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-report-bundle | structured-artifact | yes | next workflow or operator |
| x-twitter-report-bundle-scorecard | scorecard | yes | reviewer |
| x-twitter-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for X Twitter Report Builder, then confirm which X Twitter entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from x twitter activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `x-twitter-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:report-builder` consumers with approval and credential context attached.
