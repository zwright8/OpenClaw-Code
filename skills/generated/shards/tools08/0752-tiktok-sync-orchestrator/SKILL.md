---
name: tiktok-sync-orchestrator
description: Use when tasks require tiktok sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Sync Orchestrator
...
| Primary artifact | `tiktok-reconciliation-bundle` |
| Routing tag | `tiktok:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for tiktok records..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| tiktok-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| tiktok-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for TikTok Sync Orchestrator, then checkpoint the last known good cursor on both sides of TikTok.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for tiktok records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `tiktok-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:sync-orchestrator` consumers with approval and credential context attached.
