---
name: youtube-data-exporter
description: Use when tasks require youtube data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `youtube-export-package` |
| Routing tag | `youtube:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because YouTube workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns YouTube Data Exporter into a deterministic, auth-checked workflow for exports youtube entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `YouTube Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| youtube-export-package | structured-artifact | yes | next workflow or operator |
| youtube-export-package-scorecard | scorecard | yes | reviewer |
| youtube-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for YouTube Data Exporter, then constrain the export scope for YouTube before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports youtube entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `youtube-export-package` so downstream owners do not need to rediscover context.
...
- [ ] youtube-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `youtube-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `youtube:data-exporter` consumers with approval and credential context attached.
