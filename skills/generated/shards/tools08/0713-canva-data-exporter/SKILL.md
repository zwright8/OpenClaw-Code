---
name: canva-data-exporter
description: Use when tasks require canva data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `canva-export-package` |
| Routing tag | `canva:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Data Exporter into a deterministic, auth-checked workflow for exports canva entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-export-package | structured-artifact | yes | next workflow or operator |
| canva-export-package-scorecard | scorecard | yes | reviewer |
| canva-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Canva Data Exporter, then constrain the export scope for Canva before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports canva entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `canva-export-package` so downstream owners do not need to rediscover context.
...
- [ ] canva-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:data-exporter` consumers with approval and credential context attached.
