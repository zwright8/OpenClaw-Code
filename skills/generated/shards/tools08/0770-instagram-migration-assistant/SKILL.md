---
name: instagram-migration-assistant
description: Use when tasks require instagram migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Instagram Migration Assistant
...
| Primary artifact | `instagram-migration-cutover-plan` |
| Routing tag | `instagram:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when state changes are hard to undo once they begin. This specific skill turns Instagram Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from instagram..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| instagram-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| instagram-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Instagram Migration Assistant before any Instagram rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from instagram. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `instagram-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] instagram-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:migration-assistant` consumers with approval and credential context attached.
