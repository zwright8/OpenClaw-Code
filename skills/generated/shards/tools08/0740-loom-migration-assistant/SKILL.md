---
name: loom-migration-assistant
description: Use when tasks require loom migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Loom Migration Assistant
...
| Primary artifact | `loom-migration-cutover-plan` |
| Routing tag | `loom:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when state changes are hard to undo once they begin. This specific skill turns Loom Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from loom..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| loom-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| loom-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Loom Migration Assistant before any Loom rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from loom. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `loom-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] loom-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:migration-assistant` consumers with approval and credential context attached.
