---
name: tiktok-report-builder
description: Use when tasks require tiktok report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Report Builder
...
| Primary artifact | `tiktok-report-bundle` |
| Routing tag | `tiktok:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from tiktok activity..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-report-bundle | structured-artifact | yes | next workflow or operator |
| tiktok-report-bundle-scorecard | scorecard | yes | reviewer |
| tiktok-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for TikTok Report Builder, then confirm which TikTok entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from tiktok activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `tiktok-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:report-builder` consumers with approval and credential context attached.
