---
name: b08-bio-shell-devops-audio
description: Use when combining bioinformatics command pipelines, safer shell operations, release automation, Kubernetes delivery, and audio/media workstation tasks.
---

# Bioinformatics, shell productivity, DevOps, and audio toolchains

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-bio-shell-devops-audio` |
| Tool count | `26` |
| Inventory rank span | `3749-3779` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for bioinformatics, shell productivity, devops, audio toolchains tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `seqtk` | `rm-improved` | no | no |
| bounded mutation / transformation | `rm-improved` | `seqtk` | yes | yes |
| validation & artifact checks | `seqtk` | `rm-improved` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `seqtk` | Toolkit for processing sequences in FASTA/Q formats | local-installable |
| `rm-improved` | Command-line deletion tool focused on safety, ergonomics, and performance | local-installable |
| `librime` | Rime Input Method Engine | local-installable |
| `libsoxr` | High quality, one-dimensional sample-rate conversion library | local-installable |
| `nu` | Object-oriented, Lisp-like programming language | local-installable |
| `noir` | Attack surface detector that identifies endpoints by static analysis | local-installable |
| `daemon` | Turn other processes into daemons | local-installable |
| `tcpflow` | TCP/IP packet demultiplexer | local-installable |
| `picard-tools` | Tools for manipulating HTS data and formats | local-installable |
| `ruff-lsp` | Language Server Protocol implementation for Ruff | local-installable |
| `qjackctl` | Simple Qt application to control the JACK sound server daemon | local-installable |
| `chart-releaser` | Hosting Helm Charts via GitHub Pages and Releases | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `seqtk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rm-improved` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `librime` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsoxr` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `nu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `noir` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `daemon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tcpflow` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `picard-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruff-lsp` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qjackctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chart-releaser` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `qt5compat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nanobind` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dfu-programmer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudflare-quiche` | QUIC, HTTP/3, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `davmail` | LDAP, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pangomm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `corsixth` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jupyter-r` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kubevela` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `supertux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ansible-language-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `woodpecker-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ckan` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `chart-releaser`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `kubevela`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `seqtk` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `rm-improved` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `librime` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `libsoxr` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `nu` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `noir` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a bioinformatics, shell productivity, devops, audio toolchains request, run dry-run first, then produce a validated artifact bundle.
