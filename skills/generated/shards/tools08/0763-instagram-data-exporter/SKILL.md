---
name: instagram-data-exporter
description: Use when tasks require instagram data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `instagram-export-package` |
| Routing tag | `instagram:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Data Exporter into a deterministic, auth-checked workflow for exports instagram entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-export-package | structured-artifact | yes | next workflow or operator |
| instagram-export-package-scorecard | scorecard | yes | reviewer |
| instagram-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Instagram Data Exporter, then constrain the export scope for Instagram before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports instagram entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `instagram-export-package` so downstream owners do not need to rediscover context.
...
- [ ] instagram-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:data-exporter` consumers with approval and credential context attached.
