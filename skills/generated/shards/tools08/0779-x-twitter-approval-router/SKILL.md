---
name: x-twitter-approval-router
description: Use when tasks require x twitter approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Approval Router
...
| Primary artifact | `x-twitter-approval-decision-packet` |
| Routing tag | `x-twitter:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns X Twitter Approval Router into a deterministic, auth-checked workflow for routes x twitter-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| x-twitter-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| x-twitter-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for X Twitter Approval Router, then capture the manual fallback path for X Twitter.
...
3. Implement routes x twitter-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `x-twitter-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:approval-router` consumers with approval and credential context attached.
