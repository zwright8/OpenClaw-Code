---
name: linkedin-data-exporter
description: Use when tasks require linkedin data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `linkedin-export-package` |
| Routing tag | `linkedin:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Data Exporter into a deterministic, auth-checked workflow for exports linkedin entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-export-package | structured-artifact | yes | next workflow or operator |
| linkedin-export-package-scorecard | scorecard | yes | reviewer |
| linkedin-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for LinkedIn Data Exporter, then constrain the export scope for LinkedIn before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports linkedin entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `linkedin-export-package` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:data-exporter` consumers with approval and credential context attached.
