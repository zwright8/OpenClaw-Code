---
name: linkedin-sync-orchestrator
description: Use when tasks require linkedin sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# LinkedIn Sync Orchestrator
...
| Primary artifact | `linkedin-reconciliation-bundle` |
| Routing tag | `linkedin:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns LinkedIn Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for linkedin records..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| linkedin-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| linkedin-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for LinkedIn Sync Orchestrator, then checkpoint the last known good cursor on both sides of LinkedIn.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for linkedin records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `linkedin-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:sync-orchestrator` consumers with approval and credential context attached.
