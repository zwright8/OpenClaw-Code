---
name: figma-approval-router
description: Use when tasks require figma approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Figma Approval Router
...
| Primary artifact | `figma-approval-decision-packet` |
| Routing tag | `figma:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Approval Router into a deterministic, auth-checked workflow for routes figma-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| figma-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| figma-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Figma Approval Router, then capture the manual fallback path for Figma.
...
3. Implement routes figma-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `figma-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] figma-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:approval-router` consumers with approval and credential context attached.
