---
name: canva-sync-orchestrator
description: Use when tasks require canva sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Canva Sync Orchestrator
...
| Primary artifact | `canva-reconciliation-bundle` |
| Routing tag | `canva:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for canva records..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| canva-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| canva-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Canva Sync Orchestrator, then checkpoint the last known good cursor on both sides of Canva.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for canva records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `canva-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] canva-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:sync-orchestrator` consumers with approval and credential context attached.
