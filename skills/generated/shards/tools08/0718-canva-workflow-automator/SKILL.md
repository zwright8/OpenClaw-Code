---
name: canva-workflow-automator
description: Use when tasks require canva workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Canva Workflow Automator
...
| Primary artifact | `canva-workflow-runbook` |
| Routing tag | `canva:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using canva..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-workflow-runbook | structured-artifact | yes | next workflow or operator |
| canva-workflow-runbook-scorecard | scorecard | yes | reviewer |
| canva-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Canva Workflow Automator, then checkpoint where Canva state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using canva. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `canva-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] canva-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:workflow-automator` consumers with approval and credential context attached.
