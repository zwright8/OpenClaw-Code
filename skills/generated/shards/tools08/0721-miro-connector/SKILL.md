---
name: miro-connector
description: Use when tasks require miro connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `miro-connection-trace-bundle` |
| Routing tag | `miro:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Connector into a deterministic, auth-checked workflow for unified read/write connector for miro apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| miro-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| miro-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Miro Connector, then confirm the smallest read-only probe that proves access to Miro.
...
3. Specify the request and response contract for unified read/write connector for miro apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `miro-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] miro-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:connector` consumers with approval and credential context attached.
