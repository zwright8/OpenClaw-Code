---
name: tiktok-approval-router
description: Use when tasks require tiktok approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Approval Router
...
| Primary artifact | `tiktok-approval-decision-packet` |
| Routing tag | `tiktok:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Approval Router into a deterministic, auth-checked workflow for routes tiktok-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| tiktok-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| tiktok-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for TikTok Approval Router, then capture the manual fallback path for TikTok.
...
3. Implement routes tiktok-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `tiktok-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:approval-router` consumers with approval and credential context attached.
