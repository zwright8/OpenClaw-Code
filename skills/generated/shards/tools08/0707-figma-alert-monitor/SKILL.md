---
name: figma-alert-monitor
description: Use when tasks require figma alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-alert-state-bundle` |
| Routing tag | `figma:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Alert Monitor into a deterministic, auth-checked workflow for monitors key figma signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| figma-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| figma-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Figma Alert Monitor, then lock the reviewed threshold set for Figma.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key figma signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `figma-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] figma-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:alert-monitor` consumers with approval and credential context attached.
