---
name: loom-workflow-automator
description: Use when tasks require loom workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Loom Workflow Automator
...
| Primary artifact | `loom-workflow-runbook` |
| Routing tag | `loom:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using loom..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-workflow-runbook | structured-artifact | yes | next workflow or operator |
| loom-workflow-runbook-scorecard | scorecard | yes | reviewer |
| loom-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Loom Workflow Automator, then checkpoint where Loom state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using loom. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `loom-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] loom-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:workflow-automator` consumers with approval and credential context attached.
