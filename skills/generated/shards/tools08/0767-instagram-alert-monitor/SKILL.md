---
name: instagram-alert-monitor
description: Use when tasks require instagram alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Instagram Alert Monitor
...
| Primary artifact | `instagram-alert-state-bundle` |
| Routing tag | `instagram:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Instagram workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Instagram Alert Monitor into a deterministic, auth-checked workflow for monitors key instagram signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Instagram Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| instagram-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| instagram-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| instagram-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Instagram Alert Monitor, then lock the reviewed threshold set for Instagram.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key instagram signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `instagram-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] instagram-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `instagram-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `instagram:alert-monitor` consumers with approval and credential context attached.
