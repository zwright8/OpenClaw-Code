---
name: figma-data-exporter
description: Use when tasks require figma data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `figma-export-package` |
| Routing tag | `figma:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Figma workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Figma Data Exporter into a deterministic, auth-checked workflow for exports figma entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Figma Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| figma-export-package | structured-artifact | yes | next workflow or operator |
| figma-export-package-scorecard | scorecard | yes | reviewer |
| figma-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Figma Data Exporter, then constrain the export scope for Figma before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports figma entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `figma-export-package` so downstream owners do not need to rediscover context.
...
- [ ] figma-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `figma-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `figma:data-exporter` consumers with approval and credential context attached.
