---
name: tiktok-data-exporter
description: Use when tasks require tiktok data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# TikTok Data Exporter
...
| Primary artifact | `tiktok-export-package` |
| Routing tag | `tiktok:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because TikTok workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns TikTok Data Exporter into a deterministic, auth-checked workflow for exports tiktok entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `TikTok Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| tiktok-export-package | structured-artifact | yes | next workflow or operator |
| tiktok-export-package-scorecard | scorecard | yes | reviewer |
| tiktok-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for TikTok Data Exporter, then constrain the export scope for TikTok before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports tiktok entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `tiktok-export-package` so downstream owners do not need to rediscover context.
...
- [ ] tiktok-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `tiktok-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `tiktok:data-exporter` consumers with approval and credential context attached.
