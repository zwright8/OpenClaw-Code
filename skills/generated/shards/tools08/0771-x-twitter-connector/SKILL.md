...
description: Use when tasks require x twitter connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `x-twitter-connection-trace-bundle` |
| Routing tag | `x-twitter:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns X Twitter Connector into a deterministic, auth-checked workflow for unified read/write connector for x twitter apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| x-twitter-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| x-twitter-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for X Twitter Connector, then confirm the smallest read-only probe that proves access to X Twitter.
...
3. Specify the request and response contract for unified read/write connector for x twitter apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `x-twitter-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:connector` consumers with approval and credential context attached.
