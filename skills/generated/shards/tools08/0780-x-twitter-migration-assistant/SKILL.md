---
name: x-twitter-migration-assistant
description: Use when tasks require x twitter migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# X Twitter Migration Assistant
...
| Primary artifact | `x-twitter-migration-cutover-plan` |
| Routing tag | `x-twitter:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because X Twitter workflows degrade when state changes are hard to undo once they begin. This specific skill turns X Twitter Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from x twitter..

## Trigger Checklist
- [ ] The task explicitly requires `X Twitter Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| x-twitter-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| x-twitter-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| x-twitter-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for X Twitter Migration Assistant before any X Twitter rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from x twitter. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `x-twitter-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] x-twitter-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `x-twitter-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `x-twitter:migration-assistant` consumers with approval and credential context attached.
