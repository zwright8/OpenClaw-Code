---
name: canva-alert-monitor
description: Use when tasks require canva alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `canva-alert-state-bundle` |
| Routing tag | `canva:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Alert Monitor into a deterministic, auth-checked workflow for monitors key canva signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| canva-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| canva-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Canva Alert Monitor, then lock the reviewed threshold set for Canva.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key canva signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `canva-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] canva-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:alert-monitor` consumers with approval and credential context attached.
