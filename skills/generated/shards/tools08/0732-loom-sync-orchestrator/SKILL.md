---
name: loom-sync-orchestrator
description: Use when tasks require loom sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Loom Sync Orchestrator
...
| Primary artifact | `loom-reconciliation-bundle` |
| Routing tag | `loom:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for loom records..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| loom-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| loom-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Loom Sync Orchestrator, then checkpoint the last known good cursor on both sides of Loom.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for loom records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `loom-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] loom-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:sync-orchestrator` consumers with approval and credential context attached.
