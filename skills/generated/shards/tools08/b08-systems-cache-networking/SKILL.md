---
name: b08-systems-cache-networking
description: Use when handling low-level ELF/capability tooling, cache/proxy/network services, artifact repositories, graph databases, CI integrations, and scientific compute stacks.
---

# Systems internals, cache/network services, and scientific runtimes

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-systems-cache-networking` |
| Tool count | `24` |
| Inventory rank span | `3973-4000` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for systems internals, cache/network services, scientific runtimes tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `roswell` | `nexus` | no | no |
| bounded mutation / transformation | `nexus` | `roswell` | yes | yes |
| validation & artifact checks | `roswell` | `nexus` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `roswell` | Lisp installer and launcher for major environments | local-installable |
| `nexus` | Repository manager for binary software components | local-installable |
| `tre-command` | Tree command, improved | local-installable |
| `elfutils` | Libraries and utilities for handling ELF objects | local-installable |
| `aalib` | Portable ASCII art graphics library | local-installable |
| `libvncserver` | VNC server and client libraries | local-installable |
| `nmrpflash` | Netgear Unbrick Utility | local-installable |
| `dzr` | Command-line Deezer.com player | local-installable |
| `libepoxy` | Library for handling OpenGL function pointer management | local-installable |
| `varnish` | High-performance HTTP accelerator | local-installable |
| `http-prompt` | Interactive command-line HTTP client with autocomplete and syntax highlighting | local-installable |
| `qman` | Modern man page viewer | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `roswell` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nexus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tre-command` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `elfutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aalib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `libvncserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nmrpflash` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dzr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libepoxy` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `varnish` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `http-prompt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `claudekit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `babl` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `asc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kuzu` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `puzzles` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ivy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `travis` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nwchem` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opkssh` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `criterion` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpaho-mqtt` | MQTT, Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
...
1. `roswell` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `nexus` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `tre-command` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `elfutils` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `aalib` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libvncserver` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a systems internals, cache/network services, scientific runtimes request, run dry-run first, then produce a validated artifact bundle.
