---
name: canva-report-builder
description: Use when tasks require canva report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Canva Report Builder
...
| Primary artifact | `canva-report-bundle` |
| Routing tag | `canva:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Canva workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Canva Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from canva activity..

## Trigger Checklist
- [ ] The task explicitly requires `Canva Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| canva-report-bundle | structured-artifact | yes | next workflow or operator |
| canva-report-bundle-scorecard | scorecard | yes | reviewer |
| canva-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Canva Report Builder, then confirm which Canva entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from canva activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `canva-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] canva-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `canva-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `canva:report-builder` consumers with approval and credential context attached.
