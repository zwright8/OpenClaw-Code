---
name: miro-workflow-automator
description: Use when tasks require miro workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Miro Workflow Automator
...
| Primary artifact | `miro-workflow-runbook` |
| Routing tag | `miro:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Miro workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Miro Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using miro..

## Trigger Checklist
- [ ] The task explicitly requires `Miro Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| miro-workflow-runbook | structured-artifact | yes | next workflow or operator |
| miro-workflow-runbook-scorecard | scorecard | yes | reviewer |
| miro-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Miro Workflow Automator, then checkpoint where Miro state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using miro. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `miro-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] miro-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `miro-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `miro:workflow-automator` consumers with approval and credential context attached.
