---
name: linkedin-data-importer
description: Use when tasks require linkedin data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `linkedin-import-ledger` |
| Routing tag | `linkedin:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because LinkedIn workflows degrade when state changes are hard to undo once they begin. This specific skill turns LinkedIn Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into linkedin..

## Trigger Checklist
- [ ] The task explicitly requires `LinkedIn Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linkedin-import-ledger | structured-artifact | yes | next workflow or operator |
| linkedin-import-ledger-scorecard | scorecard | yes | reviewer |
| linkedin-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for LinkedIn Data Importer before any LinkedIn write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into linkedin. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `linkedin-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] linkedin-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linkedin-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linkedin:data-importer` consumers with approval and credential context attached.
