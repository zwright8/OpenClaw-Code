---
name: b08-tunnel-embedded-observability
description: Use when setting up secure tunnels, embedded cross-toolchains, infra visualization, PDF/data extraction, and linting plus observability workflows.
---

# Tunneling, embedded toolchains, and observability

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b08-tunnel-embedded-observability` |
| Tool count | `26` |
| Inventory rank span | `3582-3613` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for tunneling, embedded toolchains, observability tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `wstunnel` | `qttools` | no | no |
| bounded mutation / transformation | `qttools` | `wstunnel` | yes | yes |
| validation & artifact checks | `wstunnel` | `qttools` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `wstunnel` | Tunnel all your traffic over Websocket or HTTP2 | local-installable |
| `qttools` | Facilitate the design, development, testing and deployment of applications | local-installable |
| `pinocchio` | Efficient and fast C++ library implementing Rigid Body Dynamics algorithms | local-installable |
| `clang-uml` | Customizable automatic UML diagram generator for C++ based on Clang | local-installable |
| `riscv64-elf-binutils` | GNU Binutils for riscv64-elf cross development | local-installable |
| `libvirt-glib` | Libvirt API for glib-based programs | local-installable |
| `bindgen` | Automatically generates Rust FFI bindings to C (and some C++) libraries | local-installable |
| `jupytext` | Jupyter notebooks as Markdown documents, Julia, Python or R scripts | local-installable |
| `nvc` | VHDL compiler and simulator | local-installable |
| `iso-codes` | Provides lists of various ISO standards | local-installable |
| `tomcat-native` | Lets Tomcat use some native resources for performance | local-installable |
| `hysteria` | Feature-packed proxy & relay tool optimized for lossy, unstable connections | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `wstunnel` | JSON-RPC/WebSocket, HTTP/2, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qttools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pinocchio` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `clang-uml` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `riscv64-elf-binutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libvirt-glib` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bindgen` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jupytext` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nvc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `iso-codes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tomcat-native` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hysteria` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `i386-elf-gdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdftohtml` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fennel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotenv-linter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `helmsman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `czg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bulk_extractor` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `grsync` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libbladerf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `terramaid` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opensearch-dashboards` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `exif` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxi` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `golangci-lint-langserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `wstunnel` (JSON-RPC/WebSocket, HTTP/2, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `qttools` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `pinocchio` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `clang-uml` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `riscv64-elf-binutils` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libvirt-glib` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a tunneling, embedded toolchains, observability request, run dry-run first, then produce a validated artifact bundle.
