---
name: loom-data-exporter
description: Use when tasks require loom data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `loom-export-package` |
| Routing tag | `loom:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Loom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Loom Data Exporter into a deterministic, auth-checked workflow for exports loom entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Loom Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| loom-export-package | structured-artifact | yes | next workflow or operator |
| loom-export-package-scorecard | scorecard | yes | reviewer |
| loom-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Loom Data Exporter, then constrain the export scope for Loom before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports loom entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `loom-export-package` so downstream owners do not need to rediscover context.
...
- [ ] loom-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `loom-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `loom:data-exporter` consumers with approval and credential context attached.
