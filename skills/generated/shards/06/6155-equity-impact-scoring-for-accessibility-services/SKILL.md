...
## When To Use
Use this skill only when the request explicitly needs `Equity Impact Scoring for accessibility services` in accessibility services and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Equity Impact Scoring for accessibility services** outcomes (not generic brainstorming).
...
1. **Incident stabilization in accessibility services" capability for accessibility services**
   - Input: noisy upstream payload requiring equity impact scoring for accessibility services normalization/assessment.
