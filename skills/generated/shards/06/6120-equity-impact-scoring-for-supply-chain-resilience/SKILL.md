...
## When To Use
Use this skill only when the request explicitly needs `Equity impact scoring for supply chain resilience` in supply chain resilience and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Equity impact scoring for supply chain resilience** outcomes (not generic brainstorming).
...
1. **Incident stabilization in supply chain resilience" capability in production for supply chain resilience workflows**
   - Input: noisy upstream payload requiring equity impact scoring for supply chain resilience normalization/assessment.
