---
name: google-ai-sync-orchestrator
description: Use when tasks require google ai sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Sync Orchestrator
...
| Primary artifact | `google-ai-reconciliation-bundle` |
| Routing tag | `google-ai:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google AI Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for google ai records..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| google-ai-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| google-ai-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Google AI Sync Orchestrator, then checkpoint the last known good cursor on both sides of Google AI.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for google ai records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `google-ai-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:sync-orchestrator` consumers with approval and credential context attached.
