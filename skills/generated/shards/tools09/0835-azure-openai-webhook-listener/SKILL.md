---
name: azure-openai-webhook-listener
description: Use when tasks require azure openai webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Webhook Listener
...
| Domain | `AI and model platforms` |
| Runtime archetype | `signed-event-ingestion-engine` |
| Core method | `signed event intake and routing` |
| Primary artifact | `azure-openai-event-intake-log` |
| Routing tag | `azure-openai:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Azure OpenAI Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes azure openai webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `AZURE_OPENAI_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Webhook signing secret | `signing_secret` | `AZURE_OPENAI_WEBHOOK_SECRET`, `AZURE_OPENAI_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `AZURE_OPENAI_TOKEN`, `AZURE_OPENAI_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| azure-openai-event-intake-log | structured-artifact | yes | next workflow or operator |
| azure-openai-event-intake-log-scorecard | scorecard | yes | reviewer |
| azure-openai-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Azure OpenAI Webhook Listener, then register the signing-secret policy for Azure OpenAI.
...
3. Implement captures, verifies, and routes azure openai webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `azure-openai-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:webhook-listener` consumers with approval and credential context attached.
