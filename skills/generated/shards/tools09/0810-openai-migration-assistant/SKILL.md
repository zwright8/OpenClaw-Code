---
name: openai-migration-assistant
description: Use when tasks require openai migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Migration Assistant
...
| Primary artifact | `openai-migration-cutover-plan` |
| Routing tag | `openai:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when state changes are hard to undo once they begin. This specific skill turns OpenAI Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from openai..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| openai-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| openai-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for OpenAI Migration Assistant before any OpenAI rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from openai. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `openai-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] openai-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:migration-assistant` consumers with approval and credential context attached.
