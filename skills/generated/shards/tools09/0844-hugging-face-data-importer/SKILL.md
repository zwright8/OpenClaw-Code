---
name: hugging-face-data-importer
description: Use when tasks require hugging face data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Data Importer
...
| Primary artifact | `hugging-face-import-ledger` |
| Routing tag | `hugging-face:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when state changes are hard to undo once they begin. This specific skill turns Hugging Face Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into hugging face..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-import-ledger | structured-artifact | yes | next workflow or operator |
| hugging-face-import-ledger-scorecard | scorecard | yes | reviewer |
| hugging-face-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Hugging Face Data Importer before any Hugging Face write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into hugging face. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `hugging-face-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:data-importer` consumers with approval and credential context attached.
