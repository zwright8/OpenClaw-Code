...
description: Use when tasks require azure openai connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-openai-connection-trace-bundle` |
| Routing tag | `azure-openai:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Connector into a deterministic, auth-checked workflow for unified read/write connector for azure openai apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| azure-openai-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| azure-openai-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Azure OpenAI Connector, then confirm the smallest read-only probe that proves access to Azure OpenAI.
...
3. Specify the request and response contract for unified read/write connector for azure openai apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `azure-openai-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:connector` consumers with approval and credential context attached.
