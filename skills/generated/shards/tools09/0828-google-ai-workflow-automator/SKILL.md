---
name: google-ai-workflow-automator
description: Use when tasks require google ai workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Workflow Automator
...
| Primary artifact | `google-ai-workflow-runbook` |
| Routing tag | `google-ai:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google AI Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using google ai..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-workflow-runbook | structured-artifact | yes | next workflow or operator |
| google-ai-workflow-runbook-scorecard | scorecard | yes | reviewer |
| google-ai-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Google AI Workflow Automator, then checkpoint where Google AI state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using google ai. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `google-ai-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:workflow-automator` consumers with approval and credential context attached.
