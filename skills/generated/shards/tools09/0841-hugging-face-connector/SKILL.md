...
description: Use when tasks require hugging face connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `hugging-face-connection-trace-bundle` |
| Routing tag | `hugging-face:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Connector into a deterministic, auth-checked workflow for unified read/write connector for hugging face apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| hugging-face-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| hugging-face-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Hugging Face Connector, then confirm the smallest read-only probe that proves access to Hugging Face.
...
3. Specify the request and response contract for unified read/write connector for hugging face apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `hugging-face-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:connector` consumers with approval and credential context attached.
