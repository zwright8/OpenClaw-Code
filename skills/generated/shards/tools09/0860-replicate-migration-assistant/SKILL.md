---
name: replicate-migration-assistant
description: Use when tasks require replicate migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Migration Assistant
...
| Primary artifact | `replicate-migration-cutover-plan` |
| Routing tag | `replicate:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when state changes are hard to undo once they begin. This specific skill turns Replicate Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from replicate..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| replicate-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| replicate-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Replicate Migration Assistant before any Replicate rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from replicate. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `replicate-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] replicate-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:migration-assistant` consumers with approval and credential context attached.
