---
name: elevenlabs-sync-orchestrator
description: Use when tasks require elevenlabs sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Sync Orchestrator
...
| Primary artifact | `elevenlabs-reconciliation-bundle` |
| Routing tag | `elevenlabs:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for elevenlabs records..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| elevenlabs-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| elevenlabs-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for ElevenLabs Sync Orchestrator, then checkpoint the last known good cursor on both sides of ElevenLabs.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for elevenlabs records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `elevenlabs-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:sync-orchestrator` consumers with approval and credential context attached.
