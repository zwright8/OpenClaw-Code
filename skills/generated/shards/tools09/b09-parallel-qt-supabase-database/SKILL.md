---
name: b09-parallel-qt-supabase-database
description: Use when orchestrating distributed shell commands, Qt feature modules, Supabase/MCP integrations, and Postgres-centric maintenance workflows.
---

# Parallel shell, Qt modules, Supabase, and database clients

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-parallel-qt-supabase-database` |
| Tool count | `26` |
| Inventory rank span | `4231-4261` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for parallel shell, qt modules, supabase, database clients tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `pdsh` | `qtscxml` | no | no |
| bounded mutation / transformation | `qtscxml` | `pdsh` | yes | yes |
| validation & artifact checks | `pdsh` | `qtscxml` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `pdsh` | Efficient rsh-like utility, for using hosts in parallel | local-installable |
| `qtscxml` | Provides functionality to create state machines from SCXML files | local-installable |
| `gtranslator` | GNOME gettext PO file editor | local-installable |
| `supabase-mcp-server` | MCP Server for Supabase | local-installable |
| `pgxnclient` | Command-line client for the PostgreSQL Extension Network | local-installable |
| `qtconnectivity` | Provides access to Bluetooth hardware | local-installable |
| `cpr` | C++ Requests, a spiritual port of Python Requests | local-installable |
| `mjml` | JavaScript framework that makes responsive-email easy | local-installable |
| `dockerfmt` | Dockerfile format and parser. a modern dockfmt | local-installable |
| `sloc` | Simple tool to count source lines of code | local-installable |
| `avro-cpp` | Data serialization system | local-installable |
| `m1ddc` | Control external displays (USB-C/DisplayPort Alt Mode) using DDC/CI on M1 Macs | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `pdsh` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtscxml` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gtranslator` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `supabase-mcp-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgxnclient` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtconnectivity` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cpr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mjml` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dockerfmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sloc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `avro-cpp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `m1ddc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zsv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cpio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libshumate` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgsync` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osm-pbf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jid` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `msc-generator` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotbot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `moon-buggy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rhino` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `docker-gen` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gitlint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kondo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtsensors` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `docker-gen`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `pdsh` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `qtscxml` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `gtranslator` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `supabase-mcp-server` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `pgxnclient` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `qtconnectivity` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a parallel shell, qt modules, supabase, database clients request, run dry-run first, then produce a validated artifact bundle.
