---
name: openai-report-builder
description: Use when tasks require openai report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Report Builder
...
| Primary artifact | `openai-report-bundle` |
| Routing tag | `openai:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from openai activity..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-report-bundle | structured-artifact | yes | next workflow or operator |
| openai-report-bundle-scorecard | scorecard | yes | reviewer |
| openai-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for OpenAI Report Builder, then confirm which OpenAI entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from openai activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `openai-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] openai-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:report-builder` consumers with approval and credential context attached.
