---
name: replicate-sync-orchestrator
description: Use when tasks require replicate sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Sync Orchestrator
...
| Primary artifact | `replicate-reconciliation-bundle` |
| Routing tag | `replicate:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for replicate records..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| replicate-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| replicate-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Replicate Sync Orchestrator, then checkpoint the last known good cursor on both sides of Replicate.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for replicate records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `replicate-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] replicate-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:sync-orchestrator` consumers with approval and credential context attached.
