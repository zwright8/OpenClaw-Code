...
description: Use when tasks require playwright connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `playwright-connection-trace-bundle` |
| Routing tag | `playwright:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Connector into a deterministic, auth-checked workflow for unified read/write connector for playwright apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| playwright-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| playwright-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Playwright Connector, then confirm the smallest read-only probe that proves access to Playwright.
...
3. Specify the request and response contract for unified read/write connector for playwright apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `playwright-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] playwright-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:connector` consumers with approval and credential context attached.
