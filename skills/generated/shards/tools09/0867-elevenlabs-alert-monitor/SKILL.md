---
name: elevenlabs-alert-monitor
description: Use when tasks require elevenlabs alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Alert Monitor
...
| Primary artifact | `elevenlabs-alert-state-bundle` |
| Routing tag | `elevenlabs:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Alert Monitor into a deterministic, auth-checked workflow for monitors key elevenlabs signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| elevenlabs-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| elevenlabs-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for ElevenLabs Alert Monitor, then lock the reviewed threshold set for ElevenLabs.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key elevenlabs signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `elevenlabs-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:alert-monitor` consumers with approval and credential context attached.
