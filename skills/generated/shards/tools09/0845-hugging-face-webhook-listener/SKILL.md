---
name: hugging-face-webhook-listener
description: Use when tasks require hugging face webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Webhook Listener
...
| Primary artifact | `hugging-face-event-intake-log` |
| Routing tag | `hugging-face:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Hugging Face Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes hugging face webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `HUGGING_FACE_CLIENT_ID`, `HUGGING_FACE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `HUGGING_FACE_TOKEN`, `HUGGING_FACE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `HUGGING_FACE_WEBHOOK_SECRET`, `HUGGING_FACE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| hugging-face-event-intake-log | structured-artifact | yes | next workflow or operator |
| hugging-face-event-intake-log-scorecard | scorecard | yes | reviewer |
| hugging-face-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Hugging Face Webhook Listener, then register the signing-secret policy for Hugging Face.
...
3. Implement captures, verifies, and routes hugging face webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `hugging-face-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:webhook-listener` consumers with approval and credential context attached.
