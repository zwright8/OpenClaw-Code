---
name: playwright-webhook-listener
description: Use when tasks require playwright webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Webhook Listener
...
| Primary artifact | `playwright-event-intake-log` |
| Routing tag | `playwright:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Playwright Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes playwright webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `PLAYWRIGHT_CLIENT_ID`, `PLAYWRIGHT_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `PLAYWRIGHT_TOKEN`, `PLAYWRIGHT_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `PLAYWRIGHT_WEBHOOK_SECRET`, `PLAYWRIGHT_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| playwright-event-intake-log | structured-artifact | yes | next workflow or operator |
| playwright-event-intake-log-scorecard | scorecard | yes | reviewer |
| playwright-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Playwright Webhook Listener, then register the signing-secret policy for Playwright.
...
3. Implement captures, verifies, and routes playwright webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `playwright-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] playwright-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:webhook-listener` consumers with approval and credential context attached.
