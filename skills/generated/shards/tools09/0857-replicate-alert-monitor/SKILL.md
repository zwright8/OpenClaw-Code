---
name: replicate-alert-monitor
description: Use when tasks require replicate alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Alert Monitor
...
| Primary artifact | `replicate-alert-state-bundle` |
| Routing tag | `replicate:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Alert Monitor into a deterministic, auth-checked workflow for monitors key replicate signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| replicate-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| replicate-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Replicate Alert Monitor, then lock the reviewed threshold set for Replicate.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key replicate signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `replicate-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] replicate-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:alert-monitor` consumers with approval and credential context attached.
