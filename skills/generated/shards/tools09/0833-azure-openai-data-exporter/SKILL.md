---
name: azure-openai-data-exporter
description: Use when tasks require azure openai data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Data Exporter
...
| Primary artifact | `azure-openai-export-package` |
| Routing tag | `azure-openai:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Data Exporter into a deterministic, auth-checked workflow for exports azure openai entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-export-package | structured-artifact | yes | next workflow or operator |
| azure-openai-export-package-scorecard | scorecard | yes | reviewer |
| azure-openai-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Azure OpenAI Data Exporter, then constrain the export scope for Azure OpenAI before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports azure openai entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `azure-openai-export-package` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:data-exporter` consumers with approval and credential context attached.
