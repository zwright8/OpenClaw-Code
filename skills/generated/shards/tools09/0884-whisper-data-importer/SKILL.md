---
name: whisper-data-importer
description: Use when tasks require whisper data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Data Importer
...
| Primary artifact | `whisper-import-ledger` |
| Routing tag | `whisper:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when state changes are hard to undo once they begin. This specific skill turns Whisper Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into whisper..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-import-ledger | structured-artifact | yes | next workflow or operator |
| whisper-import-ledger-scorecard | scorecard | yes | reviewer |
| whisper-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Whisper Data Importer before any Whisper write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into whisper. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `whisper-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] whisper-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:data-importer` consumers with approval and credential context attached.
