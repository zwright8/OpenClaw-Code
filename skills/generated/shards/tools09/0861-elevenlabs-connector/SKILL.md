...
description: Use when tasks require elevenlabs connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `elevenlabs-connection-trace-bundle` |
| Routing tag | `elevenlabs:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Connector into a deterministic, auth-checked workflow for unified read/write connector for elevenlabs apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| elevenlabs-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| elevenlabs-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for ElevenLabs Connector, then confirm the smallest read-only probe that proves access to ElevenLabs.
...
3. Specify the request and response contract for unified read/write connector for elevenlabs apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `elevenlabs-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:connector` consumers with approval and credential context attached.
