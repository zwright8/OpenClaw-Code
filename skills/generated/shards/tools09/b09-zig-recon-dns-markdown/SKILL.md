---
name: b09-zig-recon-dns-markdown
description: Use when managing language/runtime versions, performing recon/DNS analysis, and enforcing markdown, schema, or dependency hygiene.
---

# Zig/runtime managers, recon, DNS, and markdown pipelines

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-zig-recon-dns-markdown` |
| Tool count | `26` |
| Inventory rank span | `4262-4294` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for zig/runtime managers, recon, dns, markdown pipelines tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `zigup` | `arjun` | no | no |
| bounded mutation / transformation | `arjun` | `zigup` | yes | yes |
| validation & artifact checks | `zigup` | `arjun` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `zigup` | Download and manage zig compilers | local-installable |
| `arjun` | HTTP parameter discovery suite | local-installable |
| `drill` | HTTP load testing application written in Rust | local-installable |
| `mdformat` | CommonMark compliant Markdown formatter | local-installable |
| `libcouchbase` | C library for Couchbase | local-installable |
| `modules` | Dynamic modification of a user's environment via modulefiles | local-installable |
| `sylpheed` | Simple, lightweight email-client | local-installable |
| `clazy` | Qt oriented static code analyzer | local-installable |
| `gcc@10` | GNU compiler collection | local-installable |
| `erlang@25` | Programming language for highly scalable real-time systems | local-installable |
| `carla` | Audio plugin host supporting LADSPA, LV2, VST2/3, SF2 and more | local-installable |
| `dnsdist` | Highly DNS-, DoS- and abuse-aware loadbalancer | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `zigup` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `arjun` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `drill` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mdformat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcouchbase` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `modules` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sylpheed` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clazy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gcc@10` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `erlang@25` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `carla` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dnsdist` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gammaray` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `atop` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rmpc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kaitai-struct-compiler` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wayland-protocols` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `binsider` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxcursor` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `bench` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lasso` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `alot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fedify` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `up` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qt-mysql` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtpositioning` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `zigup` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `arjun` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `drill` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `mdformat` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `libcouchbase` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `modules` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a zig/runtime managers, recon, dns, markdown pipelines request, run dry-run first, then produce a validated artifact bundle.
