---
name: whisper-sync-orchestrator
description: Use when tasks require whisper sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Sync Orchestrator
...
| Primary artifact | `whisper-reconciliation-bundle` |
| Routing tag | `whisper:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for whisper records..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| whisper-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| whisper-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Whisper Sync Orchestrator, then checkpoint the last known good cursor on both sides of Whisper.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for whisper records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `whisper-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] whisper-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:sync-orchestrator` consumers with approval and credential context attached.
