---
name: whisper-alert-monitor
description: Use when tasks require whisper alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Alert Monitor
...
| Primary artifact | `whisper-alert-state-bundle` |
| Routing tag | `whisper:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Alert Monitor into a deterministic, auth-checked workflow for monitors key whisper signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| whisper-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| whisper-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Whisper Alert Monitor, then lock the reviewed threshold set for Whisper.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key whisper signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `whisper-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] whisper-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:alert-monitor` consumers with approval and credential context attached.
