---
name: b09-ml-voip-docs-data
description: Use when working across ML runtimes, SIP/VoIP packet capture, document transforms, vector/data services, and CLI-heavy systems triage.
---

# ML, VoIP capture, docs, and data services

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-ml-voip-docs-data` |
| Tool count | `26` |
| Inventory rank span | `4001-4032` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for ml, voip capture, docs, data services tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `pocl` | `sngrep` | no | no |
| bounded mutation / transformation | `sngrep` | `pocl` | yes | yes |
| validation & artifact checks | `pocl` | `sngrep` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `pocl` | Portable Computing Language | local-installable |
| `sngrep` | Command-line tool for displaying SIP calls message flows | local-installable |
| `pdfsandwich` | Generate sandwich OCR PDFs from scanned file | local-installable |
| `weaviate-cli` | Command-line interface for managing and interacting with Weaviate | local-installable |
| `xfig` | Facility for interactive generation of figures | local-installable |
| `dxpy` | DNAnexus toolkit utilities and platform API bindings for Python | local-installable |
| `kagent` | Kubernetes native framework for building AI agents | local-installable |
| `libxlsxwriter` | C library for creating Excel XLSX files | local-installable |
| `charmcraft` | Tool to build charms and publish them on Charmhub | local-installable |
| `vet` | Policy driven vetting of open source dependencies | local-installable |
| `sleek` | CLI tool for formatting SQL | local-installable |
| `qtvirtualkeyboard` | Provides an input framework and reference keyboard frontend | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `pocl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sngrep` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdfsandwich` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `weaviate-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xfig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dxpy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kagent` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `libxlsxwriter` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `charmcraft` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sleek` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtvirtualkeyboard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qt3d` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wavpack` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `credstash` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vapoursynth-sub` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `emqx` | MQTT, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sox_ng` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hcledit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libfreenect` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `instead` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cimg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gojq` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nyx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-expand` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dafny` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kagent`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `pocl` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `sngrep` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `pdfsandwich` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `weaviate-cli` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `xfig` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `dxpy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a ml, voip capture, docs, data services request, run dry-run first, then produce a validated artifact bundle.
