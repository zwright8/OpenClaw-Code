---
name: playwright-data-exporter
description: Use when tasks require playwright data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Data Exporter
...
| Primary artifact | `playwright-export-package` |
| Routing tag | `playwright:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Data Exporter into a deterministic, auth-checked workflow for exports playwright entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-export-package | structured-artifact | yes | next workflow or operator |
| playwright-export-package-scorecard | scorecard | yes | reviewer |
| playwright-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Playwright Data Exporter, then constrain the export scope for Playwright before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports playwright entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `playwright-export-package` so downstream owners do not need to rediscover context.
...
- [ ] playwright-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:data-exporter` consumers with approval and credential context attached.
