---
name: playwright-report-builder
description: Use when tasks require playwright report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Report Builder
...
| Primary artifact | `playwright-report-bundle` |
| Routing tag | `playwright:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from playwright activity..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-report-bundle | structured-artifact | yes | next workflow or operator |
| playwright-report-bundle-scorecard | scorecard | yes | reviewer |
| playwright-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Playwright Report Builder, then confirm which Playwright entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from playwright activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `playwright-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] playwright-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:report-builder` consumers with approval and credential context attached.
