---
name: azure-openai-report-builder
description: Use when tasks require azure openai report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Report Builder
...
| Primary artifact | `azure-openai-report-bundle` |
| Routing tag | `azure-openai:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from azure openai activity..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-report-bundle | structured-artifact | yes | next workflow or operator |
| azure-openai-report-bundle-scorecard | scorecard | yes | reviewer |
| azure-openai-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Azure OpenAI Report Builder, then confirm which Azure OpenAI entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from azure openai activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `azure-openai-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:report-builder` consumers with approval and credential context attached.
