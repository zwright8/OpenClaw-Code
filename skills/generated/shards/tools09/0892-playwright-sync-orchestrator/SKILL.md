---
name: playwright-sync-orchestrator
description: Use when tasks require playwright sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Sync Orchestrator
...
| Primary artifact | `playwright-reconciliation-bundle` |
| Routing tag | `playwright:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for playwright records..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| playwright-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| playwright-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Playwright Sync Orchestrator, then checkpoint the last known good cursor on both sides of Playwright.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for playwright records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `playwright-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] playwright-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:sync-orchestrator` consumers with approval and credential context attached.
