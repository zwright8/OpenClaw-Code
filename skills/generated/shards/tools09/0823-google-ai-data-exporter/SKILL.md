---
name: google-ai-data-exporter
description: Use when tasks require google ai data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Data Exporter
...
| Primary artifact | `google-ai-export-package` |
| Routing tag | `google-ai:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google AI Data Exporter into a deterministic, auth-checked workflow for exports google ai entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-export-package | structured-artifact | yes | next workflow or operator |
| google-ai-export-package-scorecard | scorecard | yes | reviewer |
| google-ai-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Google AI Data Exporter, then constrain the export scope for Google AI before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports google ai entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `google-ai-export-package` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:data-exporter` consumers with approval and credential context attached.
