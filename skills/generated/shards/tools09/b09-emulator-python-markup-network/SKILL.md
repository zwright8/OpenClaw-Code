...
| Field | Value |
|---|---|
| Skill pack | `b09-emulator-python-markup-network` |
| Tool count | `26` |
| Inventory rank span | `4201-4229` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for emulation, python packaging, markup, network tools tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `openmsx` | `selene` | no | no |
| bounded mutation / transformation | `selene` | `openmsx` | yes | yes |
| validation & artifact checks | `openmsx` | `selene` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `openmsx` | MSX emulator | local-installable |
| `selene` | Blazing-fast modern Lua linter | local-installable |
| `flit` | Simplified packaging of Python modules | local-installable |
| `html2markdown` | Convert HTML to Markdown | local-installable |
| `gtk-gnutella` | Share files in a peer-to-peer (P2P) network | local-installable |
| `fcgi` | Protocol for interfacing interactive programs with a web server | local-installable |
| `latex2html` | LaTeX-to-HTML translator | local-installable |
| `nmstatectl` | Command-line tool that manages host networking settings in a declarative manner | local-installable |
| `gnome-autoar` | GNOME library for archive handling | local-installable |
| `libxmu` | X.Org: X miscellaneous utility routines library | local-installable |
| `ansible-cmdb` | Generates static HTML overview page from Ansible facts | local-installable |
| `scotch` | Package for graph partitioning, graph clustering, and sparse matrix ordering | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `openmsx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `selene` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `flit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `html2markdown` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gtk-gnutella` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fcgi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `latex2html` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nmstatectl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnome-autoar` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `libxmu` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `ansible-cmdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scotch` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `leela-zero` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `oauth2_proxy` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gau` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libmodplug` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kamel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lilv` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `nudoku` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qbittorrent-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `showkey` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `stencil` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chronograf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `icoutils` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnustep-base` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `wifi-password` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `oauth2_proxy`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `openmsx` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `selene` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `flit` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `html2markdown` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `gtk-gnutella` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `fcgi` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a emulation, python packaging, markup, network tools request, run dry-run first, then produce a validated artifact bundle.
