---
name: playwright-approval-router
description: Use when tasks require playwright approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Approval Router
...
| Primary artifact | `playwright-approval-decision-packet` |
| Routing tag | `playwright:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Approval Router into a deterministic, auth-checked workflow for routes playwright-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| playwright-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| playwright-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Playwright Approval Router, then capture the manual fallback path for Playwright.
...
3. Implement routes playwright-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `playwright-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] playwright-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:approval-router` consumers with approval and credential context attached.
