---
name: anthropic-approval-router
description: Use when tasks require anthropic approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Approval Router
...
| Primary artifact | `anthropic-approval-decision-packet` |
| Routing tag | `anthropic:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Approval Router into a deterministic, auth-checked workflow for routes anthropic-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| anthropic-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| anthropic-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Anthropic Approval Router, then capture the manual fallback path for Anthropic.
...
3. Implement routes anthropic-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `anthropic-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:approval-router` consumers with approval and credential context attached.
