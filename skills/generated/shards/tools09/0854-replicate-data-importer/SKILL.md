---
name: replicate-data-importer
description: Use when tasks require replicate data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Data Importer
...
| Primary artifact | `replicate-import-ledger` |
| Routing tag | `replicate:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when state changes are hard to undo once they begin. This specific skill turns Replicate Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into replicate..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-import-ledger | structured-artifact | yes | next workflow or operator |
| replicate-import-ledger-scorecard | scorecard | yes | reviewer |
| replicate-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Replicate Data Importer before any Replicate write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into replicate. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `replicate-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] replicate-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:data-importer` consumers with approval and credential context attached.
