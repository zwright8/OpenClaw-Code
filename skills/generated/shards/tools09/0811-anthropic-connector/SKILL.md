...
description: Use when tasks require anthropic connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `anthropic-connection-trace-bundle` |
| Routing tag | `anthropic:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Connector into a deterministic, auth-checked workflow for unified read/write connector for anthropic apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| anthropic-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| anthropic-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Anthropic Connector, then confirm the smallest read-only probe that proves access to Anthropic.
...
3. Specify the request and response contract for unified read/write connector for anthropic apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `anthropic-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:connector` consumers with approval and credential context attached.
