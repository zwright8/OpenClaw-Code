---
name: hugging-face-alert-monitor
description: Use when tasks require hugging face alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Alert Monitor
...
| Primary artifact | `hugging-face-alert-state-bundle` |
| Routing tag | `hugging-face:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Alert Monitor into a deterministic, auth-checked workflow for monitors key hugging face signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| hugging-face-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| hugging-face-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Hugging Face Alert Monitor, then lock the reviewed threshold set for Hugging Face.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key hugging face signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `hugging-face-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:alert-monitor` consumers with approval and credential context attached.
