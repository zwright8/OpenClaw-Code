---
name: whisper-connector
description: Use when tasks require whisper connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `whisper-connection-trace-bundle` |
| Routing tag | `whisper:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Connector into a deterministic, auth-checked workflow for unified read/write connector for whisper apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| whisper-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| whisper-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Whisper Connector, then confirm the smallest read-only probe that proves access to Whisper.
...
3. Specify the request and response contract for unified read/write connector for whisper apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `whisper-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] whisper-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:connector` consumers with approval and credential context attached.
