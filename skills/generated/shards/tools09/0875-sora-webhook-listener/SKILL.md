---
name: sora-webhook-listener
description: Use when tasks require sora webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sora Webhook Listener
...
| Primary artifact | `sora-event-intake-log` |
| Routing tag | `sora:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Sora Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes sora webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SORA_CLIENT_ID`, `SORA_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SORA_TOKEN`, `SORA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SORA_WEBHOOK_SECRET`, `SORA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| sora-event-intake-log | structured-artifact | yes | next workflow or operator |
| sora-event-intake-log-scorecard | scorecard | yes | reviewer |
| sora-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Sora Webhook Listener, then register the signing-secret policy for Sora.
...
3. Implement captures, verifies, and routes sora webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `sora-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] sora-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:webhook-listener` consumers with approval and credential context attached.
