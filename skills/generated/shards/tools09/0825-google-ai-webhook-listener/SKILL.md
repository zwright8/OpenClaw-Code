---
name: google-ai-webhook-listener
description: Use when tasks require google ai webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Webhook Listener
...
| Primary artifact | `google-ai-event-intake-log` |
| Routing tag | `google-ai:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Google AI Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes google ai webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `GOOGLE_AI_CLIENT_ID`, `GOOGLE_AI_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `GOOGLE_AI_TOKEN`, `GOOGLE_AI_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GOOGLE_AI_WEBHOOK_SECRET`, `GOOGLE_AI_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| google-ai-event-intake-log | structured-artifact | yes | next workflow or operator |
| google-ai-event-intake-log-scorecard | scorecard | yes | reviewer |
| google-ai-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Google AI Webhook Listener, then register the signing-secret policy for Google AI.
...
3. Implement captures, verifies, and routes google ai webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `google-ai-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:webhook-listener` consumers with approval and credential context attached.
