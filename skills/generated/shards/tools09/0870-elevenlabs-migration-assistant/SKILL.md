---
name: elevenlabs-migration-assistant
description: Use when tasks require elevenlabs migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Migration Assistant
...
| Primary artifact | `elevenlabs-migration-cutover-plan` |
| Routing tag | `elevenlabs:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when state changes are hard to undo once they begin. This specific skill turns ElevenLabs Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from elevenlabs..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| elevenlabs-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| elevenlabs-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for ElevenLabs Migration Assistant before any ElevenLabs rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from elevenlabs. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `elevenlabs-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:migration-assistant` consumers with approval and credential context attached.
