...
| Field | Value |
|---|---|
| Skill pack | `b09-benchmark-messaging-containers-crypto` |
| Tool count | `26` |
| Inventory rank span | `4064-4095` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for benchmarking, messaging, containers, crypto ops tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `phoronix-test-suite` | `tdlib` | no | no |
| bounded mutation / transformation | `tdlib` | `phoronix-test-suite` | yes | yes |
| validation & artifact checks | `phoronix-test-suite` | `tdlib` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `phoronix-test-suite` | Open-source automated testing/benchmarking software | local-installable |
| `tdlib` | Cross-platform library for building Telegram clients | local-installable |
| `diffoci` | Diff for Docker and OCI container images | local-installable |
| `popt` | Library like getopt(3) with a number of enhancements | local-installable |
| `dmtx-utils` | Read and write data matrix barcodes | local-installable |
| `csview` | High performance csv viewer for cli | local-installable |
| `packmol` | Packing optimization for molecular dynamics simulations | local-installable |
| `sparse` | Static C code analysis tool | local-installable |
| `sendme` | Tool to send files and directories, based on iroh | local-installable |
| `libvidstab` | Transcode video stabilization plugin | local-installable |
| `cargo-zigbuild` | Compile Cargo project with zig as linker | local-installable |
| `yubikey-agent` | Seamless ssh-agent for YubiKeys and other PIV tokens | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `phoronix-test-suite` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tdlib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `diffoci` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `popt` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `dmtx-utils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `csview` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `packmol` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sparse` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sendme` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libvidstab` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-zigbuild` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yubikey-agent` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xml-coreutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swagger-codegen@2` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `coal` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `spirv-headers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pbzip2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `termshark` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clair` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `unisonlang` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-shear` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jena` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mqttui` | MQTT, QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `thanos` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xtrans` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnupg@1.4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `diffoci`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `phoronix-test-suite` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `tdlib` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `diffoci` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `popt` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `dmtx-utils` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `csview` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a benchmarking, messaging, containers, crypto ops request, run dry-run first, then produce a validated artifact bundle.
