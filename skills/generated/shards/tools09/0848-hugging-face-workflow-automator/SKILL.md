---
name: hugging-face-workflow-automator
description: Use when tasks require hugging face workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Workflow Automator
...
| Primary artifact | `hugging-face-workflow-runbook` |
| Routing tag | `hugging-face:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using hugging face..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-workflow-runbook | structured-artifact | yes | next workflow or operator |
| hugging-face-workflow-runbook-scorecard | scorecard | yes | reviewer |
| hugging-face-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Hugging Face Workflow Automator, then checkpoint where Hugging Face state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using hugging face. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `hugging-face-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:workflow-automator` consumers with approval and credential context attached.
