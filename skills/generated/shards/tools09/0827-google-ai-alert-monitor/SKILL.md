---
name: google-ai-alert-monitor
description: Use when tasks require google ai alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Alert Monitor
...
| Primary artifact | `google-ai-alert-state-bundle` |
| Routing tag | `google-ai:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google AI Alert Monitor into a deterministic, auth-checked workflow for monitors key google ai signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| google-ai-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| google-ai-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Google AI Alert Monitor, then lock the reviewed threshold set for Google AI.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key google ai signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `google-ai-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:alert-monitor` consumers with approval and credential context attached.
