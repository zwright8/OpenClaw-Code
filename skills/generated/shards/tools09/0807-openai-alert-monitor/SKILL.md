---
name: openai-alert-monitor
description: Use when tasks require openai alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Alert Monitor
...
| Primary artifact | `openai-alert-state-bundle` |
| Routing tag | `openai:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Alert Monitor into a deterministic, auth-checked workflow for monitors key openai signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| openai-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| openai-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for OpenAI Alert Monitor, then lock the reviewed threshold set for OpenAI.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key openai signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `openai-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] openai-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:alert-monitor` consumers with approval and credential context attached.
