---
name: replicate-webhook-listener
description: Use when tasks require replicate webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Webhook Listener
...
| Primary artifact | `replicate-event-intake-log` |
| Routing tag | `replicate:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Replicate Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes replicate webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `REPLICATE_CLIENT_ID`, `REPLICATE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `REPLICATE_TOKEN`, `REPLICATE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `REPLICATE_WEBHOOK_SECRET`, `REPLICATE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| replicate-event-intake-log | structured-artifact | yes | next workflow or operator |
| replicate-event-intake-log-scorecard | scorecard | yes | reviewer |
| replicate-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Replicate Webhook Listener, then register the signing-secret policy for Replicate.
...
3. Implement captures, verifies, and routes replicate webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `replicate-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] replicate-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:webhook-listener` consumers with approval and credential context attached.
