---
name: anthropic-webhook-listener
description: Use when tasks require anthropic webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Webhook Listener
...
| Primary artifact | `anthropic-event-intake-log` |
| Routing tag | `anthropic:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Anthropic Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes anthropic webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `ANTHROPIC_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Webhook signing secret | `signing_secret` | `ANTHROPIC_WEBHOOK_SECRET`, `ANTHROPIC_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `ANTHROPIC_TOKEN`, `ANTHROPIC_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| anthropic-event-intake-log | structured-artifact | yes | next workflow or operator |
| anthropic-event-intake-log-scorecard | scorecard | yes | reviewer |
| anthropic-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Anthropic Webhook Listener, then register the signing-secret policy for Anthropic.
...
3. Implement captures, verifies, and routes anthropic webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `anthropic-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:webhook-listener` consumers with approval and credential context attached.
