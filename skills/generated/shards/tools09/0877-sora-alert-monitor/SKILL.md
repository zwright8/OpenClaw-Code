---
name: sora-alert-monitor
description: Use when tasks require sora alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sora-alert-state-bundle` |
| Routing tag | `sora:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Alert Monitor into a deterministic, auth-checked workflow for monitors key sora signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| sora-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| sora-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Sora Alert Monitor, then lock the reviewed threshold set for Sora.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key sora signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `sora-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sora-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:alert-monitor` consumers with approval and credential context attached.
