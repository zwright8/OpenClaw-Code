---
name: anthropic-workflow-automator
description: Use when tasks require anthropic workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Workflow Automator
...
| Primary artifact | `anthropic-workflow-runbook` |
| Routing tag | `anthropic:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using anthropic..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-workflow-runbook | structured-artifact | yes | next workflow or operator |
| anthropic-workflow-runbook-scorecard | scorecard | yes | reviewer |
| anthropic-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Anthropic Workflow Automator, then checkpoint where Anthropic state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using anthropic. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `anthropic-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:workflow-automator` consumers with approval and credential context attached.
