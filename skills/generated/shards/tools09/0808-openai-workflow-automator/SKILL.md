---
name: openai-workflow-automator
description: Use when tasks require openai workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `openai-workflow-runbook` |
| Routing tag | `openai:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using openai..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-workflow-runbook | structured-artifact | yes | next workflow or operator |
| openai-workflow-runbook-scorecard | scorecard | yes | reviewer |
| openai-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for OpenAI Workflow Automator, then checkpoint where OpenAI state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using openai. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `openai-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] openai-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:workflow-automator` consumers with approval and credential context attached.
