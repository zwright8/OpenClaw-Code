---
name: elevenlabs-report-builder
description: Use when tasks require elevenlabs report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Report Builder
...
| Primary artifact | `elevenlabs-report-bundle` |
| Routing tag | `elevenlabs:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from elevenlabs activity..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-report-bundle | structured-artifact | yes | next workflow or operator |
| elevenlabs-report-bundle-scorecard | scorecard | yes | reviewer |
| elevenlabs-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for ElevenLabs Report Builder, then confirm which ElevenLabs entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from elevenlabs activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `elevenlabs-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:report-builder` consumers with approval and credential context attached.
