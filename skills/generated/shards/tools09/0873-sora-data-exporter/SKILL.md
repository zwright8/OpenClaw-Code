---
name: sora-data-exporter
description: Use when tasks require sora data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sora-export-package` |
| Routing tag | `sora:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Data Exporter into a deterministic, auth-checked workflow for exports sora entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-export-package | structured-artifact | yes | next workflow or operator |
| sora-export-package-scorecard | scorecard | yes | reviewer |
| sora-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Sora Data Exporter, then constrain the export scope for Sora before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports sora entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `sora-export-package` so downstream owners do not need to rediscover context.
...
- [ ] sora-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:data-exporter` consumers with approval and credential context attached.
