---
name: whisper-workflow-automator
description: Use when tasks require whisper workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Workflow Automator
...
| Primary artifact | `whisper-workflow-runbook` |
| Routing tag | `whisper:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using whisper..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-workflow-runbook | structured-artifact | yes | next workflow or operator |
| whisper-workflow-runbook-scorecard | scorecard | yes | reviewer |
| whisper-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Whisper Workflow Automator, then checkpoint where Whisper state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using whisper. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `whisper-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] whisper-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:workflow-automator` consumers with approval and credential context attached.
