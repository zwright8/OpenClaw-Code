---
name: playwright-data-importer
description: Use when tasks require playwright data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Data Importer
...
| Primary artifact | `playwright-import-ledger` |
| Routing tag | `playwright:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when state changes are hard to undo once they begin. This specific skill turns Playwright Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into playwright..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-import-ledger | structured-artifact | yes | next workflow or operator |
| playwright-import-ledger-scorecard | scorecard | yes | reviewer |
| playwright-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Playwright Data Importer before any Playwright write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into playwright. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `playwright-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] playwright-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:data-importer` consumers with approval and credential context attached.
