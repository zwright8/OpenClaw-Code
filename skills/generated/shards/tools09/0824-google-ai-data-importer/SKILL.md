---
name: google-ai-data-importer
description: Use when tasks require google ai data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Data Importer
...
| Primary artifact | `google-ai-import-ledger` |
| Routing tag | `google-ai:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google AI Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into google ai..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-import-ledger | structured-artifact | yes | next workflow or operator |
| google-ai-import-ledger-scorecard | scorecard | yes | reviewer |
| google-ai-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Google AI Data Importer before any Google AI write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into google ai. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `google-ai-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:data-importer` consumers with approval and credential context attached.
