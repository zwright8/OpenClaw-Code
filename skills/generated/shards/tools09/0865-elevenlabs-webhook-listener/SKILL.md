---
name: elevenlabs-webhook-listener
description: Use when tasks require elevenlabs webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Webhook Listener
...
| Primary artifact | `elevenlabs-event-intake-log` |
| Routing tag | `elevenlabs:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns ElevenLabs Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes elevenlabs webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `ELEVENLABS_CLIENT_ID`, `ELEVENLABS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `ELEVENLABS_TOKEN`, `ELEVENLABS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `ELEVENLABS_WEBHOOK_SECRET`, `ELEVENLABS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| elevenlabs-event-intake-log | structured-artifact | yes | next workflow or operator |
| elevenlabs-event-intake-log-scorecard | scorecard | yes | reviewer |
| elevenlabs-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for ElevenLabs Webhook Listener, then register the signing-secret policy for ElevenLabs.
...
3. Implement captures, verifies, and routes elevenlabs webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `elevenlabs-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:webhook-listener` consumers with approval and credential context attached.
