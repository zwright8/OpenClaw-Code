---
name: hugging-face-sync-orchestrator
description: Use when tasks require hugging face sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Sync Orchestrator
...
| Primary artifact | `hugging-face-reconciliation-bundle` |
| Routing tag | `hugging-face:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for hugging face records..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| hugging-face-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| hugging-face-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Hugging Face Sync Orchestrator, then checkpoint the last known good cursor on both sides of Hugging Face.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for hugging face records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `hugging-face-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:sync-orchestrator` consumers with approval and credential context attached.
