---
name: whisper-approval-router
description: Use when tasks require whisper approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Approval Router
...
| Primary artifact | `whisper-approval-decision-packet` |
| Routing tag | `whisper:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Approval Router into a deterministic, auth-checked workflow for routes whisper-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| whisper-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| whisper-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Whisper Approval Router, then capture the manual fallback path for Whisper.
...
3. Implement routes whisper-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `whisper-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] whisper-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:approval-router` consumers with approval and credential context attached.
