---
name: hugging-face-report-builder
description: Use when tasks require hugging face report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Report Builder
...
| Primary artifact | `hugging-face-report-bundle` |
| Routing tag | `hugging-face:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from hugging face activity..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-report-bundle | structured-artifact | yes | next workflow or operator |
| hugging-face-report-bundle-scorecard | scorecard | yes | reviewer |
| hugging-face-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Hugging Face Report Builder, then confirm which Hugging Face entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from hugging face activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `hugging-face-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:report-builder` consumers with approval and credential context attached.
