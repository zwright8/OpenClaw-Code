---
name: whisper-report-builder
description: Use when tasks require whisper report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Report Builder
...
| Primary artifact | `whisper-report-bundle` |
| Routing tag | `whisper:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from whisper activity..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-report-bundle | structured-artifact | yes | next workflow or operator |
| whisper-report-bundle-scorecard | scorecard | yes | reviewer |
| whisper-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Whisper Report Builder, then confirm which Whisper entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from whisper activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `whisper-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] whisper-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:report-builder` consumers with approval and credential context attached.
