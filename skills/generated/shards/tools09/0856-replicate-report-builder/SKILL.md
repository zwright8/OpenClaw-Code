---
name: replicate-report-builder
description: Use when tasks require replicate report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Report Builder
...
| Primary artifact | `replicate-report-bundle` |
| Routing tag | `replicate:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from replicate activity..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-report-bundle | structured-artifact | yes | next workflow or operator |
| replicate-report-bundle-scorecard | scorecard | yes | reviewer |
| replicate-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Replicate Report Builder, then confirm which Replicate entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from replicate activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `replicate-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] replicate-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:report-builder` consumers with approval and credential context attached.
