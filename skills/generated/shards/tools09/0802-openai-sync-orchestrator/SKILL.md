---
name: openai-sync-orchestrator
description: Use when tasks require openai sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Sync Orchestrator
...
| Primary artifact | `openai-reconciliation-bundle` |
| Routing tag | `openai:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for openai records..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| openai-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| openai-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for OpenAI Sync Orchestrator, then checkpoint the last known good cursor on both sides of OpenAI.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for openai records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `openai-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] openai-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:sync-orchestrator` consumers with approval and credential context attached.
