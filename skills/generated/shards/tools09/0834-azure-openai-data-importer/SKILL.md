---
name: azure-openai-data-importer
description: Use when tasks require azure openai data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Data Importer
...
| Primary artifact | `azure-openai-import-ledger` |
| Routing tag | `azure-openai:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when state changes are hard to undo once they begin. This specific skill turns Azure OpenAI Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into azure openai..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-import-ledger | structured-artifact | yes | next workflow or operator |
| azure-openai-import-ledger-scorecard | scorecard | yes | reviewer |
| azure-openai-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Azure OpenAI Data Importer before any Azure OpenAI write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into azure openai. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `azure-openai-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:data-importer` consumers with approval and credential context attached.
