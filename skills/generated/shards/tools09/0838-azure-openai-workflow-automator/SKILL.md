---
name: azure-openai-workflow-automator
description: Use when tasks require azure openai workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Workflow Automator
...
| Primary artifact | `azure-openai-workflow-runbook` |
| Routing tag | `azure-openai:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using azure openai..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-workflow-runbook | structured-artifact | yes | next workflow or operator |
| azure-openai-workflow-runbook-scorecard | scorecard | yes | reviewer |
| azure-openai-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Azure OpenAI Workflow Automator, then checkpoint where Azure OpenAI state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using azure openai. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `azure-openai-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:workflow-automator` consumers with approval and credential context attached.
