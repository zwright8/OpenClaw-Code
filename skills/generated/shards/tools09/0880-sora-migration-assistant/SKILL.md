---
name: sora-migration-assistant
description: Use when tasks require sora migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sora Migration Assistant
...
| Primary artifact | `sora-migration-cutover-plan` |
| Routing tag | `sora:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sora Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from sora..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| sora-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| sora-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Sora Migration Assistant before any Sora rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from sora. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `sora-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] sora-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:migration-assistant` consumers with approval and credential context attached.
