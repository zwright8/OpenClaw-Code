---
name: openai-webhook-listener
description: Use when tasks require openai webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Webhook Listener
...
| Primary artifact | `openai-event-intake-log` |
| Routing tag | `openai:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns OpenAI Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes openai webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `OPENAI_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Webhook signing secret | `signing_secret` | `OPENAI_WEBHOOK_SECRET`, `OPENAI_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `OPENAI_TOKEN`, `OPENAI_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| openai-event-intake-log | structured-artifact | yes | next workflow or operator |
| openai-event-intake-log-scorecard | scorecard | yes | reviewer |
| openai-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for OpenAI Webhook Listener, then register the signing-secret policy for OpenAI.
...
3. Implement captures, verifies, and routes openai webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `openai-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] openai-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:webhook-listener` consumers with approval and credential context attached.
