---
name: replicate-workflow-automator
description: Use when tasks require replicate workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Workflow Automator
...
| Primary artifact | `replicate-workflow-runbook` |
| Routing tag | `replicate:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using replicate..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-workflow-runbook | structured-artifact | yes | next workflow or operator |
| replicate-workflow-runbook-scorecard | scorecard | yes | reviewer |
| replicate-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Replicate Workflow Automator, then checkpoint where Replicate state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using replicate. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `replicate-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] replicate-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:workflow-automator` consumers with approval and credential context attached.
