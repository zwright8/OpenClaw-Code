---
name: b09-intel-network-policy-quant
description: Use when executing reconnaissance, network/security policy tooling, finance/quant libraries, and mixed governance or compliance automation.
---

# Intel/recon telemetry, policy controls, and quantitative stacks

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-intel-network-policy-quant` |
| Tool count | `26` |
| Inventory rank span | `4390-4421` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for intel/recon telemetry, policy controls, quantitative stacks tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `censys` | `remarshal` | no | no |
| bounded mutation / transformation | `remarshal` | `censys` | yes | yes |
| validation & artifact checks | `censys` | `remarshal` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `censys` | Command-line interface for the Censys APIs (censys.io) | local-installable |
| `remarshal` | Convert between TOML, YAML and JSON | local-installable |
| `libsvg` | Library for SVG files | local-installable |
| `codesnap` | Generates code snapshots in various formats | local-installable |
| `kea` | DHCP server | local-installable |
| `klavaro` | Free touch typing tutor program | local-installable |
| `addons-linter` | Firefox Add-ons linter, written in JavaScript | local-installable |
| `gyb` | CLI for backing up and restoring Gmail messages | local-installable |
| `qtlocation` | Provides C++ interfaces to retrieve location and navigational information | local-installable |
| `qtlottie` | Display graphics and animations exported by the Bodymovin plugin | local-installable |
| `qcli` | Report audiovisual metrics via libavfilter | local-installable |
| `tmt` | Test Management Tool | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `censys` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `remarshal` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsvg` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `codesnap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kea` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `klavaro` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `addons-linter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gyb` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtlocation` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtlottie` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dockerfilegraph` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `znc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rattler-index` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `boring` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `leapp-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gwenhywfar` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `html-xml-utils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `elm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nyan` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bbftp-client` | SFTP/FTP/WebDAV, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gforth` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qthttpserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `quantlib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `censys` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `remarshal` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `libsvg` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `codesnap` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `kea` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `klavaro` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a intel/recon telemetry, policy controls, quantitative stacks request, run dry-run first, then produce a validated artifact bundle.
