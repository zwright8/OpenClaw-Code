---
name: sora-report-builder
description: Use when tasks require sora report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sora-report-bundle` |
| Routing tag | `sora:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from sora activity..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-report-bundle | structured-artifact | yes | next workflow or operator |
| sora-report-bundle-scorecard | scorecard | yes | reviewer |
| sora-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Sora Report Builder, then confirm which Sora entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from sora activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `sora-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sora-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:report-builder` consumers with approval and credential context attached.
