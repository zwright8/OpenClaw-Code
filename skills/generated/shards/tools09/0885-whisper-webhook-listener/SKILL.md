---
name: whisper-webhook-listener
description: Use when tasks require whisper webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Webhook Listener
...
| Primary artifact | `whisper-event-intake-log` |
| Routing tag | `whisper:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Whisper Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes whisper webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `WHISPER_CLIENT_ID`, `WHISPER_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `WHISPER_TOKEN`, `WHISPER_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `WHISPER_WEBHOOK_SECRET`, `WHISPER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| whisper-event-intake-log | structured-artifact | yes | next workflow or operator |
| whisper-event-intake-log-scorecard | scorecard | yes | reviewer |
| whisper-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Whisper Webhook Listener, then register the signing-secret policy for Whisper.
...
3. Implement captures, verifies, and routes whisper webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `whisper-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] whisper-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:webhook-listener` consumers with approval and credential context attached.
