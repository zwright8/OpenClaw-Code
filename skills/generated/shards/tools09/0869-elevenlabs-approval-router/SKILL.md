---
name: elevenlabs-approval-router
description: Use when tasks require elevenlabs approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Approval Router
...
| Primary artifact | `elevenlabs-approval-decision-packet` |
| Routing tag | `elevenlabs:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Approval Router into a deterministic, auth-checked workflow for routes elevenlabs-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| elevenlabs-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| elevenlabs-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for ElevenLabs Approval Router, then capture the manual fallback path for ElevenLabs.
...
3. Implement routes elevenlabs-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `elevenlabs-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:approval-router` consumers with approval and credential context attached.
