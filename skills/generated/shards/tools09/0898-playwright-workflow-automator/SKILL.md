---
name: playwright-workflow-automator
description: Use when tasks require playwright workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Workflow Automator
...
| Primary artifact | `playwright-workflow-runbook` |
| Routing tag | `playwright:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using playwright..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-workflow-runbook | structured-artifact | yes | next workflow or operator |
| playwright-workflow-runbook-scorecard | scorecard | yes | reviewer |
| playwright-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Playwright Workflow Automator, then checkpoint where Playwright state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using playwright. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `playwright-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] playwright-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:workflow-automator` consumers with approval and credential context attached.
