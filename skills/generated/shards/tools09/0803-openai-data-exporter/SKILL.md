---
name: openai-data-exporter
description: Use when tasks require openai data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `openai-export-package` |
| Routing tag | `openai:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Data Exporter into a deterministic, auth-checked workflow for exports openai entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-export-package | structured-artifact | yes | next workflow or operator |
| openai-export-package-scorecard | scorecard | yes | reviewer |
| openai-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for OpenAI Data Exporter, then constrain the export scope for OpenAI before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports openai entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `openai-export-package` so downstream owners do not need to rediscover context.
...
- [ ] openai-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:data-exporter` consumers with approval and credential context attached.
