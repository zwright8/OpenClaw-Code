...
| Field | Value |
|---|---|
| Skill pack | `b09-typesetting-fleet-runtime-web` |
| Tool count | `26` |
| Inventory rank span | `4170-4200` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for typesetting, fleet control, runtime, web services tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `sile` | `fleet-cli` | no | no |
| bounded mutation / transformation | `fleet-cli` | `sile` | yes | yes |
| validation & artifact checks | `sile` | `fleet-cli` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `sile` | Modern typesetting system inspired by TeX | local-installable |
| `fleet-cli` | Manage large fleets of Kubernetes clusters | local-installable |
| `primesieve` | Fast C/C++ prime number generator | local-installable |
| `ata` | ChatGPT in the terminal | local-installable |
| `drogon` | Modern C++ web application framework | local-installable |
| `humanlog` | Logs for humans to read | local-installable |
| `ipinfo-cli` | Official CLI for the IPinfo IP Address API | local-installable |
| `kraken2` | Taxonomic sequence classification system | local-installable |
| `plplot` | Cross-platform software package for creating scientific plots | local-installable |
| `libxinerama` | X.Org: API for Xinerama extension to X11 Protocol | local-installable |
| `wiremock-standalone` | Simulator for HTTP-based APIs | local-installable |
| `gtkspell3` | Gtk widget for highlighting and replacing misspelled words | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `sile` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fleet-cli` | HTTPS/REST, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `primesieve` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ata` | HTTPS/REST, CLI/stdin-stdout | API key | yes | yes | Check `OPENAI_API_KEY` first and validate auth with a lightweight request. Ask user for the OpenAI/ChatGPT provider API key only if missing/invalid/expired. |
| `drogon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `humanlog` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ipinfo-cli` | HTTPS/REST, CLI/stdin-stdout | API key | yes | yes | Check `IPINFO_TOKEN` first and validate auth with a lightweight request. Ask user for the IPinfo API key only if missing/invalid/expired. |
| `kraken2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `plplot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxinerama` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wiremock-standalone` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gtkspell3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `makedepend` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lockrun` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `certigo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fs-uae` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mpremote` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `anyquery` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnome-papers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudfoundry-cli` | HTTPS/REST, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `libgda` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kbld` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clojurescript` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hashlink` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtlanguageserver` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `multimarkdown` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `ata`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `cloudfoundry-cli`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `fleet-cli`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `ipinfo-cli`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. Select a minimal tool from this pack to solve a typesetting, fleet control, runtime, web services request, run dry-run first, then produce a validated artifact bundle.
