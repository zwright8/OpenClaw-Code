---
name: sora-sync-orchestrator
description: Use when tasks require sora sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sora Sync Orchestrator
...
| Primary artifact | `sora-reconciliation-bundle` |
| Routing tag | `sora:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for sora records..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| sora-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| sora-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Sora Sync Orchestrator, then checkpoint the last known good cursor on both sides of Sora.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for sora records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `sora-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sora-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:sync-orchestrator` consumers with approval and credential context attached.
