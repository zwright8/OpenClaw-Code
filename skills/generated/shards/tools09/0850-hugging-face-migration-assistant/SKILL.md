---
name: hugging-face-migration-assistant
description: Use when tasks require hugging face migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Migration Assistant
...
| Primary artifact | `hugging-face-migration-cutover-plan` |
| Routing tag | `hugging-face:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when state changes are hard to undo once they begin. This specific skill turns Hugging Face Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from hugging face..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| hugging-face-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| hugging-face-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Hugging Face Migration Assistant before any Hugging Face rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from hugging face. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `hugging-face-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:migration-assistant` consumers with approval and credential context attached.
