---
name: b09-governance-k8s-docs-observability
description: Use when enforcing code standards, managing Kubernetes configs, generating docs/manpages, and operating observability or media ingestion utilities.
---

# Governance, Kubernetes config, docs tooling, and observability

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-governance-k8s-docs-observability` |
| Tool count | `26` |
| Inventory rank span | `4423-4458` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for governance, kubernetes config, docs tooling, observability tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `go-critic` | `man2html` | no | no |
| bounded mutation / transformation | `man2html` | `go-critic` | yes | yes |
| validation & artifact checks | `go-critic` | `man2html` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `go-critic` | Opinionated Go source code linter | local-installable |
| `man2html` | Convert nroff man pages to HTML | local-installable |
| `standardebooks` | Tools for producing ebook files | local-installable |
| `screenpipe` | Library to build personalized AI powered by what you've seen, said, or heard | local-installable |
| `kubecfg` | Manage complex enterprise Kubernetes environments as code | local-installable |
| `amfora` | Fancy terminal browser for the Gemini protocol | local-installable |
| `joshuto` | Ranger-like terminal file manager written in Rust | local-installable |
| `aws-c-http` | C99 implementation of the HTTP/1.1 and HTTP/2 specifications | local-installable |
| `c` | Compile and execute C "scripts" in one go | local-installable |
| `so` | Terminal interface for StackOverflow | local-installable |
| `docmd` | Minimal Markdown documentation generator | local-installable |
| `xtensor` | Multi-dimensional arrays with broadcasting and lazy computing | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `go-critic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `man2html` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `standardebooks` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `screenpipe` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kubecfg` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `amfora` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `joshuto` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-c-http` | HTTP/2, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `c` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `so` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `docmd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xtensor` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `beancount-language-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libserialport` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `goocanvas` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `rdap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `direwolf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `liboqs` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `fobis` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libharu` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `keepassc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `git-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnu-chess` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cbmc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `martin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scdoc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-c-http`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `kubecfg`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `go-critic` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `man2html` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `standardebooks` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `screenpipe` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `kubecfg` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `amfora` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a governance, kubernetes config, docs tooling, observability request, run dry-run first, then produce a validated artifact bundle.
