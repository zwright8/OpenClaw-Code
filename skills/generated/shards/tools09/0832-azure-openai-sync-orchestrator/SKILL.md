---
name: azure-openai-sync-orchestrator
description: Use when tasks require azure openai sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Sync Orchestrator
...
| Primary artifact | `azure-openai-reconciliation-bundle` |
| Routing tag | `azure-openai:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for azure openai records..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| azure-openai-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| azure-openai-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Azure OpenAI Sync Orchestrator, then checkpoint the last known good cursor on both sides of Azure OpenAI.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for azure openai records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `azure-openai-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:sync-orchestrator` consumers with approval and credential context attached.
