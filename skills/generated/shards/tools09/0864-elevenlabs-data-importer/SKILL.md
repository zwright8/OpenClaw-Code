---
name: elevenlabs-data-importer
description: Use when tasks require elevenlabs data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Data Importer
...
| Primary artifact | `elevenlabs-import-ledger` |
| Routing tag | `elevenlabs:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when state changes are hard to undo once they begin. This specific skill turns ElevenLabs Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into elevenlabs..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-import-ledger | structured-artifact | yes | next workflow or operator |
| elevenlabs-import-ledger-scorecard | scorecard | yes | reviewer |
| elevenlabs-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for ElevenLabs Data Importer before any ElevenLabs write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into elevenlabs. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `elevenlabs-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:data-importer` consumers with approval and credential context attached.
