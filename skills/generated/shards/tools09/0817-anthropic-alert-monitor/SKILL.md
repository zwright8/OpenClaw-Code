---
name: anthropic-alert-monitor
description: Use when tasks require anthropic alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Alert Monitor
...
| Primary artifact | `anthropic-alert-state-bundle` |
| Routing tag | `anthropic:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Alert Monitor into a deterministic, auth-checked workflow for monitors key anthropic signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| anthropic-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| anthropic-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Anthropic Alert Monitor, then lock the reviewed threshold set for Anthropic.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key anthropic signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `anthropic-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:alert-monitor` consumers with approval and credential context attached.
