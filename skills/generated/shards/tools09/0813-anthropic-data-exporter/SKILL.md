---
name: anthropic-data-exporter
description: Use when tasks require anthropic data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Data Exporter
...
| Primary artifact | `anthropic-export-package` |
| Routing tag | `anthropic:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Data Exporter into a deterministic, auth-checked workflow for exports anthropic entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-export-package | structured-artifact | yes | next workflow or operator |
| anthropic-export-package-scorecard | scorecard | yes | reviewer |
| anthropic-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Anthropic Data Exporter, then constrain the export scope for Anthropic before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports anthropic entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `anthropic-export-package` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:data-exporter` consumers with approval and credential context attached.
