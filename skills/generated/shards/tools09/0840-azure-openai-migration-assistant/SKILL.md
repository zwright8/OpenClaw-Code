---
name: azure-openai-migration-assistant
description: Use when tasks require azure openai migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Migration Assistant
...
| Primary artifact | `azure-openai-migration-cutover-plan` |
| Routing tag | `azure-openai:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when state changes are hard to undo once they begin. This specific skill turns Azure OpenAI Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from azure openai..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| azure-openai-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| azure-openai-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Azure OpenAI Migration Assistant before any Azure OpenAI rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from azure openai. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `azure-openai-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:migration-assistant` consumers with approval and credential context attached.
