---
name: sora-data-importer
description: Use when tasks require sora data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sora-import-ledger` |
| Routing tag | `sora:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sora Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into sora..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-import-ledger | structured-artifact | yes | next workflow or operator |
| sora-import-ledger-scorecard | scorecard | yes | reviewer |
| sora-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Sora Data Importer before any Sora write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into sora. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `sora-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] sora-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:data-importer` consumers with approval and credential context attached.
