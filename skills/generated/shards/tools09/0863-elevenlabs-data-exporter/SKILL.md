---
name: elevenlabs-data-exporter
description: Use when tasks require elevenlabs data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Data Exporter
...
| Primary artifact | `elevenlabs-export-package` |
| Routing tag | `elevenlabs:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Data Exporter into a deterministic, auth-checked workflow for exports elevenlabs entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-export-package | structured-artifact | yes | next workflow or operator |
| elevenlabs-export-package-scorecard | scorecard | yes | reviewer |
| elevenlabs-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for ElevenLabs Data Exporter, then constrain the export scope for ElevenLabs before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports elevenlabs entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `elevenlabs-export-package` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:data-exporter` consumers with approval and credential context attached.
