---
name: anthropic-report-builder
description: Use when tasks require anthropic report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Report Builder
...
| Primary artifact | `anthropic-report-bundle` |
| Routing tag | `anthropic:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from anthropic activity..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-report-bundle | structured-artifact | yes | next workflow or operator |
| anthropic-report-bundle-scorecard | scorecard | yes | reviewer |
| anthropic-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Anthropic Report Builder, then confirm which Anthropic entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from anthropic activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `anthropic-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:report-builder` consumers with approval and credential context attached.
