---
name: b09-secrets-qt-k8s-identity
description: Use when managing secrets/config distribution, Qt ecosystem components, Kubernetes credentials, and secure messaging/identity dependencies.
---

# Secrets ops, Qt stacks, Kubernetes, and identity systems

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-secrets-qt-k8s-identity` |
| Tool count | `26` |
| Inventory rank span | `4295-4324` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for secrets ops, qt stacks, kubernetes, identity systems tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `teller` | `shntool` | no | no |
| bounded mutation / transformation | `shntool` | `teller` | yes | yes |
| validation & artifact checks | `teller` | `shntool` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `teller` | Secrets management tool for developers | local-installable |
| `shntool` | Multi-purpose tool for manipulating and analyzing WAV files | local-installable |
| `qtcharts` | UI Components for displaying visually pleasing charts | local-installable |
| `qtserialport` | Provides classes to interact with hardware and virtual serial ports | local-installable |
| `moarvm` | VM with adaptive optimization and JIT compilation, built for Rakudo | local-installable |
| `gnome-online-accounts` | Single sign-on framework for GNOME | local-installable |
| `texi2html` | Convert TeXinfo files to HTML | local-installable |
| `google-authenticator-libpam` | PAM module for two-factor authentication | local-installable |
| `atool` | Archival front-end | local-installable |
| `pandocomatic` | Automate the use of pandoc | local-installable |
| `gl2ps` | OpenGL to PostScript printing library | local-installable |
| `gnunet` | Framework for distributed, secure and privacy-preserving applications | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `teller` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shntool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtcharts` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtserialport` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `moarvm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnome-online-accounts` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `texi2html` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `google-authenticator-libpam` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `atool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pandocomatic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gl2ps` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `gnunet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libspelling` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `qtspeech` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libnatpmp` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `markdown-toc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `docx2txt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcec` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hfsutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `airshare` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ortp` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kapp` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `qtremoteobjects` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `iconsur` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kubergrunt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libolm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kapp`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `teller` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `shntool` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `qtcharts` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `qtserialport` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `moarvm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `gnome-online-accounts` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a secrets ops, qt stacks, kubernetes, identity systems request, run dry-run first, then produce a validated artifact bundle.
