---
name: playwright-alert-monitor
description: Use when tasks require playwright alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Alert Monitor
...
| Primary artifact | `playwright-alert-state-bundle` |
| Routing tag | `playwright:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Playwright Alert Monitor into a deterministic, auth-checked workflow for monitors key playwright signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| playwright-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| playwright-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Playwright Alert Monitor, then lock the reviewed threshold set for Playwright.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key playwright signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `playwright-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] playwright-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:alert-monitor` consumers with approval and credential context attached.
