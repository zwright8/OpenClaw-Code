---
name: anthropic-data-importer
description: Use when tasks require anthropic data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Data Importer
...
| Primary artifact | `anthropic-import-ledger` |
| Routing tag | `anthropic:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when state changes are hard to undo once they begin. This specific skill turns Anthropic Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into anthropic..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-import-ledger | structured-artifact | yes | next workflow or operator |
| anthropic-import-ledger-scorecard | scorecard | yes | reviewer |
| anthropic-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Anthropic Data Importer before any Anthropic write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into anthropic. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `anthropic-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:data-importer` consumers with approval and credential context attached.
