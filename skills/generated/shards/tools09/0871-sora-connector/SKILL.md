---
name: sora-connector
description: Use when tasks require sora connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sora-connection-trace-bundle` |
| Routing tag | `sora:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Connector into a deterministic, auth-checked workflow for unified read/write connector for sora apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| sora-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| sora-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Sora Connector, then confirm the smallest read-only probe that proves access to Sora.
...
3. Specify the request and response contract for unified read/write connector for sora apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `sora-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sora-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:connector` consumers with approval and credential context attached.
