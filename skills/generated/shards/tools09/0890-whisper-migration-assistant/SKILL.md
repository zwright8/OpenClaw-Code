---
name: whisper-migration-assistant
description: Use when tasks require whisper migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Migration Assistant
...
| Primary artifact | `whisper-migration-cutover-plan` |
| Routing tag | `whisper:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when state changes are hard to undo once they begin. This specific skill turns Whisper Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from whisper..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| whisper-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| whisper-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Whisper Migration Assistant before any Whisper rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from whisper. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `whisper-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] whisper-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:migration-assistant` consumers with approval and credential context attached.
