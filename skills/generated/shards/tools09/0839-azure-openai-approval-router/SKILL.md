---
name: azure-openai-approval-router
description: Use when tasks require azure openai approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure OpenAI Approval Router
...
| Primary artifact | `azure-openai-approval-decision-packet` |
| Routing tag | `azure-openai:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure OpenAI Approval Router into a deterministic, auth-checked workflow for routes azure openai-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Azure OpenAI Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-openai-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| azure-openai-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| azure-openai-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Azure OpenAI Approval Router, then capture the manual fallback path for Azure OpenAI.
...
3. Implement routes azure openai-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `azure-openai-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] azure-openai-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-openai-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure-openai:approval-router` consumers with approval and credential context attached.
