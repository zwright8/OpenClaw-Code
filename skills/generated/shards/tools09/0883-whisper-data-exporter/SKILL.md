---
name: whisper-data-exporter
description: Use when tasks require whisper data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Whisper Data Exporter
...
| Primary artifact | `whisper-export-package` |
| Routing tag | `whisper:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Whisper workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Whisper Data Exporter into a deterministic, auth-checked workflow for exports whisper entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Whisper Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| whisper-export-package | structured-artifact | yes | next workflow or operator |
| whisper-export-package-scorecard | scorecard | yes | reviewer |
| whisper-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Whisper Data Exporter, then constrain the export scope for Whisper before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports whisper entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `whisper-export-package` so downstream owners do not need to rediscover context.
...
- [ ] whisper-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `whisper-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `whisper:data-exporter` consumers with approval and credential context attached.
