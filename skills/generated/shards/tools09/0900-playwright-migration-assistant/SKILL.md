---
name: playwright-migration-assistant
description: Use when tasks require playwright migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Playwright Migration Assistant
...
| Primary artifact | `playwright-migration-cutover-plan` |
| Routing tag | `playwright:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Playwright workflows degrade when state changes are hard to undo once they begin. This specific skill turns Playwright Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from playwright..

## Trigger Checklist
- [ ] The task explicitly requires `Playwright Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| playwright-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| playwright-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| playwright-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Playwright Migration Assistant before any Playwright rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from playwright. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `playwright-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] playwright-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `playwright-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `playwright:migration-assistant` consumers with approval and credential context attached.
