...
description: Use when tasks require replicate connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `replicate-connection-trace-bundle` |
| Routing tag | `replicate:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Connector into a deterministic, auth-checked workflow for unified read/write connector for replicate apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| replicate-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| replicate-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Replicate Connector, then confirm the smallest read-only probe that proves access to Replicate.
...
3. Specify the request and response contract for unified read/write connector for replicate apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `replicate-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] replicate-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:connector` consumers with approval and credential context attached.
