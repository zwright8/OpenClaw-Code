...
description: Use when tasks require openai connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `openai-connection-trace-bundle` |
| Routing tag | `openai:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenAI Connector into a deterministic, auth-checked workflow for unified read/write connector for openai apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| openai-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| openai-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for OpenAI Connector, then confirm the smallest read-only probe that proves access to OpenAI.
...
3. Specify the request and response contract for unified read/write connector for openai apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `openai-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] openai-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:connector` consumers with approval and credential context attached.
