---
name: sora-workflow-automator
description: Use when tasks require sora workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sora Workflow Automator
...
| Primary artifact | `sora-workflow-runbook` |
| Routing tag | `sora:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using sora..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-workflow-runbook | structured-artifact | yes | next workflow or operator |
| sora-workflow-runbook-scorecard | scorecard | yes | reviewer |
| sora-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Sora Workflow Automator, then checkpoint where Sora state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using sora. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `sora-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] sora-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:workflow-automator` consumers with approval and credential context attached.
