---
name: replicate-approval-router
description: Use when tasks require replicate approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Approval Router
...
| Primary artifact | `replicate-approval-decision-packet` |
| Routing tag | `replicate:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Approval Router into a deterministic, auth-checked workflow for routes replicate-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| replicate-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| replicate-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Replicate Approval Router, then capture the manual fallback path for Replicate.
...
3. Implement routes replicate-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `replicate-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] replicate-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:approval-router` consumers with approval and credential context attached.
