---
name: b09-packaging-audio-vcs-qt
description: Use when running package/build systems, audio streaming/control services, Git-compatible workflows, and Qt analytics/UI module operations.
---

# Packaging pipelines, audio services, VCS, and Qt data tools

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b09-packaging-audio-vcs-qt` |
| Tool count | `26` |
| Inventory rank span | `4359-4389` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for packaging pipelines, audio services, vcs, qt data tools tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `rattler-build` | `libpthread-stubs` | no | no |
| bounded mutation / transformation | `libpthread-stubs` | `rattler-build` | yes | yes |
| validation & artifact checks | `rattler-build` | `libpthread-stubs` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `rattler-build` | Universal conda package builder | local-installable |
| `libpthread-stubs` | X.Org: pthread-stubs.pc | local-installable |
| `gersemi` | Formatter to make your CMake code the real treasure | local-installable |
| `librespot` | Open Source Spotify client library | local-installable |
| `got` | Version control system | local-installable |
| `podsync` | Turn YouTube or Vimeo channels, users, or playlists into podcast feeds | local-installable |
| `wasi-libc` | Libc implementation for WebAssembly | local-installable |
| `ncmdump` | Convert Netease Cloud Music ncm files to mp3/flac files | local-installable |
| `kin` | Sane PBXProj files | local-installable |
| `yorkie` | Document store for collaborative applications | local-installable |
| `libhandy` | Building blocks for modern adaptive GNOME apps | local-installable |
| `semver` | Semantic version parser for node (the one npm uses) | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `rattler-build` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpthread-stubs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gersemi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `librespot` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `got` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `podsync` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wasi-libc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ncmdump` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yorkie` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libhandy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `semver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dropbear` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ninvaders` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pkg-config-wrapper` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cmark` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `excalidraw-converter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mani` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jsign` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtnetworkauth` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `neocmakelsp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `projectm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `uuu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-generate` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtdatavis3d` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qwt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `qtnetworkauth`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `rattler-build` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `libpthread-stubs` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `gersemi` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `librespot` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `got` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `podsync` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a packaging pipelines, audio services, vcs, qt data tools request, run dry-run first, then produce a validated artifact bundle.
