---
name: hugging-face-data-exporter
description: Use when tasks require hugging face data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Hugging Face Data Exporter
...
| Primary artifact | `hugging-face-export-package` |
| Routing tag | `hugging-face:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Hugging Face workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Hugging Face Data Exporter into a deterministic, auth-checked workflow for exports hugging face entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Hugging Face Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hugging-face-export-package | structured-artifact | yes | next workflow or operator |
| hugging-face-export-package-scorecard | scorecard | yes | reviewer |
| hugging-face-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Hugging Face Data Exporter, then constrain the export scope for Hugging Face before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports hugging face entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `hugging-face-export-package` so downstream owners do not need to rediscover context.
...
- [ ] hugging-face-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hugging-face-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hugging-face:data-exporter` consumers with approval and credential context attached.
