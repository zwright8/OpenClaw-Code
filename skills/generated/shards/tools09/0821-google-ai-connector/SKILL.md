---
name: google-ai-connector
description: Use when tasks require google ai connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `google-ai-connection-trace-bundle` |
| Routing tag | `google-ai:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google AI Connector into a deterministic, auth-checked workflow for unified read/write connector for google ai apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| google-ai-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| google-ai-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Google AI Connector, then confirm the smallest read-only probe that proves access to Google AI.
...
3. Specify the request and response contract for unified read/write connector for google ai apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `google-ai-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:connector` consumers with approval and credential context attached.
