---
name: anthropic-migration-assistant
description: Use when tasks require anthropic migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Migration Assistant
...
| Primary artifact | `anthropic-migration-cutover-plan` |
| Routing tag | `anthropic:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when state changes are hard to undo once they begin. This specific skill turns Anthropic Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from anthropic..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| anthropic-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| anthropic-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Anthropic Migration Assistant before any Anthropic rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from anthropic. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `anthropic-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:migration-assistant` consumers with approval and credential context attached.
