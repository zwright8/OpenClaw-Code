---
name: openai-data-importer
description: Use when tasks require openai data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenAI Data Importer
...
| Primary artifact | `openai-import-ledger` |
| Routing tag | `openai:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenAI workflows degrade when state changes are hard to undo once they begin. This specific skill turns OpenAI Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into openai..

## Trigger Checklist
- [ ] The task explicitly requires `OpenAI Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| openai-import-ledger | structured-artifact | yes | next workflow or operator |
| openai-import-ledger-scorecard | scorecard | yes | reviewer |
| openai-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for OpenAI Data Importer before any OpenAI write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into openai. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `openai-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] openai-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `openai-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `openai:data-importer` consumers with approval and credential context attached.
