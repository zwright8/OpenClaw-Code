---
name: elevenlabs-workflow-automator
description: Use when tasks require elevenlabs workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ElevenLabs Workflow Automator
...
| Primary artifact | `elevenlabs-workflow-runbook` |
| Routing tag | `elevenlabs:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ElevenLabs workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ElevenLabs Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using elevenlabs..

## Trigger Checklist
- [ ] The task explicitly requires `ElevenLabs Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elevenlabs-workflow-runbook | structured-artifact | yes | next workflow or operator |
| elevenlabs-workflow-runbook-scorecard | scorecard | yes | reviewer |
| elevenlabs-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for ElevenLabs Workflow Automator, then checkpoint where ElevenLabs state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using elevenlabs. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `elevenlabs-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] elevenlabs-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elevenlabs-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elevenlabs:workflow-automator` consumers with approval and credential context attached.
