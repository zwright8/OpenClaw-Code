---
name: anthropic-sync-orchestrator
description: Use when tasks require anthropic sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Anthropic Sync Orchestrator
...
| Primary artifact | `anthropic-reconciliation-bundle` |
| Routing tag | `anthropic:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Anthropic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Anthropic Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for anthropic records..

## Trigger Checklist
- [ ] The task explicitly requires `Anthropic Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| anthropic-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| anthropic-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| anthropic-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Anthropic Sync Orchestrator, then checkpoint the last known good cursor on both sides of Anthropic.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for anthropic records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `anthropic-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] anthropic-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `anthropic-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `anthropic:sync-orchestrator` consumers with approval and credential context attached.
