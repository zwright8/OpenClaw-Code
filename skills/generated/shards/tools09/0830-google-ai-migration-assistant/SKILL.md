---
name: google-ai-migration-assistant
description: Use when tasks require google ai migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google AI Migration Assistant
...
| Primary artifact | `google-ai-migration-cutover-plan` |
| Routing tag | `google-ai:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google AI workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google AI Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from google ai..

## Trigger Checklist
- [ ] The task explicitly requires `Google AI Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-ai-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| google-ai-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| google-ai-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Google AI Migration Assistant before any Google AI rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from google ai. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `google-ai-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] google-ai-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-ai-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-ai:migration-assistant` consumers with approval and credential context attached.
