---
name: replicate-data-exporter
description: Use when tasks require replicate data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Replicate Data Exporter
...
| Primary artifact | `replicate-export-package` |
| Routing tag | `replicate:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Replicate workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Replicate Data Exporter into a deterministic, auth-checked workflow for exports replicate entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Replicate Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| replicate-export-package | structured-artifact | yes | next workflow or operator |
| replicate-export-package-scorecard | scorecard | yes | reviewer |
| replicate-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Replicate Data Exporter, then constrain the export scope for Replicate before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports replicate entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `replicate-export-package` so downstream owners do not need to rediscover context.
...
- [ ] replicate-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `replicate-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `replicate:data-exporter` consumers with approval and credential context attached.
