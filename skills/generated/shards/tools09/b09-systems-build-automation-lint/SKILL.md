...
| Field | Value |
|---|---|
| Skill pack | `b09-systems-build-automation-lint` |
| Tool count | `26` |
| Inventory rank span | `4326-4358` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for systems inspection, build automation, lint discipline tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `mmv` | `cpuid` | no | no |
| bounded mutation / transformation | `cpuid` | `mmv` | yes | yes |
| validation & artifact checks | `mmv` | `cpuid` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `mmv` | Move, copy, append, and link multiple files | local-installable |
| `cpuid` | CPU feature identification for Go | local-installable |
| `davix` | Library and tools for advanced file I/O with HTTP-based protocols | local-installable |
| `autobrr` | Modern, easy to use download automation for torrents and usenet | local-installable |
| `stp` | Simple Theorem Prover, an efficient SMT solver for bitvectors | local-installable |
| `libjson-rpc-cpp` | C++ framework for json-rpc | local-installable |
| `zig@0.14` | Programming language designed for robustness, optimality, and clarity | local-installable |
| `qtwebview` | Displays web content in a QML application | local-installable |
| `gensio` | Stream I/O Library | local-installable |
| `pdnsrec` | Non-authoritative/recursing DNS server | local-installable |
| `dbus-glib` | GLib bindings for the D-Bus message bus system | local-installable |
| `petsc-complex` | Portable, Extensible Toolkit for Scientific Computation (complex) | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `mmv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cpuid` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `davix` | HTTPS/REST, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `autobrr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `stp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libjson-rpc-cpp` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zig@0.14` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtwebview` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gensio` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pdnsrec` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dbus-glib` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `petsc-complex` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtwebchannel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `logstalgia` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chocolate-doom` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jscpd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtserialbus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lpeg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtgraphs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xgo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sextractor` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lensfun` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ttfautohint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `serve` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pidcat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `reorder-python-imports` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `mmv` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `cpuid` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `davix` (HTTPS/REST, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `autobrr` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `stp` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libjson-rpc-cpp` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a systems inspection, build automation, lint discipline request, run dry-run first, then produce a validated artifact bundle.
