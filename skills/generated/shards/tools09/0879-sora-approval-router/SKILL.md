---
name: sora-approval-router
description: Use when tasks require sora approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sora Approval Router
...
| Primary artifact | `sora-approval-decision-packet` |
| Routing tag | `sora:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sora workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sora Approval Router into a deterministic, auth-checked workflow for routes sora-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Sora Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sora-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| sora-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| sora-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Sora Approval Router, then capture the manual fallback path for Sora.
...
3. Implement routes sora-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `sora-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] sora-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sora-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sora:approval-router` consumers with approval and credential context attached.
