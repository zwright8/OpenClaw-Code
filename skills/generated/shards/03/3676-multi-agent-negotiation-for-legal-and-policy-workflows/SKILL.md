---
name: u3676-multi-agent-negotiation-for-legal-and-policy-workflows
description: Operate the "multi agent negotiation for legal and policy workflows" capability in production for multi agent negotiation for legal and policy workflows workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# multi agent negotiation for legal and policy workflows

## Why This Skill Exists
This skill hardens a generated capability for production execution so multi agent negotiation for legal and policy workflows workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `multi agent negotiation for legal and policy workflows` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **multi agent negotiation for legal and policy workflows** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **legal and policy workflows" capability in production for multi agent negotiation for legal and policy workflows workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new legal and policy workflows" capability in production for multi agent negotiation for legal and policy workflows workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in legal and policy workflows" capability in production for multi agent negotiation for legal and policy workflows workflows**
   - Input: noisy upstream payload requiring multi agent negotiation for legal and policy workflows normalization/assessment.
