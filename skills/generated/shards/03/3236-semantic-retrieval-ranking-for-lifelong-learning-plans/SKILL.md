---
name: u3236-semantic-retrieval-ranking-for-lifelong-learning-plans
description: Operate the "semantic retrieval ranking for lifelong learning plans" capability in production for semantic retrieval ranking for lifelong learning plans workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# semantic retrieval ranking for lifelong learning plans

## Why This Skill Exists
This skill hardens a generated capability for production execution so semantic retrieval ranking for lifelong learning plans workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `semantic retrieval ranking for lifelong learning plans` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **semantic retrieval ranking for lifelong learning plans** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **lifelong learning plans" capability in production for semantic retrieval ranking for lifelong learning plans workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new lifelong learning plans" capability in production for semantic retrieval ranking for lifelong learning plans workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in lifelong learning plans" capability in production for semantic retrieval ranking for lifelong learning plans workflows**
   - Input: noisy upstream payload requiring semantic retrieval ranking for lifelong learning plans normalization/assessment.
