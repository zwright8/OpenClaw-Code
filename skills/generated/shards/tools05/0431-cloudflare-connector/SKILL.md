...
description: Use when tasks require cloudflare connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `cloudflare-connection-trace-bundle` |
| Routing tag | `cloudflare:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Connector into a deterministic, auth-checked workflow for unified read/write connector for cloudflare apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| cloudflare-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| cloudflare-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Cloudflare Connector, then confirm the smallest read-only probe that proves access to Cloudflare.
...
3. Specify the request and response contract for unified read/write connector for cloudflare apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `cloudflare-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:connector` consumers with approval and credential context attached.
