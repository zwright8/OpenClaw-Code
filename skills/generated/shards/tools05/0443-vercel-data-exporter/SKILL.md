---
name: vercel-data-exporter
description: Use when tasks require vercel data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Data Exporter
...
| Primary artifact | `vercel-export-package` |
| Routing tag | `vercel:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Data Exporter into a deterministic, auth-checked workflow for exports vercel entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-export-package | structured-artifact | yes | next workflow or operator |
| vercel-export-package-scorecard | scorecard | yes | reviewer |
| vercel-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Vercel Data Exporter, then constrain the export scope for Vercel before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports vercel entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `vercel-export-package` so downstream owners do not need to rediscover context.
...
- [ ] vercel-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:data-exporter` consumers with approval and credential context attached.
