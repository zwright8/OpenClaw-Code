---
name: gcp-connector
description: Use when tasks require gcp connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-connection-trace-bundle` |
| Routing tag | `gcp:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Connector into a deterministic, auth-checked workflow for unified read/write connector for gcp apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| gcp-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| gcp-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for GCP Connector, then confirm the smallest read-only probe that proves access to GCP.
...
3. Specify the request and response contract for unified read/write connector for gcp apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `gcp-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gcp-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:connector` consumers with approval and credential context attached.
