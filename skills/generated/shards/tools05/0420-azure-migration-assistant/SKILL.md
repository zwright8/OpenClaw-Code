---
name: azure-migration-assistant
description: Use when tasks require azure migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-migration-cutover-plan` |
| Routing tag | `azure:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when state changes are hard to undo once they begin. This specific skill turns Azure Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from azure..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| azure-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| azure-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Azure Migration Assistant before any Azure rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from azure. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `azure-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] azure-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:migration-assistant` consumers with approval and credential context attached.
