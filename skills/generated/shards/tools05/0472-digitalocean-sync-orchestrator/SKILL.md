---
name: digitalocean-sync-orchestrator
description: Use when tasks require digitalocean sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Sync Orchestrator
...
| Primary artifact | `digitalocean-reconciliation-bundle` |
| Routing tag | `digitalocean:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for digitalocean records..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| digitalocean-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| digitalocean-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for DigitalOcean Sync Orchestrator, then checkpoint the last known good cursor on both sides of DigitalOcean.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for digitalocean records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `digitalocean-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:sync-orchestrator` consumers with approval and credential context attached.
