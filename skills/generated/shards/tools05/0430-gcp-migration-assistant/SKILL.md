---
name: gcp-migration-assistant
description: Use when tasks require gcp migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GCP Migration Assistant
...
| Primary artifact | `gcp-migration-cutover-plan` |
| Routing tag | `gcp:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when state changes are hard to undo once they begin. This specific skill turns GCP Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from gcp..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| gcp-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| gcp-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for GCP Migration Assistant before any GCP rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from gcp. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `gcp-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] gcp-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:migration-assistant` consumers with approval and credential context attached.
