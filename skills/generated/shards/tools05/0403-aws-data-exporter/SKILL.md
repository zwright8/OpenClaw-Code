---
name: aws-data-exporter
description: Use when tasks require aws data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-export-package` |
| Routing tag | `aws:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Data Exporter into a deterministic, auth-checked workflow for exports aws entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-export-package | structured-artifact | yes | next workflow or operator |
| aws-export-package-scorecard | scorecard | yes | reviewer |
| aws-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for AWS Data Exporter, then constrain the export scope for AWS before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports aws entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `aws-export-package` so downstream owners do not need to rediscover context.
...
- [ ] aws-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:data-exporter` consumers with approval and credential context attached.
