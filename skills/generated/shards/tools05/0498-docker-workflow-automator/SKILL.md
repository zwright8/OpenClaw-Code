---
name: docker-workflow-automator
description: Use when tasks require docker workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Workflow Automator
...
| Primary artifact | `docker-workflow-runbook` |
| Routing tag | `docker:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using docker..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-workflow-runbook | structured-artifact | yes | next workflow or operator |
| docker-workflow-runbook-scorecard | scorecard | yes | reviewer |
| docker-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Docker Workflow Automator, then checkpoint where Docker state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using docker. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `docker-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] docker-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:workflow-automator` consumers with approval and credential context attached.
