---
name: render-sync-orchestrator
description: Use when tasks require render sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Sync Orchestrator
...
| Primary artifact | `render-reconciliation-bundle` |
| Routing tag | `render:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for render records..

## Trigger Checklist
- [ ] The task explicitly requires `Render Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| render-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| render-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Render Sync Orchestrator, then checkpoint the last known good cursor on both sides of Render.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for render records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `render-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] render-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:sync-orchestrator` consumers with approval and credential context attached.
