---
name: aws-approval-router
description: Use when tasks require aws approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-approval-decision-packet` |
| Routing tag | `aws:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Approval Router into a deterministic, auth-checked workflow for routes aws-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| aws-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| aws-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for AWS Approval Router, then capture the manual fallback path for AWS.
...
3. Implement routes aws-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `aws-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] aws-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:approval-router` consumers with approval and credential context attached.
