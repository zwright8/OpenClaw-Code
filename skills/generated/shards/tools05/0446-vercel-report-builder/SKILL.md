...
description: Use when tasks require vercel report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `vercel-report-bundle` |
| Routing tag | `vercel:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from vercel activity..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-report-bundle | structured-artifact | yes | next workflow or operator |
| vercel-report-bundle-scorecard | scorecard | yes | reviewer |
| vercel-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Vercel Report Builder, then confirm which Vercel entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from vercel activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `vercel-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] vercel-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:report-builder` consumers with approval and credential context attached.
