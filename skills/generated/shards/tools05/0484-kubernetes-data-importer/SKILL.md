---
name: kubernetes-data-importer
description: Use when tasks require kubernetes data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `kubernetes-import-ledger` |
| Routing tag | `kubernetes:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when state changes are hard to undo once they begin. This specific skill turns Kubernetes Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into kubernetes..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-import-ledger | structured-artifact | yes | next workflow or operator |
| kubernetes-import-ledger-scorecard | scorecard | yes | reviewer |
| kubernetes-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Kubernetes Data Importer before any Kubernetes write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into kubernetes. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `kubernetes-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:data-importer` consumers with approval and credential context attached.
