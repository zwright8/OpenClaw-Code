---
name: cloudflare-migration-assistant
description: Use when tasks require cloudflare migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Migration Assistant
...
| Primary artifact | `cloudflare-migration-cutover-plan` |
| Routing tag | `cloudflare:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when state changes are hard to undo once they begin. This specific skill turns Cloudflare Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from cloudflare..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| cloudflare-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| cloudflare-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Cloudflare Migration Assistant before any Cloudflare rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from cloudflare. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `cloudflare-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:migration-assistant` consumers with approval and credential context attached.
