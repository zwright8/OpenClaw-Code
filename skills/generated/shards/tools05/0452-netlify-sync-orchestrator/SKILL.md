---
name: netlify-sync-orchestrator
description: Use when tasks require netlify sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Sync Orchestrator
...
| Primary artifact | `netlify-reconciliation-bundle` |
| Routing tag | `netlify:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for netlify records..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| netlify-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| netlify-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Netlify Sync Orchestrator, then checkpoint the last known good cursor on both sides of Netlify.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for netlify records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `netlify-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] netlify-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:sync-orchestrator` consumers with approval and credential context attached.
