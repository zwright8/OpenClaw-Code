---
name: gcp-workflow-automator
description: Use when tasks require gcp workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GCP Workflow Automator
...
| Primary artifact | `gcp-workflow-runbook` |
| Routing tag | `gcp:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using gcp..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-workflow-runbook | structured-artifact | yes | next workflow or operator |
| gcp-workflow-runbook-scorecard | scorecard | yes | reviewer |
| gcp-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for GCP Workflow Automator, then checkpoint where GCP state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using gcp. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `gcp-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] gcp-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:workflow-automator` consumers with approval and credential context attached.
