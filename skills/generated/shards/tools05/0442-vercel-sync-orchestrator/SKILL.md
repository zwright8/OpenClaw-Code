---
name: vercel-sync-orchestrator
description: Use when tasks require vercel sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Sync Orchestrator
...
| Primary artifact | `vercel-reconciliation-bundle` |
| Routing tag | `vercel:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for vercel records..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| vercel-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| vercel-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Vercel Sync Orchestrator, then checkpoint the last known good cursor on both sides of Vercel.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for vercel records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `vercel-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] vercel-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:sync-orchestrator` consumers with approval and credential context attached.
