---
name: render-webhook-listener
description: Use when tasks require render webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `render-event-intake-log` |
| Routing tag | `render:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Render workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Render Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes render webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Render Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `RENDER_SERVICE_ACCOUNT_JSON`, `RENDER_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `RENDER_TOKEN`, `RENDER_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `RENDER_WEBHOOK_SECRET`, `RENDER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| render-event-intake-log | structured-artifact | yes | next workflow or operator |
| render-event-intake-log-scorecard | scorecard | yes | reviewer |
| render-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Render Webhook Listener, then register the signing-secret policy for Render.
...
3. Implement captures, verifies, and routes render webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `render-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] render-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:webhook-listener` consumers with approval and credential context attached.
