---
name: render-approval-router
description: Use when tasks require render approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Approval Router
...
| Primary artifact | `render-approval-decision-packet` |
| Routing tag | `render:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Approval Router into a deterministic, auth-checked workflow for routes render-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Render Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| render-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| render-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Render Approval Router, then capture the manual fallback path for Render.
...
3. Implement routes render-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `render-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] render-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:approval-router` consumers with approval and credential context attached.
