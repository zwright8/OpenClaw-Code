---
name: cloudflare-sync-orchestrator
description: Use when tasks require cloudflare sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Sync Orchestrator
...
| Primary artifact | `cloudflare-reconciliation-bundle` |
| Routing tag | `cloudflare:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for cloudflare records..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| cloudflare-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| cloudflare-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Cloudflare Sync Orchestrator, then checkpoint the last known good cursor on both sides of Cloudflare.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for cloudflare records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `cloudflare-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:sync-orchestrator` consumers with approval and credential context attached.
