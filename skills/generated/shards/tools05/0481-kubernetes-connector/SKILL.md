...
description: Use when tasks require kubernetes connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `kubernetes-connection-trace-bundle` |
| Routing tag | `kubernetes:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Connector into a deterministic, auth-checked workflow for unified read/write connector for kubernetes apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| kubernetes-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| kubernetes-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Kubernetes Connector, then confirm the smallest read-only probe that proves access to Kubernetes.
...
3. Specify the request and response contract for unified read/write connector for kubernetes apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `kubernetes-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:connector` consumers with approval and credential context attached.
