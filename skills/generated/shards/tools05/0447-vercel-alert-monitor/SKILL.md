---
name: vercel-alert-monitor
description: Use when tasks require vercel alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Alert Monitor
...
| Primary artifact | `vercel-alert-state-bundle` |
| Routing tag | `vercel:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Alert Monitor into a deterministic, auth-checked workflow for monitors key vercel signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| vercel-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| vercel-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Vercel Alert Monitor, then lock the reviewed threshold set for Vercel.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key vercel signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `vercel-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] vercel-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:alert-monitor` consumers with approval and credential context attached.
