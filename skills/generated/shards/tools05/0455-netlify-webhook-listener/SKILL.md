---
name: netlify-webhook-listener
description: Use when tasks require netlify webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Webhook Listener
...
| Primary artifact | `netlify-event-intake-log` |
| Routing tag | `netlify:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Netlify Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes netlify webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `NETLIFY_SERVICE_ACCOUNT_JSON`, `NETLIFY_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `NETLIFY_TOKEN`, `NETLIFY_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `NETLIFY_WEBHOOK_SECRET`, `NETLIFY_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| netlify-event-intake-log | structured-artifact | yes | next workflow or operator |
| netlify-event-intake-log-scorecard | scorecard | yes | reviewer |
| netlify-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Netlify Webhook Listener, then register the signing-secret policy for Netlify.
...
3. Implement captures, verifies, and routes netlify webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `netlify-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] netlify-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:webhook-listener` consumers with approval and credential context attached.
