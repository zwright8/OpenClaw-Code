---
name: cloudflare-webhook-listener
description: Use when tasks require cloudflare webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Webhook Listener
...
| Primary artifact | `cloudflare-event-intake-log` |
| Routing tag | `cloudflare:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Cloudflare Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes cloudflare webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `CLOUDFLARE_SERVICE_ACCOUNT_JSON`, `CLOUDFLARE_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `CLOUDFLARE_TOKEN`, `CLOUDFLARE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CLOUDFLARE_WEBHOOK_SECRET`, `CLOUDFLARE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| cloudflare-event-intake-log | structured-artifact | yes | next workflow or operator |
| cloudflare-event-intake-log-scorecard | scorecard | yes | reviewer |
| cloudflare-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Cloudflare Webhook Listener, then register the signing-secret policy for Cloudflare.
...
3. Implement captures, verifies, and routes cloudflare webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `cloudflare-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:webhook-listener` consumers with approval and credential context attached.
