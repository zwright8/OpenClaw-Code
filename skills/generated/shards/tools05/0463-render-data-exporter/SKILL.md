---
name: render-data-exporter
description: Use when tasks require render data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Data Exporter
...
| Primary artifact | `render-export-package` |
| Routing tag | `render:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Data Exporter into a deterministic, auth-checked workflow for exports render entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Render Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-export-package | structured-artifact | yes | next workflow or operator |
| render-export-package-scorecard | scorecard | yes | reviewer |
| render-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Render Data Exporter, then constrain the export scope for Render before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports render entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `render-export-package` so downstream owners do not need to rediscover context.
...
- [ ] render-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:data-exporter` consumers with approval and credential context attached.
