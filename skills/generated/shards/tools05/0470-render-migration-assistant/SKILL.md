---
name: render-migration-assistant
description: Use when tasks require render migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Migration Assistant
...
| Primary artifact | `render-migration-cutover-plan` |
| Routing tag | `render:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Render workflows degrade when state changes are hard to undo once they begin. This specific skill turns Render Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from render..

## Trigger Checklist
- [ ] The task explicitly requires `Render Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| render-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| render-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Render Migration Assistant before any Render rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from render. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `render-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] render-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:migration-assistant` consumers with approval and credential context attached.
