---
name: netlify-connector
description: Use when tasks require netlify connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `netlify-connection-trace-bundle` |
| Routing tag | `netlify:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Connector into a deterministic, auth-checked workflow for unified read/write connector for netlify apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| netlify-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| netlify-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Netlify Connector, then confirm the smallest read-only probe that proves access to Netlify.
...
3. Specify the request and response contract for unified read/write connector for netlify apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `netlify-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] netlify-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:connector` consumers with approval and credential context attached.
