...
description: Use when tasks require docker webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docker-event-intake-log` |
| Routing tag | `docker:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Docker Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes docker webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `DOCKER_SERVICE_ACCOUNT_JSON`, `DOCKER_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `DOCKER_TOKEN`, `DOCKER_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DOCKER_WEBHOOK_SECRET`, `DOCKER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| docker-event-intake-log | structured-artifact | yes | next workflow or operator |
| docker-event-intake-log-scorecard | scorecard | yes | reviewer |
| docker-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Docker Webhook Listener, then register the signing-secret policy for Docker.
...
3. Implement captures, verifies, and routes docker webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `docker-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] docker-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:webhook-listener` consumers with approval and credential context attached.
