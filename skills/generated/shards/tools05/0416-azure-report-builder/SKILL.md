---
name: azure-report-builder
description: Use when tasks require azure report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure Report Builder
...
| Primary artifact | `azure-report-bundle` |
| Routing tag | `azure:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from azure activity..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-report-bundle | structured-artifact | yes | next workflow or operator |
| azure-report-bundle-scorecard | scorecard | yes | reviewer |
| azure-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Azure Report Builder, then confirm which Azure entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from azure activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `azure-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:report-builder` consumers with approval and credential context attached.
