---
name: docker-approval-router
description: Use when tasks require docker approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Approval Router
...
| Primary artifact | `docker-approval-decision-packet` |
| Routing tag | `docker:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Approval Router into a deterministic, auth-checked workflow for routes docker-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| docker-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| docker-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Docker Approval Router, then capture the manual fallback path for Docker.
...
3. Implement routes docker-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `docker-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] docker-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:approval-router` consumers with approval and credential context attached.
