---
name: cloudflare-data-exporter
description: Use when tasks require cloudflare data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Data Exporter
...
| Primary artifact | `cloudflare-export-package` |
| Routing tag | `cloudflare:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Data Exporter into a deterministic, auth-checked workflow for exports cloudflare entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-export-package | structured-artifact | yes | next workflow or operator |
| cloudflare-export-package-scorecard | scorecard | yes | reviewer |
| cloudflare-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Cloudflare Data Exporter, then constrain the export scope for Cloudflare before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports cloudflare entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `cloudflare-export-package` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:data-exporter` consumers with approval and credential context attached.
