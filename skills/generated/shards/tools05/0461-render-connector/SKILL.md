...
description: Use when tasks require render connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `render-connection-trace-bundle` |
| Routing tag | `render:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Connector into a deterministic, auth-checked workflow for unified read/write connector for render apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Render Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| render-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| render-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Render Connector, then confirm the smallest read-only probe that proves access to Render.
...
3. Specify the request and response contract for unified read/write connector for render apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `render-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] render-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:connector` consumers with approval and credential context attached.
