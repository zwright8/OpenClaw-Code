---
name: gcp-approval-router
description: Use when tasks require gcp approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-approval-decision-packet` |
| Routing tag | `gcp:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Approval Router into a deterministic, auth-checked workflow for routes gcp-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| gcp-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| gcp-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for GCP Approval Router, then capture the manual fallback path for GCP.
...
3. Implement routes gcp-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `gcp-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] gcp-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:approval-router` consumers with approval and credential context attached.
