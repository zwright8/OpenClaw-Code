---
name: cloudflare-data-importer
description: Use when tasks require cloudflare data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Data Importer
...
| Primary artifact | `cloudflare-import-ledger` |
| Routing tag | `cloudflare:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when state changes are hard to undo once they begin. This specific skill turns Cloudflare Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into cloudflare..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-import-ledger | structured-artifact | yes | next workflow or operator |
| cloudflare-import-ledger-scorecard | scorecard | yes | reviewer |
| cloudflare-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Cloudflare Data Importer before any Cloudflare write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into cloudflare. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `cloudflare-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:data-importer` consumers with approval and credential context attached.
