---
name: kubernetes-report-builder
description: Use when tasks require kubernetes report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Report Builder
...
| Primary artifact | `kubernetes-report-bundle` |
| Routing tag | `kubernetes:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from kubernetes activity..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-report-bundle | structured-artifact | yes | next workflow or operator |
| kubernetes-report-bundle-scorecard | scorecard | yes | reviewer |
| kubernetes-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Kubernetes Report Builder, then confirm which Kubernetes entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from kubernetes activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `kubernetes-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:report-builder` consumers with approval and credential context attached.
