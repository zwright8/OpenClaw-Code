...
This pack provides a curated operational toolkit for s5cmd tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
1. Select a minimal tool from this pack to solve a s5cmd request, run dry-run first, then produce a validated artifact bundle.
