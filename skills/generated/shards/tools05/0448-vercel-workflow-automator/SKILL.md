---
name: vercel-workflow-automator
description: Use when tasks require vercel workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Workflow Automator
...
| Primary artifact | `vercel-workflow-runbook` |
| Routing tag | `vercel:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using vercel..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-workflow-runbook | structured-artifact | yes | next workflow or operator |
| vercel-workflow-runbook-scorecard | scorecard | yes | reviewer |
| vercel-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Vercel Workflow Automator, then checkpoint where Vercel state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using vercel. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `vercel-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] vercel-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:workflow-automator` consumers with approval and credential context attached.
