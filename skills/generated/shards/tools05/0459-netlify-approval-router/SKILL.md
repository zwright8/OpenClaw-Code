---
name: netlify-approval-router
description: Use when tasks require netlify approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Approval Router
...
| Primary artifact | `netlify-approval-decision-packet` |
| Routing tag | `netlify:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Approval Router into a deterministic, auth-checked workflow for routes netlify-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| netlify-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| netlify-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Netlify Approval Router, then capture the manual fallback path for Netlify.
...
3. Implement routes netlify-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `netlify-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] netlify-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:approval-router` consumers with approval and credential context attached.
