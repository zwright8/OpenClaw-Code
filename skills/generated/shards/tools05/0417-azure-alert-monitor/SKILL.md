---
name: azure-alert-monitor
description: Use when tasks require azure alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-alert-state-bundle` |
| Routing tag | `azure:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Alert Monitor into a deterministic, auth-checked workflow for monitors key azure signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| azure-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| azure-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Azure Alert Monitor, then lock the reviewed threshold set for Azure.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key azure signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `azure-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:alert-monitor` consumers with approval and credential context attached.
