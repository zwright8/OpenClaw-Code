---
name: vercel-webhook-listener
description: Use when tasks require vercel webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `vercel-event-intake-log` |
| Routing tag | `vercel:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Vercel Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes vercel webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `VERCEL_SERVICE_ACCOUNT_JSON`, `VERCEL_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `VERCEL_TOKEN`, `VERCEL_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `VERCEL_WEBHOOK_SECRET`, `VERCEL_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| vercel-event-intake-log | structured-artifact | yes | next workflow or operator |
| vercel-event-intake-log-scorecard | scorecard | yes | reviewer |
| vercel-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Vercel Webhook Listener, then register the signing-secret policy for Vercel.
...
3. Implement captures, verifies, and routes vercel webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `vercel-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] vercel-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:webhook-listener` consumers with approval and credential context attached.
