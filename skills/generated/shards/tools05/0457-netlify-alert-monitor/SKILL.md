---
name: netlify-alert-monitor
description: Use when tasks require netlify alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Alert Monitor
...
| Primary artifact | `netlify-alert-state-bundle` |
| Routing tag | `netlify:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Alert Monitor into a deterministic, auth-checked workflow for monitors key netlify signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| netlify-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| netlify-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Netlify Alert Monitor, then lock the reviewed threshold set for Netlify.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key netlify signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `netlify-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] netlify-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:alert-monitor` consumers with approval and credential context attached.
