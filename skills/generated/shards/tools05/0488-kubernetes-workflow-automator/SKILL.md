---
name: kubernetes-workflow-automator
description: Use when tasks require kubernetes workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Workflow Automator
...
| Primary artifact | `kubernetes-workflow-runbook` |
| Routing tag | `kubernetes:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using kubernetes..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-workflow-runbook | structured-artifact | yes | next workflow or operator |
| kubernetes-workflow-runbook-scorecard | scorecard | yes | reviewer |
| kubernetes-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Kubernetes Workflow Automator, then checkpoint where Kubernetes state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using kubernetes. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `kubernetes-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:workflow-automator` consumers with approval and credential context attached.
