---
name: digitalocean-workflow-automator
description: Use when tasks require digitalocean workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Workflow Automator
...
| Primary artifact | `digitalocean-workflow-runbook` |
| Routing tag | `digitalocean:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using digitalocean..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-workflow-runbook | structured-artifact | yes | next workflow or operator |
| digitalocean-workflow-runbook-scorecard | scorecard | yes | reviewer |
| digitalocean-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for DigitalOcean Workflow Automator, then checkpoint where DigitalOcean state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using digitalocean. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `digitalocean-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:workflow-automator` consumers with approval and credential context attached.
