---
name: docker-report-builder
description: Use when tasks require docker report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Report Builder
...
| Primary artifact | `docker-report-bundle` |
| Routing tag | `docker:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from docker activity..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-report-bundle | structured-artifact | yes | next workflow or operator |
| docker-report-bundle-scorecard | scorecard | yes | reviewer |
| docker-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Docker Report Builder, then confirm which Docker entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from docker activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `docker-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docker-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:report-builder` consumers with approval and credential context attached.
