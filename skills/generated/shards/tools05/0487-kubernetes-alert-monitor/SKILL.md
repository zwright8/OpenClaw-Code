---
name: kubernetes-alert-monitor
description: Use when tasks require kubernetes alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Alert Monitor
...
| Primary artifact | `kubernetes-alert-state-bundle` |
| Routing tag | `kubernetes:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Alert Monitor into a deterministic, auth-checked workflow for monitors key kubernetes signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| kubernetes-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| kubernetes-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Kubernetes Alert Monitor, then lock the reviewed threshold set for Kubernetes.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key kubernetes signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `kubernetes-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:alert-monitor` consumers with approval and credential context attached.
