---
name: netlify-data-importer
description: Use when tasks require netlify data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Data Importer
...
| Primary artifact | `netlify-import-ledger` |
| Routing tag | `netlify:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when state changes are hard to undo once they begin. This specific skill turns Netlify Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into netlify..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-import-ledger | structured-artifact | yes | next workflow or operator |
| netlify-import-ledger-scorecard | scorecard | yes | reviewer |
| netlify-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Netlify Data Importer before any Netlify write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into netlify. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `netlify-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] netlify-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:data-importer` consumers with approval and credential context attached.
