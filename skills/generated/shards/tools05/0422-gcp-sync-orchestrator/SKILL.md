---
name: gcp-sync-orchestrator
description: Use when tasks require gcp sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GCP Sync Orchestrator
...
| Primary artifact | `gcp-reconciliation-bundle` |
| Routing tag | `gcp:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for gcp records..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| gcp-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| gcp-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for GCP Sync Orchestrator, then checkpoint the last known good cursor on both sides of GCP.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for gcp records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `gcp-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gcp-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:sync-orchestrator` consumers with approval and credential context attached.
