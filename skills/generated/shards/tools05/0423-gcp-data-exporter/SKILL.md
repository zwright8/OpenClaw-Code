---
name: gcp-data-exporter
description: Use when tasks require gcp data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-export-package` |
| Routing tag | `gcp:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Data Exporter into a deterministic, auth-checked workflow for exports gcp entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-export-package | structured-artifact | yes | next workflow or operator |
| gcp-export-package-scorecard | scorecard | yes | reviewer |
| gcp-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for GCP Data Exporter, then constrain the export scope for GCP before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports gcp entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `gcp-export-package` so downstream owners do not need to rediscover context.
...
- [ ] gcp-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:data-exporter` consumers with approval and credential context attached.
