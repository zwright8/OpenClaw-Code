---
name: kubernetes-data-exporter
description: Use when tasks require kubernetes data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Data Exporter
...
| Primary artifact | `kubernetes-export-package` |
| Routing tag | `kubernetes:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Data Exporter into a deterministic, auth-checked workflow for exports kubernetes entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-export-package | structured-artifact | yes | next workflow or operator |
| kubernetes-export-package-scorecard | scorecard | yes | reviewer |
| kubernetes-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Kubernetes Data Exporter, then constrain the export scope for Kubernetes before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports kubernetes entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `kubernetes-export-package` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:data-exporter` consumers with approval and credential context attached.
