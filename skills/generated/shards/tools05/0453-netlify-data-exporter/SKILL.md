---
name: netlify-data-exporter
description: Use when tasks require netlify data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Data Exporter
...
| Primary artifact | `netlify-export-package` |
| Routing tag | `netlify:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Data Exporter into a deterministic, auth-checked workflow for exports netlify entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-export-package | structured-artifact | yes | next workflow or operator |
| netlify-export-package-scorecard | scorecard | yes | reviewer |
| netlify-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Netlify Data Exporter, then constrain the export scope for Netlify before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports netlify entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `netlify-export-package` so downstream owners do not need to rediscover context.
...
- [ ] netlify-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:data-exporter` consumers with approval and credential context attached.
