---
name: gcp-webhook-listener
description: Use when tasks require gcp webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GCP Webhook Listener
...
| Primary artifact | `gcp-event-intake-log` |
| Routing tag | `gcp:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns GCP Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes gcp webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `GCP_SERVICE_ACCOUNT_JSON`, `GCP_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `GCP_TOKEN`, `GCP_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GCP_WEBHOOK_SECRET`, `GCP_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| gcp-event-intake-log | structured-artifact | yes | next workflow or operator |
| gcp-event-intake-log-scorecard | scorecard | yes | reviewer |
| gcp-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for GCP Webhook Listener, then register the signing-secret policy for GCP.
...
3. Implement captures, verifies, and routes gcp webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `gcp-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] gcp-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:webhook-listener` consumers with approval and credential context attached.
