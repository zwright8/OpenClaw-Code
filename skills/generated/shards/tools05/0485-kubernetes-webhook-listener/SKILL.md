---
name: kubernetes-webhook-listener
description: Use when tasks require kubernetes webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Webhook Listener
...
| Domain | `Cloud and infrastructure platforms` |
| Runtime archetype | `signed-event-ingestion-engine` |
| Core method | `signed event intake and routing` |
| Primary artifact | `kubernetes-event-intake-log` |
| Routing tag | `kubernetes:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Kubernetes Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes kubernetes webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `KUBERNETES_SERVICE_ACCOUNT_JSON`, `KUBERNETES_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `KUBERNETES_TOKEN`, `KUBERNETES_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `KUBERNETES_WEBHOOK_SECRET`, `KUBERNETES_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| kubernetes-event-intake-log | structured-artifact | yes | next workflow or operator |
| kubernetes-event-intake-log-scorecard | scorecard | yes | reviewer |
| kubernetes-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Kubernetes Webhook Listener, then register the signing-secret policy for Kubernetes.
...
3. Implement captures, verifies, and routes kubernetes webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `kubernetes-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:webhook-listener` consumers with approval and credential context attached.
