...
description: Use when tasks require docker connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docker-connection-trace-bundle` |
| Routing tag | `docker:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Connector into a deterministic, auth-checked workflow for unified read/write connector for docker apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| docker-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| docker-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Docker Connector, then confirm the smallest read-only probe that proves access to Docker.
...
3. Specify the request and response contract for unified read/write connector for docker apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `docker-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docker-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:connector` consumers with approval and credential context attached.
