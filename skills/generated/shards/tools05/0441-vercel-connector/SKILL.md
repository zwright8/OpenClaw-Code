...
description: Use when tasks require vercel connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `vercel-connection-trace-bundle` |
| Routing tag | `vercel:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Connector into a deterministic, auth-checked workflow for unified read/write connector for vercel apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| vercel-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| vercel-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Vercel Connector, then confirm the smallest read-only probe that proves access to Vercel.
...
3. Specify the request and response contract for unified read/write connector for vercel apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `vercel-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] vercel-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:connector` consumers with approval and credential context attached.
