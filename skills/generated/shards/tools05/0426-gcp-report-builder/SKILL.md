---
name: gcp-report-builder
description: Use when tasks require gcp report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-report-bundle` |
| Routing tag | `gcp:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from gcp activity..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-report-bundle | structured-artifact | yes | next workflow or operator |
| gcp-report-bundle-scorecard | scorecard | yes | reviewer |
| gcp-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for GCP Report Builder, then confirm which GCP entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from gcp activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `gcp-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gcp-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:report-builder` consumers with approval and credential context attached.
