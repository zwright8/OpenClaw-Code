---
name: digitalocean-data-exporter
description: Use when tasks require digitalocean data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Data Exporter
...
| Primary artifact | `digitalocean-export-package` |
| Routing tag | `digitalocean:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Data Exporter into a deterministic, auth-checked workflow for exports digitalocean entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-export-package | structured-artifact | yes | next workflow or operator |
| digitalocean-export-package-scorecard | scorecard | yes | reviewer |
| digitalocean-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for DigitalOcean Data Exporter, then constrain the export scope for DigitalOcean before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports digitalocean entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `digitalocean-export-package` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:data-exporter` consumers with approval and credential context attached.
