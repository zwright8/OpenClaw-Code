---
name: gcp-data-importer
description: Use when tasks require gcp data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-import-ledger` |
| Routing tag | `gcp:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when state changes are hard to undo once they begin. This specific skill turns GCP Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into gcp..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-import-ledger | structured-artifact | yes | next workflow or operator |
| gcp-import-ledger-scorecard | scorecard | yes | reviewer |
| gcp-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for GCP Data Importer before any GCP write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into gcp. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `gcp-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] gcp-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:data-importer` consumers with approval and credential context attached.
