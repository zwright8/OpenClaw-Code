---
name: vercel-approval-router
description: Use when tasks require vercel approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Approval Router
...
| Primary artifact | `vercel-approval-decision-packet` |
| Routing tag | `vercel:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Vercel Approval Router into a deterministic, auth-checked workflow for routes vercel-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| vercel-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| vercel-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Vercel Approval Router, then capture the manual fallback path for Vercel.
...
3. Implement routes vercel-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `vercel-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] vercel-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:approval-router` consumers with approval and credential context attached.
