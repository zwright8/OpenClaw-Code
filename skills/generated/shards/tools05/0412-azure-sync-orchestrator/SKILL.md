---
name: azure-sync-orchestrator
description: Use when tasks require azure sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure Sync Orchestrator
...
| Primary artifact | `azure-reconciliation-bundle` |
| Routing tag | `azure:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for azure records..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| azure-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| azure-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Azure Sync Orchestrator, then checkpoint the last known good cursor on both sides of Azure.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for azure records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `azure-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:sync-orchestrator` consumers with approval and credential context attached.
