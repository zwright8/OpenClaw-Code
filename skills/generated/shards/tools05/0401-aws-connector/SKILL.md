---
name: aws-connector
description: Use when tasks require aws connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-connection-trace-bundle` |
| Routing tag | `aws:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Connector into a deterministic, auth-checked workflow for unified read/write connector for aws apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| aws-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| aws-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for AWS Connector, then confirm the smallest read-only probe that proves access to AWS.
...
3. Specify the request and response contract for unified read/write connector for aws apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `aws-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] aws-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:connector` consumers with approval and credential context attached.
