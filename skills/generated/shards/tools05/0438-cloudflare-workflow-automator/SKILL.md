---
name: cloudflare-workflow-automator
description: Use when tasks require cloudflare workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Workflow Automator
...
| Primary artifact | `cloudflare-workflow-runbook` |
| Routing tag | `cloudflare:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using cloudflare..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-workflow-runbook | structured-artifact | yes | next workflow or operator |
| cloudflare-workflow-runbook-scorecard | scorecard | yes | reviewer |
| cloudflare-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Cloudflare Workflow Automator, then checkpoint where Cloudflare state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using cloudflare. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `cloudflare-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:workflow-automator` consumers with approval and credential context attached.
