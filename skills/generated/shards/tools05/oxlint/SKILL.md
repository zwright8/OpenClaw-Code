...
This pack provides a curated operational toolkit for oxlint tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
1. Select a minimal tool from this pack to solve a oxlint request, run dry-run first, then produce a validated artifact bundle.
