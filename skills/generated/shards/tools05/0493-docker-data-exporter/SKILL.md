---
name: docker-data-exporter
description: Use when tasks require docker data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docker-export-package` |
| Routing tag | `docker:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Data Exporter into a deterministic, auth-checked workflow for exports docker entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-export-package | structured-artifact | yes | next workflow or operator |
| docker-export-package-scorecard | scorecard | yes | reviewer |
| docker-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Docker Data Exporter, then constrain the export scope for Docker before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports docker entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `docker-export-package` so downstream owners do not need to rediscover context.
...
- [ ] docker-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:data-exporter` consumers with approval and credential context attached.
