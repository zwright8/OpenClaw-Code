---
name: render-report-builder
description: Use when tasks require render report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `render-report-bundle` |
| Routing tag | `render:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from render activity..

## Trigger Checklist
- [ ] The task explicitly requires `Render Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-report-bundle | structured-artifact | yes | next workflow or operator |
| render-report-bundle-scorecard | scorecard | yes | reviewer |
| render-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Render Report Builder, then confirm which Render entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from render activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `render-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] render-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:report-builder` consumers with approval and credential context attached.
