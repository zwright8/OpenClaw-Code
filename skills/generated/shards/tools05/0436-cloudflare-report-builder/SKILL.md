---
name: cloudflare-report-builder
description: Use when tasks require cloudflare report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `cloudflare-report-bundle` |
| Routing tag | `cloudflare:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from cloudflare activity..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-report-bundle | structured-artifact | yes | next workflow or operator |
| cloudflare-report-bundle-scorecard | scorecard | yes | reviewer |
| cloudflare-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Cloudflare Report Builder, then confirm which Cloudflare entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from cloudflare activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `cloudflare-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:report-builder` consumers with approval and credential context attached.
