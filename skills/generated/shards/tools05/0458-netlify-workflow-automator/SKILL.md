---
name: netlify-workflow-automator
description: Use when tasks require netlify workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Workflow Automator
...
| Primary artifact | `netlify-workflow-runbook` |
| Routing tag | `netlify:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using netlify..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-workflow-runbook | structured-artifact | yes | next workflow or operator |
| netlify-workflow-runbook-scorecard | scorecard | yes | reviewer |
| netlify-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Netlify Workflow Automator, then checkpoint where Netlify state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using netlify. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `netlify-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] netlify-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:workflow-automator` consumers with approval and credential context attached.
