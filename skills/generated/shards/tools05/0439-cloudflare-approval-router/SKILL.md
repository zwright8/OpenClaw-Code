---
name: cloudflare-approval-router
description: Use when tasks require cloudflare approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Approval Router
...
| Primary artifact | `cloudflare-approval-decision-packet` |
| Routing tag | `cloudflare:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Approval Router into a deterministic, auth-checked workflow for routes cloudflare-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| cloudflare-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| cloudflare-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Cloudflare Approval Router, then capture the manual fallback path for Cloudflare.
...
3. Implement routes cloudflare-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `cloudflare-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:approval-router` consumers with approval and credential context attached.
