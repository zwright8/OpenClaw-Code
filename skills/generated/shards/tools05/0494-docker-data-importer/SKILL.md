---
name: docker-data-importer
description: Use when tasks require docker data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Data Importer
...
| Primary artifact | `docker-import-ledger` |
| Routing tag | `docker:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when state changes are hard to undo once they begin. This specific skill turns Docker Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into docker..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-import-ledger | structured-artifact | yes | next workflow or operator |
| docker-import-ledger-scorecard | scorecard | yes | reviewer |
| docker-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Docker Data Importer before any Docker write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into docker. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `docker-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] docker-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:data-importer` consumers with approval and credential context attached.
