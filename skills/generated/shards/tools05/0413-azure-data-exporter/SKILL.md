---
name: azure-data-exporter
description: Use when tasks require azure data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-export-package` |
| Routing tag | `azure:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Data Exporter into a deterministic, auth-checked workflow for exports azure entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-export-package | structured-artifact | yes | next workflow or operator |
| azure-export-package-scorecard | scorecard | yes | reviewer |
| azure-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Azure Data Exporter, then constrain the export scope for Azure before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports azure entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `azure-export-package` so downstream owners do not need to rediscover context.
...
- [ ] azure-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:data-exporter` consumers with approval and credential context attached.
