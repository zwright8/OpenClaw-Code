---
name: azure-data-importer
description: Use when tasks require azure data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-import-ledger` |
| Routing tag | `azure:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when state changes are hard to undo once they begin. This specific skill turns Azure Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into azure..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-import-ledger | structured-artifact | yes | next workflow or operator |
| azure-import-ledger-scorecard | scorecard | yes | reviewer |
| azure-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Azure Data Importer before any Azure write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into azure. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `azure-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] azure-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:data-importer` consumers with approval and credential context attached.
