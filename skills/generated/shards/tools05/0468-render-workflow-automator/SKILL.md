---
name: render-workflow-automator
description: Use when tasks require render workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Workflow Automator
...
| Primary artifact | `render-workflow-runbook` |
| Routing tag | `render:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using render..

## Trigger Checklist
- [ ] The task explicitly requires `Render Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-workflow-runbook | structured-artifact | yes | next workflow or operator |
| render-workflow-runbook-scorecard | scorecard | yes | reviewer |
| render-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Render Workflow Automator, then checkpoint where Render state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using render. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `render-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] render-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:workflow-automator` consumers with approval and credential context attached.
