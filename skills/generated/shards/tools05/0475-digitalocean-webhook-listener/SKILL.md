---
name: digitalocean-webhook-listener
description: Use when tasks require digitalocean webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Webhook Listener
...
| Primary artifact | `digitalocean-event-intake-log` |
| Routing tag | `digitalocean:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns DigitalOcean Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes digitalocean webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `DIGITALOCEAN_CLIENT_ID`, `DIGITALOCEAN_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `DIGITALOCEAN_TOKEN`, `DIGITALOCEAN_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DIGITALOCEAN_WEBHOOK_SECRET`, `DIGITALOCEAN_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| digitalocean-event-intake-log | structured-artifact | yes | next workflow or operator |
| digitalocean-event-intake-log-scorecard | scorecard | yes | reviewer |
| digitalocean-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for DigitalOcean Webhook Listener, then register the signing-secret policy for DigitalOcean.
...
3. Implement captures, verifies, and routes digitalocean webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `digitalocean-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:webhook-listener` consumers with approval and credential context attached.
