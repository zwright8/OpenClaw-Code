---
name: digitalocean-alert-monitor
description: Use when tasks require digitalocean alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Alert Monitor
...
| Primary artifact | `digitalocean-alert-state-bundle` |
| Routing tag | `digitalocean:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Alert Monitor into a deterministic, auth-checked workflow for monitors key digitalocean signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| digitalocean-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| digitalocean-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for DigitalOcean Alert Monitor, then lock the reviewed threshold set for DigitalOcean.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key digitalocean signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `digitalocean-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:alert-monitor` consumers with approval and credential context attached.
