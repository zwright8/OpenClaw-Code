---
name: digitalocean-report-builder
description: Use when tasks require digitalocean report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Report Builder
...
| Primary artifact | `digitalocean-report-bundle` |
| Routing tag | `digitalocean:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from digitalocean activity..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-report-bundle | structured-artifact | yes | next workflow or operator |
| digitalocean-report-bundle-scorecard | scorecard | yes | reviewer |
| digitalocean-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for DigitalOcean Report Builder, then confirm which DigitalOcean entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from digitalocean activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `digitalocean-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:report-builder` consumers with approval and credential context attached.
