---
name: render-data-importer
description: Use when tasks require render data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Data Importer
...
| Primary artifact | `render-import-ledger` |
| Routing tag | `render:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Render workflows degrade when state changes are hard to undo once they begin. This specific skill turns Render Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into render..

## Trigger Checklist
- [ ] The task explicitly requires `Render Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-import-ledger | structured-artifact | yes | next workflow or operator |
| render-import-ledger-scorecard | scorecard | yes | reviewer |
| render-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Render Data Importer before any Render write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into render. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `render-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] render-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:data-importer` consumers with approval and credential context attached.
