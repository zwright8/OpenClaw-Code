---
name: vercel-migration-assistant
description: Use when tasks require vercel migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Vercel Migration Assistant
...
| Primary artifact | `vercel-migration-cutover-plan` |
| Routing tag | `vercel:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when state changes are hard to undo once they begin. This specific skill turns Vercel Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from vercel..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| vercel-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| vercel-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Vercel Migration Assistant before any Vercel rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from vercel. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `vercel-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] vercel-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:migration-assistant` consumers with approval and credential context attached.
