---
name: kubernetes-approval-router
description: Use when tasks require kubernetes approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Approval Router
...
| Primary artifact | `kubernetes-approval-decision-packet` |
| Routing tag | `kubernetes:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Approval Router into a deterministic, auth-checked workflow for routes kubernetes-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| kubernetes-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| kubernetes-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Kubernetes Approval Router, then capture the manual fallback path for Kubernetes.
...
3. Implement routes kubernetes-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `kubernetes-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:approval-router` consumers with approval and credential context attached.
