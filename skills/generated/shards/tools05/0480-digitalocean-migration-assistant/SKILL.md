---
name: digitalocean-migration-assistant
description: Use when tasks require digitalocean migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Migration Assistant
...
| Primary artifact | `digitalocean-migration-cutover-plan` |
| Routing tag | `digitalocean:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when state changes are hard to undo once they begin. This specific skill turns DigitalOcean Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from digitalocean..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| digitalocean-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| digitalocean-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for DigitalOcean Migration Assistant before any DigitalOcean rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from digitalocean. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `digitalocean-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:migration-assistant` consumers with approval and credential context attached.
