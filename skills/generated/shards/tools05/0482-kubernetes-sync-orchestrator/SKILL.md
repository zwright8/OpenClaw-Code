---
name: kubernetes-sync-orchestrator
description: Use when tasks require kubernetes sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Kubernetes Sync Orchestrator
...
| Primary artifact | `kubernetes-reconciliation-bundle` |
| Routing tag | `kubernetes:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Kubernetes Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for kubernetes records..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| kubernetes-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| kubernetes-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Kubernetes Sync Orchestrator, then checkpoint the last known good cursor on both sides of Kubernetes.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for kubernetes records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `kubernetes-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:sync-orchestrator` consumers with approval and credential context attached.
