---
name: gcp-alert-monitor
description: Use when tasks require gcp alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gcp-alert-state-bundle` |
| Routing tag | `gcp:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GCP workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GCP Alert Monitor into a deterministic, auth-checked workflow for monitors key gcp signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `GCP Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gcp-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| gcp-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| gcp-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for GCP Alert Monitor, then lock the reviewed threshold set for GCP.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key gcp signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `gcp-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gcp-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gcp-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gcp:alert-monitor` consumers with approval and credential context attached.
