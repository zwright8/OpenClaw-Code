---
name: cloudflare-alert-monitor
description: Use when tasks require cloudflare alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Cloudflare Alert Monitor
...
| Primary artifact | `cloudflare-alert-state-bundle` |
| Routing tag | `cloudflare:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Cloudflare workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Cloudflare Alert Monitor into a deterministic, auth-checked workflow for monitors key cloudflare signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Cloudflare Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| cloudflare-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| cloudflare-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| cloudflare-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Cloudflare Alert Monitor, then lock the reviewed threshold set for Cloudflare.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key cloudflare signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `cloudflare-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] cloudflare-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `cloudflare-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `cloudflare:alert-monitor` consumers with approval and credential context attached.
