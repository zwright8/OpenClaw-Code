---
name: docker-migration-assistant
description: Use when tasks require docker migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Migration Assistant
...
| Primary artifact | `docker-migration-cutover-plan` |
| Routing tag | `docker:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when state changes are hard to undo once they begin. This specific skill turns Docker Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from docker..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| docker-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| docker-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Docker Migration Assistant before any Docker rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from docker. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `docker-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] docker-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:migration-assistant` consumers with approval and credential context attached.
