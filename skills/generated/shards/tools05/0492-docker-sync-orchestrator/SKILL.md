---
name: docker-sync-orchestrator
description: Use when tasks require docker sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Sync Orchestrator
...
| Primary artifact | `docker-reconciliation-bundle` |
| Routing tag | `docker:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for docker records..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| docker-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| docker-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Docker Sync Orchestrator, then checkpoint the last known good cursor on both sides of Docker.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for docker records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `docker-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docker-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:sync-orchestrator` consumers with approval and credential context attached.
