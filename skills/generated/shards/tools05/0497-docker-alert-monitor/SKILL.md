---
name: docker-alert-monitor
description: Use when tasks require docker alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Docker Alert Monitor
...
| Primary artifact | `docker-alert-state-bundle` |
| Routing tag | `docker:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Docker workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Docker Alert Monitor into a deterministic, auth-checked workflow for monitors key docker signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Docker Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docker-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| docker-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| docker-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Docker Alert Monitor, then lock the reviewed threshold set for Docker.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key docker signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `docker-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docker-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docker-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docker:alert-monitor` consumers with approval and credential context attached.
