---
name: azure-approval-router
description: Use when tasks require azure approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure Approval Router
...
| Primary artifact | `azure-approval-decision-packet` |
| Routing tag | `azure:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Approval Router into a deterministic, auth-checked workflow for routes azure-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| azure-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| azure-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Azure Approval Router, then capture the manual fallback path for Azure.
...
3. Implement routes azure-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `azure-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] azure-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:approval-router` consumers with approval and credential context attached.
