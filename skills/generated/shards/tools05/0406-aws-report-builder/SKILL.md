---
name: aws-report-builder
description: Use when tasks require aws report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-report-bundle` |
| Routing tag | `aws:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from aws activity..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-report-bundle | structured-artifact | yes | next workflow or operator |
| aws-report-bundle-scorecard | scorecard | yes | reviewer |
| aws-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for AWS Report Builder, then confirm which AWS entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from aws activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `aws-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] aws-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:report-builder` consumers with approval and credential context attached.
