...
description: Use when tasks require azure connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `azure-connection-trace-bundle` |
| Routing tag | `azure:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Connector into a deterministic, auth-checked workflow for unified read/write connector for azure apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| azure-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| azure-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Azure Connector, then confirm the smallest read-only probe that proves access to Azure.
...
3. Specify the request and response contract for unified read/write connector for azure apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `azure-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] azure-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:connector` consumers with approval and credential context attached.
