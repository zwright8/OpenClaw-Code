---
name: netlify-migration-assistant
description: Use when tasks require netlify migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Migration Assistant
...
| Primary artifact | `netlify-migration-cutover-plan` |
| Routing tag | `netlify:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when state changes are hard to undo once they begin. This specific skill turns Netlify Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from netlify..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| netlify-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| netlify-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Netlify Migration Assistant before any Netlify rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from netlify. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `netlify-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] netlify-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:migration-assistant` consumers with approval and credential context attached.
