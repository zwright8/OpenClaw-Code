...
description: Use when tasks require digitalocean connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `digitalocean-connection-trace-bundle` |
| Routing tag | `digitalocean:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Connector into a deterministic, auth-checked workflow for unified read/write connector for digitalocean apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| digitalocean-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| digitalocean-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for DigitalOcean Connector, then confirm the smallest read-only probe that proves access to DigitalOcean.
...
3. Specify the request and response contract for unified read/write connector for digitalocean apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `digitalocean-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:connector` consumers with approval and credential context attached.
