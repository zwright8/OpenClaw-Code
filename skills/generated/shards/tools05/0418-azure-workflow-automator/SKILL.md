---
name: azure-workflow-automator
description: Use when tasks require azure workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure Workflow Automator
...
| Primary artifact | `azure-workflow-runbook` |
| Routing tag | `azure:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Azure Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using azure..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| azure-workflow-runbook | structured-artifact | yes | next workflow or operator |
| azure-workflow-runbook-scorecard | scorecard | yes | reviewer |
| azure-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Azure Workflow Automator, then checkpoint where Azure state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using azure. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `azure-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] azure-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:workflow-automator` consumers with approval and credential context attached.
