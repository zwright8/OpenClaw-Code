---
name: aws-webhook-listener
description: Use when tasks require aws webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-event-intake-log` |
| Routing tag | `aws:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns AWS Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes aws webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `AWS_SERVICE_ACCOUNT_JSON`, `AWS_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `AWS_TOKEN`, `AWS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `AWS_WEBHOOK_SECRET`, `AWS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| aws-event-intake-log | structured-artifact | yes | next workflow or operator |
| aws-event-intake-log-scorecard | scorecard | yes | reviewer |
| aws-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for AWS Webhook Listener, then register the signing-secret policy for AWS.
...
3. Implement captures, verifies, and routes aws webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `aws-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] aws-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:webhook-listener` consumers with approval and credential context attached.
