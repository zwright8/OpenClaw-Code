---
name: aws-workflow-automator
description: Use when tasks require aws workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# AWS Workflow Automator
...
| Primary artifact | `aws-workflow-runbook` |
| Routing tag | `aws:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using aws..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-workflow-runbook | structured-artifact | yes | next workflow or operator |
| aws-workflow-runbook-scorecard | scorecard | yes | reviewer |
| aws-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for AWS Workflow Automator, then checkpoint where AWS state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using aws. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `aws-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] aws-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:workflow-automator` consumers with approval and credential context attached.
