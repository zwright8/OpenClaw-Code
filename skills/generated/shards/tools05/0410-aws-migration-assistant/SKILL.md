---
name: aws-migration-assistant
description: Use when tasks require aws migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-migration-cutover-plan` |
| Routing tag | `aws:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when state changes are hard to undo once they begin. This specific skill turns AWS Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from aws..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| aws-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| aws-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for AWS Migration Assistant before any AWS rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from aws. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `aws-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] aws-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:migration-assistant` consumers with approval and credential context attached.
