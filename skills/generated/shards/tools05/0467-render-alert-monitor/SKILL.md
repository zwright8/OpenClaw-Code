---
name: render-alert-monitor
description: Use when tasks require render alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Render Alert Monitor
...
| Primary artifact | `render-alert-state-bundle` |
| Routing tag | `render:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Render workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Render Alert Monitor into a deterministic, auth-checked workflow for monitors key render signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Render Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| render-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| render-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| render-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Render Alert Monitor, then lock the reviewed threshold set for Render.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key render signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `render-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] render-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `render-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `render:alert-monitor` consumers with approval and credential context attached.
