---
name: azure-webhook-listener
description: Use when tasks require azure webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Azure Webhook Listener
...
| Primary artifact | `azure-event-intake-log` |
| Routing tag | `azure:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Azure workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Azure Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes azure webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Azure Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `AZURE_SERVICE_ACCOUNT_JSON`, `AZURE_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Access token or personal access token | `token` | `AZURE_TOKEN`, `AZURE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `AZURE_WEBHOOK_SECRET`, `AZURE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| azure-event-intake-log | structured-artifact | yes | next workflow or operator |
| azure-event-intake-log-scorecard | scorecard | yes | reviewer |
| azure-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Azure Webhook Listener, then register the signing-secret policy for Azure.
...
3. Implement captures, verifies, and routes azure webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `azure-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] azure-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `azure-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `azure:webhook-listener` consumers with approval and credential context attached.
