---
name: aws-sync-orchestrator
description: Use when tasks require aws sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# AWS Sync Orchestrator
...
| Primary artifact | `aws-reconciliation-bundle` |
| Routing tag | `aws:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for aws records..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| aws-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| aws-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for AWS Sync Orchestrator, then checkpoint the last known good cursor on both sides of AWS.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for aws records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `aws-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] aws-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:sync-orchestrator` consumers with approval and credential context attached.
