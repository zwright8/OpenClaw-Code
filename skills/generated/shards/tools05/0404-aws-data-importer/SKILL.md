---
name: aws-data-importer
description: Use when tasks require aws data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-import-ledger` |
| Routing tag | `aws:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when state changes are hard to undo once they begin. This specific skill turns AWS Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into aws..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-import-ledger | structured-artifact | yes | next workflow or operator |
| aws-import-ledger-scorecard | scorecard | yes | reviewer |
| aws-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for AWS Data Importer before any AWS write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into aws. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `aws-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] aws-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:data-importer` consumers with approval and credential context attached.
