---
name: netlify-report-builder
description: Use when tasks require netlify report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Netlify Report Builder
...
| Primary artifact | `netlify-report-bundle` |
| Routing tag | `netlify:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Netlify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Netlify Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from netlify activity..

## Trigger Checklist
- [ ] The task explicitly requires `Netlify Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| netlify-report-bundle | structured-artifact | yes | next workflow or operator |
| netlify-report-bundle-scorecard | scorecard | yes | reviewer |
| netlify-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Netlify Report Builder, then confirm which Netlify entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from netlify activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `netlify-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] netlify-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `netlify-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `netlify:report-builder` consumers with approval and credential context attached.
