...
description: Use when tasks require aws alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `aws-alert-state-bundle` |
| Routing tag | `aws:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because AWS workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns AWS Alert Monitor into a deterministic, auth-checked workflow for monitors key aws signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `AWS Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| aws-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| aws-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| aws-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for AWS Alert Monitor, then lock the reviewed threshold set for AWS.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key aws signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `aws-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] aws-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `aws-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `aws:alert-monitor` consumers with approval and credential context attached.
