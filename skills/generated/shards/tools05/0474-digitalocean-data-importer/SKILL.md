---
name: digitalocean-data-importer
description: Use when tasks require digitalocean data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Data Importer
...
| Primary artifact | `digitalocean-import-ledger` |
| Routing tag | `digitalocean:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when state changes are hard to undo once they begin. This specific skill turns DigitalOcean Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into digitalocean..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-import-ledger | structured-artifact | yes | next workflow or operator |
| digitalocean-import-ledger-scorecard | scorecard | yes | reviewer |
| digitalocean-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for DigitalOcean Data Importer before any DigitalOcean write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into digitalocean. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `digitalocean-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:data-importer` consumers with approval and credential context attached.
