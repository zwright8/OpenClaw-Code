---
name: digitalocean-approval-router
description: Use when tasks require digitalocean approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DigitalOcean Approval Router
...
| Primary artifact | `digitalocean-approval-decision-packet` |
| Routing tag | `digitalocean:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DigitalOcean workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DigitalOcean Approval Router into a deterministic, auth-checked workflow for routes digitalocean-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `DigitalOcean Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| digitalocean-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| digitalocean-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| digitalocean-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for DigitalOcean Approval Router, then capture the manual fallback path for DigitalOcean.
...
3. Implement routes digitalocean-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `digitalocean-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] digitalocean-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `digitalocean-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `digitalocean:approval-router` consumers with approval and credential context attached.
