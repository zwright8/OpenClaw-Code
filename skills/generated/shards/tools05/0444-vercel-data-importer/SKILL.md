---
name: vercel-data-importer
description: Use when tasks require vercel data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `vercel-import-ledger` |
| Routing tag | `vercel:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Vercel workflows degrade when state changes are hard to undo once they begin. This specific skill turns Vercel Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into vercel..

## Trigger Checklist
- [ ] The task explicitly requires `Vercel Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| vercel-import-ledger | structured-artifact | yes | next workflow or operator |
| vercel-import-ledger-scorecard | scorecard | yes | reviewer |
| vercel-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Vercel Data Importer before any Vercel write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into vercel. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `vercel-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] vercel-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `vercel-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `vercel:data-importer` consumers with approval and credential context attached.
