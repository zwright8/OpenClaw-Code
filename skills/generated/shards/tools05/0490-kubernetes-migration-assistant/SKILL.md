---
name: kubernetes-migration-assistant
description: Use when tasks require kubernetes migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `kubernetes-migration-cutover-plan` |
| Routing tag | `kubernetes:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Kubernetes workflows degrade when state changes are hard to undo once they begin. This specific skill turns Kubernetes Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from kubernetes..

## Trigger Checklist
- [ ] The task explicitly requires `Kubernetes Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| kubernetes-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| kubernetes-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| kubernetes-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Kubernetes Migration Assistant before any Kubernetes rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from kubernetes. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `kubernetes-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] kubernetes-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `kubernetes-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `kubernetes:migration-assistant` consumers with approval and credential context attached.
