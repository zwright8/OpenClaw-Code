...
This pack provides a curated operational toolkit for zabbix tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
1. Select a minimal tool from this pack to solve a zabbix request, run dry-run first, then produce a validated artifact bundle.
