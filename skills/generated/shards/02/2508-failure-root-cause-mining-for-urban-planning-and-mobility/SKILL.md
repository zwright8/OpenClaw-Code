---
name: u2508-failure-root-cause-mining-for-urban-planning-and-mobility
description: Operate the "failure root cause mining for urban planning and mobility" capability in production for failure root cause mining for urban planning and mobility workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for urban planning and mobility

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for urban planning and mobility workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for urban planning and mobility` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for urban planning and mobility** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **urban planning and mobility" capability in production for failure root cause mining for urban planning and mobility workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new urban planning and mobility" capability in production for failure root cause mining for urban planning and mobility workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in urban planning and mobility" capability in production for failure root cause mining for urban planning and mobility workflows**
   - Input: noisy upstream payload requiring failure root cause mining for urban planning and mobility normalization/assessment.
