---
name: u2461-failure-root-cause-mining-for-design-and-user-research
description: Operate the "failure root cause mining for design and user research" capability in production for failure root cause mining for design and user research workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for design and user research

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for design and user research workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for design and user research` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for design and user research** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **design and user research" capability in production for failure root cause mining for design and user research workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new design and user research" capability in production for failure root cause mining for design and user research workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in design and user research" capability in production for failure root cause mining for design and user research workflows**
   - Input: noisy upstream payload requiring failure root cause mining for design and user research normalization/assessment.
