---
name: u2117-tool-health-monitoring-for-supply-chain-resilience
description: Operate the "tool health monitoring for supply chain resilience" capability in production for tool health monitoring for supply chain resilience workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# tool health monitoring for supply chain resilience

## Why This Skill Exists
This skill hardens a generated capability for production execution so tool health monitoring for supply chain resilience workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `tool health monitoring for supply chain resilience` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **tool health monitoring for supply chain resilience** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **supply chain resilience" capability in production for tool health monitoring for supply chain resilience workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new supply chain resilience" capability in production for tool health monitoring for supply chain resilience workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in supply chain resilience" capability in production for tool health monitoring for supply chain resilience workflows**
   - Input: noisy upstream payload requiring tool health monitoring for supply chain resilience normalization/assessment.
