---
name: u2466-failure-root-cause-mining-for-sales-and-client-success
description: Operate the "failure root cause mining for sales and client success" capability in production for failure root cause mining for sales and client success workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for sales and client success

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for sales and client success workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for sales and client success` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for sales and client success** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **sales and client success" capability in production for failure root cause mining for sales and client success workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new sales and client success" capability in production for failure root cause mining for sales and client success workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in sales and client success" capability in production for failure root cause mining for sales and client success workflows**
   - Input: noisy upstream payload requiring failure root cause mining for sales and client success normalization/assessment.
