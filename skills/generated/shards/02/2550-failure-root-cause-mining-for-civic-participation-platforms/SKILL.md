---
name: u2550-failure-root-cause-mining-for-civic-participation-platforms
description: Operate the "failure root cause mining for civic participation platforms" capability in production for failure root cause mining for civic participation platforms workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for civic participation platforms

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for civic participation platforms workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for civic participation platforms` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for civic participation platforms** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **civic participation platforms" capability in production for failure root cause mining for civic participation platforms workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new civic participation platforms" capability in production for failure root cause mining for civic participation platforms workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in civic participation platforms" capability in production for failure root cause mining for civic participation platforms workflows**
   - Input: noisy upstream payload requiring failure root cause mining for civic participation platforms normalization/assessment.
