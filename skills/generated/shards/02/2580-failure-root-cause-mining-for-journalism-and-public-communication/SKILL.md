---
name: u2580-failure-root-cause-mining-for-journalism-and-public-communication
description: Operate the "failure root cause mining for journalism and public communication" capability in production for failure root cause mining for journalism and public communication workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for journalism and public communication

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for journalism and public communication workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for journalism and public communication` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for journalism and public communication** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **journalism and public communication" capability in production for failure root cause mining for journalism and public communication workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new journalism and public communication" capability in production for failure root cause mining for journalism and public communication workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in journalism and public communication" capability in production for failure root cause mining for journalism and public communication workflows**
   - Input: noisy upstream payload requiring failure root cause mining for journalism and public communication normalization/assessment.
