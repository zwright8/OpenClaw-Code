---
name: u2426-failure-root-cause-mining-for-fitness-and-recovery-training
description: Operate the "failure root cause mining for fitness and recovery training" capability in production for failure root cause mining for fitness and recovery training workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for fitness and recovery training

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for fitness and recovery training workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for fitness and recovery training` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for fitness and recovery training** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **fitness and recovery training" capability in production for failure root cause mining for fitness and recovery training workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new fitness and recovery training" capability in production for failure root cause mining for fitness and recovery training workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in fitness and recovery training" capability in production for failure root cause mining for fitness and recovery training workflows**
   - Input: noisy upstream payload requiring failure root cause mining for fitness and recovery training normalization/assessment.
