---
name: u02084-tool-health-monitoring-for-healthcare-operations
description: Run the Tool Health Monitoring for healthcare operations capability for healthcare operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Primary artifact | `tool-health-monitoring-artifact-healthcare-operations` |
| Routing tag | `healthcare-operations:monitoring-engine` |
| Feature flag | `skill_02084_tool-health-monitoring` |
| Release cycles | `2` |

## Why This Skill Exists
Use tool health monitoring in healthcare operations with emphasis on throughput, reliability, leverage, and execution speed.

## Trigger Checklist
- [ ] The task explicitly needs Tool Health Monitoring for healthcare operations (not generic brainstorming).
...
|---|---|---|---|
| tool-health-monitoring-artifact-healthcare-operations | structured-artifact | yes | downstream orchestrator |
| tool-health-monitoring-artifact-healthcare-operations-scorecard | scorecard | yes | operator / reviewer |
| tool-health-monitoring-artifact-healthcare-operations-handoff | handoff-packet | yes | next owner |
...
5. Run unit, integration, simulation, and regression suites for Tool Health Monitoring for healthcare operations under hyper-productive autonomy conditions.
...
## Operator Use Cases
- Run Tool Health Monitoring for healthcare operations as a repeatable production workflow for humans and agents.
- Use Tool Health Monitoring for healthcare operations to accelerate decisions while preserving safety, quality, and auditability.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `healthcare-operations:monitoring-engine`.
...
## Traceability Map
- **Scope:** Define measurable outcomes for Tool Health Monitoring for healthcare operations, including baseline and target metrics for healthcare operations.
...
- **Validation:** Run unit, integration, simulation, and regression suites for Tool Health Monitoring for healthcare operations under hyper-productive autonomy conditions.
...
1. `Prometheus + Alertmanager` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `tool-health-monitoring-artifact-healthcare-operations`.
2. `OpenTelemetry Collector` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `tool-health-monitoring-artifact-healthcare-operations`.
3. `Grafana/Metabase/Superset` -> auth preflight, execute read+write/orchestrate call(s), normalize output, and attach trace to `tool-health-monitoring-artifact-healthcare-operations`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `tool-health-monitoring-artifact-healthcare-operations`.
...
1. Incident recovery in healthcare operations: ingest noisy signals, execute tool health monitoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tool Health Monitoring for healthcare operations against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-operations:monitoring-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
## Handoff Contract
- **Produces:** `tool-health-monitoring-artifact-healthcare-operations`, scorecard, risk/confidence metadata, remediation backlog.
- **Consumes:** `queue load`, `cycle time`, `throughput metrics`.
- **Readiness rule:** release only when schema, determinism, policy, and reliability gates all pass.
- **Downstream hint:** route only to `healthcare-operations:monitoring-engine` consumers with approval context attached.
