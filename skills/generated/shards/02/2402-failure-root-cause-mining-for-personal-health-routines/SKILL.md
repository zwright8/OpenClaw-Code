---
name: u2402-failure-root-cause-mining-for-personal-health-routines
description: Operate the "failure root cause mining for personal health routines" capability in production for failure root cause mining for personal health routines workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for personal health routines

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for personal health routines workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for personal health routines` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for personal health routines** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **personal health routines" capability in production for failure root cause mining for personal health routines workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new personal health routines" capability in production for failure root cause mining for personal health routines workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in personal health routines" capability in production for failure root cause mining for personal health routines workflows**
   - Input: noisy upstream payload requiring failure root cause mining for personal health routines normalization/assessment.
