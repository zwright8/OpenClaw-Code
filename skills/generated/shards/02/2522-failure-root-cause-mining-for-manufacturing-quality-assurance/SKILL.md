---
name: u2522-failure-root-cause-mining-for-manufacturing-quality-assurance
description: Operate the "failure root cause mining for manufacturing quality assurance" capability in production for failure root cause mining for manufacturing quality assurance workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---

# failure root cause mining for manufacturing quality assurance

## Why This Skill Exists
This skill hardens a generated capability for production execution so failure root cause mining for manufacturing quality assurance workflows remain deterministic, auditable, and fail-closed under risk.

## When To Use
Use this skill only when the request explicitly needs `failure root cause mining for manufacturing quality assurance` and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **failure root cause mining for manufacturing quality assurance** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **manufacturing quality assurance" capability in production for failure root cause mining for manufacturing quality assurance workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new manufacturing quality assurance" capability in production for failure root cause mining for manufacturing quality assurance workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in manufacturing quality assurance" capability in production for failure root cause mining for manufacturing quality assurance workflows**
   - Input: noisy upstream payload requiring failure root cause mining for manufacturing quality assurance normalization/assessment.
