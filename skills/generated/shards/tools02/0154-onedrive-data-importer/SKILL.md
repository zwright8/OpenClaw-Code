---
name: onedrive-data-importer
description: Use when tasks require onedrive data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Data Importer
...
| Primary artifact | `onedrive-import-ledger` |
| Routing tag | `onedrive:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when state changes are hard to undo once they begin. This specific skill turns OneDrive Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into onedrive..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-import-ledger | structured-artifact | yes | next workflow or operator |
| onedrive-import-ledger-scorecard | scorecard | yes | reviewer |
| onedrive-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for OneDrive Data Importer before any OneDrive write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into onedrive. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `onedrive-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:data-importer` consumers with approval and credential context attached.
