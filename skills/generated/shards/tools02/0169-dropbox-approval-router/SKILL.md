---
name: dropbox-approval-router
description: Use when tasks require dropbox approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Approval Router
...
| Primary artifact | `dropbox-approval-decision-packet` |
| Routing tag | `dropbox:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Approval Router into a deterministic, auth-checked workflow for routes dropbox-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| dropbox-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| dropbox-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Dropbox Approval Router, then capture the manual fallback path for Dropbox.
...
3. Implement routes dropbox-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `dropbox-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:approval-router` consumers with approval and credential context attached.
