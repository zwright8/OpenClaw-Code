...
description: Use when tasks require outlook connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `outlook-connection-trace-bundle` |
| Routing tag | `outlook:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Connector into a deterministic, auth-checked workflow for unified read/write connector for outlook apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| outlook-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| outlook-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Outlook Connector, then confirm the smallest read-only probe that proves access to Outlook.
...
3. Specify the request and response contract for unified read/write connector for outlook apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `outlook-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] outlook-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:connector` consumers with approval and credential context attached.
