---
name: outlook-data-exporter
description: Use when tasks require outlook data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `outlook-export-package` |
| Routing tag | `outlook:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Data Exporter into a deterministic, auth-checked workflow for exports outlook entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-export-package | structured-artifact | yes | next workflow or operator |
| outlook-export-package-scorecard | scorecard | yes | reviewer |
| outlook-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Outlook Data Exporter, then constrain the export scope for Outlook before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports outlook entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `outlook-export-package` so downstream owners do not need to rediscover context.
...
- [ ] outlook-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:data-exporter` consumers with approval and credential context attached.
