...
| Field | Value |
|---|---|
| Skill pack | `ai-assistant-and-coding-clis` |
| Tool count | `8` |
| Inventory rank span | `502-995` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for ai-assistant-and-coding-clis tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `qwen-code` | `block-goose-cli` | no | no |
| bounded mutation / transformation | `block-goose-cli` | `qwen-code` | yes | yes |
| validation & artifact checks | `qwen-code` | `block-goose-cli` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `qwen-code` | AI-powered command-line workflow tool for developers | local-installable |
| `block-goose-cli` | Open source, extensible AI agent that goes beyond code suggestions | local-installable |
| `aider` | AI pair programming in your terminal | local-installable |
| `fabric-ai` | Open-source framework for augmenting humans using AI | local-installable |
| `repomix` | Pack repository contents into a single AI-friendly file | local-installable |
| `llm` | Access large language models from the command-line | local-installable |
| `code-server` | Access VS Code through the browser | local-installable |
| `cliproxyapi` | Wrap Gemini CLI, Codex, Claude Code, Qwen Code as an API service | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `qwen-code` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `block-goose-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aider` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fabric-ai` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `repomix` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `llm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `code-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cliproxyapi` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `qwen-code` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `block-goose-cli` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `aider` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `fabric-ai` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `repomix` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `llm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a ai-assistant-and-coding-clis request, run dry-run first, then produce a validated artifact bundle.
