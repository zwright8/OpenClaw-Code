...
description: Use when tasks require airtable connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-connection-trace-bundle` |
| Routing tag | `airtable:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Connector into a deterministic, auth-checked workflow for unified read/write connector for airtable apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| airtable-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| airtable-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Airtable Connector, then confirm the smallest read-only probe that proves access to Airtable.
...
3. Specify the request and response contract for unified read/write connector for airtable apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `airtable-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] airtable-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:connector` consumers with approval and credential context attached.
