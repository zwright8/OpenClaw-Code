---
name: gmail-approval-router
description: Use when tasks require gmail approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Gmail Approval Router
...
| Primary artifact | `gmail-approval-decision-packet` |
| Routing tag | `gmail:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Approval Router into a deterministic, auth-checked workflow for routes gmail-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| gmail-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| gmail-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Gmail Approval Router, then capture the manual fallback path for Gmail.
...
3. Implement routes gmail-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `gmail-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] gmail-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:approval-router` consumers with approval and credential context attached.
