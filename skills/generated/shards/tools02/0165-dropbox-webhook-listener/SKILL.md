---
name: dropbox-webhook-listener
description: Use when tasks require dropbox webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Webhook Listener
...
| Primary artifact | `dropbox-event-intake-log` |
| Routing tag | `dropbox:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Dropbox Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes dropbox webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `DROPBOX_CLIENT_ID`, `DROPBOX_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `DROPBOX_WEBHOOK_SECRET`, `DROPBOX_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `DROPBOX_TOKEN`, `DROPBOX_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| dropbox-event-intake-log | structured-artifact | yes | next workflow or operator |
| dropbox-event-intake-log-scorecard | scorecard | yes | reviewer |
| dropbox-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Dropbox Webhook Listener, then register the signing-secret policy for Dropbox.
...
3. Implement captures, verifies, and routes dropbox webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `dropbox-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:webhook-listener` consumers with approval and credential context attached.
