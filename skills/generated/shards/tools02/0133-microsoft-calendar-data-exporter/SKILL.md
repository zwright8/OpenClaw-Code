---
name: microsoft-calendar-data-exporter
description: Use when tasks require microsoft calendar data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Data Exporter
...
| Primary artifact | `microsoft-calendar-export-package` |
| Routing tag | `microsoft-calendar:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Data Exporter into a deterministic, auth-checked workflow for exports microsoft calendar entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-export-package | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-export-package-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Microsoft Calendar Data Exporter, then constrain the export scope for Microsoft Calendar before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports microsoft calendar entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `microsoft-calendar-export-package` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:data-exporter` consumers with approval and credential context attached.
