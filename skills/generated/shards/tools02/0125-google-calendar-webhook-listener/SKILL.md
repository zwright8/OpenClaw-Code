---
name: google-calendar-webhook-listener
description: Use when tasks require google calendar webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `google-calendar-event-intake-log` |
| Routing tag | `google-calendar:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Google Calendar Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes google calendar webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `GOOGLE_CALENDAR_CLIENT_ID`, `GOOGLE_CALENDAR_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `GOOGLE_CALENDAR_WEBHOOK_SECRET`, `GOOGLE_CALENDAR_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `GOOGLE_CALENDAR_TOKEN`, `GOOGLE_CALENDAR_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| google-calendar-event-intake-log | structured-artifact | yes | next workflow or operator |
| google-calendar-event-intake-log-scorecard | scorecard | yes | reviewer |
| google-calendar-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Google Calendar Webhook Listener, then register the signing-secret policy for Google Calendar.
...
3. Implement captures, verifies, and routes google calendar webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `google-calendar-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:webhook-listener` consumers with approval and credential context attached.
