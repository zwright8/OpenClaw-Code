---
name: coda-report-builder
description: Use when tasks require coda report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `coda-report-bundle` |
| Routing tag | `coda:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from coda activity..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-report-bundle | structured-artifact | yes | next workflow or operator |
| coda-report-bundle-scorecard | scorecard | yes | reviewer |
| coda-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Coda Report Builder, then confirm which Coda entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from coda activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `coda-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] coda-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:report-builder` consumers with approval and credential context attached.
