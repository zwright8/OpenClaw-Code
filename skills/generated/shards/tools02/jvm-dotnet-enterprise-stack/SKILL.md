---
name: jvm-dotnet-enterprise-stack
description: Run JVM/.NET enterprise build and runtime tooling. Use when managing Java or .NET projects, CI servers, load testing, language servers, or enterprise app packaging.
---

# jvm-dotnet-enterprise-stack

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `jvm-dotnet-enterprise-stack` |
| Tool count | `15` |
| Inventory rank span | `542-992` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for jvm-dotnet-enterprise-stack tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `nuget` | `scala` | no | no |
| bounded mutation / transformation | `scala` | `nuget` | yes | yes |
| validation & artifact checks | `nuget` | `scala` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `nuget` | Package manager for Microsoft development platform including .NET | local-installable |
| `scala` | JVM-based programming language | local-installable |
| `jenkins-lts` | Extendable open source continuous integration server | local-installable |
| `openjdk@8` | Development kit for the Java programming language | local-installable |
| `dotnet@8` | .NET Core | local-installable |
| `ant` | Java build tool | local-installable |
| `bundletool` | Command-line tool to manipulate Android App Bundles | local-installable |
| `groovy` | Java-based scripting language | local-installable |
| `jmeter` | Load testing and performance measurement application | local-installable |
| `tomcat` | Implementation of Java Servlet and JavaServer Pages | local-installable |
| `jenkins` | Extendable open source continuous integration server | local-installable |
| `exercism` | Command-line tool to interact with exercism.io | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `nuget` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scala` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jenkins-lts` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openjdk@8` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotnet@8` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ant` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bundletool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `groovy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jmeter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tomcat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jenkins` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `exercism` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ktlint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jdtls` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clojure` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `nuget` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `scala` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `jenkins-lts` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `openjdk@8` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `dotnet@8` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `ant` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a jvm-dotnet-enterprise-stack request, run dry-run first, then produce a validated artifact bundle.
