---
name: git-ci-and-release-automation
description: Automate source-control workflows, CI orchestration, and release hygiene. Use when managing branching models, Git hooks, changelog/release uploads, or dependency update bots.
---

# git-ci-and-release-automation

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `git-ci-and-release-automation` |
| Tool count | `17` |
| Inventory rank span | `535-990` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for git-ci-and-release-automation tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `libgit2` | `lychee` | no | no |
| bounded mutation / transformation | `lychee` | `libgit2` | yes | yes |
| validation & artifact checks | `libgit2` | `lychee` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `libgit2` | C library of Git core methods that is re-entrant and linkable | local-installable |
| `lychee` | Fast, async, resource-friendly link checker | local-installable |
| `circleci` | Enables you to reproduce the CircleCI environment locally | local-installable |
| `gitlab-runner` | Official GitLab CI runner | local-installable |
| `renovate` | Automated dependency updates. Flexible so you don't need to be | local-installable |
| `tig` | Text interface for Git repositories | local-installable |
| `ghr` | Upload multiple artifacts to GitHub Release in parallel | local-installable |
| `git-flow` | Extensions to follow Vincent Driessen's branching model | local-installable |
| `sccache` | Used as a compiler wrapper and avoids compilation when possible | local-installable |
| `difftastic` | Diff that understands syntax | local-installable |
| `commitizen` | Defines a standard way of committing rules and communicating it | local-installable |
| `bfg` | Remove large files or passwords from Git history like git-filter-branch | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `libgit2` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lychee` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `circleci` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gitlab-runner` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `renovate` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ghr` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `git-flow` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sccache` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `difftastic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `commitizen` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bfg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lefthook` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fetch` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `tokei` | JSON-RPC/WebSocket, QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hub` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gitui` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `fetch`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `ghr`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `gitlab-runner`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `hub`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `libgit2` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `lychee` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `circleci` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `gitlab-runner` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `renovate` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `tig` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a git-ci-and-release-automation request, run dry-run first, then produce a validated artifact bundle.
