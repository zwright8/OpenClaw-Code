---
name: dropbox-workflow-automator
description: Use when tasks require dropbox workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Workflow Automator
...
| Primary artifact | `dropbox-workflow-runbook` |
| Routing tag | `dropbox:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using dropbox..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-workflow-runbook | structured-artifact | yes | next workflow or operator |
| dropbox-workflow-runbook-scorecard | scorecard | yes | reviewer |
| dropbox-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Dropbox Workflow Automator, then checkpoint where Dropbox state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using dropbox. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `dropbox-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:workflow-automator` consumers with approval and credential context attached.
