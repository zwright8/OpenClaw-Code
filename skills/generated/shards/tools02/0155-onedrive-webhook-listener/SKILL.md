---
name: onedrive-webhook-listener
description: Use when tasks require onedrive webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Webhook Listener
...
| Primary artifact | `onedrive-event-intake-log` |
| Routing tag | `onedrive:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns OneDrive Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes onedrive webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `ONEDRIVE_CLIENT_ID`, `ONEDRIVE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `ONEDRIVE_WEBHOOK_SECRET`, `ONEDRIVE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `ONEDRIVE_TOKEN`, `ONEDRIVE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| onedrive-event-intake-log | structured-artifact | yes | next workflow or operator |
| onedrive-event-intake-log-scorecard | scorecard | yes | reviewer |
| onedrive-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for OneDrive Webhook Listener, then register the signing-secret policy for OneDrive.
...
3. Implement captures, verifies, and routes onedrive webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `onedrive-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:webhook-listener` consumers with approval and credential context attached.
