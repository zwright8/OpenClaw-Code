---
name: google-drive-alert-monitor
description: Use when tasks require google drive alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Alert Monitor
...
| Primary artifact | `google-drive-alert-state-bundle` |
| Routing tag | `google-drive:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Drive Alert Monitor into a deterministic, auth-checked workflow for monitors key google drive signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| google-drive-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| google-drive-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Google Drive Alert Monitor, then lock the reviewed threshold set for Google Drive.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key google drive signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `google-drive-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:alert-monitor` consumers with approval and credential context attached.
