---
name: gmail-data-exporter
description: Use when tasks require gmail data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-export-package` |
| Routing tag | `gmail:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Data Exporter into a deterministic, auth-checked workflow for exports gmail entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-export-package | structured-artifact | yes | next workflow or operator |
| gmail-export-package-scorecard | scorecard | yes | reviewer |
| gmail-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Gmail Data Exporter, then constrain the export scope for Gmail before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports gmail entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `gmail-export-package` so downstream owners do not need to rediscover context.
...
- [ ] gmail-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:data-exporter` consumers with approval and credential context attached.
