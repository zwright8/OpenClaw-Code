...
description: Use when tasks require airtable workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-workflow-runbook` |
| Routing tag | `airtable:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using airtable..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-workflow-runbook | structured-artifact | yes | next workflow or operator |
| airtable-workflow-runbook-scorecard | scorecard | yes | reviewer |
| airtable-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Airtable Workflow Automator, then checkpoint where Airtable state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using airtable. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `airtable-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] airtable-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:workflow-automator` consumers with approval and credential context attached.
