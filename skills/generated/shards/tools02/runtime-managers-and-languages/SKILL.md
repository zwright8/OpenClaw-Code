---
name: runtime-managers-and-languages
description: Manage language runtimes and ecosystem-specific package/version managers. Use when bootstrapping language environments or switching runtime versions across projects.
---
...
| Field | Value |
|---|---|
| Skill pack | `runtime-managers-and-languages` |
| Tool count | `19` |
| Inventory rank span | `587-981` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for runtime-managers-and-languages tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `mit-scheme` | `chicken` | no | no |
| bounded mutation / transformation | `chicken` | `mit-scheme` | yes | yes |
| validation & artifact checks | `mit-scheme` | `chicken` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `mit-scheme` | MIT/GNU Scheme development tools and runtime library | local-installable |
| `chicken` | Compiler for the Scheme programming language | local-installable |
| `octave` | High-level interpreted language for numerical computing | local-installable |
| `pixi` | Package management made easy | local-installable |
| `ruby-install` | Install Ruby, JRuby, Rubinius, TruffleRuby, or mruby | local-installable |
| `lua-language-server` | Language Server for the Lua language | local-installable |
| `ta-lib` | Tools for market analysis | local-installable |
| `sbcl` | Steel Bank Common Lisp system | local-installable |
| `micromamba` | Fast Cross-Platform Package Manager | local-installable |
| `ruby@3.2` | Powerful, clean, object-oriented scripting language | local-installable |
| `opam` | OCaml package manager | local-installable |
| `julia` | Fast, Dynamic Programming Language | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `mit-scheme` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `chicken` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `octave` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pixi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruby-install` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lua-language-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ta-lib` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sbcl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `micromamba` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruby@3.2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opam` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `julia` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pmix` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tcl-tk@8` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `php@8.4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chruby` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `solidity` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `root` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruby@3.3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `mit-scheme` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `chicken` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `octave` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `pixi` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `ruby-install` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `lua-language-server` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a runtime-managers-and-languages request, run dry-run first, then produce a validated artifact bundle.
