...
description: Use when tasks require gmail workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-workflow-runbook` |
| Routing tag | `gmail:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using gmail..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-workflow-runbook | structured-artifact | yes | next workflow or operator |
| gmail-workflow-runbook-scorecard | scorecard | yes | reviewer |
| gmail-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Gmail Workflow Automator, then checkpoint where Gmail state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using gmail. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `gmail-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] gmail-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:workflow-automator` consumers with approval and credential context attached.
