---
name: gmail-data-importer
description: Use when tasks require gmail data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-import-ledger` |
| Routing tag | `gmail:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when state changes are hard to undo once they begin. This specific skill turns Gmail Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into gmail..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-import-ledger | structured-artifact | yes | next workflow or operator |
| gmail-import-ledger-scorecard | scorecard | yes | reviewer |
| gmail-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Gmail Data Importer before any Gmail write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into gmail. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `gmail-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] gmail-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:data-importer` consumers with approval and credential context attached.
