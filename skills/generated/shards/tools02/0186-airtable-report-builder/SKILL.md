---
name: airtable-report-builder
description: Use when tasks require airtable report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-report-bundle` |
| Routing tag | `airtable:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from airtable activity..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-report-bundle | structured-artifact | yes | next workflow or operator |
| airtable-report-bundle-scorecard | scorecard | yes | reviewer |
| airtable-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Airtable Report Builder, then confirm which Airtable entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from airtable activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `airtable-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] airtable-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:report-builder` consumers with approval and credential context attached.
