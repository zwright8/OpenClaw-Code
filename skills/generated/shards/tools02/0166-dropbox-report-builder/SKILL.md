---
name: dropbox-report-builder
description: Use when tasks require dropbox report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Report Builder
...
| Primary artifact | `dropbox-report-bundle` |
| Routing tag | `dropbox:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from dropbox activity..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-report-bundle | structured-artifact | yes | next workflow or operator |
| dropbox-report-bundle-scorecard | scorecard | yes | reviewer |
| dropbox-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Dropbox Report Builder, then confirm which Dropbox entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from dropbox activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `dropbox-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:report-builder` consumers with approval and credential context attached.
