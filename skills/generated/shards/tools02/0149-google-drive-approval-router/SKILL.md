---
name: google-drive-approval-router
description: Use when tasks require google drive approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Approval Router
...
| Primary artifact | `google-drive-approval-decision-packet` |
| Routing tag | `google-drive:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Drive Approval Router into a deterministic, auth-checked workflow for routes google drive-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| google-drive-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| google-drive-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Google Drive Approval Router, then capture the manual fallback path for Google Drive.
...
3. Implement routes google drive-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `google-drive-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:approval-router` consumers with approval and credential context attached.
