---
name: box-sync-orchestrator
description: Use when tasks require box sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Box Sync Orchestrator
...
| Primary artifact | `box-reconciliation-bundle` |
| Routing tag | `box:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for box records..

## Trigger Checklist
- [ ] The task explicitly requires `Box Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| box-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| box-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Box Sync Orchestrator, then checkpoint the last known good cursor on both sides of Box.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for box records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `box-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] box-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:sync-orchestrator` consumers with approval and credential context attached.
