---
name: onedrive-approval-router
description: Use when tasks require onedrive approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Approval Router
...
| Primary artifact | `onedrive-approval-decision-packet` |
| Routing tag | `onedrive:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Approval Router into a deterministic, auth-checked workflow for routes onedrive-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| onedrive-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| onedrive-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for OneDrive Approval Router, then capture the manual fallback path for OneDrive.
...
3. Implement routes onedrive-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `onedrive-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:approval-router` consumers with approval and credential context attached.
