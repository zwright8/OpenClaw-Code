---
name: coda-workflow-automator
description: Use when tasks require coda workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Coda Workflow Automator
...
| Primary artifact | `coda-workflow-runbook` |
| Routing tag | `coda:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using coda..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-workflow-runbook | structured-artifact | yes | next workflow or operator |
| coda-workflow-runbook-scorecard | scorecard | yes | reviewer |
| coda-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Coda Workflow Automator, then checkpoint where Coda state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using coda. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `coda-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] coda-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:workflow-automator` consumers with approval and credential context attached.
