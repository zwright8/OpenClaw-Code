---
name: coda-sync-orchestrator
description: Use when tasks require coda sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Coda Sync Orchestrator
...
| Primary artifact | `coda-reconciliation-bundle` |
| Routing tag | `coda:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for coda records..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| coda-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| coda-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Coda Sync Orchestrator, then checkpoint the last known good cursor on both sides of Coda.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for coda records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `coda-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] coda-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:sync-orchestrator` consumers with approval and credential context attached.
