---
name: google-calendar-data-exporter
description: Use when tasks require google calendar data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Data Exporter
...
| Primary artifact | `google-calendar-export-package` |
| Routing tag | `google-calendar:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Data Exporter into a deterministic, auth-checked workflow for exports google calendar entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-export-package | structured-artifact | yes | next workflow or operator |
| google-calendar-export-package-scorecard | scorecard | yes | reviewer |
| google-calendar-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Google Calendar Data Exporter, then constrain the export scope for Google Calendar before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports google calendar entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `google-calendar-export-package` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:data-exporter` consumers with approval and credential context attached.
