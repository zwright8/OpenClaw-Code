---
name: go-toolchain-and-release
description: Operate Go language development and release toolchain. Use when switching Go versions, debugging Go services, generating protobuf stubs, creating mocks, or shipping binaries.
---

# go-toolchain-and-release

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `go-toolchain-and-release` |
| Tool count | `9` |
| Inventory rank span | `662-916` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for go-toolchain-and-release tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `goreleaser` | `mockery` | no | no |
| bounded mutation / transformation | `mockery` | `goreleaser` | yes | yes |
| validation & artifact checks | `goreleaser` | `mockery` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `goreleaser` | Deliver Go binaries as fast and easily as possible | local-installable |
| `mockery` | Mock code autogenerator for Golang | local-installable |
| `protoc-gen-go` | Go support for Google's protocol buffers | local-installable |
| `cargo-binstall` | Binary installation for rust projects | local-installable |
| `goenv` | Go version management | local-installable |
| `gopls` | Language server for the Go language | local-installable |
| `go@1.23` | Open source programming language to build simple/reliable/efficient software | local-installable |
| `go@1.24` | Open source programming language to build simple/reliable/efficient software | local-installable |
| `delve` | Debugger for the Go programming language | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `goreleaser` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mockery` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `protoc-gen-go` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-binstall` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `goenv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gopls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `go@1.23` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `go@1.24` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `delve` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `goreleaser` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `mockery` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `protoc-gen-go` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `cargo-binstall` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `goenv` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `gopls` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a go-toolchain-and-release request, run dry-run first, then produce a validated artifact bundle.
