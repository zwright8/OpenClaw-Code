---
name: mobile-and-embedded-devices
description: Operate iOS/Android and embedded device CLIs. Use when flashing firmware, managing mobile app artifacts, debugging device installs, or automating hardware-focused build flows.
---
...
| Field | Value |
|---|---|
| Skill pack | `mobile-and-embedded-devices` |
| Tool count | `16` |
| Inventory rank span | `524-991` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for mobile-and-embedded-devices tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `winetricks` | `jadx` | no | no |
| bounded mutation / transformation | `jadx` | `winetricks` | yes | yes |
| validation & artifact checks | `winetricks` | `jadx` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `winetricks` | Automatic workarounds for problems in Wine | local-installable |
| `jadx` | Dex to Java decompiler | local-installable |
| `ios-deploy` | Install and debug iPhone apps from the command-line | local-installable |
| `swift-format` | Formatting technology for Swift source code | local-installable |
| `carthage` | Decentralized dependency manager for Cocoa | local-installable |
| `esphome` | Make creating custom firmwares for ESP32/ESP8266 super easy | local-installable |
| `ideviceinstaller` | Tool for managing apps on iOS devices | local-installable |
| `platformio` | Your Gateway to Embedded Software Development Excellence | local-installable |
| `arduino-cli` | Arduino command-line interface | local-installable |
| `dfu-util` | USB programmer | local-installable |
| `libusbmuxd` | USB multiplexor library for iOS devices | local-installable |
| `nim` | Statically typed compiled systems programming language | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `winetricks` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jadx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ios-deploy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swift-format` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `carthage` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `esphome` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ideviceinstaller` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `platformio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `arduino-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dfu-util` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libusbmuxd` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `nim` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `esptool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xcode-build-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rswift` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `repo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `winetricks` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `jadx` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `ios-deploy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `swift-format` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `carthage` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `esphome` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a mobile-and-embedded-devices request, run dry-run first, then produce a validated artifact bundle.
