...
description: Run observability dashboards, logs, and performance diagnostics. Use when measuring system stress, benchmarking commands, or querying metrics and centralized logs.
---
...
| Field | Value |
|---|---|
| Skill pack | `observability-and-performance` |
| Tool count | `10` |
| Inventory rank span | `530-939` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for observability-and-performance tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `bottom` | `stress-ng` | no | no |
| bounded mutation / transformation | `stress-ng` | `bottom` | yes | yes |
| validation & artifact checks | `bottom` | `stress-ng` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `bottom` | Yet another cross-platform graphical process/system monitor | local-installable |
| `stress-ng` | Stress test a computer system in various selectable ways | local-installable |
| `grafana` | Gorgeous metric visualizations and dashboards for timeseries databases | local-installable |
| `prometheus` | Service monitoring system and time series database | local-installable |
| `mactop` | Apple Silicon Monitor Top written in Go Lang | local-installable |
| `hyperfine` | Command-line benchmarking tool | local-installable |
| `logcli` | Run LogQL queries against a Loki server | local-installable |
| `lnav` | Curses-based tool for viewing and analyzing log files | local-installable |
| `sentry-cli` | Command-line utility to interact with Sentry | local-installable |
| `hwloc` | Portable abstraction of the hierarchical topology of modern architectures | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `bottom` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `stress-ng` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `grafana` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `prometheus` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mactop` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hyperfine` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `logcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lnav` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sentry-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hwloc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `bottom` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `stress-ng` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `grafana` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `prometheus` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `mactop` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `hyperfine` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a observability-and-performance request, run dry-run first, then produce a validated artifact bundle.
