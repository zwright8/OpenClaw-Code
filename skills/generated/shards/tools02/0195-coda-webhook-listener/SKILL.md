---
name: coda-webhook-listener
description: Use when tasks require coda webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Coda Webhook Listener
...
| Primary artifact | `coda-event-intake-log` |
| Routing tag | `coda:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Coda Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes coda webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `CODA_CLIENT_ID`, `CODA_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `CODA_TOKEN`, `CODA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CODA_WEBHOOK_SECRET`, `CODA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| coda-event-intake-log | structured-artifact | yes | next workflow or operator |
| coda-event-intake-log-scorecard | scorecard | yes | reviewer |
| coda-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Coda Webhook Listener, then register the signing-secret policy for Coda.
...
3. Implement captures, verifies, and routes coda webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `coda-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] coda-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:webhook-listener` consumers with approval and credential context attached.
