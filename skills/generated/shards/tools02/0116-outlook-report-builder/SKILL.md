---
name: outlook-report-builder
description: Use when tasks require outlook report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Report Builder
...
| Primary artifact | `outlook-report-bundle` |
| Routing tag | `outlook:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from outlook activity..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-report-bundle | structured-artifact | yes | next workflow or operator |
| outlook-report-bundle-scorecard | scorecard | yes | reviewer |
| outlook-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Outlook Report Builder, then confirm which Outlook entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from outlook activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `outlook-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] outlook-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:report-builder` consumers with approval and credential context attached.
