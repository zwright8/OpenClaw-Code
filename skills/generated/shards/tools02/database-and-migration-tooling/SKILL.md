---
name: database-and-migration-tooling
description: Work with SQL databases, migrations, and query CLIs. Use when planning schema changes, applying migrations, tuning DB ops, or running interactive SQL workflows.
---
...
| Field | Value |
|---|---|
| Skill pack | `database-and-migration-tooling` |
| Tool count | `18` |
| Inventory rank span | `512-886` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for database-and-migration-tooling tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `postgis` | `dbmate` | no | no |
| bounded mutation / transformation | `dbmate` | `postgis` | yes | yes |
| validation & artifact checks | `postgis` | `dbmate` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `postgis` | Adds support for geographic objects to PostgreSQL | local-installable |
| `dbmate` | Lightweight, framework-agnostic database migration tool | local-installable |
| `percona-toolkit` | Command-line tools for MySQL, MariaDB and system tasks | local-installable |
| `mycli` | CLI for MySQL with auto-completion and syntax highlighting | local-installable |
| `pgvector` | Open-source vector similarity search for Postgres | local-installable |
| `sqlcmd` | Microsoft SQL Server command-line interface | local-installable |
| `openldap` | Open source suite of directory software | local-installable |
| `flyway` | Database version control to control migrations | local-installable |
| `groonga` | Fulltext search engine and column store | local-installable |
| `memcached` | High performance, distributed memory object caching system | local-installable |
| `libspatialite` | Adds spatial SQL capabilities to SQLite | local-installable |
| `mysql-client@8.4` | Open source relational database management system | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `postgis` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dbmate` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `percona-toolkit` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mycli` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgvector` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sqlcmd` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openldap` | LDAP, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `flyway` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `groonga` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `memcached` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libspatialite` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mysql-client@8.4` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `golang-migrate` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgcli` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mysql-client@8.0` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spatialite-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `neo4j` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `postgresql@13` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `postgis` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `dbmate` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `percona-toolkit` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `mycli` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `pgvector` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `sqlcmd` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a database-and-migration-tooling request, run dry-run first, then produce a validated artifact bundle.
