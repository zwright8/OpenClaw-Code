---
name: onedrive-data-exporter
description: Use when tasks require onedrive data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Data Exporter
...
| Primary artifact | `onedrive-export-package` |
| Routing tag | `onedrive:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Data Exporter into a deterministic, auth-checked workflow for exports onedrive entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-export-package | structured-artifact | yes | next workflow or operator |
| onedrive-export-package-scorecard | scorecard | yes | reviewer |
| onedrive-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for OneDrive Data Exporter, then constrain the export scope for OneDrive before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports onedrive entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `onedrive-export-package` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:data-exporter` consumers with approval and credential context attached.
