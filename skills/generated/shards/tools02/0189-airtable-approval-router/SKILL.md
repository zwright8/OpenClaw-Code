---
name: airtable-approval-router
description: Use when tasks require airtable approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Airtable Approval Router
...
| Primary artifact | `airtable-approval-decision-packet` |
| Routing tag | `airtable:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Approval Router into a deterministic, auth-checked workflow for routes airtable-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| airtable-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| airtable-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Airtable Approval Router, then capture the manual fallback path for Airtable.
...
3. Implement routes airtable-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `airtable-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] airtable-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:approval-router` consumers with approval and credential context attached.
