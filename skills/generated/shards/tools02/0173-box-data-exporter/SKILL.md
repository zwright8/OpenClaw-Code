---
name: box-data-exporter
description: Use when tasks require box data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-export-package` |
| Routing tag | `box:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Data Exporter into a deterministic, auth-checked workflow for exports box entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Box Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-export-package | structured-artifact | yes | next workflow or operator |
| box-export-package-scorecard | scorecard | yes | reviewer |
| box-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Box Data Exporter, then constrain the export scope for Box before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports box entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `box-export-package` so downstream owners do not need to rediscover context.
...
- [ ] box-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:data-exporter` consumers with approval and credential context attached.
