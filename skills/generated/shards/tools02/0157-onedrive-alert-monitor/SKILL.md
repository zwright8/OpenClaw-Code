---
name: onedrive-alert-monitor
description: Use when tasks require onedrive alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Alert Monitor
...
| Primary artifact | `onedrive-alert-state-bundle` |
| Routing tag | `onedrive:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Alert Monitor into a deterministic, auth-checked workflow for monitors key onedrive signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| onedrive-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| onedrive-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for OneDrive Alert Monitor, then lock the reviewed threshold set for OneDrive.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key onedrive signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `onedrive-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:alert-monitor` consumers with approval and credential context attached.
