---
name: airtable-sync-orchestrator
description: Use when tasks require airtable sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Airtable Sync Orchestrator
...
| Primary artifact | `airtable-reconciliation-bundle` |
| Routing tag | `airtable:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for airtable records..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| airtable-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| airtable-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Airtable Sync Orchestrator, then checkpoint the last known good cursor on both sides of Airtable.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for airtable records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `airtable-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] airtable-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:sync-orchestrator` consumers with approval and credential context attached.
