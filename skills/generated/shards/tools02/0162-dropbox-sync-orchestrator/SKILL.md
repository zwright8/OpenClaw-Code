---
name: dropbox-sync-orchestrator
description: Use when tasks require dropbox sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Sync Orchestrator
...
| Primary artifact | `dropbox-reconciliation-bundle` |
| Routing tag | `dropbox:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for dropbox records..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| dropbox-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| dropbox-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Dropbox Sync Orchestrator, then checkpoint the last known good cursor on both sides of Dropbox.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for dropbox records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `dropbox-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:sync-orchestrator` consumers with approval and credential context attached.
