---
name: microsoft-calendar-approval-router
description: Use when tasks require microsoft calendar approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Approval Router
...
| Primary artifact | `microsoft-calendar-approval-decision-packet` |
| Routing tag | `microsoft-calendar:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Approval Router into a deterministic, auth-checked workflow for routes microsoft calendar-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Microsoft Calendar Approval Router, then capture the manual fallback path for Microsoft Calendar.
...
3. Implement routes microsoft calendar-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `microsoft-calendar-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:approval-router` consumers with approval and credential context attached.
