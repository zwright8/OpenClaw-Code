---
name: google-drive-data-exporter
description: Use when tasks require google drive data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Data Exporter
...
| Primary artifact | `google-drive-export-package` |
| Routing tag | `google-drive:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Drive Data Exporter into a deterministic, auth-checked workflow for exports google drive entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-export-package | structured-artifact | yes | next workflow or operator |
| google-drive-export-package-scorecard | scorecard | yes | reviewer |
| google-drive-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Google Drive Data Exporter, then constrain the export scope for Google Drive before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports google drive entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `google-drive-export-package` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:data-exporter` consumers with approval and credential context attached.
