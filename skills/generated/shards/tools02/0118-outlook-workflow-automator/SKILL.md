---
name: outlook-workflow-automator
description: Use when tasks require outlook workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Workflow Automator
...
| Primary artifact | `outlook-workflow-runbook` |
| Routing tag | `outlook:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using outlook..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-workflow-runbook | structured-artifact | yes | next workflow or operator |
| outlook-workflow-runbook-scorecard | scorecard | yes | reviewer |
| outlook-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Outlook Workflow Automator, then checkpoint where Outlook state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using outlook. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `outlook-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] outlook-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:workflow-automator` consumers with approval and credential context attached.
