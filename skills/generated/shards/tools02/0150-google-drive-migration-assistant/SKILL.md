---
name: google-drive-migration-assistant
description: Use when tasks require google drive migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `google-drive-migration-cutover-plan` |
| Routing tag | `google-drive:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google Drive Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from google drive..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| google-drive-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| google-drive-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Google Drive Migration Assistant before any Google Drive rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from google drive. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `google-drive-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:migration-assistant` consumers with approval and credential context attached.
