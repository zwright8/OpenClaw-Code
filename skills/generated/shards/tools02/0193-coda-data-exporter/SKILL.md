---
name: coda-data-exporter
description: Use when tasks require coda data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `coda-export-package` |
| Routing tag | `coda:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Data Exporter into a deterministic, auth-checked workflow for exports coda entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-export-package | structured-artifact | yes | next workflow or operator |
| coda-export-package-scorecard | scorecard | yes | reviewer |
| coda-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Coda Data Exporter, then constrain the export scope for Coda before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports coda entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `coda-export-package` so downstream owners do not need to rediscover context.
...
- [ ] coda-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:data-exporter` consumers with approval and credential context attached.
