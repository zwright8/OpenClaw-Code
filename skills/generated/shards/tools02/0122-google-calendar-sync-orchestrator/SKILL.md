---
name: google-calendar-sync-orchestrator
description: Use when tasks require google calendar sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `google-calendar-reconciliation-bundle` |
| Routing tag | `google-calendar:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for google calendar records..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| google-calendar-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| google-calendar-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Google Calendar Sync Orchestrator, then checkpoint the last known good cursor on both sides of Google Calendar.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for google calendar records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `google-calendar-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:sync-orchestrator` consumers with approval and credential context attached.
