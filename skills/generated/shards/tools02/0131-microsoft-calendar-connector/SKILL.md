---
name: microsoft-calendar-connector
description: Use when tasks require microsoft calendar connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `microsoft-calendar-connection-trace-bundle` |
| Routing tag | `microsoft-calendar:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Connector into a deterministic, auth-checked workflow for unified read/write connector for microsoft calendar apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Microsoft Calendar Connector, then confirm the smallest read-only probe that proves access to Microsoft Calendar.
...
3. Specify the request and response contract for unified read/write connector for microsoft calendar apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `microsoft-calendar-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:connector` consumers with approval and credential context attached.
