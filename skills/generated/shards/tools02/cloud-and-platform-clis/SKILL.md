---
name: cloud-and-platform-clis
description: Run cloud-provider and platform CLIs for auth, deploy, and service operations. Use when working with AWS identity flows, PaaS deployments, edge workers, managed DB access, or vendor cloud APIs.
---
...
| Field | Value |
|---|---|
| Skill pack | `cloud-and-platform-clis` |
| Tool count | `17` |
| Inventory rank span | `558-1000` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for cloud-and-platform-clis tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `okta-aws-cli` | `railway` | no | no |
| bounded mutation / transformation | `railway` | `okta-aws-cli` | yes | yes |
| validation & artifact checks | `okta-aws-cli` | `railway` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `okta-aws-cli` | Okta federated identity for AWS CLI | local-installable |
| `railway` | Develop and deploy code with zero configuration | local-installable |
| `cloudflare-wrangler` | CLI tool for Cloudflare Workers | local-installable |
| `heroku` | CLI for Heroku | local-installable |
| `localstack` | Fully functional local AWS cloud stack | local-installable |
| `supabase` | Open source Firebase alternative | local-installable |
| `tenv` | OpenTofu / Terraform / Terragrunt / Terramate / Atmos version manager | local-installable |
| `saml2aws` | Login and retrieve AWS temporary credentials using a SAML IDP | local-installable |
| `hasura-cli` | Command-Line Interface for Hasura GraphQL Engine | local-installable |
| `aws-elasticbeanstalk` | Client for Amazon Elastic Beanstalk web service | local-installable |
| `akamai` | CLI toolkit for working with Akamai's APIs | local-installable |
| `cloud-sql-proxy` | Utility for connecting securely to your Cloud SQL instances | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `okta-aws-cli` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `railway` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudflare-wrangler` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `heroku` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `localstack` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `supabase` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tenv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `saml2aws` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `hasura-cli` | GraphQL over HTTPS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-elasticbeanstalk` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `akamai` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloud-sql-proxy` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `minio-mc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `phrase-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hcloud` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rain` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `aliyun-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-elasticbeanstalk`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `localstack`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `okta-aws-cli`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `rain`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `saml2aws`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `okta-aws-cli` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `railway` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `cloudflare-wrangler` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `heroku` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `localstack` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `supabase` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a cloud-and-platform-clis request, run dry-run first, then produce a validated artifact bundle.
