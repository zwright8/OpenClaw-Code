---
name: gmail-sync-orchestrator
description: Use when tasks require gmail sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-reconciliation-bundle` |
| Routing tag | `gmail:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for gmail records..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| gmail-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| gmail-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Gmail Sync Orchestrator, then checkpoint the last known good cursor on both sides of Gmail.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for gmail records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `gmail-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gmail-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:sync-orchestrator` consumers with approval and credential context attached.
