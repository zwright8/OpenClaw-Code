---
name: onedrive-migration-assistant
description: Use when tasks require onedrive migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Migration Assistant
...
| Primary artifact | `onedrive-migration-cutover-plan` |
| Routing tag | `onedrive:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when state changes are hard to undo once they begin. This specific skill turns OneDrive Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from onedrive..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| onedrive-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| onedrive-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for OneDrive Migration Assistant before any OneDrive rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from onedrive. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `onedrive-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:migration-assistant` consumers with approval and credential context attached.
