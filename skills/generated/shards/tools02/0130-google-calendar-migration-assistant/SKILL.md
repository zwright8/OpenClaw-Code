---
name: google-calendar-migration-assistant
description: Use when tasks require google calendar migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Migration Assistant
...
| Primary artifact | `google-calendar-migration-cutover-plan` |
| Routing tag | `google-calendar:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google Calendar Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from google calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| google-calendar-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| google-calendar-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Google Calendar Migration Assistant before any Google Calendar rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from google calendar. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `google-calendar-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:migration-assistant` consumers with approval and credential context attached.
