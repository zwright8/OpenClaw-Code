---
name: onedrive-report-builder
description: Use when tasks require onedrive report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Report Builder
...
| Primary artifact | `onedrive-report-bundle` |
| Routing tag | `onedrive:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from onedrive activity..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-report-bundle | structured-artifact | yes | next workflow or operator |
| onedrive-report-bundle-scorecard | scorecard | yes | reviewer |
| onedrive-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for OneDrive Report Builder, then confirm which OneDrive entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from onedrive activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `onedrive-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:report-builder` consumers with approval and credential context attached.
