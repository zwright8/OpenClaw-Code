---
name: box-alert-monitor
description: Use when tasks require box alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-alert-state-bundle` |
| Routing tag | `box:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Alert Monitor into a deterministic, auth-checked workflow for monitors key box signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Box Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| box-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| box-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Box Alert Monitor, then lock the reviewed threshold set for Box.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key box signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `box-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] box-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:alert-monitor` consumers with approval and credential context attached.
