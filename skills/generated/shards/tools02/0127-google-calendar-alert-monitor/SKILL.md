---
name: google-calendar-alert-monitor
description: Use when tasks require google calendar alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Alert Monitor
...
| Primary artifact | `google-calendar-alert-state-bundle` |
| Routing tag | `google-calendar:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Alert Monitor into a deterministic, auth-checked workflow for monitors key google calendar signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| google-calendar-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| google-calendar-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Google Calendar Alert Monitor, then lock the reviewed threshold set for Google Calendar.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key google calendar signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `google-calendar-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:alert-monitor` consumers with approval and credential context attached.
