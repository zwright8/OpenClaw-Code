---
name: box-data-importer
description: Use when tasks require box data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-import-ledger` |
| Routing tag | `box:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Box workflows degrade when state changes are hard to undo once they begin. This specific skill turns Box Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into box..

## Trigger Checklist
- [ ] The task explicitly requires `Box Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-import-ledger | structured-artifact | yes | next workflow or operator |
| box-import-ledger-scorecard | scorecard | yes | reviewer |
| box-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Box Data Importer before any Box write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into box. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `box-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] box-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:data-importer` consumers with approval and credential context attached.
