...
description: Use when tasks require onedrive connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `onedrive-connection-trace-bundle` |
| Routing tag | `onedrive:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Connector into a deterministic, auth-checked workflow for unified read/write connector for onedrive apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| onedrive-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| onedrive-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for OneDrive Connector, then confirm the smallest read-only probe that proves access to OneDrive.
...
3. Specify the request and response contract for unified read/write connector for onedrive apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `onedrive-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:connector` consumers with approval and credential context attached.
