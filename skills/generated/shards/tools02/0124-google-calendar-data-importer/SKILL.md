---
name: google-calendar-data-importer
description: Use when tasks require google calendar data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Data Importer
...
| Primary artifact | `google-calendar-import-ledger` |
| Routing tag | `google-calendar:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google Calendar Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into google calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-import-ledger | structured-artifact | yes | next workflow or operator |
| google-calendar-import-ledger-scorecard | scorecard | yes | reviewer |
| google-calendar-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Google Calendar Data Importer before any Google Calendar write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into google calendar. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `google-calendar-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:data-importer` consumers with approval and credential context attached.
