---
name: outlook-migration-assistant
description: Use when tasks require outlook migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Migration Assistant
...
| Primary artifact | `outlook-migration-cutover-plan` |
| Routing tag | `outlook:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when state changes are hard to undo once they begin. This specific skill turns Outlook Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from outlook..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| outlook-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| outlook-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Outlook Migration Assistant before any Outlook rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from outlook. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `outlook-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] outlook-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:migration-assistant` consumers with approval and credential context attached.
