---
name: google-calendar-workflow-automator
description: Use when tasks require google calendar workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Workflow Automator
...
| Primary artifact | `google-calendar-workflow-runbook` |
| Routing tag | `google-calendar:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using google calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-workflow-runbook | structured-artifact | yes | next workflow or operator |
| google-calendar-workflow-runbook-scorecard | scorecard | yes | reviewer |
| google-calendar-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Google Calendar Workflow Automator, then checkpoint where Google Calendar state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using google calendar. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `google-calendar-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:workflow-automator` consumers with approval and credential context attached.
