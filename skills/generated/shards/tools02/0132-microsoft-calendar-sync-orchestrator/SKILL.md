---
name: microsoft-calendar-sync-orchestrator
description: Use when tasks require microsoft calendar sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Sync Orchestrator
...
| Primary artifact | `microsoft-calendar-reconciliation-bundle` |
| Routing tag | `microsoft-calendar:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for microsoft calendar records..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Microsoft Calendar Sync Orchestrator, then checkpoint the last known good cursor on both sides of Microsoft Calendar.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for microsoft calendar records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `microsoft-calendar-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:sync-orchestrator` consumers with approval and credential context attached.
