---
name: dropbox-data-exporter
description: Use when tasks require dropbox data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Data Exporter
...
| Primary artifact | `dropbox-export-package` |
| Routing tag | `dropbox:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Data Exporter into a deterministic, auth-checked workflow for exports dropbox entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-export-package | structured-artifact | yes | next workflow or operator |
| dropbox-export-package-scorecard | scorecard | yes | reviewer |
| dropbox-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Dropbox Data Exporter, then constrain the export scope for Dropbox before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports dropbox entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `dropbox-export-package` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:data-exporter` consumers with approval and credential context attached.
