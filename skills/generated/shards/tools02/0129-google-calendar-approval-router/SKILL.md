---
name: google-calendar-approval-router
description: Use when tasks require google calendar approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Approval Router
...
| Primary artifact | `google-calendar-approval-decision-packet` |
| Routing tag | `google-calendar:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Approval Router into a deterministic, auth-checked workflow for routes google calendar-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| google-calendar-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| google-calendar-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Google Calendar Approval Router, then capture the manual fallback path for Google Calendar.
...
3. Implement routes google calendar-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `google-calendar-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:approval-router` consumers with approval and credential context attached.
