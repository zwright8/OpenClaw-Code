---
name: google-drive-workflow-automator
description: Use when tasks require google drive workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Workflow Automator
...
| Primary artifact | `google-drive-workflow-runbook` |
| Routing tag | `google-drive:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Drive Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using google drive..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-workflow-runbook | structured-artifact | yes | next workflow or operator |
| google-drive-workflow-runbook-scorecard | scorecard | yes | reviewer |
| google-drive-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Google Drive Workflow Automator, then checkpoint where Google Drive state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using google drive. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `google-drive-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:workflow-automator` consumers with approval and credential context attached.
