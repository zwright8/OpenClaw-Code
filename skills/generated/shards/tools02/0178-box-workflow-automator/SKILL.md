---
name: box-workflow-automator
description: Use when tasks require box workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Box Workflow Automator
...
| Primary artifact | `box-workflow-runbook` |
| Routing tag | `box:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using box..

## Trigger Checklist
- [ ] The task explicitly requires `Box Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-workflow-runbook | structured-artifact | yes | next workflow or operator |
| box-workflow-runbook-scorecard | scorecard | yes | reviewer |
| box-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Box Workflow Automator, then checkpoint where Box state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using box. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `box-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] box-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:workflow-automator` consumers with approval and credential context attached.
