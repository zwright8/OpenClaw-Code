---
name: outlook-sync-orchestrator
description: Use when tasks require outlook sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Sync Orchestrator
...
| Primary artifact | `outlook-reconciliation-bundle` |
| Routing tag | `outlook:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for outlook records..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| outlook-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| outlook-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Outlook Sync Orchestrator, then checkpoint the last known good cursor on both sides of Outlook.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for outlook records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `outlook-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] outlook-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:sync-orchestrator` consumers with approval and credential context attached.
