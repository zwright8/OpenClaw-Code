---
name: coda-alert-monitor
description: Use when tasks require coda alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `coda-alert-state-bundle` |
| Routing tag | `coda:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Alert Monitor into a deterministic, auth-checked workflow for monitors key coda signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| coda-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| coda-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Coda Alert Monitor, then lock the reviewed threshold set for Coda.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key coda signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `coda-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] coda-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:alert-monitor` consumers with approval and credential context attached.
