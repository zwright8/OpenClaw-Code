---
name: google-calendar-report-builder
description: Use when tasks require google calendar report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Calendar Report Builder
...
| Primary artifact | `google-calendar-report-bundle` |
| Routing tag | `google-calendar:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from google calendar activity..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-report-bundle | structured-artifact | yes | next workflow or operator |
| google-calendar-report-bundle-scorecard | scorecard | yes | reviewer |
| google-calendar-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Google Calendar Report Builder, then confirm which Google Calendar entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from google calendar activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `google-calendar-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:report-builder` consumers with approval and credential context attached.
