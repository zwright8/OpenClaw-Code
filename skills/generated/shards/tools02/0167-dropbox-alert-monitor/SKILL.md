---
name: dropbox-alert-monitor
description: Use when tasks require dropbox alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Alert Monitor
...
| Primary artifact | `dropbox-alert-state-bundle` |
| Routing tag | `dropbox:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Alert Monitor into a deterministic, auth-checked workflow for monitors key dropbox signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| dropbox-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| dropbox-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Dropbox Alert Monitor, then lock the reviewed threshold set for Dropbox.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key dropbox signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `dropbox-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:alert-monitor` consumers with approval and credential context attached.
