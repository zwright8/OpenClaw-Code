...
description: Use when tasks require dropbox connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `dropbox-connection-trace-bundle` |
| Routing tag | `dropbox:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Dropbox Connector into a deterministic, auth-checked workflow for unified read/write connector for dropbox apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| dropbox-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| dropbox-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Dropbox Connector, then confirm the smallest read-only probe that proves access to Dropbox.
...
3. Specify the request and response contract for unified read/write connector for dropbox apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `dropbox-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:connector` consumers with approval and credential context attached.
