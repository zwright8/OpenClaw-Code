---
name: coda-migration-assistant
description: Use when tasks require coda migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Coda Migration Assistant
...
| Primary artifact | `coda-migration-cutover-plan` |
| Routing tag | `coda:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when state changes are hard to undo once they begin. This specific skill turns Coda Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from coda..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| coda-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| coda-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Coda Migration Assistant before any Coda rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from coda. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `coda-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] coda-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:migration-assistant` consumers with approval and credential context attached.
