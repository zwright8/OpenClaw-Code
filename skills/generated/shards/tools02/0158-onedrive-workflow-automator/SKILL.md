---
name: onedrive-workflow-automator
description: Use when tasks require onedrive workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OneDrive Workflow Automator
...
| Primary artifact | `onedrive-workflow-runbook` |
| Routing tag | `onedrive:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using onedrive..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-workflow-runbook | structured-artifact | yes | next workflow or operator |
| onedrive-workflow-runbook-scorecard | scorecard | yes | reviewer |
| onedrive-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for OneDrive Workflow Automator, then checkpoint where OneDrive state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using onedrive. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `onedrive-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:workflow-automator` consumers with approval and credential context attached.
