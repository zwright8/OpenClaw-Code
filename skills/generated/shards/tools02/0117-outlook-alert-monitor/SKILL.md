---
name: outlook-alert-monitor
description: Use when tasks require outlook alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Alert Monitor
...
| Primary artifact | `outlook-alert-state-bundle` |
| Routing tag | `outlook:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Alert Monitor into a deterministic, auth-checked workflow for monitors key outlook signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| outlook-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| outlook-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Outlook Alert Monitor, then lock the reviewed threshold set for Outlook.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key outlook signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `outlook-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] outlook-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:alert-monitor` consumers with approval and credential context attached.
