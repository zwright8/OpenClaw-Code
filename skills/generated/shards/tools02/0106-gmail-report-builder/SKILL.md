---
name: gmail-report-builder
description: Use when tasks require gmail report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Gmail Report Builder
...
| Primary artifact | `gmail-report-bundle` |
| Routing tag | `gmail:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from gmail activity..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-report-bundle | structured-artifact | yes | next workflow or operator |
| gmail-report-bundle-scorecard | scorecard | yes | reviewer |
| gmail-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Gmail Report Builder, then confirm which Gmail entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from gmail activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `gmail-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gmail-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:report-builder` consumers with approval and credential context attached.
