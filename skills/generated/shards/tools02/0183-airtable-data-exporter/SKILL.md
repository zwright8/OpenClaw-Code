---
name: airtable-data-exporter
description: Use when tasks require airtable data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-export-package` |
| Routing tag | `airtable:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Data Exporter into a deterministic, auth-checked workflow for exports airtable entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-export-package | structured-artifact | yes | next workflow or operator |
| airtable-export-package-scorecard | scorecard | yes | reviewer |
| airtable-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Airtable Data Exporter, then constrain the export scope for Airtable before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports airtable entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `airtable-export-package` so downstream owners do not need to rediscover context.
...
- [ ] airtable-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:data-exporter` consumers with approval and credential context attached.
