---
name: dropbox-data-importer
description: Use when tasks require dropbox data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Data Importer
...
| Primary artifact | `dropbox-import-ledger` |
| Routing tag | `dropbox:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when state changes are hard to undo once they begin. This specific skill turns Dropbox Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into dropbox..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-import-ledger | structured-artifact | yes | next workflow or operator |
| dropbox-import-ledger-scorecard | scorecard | yes | reviewer |
| dropbox-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Dropbox Data Importer before any Dropbox write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into dropbox. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `dropbox-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:data-importer` consumers with approval and credential context attached.
