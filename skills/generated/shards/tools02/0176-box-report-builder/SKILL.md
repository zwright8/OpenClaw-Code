---
name: box-report-builder
description: Use when tasks require box report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-report-bundle` |
| Routing tag | `box:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from box activity..

## Trigger Checklist
- [ ] The task explicitly requires `Box Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-report-bundle | structured-artifact | yes | next workflow or operator |
| box-report-bundle-scorecard | scorecard | yes | reviewer |
| box-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Box Report Builder, then confirm which Box entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from box activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `box-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] box-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:report-builder` consumers with approval and credential context attached.
