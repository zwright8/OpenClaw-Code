---
name: google-drive-report-builder
description: Use when tasks require google drive report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Report Builder
...
| Primary artifact | `google-drive-report-bundle` |
| Routing tag | `google-drive:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Drive Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from google drive activity..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-report-bundle | structured-artifact | yes | next workflow or operator |
| google-drive-report-bundle-scorecard | scorecard | yes | reviewer |
| google-drive-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Google Drive Report Builder, then confirm which Google Drive entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from google drive activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `google-drive-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:report-builder` consumers with approval and credential context attached.
