---
name: onedrive-sync-orchestrator
description: Use when tasks require onedrive sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `onedrive-reconciliation-bundle` |
| Routing tag | `onedrive:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OneDrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OneDrive Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for onedrive records..

## Trigger Checklist
- [ ] The task explicitly requires `OneDrive Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| onedrive-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| onedrive-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| onedrive-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for OneDrive Sync Orchestrator, then checkpoint the last known good cursor on both sides of OneDrive.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for onedrive records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `onedrive-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] onedrive-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `onedrive-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `onedrive:sync-orchestrator` consumers with approval and credential context attached.
