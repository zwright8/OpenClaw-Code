...
description: Use when tasks require airtable webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-event-intake-log` |
| Routing tag | `airtable:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Airtable Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes airtable webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `AIRTABLE_CLIENT_ID`, `AIRTABLE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `AIRTABLE_TOKEN`, `AIRTABLE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `AIRTABLE_WEBHOOK_SECRET`, `AIRTABLE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| airtable-event-intake-log | structured-artifact | yes | next workflow or operator |
| airtable-event-intake-log-scorecard | scorecard | yes | reviewer |
| airtable-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Airtable Webhook Listener, then register the signing-secret policy for Airtable.
...
3. Implement captures, verifies, and routes airtable webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `airtable-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] airtable-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:webhook-listener` consumers with approval and credential context attached.
