---
name: box-connector
description: Use when tasks require box connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-connection-trace-bundle` |
| Routing tag | `box:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Connector into a deterministic, auth-checked workflow for unified read/write connector for box apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Box Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| box-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| box-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Box Connector, then confirm the smallest read-only probe that proves access to Box.
...
3. Specify the request and response contract for unified read/write connector for box apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `box-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] box-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:connector` consumers with approval and credential context attached.
