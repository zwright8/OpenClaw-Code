...
description: Use when tasks require gmail connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-connection-trace-bundle` |
| Routing tag | `gmail:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Connector into a deterministic, auth-checked workflow for unified read/write connector for gmail apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| gmail-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| gmail-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Gmail Connector, then confirm the smallest read-only probe that proves access to Gmail.
...
3. Specify the request and response contract for unified read/write connector for gmail apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `gmail-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gmail-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:connector` consumers with approval and credential context attached.
