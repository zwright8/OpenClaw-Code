---
name: box-webhook-listener
description: Use when tasks require box webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-event-intake-log` |
| Routing tag | `box:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Box workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Box Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes box webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Box Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `BOX_WEBHOOK_SECRET`, `BOX_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `BOX_TOKEN`, `BOX_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| box-event-intake-log | structured-artifact | yes | next workflow or operator |
| box-event-intake-log-scorecard | scorecard | yes | reviewer |
| box-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Box Webhook Listener, then register the signing-secret policy for Box.
...
3. Implement captures, verifies, and routes box webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `box-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] box-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:webhook-listener` consumers with approval and credential context attached.
