...
description: Use when tasks require google calendar connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `google-calendar-connection-trace-bundle` |
| Routing tag | `google-calendar:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Google Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Google Calendar Connector into a deterministic, auth-checked workflow for unified read/write connector for google calendar apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Google Calendar Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-calendar-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| google-calendar-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| google-calendar-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Google Calendar Connector, then confirm the smallest read-only probe that proves access to Google Calendar.
...
3. Specify the request and response contract for unified read/write connector for google calendar apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `google-calendar-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] google-calendar-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-calendar-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-calendar:connector` consumers with approval and credential context attached.
