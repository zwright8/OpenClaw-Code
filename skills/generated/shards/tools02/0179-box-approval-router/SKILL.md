---
name: box-approval-router
description: Use when tasks require box approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `box-approval-decision-packet` |
| Routing tag | `box:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Box workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Box Approval Router into a deterministic, auth-checked workflow for routes box-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Box Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| box-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| box-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| box-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Box Approval Router, then capture the manual fallback path for Box.
...
3. Implement routes box-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `box-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] box-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `box-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `box:approval-router` consumers with approval and credential context attached.
