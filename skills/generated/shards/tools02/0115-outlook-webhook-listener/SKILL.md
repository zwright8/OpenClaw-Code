---
name: outlook-webhook-listener
description: Use when tasks require outlook webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Webhook Listener
...
| Primary artifact | `outlook-event-intake-log` |
| Routing tag | `outlook:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Outlook Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes outlook webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `OUTLOOK_CLIENT_ID`, `OUTLOOK_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `OUTLOOK_WEBHOOK_SECRET`, `OUTLOOK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `OUTLOOK_TOKEN`, `OUTLOOK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| outlook-event-intake-log | structured-artifact | yes | next workflow or operator |
| outlook-event-intake-log-scorecard | scorecard | yes | reviewer |
| outlook-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Outlook Webhook Listener, then register the signing-secret policy for Outlook.
...
3. Implement captures, verifies, and routes outlook webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `outlook-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] outlook-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:webhook-listener` consumers with approval and credential context attached.
