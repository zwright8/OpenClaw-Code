---
name: airtable-data-importer
description: Use when tasks require airtable data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `airtable-import-ledger` |
| Routing tag | `airtable:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when state changes are hard to undo once they begin. This specific skill turns Airtable Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into airtable..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-import-ledger | structured-artifact | yes | next workflow or operator |
| airtable-import-ledger-scorecard | scorecard | yes | reviewer |
| airtable-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Airtable Data Importer before any Airtable write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into airtable. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `airtable-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] airtable-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:data-importer` consumers with approval and credential context attached.
