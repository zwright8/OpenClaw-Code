---
name: dropbox-migration-assistant
description: Use when tasks require dropbox migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Dropbox Migration Assistant
...
| Primary artifact | `dropbox-migration-cutover-plan` |
| Routing tag | `dropbox:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Dropbox workflows degrade when state changes are hard to undo once they begin. This specific skill turns Dropbox Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from dropbox..

## Trigger Checklist
- [ ] The task explicitly requires `Dropbox Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| dropbox-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| dropbox-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| dropbox-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Dropbox Migration Assistant before any Dropbox rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from dropbox. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `dropbox-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] dropbox-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `dropbox-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `dropbox:migration-assistant` consumers with approval and credential context attached.
