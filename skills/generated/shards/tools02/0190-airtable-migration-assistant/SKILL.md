---
name: airtable-migration-assistant
description: Use when tasks require airtable migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Airtable Migration Assistant
...
| Primary artifact | `airtable-migration-cutover-plan` |
| Routing tag | `airtable:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when state changes are hard to undo once they begin. This specific skill turns Airtable Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from airtable..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| airtable-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| airtable-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Airtable Migration Assistant before any Airtable rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from airtable. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `airtable-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] airtable-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:migration-assistant` consumers with approval and credential context attached.
