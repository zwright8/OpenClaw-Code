---
name: outlook-data-importer
description: Use when tasks require outlook data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Data Importer
...
| Primary artifact | `outlook-import-ledger` |
| Routing tag | `outlook:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when state changes are hard to undo once they begin. This specific skill turns Outlook Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into outlook..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-import-ledger | structured-artifact | yes | next workflow or operator |
| outlook-import-ledger-scorecard | scorecard | yes | reviewer |
| outlook-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Outlook Data Importer before any Outlook write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into outlook. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `outlook-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] outlook-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:data-importer` consumers with approval and credential context attached.
