---
name: microsoft-calendar-migration-assistant
description: Use when tasks require microsoft calendar migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Migration Assistant
...
| Primary artifact | `microsoft-calendar-migration-cutover-plan` |
| Routing tag | `microsoft-calendar:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when state changes are hard to undo once they begin. This specific skill turns Microsoft Calendar Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from microsoft calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Microsoft Calendar Migration Assistant before any Microsoft Calendar rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from microsoft calendar. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `microsoft-calendar-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:migration-assistant` consumers with approval and credential context attached.
