---
name: microsoft-calendar-workflow-automator
description: Use when tasks require microsoft calendar workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Workflow Automator
...
| Primary artifact | `microsoft-calendar-workflow-runbook` |
| Routing tag | `microsoft-calendar:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using microsoft calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-workflow-runbook | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-workflow-runbook-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Microsoft Calendar Workflow Automator, then checkpoint where Microsoft Calendar state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using microsoft calendar. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `microsoft-calendar-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:workflow-automator` consumers with approval and credential context attached.
