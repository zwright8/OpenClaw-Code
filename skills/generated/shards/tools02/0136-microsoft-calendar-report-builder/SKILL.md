---
name: microsoft-calendar-report-builder
description: Use when tasks require microsoft calendar report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Report Builder
...
| Primary artifact | `microsoft-calendar-report-bundle` |
| Routing tag | `microsoft-calendar:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Calendar Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from microsoft calendar activity..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-report-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-report-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Microsoft Calendar Report Builder, then confirm which Microsoft Calendar entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from microsoft calendar activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `microsoft-calendar-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:report-builder` consumers with approval and credential context attached.
