---
name: gmail-migration-assistant
description: Use when tasks require gmail migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Gmail Migration Assistant
...
| Primary artifact | `gmail-migration-cutover-plan` |
| Routing tag | `gmail:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when state changes are hard to undo once they begin. This specific skill turns Gmail Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from gmail..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| gmail-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| gmail-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Gmail Migration Assistant before any Gmail rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from gmail. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `gmail-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] gmail-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:migration-assistant` consumers with approval and credential context attached.
