---
name: microsoft-calendar-data-importer
description: Use when tasks require microsoft calendar data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Calendar Data Importer
...
| Primary artifact | `microsoft-calendar-import-ledger` |
| Routing tag | `microsoft-calendar:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Calendar workflows degrade when state changes are hard to undo once they begin. This specific skill turns Microsoft Calendar Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into microsoft calendar..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Calendar Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-calendar-import-ledger | structured-artifact | yes | next workflow or operator |
| microsoft-calendar-import-ledger-scorecard | scorecard | yes | reviewer |
| microsoft-calendar-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Microsoft Calendar Data Importer before any Microsoft Calendar write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into microsoft calendar. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `microsoft-calendar-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-calendar-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-calendar-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-calendar:data-importer` consumers with approval and credential context attached.
