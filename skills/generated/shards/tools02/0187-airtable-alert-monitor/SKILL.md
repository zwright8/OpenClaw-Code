---
name: airtable-alert-monitor
description: Use when tasks require airtable alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Airtable Alert Monitor
...
| Primary artifact | `airtable-alert-state-bundle` |
| Routing tag | `airtable:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Airtable workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Airtable Alert Monitor into a deterministic, auth-checked workflow for monitors key airtable signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Airtable Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| airtable-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| airtable-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| airtable-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Airtable Alert Monitor, then lock the reviewed threshold set for Airtable.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key airtable signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `airtable-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] airtable-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `airtable-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `airtable:alert-monitor` consumers with approval and credential context attached.
