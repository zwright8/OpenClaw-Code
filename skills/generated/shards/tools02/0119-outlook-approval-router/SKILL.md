---
name: outlook-approval-router
description: Use when tasks require outlook approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Outlook Approval Router
...
| Primary artifact | `outlook-approval-decision-packet` |
| Routing tag | `outlook:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Outlook workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Outlook Approval Router into a deterministic, auth-checked workflow for routes outlook-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Outlook Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| outlook-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| outlook-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| outlook-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Outlook Approval Router, then capture the manual fallback path for Outlook.
...
3. Implement routes outlook-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `outlook-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] outlook-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `outlook-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `outlook:approval-router` consumers with approval and credential context attached.
