---
name: google-drive-data-importer
description: Use when tasks require google drive data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Google Drive Data Importer
...
| Primary artifact | `google-drive-import-ledger` |
| Routing tag | `google-drive:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Google Drive workflows degrade when state changes are hard to undo once they begin. This specific skill turns Google Drive Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into google drive..

## Trigger Checklist
- [ ] The task explicitly requires `Google Drive Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| google-drive-import-ledger | structured-artifact | yes | next workflow or operator |
| google-drive-import-ledger-scorecard | scorecard | yes | reviewer |
| google-drive-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Google Drive Data Importer before any Google Drive write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into google drive. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `google-drive-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] google-drive-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `google-drive-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `google-drive:data-importer` consumers with approval and credential context attached.
