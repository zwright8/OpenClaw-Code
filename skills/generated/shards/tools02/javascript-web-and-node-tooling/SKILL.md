---
name: javascript-web-and-node-tooling
description: Build and validate modern web stacks with JavaScript/TypeScript tools. Use when formatting JS/TS, linting frontend code, running bundlers, or preparing web release artifacts.
---

# javascript-web-and-node-tooling

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `javascript-web-and-node-tooling` |
| Tool count | `12` |
| Inventory rank span | `573-938` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for javascript-web-and-node-tooling tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `prettier` | `angular-cli` | no | no |
| bounded mutation / transformation | `angular-cli` | `prettier` | yes | yes |
| validation & artifact checks | `prettier` | `angular-cli` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `prettier` | Code formatter for JavaScript, CSS, JSON, GraphQL, Markdown, YAML | local-installable |
| `angular-cli` | CLI tool for Angular | local-installable |
| `vite` | Next generation frontend tooling. It's fast! | local-installable |
| `typescript` | Language for application scale JavaScript development | local-installable |
| `node-build` | Install NodeJS versions | local-installable |
| `eslint` | AST-based pattern checker for JavaScript | local-installable |
| `markdownlint-cli` | CLI for Node.js style checker and lint tool for Markdown files | local-installable |
| `n` | Node version management | local-installable |
| `nodenv` | Node.js version manager | local-installable |
| `swagger-codegen` | Generate clients, server stubs, and docs from an OpenAPI spec | local-installable |
| `tailwindcss` | Utility-first CSS framework | local-installable |
| `stylua` | Opinionated Lua code formatter | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `prettier` | GraphQL over HTTPS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `angular-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vite` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `typescript` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `node-build` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `eslint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `markdownlint-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `n` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nodenv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swagger-codegen` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tailwindcss` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `stylua` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `prettier` (GraphQL over HTTPS, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `angular-cli` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `vite` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `typescript` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `node-build` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `eslint` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a javascript-web-and-node-tooling request, run dry-run first, then produce a validated artifact bundle.
