...
| Field | Value |
|---|---|
| Skill pack | `files-compression-and-os-utils` |
| Tool count | `33` |
| Inventory rank span | `520-967` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for files-compression-and-os-utils tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `libmagic` | `libvirt` | no | no |
| bounded mutation / transformation | `libvirt` | `libmagic` | yes | yes |
| validation & artifact checks | `libmagic` | `libvirt` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `libmagic` | Implementation of the file(1) command | local-installable |
| `libvirt` | C virtualization API | local-installable |
| `libxslt` | C XSLT library for GNOME | local-installable |
| `md5sha1sum` | Hash utilities | local-installable |
| `testdisk` | Powerful free data recovery utility | local-installable |
| `pigz` | Parallel gzip | local-installable |
| `expat` | XML 1.0 parser | local-installable |
| `fx` | Terminal JSON viewer | local-installable |
| `jsonnet` | Domain specific configuration language for defining JSON data | local-installable |
| `lz4` | Extremely Fast Compression algorithm | local-installed |
| `awk` | Text processing scripting language | local-installable |
| `bzip2` | Freely available high-quality data compressor | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `libmagic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libvirt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxslt` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `md5sha1sum` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `testdisk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pigz` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `expat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jsonnet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lz4` | CLI/local file I/O | None or local runtime | no | no | No API key required for local/offline use. |
| `awk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bzip2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `utf8proc` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `dpkg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `snappy` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `minicom` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yamlfmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yj` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libunistring` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `util-linux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `flux` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libev` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `virt-manager` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `diffutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zip` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `upx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dbus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `e2fsprogs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `psutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `enchant` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `libiconv` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `csvkit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `libmagic` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `libvirt` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `libxslt` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `md5sha1sum` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `testdisk` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `pigz` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a files-compression-and-os-utils request, run dry-run first, then produce a validated artifact bundle.
