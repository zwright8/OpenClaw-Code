---
name: coda-data-importer
description: Use when tasks require coda data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `coda-import-ledger` |
| Routing tag | `coda:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when state changes are hard to undo once they begin. This specific skill turns Coda Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into coda..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-import-ledger | structured-artifact | yes | next workflow or operator |
| coda-import-ledger-scorecard | scorecard | yes | reviewer |
| coda-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Coda Data Importer before any Coda write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into coda. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `coda-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] coda-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:data-importer` consumers with approval and credential context attached.
