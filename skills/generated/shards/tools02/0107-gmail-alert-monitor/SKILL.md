---
name: gmail-alert-monitor
description: Use when tasks require gmail alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-alert-state-bundle` |
| Routing tag | `gmail:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Gmail Alert Monitor into a deterministic, auth-checked workflow for monitors key gmail signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gmail-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| gmail-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| gmail-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Gmail Alert Monitor, then lock the reviewed threshold set for Gmail.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key gmail signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `gmail-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gmail-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:alert-monitor` consumers with approval and credential context attached.
