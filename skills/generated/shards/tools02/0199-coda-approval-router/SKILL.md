---
name: coda-approval-router
description: Use when tasks require coda approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Coda Approval Router
...
| Primary artifact | `coda-approval-decision-packet` |
| Routing tag | `coda:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Approval Router into a deterministic, auth-checked workflow for routes coda-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| coda-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| coda-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Coda Approval Router, then capture the manual fallback path for Coda.
...
3. Implement routes coda-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `coda-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] coda-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:approval-router` consumers with approval and credential context attached.
