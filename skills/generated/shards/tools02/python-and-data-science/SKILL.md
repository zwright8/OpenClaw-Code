...
description: Run Python quality tooling and scientific computing workflows. Use when formatting/linting Python, managing typed checks, running test suites, or operating data/ML numerical stacks.
---
...
| Field | Value |
|---|---|
| Skill pack | `python-and-data-science` |
| Tool count | `25` |
| Inventory rank span | `514-996` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for python-and-data-science tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `black` | `pytest` | no | no |
| bounded mutation / transformation | `pytest` | `black` | yes | yes |
| validation & artifact checks | `black` | `pytest` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `black` | Python code formatter | local-installable |
| `pytest` | Simple powerful testing with Python | local-installable |
| `python-tk@3.11` | Python interface to Tcl/Tk | local-installable |
| `pycparser` | C parser in Python | local-installable |
| `python-matplotlib` | Python library for creating static, animated, and interactive visualizations | local-installable |
| `python-tk@3.12` | Python interface to Tcl/Tk | local-installable |
| `cffi` | C Foreign Function Interface for Python | local-installable |
| `netcdf` | Libraries and data formats for array-oriented scientific data | local-installable |
| `python-tk@3.10` | Python interface to Tcl/Tk | local-installable |
| `scipy` | Software for mathematics, science, and engineering | local-installable |
| `pyqt@5` | Python bindings for v5 of Qt | local-installable |
| `pytorch` | Tensors and dynamic neural networks | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `black` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pytest` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-tk@3.11` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pycparser` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-matplotlib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `python-tk@3.12` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cffi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `netcdf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-tk@3.10` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scipy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pyqt@5` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pytorch` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apache-spark` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `py3cairo` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pyright` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `weasyprint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-tk@3.9` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cookiecutter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pylint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cython` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mypy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sip` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pybind11` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `black` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `pytest` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `python-tk@3.11` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `pycparser` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `python-matplotlib` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `python-tk@3.12` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a python-and-data-science request, run dry-run first, then produce a validated artifact bundle.
