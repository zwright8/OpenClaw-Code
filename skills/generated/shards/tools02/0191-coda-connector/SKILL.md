---
name: coda-connector
description: Use when tasks require coda connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `coda-connection-trace-bundle` |
| Routing tag | `coda:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Coda workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Coda Connector into a deterministic, auth-checked workflow for unified read/write connector for coda apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Coda Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| coda-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| coda-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| coda-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Coda Connector, then confirm the smallest read-only probe that proves access to Coda.
...
3. Specify the request and response contract for unified read/write connector for coda apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `coda-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] coda-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `coda-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `coda:connector` consumers with approval and credential context attached.
