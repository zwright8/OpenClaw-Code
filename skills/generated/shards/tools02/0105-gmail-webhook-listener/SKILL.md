---
name: gmail-webhook-listener
description: Use when tasks require gmail webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gmail-event-intake-log` |
| Routing tag | `gmail:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Gmail workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Gmail Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes gmail webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Gmail Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `GMAIL_WEBHOOK_SECRET`, `GMAIL_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `GMAIL_TOKEN`, `GMAIL_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| gmail-event-intake-log | structured-artifact | yes | next workflow or operator |
| gmail-event-intake-log-scorecard | scorecard | yes | reviewer |
| gmail-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Gmail Webhook Listener, then register the signing-secret policy for Gmail.
...
3. Implement captures, verifies, and routes gmail webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `gmail-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] gmail-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gmail-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gmail:webhook-listener` consumers with approval and credential context attached.
