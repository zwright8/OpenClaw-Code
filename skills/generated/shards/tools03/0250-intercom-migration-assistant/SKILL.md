---
name: intercom-migration-assistant
description: Use when tasks require intercom migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Migration Assistant
...
| Primary artifact | `intercom-migration-cutover-plan` |
| Routing tag | `intercom:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when state changes are hard to undo once they begin. This specific skill turns Intercom Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from intercom..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| intercom-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| intercom-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Intercom Migration Assistant before any Intercom rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from intercom. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `intercom-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] intercom-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:migration-assistant` consumers with approval and credential context attached.
