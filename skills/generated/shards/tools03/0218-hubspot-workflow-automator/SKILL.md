---
name: hubspot-workflow-automator
description: Use when tasks require hubspot workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Workflow Automator
...
| Primary artifact | `hubspot-workflow-runbook` |
| Routing tag | `hubspot:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using hubspot..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-workflow-runbook | structured-artifact | yes | next workflow or operator |
| hubspot-workflow-runbook-scorecard | scorecard | yes | reviewer |
| hubspot-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for HubSpot Workflow Automator, then checkpoint where HubSpot state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using hubspot. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `hubspot-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:workflow-automator` consumers with approval and credential context attached.
