---
name: paypal-report-builder
description: Use when tasks require paypal report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Report Builder
...
| Primary artifact | `paypal-report-bundle` |
| Routing tag | `paypal:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from paypal activity..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-report-bundle | structured-artifact | yes | next workflow or operator |
| paypal-report-bundle-scorecard | scorecard | yes | reviewer |
| paypal-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for PayPal Report Builder, then confirm which PayPal entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from paypal activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `paypal-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] paypal-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:report-builder` consumers with approval and credential context attached.
