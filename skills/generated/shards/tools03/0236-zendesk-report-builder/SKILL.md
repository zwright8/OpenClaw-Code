---
name: zendesk-report-builder
description: Use when tasks require zendesk report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Report Builder
...
| Primary artifact | `zendesk-report-bundle` |
| Routing tag | `zendesk:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from zendesk activity..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-report-bundle | structured-artifact | yes | next workflow or operator |
| zendesk-report-bundle-scorecard | scorecard | yes | reviewer |
| zendesk-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Zendesk Report Builder, then confirm which Zendesk entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from zendesk activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `zendesk-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:report-builder` consumers with approval and credential context attached.
