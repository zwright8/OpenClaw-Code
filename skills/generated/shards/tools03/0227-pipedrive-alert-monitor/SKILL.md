---
name: pipedrive-alert-monitor
description: Use when tasks require pipedrive alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Alert Monitor
...
| Primary artifact | `pipedrive-alert-state-bundle` |
| Routing tag | `pipedrive:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Alert Monitor into a deterministic, auth-checked workflow for monitors key pipedrive signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| pipedrive-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| pipedrive-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Pipedrive Alert Monitor, then lock the reviewed threshold set for Pipedrive.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key pipedrive signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `pipedrive-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:alert-monitor` consumers with approval and credential context attached.
