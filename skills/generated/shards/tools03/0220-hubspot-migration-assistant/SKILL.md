---
name: hubspot-migration-assistant
description: Use when tasks require hubspot migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Migration Assistant
...
| Primary artifact | `hubspot-migration-cutover-plan` |
| Routing tag | `hubspot:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when state changes are hard to undo once they begin. This specific skill turns HubSpot Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from hubspot..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| hubspot-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| hubspot-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for HubSpot Migration Assistant before any HubSpot rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from hubspot. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `hubspot-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:migration-assistant` consumers with approval and credential context attached.
