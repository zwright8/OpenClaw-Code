---
name: hubspot-report-builder
description: Use when tasks require hubspot report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `hubspot-report-bundle` |
| Routing tag | `hubspot:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from hubspot activity..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-report-bundle | structured-artifact | yes | next workflow or operator |
| hubspot-report-bundle-scorecard | scorecard | yes | reviewer |
| hubspot-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for HubSpot Report Builder, then confirm which HubSpot entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from hubspot activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `hubspot-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:report-builder` consumers with approval and credential context attached.
