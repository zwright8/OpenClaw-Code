---
name: xero-alert-monitor
description: Use when tasks require xero alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-alert-state-bundle` |
| Routing tag | `xero:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Alert Monitor into a deterministic, auth-checked workflow for monitors key xero signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| xero-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| xero-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Xero Alert Monitor, then lock the reviewed threshold set for Xero.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key xero signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `xero-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] xero-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:alert-monitor` consumers with approval and credential context attached.
