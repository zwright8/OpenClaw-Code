---
name: paypal-data-exporter
description: Use when tasks require paypal data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Data Exporter
...
| Primary artifact | `paypal-export-package` |
| Routing tag | `paypal:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Data Exporter into a deterministic, auth-checked workflow for exports paypal entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-export-package | structured-artifact | yes | next workflow or operator |
| paypal-export-package-scorecard | scorecard | yes | reviewer |
| paypal-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for PayPal Data Exporter, then constrain the export scope for PayPal before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports paypal entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `paypal-export-package` so downstream owners do not need to rediscover context.
...
- [ ] paypal-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:data-exporter` consumers with approval and credential context attached.
