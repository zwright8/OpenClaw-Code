---
name: stripe-migration-assistant
description: Use when tasks require stripe migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Migration Assistant
...
| Primary artifact | `stripe-migration-cutover-plan` |
| Routing tag | `stripe:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when state changes are hard to undo once they begin. This specific skill turns Stripe Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from stripe..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| stripe-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| stripe-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Stripe Migration Assistant before any Stripe rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from stripe. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `stripe-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] stripe-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:migration-assistant` consumers with approval and credential context attached.
