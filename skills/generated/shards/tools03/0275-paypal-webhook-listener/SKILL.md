...
description: Use when tasks require paypal webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Domain | `Payments and finance systems` |
| Runtime archetype | `signed-event-ingestion-engine` |
| Core method | `signed event intake and routing` |
| Primary artifact | `paypal-event-intake-log` |
| Routing tag | `paypal:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns PayPal Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes paypal webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `PAYPAL_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `PAYPAL_TOKEN`, `PAYPAL_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `PAYPAL_WEBHOOK_SECRET`, `PAYPAL_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| paypal-event-intake-log | structured-artifact | yes | next workflow or operator |
| paypal-event-intake-log-scorecard | scorecard | yes | reviewer |
| paypal-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for PayPal Webhook Listener, then register the signing-secret policy for PayPal.
...
3. Implement captures, verifies, and routes paypal webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `paypal-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] paypal-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:webhook-listener` consumers with approval and credential context attached.
