...
description: Use when tasks require pipedrive approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pipedrive-approval-decision-packet` |
| Routing tag | `pipedrive:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Approval Router into a deterministic, auth-checked workflow for routes pipedrive-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| pipedrive-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| pipedrive-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Pipedrive Approval Router, then capture the manual fallback path for Pipedrive.
...
3. Implement routes pipedrive-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `pipedrive-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:approval-router` consumers with approval and credential context attached.
