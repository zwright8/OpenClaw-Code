...
description: Use when tasks require salesforce data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-export-package` |
| Routing tag | `salesforce:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Data Exporter into a deterministic, auth-checked workflow for exports salesforce entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-export-package | structured-artifact | yes | next workflow or operator |
| salesforce-export-package-scorecard | scorecard | yes | reviewer |
| salesforce-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Salesforce Data Exporter, then constrain the export scope for Salesforce before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports salesforce entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `salesforce-export-package` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:data-exporter` consumers with approval and credential context attached.
