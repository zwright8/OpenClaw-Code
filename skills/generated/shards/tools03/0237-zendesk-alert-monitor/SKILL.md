---
name: zendesk-alert-monitor
description: Use when tasks require zendesk alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Alert Monitor
...
| Primary artifact | `zendesk-alert-state-bundle` |
| Routing tag | `zendesk:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Alert Monitor into a deterministic, auth-checked workflow for monitors key zendesk signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| zendesk-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| zendesk-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Zendesk Alert Monitor, then lock the reviewed threshold set for Zendesk.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key zendesk signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `zendesk-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:alert-monitor` consumers with approval and credential context attached.
