...
description: Use when tasks require salesforce report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-report-bundle` |
| Routing tag | `salesforce:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from salesforce activity..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-report-bundle | structured-artifact | yes | next workflow or operator |
| salesforce-report-bundle-scorecard | scorecard | yes | reviewer |
| salesforce-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Salesforce Report Builder, then confirm which Salesforce entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from salesforce activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `salesforce-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:report-builder` consumers with approval and credential context attached.
