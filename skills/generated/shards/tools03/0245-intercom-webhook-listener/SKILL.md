---
name: intercom-webhook-listener
description: Use when tasks require intercom webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Webhook Listener
...
| Primary artifact | `intercom-event-intake-log` |
| Routing tag | `intercom:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Intercom Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes intercom webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `INTERCOM_CLIENT_ID`, `INTERCOM_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `INTERCOM_TOKEN`, `INTERCOM_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `INTERCOM_WEBHOOK_SECRET`, `INTERCOM_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| intercom-event-intake-log | structured-artifact | yes | next workflow or operator |
| intercom-event-intake-log-scorecard | scorecard | yes | reviewer |
| intercom-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Intercom Webhook Listener, then register the signing-secret policy for Intercom.
...
3. Implement captures, verifies, and routes intercom webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `intercom-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] intercom-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:webhook-listener` consumers with approval and credential context attached.
