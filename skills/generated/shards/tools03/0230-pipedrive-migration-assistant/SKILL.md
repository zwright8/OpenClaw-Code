---
name: pipedrive-migration-assistant
description: Use when tasks require pipedrive migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Migration Assistant
...
| Primary artifact | `pipedrive-migration-cutover-plan` |
| Routing tag | `pipedrive:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when state changes are hard to undo once they begin. This specific skill turns Pipedrive Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from pipedrive..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| pipedrive-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| pipedrive-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Pipedrive Migration Assistant before any Pipedrive rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from pipedrive. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `pipedrive-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:migration-assistant` consumers with approval and credential context attached.
