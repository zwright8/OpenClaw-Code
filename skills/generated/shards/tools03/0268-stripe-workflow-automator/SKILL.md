---
name: stripe-workflow-automator
description: Use when tasks require stripe workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Workflow Automator
...
| Primary artifact | `stripe-workflow-runbook` |
| Routing tag | `stripe:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using stripe..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-workflow-runbook | structured-artifact | yes | next workflow or operator |
| stripe-workflow-runbook-scorecard | scorecard | yes | reviewer |
| stripe-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Stripe Workflow Automator, then checkpoint where Stripe state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using stripe. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `stripe-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] stripe-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:workflow-automator` consumers with approval and credential context attached.
