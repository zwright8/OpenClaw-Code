---
name: hubspot-data-importer
description: Use when tasks require hubspot data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Data Importer
...
| Primary artifact | `hubspot-import-ledger` |
| Routing tag | `hubspot:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when state changes are hard to undo once they begin. This specific skill turns HubSpot Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into hubspot..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-import-ledger | structured-artifact | yes | next workflow or operator |
| hubspot-import-ledger-scorecard | scorecard | yes | reviewer |
| hubspot-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for HubSpot Data Importer before any HubSpot write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into hubspot. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `hubspot-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:data-importer` consumers with approval and credential context attached.
