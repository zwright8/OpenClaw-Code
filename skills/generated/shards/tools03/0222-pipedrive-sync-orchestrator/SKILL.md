---
name: pipedrive-sync-orchestrator
description: Use when tasks require pipedrive sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Sync Orchestrator
...
| Primary artifact | `pipedrive-reconciliation-bundle` |
| Routing tag | `pipedrive:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for pipedrive records..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| pipedrive-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| pipedrive-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Pipedrive Sync Orchestrator, then checkpoint the last known good cursor on both sides of Pipedrive.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for pipedrive records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `pipedrive-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:sync-orchestrator` consumers with approval and credential context attached.
