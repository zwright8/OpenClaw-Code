---
name: xero-data-exporter
description: Use when tasks require xero data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-export-package` |
| Routing tag | `xero:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Data Exporter into a deterministic, auth-checked workflow for exports xero entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-export-package | structured-artifact | yes | next workflow or operator |
| xero-export-package-scorecard | scorecard | yes | reviewer |
| xero-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Xero Data Exporter, then constrain the export scope for Xero before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports xero entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `xero-export-package` so downstream owners do not need to rediscover context.
...
- [ ] xero-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:data-exporter` consumers with approval and credential context attached.
