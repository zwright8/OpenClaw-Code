---
name: salesforce-workflow-automator
description: Use when tasks require salesforce workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Salesforce Workflow Automator
...
| Primary artifact | `salesforce-workflow-runbook` |
| Routing tag | `salesforce:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using salesforce..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-workflow-runbook | structured-artifact | yes | next workflow or operator |
| salesforce-workflow-runbook-scorecard | scorecard | yes | reviewer |
| salesforce-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Salesforce Workflow Automator, then checkpoint where Salesforce state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using salesforce. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `salesforce-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:workflow-automator` consumers with approval and credential context attached.
