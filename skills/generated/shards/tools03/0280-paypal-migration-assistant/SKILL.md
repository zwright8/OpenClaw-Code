---
name: paypal-migration-assistant
description: Use when tasks require paypal migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Migration Assistant
...
| Primary artifact | `paypal-migration-cutover-plan` |
| Routing tag | `paypal:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when state changes are hard to undo once they begin. This specific skill turns PayPal Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from paypal..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| paypal-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| paypal-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for PayPal Migration Assistant before any PayPal rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from paypal. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `paypal-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] paypal-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:migration-assistant` consumers with approval and credential context attached.
