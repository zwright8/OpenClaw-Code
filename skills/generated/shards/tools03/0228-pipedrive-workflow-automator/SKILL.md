---
name: pipedrive-workflow-automator
description: Use when tasks require pipedrive workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Workflow Automator
...
| Primary artifact | `pipedrive-workflow-runbook` |
| Routing tag | `pipedrive:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using pipedrive..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-workflow-runbook | structured-artifact | yes | next workflow or operator |
| pipedrive-workflow-runbook-scorecard | scorecard | yes | reviewer |
| pipedrive-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Pipedrive Workflow Automator, then checkpoint where Pipedrive state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using pipedrive. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `pipedrive-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:workflow-automator` consumers with approval and credential context attached.
