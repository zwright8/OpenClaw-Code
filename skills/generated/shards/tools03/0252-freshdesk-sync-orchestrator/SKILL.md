---
name: freshdesk-sync-orchestrator
description: Use when tasks require freshdesk sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Freshdesk Sync Orchestrator
...
| Primary artifact | `freshdesk-reconciliation-bundle` |
| Routing tag | `freshdesk:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Freshdesk Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for freshdesk records..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| freshdesk-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| freshdesk-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Freshdesk Sync Orchestrator, then checkpoint the last known good cursor on both sides of Freshdesk.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for freshdesk records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `freshdesk-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:sync-orchestrator` consumers with approval and credential context attached.
