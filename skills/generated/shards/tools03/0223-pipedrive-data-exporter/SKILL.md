---
name: pipedrive-data-exporter
description: Use when tasks require pipedrive data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Data Exporter
...
| Primary artifact | `pipedrive-export-package` |
| Routing tag | `pipedrive:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Data Exporter into a deterministic, auth-checked workflow for exports pipedrive entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-export-package | structured-artifact | yes | next workflow or operator |
| pipedrive-export-package-scorecard | scorecard | yes | reviewer |
| pipedrive-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Pipedrive Data Exporter, then constrain the export scope for Pipedrive before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports pipedrive entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `pipedrive-export-package` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:data-exporter` consumers with approval and credential context attached.
