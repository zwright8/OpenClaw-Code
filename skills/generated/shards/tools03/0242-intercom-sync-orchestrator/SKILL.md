---
name: intercom-sync-orchestrator
description: Use when tasks require intercom sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Sync Orchestrator
...
| Primary artifact | `intercom-reconciliation-bundle` |
| Routing tag | `intercom:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for intercom records..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| intercom-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| intercom-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Intercom Sync Orchestrator, then checkpoint the last known good cursor on both sides of Intercom.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for intercom records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `intercom-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] intercom-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:sync-orchestrator` consumers with approval and credential context attached.
