...
description: Use when tasks require zendesk connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `zendesk-connection-trace-bundle` |
| Routing tag | `zendesk:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Connector into a deterministic, auth-checked workflow for unified read/write connector for zendesk apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| zendesk-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| zendesk-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Zendesk Connector, then confirm the smallest read-only probe that proves access to Zendesk.
...
3. Specify the request and response contract for unified read/write connector for zendesk apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `zendesk-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:connector` consumers with approval and credential context attached.
