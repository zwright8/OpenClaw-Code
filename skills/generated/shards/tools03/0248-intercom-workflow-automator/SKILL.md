---
name: intercom-workflow-automator
description: Use when tasks require intercom workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Workflow Automator
...
| Primary artifact | `intercom-workflow-runbook` |
| Routing tag | `intercom:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using intercom..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-workflow-runbook | structured-artifact | yes | next workflow or operator |
| intercom-workflow-runbook-scorecard | scorecard | yes | reviewer |
| intercom-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Intercom Workflow Automator, then checkpoint where Intercom state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using intercom. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `intercom-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] intercom-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:workflow-automator` consumers with approval and credential context attached.
