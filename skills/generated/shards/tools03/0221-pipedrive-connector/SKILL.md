...
description: Use when tasks require pipedrive connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pipedrive-connection-trace-bundle` |
| Routing tag | `pipedrive:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Connector into a deterministic, auth-checked workflow for unified read/write connector for pipedrive apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| pipedrive-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| pipedrive-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Pipedrive Connector, then confirm the smallest read-only probe that proves access to Pipedrive.
...
3. Specify the request and response contract for unified read/write connector for pipedrive apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `pipedrive-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:connector` consumers with approval and credential context attached.
