---
name: hubspot-webhook-listener
description: Use when tasks require hubspot webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `hubspot-event-intake-log` |
| Routing tag | `hubspot:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns HubSpot Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes hubspot webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `HUBSPOT_CLIENT_ID`, `HUBSPOT_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `HUBSPOT_TOKEN`, `HUBSPOT_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `HUBSPOT_WEBHOOK_SECRET`, `HUBSPOT_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| hubspot-event-intake-log | structured-artifact | yes | next workflow or operator |
| hubspot-event-intake-log-scorecard | scorecard | yes | reviewer |
| hubspot-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for HubSpot Webhook Listener, then register the signing-secret policy for HubSpot.
...
3. Implement captures, verifies, and routes hubspot webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `hubspot-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:webhook-listener` consumers with approval and credential context attached.
