...
description: Use when tasks require paypal connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `paypal-connection-trace-bundle` |
| Routing tag | `paypal:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Connector into a deterministic, auth-checked workflow for unified read/write connector for paypal apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| paypal-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| paypal-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for PayPal Connector, then confirm the smallest read-only probe that proves access to PayPal.
...
3. Specify the request and response contract for unified read/write connector for paypal apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `paypal-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] paypal-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:connector` consumers with approval and credential context attached.
