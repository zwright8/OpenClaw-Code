---
name: intercom-data-exporter
description: Use when tasks require intercom data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `intercom-export-package` |
| Routing tag | `intercom:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Data Exporter into a deterministic, auth-checked workflow for exports intercom entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-export-package | structured-artifact | yes | next workflow or operator |
| intercom-export-package-scorecard | scorecard | yes | reviewer |
| intercom-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Intercom Data Exporter, then constrain the export scope for Intercom before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports intercom entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `intercom-export-package` so downstream owners do not need to rediscover context.
...
- [ ] intercom-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:data-exporter` consumers with approval and credential context attached.
