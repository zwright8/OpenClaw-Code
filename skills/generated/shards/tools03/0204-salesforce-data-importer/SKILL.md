---
name: salesforce-data-importer
description: Use when tasks require salesforce data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-import-ledger` |
| Routing tag | `salesforce:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when state changes are hard to undo once they begin. This specific skill turns Salesforce Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into salesforce..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-import-ledger | structured-artifact | yes | next workflow or operator |
| salesforce-import-ledger-scorecard | scorecard | yes | reviewer |
| salesforce-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Salesforce Data Importer before any Salesforce write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into salesforce. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `salesforce-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:data-importer` consumers with approval and credential context attached.
