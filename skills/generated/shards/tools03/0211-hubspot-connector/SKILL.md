...
description: Use when tasks require hubspot connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `hubspot-connection-trace-bundle` |
| Routing tag | `hubspot:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Connector into a deterministic, auth-checked workflow for unified read/write connector for hubspot apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| hubspot-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| hubspot-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for HubSpot Connector, then confirm the smallest read-only probe that proves access to HubSpot.
...
3. Specify the request and response contract for unified read/write connector for hubspot apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `hubspot-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:connector` consumers with approval and credential context attached.
