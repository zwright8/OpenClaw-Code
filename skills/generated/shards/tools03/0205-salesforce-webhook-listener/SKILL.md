---
name: salesforce-webhook-listener
description: Use when tasks require salesforce webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-event-intake-log` |
| Routing tag | `salesforce:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Salesforce Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes salesforce webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SALESFORCE_CLIENT_ID`, `SALESFORCE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SALESFORCE_TOKEN`, `SALESFORCE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SALESFORCE_WEBHOOK_SECRET`, `SALESFORCE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| salesforce-event-intake-log | structured-artifact | yes | next workflow or operator |
| salesforce-event-intake-log-scorecard | scorecard | yes | reviewer |
| salesforce-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Salesforce Webhook Listener, then register the signing-secret policy for Salesforce.
...
3. Implement captures, verifies, and routes salesforce webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `salesforce-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:webhook-listener` consumers with approval and credential context attached.
