---
name: paypal-workflow-automator
description: Use when tasks require paypal workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Workflow Automator
...
| Primary artifact | `paypal-workflow-runbook` |
| Routing tag | `paypal:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using paypal..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-workflow-runbook | structured-artifact | yes | next workflow or operator |
| paypal-workflow-runbook-scorecard | scorecard | yes | reviewer |
| paypal-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for PayPal Workflow Automator, then checkpoint where PayPal state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using paypal. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `paypal-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] paypal-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:workflow-automator` consumers with approval and credential context attached.
