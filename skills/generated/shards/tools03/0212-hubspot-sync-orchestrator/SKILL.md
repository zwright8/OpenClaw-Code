---
name: hubspot-sync-orchestrator
description: Use when tasks require hubspot sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Sync Orchestrator
...
| Primary artifact | `hubspot-reconciliation-bundle` |
| Routing tag | `hubspot:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for hubspot records..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| hubspot-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| hubspot-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for HubSpot Sync Orchestrator, then checkpoint the last known good cursor on both sides of HubSpot.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for hubspot records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `hubspot-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:sync-orchestrator` consumers with approval and credential context attached.
