---
name: xero-report-builder
description: Use when tasks require xero report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-report-bundle` |
| Routing tag | `xero:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from xero activity..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-report-bundle | structured-artifact | yes | next workflow or operator |
| xero-report-bundle-scorecard | scorecard | yes | reviewer |
| xero-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Xero Report Builder, then confirm which Xero entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from xero activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `xero-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] xero-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:report-builder` consumers with approval and credential context attached.
