---
name: quickbooks-webhook-listener
description: Use when tasks require quickbooks webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# QuickBooks Webhook Listener
...
| Primary artifact | `quickbooks-event-intake-log` |
| Routing tag | `quickbooks:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns QuickBooks Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes quickbooks webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `QUICKBOOKS_CLIENT_ID`, `QUICKBOOKS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `QUICKBOOKS_TOKEN`, `QUICKBOOKS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `QUICKBOOKS_WEBHOOK_SECRET`, `QUICKBOOKS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| quickbooks-event-intake-log | structured-artifact | yes | next workflow or operator |
| quickbooks-event-intake-log-scorecard | scorecard | yes | reviewer |
| quickbooks-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for QuickBooks Webhook Listener, then register the signing-secret policy for QuickBooks.
...
3. Implement captures, verifies, and routes quickbooks webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `quickbooks-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:webhook-listener` consumers with approval and credential context attached.
