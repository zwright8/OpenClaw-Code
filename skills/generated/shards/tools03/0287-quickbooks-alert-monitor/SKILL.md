---
name: quickbooks-alert-monitor
description: Use when tasks require quickbooks alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# QuickBooks Alert Monitor
...
| Primary artifact | `quickbooks-alert-state-bundle` |
| Routing tag | `quickbooks:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Alert Monitor into a deterministic, auth-checked workflow for monitors key quickbooks signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| quickbooks-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| quickbooks-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for QuickBooks Alert Monitor, then lock the reviewed threshold set for QuickBooks.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key quickbooks signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `quickbooks-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:alert-monitor` consumers with approval and credential context attached.
