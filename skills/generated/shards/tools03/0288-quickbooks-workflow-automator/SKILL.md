---
name: quickbooks-workflow-automator
description: Use when tasks require quickbooks workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# QuickBooks Workflow Automator
...
| Primary artifact | `quickbooks-workflow-runbook` |
| Routing tag | `quickbooks:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using quickbooks..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-workflow-runbook | structured-artifact | yes | next workflow or operator |
| quickbooks-workflow-runbook-scorecard | scorecard | yes | reviewer |
| quickbooks-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for QuickBooks Workflow Automator, then checkpoint where QuickBooks state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using quickbooks. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `quickbooks-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:workflow-automator` consumers with approval and credential context attached.
