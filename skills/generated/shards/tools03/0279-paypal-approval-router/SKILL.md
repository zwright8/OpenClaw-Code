---
name: paypal-approval-router
description: Use when tasks require paypal approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Approval Router
...
| Primary artifact | `paypal-approval-decision-packet` |
| Routing tag | `paypal:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Approval Router into a deterministic, auth-checked workflow for routes paypal-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| paypal-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| paypal-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for PayPal Approval Router, then capture the manual fallback path for PayPal.
...
3. Implement routes paypal-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `paypal-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] paypal-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:approval-router` consumers with approval and credential context attached.
