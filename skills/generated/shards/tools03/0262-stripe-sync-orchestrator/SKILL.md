---
name: stripe-sync-orchestrator
description: Use when tasks require stripe sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Sync Orchestrator
...
| Primary artifact | `stripe-reconciliation-bundle` |
| Routing tag | `stripe:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for stripe records..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| stripe-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| stripe-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Stripe Sync Orchestrator, then checkpoint the last known good cursor on both sides of Stripe.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for stripe records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `stripe-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] stripe-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:sync-orchestrator` consumers with approval and credential context attached.
