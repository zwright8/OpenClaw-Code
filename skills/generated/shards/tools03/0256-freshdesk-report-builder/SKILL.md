---
name: freshdesk-report-builder
description: Use when tasks require freshdesk report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Freshdesk Report Builder
...
| Primary artifact | `freshdesk-report-bundle` |
| Routing tag | `freshdesk:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Freshdesk Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from freshdesk activity..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-report-bundle | structured-artifact | yes | next workflow or operator |
| freshdesk-report-bundle-scorecard | scorecard | yes | reviewer |
| freshdesk-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Freshdesk Report Builder, then confirm which Freshdesk entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from freshdesk activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `freshdesk-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:report-builder` consumers with approval and credential context attached.
