---
name: stripe-data-importer
description: Use when tasks require stripe data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Data Importer
...
| Primary artifact | `stripe-import-ledger` |
| Routing tag | `stripe:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when state changes are hard to undo once they begin. This specific skill turns Stripe Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into stripe..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-import-ledger | structured-artifact | yes | next workflow or operator |
| stripe-import-ledger-scorecard | scorecard | yes | reviewer |
| stripe-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Stripe Data Importer before any Stripe write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into stripe. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `stripe-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] stripe-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:data-importer` consumers with approval and credential context attached.
