---
name: zendesk-data-importer
description: Use when tasks require zendesk data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `zendesk-import-ledger` |
| Routing tag | `zendesk:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when state changes are hard to undo once they begin. This specific skill turns Zendesk Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into zendesk..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-import-ledger | structured-artifact | yes | next workflow or operator |
| zendesk-import-ledger-scorecard | scorecard | yes | reviewer |
| zendesk-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Zendesk Data Importer before any Zendesk write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into zendesk. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `zendesk-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:data-importer` consumers with approval and credential context attached.
