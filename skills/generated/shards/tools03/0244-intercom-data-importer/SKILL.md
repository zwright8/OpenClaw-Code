---
name: intercom-data-importer
description: Use when tasks require intercom data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Data Importer
...
| Primary artifact | `intercom-import-ledger` |
| Routing tag | `intercom:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when state changes are hard to undo once they begin. This specific skill turns Intercom Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into intercom..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-import-ledger | structured-artifact | yes | next workflow or operator |
| intercom-import-ledger-scorecard | scorecard | yes | reviewer |
| intercom-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Intercom Data Importer before any Intercom write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into intercom. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `intercom-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] intercom-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:data-importer` consumers with approval and credential context attached.
