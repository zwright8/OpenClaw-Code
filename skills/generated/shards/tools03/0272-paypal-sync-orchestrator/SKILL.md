---
name: paypal-sync-orchestrator
description: Use when tasks require paypal sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Sync Orchestrator
...
| Primary artifact | `paypal-reconciliation-bundle` |
| Routing tag | `paypal:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for paypal records..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| paypal-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| paypal-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for PayPal Sync Orchestrator, then checkpoint the last known good cursor on both sides of PayPal.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for paypal records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `paypal-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] paypal-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:sync-orchestrator` consumers with approval and credential context attached.
