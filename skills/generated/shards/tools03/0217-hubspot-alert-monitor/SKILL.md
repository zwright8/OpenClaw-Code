---
name: hubspot-alert-monitor
description: Use when tasks require hubspot alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Alert Monitor
...
| Primary artifact | `hubspot-alert-state-bundle` |
| Routing tag | `hubspot:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Alert Monitor into a deterministic, auth-checked workflow for monitors key hubspot signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| hubspot-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| hubspot-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for HubSpot Alert Monitor, then lock the reviewed threshold set for HubSpot.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key hubspot signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `hubspot-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:alert-monitor` consumers with approval and credential context attached.
