---
name: xero-migration-assistant
description: Use when tasks require xero migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Xero Migration Assistant
...
| Primary artifact | `xero-migration-cutover-plan` |
| Routing tag | `xero:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when state changes are hard to undo once they begin. This specific skill turns Xero Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from xero..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| xero-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| xero-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Xero Migration Assistant before any Xero rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from xero. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `xero-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] xero-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:migration-assistant` consumers with approval and credential context attached.
