---
name: paypal-data-importer
description: Use when tasks require paypal data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Data Importer
...
| Primary artifact | `paypal-import-ledger` |
| Routing tag | `paypal:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when state changes are hard to undo once they begin. This specific skill turns PayPal Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into paypal..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-import-ledger | structured-artifact | yes | next workflow or operator |
| paypal-import-ledger-scorecard | scorecard | yes | reviewer |
| paypal-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for PayPal Data Importer before any PayPal write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into paypal. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `paypal-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] paypal-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:data-importer` consumers with approval and credential context attached.
