---
name: zendesk-migration-assistant
description: Use when tasks require zendesk migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Migration Assistant
...
| Primary artifact | `zendesk-migration-cutover-plan` |
| Routing tag | `zendesk:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when state changes are hard to undo once they begin. This specific skill turns Zendesk Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from zendesk..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| zendesk-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| zendesk-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Zendesk Migration Assistant before any Zendesk rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from zendesk. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `zendesk-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:migration-assistant` consumers with approval and credential context attached.
