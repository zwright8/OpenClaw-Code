---
name: xero-workflow-automator
description: Use when tasks require xero workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Xero Workflow Automator
...
| Primary artifact | `xero-workflow-runbook` |
| Routing tag | `xero:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using xero..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-workflow-runbook | structured-artifact | yes | next workflow or operator |
| xero-workflow-runbook-scorecard | scorecard | yes | reviewer |
| xero-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Xero Workflow Automator, then checkpoint where Xero state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using xero. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `xero-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] xero-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:workflow-automator` consumers with approval and credential context attached.
