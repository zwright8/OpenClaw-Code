---
name: stripe-report-builder
description: Use when tasks require stripe report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Report Builder
...
| Primary artifact | `stripe-report-bundle` |
| Routing tag | `stripe:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from stripe activity..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-report-bundle | structured-artifact | yes | next workflow or operator |
| stripe-report-bundle-scorecard | scorecard | yes | reviewer |
| stripe-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Stripe Report Builder, then confirm which Stripe entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from stripe activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `stripe-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] stripe-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:report-builder` consumers with approval and credential context attached.
