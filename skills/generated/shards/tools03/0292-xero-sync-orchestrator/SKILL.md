---
name: xero-sync-orchestrator
description: Use when tasks require xero sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-reconciliation-bundle` |
| Routing tag | `xero:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for xero records..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| xero-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| xero-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Xero Sync Orchestrator, then checkpoint the last known good cursor on both sides of Xero.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for xero records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `xero-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] xero-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:sync-orchestrator` consumers with approval and credential context attached.
