---
name: intercom-report-builder
description: Use when tasks require intercom report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Report Builder
...
| Primary artifact | `intercom-report-bundle` |
| Routing tag | `intercom:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from intercom activity..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-report-bundle | structured-artifact | yes | next workflow or operator |
| intercom-report-bundle-scorecard | scorecard | yes | reviewer |
| intercom-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Intercom Report Builder, then confirm which Intercom entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from intercom activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `intercom-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] intercom-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:report-builder` consumers with approval and credential context attached.
