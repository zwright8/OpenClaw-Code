---
name: salesforce-sync-orchestrator
description: Use when tasks require salesforce sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Salesforce Sync Orchestrator
...
| Primary artifact | `salesforce-reconciliation-bundle` |
| Routing tag | `salesforce:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for salesforce records..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| salesforce-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| salesforce-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Salesforce Sync Orchestrator, then checkpoint the last known good cursor on both sides of Salesforce.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for salesforce records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `salesforce-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:sync-orchestrator` consumers with approval and credential context attached.
