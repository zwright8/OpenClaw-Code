...
description: Use when tasks require stripe connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `stripe-connection-trace-bundle` |
| Routing tag | `stripe:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Connector into a deterministic, auth-checked workflow for unified read/write connector for stripe apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| stripe-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| stripe-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Stripe Connector, then confirm the smallest read-only probe that proves access to Stripe.
...
3. Specify the request and response contract for unified read/write connector for stripe apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `stripe-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] stripe-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:connector` consumers with approval and credential context attached.
