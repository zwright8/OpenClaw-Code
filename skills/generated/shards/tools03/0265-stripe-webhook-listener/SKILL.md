...
description: Use when tasks require stripe webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `stripe-event-intake-log` |
| Routing tag | `stripe:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Stripe Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes stripe webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `STRIPE_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `STRIPE_TOKEN`, `STRIPE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `STRIPE_WEBHOOK_SECRET`, `STRIPE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| stripe-event-intake-log | structured-artifact | yes | next workflow or operator |
| stripe-event-intake-log-scorecard | scorecard | yes | reviewer |
| stripe-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Stripe Webhook Listener, then register the signing-secret policy for Stripe.
...
3. Implement captures, verifies, and routes stripe webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `stripe-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] stripe-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:webhook-listener` consumers with approval and credential context attached.
