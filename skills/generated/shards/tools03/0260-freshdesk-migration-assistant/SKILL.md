---
name: freshdesk-migration-assistant
description: Use when tasks require freshdesk migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Freshdesk Migration Assistant
...
| Primary artifact | `freshdesk-migration-cutover-plan` |
| Routing tag | `freshdesk:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when state changes are hard to undo once they begin. This specific skill turns Freshdesk Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from freshdesk..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| freshdesk-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| freshdesk-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Freshdesk Migration Assistant before any Freshdesk rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from freshdesk. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `freshdesk-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:migration-assistant` consumers with approval and credential context attached.
