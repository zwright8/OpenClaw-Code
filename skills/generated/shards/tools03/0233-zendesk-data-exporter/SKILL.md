---
name: zendesk-data-exporter
description: Use when tasks require zendesk data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Data Exporter
...
| Primary artifact | `zendesk-export-package` |
| Routing tag | `zendesk:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Data Exporter into a deterministic, auth-checked workflow for exports zendesk entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-export-package | structured-artifact | yes | next workflow or operator |
| zendesk-export-package-scorecard | scorecard | yes | reviewer |
| zendesk-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Zendesk Data Exporter, then constrain the export scope for Zendesk before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports zendesk entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `zendesk-export-package` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:data-exporter` consumers with approval and credential context attached.
