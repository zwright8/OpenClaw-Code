---
name: xero-approval-router
description: Use when tasks require xero approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Xero Approval Router
...
| Primary artifact | `xero-approval-decision-packet` |
| Routing tag | `xero:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Approval Router into a deterministic, auth-checked workflow for routes xero-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| xero-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| xero-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Xero Approval Router, then capture the manual fallback path for Xero.
...
3. Implement routes xero-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `xero-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] xero-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:approval-router` consumers with approval and credential context attached.
