---
name: freshdesk-alert-monitor
description: Use when tasks require freshdesk alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Freshdesk Alert Monitor
...
| Primary artifact | `freshdesk-alert-state-bundle` |
| Routing tag | `freshdesk:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Freshdesk Alert Monitor into a deterministic, auth-checked workflow for monitors key freshdesk signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| freshdesk-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| freshdesk-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Freshdesk Alert Monitor, then lock the reviewed threshold set for Freshdesk.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key freshdesk signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `freshdesk-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:alert-monitor` consumers with approval and credential context attached.
