...
description: Use when tasks require intercom connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `intercom-connection-trace-bundle` |
| Routing tag | `intercom:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Connector into a deterministic, auth-checked workflow for unified read/write connector for intercom apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| intercom-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| intercom-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Intercom Connector, then confirm the smallest read-only probe that proves access to Intercom.
...
3. Specify the request and response contract for unified read/write connector for intercom apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `intercom-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] intercom-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:connector` consumers with approval and credential context attached.
