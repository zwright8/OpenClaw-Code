---
name: zendesk-approval-router
description: Use when tasks require zendesk approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Approval Router
...
| Primary artifact | `zendesk-approval-decision-packet` |
| Routing tag | `zendesk:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Approval Router into a deterministic, auth-checked workflow for routes zendesk-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| zendesk-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| zendesk-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Zendesk Approval Router, then capture the manual fallback path for Zendesk.
...
3. Implement routes zendesk-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `zendesk-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:approval-router` consumers with approval and credential context attached.
