---
name: pipedrive-data-importer
description: Use when tasks require pipedrive data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pipedrive-import-ledger` |
| Routing tag | `pipedrive:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when state changes are hard to undo once they begin. This specific skill turns Pipedrive Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into pipedrive..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-import-ledger | structured-artifact | yes | next workflow or operator |
| pipedrive-import-ledger-scorecard | scorecard | yes | reviewer |
| pipedrive-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Pipedrive Data Importer before any Pipedrive write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into pipedrive. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `pipedrive-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:data-importer` consumers with approval and credential context attached.
