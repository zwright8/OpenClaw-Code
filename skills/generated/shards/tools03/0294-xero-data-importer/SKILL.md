---
name: xero-data-importer
description: Use when tasks require xero data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-import-ledger` |
| Routing tag | `xero:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when state changes are hard to undo once they begin. This specific skill turns Xero Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into xero..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-import-ledger | structured-artifact | yes | next workflow or operator |
| xero-import-ledger-scorecard | scorecard | yes | reviewer |
| xero-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Xero Data Importer before any Xero write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into xero. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `xero-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] xero-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:data-importer` consumers with approval and credential context attached.
