---
name: stripe-data-exporter
description: Use when tasks require stripe data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Data Exporter
...
| Primary artifact | `stripe-export-package` |
| Routing tag | `stripe:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Data Exporter into a deterministic, auth-checked workflow for exports stripe entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-export-package | structured-artifact | yes | next workflow or operator |
| stripe-export-package-scorecard | scorecard | yes | reviewer |
| stripe-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Stripe Data Exporter, then constrain the export scope for Stripe before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports stripe entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `stripe-export-package` so downstream owners do not need to rediscover context.
...
- [ ] stripe-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:data-exporter` consumers with approval and credential context attached.
