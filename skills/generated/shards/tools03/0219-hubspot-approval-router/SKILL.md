---
name: hubspot-approval-router
description: Use when tasks require hubspot approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# HubSpot Approval Router
...
| Primary artifact | `hubspot-approval-decision-packet` |
| Routing tag | `hubspot:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Approval Router into a deterministic, auth-checked workflow for routes hubspot-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| hubspot-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| hubspot-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for HubSpot Approval Router, then capture the manual fallback path for HubSpot.
...
3. Implement routes hubspot-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `hubspot-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:approval-router` consumers with approval and credential context attached.
