---
name: pipedrive-report-builder
description: Use when tasks require pipedrive report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Pipedrive Report Builder
...
| Primary artifact | `pipedrive-report-bundle` |
| Routing tag | `pipedrive:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Pipedrive Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from pipedrive activity..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pipedrive-report-bundle | structured-artifact | yes | next workflow or operator |
| pipedrive-report-bundle-scorecard | scorecard | yes | reviewer |
| pipedrive-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Pipedrive Report Builder, then confirm which Pipedrive entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from pipedrive activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `pipedrive-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:report-builder` consumers with approval and credential context attached.
