---
name: quickbooks-migration-assistant
description: Use when tasks require quickbooks migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `quickbooks-migration-cutover-plan` |
| Routing tag | `quickbooks:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when state changes are hard to undo once they begin. This specific skill turns QuickBooks Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from quickbooks..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| quickbooks-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| quickbooks-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for QuickBooks Migration Assistant before any QuickBooks rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from quickbooks. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `quickbooks-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:migration-assistant` consumers with approval and credential context attached.
