---
name: quickbooks-sync-orchestrator
description: Use when tasks require quickbooks sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `quickbooks-reconciliation-bundle` |
| Routing tag | `quickbooks:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for quickbooks records..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| quickbooks-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| quickbooks-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for QuickBooks Sync Orchestrator, then checkpoint the last known good cursor on both sides of QuickBooks.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for quickbooks records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `quickbooks-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:sync-orchestrator` consumers with approval and credential context attached.
