---
name: intercom-approval-router
description: Use when tasks require intercom approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Approval Router
...
| Primary artifact | `intercom-approval-decision-packet` |
| Routing tag | `intercom:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Approval Router into a deterministic, auth-checked workflow for routes intercom-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| intercom-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| intercom-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Intercom Approval Router, then capture the manual fallback path for Intercom.
...
3. Implement routes intercom-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `intercom-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] intercom-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:approval-router` consumers with approval and credential context attached.
