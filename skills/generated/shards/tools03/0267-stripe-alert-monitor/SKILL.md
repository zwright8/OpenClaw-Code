---
name: stripe-alert-monitor
description: Use when tasks require stripe alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Alert Monitor
...
| Primary artifact | `stripe-alert-state-bundle` |
| Routing tag | `stripe:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Alert Monitor into a deterministic, auth-checked workflow for monitors key stripe signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| stripe-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| stripe-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Stripe Alert Monitor, then lock the reviewed threshold set for Stripe.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key stripe signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `stripe-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] stripe-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:alert-monitor` consumers with approval and credential context attached.
