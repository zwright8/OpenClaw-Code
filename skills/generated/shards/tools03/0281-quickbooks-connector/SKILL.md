...
description: Use when tasks require quickbooks connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `quickbooks-connection-trace-bundle` |
| Routing tag | `quickbooks:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Connector into a deterministic, auth-checked workflow for unified read/write connector for quickbooks apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| quickbooks-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| quickbooks-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for QuickBooks Connector, then confirm the smallest read-only probe that proves access to QuickBooks.
...
3. Specify the request and response contract for unified read/write connector for quickbooks apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `quickbooks-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:connector` consumers with approval and credential context attached.
