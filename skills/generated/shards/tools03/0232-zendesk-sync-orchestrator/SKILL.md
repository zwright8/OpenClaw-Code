---
name: zendesk-sync-orchestrator
description: Use when tasks require zendesk sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Sync Orchestrator
...
| Primary artifact | `zendesk-reconciliation-bundle` |
| Routing tag | `zendesk:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for zendesk records..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| zendesk-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| zendesk-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Zendesk Sync Orchestrator, then checkpoint the last known good cursor on both sides of Zendesk.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for zendesk records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `zendesk-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:sync-orchestrator` consumers with approval and credential context attached.
