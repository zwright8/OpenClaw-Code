---
name: freshdesk-workflow-automator
description: Use when tasks require freshdesk workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Freshdesk Workflow Automator
...
| Primary artifact | `freshdesk-workflow-runbook` |
| Routing tag | `freshdesk:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Freshdesk Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using freshdesk..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-workflow-runbook | structured-artifact | yes | next workflow or operator |
| freshdesk-workflow-runbook-scorecard | scorecard | yes | reviewer |
| freshdesk-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Freshdesk Workflow Automator, then checkpoint where Freshdesk state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using freshdesk. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `freshdesk-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:workflow-automator` consumers with approval and credential context attached.
