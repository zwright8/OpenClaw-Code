---
name: salesforce-alert-monitor
description: Use when tasks require salesforce alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Salesforce Alert Monitor
...
| Primary artifact | `salesforce-alert-state-bundle` |
| Routing tag | `salesforce:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Alert Monitor into a deterministic, auth-checked workflow for monitors key salesforce signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| salesforce-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| salesforce-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Salesforce Alert Monitor, then lock the reviewed threshold set for Salesforce.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key salesforce signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `salesforce-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:alert-monitor` consumers with approval and credential context attached.
