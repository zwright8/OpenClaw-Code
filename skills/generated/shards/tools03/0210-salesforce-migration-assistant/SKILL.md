---
name: salesforce-migration-assistant
description: Use when tasks require salesforce migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-migration-cutover-plan` |
| Routing tag | `salesforce:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when state changes are hard to undo once they begin. This specific skill turns Salesforce Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from salesforce..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| salesforce-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| salesforce-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Salesforce Migration Assistant before any Salesforce rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from salesforce. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `salesforce-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:migration-assistant` consumers with approval and credential context attached.
