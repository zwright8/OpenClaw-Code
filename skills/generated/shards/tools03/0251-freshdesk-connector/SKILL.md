...
description: Use when tasks require freshdesk connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `freshdesk-connection-trace-bundle` |
| Routing tag | `freshdesk:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Freshdesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Freshdesk Connector into a deterministic, auth-checked workflow for unified read/write connector for freshdesk apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Freshdesk Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| freshdesk-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| freshdesk-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| freshdesk-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Freshdesk Connector, then confirm the smallest read-only probe that proves access to Freshdesk.
...
3. Specify the request and response contract for unified read/write connector for freshdesk apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `freshdesk-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] freshdesk-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `freshdesk-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `freshdesk:connector` consumers with approval and credential context attached.
