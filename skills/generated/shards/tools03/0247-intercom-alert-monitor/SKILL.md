---
name: intercom-alert-monitor
description: Use when tasks require intercom alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Intercom Alert Monitor
...
| Primary artifact | `intercom-alert-state-bundle` |
| Routing tag | `intercom:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Intercom workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Intercom Alert Monitor into a deterministic, auth-checked workflow for monitors key intercom signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Intercom Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| intercom-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| intercom-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| intercom-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Intercom Alert Monitor, then lock the reviewed threshold set for Intercom.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key intercom signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `intercom-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] intercom-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `intercom-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `intercom:alert-monitor` consumers with approval and credential context attached.
