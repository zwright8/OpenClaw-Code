---
name: quickbooks-data-exporter
description: Use when tasks require quickbooks data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# QuickBooks Data Exporter
...
| Primary artifact | `quickbooks-export-package` |
| Routing tag | `quickbooks:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Data Exporter into a deterministic, auth-checked workflow for exports quickbooks entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-export-package | structured-artifact | yes | next workflow or operator |
| quickbooks-export-package-scorecard | scorecard | yes | reviewer |
| quickbooks-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for QuickBooks Data Exporter, then constrain the export scope for QuickBooks before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports quickbooks entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `quickbooks-export-package` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:data-exporter` consumers with approval and credential context attached.
