---
name: xero-connector
description: Use when tasks require xero connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-connection-trace-bundle` |
| Routing tag | `xero:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Xero Connector into a deterministic, auth-checked workflow for unified read/write connector for xero apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| xero-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| xero-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| xero-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Xero Connector, then confirm the smallest read-only probe that proves access to Xero.
...
3. Specify the request and response contract for unified read/write connector for xero apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `xero-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] xero-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:connector` consumers with approval and credential context attached.
