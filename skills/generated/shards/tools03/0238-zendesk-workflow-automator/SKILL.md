---
name: zendesk-workflow-automator
description: Use when tasks require zendesk workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Zendesk Workflow Automator
...
| Primary artifact | `zendesk-workflow-runbook` |
| Routing tag | `zendesk:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Zendesk Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using zendesk..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| zendesk-workflow-runbook | structured-artifact | yes | next workflow or operator |
| zendesk-workflow-runbook-scorecard | scorecard | yes | reviewer |
| zendesk-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Zendesk Workflow Automator, then checkpoint where Zendesk state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using zendesk. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `zendesk-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:workflow-automator` consumers with approval and credential context attached.
