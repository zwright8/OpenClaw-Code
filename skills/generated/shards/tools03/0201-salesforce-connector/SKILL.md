...
description: Use when tasks require salesforce connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `salesforce-connection-trace-bundle` |
| Routing tag | `salesforce:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Salesforce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Salesforce Connector into a deterministic, auth-checked workflow for unified read/write connector for salesforce apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Salesforce Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| salesforce-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| salesforce-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| salesforce-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Salesforce Connector, then confirm the smallest read-only probe that proves access to Salesforce.
...
3. Specify the request and response contract for unified read/write connector for salesforce apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `salesforce-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] salesforce-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `salesforce-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `salesforce:connector` consumers with approval and credential context attached.
