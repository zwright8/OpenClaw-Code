---
name: quickbooks-data-importer
description: Use when tasks require quickbooks data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `quickbooks-import-ledger` |
| Routing tag | `quickbooks:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when state changes are hard to undo once they begin. This specific skill turns QuickBooks Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into quickbooks..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-import-ledger | structured-artifact | yes | next workflow or operator |
| quickbooks-import-ledger-scorecard | scorecard | yes | reviewer |
| quickbooks-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for QuickBooks Data Importer before any QuickBooks write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into quickbooks. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `quickbooks-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:data-importer` consumers with approval and credential context attached.
