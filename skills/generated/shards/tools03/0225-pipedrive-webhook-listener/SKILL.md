---
name: pipedrive-webhook-listener
description: Use when tasks require pipedrive webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pipedrive-event-intake-log` |
| Routing tag | `pipedrive:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Pipedrive workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Pipedrive Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes pipedrive webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Pipedrive Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `PIPEDRIVE_CLIENT_ID`, `PIPEDRIVE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Webhook signing secret | `signing_secret` | `PIPEDRIVE_WEBHOOK_SECRET`, `PIPEDRIVE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `PIPEDRIVE_TOKEN`, `PIPEDRIVE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| pipedrive-event-intake-log | structured-artifact | yes | next workflow or operator |
| pipedrive-event-intake-log-scorecard | scorecard | yes | reviewer |
| pipedrive-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Pipedrive Webhook Listener, then register the signing-secret policy for Pipedrive.
...
3. Implement captures, verifies, and routes pipedrive webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `pipedrive-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] pipedrive-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pipedrive-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pipedrive:webhook-listener` consumers with approval and credential context attached.
