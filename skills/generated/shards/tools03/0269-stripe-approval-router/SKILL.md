---
name: stripe-approval-router
description: Use when tasks require stripe approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Stripe Approval Router
...
| Primary artifact | `stripe-approval-decision-packet` |
| Routing tag | `stripe:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Stripe workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Stripe Approval Router into a deterministic, auth-checked workflow for routes stripe-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Stripe Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| stripe-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| stripe-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| stripe-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Stripe Approval Router, then capture the manual fallback path for Stripe.
...
3. Implement routes stripe-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `stripe-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] stripe-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `stripe-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `stripe:approval-router` consumers with approval and credential context attached.
