---
name: hubspot-data-exporter
description: Use when tasks require hubspot data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `hubspot-export-package` |
| Routing tag | `hubspot:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because HubSpot workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns HubSpot Data Exporter into a deterministic, auth-checked workflow for exports hubspot entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `HubSpot Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| hubspot-export-package | structured-artifact | yes | next workflow or operator |
| hubspot-export-package-scorecard | scorecard | yes | reviewer |
| hubspot-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for HubSpot Data Exporter, then constrain the export scope for HubSpot before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports hubspot entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `hubspot-export-package` so downstream owners do not need to rediscover context.
...
- [ ] hubspot-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `hubspot-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `hubspot:data-exporter` consumers with approval and credential context attached.
