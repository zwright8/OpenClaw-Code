---
name: xero-webhook-listener
description: Use when tasks require xero webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `xero-event-intake-log` |
| Routing tag | `xero:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Xero workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Xero Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes xero webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Xero Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `XERO_CLIENT_ID`, `XERO_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `XERO_TOKEN`, `XERO_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `XERO_WEBHOOK_SECRET`, `XERO_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| xero-event-intake-log | structured-artifact | yes | next workflow or operator |
| xero-event-intake-log-scorecard | scorecard | yes | reviewer |
| xero-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Xero Webhook Listener, then register the signing-secret policy for Xero.
...
3. Implement captures, verifies, and routes xero webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `xero-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] xero-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `xero-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `xero:webhook-listener` consumers with approval and credential context attached.
