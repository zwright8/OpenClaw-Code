---
name: paypal-alert-monitor
description: Use when tasks require paypal alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PayPal Alert Monitor
...
| Primary artifact | `paypal-alert-state-bundle` |
| Routing tag | `paypal:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PayPal workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PayPal Alert Monitor into a deterministic, auth-checked workflow for monitors key paypal signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `PayPal Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| paypal-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| paypal-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| paypal-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for PayPal Alert Monitor, then lock the reviewed threshold set for PayPal.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key paypal signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `paypal-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] paypal-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `paypal-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `paypal:alert-monitor` consumers with approval and credential context attached.
