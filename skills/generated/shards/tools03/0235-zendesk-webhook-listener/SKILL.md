...
description: Use when tasks require zendesk webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `zendesk-event-intake-log` |
| Routing tag | `zendesk:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Zendesk workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Zendesk Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes zendesk webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Zendesk Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `ZENDESK_CLIENT_ID`, `ZENDESK_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `ZENDESK_TOKEN`, `ZENDESK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `ZENDESK_WEBHOOK_SECRET`, `ZENDESK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| zendesk-event-intake-log | structured-artifact | yes | next workflow or operator |
| zendesk-event-intake-log-scorecard | scorecard | yes | reviewer |
| zendesk-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Zendesk Webhook Listener, then register the signing-secret policy for Zendesk.
...
3. Implement captures, verifies, and routes zendesk webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `zendesk-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] zendesk-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `zendesk-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `zendesk:webhook-listener` consumers with approval and credential context attached.
