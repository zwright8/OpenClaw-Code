---
name: quickbooks-report-builder
description: Use when tasks require quickbooks report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# QuickBooks Report Builder
...
| Primary artifact | `quickbooks-report-bundle` |
| Routing tag | `quickbooks:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because QuickBooks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns QuickBooks Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from quickbooks activity..

## Trigger Checklist
- [ ] The task explicitly requires `QuickBooks Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| quickbooks-report-bundle | structured-artifact | yes | next workflow or operator |
| quickbooks-report-bundle-scorecard | scorecard | yes | reviewer |
| quickbooks-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for QuickBooks Report Builder, then confirm which QuickBooks entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from quickbooks activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `quickbooks-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] quickbooks-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `quickbooks-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `quickbooks:report-builder` consumers with approval and credential context attached.
