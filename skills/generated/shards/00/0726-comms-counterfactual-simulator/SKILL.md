...
description: Run the Comms Counterfactual Simulator capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0726_comms-counterfactual-simulator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Comms Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Counterfactual Simulator.
...
## Operator Use Cases
- Operate Comms Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `communication-and-explainability:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Counterfactual Simulator`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.21, execution=0.31, safety=0.30, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 42
  - `review_risk`: risk >= 38
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for communication-and-explainability:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `communication-and-explainability:simulation-lab` consumers with approval context attached.
