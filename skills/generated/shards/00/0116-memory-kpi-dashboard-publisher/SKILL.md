...
description: Build and operate the "Memory KPI Dashboard Publisher" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory KPI Dashboard Publisher" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-kpi-dashboard-publisher
...
## Output Contract
- `memory_kpi_dashboard_publisher_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_kpi_dashboard_publisher_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_kpi_dashboard_publisher_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory KPI Dashboard Publisher** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory kpi dashboard publisher normalization/assessment.
