---
name: u0116-memory-kpi-dashboard-publisher
description: Run the Memory KPI Dashboard Publisher capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory KPI Dashboard Publisher
...
| Feature flag | `skill_0116_memory-kpi-dashboard-publisher` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Memory KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory KPI Dashboard Publisher`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.19, execution=0.26, safety=0.35, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 46
  - `review_risk`: risk >= 45
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
