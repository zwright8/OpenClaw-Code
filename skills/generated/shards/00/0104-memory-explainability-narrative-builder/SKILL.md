...
description: Build and operate the "Memory Explainability Narrative Builder" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Explainability Narrative Builder" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-explainability-narrative-builder
...
## Output Contract
- `memory_explainability_narrative_builder_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_explainability_narrative_builder_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_explainability_narrative_builder_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Explainability Narrative Builder** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory explainability narrative builder normalization/assessment.
