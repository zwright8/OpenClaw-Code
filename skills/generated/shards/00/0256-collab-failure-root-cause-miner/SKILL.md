...
## When To Use
Use this skill only when the request explicitly needs `Collab Failure Root-Cause Miner` in Collaboration and Negotiation and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Collab Failure Root-Cause Miner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Collaboration and Negotiation workflows**
   - Input: noisy upstream payload requiring collab failure root-cause miner normalization/assessment.
