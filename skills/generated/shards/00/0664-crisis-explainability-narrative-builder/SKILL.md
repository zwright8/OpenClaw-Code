...
description: Run the Crisis Explainability Narrative Builder capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0664_crisis-explainability-narrative-` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Crisis Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `crisis-and-incident-response:communication-engine`.
- **Review posture (score >= 57 or risk >= 51):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 84):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Explainability Narrative Builder`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.35, execution=0.35, safety=0.14, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 57
  - `review_risk`: risk >= 51
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for crisis-and-incident-response:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `crisis-and-incident-response:communication-engine` consumers with approval context attached.
