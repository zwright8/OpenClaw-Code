---
name: u0241-collab-signal-ingestion-normalizer
description: Run the Collab Signal Ingestion Normalizer capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Signal Ingestion Normalizer
...
| Feature flag | `skill_0241_collab-signal-ingestion-normaliz` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Collab Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Collab Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `collaboration-and-negotiation:normalization-engine`.
- **Review posture (score >= 46 or risk >= 37):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Signal Ingestion Normalizer`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.29, execution=0.18, safety=0.35, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 46
  - `review_risk`: risk >= 37
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
