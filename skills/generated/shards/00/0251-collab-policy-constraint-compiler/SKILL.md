---
name: u0251-collab-policy-constraint-compiler
description: Run the Collab Policy Constraint Compiler capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Policy Constraint Compiler
...
| Routing tag | `collaboration-and-negotiation:contract-compiler` |
| Feature flag | `skill_0251_collab-policy-constraint-compile` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Collab Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Collab Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `collaboration-and-negotiation:contract-compiler`.
- **Review posture (score >= 44 or risk >= 45):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 73):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Policy Constraint Compiler`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.26, execution=0.25, safety=0.33, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 44
  - `review_risk`: risk >= 45
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for collaboration-and-negotiation:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `collaboration-and-negotiation:contract-compiler` consumers with approval context attached.
