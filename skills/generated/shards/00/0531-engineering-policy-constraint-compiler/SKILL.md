---
name: u0531-engineering-policy-constraint-compiler
description: Run the Engineering Policy Constraint Compiler capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Policy Constraint Compiler
...
| Routing tag | `software-engineering-automation:contract-compiler` |
| Feature flag | `skill_0531_engineering-policy-constraint-co` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Engineering Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `software-engineering-automation:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Policy Constraint Compiler`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.16, execution=0.14, safety=0.29, impact=0.42`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 57
  - `review_risk`: risk >= 57
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for software-engineering-automation:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `software-engineering-automation:contract-compiler` consumers with approval context attached.
