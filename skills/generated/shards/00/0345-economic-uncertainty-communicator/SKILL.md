...
description: Run the Economic Uncertainty Communicator capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0345_economic-uncertainty-communicato` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Economic Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Uncertainty Communicator.
...
## Operator Use Cases
- Operate Economic Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `economic-optimization:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Uncertainty Communicator`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.18, execution=0.31, safety=0.18, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 53
  - `review_risk`: risk >= 51
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
