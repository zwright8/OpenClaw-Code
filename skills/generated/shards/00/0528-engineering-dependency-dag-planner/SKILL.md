---
name: u0528-engineering-dependency-dag-planner
description: Run the Engineering Dependency DAG Planner capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Dependency DAG Planner
...
| Feature flag | `skill_0528_engineering-dependency-dag-plann` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill prevents sequencing errors and hidden blockers.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Dependency DAG Planner.
...
## Operator Use Cases
- Operate Engineering Dependency DAG Planner as a reliable, reusable production workflow.
...
- **Review posture (score >= 62 or risk >= 50):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Dependency DAG Planner`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.15, execution=0.26, safety=0.30, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 50
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
