...
description: Build and operate the "Economic Training Curriculum Composer" capability for Economic Optimization. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Economic Training Curriculum Composer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: economic-optimization:economic-training-curriculum-composer
...
## Output Contract
- `economic_training_curriculum_composer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `economic_training_curriculum_composer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `economic_training_curriculum_composer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Training Curriculum Composer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Economic Optimization**
   - Input: noisy upstream payload requiring economic training curriculum composer normalization/assessment.
