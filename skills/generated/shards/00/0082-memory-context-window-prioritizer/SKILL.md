---
name: u0082-memory-context-window-prioritizer
description: Run the Memory Context Window Prioritizer capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0082_memory-context-window-prioritize` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Context Window Prioritizer.
...
## Operator Use Cases
- Operate Memory Context Window Prioritizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Context Window Prioritizer`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.22, execution=0.25, safety=0.23, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 56
  - `review_risk`: risk >= 58
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
