...
description: Build and operate the "Memory Context Window Prioritizer" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Context Window Prioritizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-context-window-prioritizer
...
## Output Contract
- `memory_context_window_prioritizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_context_window_prioritizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_context_window_prioritizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Context Window Prioritizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory context window prioritizer normalization/assessment.
