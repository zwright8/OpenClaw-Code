---
name: u0564-observability-contradiction-detector
description: Run the Observability Contradiction Detector capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Contradiction Detector
...
| Feature flag | `skill_0564_observability-contradiction-dete` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill catches internal and external claim conflicts early.

## Trigger Checklist
- [ ] The task explicitly needs Observability Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Contradiction Detector.
...
## Operator Use Cases
- Operate Observability Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `data-quality-and-observability:detection-guard`.
- **Review posture (score >= 55 or risk >= 49):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 87):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Contradiction Detector`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.25, execution=0.32, safety=0.16, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 55
  - `review_risk`: risk >= 49
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
