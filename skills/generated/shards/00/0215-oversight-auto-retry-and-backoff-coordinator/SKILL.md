---
name: u0215-oversight-auto-retry-and-backoff-coordinator
description: Run the Oversight Auto-Retry and Backoff Coordinator capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Oversight Auto-Retry and Backoff Coordinator
...
| Feature flag | `skill_0215_oversight-auto-retry-and-backoff` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human teams need fast, legible control when stakes are high. This specific skill improves success rates while preventing retry storms.

## Trigger Checklist
- [ ] The task explicitly needs Oversight Auto-Retry and Backoff Coordinator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Auto-Retry and Backoff Coordinator.
...
## Operator Use Cases
- Operate Oversight Auto-Retry and Backoff Coordinator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `human-oversight-and-operator-ux:general-capability`.
- **Review posture (score >= 51 or risk >= 36):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 67):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Auto-Retry and Backoff Coordinator`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.25, execution=0.13, safety=0.39, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 51
  - `review_risk`: risk >= 36
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute adaptive backoff control, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Auto-Retry and Backoff Coordinator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
