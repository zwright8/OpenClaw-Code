---
name: u0487-research-goal-decomposer
description: Run the Research Goal Decomposer capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Goal Decomposer
...
| Feature flag | `skill_0487_research-goal-decomposer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill breaks ambiguous missions into executable units.

## Trigger Checklist
- [ ] The task explicitly needs Research Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Goal Decomposer.
...
## Operator Use Cases
- Operate Research Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `scientific-research-systems:planning-router`.
- **Review posture (score >= 55 or risk >= 47):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Goal Decomposer`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.18, execution=0.16, safety=0.38, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 55
  - `review_risk`: risk >= 47
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
