...
description: Run the Evolution Goal Decomposer capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0967_evolution-goal-decomposer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill breaks ambiguous missions into executable units.

## Trigger Checklist
- [ ] The task explicitly needs Evolution Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Goal Decomposer.
...
## Operator Use Cases
- Operate Evolution Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Goal Decomposer`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.19, execution=0.14, safety=0.27, impact=0.40`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 52
  - `review_risk`: risk >= 50
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for autonomous-learning-and-evolution:planning-router, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `autonomous-learning-and-evolution:planning-router` consumers with approval context attached.
