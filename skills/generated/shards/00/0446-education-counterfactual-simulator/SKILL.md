---
name: u0446-education-counterfactual-simulator
description: Run the Education Counterfactual Simulator capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Counterfactual Simulator
...
| Routing tag | `education-and-upskilling:simulation-lab` |
| Feature flag | `skill_0446_education-counterfactual-simulat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Education Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Counterfactual Simulator.
...
## Operator Use Cases
- Operate Education Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `education-and-upskilling:simulation-lab`.
- **Review posture (score >= 56 or risk >= 60):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 76):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Counterfactual Simulator`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.22, execution=0.23, safety=0.37, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 56
  - `review_risk`: risk >= 60
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for education-and-upskilling:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `education-and-upskilling:simulation-lab` consumers with approval context attached.
