---
name: u0806-inclusion-counterfactual-simulator
description: Run the Inclusion Counterfactual Simulator capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Inclusion Counterfactual Simulator
...
| Routing tag | `accessibility-and-inclusion:simulation-lab` |
| Feature flag | `skill_0806_inclusion-counterfactual-simulat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Counterfactual Simulator.
...
## Operator Use Cases
- Operate Inclusion Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `accessibility-and-inclusion:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Counterfactual Simulator`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.40, execution=0.28, safety=0.18, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 51
  - `review_risk`: risk >= 39
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for accessibility-and-inclusion:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `accessibility-and-inclusion:simulation-lab` consumers with approval context attached.
