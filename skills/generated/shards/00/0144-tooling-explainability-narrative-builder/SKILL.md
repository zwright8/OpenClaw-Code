---
name: u0144-tooling-explainability-narrative-builder
description: Run the Tooling Explainability Narrative Builder capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Explainability Narrative Builder
...
| Feature flag | `skill_0144_tooling-explainability-narrative` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Tooling Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:communication-engine`.
- **Review posture (score >= 53 or risk >= 56):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 84):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Explainability Narrative Builder`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.21, execution=0.26, safety=0.26, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 53
  - `review_risk`: risk >= 56
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
