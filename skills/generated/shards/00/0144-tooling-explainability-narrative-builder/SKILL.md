...
description: Build and operate the "Tooling Explainability Narrative Builder" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Explainability Narrative Builder" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-explainability-narrative-builder
...
## Output Contract
- `tooling_explainability_narrative_builder_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_explainability_narrative_builder_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_explainability_narrative_builder_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Explainability Narrative Builder** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling explainability narrative builder normalization/assessment.
