...
## When To Use
Use this skill when you need "Tooling A/B Rollout Governor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-a-b-rollout-governor
...
## Output Contract
- `tooling_a_b_rollout_governor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_a_b_rollout_governor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_a_b_rollout_governor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling A/B Rollout Governor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling a/b rollout governor normalization/assessment.
