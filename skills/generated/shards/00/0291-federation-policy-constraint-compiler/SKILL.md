---
name: u0291-federation-policy-constraint-compiler
description: Run the Federation Policy Constraint Compiler capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Policy Constraint Compiler
...
| Feature flag | `skill_0291_federation-policy-constraint-com` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Federation Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Federation Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `federation-and-interoperability:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Policy Constraint Compiler`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.28, execution=0.25, safety=0.27, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 43
  - `review_risk`: risk >= 42
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
