---
name: u0695-impact-auto-retry-and-backoff-coordinator
description: Run the Impact Auto-Retry and Backoff Coordinator capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Auto-Retry and Backoff Coordinator
...
| Feature flag | `skill_0695_impact-auto-retry-and-backoff-co` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill improves success rates while preventing retry storms.

## Trigger Checklist
- [ ] The task explicitly needs Impact Auto-Retry and Backoff Coordinator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Auto-Retry and Backoff Coordinator.
...
## Operator Use Cases
- Operate Impact Auto-Retry and Backoff Coordinator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
- **Review posture (score >= 54 or risk >= 49):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Auto-Retry and Backoff Coordinator`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.36, execution=0.30, safety=0.12, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 54
  - `review_risk`: risk >= 49
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute adaptive backoff control, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Auto-Retry and Backoff Coordinator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
