---
name: u0534-engineering-tool-health-monitor
description: Run the Engineering Tool Health Monitor capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Tool Health Monitor
...
| Feature flag | `skill_0534_engineering-tool-health-monitor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill detects tool flakiness before it impacts mission outcomes.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Tool Health Monitor.
...
## Operator Use Cases
- Operate Engineering Tool Health Monitor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `software-engineering-automation:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Tool Health Monitor`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.35, execution=0.28, safety=0.19, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 52
  - `review_risk`: risk >= 36
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
