...
description: Run the Governance Skill Gap Diagnoser capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0197_governance-skill-gap-diagnoser` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Skill Gap Diagnoser (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Skill Gap Diagnoser.
...
## Operator Use Cases
- Operate Governance Skill Gap Diagnoser as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `safety-and-governance:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Skill Gap Diagnoser`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.27, execution=0.24, safety=0.22, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 52
  - `review_risk`: risk >= 36
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute competency coverage analysis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Skill Gap Diagnoser against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
