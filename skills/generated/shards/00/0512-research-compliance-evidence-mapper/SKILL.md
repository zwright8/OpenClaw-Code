---
name: u0512-research-compliance-evidence-mapper
description: Run the Research Compliance Evidence Mapper capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Compliance Evidence Mapper
...
| Routing tag | `scientific-research-systems:normalization-engine` |
| Feature flag | `skill_0512_research-compliance-evidence-map` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Research Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Research Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `scientific-research-systems:normalization-engine`.
- **Review posture (score >= 50 or risk >= 45):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Compliance Evidence Mapper`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.34, execution=0.18, safety=0.15, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 50
  - `review_risk`: risk >= 45
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for scientific-research-systems:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `scientific-research-systems:normalization-engine` consumers with approval context attached.
