---
name: u0761-cultural-signal-ingestion-normalizer
description: Run the Cultural Signal Ingestion Normalizer capability for Multilingual and Cultural Adaptation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Cultural Signal Ingestion Normalizer
...
| Feature flag | `skill_0761_cultural-signal-ingestion-normal` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because global utility requires language and cultural context fidelity. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Cultural Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Cultural Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Cultural Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `multilingual-and-cultural-adaptation:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Cultural Signal Ingestion Normalizer`, including at least three measurable KPIs tied to context mismatch and exclusion.
...
- Scoring weights: `truth=0.26, execution=0.25, safety=0.35, impact=0.14`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 45
  - `review_risk`: risk >= 38
...
1. Incident recovery in Multilingual and Cultural Adaptation: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Cultural Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for multilingual-and-cultural-adaptation:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `multilingual-and-cultural-adaptation:normalization-engine` consumers with approval context attached.
