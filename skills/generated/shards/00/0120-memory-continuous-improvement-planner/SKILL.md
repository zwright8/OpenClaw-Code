...
## When To Use
Use this skill when you need "Memory Continuous Improvement Planner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-continuous-improvement-planner
...
## Output Contract
- `memory_continuous_improvement_planner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_continuous_improvement_planner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_continuous_improvement_planner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Continuous Improvement Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory continuous improvement planner normalization/assessment.
