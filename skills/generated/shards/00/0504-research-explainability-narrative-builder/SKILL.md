---
name: u0504-research-explainability-narrative-builder
description: Run the Research Explainability Narrative Builder capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Explainability Narrative Builder
...
| Routing tag | `scientific-research-systems:communication-engine` |
| Feature flag | `skill_0504_research-explainability-narrativ` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Research Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Research Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `scientific-research-systems:communication-engine`.
- **Review posture (score >= 59 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Explainability Narrative Builder`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.33, execution=0.16, safety=0.21, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 59
  - `review_risk`: risk >= 61
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for scientific-research-systems:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `scientific-research-systems:communication-engine` consumers with approval context attached.
