---
name: u0456-education-failure-root-cause-miner
description: Run the Education Failure Root-Cause Miner capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Failure Root-Cause Miner
...
| Feature flag | `skill_0456_education-failure-root-cause-min` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Education Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Education Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `education-and-upskilling:general-capability`.
- **Review posture (score >= 53 or risk >= 54):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Failure Root-Cause Miner`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.32, execution=0.28, safety=0.16, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 53
  - `review_risk`: risk >= 54
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
