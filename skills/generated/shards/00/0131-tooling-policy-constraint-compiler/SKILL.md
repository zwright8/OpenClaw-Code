---
name: u0131-tooling-policy-constraint-compiler
description: Run the Tooling Policy Constraint Compiler capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Policy Constraint Compiler
...
| Routing tag | `tool-reliability-and-execution-quality:contract-compiler` |
| Feature flag | `skill_0131_tooling-policy-constraint-compil` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Tooling Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Policy Constraint Compiler`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.23, execution=0.17, safety=0.28, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 61
  - `review_risk`: risk >= 56
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for tool-reliability-and-execution-quality:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `tool-reliability-and-execution-quality:contract-compiler` consumers with approval context attached.
