...
description: Build and operate the "Tooling Policy Constraint Compiler" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Policy Constraint Compiler" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
3. Implement the core capability using policy-gated decision routing, with deterministic scoring and reproducible artifact generation.
...
- Core method: policy-gated decision routing
- Execution mode: deterministic-first with explicit tolerances
- Routing tag: tool-reliability-and-execution-quality:tooling-policy-constraint-compiler
...
## Output Contract
- `tooling_policy_constraint_compiler_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_policy_constraint_compiler_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_policy_constraint_compiler_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Policy Constraint Compiler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling policy constraint compiler normalization/assessment.
