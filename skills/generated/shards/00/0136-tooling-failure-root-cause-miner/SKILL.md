...
description: Build and operate the "Tooling Failure Root-Cause Miner" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Failure Root-Cause Miner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-failure-root-cause-miner
...
## Output Contract
- `tooling_failure_root_cause_miner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_failure_root_cause_miner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_failure_root_cause_miner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Failure Root-Cause Miner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling failure root-cause miner normalization/assessment.
