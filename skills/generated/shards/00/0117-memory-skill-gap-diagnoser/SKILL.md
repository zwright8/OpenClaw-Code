...
description: Build and operate the "Memory Skill Gap Diagnoser" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Skill Gap Diagnoser" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-skill-gap-diagnoser
...
## Output Contract
- `memory_skill_gap_diagnoser_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_skill_gap_diagnoser_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_skill_gap_diagnoser_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Skill Gap Diagnoser** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory skill gap diagnoser normalization/assessment.
