...
description: Operate the "Economic Signal Ingestion Normalizer" capability in production for the target domain workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `321`
- **Skill Name:** `u0321-economic-signal-ingestion-normalizer`
...
4. Keep routing aligned with `economic-optimization:normalization-engine` and include approval context.
...
### Handoff Contract
- **Produces:** `Economic Signal Ingestion Normalizer normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `budgets`, `costs`, `benefits`, `opportunity values`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to economic-optimization:normalization-engine consumers with approval-gate context
