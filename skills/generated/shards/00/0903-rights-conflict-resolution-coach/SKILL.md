...
description: Run the Rights Conflict Resolution Coach capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `legal-rights-and-compliance:collaboration-mediator` |
| Feature flag | `skill_0903_rights-conflict-resolution-coach` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Rights Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `legal-rights-and-compliance:collaboration-mediator`.
- **Review posture (score >= 52 or risk >= 47):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 80):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Conflict Resolution Coach`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.17, execution=0.34, safety=0.18, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 52
  - `review_risk`: risk >= 47
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for legal-rights-and-compliance:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `legal-rights-and-compliance:collaboration-mediator` consumers with approval context attached.
