---
name: u0883-rights-evidence-provenance-tracker
description: Run the Rights Evidence Provenance Tracker capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Evidence Provenance Tracker
...
| Routing tag | `legal-rights-and-compliance:provenance-tracker` |
| Feature flag | `skill_0883_rights-evidence-provenance-track` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Rights Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Rights Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `legal-rights-and-compliance:provenance-tracker`.
- **Review posture (score >= 59 or risk >= 49):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 78):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Evidence Provenance Tracker`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.28, execution=0.21, safety=0.33, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 59
  - `review_risk`: risk >= 49
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `evidence lineage graphs`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `evidence lineage graphs`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `evidence lineage graphs`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `evidence lineage graphs`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for legal-rights-and-compliance:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `legal-rights-and-compliance:provenance-tracker` consumers with approval context attached.
