...
description: Run the PublicService Explainability Narrative Builder capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0424_publicservice-explainability-nar` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Explainability Narrative Builder.
...
## Operator Use Cases
- Operate PublicService Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `healthcare-and-public-services:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Explainability Narrative Builder`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.37, execution=0.25, safety=0.24, impact=0.14`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 59
  - `review_risk`: risk >= 57
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:communication-engine` consumers with approval context attached.
