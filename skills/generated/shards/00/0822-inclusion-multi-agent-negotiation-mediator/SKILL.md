...
description: Run the Inclusion Multi-Agent Negotiation Mediator capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0822_inclusion-multi-agent-negotiatio` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Inclusion Multi-Agent Negotiation Mediator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Multi-Agent Negotiation Mediator.
...
## Operator Use Cases
- Operate Inclusion Multi-Agent Negotiation Mediator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `accessibility-and-inclusion:collaboration-mediator`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Multi-Agent Negotiation Mediator`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.17, execution=0.21, safety=0.34, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 52
  - `review_risk`: risk >= 48
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute structured bargaining protocols, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Multi-Agent Negotiation Mediator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
