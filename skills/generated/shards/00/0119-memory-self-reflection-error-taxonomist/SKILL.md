...
## When To Use
Use this skill when you need "Memory Self-Reflection Error Taxonomist" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-self-reflection-error-taxonomist
...
## Output Contract
- `memory_self_reflection_error_taxonomist_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_self_reflection_error_taxonomist_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_self_reflection_error_taxonomist_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Self-Reflection Error Taxonomist** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory self-reflection error taxonomist normalization/assessment.
