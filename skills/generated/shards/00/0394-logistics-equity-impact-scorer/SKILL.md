---
name: u0394-logistics-equity-impact-scorer
description: Run the Logistics Equity Impact Scorer capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Equity Impact Scorer
...
| Feature flag | `skill_0394_logistics-equity-impact-scorer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill exposes uneven benefit/harm distribution before rollout.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Equity Impact Scorer.
...
## Operator Use Cases
- Operate Logistics Equity Impact Scorer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `resource-allocation-and-logistics:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Equity Impact Scorer`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.18, execution=0.23, safety=0.26, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 45
  - `review_risk`: risk >= 44
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
