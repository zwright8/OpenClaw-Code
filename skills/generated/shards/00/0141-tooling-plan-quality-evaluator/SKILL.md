...
## When To Use
Use this skill when you need "Tooling Plan Quality Evaluator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-plan-quality-evaluator
...
## Output Contract
- `tooling_plan_quality_evaluator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_plan_quality_evaluator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_plan_quality_evaluator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Plan Quality Evaluator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling plan quality evaluator normalization/assessment.
