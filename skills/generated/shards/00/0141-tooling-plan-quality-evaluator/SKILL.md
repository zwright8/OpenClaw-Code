---
name: u0141-tooling-plan-quality-evaluator
description: Run the Tooling Plan Quality Evaluator capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Plan Quality Evaluator
...
| Feature flag | `skill_0141_tooling-plan-quality-evaluator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill quantifies whether plans are complete, safe, and feasible.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Tooling Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Plan Quality Evaluator`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.13, execution=0.34, safety=0.35, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 44
  - `review_risk`: risk >= 43
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
