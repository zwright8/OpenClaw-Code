---
name: u0246-collab-counterfactual-simulator
description: Run the Collab Counterfactual Simulator capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Counterfactual Simulator
...
| Routing tag | `collaboration-and-negotiation:simulation-lab` |
| Feature flag | `skill_0246_collab-counterfactual-simulator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Collab Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Counterfactual Simulator.
...
## Operator Use Cases
- Operate Collab Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `collaboration-and-negotiation:simulation-lab`.
- **Review posture (score >= 52 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 81):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Counterfactual Simulator`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.24, execution=0.24, safety=0.37, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 52
  - `review_risk`: risk >= 61
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for collaboration-and-negotiation:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `collaboration-and-negotiation:simulation-lab` consumers with approval context attached.
