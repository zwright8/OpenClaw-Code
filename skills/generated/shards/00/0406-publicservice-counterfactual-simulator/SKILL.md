...
description: Run the PublicService Counterfactual Simulator capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `healthcare-and-public-services:simulation-lab` |
| Feature flag | `skill_0406_publicservice-counterfactual-sim` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Counterfactual Simulator.
...
## Operator Use Cases
- Operate PublicService Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `healthcare-and-public-services:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Counterfactual Simulator`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.28, execution=0.14, safety=0.28, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 47
  - `review_risk`: risk >= 32
...
- Approval gates: `policy-constraint-check`, `human-approval-router`, `safety-review`, `scenario-review`.
...
| Required approvals | `policy-constraint-check`, `human-approval-router`, `safety-review`, `scenario-review` |
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `scenario comparison reports`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:simulation-lab` consumers with approval context attached.
