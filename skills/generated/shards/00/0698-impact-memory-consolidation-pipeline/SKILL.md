...
description: Run the Impact Memory Consolidation Pipeline capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0698_impact-memory-consolidation-pipe` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Impact Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Memory Consolidation Pipeline`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.34, execution=0.19, safety=0.17, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 45
  - `review_risk`: risk >= 40
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
