...
description: Run the Security Community Feedback Harvester capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0635_security-community-feedback-harv` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Community Feedback Harvester (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Community Feedback Harvester.
...
## Operator Use Cases
- Operate Security Community Feedback Harvester as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `security-and-privacy:normalization-engine`.
- **Review posture (score >= 46 or risk >= 38):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 62):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Community Feedback Harvester`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.18, execution=0.21, safety=0.22, impact=0.39`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 46
  - `review_risk`: risk >= 38
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute feedback normalization and clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Community Feedback Harvester against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
