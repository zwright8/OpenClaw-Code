...
## When To Use
Use this skill when you need "Logistics Counterfactual Simulator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-counterfactual-simulator
...
## Output Contract
- `logistics_counterfactual_simulator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_counterfactual_simulator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_counterfactual_simulator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Counterfactual Simulator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics counterfactual simulator normalization/assessment.
