---
name: u0366-logistics-counterfactual-simulator
description: Run the Logistics Counterfactual Simulator capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Counterfactual Simulator
...
| Feature flag | `skill_0366_logistics-counterfactual-simulat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Counterfactual Simulator.
...
## Operator Use Cases
- Operate Logistics Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `resource-allocation-and-logistics:simulation-lab`.
- **Review posture (score >= 56 or risk >= 47):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Counterfactual Simulator`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.19, execution=0.26, safety=0.22, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 56
  - `review_risk`: risk >= 47
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
