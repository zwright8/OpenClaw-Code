...
description: Run the Engineering KPI Dashboard Publisher capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0556_engineering-kpi-dashboard-publis` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Engineering KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering KPI Dashboard Publisher`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.26, execution=0.22, safety=0.27, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 52
  - `review_risk`: risk >= 40
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
