...
description: Run the Governance Counterfactual Simulator capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0166_governance-counterfactual-simula` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Counterfactual Simulator.
...
## Operator Use Cases
- Operate Governance Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `safety-and-governance:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Counterfactual Simulator`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.31, execution=0.34, safety=0.17, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 53
  - `review_risk`: risk >= 47
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for safety-and-governance:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `safety-and-governance:simulation-lab` consumers with approval context attached.
