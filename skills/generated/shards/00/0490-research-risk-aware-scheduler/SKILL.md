...
description: Run the Research Risk-Aware Scheduler capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0490_research-risk-aware-scheduler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Research Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Research Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `scientific-research-systems:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Risk-Aware Scheduler`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.19, execution=0.32, safety=0.20, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 53
  - `review_risk`: risk >= 51
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
