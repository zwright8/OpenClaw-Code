---
name: u0754-comms-equity-impact-scorer
description: Run the Comms Equity Impact Scorer capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Equity Impact Scorer
...
| Feature flag | `skill_0754_comms-equity-impact-scorer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill exposes uneven benefit/harm distribution before rollout.

## Trigger Checklist
- [ ] The task explicitly needs Comms Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Equity Impact Scorer.
...
## Operator Use Cases
- Operate Comms Equity Impact Scorer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Equity Impact Scorer`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.14, execution=0.36, safety=0.34, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 47
  - `review_risk`: risk >= 32
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
