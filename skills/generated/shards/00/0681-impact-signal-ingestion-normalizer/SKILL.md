---
name: u0681-impact-signal-ingestion-normalizer
description: Run the Impact Signal Ingestion Normalizer capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Signal Ingestion Normalizer
...
| Feature flag | `skill_0681_impact-signal-ingestion-normaliz` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Impact Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Impact Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `social-impact-measurement:normalization-engine`.
- **Review posture (score >= 59 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 83):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Signal Ingestion Normalizer`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.18, execution=0.31, safety=0.35, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 59
  - `review_risk`: risk >= 46
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
