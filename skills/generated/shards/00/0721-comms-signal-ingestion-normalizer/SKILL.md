---
name: u0721-comms-signal-ingestion-normalizer
description: Run the Comms Signal Ingestion Normalizer capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Signal Ingestion Normalizer
...
| Feature flag | `skill_0721_comms-signal-ingestion-normalize` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Comms Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Comms Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `communication-and-explainability:normalization-engine`.
- **Review posture (score >= 48 or risk >= 31):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Signal Ingestion Normalizer`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.26, execution=0.26, safety=0.21, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 48
  - `review_risk`: risk >= 31
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
