...
description: Run the Governance Security Threat Modeler capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0191_governance-security-threat-model` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Security Threat Modeler.
...
## Operator Use Cases
- Operate Governance Security Threat Modeler as a reliable, reusable production workflow.
...
- **Review posture (score >= 62 or risk >= 57):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 83):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Security Threat Modeler`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.22, execution=0.30, safety=0.19, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 62
  - `review_risk`: risk >= 57
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
