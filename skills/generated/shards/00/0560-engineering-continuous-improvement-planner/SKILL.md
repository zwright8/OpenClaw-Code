...
description: Run the Engineering Continuous Improvement Planner capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0560_engineering-continuous-improveme` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Engineering Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `software-engineering-automation:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Continuous Improvement Planner`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.27, execution=0.13, safety=0.27, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 52
  - `review_risk`: risk >= 36
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
