...
description: Build and operate the "Memory Human Approval Router" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Human Approval Router" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-human-approval-router
...
## Output Contract
- `memory_human_approval_router_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_human_approval_router_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_human_approval_router_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Human Approval Router** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory human approval router normalization/assessment.
