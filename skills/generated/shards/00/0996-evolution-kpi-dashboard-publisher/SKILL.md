---
name: u0996-evolution-kpi-dashboard-publisher
description: Run the Evolution KPI Dashboard Publisher capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Evolution KPI Dashboard Publisher
...
| Feature flag | `skill_0996_evolution-kpi-dashboard-publishe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill keeps mission status observable in real time.

## Trigger Checklist
- [ ] The task explicitly needs Evolution KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Evolution KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:communication-engine`.
- **Review posture (score >= 62 or risk >= 49):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 86):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution KPI Dashboard Publisher`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.19, execution=0.25, safety=0.30, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 62
  - `review_risk`: risk >= 49
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
