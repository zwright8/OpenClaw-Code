...
description: Run the PublicService Skill Gap Diagnoser capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0437_publicservice-skill-gap-diagnose` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Skill Gap Diagnoser (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Skill Gap Diagnoser.
...
## Operator Use Cases
- Operate PublicService Skill Gap Diagnoser as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Skill Gap Diagnoser`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.21, execution=0.37, safety=0.24, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 57
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute competency coverage analysis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Skill Gap Diagnoser against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
