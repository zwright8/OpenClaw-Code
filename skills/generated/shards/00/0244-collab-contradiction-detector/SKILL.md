---
name: u0244-collab-contradiction-detector
description: Run the Collab Contradiction Detector capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Contradiction Detector
...
| Routing tag | `collaboration-and-negotiation:detection-guard` |
| Feature flag | `skill_0244_collab-contradiction-detector` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill catches internal and external claim conflicts early.

## Trigger Checklist
- [ ] The task explicitly needs Collab Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Contradiction Detector.
...
## Operator Use Cases
- Operate Collab Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `collaboration-and-negotiation:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Contradiction Detector`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.33, execution=0.21, safety=0.22, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 52
  - `review_risk`: risk >= 50
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for collaboration-and-negotiation:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `collaboration-and-negotiation:detection-guard` consumers with approval context attached.
