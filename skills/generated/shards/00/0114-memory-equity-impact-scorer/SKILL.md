...
description: Run the Memory Equity Impact Scorer capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0114_memory-equity-impact-scorer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Equity Impact Scorer.
...
## Operator Use Cases
- Operate Memory Equity Impact Scorer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `memory-and-knowledge-operations:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Equity Impact Scorer`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.13, execution=0.33, safety=0.19, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 43
  - `review_risk`: risk >= 38
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
