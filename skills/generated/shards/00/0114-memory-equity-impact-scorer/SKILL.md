...
description: Build and operate the "Memory Equity Impact Scorer" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Equity Impact Scorer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-equity-impact-scorer
...
## Output Contract
- `memory_equity_impact_scorer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_equity_impact_scorer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_equity_impact_scorer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Equity Impact Scorer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory equity impact scorer normalization/assessment.
