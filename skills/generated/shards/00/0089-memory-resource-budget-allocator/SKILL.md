...
description: Build and operate the "Memory Resource Budget Allocator" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Resource Budget Allocator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-resource-budget-allocator
...
## Output Contract
- `memory_resource_budget_allocator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_resource_budget_allocator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_resource_budget_allocator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Resource Budget Allocator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory resource budget allocator normalization/assessment.
