---
name: u0249-collab-resource-budget-allocator
description: Run the Collab Resource Budget Allocator capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Resource Budget Allocator
...
| Feature flag | `skill_0249_collab-resource-budget-allocator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill matches mission ambition to finite execution capacity.

## Trigger Checklist
- [ ] The task explicitly needs Collab Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Resource Budget Allocator.
...
## Operator Use Cases
- Operate Collab Resource Budget Allocator as a reliable, reusable production workflow.
...
- **Review posture (score >= 60 or risk >= 56):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 84):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Resource Budget Allocator`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.18, execution=0.30, safety=0.39, impact=0.13`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 60
  - `review_risk`: risk >= 56
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
