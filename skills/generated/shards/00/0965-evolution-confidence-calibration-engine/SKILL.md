...
description: Run the Evolution Confidence Calibration Engine capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0965_evolution-confidence-calibration` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Evolution Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Evolution Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Confidence Calibration Engine`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.26, execution=0.25, safety=0.29, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 43
  - `review_risk`: risk >= 31
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for autonomous-learning-and-evolution:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `autonomous-learning-and-evolution:optimization-engine` consumers with approval context attached.
