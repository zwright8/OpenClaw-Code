...
## When To Use
Use this skill only when the request explicitly needs `Federation Tool Health Monitor` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Tool Health Monitor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation tool health monitor normalization/assessment.
