...
description: Run the Federation Tool Health Monitor capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0294_federation-tool-health-monitor` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Federation Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Tool Health Monitor.
...
## Operator Use Cases
- Operate Federation Tool Health Monitor as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Tool Health Monitor`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.24, execution=0.24, safety=0.28, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 47
  - `review_risk`: risk >= 44
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
