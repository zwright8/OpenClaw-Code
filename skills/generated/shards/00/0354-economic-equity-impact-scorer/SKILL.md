...
description: Build and operate the "Economic Equity Impact Scorer" capability for Economic Optimization. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Economic Equity Impact Scorer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: economic-optimization:economic-equity-impact-scorer
...
## Output Contract
- `economic_equity_impact_scorer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `economic_equity_impact_scorer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `economic_equity_impact_scorer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Equity Impact Scorer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Economic Optimization**
   - Input: noisy upstream payload requiring economic equity impact scorer normalization/assessment.
