...
description: Run the Observability Resource Budget Allocator capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0569_observability-resource-budget-al` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Resource Budget Allocator.
...
## Operator Use Cases
- Operate Observability Resource Budget Allocator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `data-quality-and-observability:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Resource Budget Allocator`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.25, execution=0.17, safety=0.18, impact=0.39`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 58
  - `review_risk`: risk >= 54
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
