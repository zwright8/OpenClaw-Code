...
description: Build and operate the "Memory Tool Health Monitor" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Tool Health Monitor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-tool-health-monitor
...
## Output Contract
- `memory_tool_health_monitor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_tool_health_monitor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_tool_health_monitor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Tool Health Monitor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory tool health monitor normalization/assessment.
