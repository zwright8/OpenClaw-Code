---
name: u0611-security-policy-constraint-compiler
description: Run the Security Policy Constraint Compiler capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Policy Constraint Compiler
...
| Routing tag | `security-and-privacy:contract-compiler` |
| Feature flag | `skill_0611_security-policy-constraint-compi` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Security Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Security Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `security-and-privacy:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Policy Constraint Compiler`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.37, execution=0.21, safety=0.18, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 59
  - `review_risk`: risk >= 49
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `executable policy bundles`.
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for security-and-privacy:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `security-and-privacy:contract-compiler` consumers with approval context attached.
