---
name: u0154-tooling-equity-impact-scorer
description: Run the Tooling Equity Impact Scorer capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Equity Impact Scorer
...
| Feature flag | `skill_0154_tooling-equity-impact-scorer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill exposes uneven benefit/harm distribution before rollout.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Equity Impact Scorer.
...
## Operator Use Cases
- Operate Tooling Equity Impact Scorer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Equity Impact Scorer`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.21, execution=0.25, safety=0.33, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 55
  - `review_risk`: risk >= 49
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
