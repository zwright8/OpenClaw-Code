...
description: Build and operate the "Tooling Equity Impact Scorer" capability for Tool Reliability and Execution Quality. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Tooling Equity Impact Scorer** in the **Tool Reliability and Execution Quality** capability family.
...
## Handoff Contract
- Produces: Tooling Equity Impact Scorer normalized artifacts; execution scorecard; risk posture
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Equity Impact Scorer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling equity impact scorer normalization/assessment.
...
## When To Use
Use this when a request in **Tool Reliability and Execution Quality** depends on **Tooling Equity Impact Scorer** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
