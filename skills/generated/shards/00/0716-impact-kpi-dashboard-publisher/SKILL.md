---
name: u0716-impact-kpi-dashboard-publisher
description: Run the Impact KPI Dashboard Publisher capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact KPI Dashboard Publisher
...
| Feature flag | `skill_0716_impact-kpi-dashboard-publisher` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill keeps mission status observable in real time.

## Trigger Checklist
- [ ] The task explicitly needs Impact KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Impact KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `social-impact-measurement:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact KPI Dashboard Publisher`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.26, execution=0.29, safety=0.27, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 53
  - `review_risk`: risk >= 50
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
