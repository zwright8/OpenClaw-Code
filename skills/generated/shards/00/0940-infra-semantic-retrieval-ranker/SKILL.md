...
description: Run the Infra Semantic Retrieval Ranker capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0940_infra-semantic-retrieval-ranker` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill improves recall precision for downstream decision quality.

## Trigger Checklist
- [ ] The task explicitly needs Infra Semantic Retrieval Ranker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Semantic Retrieval Ranker.
...
## Operator Use Cases
- Operate Infra Semantic Retrieval Ranker as a reliable, reusable production workflow.
...
- **Review posture (score >= 45 or risk >= 41):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Semantic Retrieval Ranker`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.29, execution=0.23, safety=0.20, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 45
  - `review_risk`: risk >= 41
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute semantic relevance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Semantic Retrieval Ranker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
