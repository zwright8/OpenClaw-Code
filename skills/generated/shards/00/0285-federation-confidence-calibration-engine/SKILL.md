...
description: Operate the "Federation Confidence Calibration Engine" capability in production for Federation and Interoperability workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `285`
- **Skill Name:** `u0285-federation-confidence-calibration-engine`
...
4. Keep routing aligned with `federation-and-interoperability:optimization-engine` and include approval context.
...
### Handoff Contract
- **Produces:** `Federation Confidence Calibration Engine normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `envelopes`, `tenant boundaries`, `protocol bridges`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to federation-and-interoperability:optimization-engine consumers with approval-gate context
