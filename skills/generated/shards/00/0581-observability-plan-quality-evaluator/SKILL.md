...
description: Run the Observability Plan Quality Evaluator capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0581_observability-plan-quality-evalu` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Observability Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `data-quality-and-observability:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Plan Quality Evaluator`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.27, execution=0.23, safety=0.21, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 53
  - `review_risk`: risk >= 51
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
