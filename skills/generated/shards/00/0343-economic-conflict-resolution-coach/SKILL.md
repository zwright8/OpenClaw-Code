---
name: u0343-economic-conflict-resolution-coach
description: Run the Economic Conflict Resolution Coach capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Conflict Resolution Coach
...
| Feature flag | `skill_0343_economic-conflict-resolution-coa` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Economic Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Economic Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `economic-optimization:collaboration-mediator`.
- **Review posture (score >= 53 or risk >= 51):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 86):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Conflict Resolution Coach`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.20, execution=0.23, safety=0.23, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 53
  - `review_risk`: risk >= 51
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
