...
description: Run the Collab Human Approval Router capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0252_collab-human-approval-router` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Collab Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Human Approval Router.
...
## Operator Use Cases
- Operate Collab Human Approval Router as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Human Approval Router`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.19, execution=0.36, safety=0.21, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 48
  - `review_risk`: risk >= 37
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
