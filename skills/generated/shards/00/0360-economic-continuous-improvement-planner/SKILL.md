...
description: Build and operate the "Economic Continuous Improvement Planner" capability for Economic Optimization. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Economic Continuous Improvement Planner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: economic-optimization:economic-continuous-improvement-planner
...
## Output Contract
- `economic_continuous_improvement_planner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `economic_continuous_improvement_planner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `economic_continuous_improvement_planner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Continuous Improvement Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Economic Optimization**
   - Input: noisy upstream payload requiring economic continuous improvement planner normalization/assessment.
