...
description: Run the Rights Equity Impact Scorer capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0914_rights-equity-impact-scorer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Equity Impact Scorer.
...
## Operator Use Cases
- Operate Rights Equity Impact Scorer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `legal-rights-and-compliance:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Equity Impact Scorer`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.21, execution=0.23, safety=0.40, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 46
  - `review_risk`: risk >= 43
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
