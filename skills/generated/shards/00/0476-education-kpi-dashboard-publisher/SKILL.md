---
name: u0476-education-kpi-dashboard-publisher
description: Run the Education KPI Dashboard Publisher capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education KPI Dashboard Publisher
...
| Feature flag | `skill_0476_education-kpi-dashboard-publishe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill keeps mission status observable in real time.

## Trigger Checklist
- [ ] The task explicitly needs Education KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Education KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `education-and-upskilling:communication-engine`.
- **Review posture (score >= 58 or risk >= 56):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 87):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education KPI Dashboard Publisher`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.22, execution=0.27, safety=0.28, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 58
  - `review_risk`: risk >= 56
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
