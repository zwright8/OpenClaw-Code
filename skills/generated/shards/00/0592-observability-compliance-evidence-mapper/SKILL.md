...
description: Run the Observability Compliance Evidence Mapper capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `data-quality-and-observability:normalization-engine` |
| Feature flag | `skill_0592_observability-compliance-evidenc` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Observability Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `data-quality-and-observability:normalization-engine`.
- **Review posture (score >= 44 or risk >= 38):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Compliance Evidence Mapper`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.23, execution=0.22, safety=0.29, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 44
  - `review_risk`: risk >= 38
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for data-quality-and-observability:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `data-quality-and-observability:normalization-engine` consumers with approval context attached.
