---
name: u0755-comms-community-feedback-harvester
description: Run the Comms Community Feedback Harvester capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Community Feedback Harvester
...
| Feature flag | `skill_0755_comms-community-feedback-harvest` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill integrates lived user feedback into planning cycles.

## Trigger Checklist
- [ ] The task explicitly needs Comms Community Feedback Harvester (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Community Feedback Harvester.
...
## Operator Use Cases
- Operate Comms Community Feedback Harvester as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `communication-and-explainability:normalization-engine`.
- **Review posture (score >= 42 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Community Feedback Harvester`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.16, execution=0.30, safety=0.29, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 42
  - `review_risk`: risk >= 32
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute feedback normalization and clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Community Feedback Harvester against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
