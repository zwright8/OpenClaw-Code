...
description: Run the PublicService Dependency DAG Planner capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0408_publicservice-dependency-dag-pla` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Dependency DAG Planner.
...
## Operator Use Cases
- Operate PublicService Dependency DAG Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `healthcare-and-public-services:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Dependency DAG Planner`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.29, execution=0.31, safety=0.18, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 55
  - `review_risk`: risk >= 61
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
