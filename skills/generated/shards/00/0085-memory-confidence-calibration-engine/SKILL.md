...
description: Build and operate the "Memory Confidence Calibration Engine" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Confidence Calibration Engine" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-confidence-calibration-engine
...
## Output Contract
- `memory_confidence_calibration_engine_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_confidence_calibration_engine_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_confidence_calibration_engine_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Confidence Calibration Engine** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory confidence calibration engine normalization/assessment.
