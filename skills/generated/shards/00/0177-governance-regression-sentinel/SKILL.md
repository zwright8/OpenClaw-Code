...
description: Run the Governance Regression Sentinel capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Regression Sentinel.
...
## Operator Use Cases
- Operate Governance Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `safety-and-governance:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Regression Sentinel`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.35, execution=0.29, safety=0.18, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 56
  - `review_risk`: risk >= 47
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
