...
description: Run the Logistics Conflict Resolution Coach capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0383_logistics-conflict-resolution-co` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Logistics Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Logistics Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `resource-allocation-and-logistics:collaboration-mediator`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Conflict Resolution Coach`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.15, execution=0.33, safety=0.36, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 52
  - `review_risk`: risk >= 34
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:collaboration-mediator` consumers with approval context attached.
