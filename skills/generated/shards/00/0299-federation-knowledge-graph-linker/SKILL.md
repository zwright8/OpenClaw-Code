...
## When To Use
Use this skill only when the request explicitly needs `Federation Knowledge Graph Linker` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Knowledge Graph Linker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation knowledge graph linker normalization/assessment.
