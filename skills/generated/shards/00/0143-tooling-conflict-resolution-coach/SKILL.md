...
## When To Use
Use this skill when you need "Tooling Conflict Resolution Coach" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-conflict-resolution-coach
...
## Output Contract
- `tooling_conflict_resolution_coach_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_conflict_resolution_coach_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_conflict_resolution_coach_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Conflict Resolution Coach** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling conflict resolution coach normalization/assessment.
