...
description: Run the Tooling Conflict Resolution Coach capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Tooling Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Tooling Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:collaboration-mediator`.
- **Review posture (score >= 51 or risk >= 44):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Conflict Resolution Coach`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.25, execution=0.24, safety=0.22, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 51
  - `review_risk`: risk >= 44
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for tool-reliability-and-execution-quality:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `tool-reliability-and-execution-quality:collaboration-mediator` consumers with approval context attached.
