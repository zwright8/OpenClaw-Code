---
name: u0338-economic-memory-consolidation-pipeline
description: Run the Economic Memory Consolidation Pipeline capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Memory Consolidation Pipeline
...
| Feature flag | `skill_0338_economic-memory-consolidation-pi` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs Economic Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Economic Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `economic-optimization:general-capability`.
- **Review posture (score >= 51 or risk >= 43):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Memory Consolidation Pipeline`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.26, execution=0.25, safety=0.19, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 51
  - `review_risk`: risk >= 43
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
