...
description: Build and operate the "Memory Multi-Agent Negotiation Mediator" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Multi-Agent Negotiation Mediator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-multi-agent-negotiation-mediator
...
## Output Contract
- `memory_multi_agent_negotiation_mediator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_multi_agent_negotiation_mediator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_multi_agent_negotiation_mediator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Multi-Agent Negotiation Mediator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory multi-agent negotiation mediator normalization/assessment.
