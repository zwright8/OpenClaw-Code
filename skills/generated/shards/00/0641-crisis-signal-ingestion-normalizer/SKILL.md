...
description: Run the Crisis Signal Ingestion Normalizer capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0641_crisis-signal-ingestion-normaliz` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Crisis Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `crisis-and-incident-response:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Signal Ingestion Normalizer`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.21, execution=0.19, safety=0.26, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 54
  - `review_risk`: risk >= 51
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
