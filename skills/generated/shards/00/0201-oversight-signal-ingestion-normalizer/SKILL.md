...
description: Run the Oversight Signal Ingestion Normalizer capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0201_oversight-signal-ingestion-norma` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Oversight Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Oversight Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `human-oversight-and-operator-ux:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Signal Ingestion Normalizer`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.13, execution=0.27, safety=0.31, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 62
  - `review_risk`: risk >= 52
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
