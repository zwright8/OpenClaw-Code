...
description: Run the Security Disaster Recovery Orchestrator capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0629_security-disaster-recovery-orche` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate Security Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `security-and-privacy:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.17, execution=0.21, safety=0.25, impact=0.38`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 51
  - `review_risk`: risk >= 32
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
