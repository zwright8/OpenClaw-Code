...
description: Run the Rights Resource Budget Allocator capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0889_rights-resource-budget-allocator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Resource Budget Allocator.
...
## Operator Use Cases
- Operate Rights Resource Budget Allocator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Resource Budget Allocator`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.16, execution=0.37, safety=0.28, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 57
  - `review_risk`: risk >= 52
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
