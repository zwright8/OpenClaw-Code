...
description: Operate the "Economic Multi-Agent Negotiation Mediator" capability in production for the target domain workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill only when the request explicitly needs `Economic Multi-Agent Negotiation Mediator` in the target domain and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Multi-Agent Negotiation Mediator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in the target domain workflows**
   - Input: noisy upstream payload requiring economic multi-agent negotiation mediator normalization/assessment.
