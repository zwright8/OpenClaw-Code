---
name: u0481-research-signal-ingestion-normalizer
description: Run the Research Signal Ingestion Normalizer capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Signal Ingestion Normalizer
...
| Feature flag | `skill_0481_research-signal-ingestion-normal` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Research Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Research Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `scientific-research-systems:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Signal Ingestion Normalizer`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.30, execution=0.18, safety=0.33, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 54
  - `review_risk`: risk >= 53
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
