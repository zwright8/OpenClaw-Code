---
name: u0636-security-kpi-dashboard-publisher
description: Run the Security KPI Dashboard Publisher capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security KPI Dashboard Publisher
...
| Feature flag | `skill_0636_security-kpi-dashboard-publisher` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill keeps mission status observable in real time.

## Trigger Checklist
- [ ] The task explicitly needs Security KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Security KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `security-and-privacy:communication-engine`.
- **Review posture (score >= 62 or risk >= 51):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 84):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security KPI Dashboard Publisher`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.17, execution=0.25, safety=0.41, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 62
  - `review_risk`: risk >= 51
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
