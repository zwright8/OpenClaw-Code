...
description: Run the Governance Equity Impact Scorer capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0194_governance-equity-impact-scorer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Equity Impact Scorer.
...
## Operator Use Cases
- Operate Governance Equity Impact Scorer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `safety-and-governance:general-capability`.
- **Review posture (score >= 61 or risk >= 60):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Equity Impact Scorer`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.25, execution=0.14, safety=0.31, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 61
  - `review_risk`: risk >= 60
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
