...
description: Run the Comms Continuous Improvement Planner capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0760_comms-continuous-improvement-pla` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Comms Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Comms Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `communication-and-explainability:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Continuous Improvement Planner`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.16, execution=0.30, safety=0.14, impact=0.41`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 42
  - `review_risk`: risk >= 42
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
