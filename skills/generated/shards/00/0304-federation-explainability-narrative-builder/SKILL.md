---
name: u0304-federation-explainability-narrative-builder
description: Run the Federation Explainability Narrative Builder capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Explainability Narrative Builder
...
| Feature flag | `skill_0304_federation-explainability-narrat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Federation Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Federation Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `federation-and-interoperability:communication-engine`.
- **Review posture (score >= 58 or risk >= 52):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 79):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Explainability Narrative Builder`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.33, execution=0.21, safety=0.19, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 58
  - `review_risk`: risk >= 52
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
