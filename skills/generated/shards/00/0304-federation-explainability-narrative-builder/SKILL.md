...
## When To Use
Use this skill only when the request explicitly needs `Federation Explainability Narrative Builder` in the target domain and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Explainability Narrative Builder** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **the target domain workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new the target domain workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in the target domain workflows**
   - Input: noisy upstream payload requiring federation explainability narrative builder normalization/assessment.
