...
description: Run the Observability Signal Ingestion Normalizer capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0561_observability-signal-ingestion-n` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Observability Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `data-quality-and-observability:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Signal Ingestion Normalizer`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.16, execution=0.39, safety=0.17, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 46
  - `review_risk`: risk >= 40
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
