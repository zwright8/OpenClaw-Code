...
description: Run the Observability Confidence Calibration Engine capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `data-quality-and-observability:optimization-engine` |
| Feature flag | `skill_0565_observability-confidence-calibra` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Observability Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Observability Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `data-quality-and-observability:optimization-engine`.
- **Review posture (score >= 44 or risk >= 41):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 71):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Confidence Calibration Engine`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.25, execution=0.23, safety=0.19, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 44
  - `review_risk`: risk >= 41
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for data-quality-and-observability:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `data-quality-and-observability:optimization-engine` consumers with approval context attached.
