...
description: Run the Tooling Security Threat Modeler capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0151_tooling-security-threat-modeler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Tooling Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Security Threat Modeler.
...
## Operator Use Cases
- Operate Tooling Security Threat Modeler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Security Threat Modeler`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.18, execution=0.20, safety=0.28, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 50
  - `review_risk`: risk >= 36
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
