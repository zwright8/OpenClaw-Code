...
## When To Use
Use this skill only when the request explicitly needs `Federation Counterfactual Simulator` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Counterfactual Simulator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation counterfactual simulator normalization/assessment.
