---
name: u0286-federation-counterfactual-simulator
description: Run the Federation Counterfactual Simulator capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Counterfactual Simulator
...
| Routing tag | `federation-and-interoperability:simulation-lab` |
| Feature flag | `skill_0286_federation-counterfactual-simula` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Federation Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Counterfactual Simulator.
...
## Operator Use Cases
- Operate Federation Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `federation-and-interoperability:simulation-lab`.
- **Review posture (score >= 44 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Counterfactual Simulator`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.19, execution=0.32, safety=0.23, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 44
  - `review_risk`: risk >= 46
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for federation-and-interoperability:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `federation-and-interoperability:simulation-lab` consumers with approval context attached.
