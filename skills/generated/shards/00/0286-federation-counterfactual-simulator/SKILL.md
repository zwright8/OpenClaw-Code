...
description: Operate the "Federation Counterfactual Simulator" capability in production for Federation and Interoperability workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `286`
- **Skill Name:** `u0286-federation-counterfactual-simulator`
- **Domain:** `Federation and Interoperability`
- **Domain Slug:** `federation-and-interoperability`
...
|---|---|---|---|
| envelopes | signal | true | upstream |
| tenant boundaries | signal | true | upstream |
| protocol bridges | signal | true | upstream |
...
|---|---|---|---|
| scenario_comparison_reports_report | structured-report | true | orchestrator |
| scenario_comparison_reports_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `counterfactual replay` with deterministic settings and trace capture enabled.
3. Produce `scenario comparison reports` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `federation-and-interoperability:simulation-lab` and include approval context.
5. Tune thresholds incrementally based on observed KPI drift and incident learnings.

## Musts
- Enforce approval gates: `policy-constraint-check`, `human-approval-router`, `scenario-review`.
...
- **Week:** Review KPI focus (`integration breakage`, `trust boundary violations`, `decision drift`), failure trends, and approval/retry performance.
...
### Handoff Contract
- **Produces:** `Federation Counterfactual Simulator normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `envelopes`, `tenant boundaries`, `protocol bridges`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to federation-and-interoperability:simulation-lab consumers with approval-gate context
