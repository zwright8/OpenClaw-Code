---
name: u0969-evolution-resource-budget-allocator
description: Run the Evolution Resource Budget Allocator capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Evolution Resource Budget Allocator
...
| Feature flag | `skill_0969_evolution-resource-budget-alloca` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill matches mission ambition to finite execution capacity.

## Trigger Checklist
- [ ] The task explicitly needs Evolution Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Resource Budget Allocator.
...
## Operator Use Cases
- Operate Evolution Resource Budget Allocator as a reliable, reusable production workflow.
...
- **Review posture (score >= 48 or risk >= 37):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Resource Budget Allocator`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.27, execution=0.27, safety=0.21, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 48
  - `review_risk`: risk >= 37
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
