---
name: u0326-economic-counterfactual-simulator
description: Run the Economic Counterfactual Simulator capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Counterfactual Simulator
...
| Feature flag | `skill_0326_economic-counterfactual-simulato` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Economic Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Counterfactual Simulator.
...
## Operator Use Cases
- Operate Economic Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `economic-optimization:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Counterfactual Simulator`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.34, execution=0.22, safety=0.26, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 47
  - `review_risk`: risk >= 41
...
- Approval gates: `policy-constraint-check`, `human-approval-router`, `budget-review`, `scenario-review`.
...
| Required approvals | `policy-constraint-check`, `human-approval-router`, `budget-review`, `scenario-review` |
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `scenario comparison reports`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `scenario comparison reports`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `scenario comparison reports`.
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:simulation-lab` consumers with approval context attached.
