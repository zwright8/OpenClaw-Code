...
description: Operate the "Security Continuous Improvement Planner" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill when the request explicitly needs "Security Continuous Improvement Planner" outcomes in the Security and Privacy domain.
...
## Handoff Contract
- Produces: Security Continuous Improvement Planner normalized artifacts; execution scorecard; risk posture
- Consumes: permissions; sensitive data flows; threat events; claims; evidence; confidence traces
- Downstream routing hint: Route next to security-and-privacy:planning-router consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Security Continuous Improvement Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in workflows**
   - Input: noisy upstream payload requiring security continuous improvement planner normalization/assessment.
