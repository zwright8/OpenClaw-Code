---
name: u0047-planning-goal-decomposer
description: Run the Planning Goal Decomposer capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Planning Goal Decomposer
...
| Primary artifact | `atomic task trees` |
| Routing tag | `strategic-planning-and-decomposition:planning-router` |
| Feature flag | `skill_0047_planning-goal-decomposer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because large goals fail when decomposition is inconsistent or incomplete. This specific skill breaks ambiguous missions into executable units.

## Trigger Checklist
- [ ] The task explicitly needs Planning Goal Decomposer (not generic brainstorming).
...
|---|---|---|---|
| atomic task trees | structured-artifact | yes | downstream orchestrator |
| atomic task trees-scorecard | scorecard | yes | operator / reviewer |
| atomic task trees-handoff | handoff-packet | yes | next owner |
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Goal Decomposer.

### Execution
- Execute hierarchical decomposition deterministically and capture reproducible traces.
...
## Operator Use Cases
- Operate Planning Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:planning-router`.
- **Review posture (score >= 44 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Goal Decomposer`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- **Core:** Implement the core capability using hierarchical decomposition, and produce atomic task trees with deterministic scoring.
...
- Scoring weights: `truth=0.36, execution=0.20, safety=0.23, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 44
  - `review_risk`: risk >= 40
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `atomic task trees`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `atomic task trees`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `atomic task trees`.
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for strategic-planning-and-decomposition:planning-router, enforce approvals, then handoff to downstream orchestrator with next actions.
...
## Handoff Contract
- **Produces:** `atomic task trees`, scorecard, risk/confidence metadata, remediation backlog.
- **Consumes:** `goals`, `dependencies`, `milestones`, `constraints`, `claims`, `evidence`, `confidence traces`.
- **Readiness rule:** release only when schema, determinism, policy, and reliability gates all pass.
- **Downstream hint:** route only to `strategic-planning-and-decomposition:planning-router` consumers with approval context attached.
