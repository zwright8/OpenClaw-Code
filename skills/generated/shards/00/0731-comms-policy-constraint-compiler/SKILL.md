---
name: u0731-comms-policy-constraint-compiler
description: Run the Comms Policy Constraint Compiler capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Policy Constraint Compiler
...
| Feature flag | `skill_0731_comms-policy-constraint-compiler` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Comms Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Comms Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `communication-and-explainability:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Policy Constraint Compiler`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.26, execution=0.33, safety=0.14, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 52
  - `review_risk`: risk >= 37
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
