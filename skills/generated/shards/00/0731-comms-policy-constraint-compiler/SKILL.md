...
description: Operate the "Comms Policy Constraint Compiler" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill when the request explicitly needs "Comms Policy Constraint Compiler" outcomes in the Communication and Explainability domain.
...
## Handoff Contract
- Produces: Comms Policy Constraint Compiler normalized artifacts; execution scorecard; risk posture
- Consumes: decision factors; uncertainty markers; audience summaries; claims; evidence; confidence traces
- Downstream routing hint: Route next to communication-and-explainability:contract-compiler consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Comms Policy Constraint Compiler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in workflows**
   - Input: noisy upstream payload requiring comms policy constraint compiler normalization/assessment.
