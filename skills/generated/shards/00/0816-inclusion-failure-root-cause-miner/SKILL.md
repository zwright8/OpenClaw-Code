---
name: u0816-inclusion-failure-root-cause-miner
description: Run the Inclusion Failure Root-Cause Miner capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Inclusion Failure Root-Cause Miner
...
| Feature flag | `skill_0816_inclusion-failure-root-cause-min` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Inclusion Failure Root-Cause Miner as a reliable, reusable production workflow.
...
- **Review posture (score >= 48 or risk >= 38):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Failure Root-Cause Miner`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.29, execution=0.25, safety=0.29, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 48
  - `review_risk`: risk >= 38
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
