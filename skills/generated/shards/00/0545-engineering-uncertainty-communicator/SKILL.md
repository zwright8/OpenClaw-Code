---
name: u0545-engineering-uncertainty-communicator
description: Run the Engineering Uncertainty Communicator capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Uncertainty Communicator
...
| Feature flag | `skill_0545_engineering-uncertainty-communic` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Uncertainty Communicator.
...
## Operator Use Cases
- Operate Engineering Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `software-engineering-automation:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Uncertainty Communicator`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.35, execution=0.29, safety=0.20, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 52
  - `review_risk`: risk >= 47
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
