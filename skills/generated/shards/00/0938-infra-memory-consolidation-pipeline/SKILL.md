---
name: u0938-infra-memory-consolidation-pipeline
description: Run the Infra Memory Consolidation Pipeline capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Infra Memory Consolidation Pipeline
...
| Feature flag | `skill_0938_infra-memory-consolidation-pipel` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs Infra Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Infra Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `infrastructure-and-sustainability:general-capability`.
- **Review posture (score >= 53 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 78):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Memory Consolidation Pipeline`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.28, execution=0.26, safety=0.24, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 53
  - `review_risk`: risk >= 46
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
