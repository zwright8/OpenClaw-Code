...
description: Operate the "Federation Context Window Prioritizer" capability in production for Federation and Interoperability workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `282`
- **Skill Name:** `u0282-federation-context-window-prioritizer`
...
|---|---|---|---|
| ranked_context_bundles_report | structured-report | true | orchestrator |
| ranked_context_bundles_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `importance scoring` with deterministic settings and trace capture enabled.
3. Produce `ranked context bundles` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `federation-and-interoperability:planning-router` and include approval context.
...
### Handoff Contract
- **Produces:** `Federation Context Window Prioritizer normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `envelopes`, `tenant boundaries`, `protocol bridges`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to federation-and-interoperability:planning-router consumers with approval-gate context
