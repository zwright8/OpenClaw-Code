...
description: Run the Security Security Threat Modeler capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0631_security-security-threat-modeler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Security Threat Modeler.
...
## Operator Use Cases
- Operate Security Security Threat Modeler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `security-and-privacy:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Security Threat Modeler`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.19, execution=0.32, safety=0.17, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 54
  - `review_risk`: risk >= 50
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
