...
description: Build and operate the "Tooling Signal Ingestion Normalizer" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Signal Ingestion Normalizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-signal-ingestion-normalizer
...
## Output Contract
- `tooling_signal_ingestion_normalizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_signal_ingestion_normalizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_signal_ingestion_normalizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Signal Ingestion Normalizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling signal ingestion normalizer normalization/assessment.
