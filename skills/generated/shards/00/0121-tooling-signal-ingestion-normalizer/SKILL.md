...
description: Run the Tooling Signal Ingestion Normalizer capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0121_tooling-signal-ingestion-normali` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Tooling Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Tooling Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Signal Ingestion Normalizer`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.33, execution=0.18, safety=0.22, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 48
  - `review_risk`: risk >= 43
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
