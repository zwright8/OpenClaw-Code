...
description: Build and operate the "Tooling Skill Gap Diagnoser" capability for Tool Reliability and Execution Quality. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Tooling Skill Gap Diagnoser** in the **Tool Reliability and Execution Quality** capability family.
...
## Handoff Contract
- Produces: Tooling Skill Gap Diagnoser normalized artifacts; execution scorecard; risk posture
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Skill Gap Diagnoser** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling skill gap diagnoser normalization/assessment.
...
## When To Use
Use this when a request in **Tool Reliability and Execution Quality** depends on **Tooling Skill Gap Diagnoser** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
