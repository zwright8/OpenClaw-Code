...
description: Operate the "Federation KPI Dashboard Publisher" capability in production for the target domain workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `316`
- **Skill Name:** `u0316-federation-kpi-dashboard-publisher`
...
4. Keep routing aligned with `federation-and-interoperability:communication-engine` and include approval context.
...
### Handoff Contract
- **Produces:** `Federation KPI Dashboard Publisher normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `envelopes`, `tenant boundaries`, `protocol bridges`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to federation-and-interoperability:communication-engine consumers with approval-gate context
