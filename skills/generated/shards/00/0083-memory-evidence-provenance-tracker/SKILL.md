---
name: u0083-memory-evidence-provenance-tracker
description: Run the Memory Evidence Provenance Tracker capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory Evidence Provenance Tracker
...
| Routing tag | `memory-and-knowledge-operations:provenance-tracker` |
| Feature flag | `skill_0083_memory-evidence-provenance-track` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents lose performance when lessons are not retained and reused. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Memory Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Memory Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `memory-and-knowledge-operations:provenance-tracker`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Evidence Provenance Tracker`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.22, execution=0.26, safety=0.35, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 49
  - `review_risk`: risk >= 46
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for memory-and-knowledge-operations:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `memory-and-knowledge-operations:provenance-tracker` consumers with approval context attached.
