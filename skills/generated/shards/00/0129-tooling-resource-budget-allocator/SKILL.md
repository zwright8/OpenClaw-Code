...
description: Build and operate the "Tooling Resource Budget Allocator" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Resource Budget Allocator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-resource-budget-allocator
...
## Output Contract
- `tooling_resource_budget_allocator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_resource_budget_allocator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_resource_budget_allocator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Resource Budget Allocator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling resource budget allocator normalization/assessment.
