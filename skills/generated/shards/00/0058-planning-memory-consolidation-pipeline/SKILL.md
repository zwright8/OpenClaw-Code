...
description: Run the Planning Memory Consolidation Pipeline capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0058_planning-memory-consolidation-pi` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Planning Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Planning Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:general-capability`.
- **Review posture (score >= 54 or risk >= 52):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Memory Consolidation Pipeline`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.20, execution=0.30, safety=0.23, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 54
  - `review_risk`: risk >= 52
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
