---
name: u0146-tooling-experiment-design-generator
description: Run the Tooling Experiment Design Generator capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Experiment Design Generator
...
| Feature flag | `skill_0146_tooling-experiment-design-genera` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill converts unknowns into testable learning loops.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Experiment Design Generator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Experiment Design Generator.
...
## Operator Use Cases
- Operate Tooling Experiment Design Generator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:general-capability`.
- **Review posture (score >= 50 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 65):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Experiment Design Generator`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.19, execution=0.37, safety=0.32, impact=0.12`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 50
  - `review_risk`: risk >= 46
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute hypothesis-driven design, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Experiment Design Generator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
