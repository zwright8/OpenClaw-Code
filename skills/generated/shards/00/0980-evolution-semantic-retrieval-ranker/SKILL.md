...
description: Operate the "Evolution Semantic Retrieval Ranker" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `980`
- **Skill Name:** `u0980-evolution-semantic-retrieval-ranker`
...
|---|---|---|---|
| ranked_retrieval_results_report | structured-report | true | orchestrator |
| ranked_retrieval_results_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `semantic relevance scoring` with deterministic settings and trace capture enabled.
3. Produce `ranked retrieval results` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `autonomous-learning-and-evolution:planning-router` and include approval context.
...
### Handoff Contract
- **Produces:** `Evolution Semantic Retrieval Ranker normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `outcomes`, `error taxonomies`, `adaptation decisions`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to autonomous-learning-and-evolution:planning-router consumers with approval-gate context
