---
name: u0656-crisis-failure-root-cause-miner
description: Run the Crisis Failure Root-Cause Miner capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Failure Root-Cause Miner
...
| Feature flag | `skill_0656_crisis-failure-root-cause-miner` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Crisis Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `crisis-and-incident-response:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Failure Root-Cause Miner`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.26, execution=0.21, safety=0.28, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 52
  - `review_risk`: risk >= 40
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
