...
description: Operate the "Federation Task Handoff Contractor" capability in production for Federation and Interoperability workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill only when the request explicitly needs `Federation Task Handoff Contractor` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Task Handoff Contractor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation task handoff contractor normalization/assessment.
