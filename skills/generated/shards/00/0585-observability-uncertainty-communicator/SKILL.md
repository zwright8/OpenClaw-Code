---
name: u0585-observability-uncertainty-communicator
description: Run the Observability Uncertainty Communicator capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Uncertainty Communicator
...
| Feature flag | `skill_0585_observability-uncertainty-commun` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Observability Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Uncertainty Communicator.
...
## Operator Use Cases
- Operate Observability Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `data-quality-and-observability:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Uncertainty Communicator`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.21, execution=0.19, safety=0.33, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 55
  - `review_risk`: risk >= 52
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
