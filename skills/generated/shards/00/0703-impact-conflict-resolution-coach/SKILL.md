---
name: u0703-impact-conflict-resolution-coach
description: Run the Impact Conflict Resolution Coach capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Conflict Resolution Coach
...
| Feature flag | `skill_0703_impact-conflict-resolution-coach` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Impact Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Impact Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `social-impact-measurement:collaboration-mediator`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Conflict Resolution Coach`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.31, execution=0.16, safety=0.20, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 51
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:collaboration-mediator` consumers with approval context attached.
