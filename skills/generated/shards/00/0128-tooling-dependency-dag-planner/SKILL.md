...
description: Build and operate the "Tooling Dependency DAG Planner" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Dependency DAG Planner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-dependency-dag-planner
...
## Output Contract
- `tooling_dependency_dag_planner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_dependency_dag_planner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_dependency_dag_planner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Dependency DAG Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling dependency dag planner normalization/assessment.
