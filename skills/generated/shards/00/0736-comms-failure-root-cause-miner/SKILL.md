---
name: u0736-comms-failure-root-cause-miner
description: Run the Comms Failure Root-Cause Miner capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Failure Root-Cause Miner
...
| Feature flag | `skill_0736_comms-failure-root-cause-miner` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Comms Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Comms Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Failure Root-Cause Miner`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.27, execution=0.22, safety=0.21, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 47
  - `review_risk`: risk >= 40
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
