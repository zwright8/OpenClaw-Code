...
description: Run the Rights Confidence Calibration Engine capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `legal-rights-and-compliance:optimization-engine` |
| Feature flag | `skill_0885_rights-confidence-calibration-en` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Rights Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Rights Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `legal-rights-and-compliance:optimization-engine`.
- **Review posture (score >= 48 or risk >= 45):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Confidence Calibration Engine`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.35, execution=0.17, safety=0.35, impact=0.13`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 48
  - `review_risk`: risk >= 45
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for legal-rights-and-compliance:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `legal-rights-and-compliance:optimization-engine` consumers with approval context attached.
