...
description: Run the Planning Evidence Provenance Tracker capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `strategic-planning-and-decomposition:provenance-tracker` |
| Feature flag | `skill_0043_planning-evidence-provenance-tra` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Planning Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Planning Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:provenance-tracker`.
- **Review posture (score >= 61 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 89):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Evidence Provenance Tracker`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for strategic-planning-and-decomposition:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `strategic-planning-and-decomposition:provenance-tracker` consumers with approval context attached.
