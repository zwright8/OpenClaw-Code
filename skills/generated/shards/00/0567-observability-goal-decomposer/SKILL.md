...
description: Run the Observability Goal Decomposer capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0567_observability-goal-decomposer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Goal Decomposer.
...
## Operator Use Cases
- Operate Observability Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `data-quality-and-observability:planning-router`.
- **Review posture (score >= 48 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 59):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Goal Decomposer`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.22, execution=0.19, safety=0.27, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 48
  - `review_risk`: risk >= 40
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
