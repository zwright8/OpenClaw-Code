---
name: u0429-publicservice-disaster-recovery-orchestrator
description: Run the PublicService Disaster Recovery Orchestrator capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Disaster Recovery Orchestrator
...
| Feature flag | `skill_0429_publicservice-disaster-recovery-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill improves recovery speed after severe outages.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate PublicService Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `healthcare-and-public-services:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.26, execution=0.24, safety=0.31, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 52
  - `review_risk`: risk >= 48
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `recovery mission plans`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
