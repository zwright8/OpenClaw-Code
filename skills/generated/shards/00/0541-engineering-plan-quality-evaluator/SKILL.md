---
name: u0541-engineering-plan-quality-evaluator
description: Run the Engineering Plan Quality Evaluator capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Plan Quality Evaluator
...
| Feature flag | `skill_0541_engineering-plan-quality-evaluat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill quantifies whether plans are complete, safe, and feasible.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Engineering Plan Quality Evaluator as a reliable, reusable production workflow.
...
- **Review posture (score >= 57 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 79):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Plan Quality Evaluator`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.24, execution=0.29, safety=0.18, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 57
  - `review_risk`: risk >= 46
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
