---
name: u0169-governance-resource-budget-allocator
description: Run the Governance Resource Budget Allocator capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0169_governance-resource-budget-alloc` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Resource Budget Allocator.
...
## Operator Use Cases
- Operate Governance Resource Budget Allocator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `safety-and-governance:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Resource Budget Allocator`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.33, execution=0.17, safety=0.27, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 60
  - `review_risk`: risk >= 48
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
