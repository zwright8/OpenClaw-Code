...
description: Run the Cultural Tool Health Monitor capability for Multilingual and Cultural Adaptation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0774_cultural-tool-health-monitor` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Cultural Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Cultural Tool Health Monitor.
...
## Operator Use Cases
- Operate Cultural Tool Health Monitor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `multilingual-and-cultural-adaptation:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Cultural Tool Health Monitor`, including at least three measurable KPIs tied to context mismatch and exclusion.
...
- Scoring weights: `truth=0.23, execution=0.25, safety=0.26, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 53
  - `review_risk`: risk >= 61
...
1. Incident recovery in Multilingual and Cultural Adaptation: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Cultural Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
