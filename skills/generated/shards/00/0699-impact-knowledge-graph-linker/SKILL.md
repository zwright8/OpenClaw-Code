...
description: Run the Impact Knowledge Graph Linker capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0699_impact-knowledge-graph-linker` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Knowledge Graph Linker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Knowledge Graph Linker.
...
## Operator Use Cases
- Operate Impact Knowledge Graph Linker as a reliable, reusable production workflow.
...
- **Review posture (score >= 43 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 67):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Knowledge Graph Linker`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.33, execution=0.15, safety=0.19, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 43
  - `review_risk`: risk >= 40
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute entity and relation linking, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Knowledge Graph Linker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
