---
name: u0333-economic-task-handoff-contractor
description: Run the Economic Task Handoff Contractor capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Task Handoff Contractor
...
| Routing tag | `economic-optimization:contract-compiler` |
| Feature flag | `skill_0333_economic-task-handoff-contractor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill standardizes handoffs between agents and humans.

## Trigger Checklist
- [ ] The task explicitly needs Economic Task Handoff Contractor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Task Handoff Contractor.
...
## Operator Use Cases
- Operate Economic Task Handoff Contractor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `economic-optimization:contract-compiler`.
- **Review posture (score >= 59 or risk >= 52):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 75):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Task Handoff Contractor`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.29, execution=0.29, safety=0.20, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 59
  - `review_risk`: risk >= 52
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute contracted payload schemas, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Task Handoff Contractor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:contract-compiler` consumers with approval context attached.
