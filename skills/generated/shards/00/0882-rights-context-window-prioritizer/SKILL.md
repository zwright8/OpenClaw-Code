---
name: u0882-rights-context-window-prioritizer
description: Run the Rights Context Window Prioritizer capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Context Window Prioritizer
...
| Feature flag | `skill_0882_rights-context-window-prioritize` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill surfaces the most decision-relevant context under tight token budgets.

## Trigger Checklist
- [ ] The task explicitly needs Rights Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Context Window Prioritizer.
...
## Operator Use Cases
- Operate Rights Context Window Prioritizer as a reliable, reusable production workflow.
...
- **Review posture (score >= 48 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 65):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Context Window Prioritizer`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.25, execution=0.20, safety=0.28, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 48
  - `review_risk`: risk >= 46
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `ranked context bundles`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
