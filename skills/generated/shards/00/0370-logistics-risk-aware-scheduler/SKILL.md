...
## When To Use
Use this skill when you need "Logistics Risk-Aware Scheduler" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-risk-aware-scheduler
...
## Output Contract
- `logistics_risk_aware_scheduler_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_risk_aware_scheduler_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_risk_aware_scheduler_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Risk-Aware Scheduler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics risk-aware scheduler normalization/assessment.
