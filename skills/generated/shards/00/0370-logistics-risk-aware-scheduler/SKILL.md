---
name: u0370-logistics-risk-aware-scheduler
description: Run the Logistics Risk-Aware Scheduler capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Risk-Aware Scheduler
...
| Feature flag | `skill_0370_logistics-risk-aware-scheduler` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill times execution based on risk posture instead of urgency alone.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Logistics Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `resource-allocation-and-logistics:planning-router`.
- **Review posture (score >= 48 or risk >= 35):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 61):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Risk-Aware Scheduler`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.26, execution=0.31, safety=0.21, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 48
  - `review_risk`: risk >= 35
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
