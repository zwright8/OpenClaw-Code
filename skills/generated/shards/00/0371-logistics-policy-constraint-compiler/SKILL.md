...
description: Run the Logistics Policy Constraint Compiler capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0371_logistics-policy-constraint-comp` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Logistics Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Logistics Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `resource-allocation-and-logistics:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Policy Constraint Compiler`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.23, execution=0.14, safety=0.37, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 46
  - `review_risk`: risk >= 32
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:contract-compiler` consumers with approval context attached.
