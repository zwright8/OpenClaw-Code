...
description: Operate the "Inclusion Goal Decomposer" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `807`
- **Skill Name:** `u0807-inclusion-goal-decomposer`
...
|---|---|---|---|
| atomic_task_trees_report | structured-report | true | orchestrator |
| atomic_task_trees_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `hierarchical decomposition` with deterministic settings and trace capture enabled.
3. Produce `atomic task trees` outputs in machine-readable form for orchestrator/operator use.
...
### Handoff Contract
- **Produces:** `Inclusion Goal Decomposer normalized artifacts`, `execution scorecard`, `risk posture`
