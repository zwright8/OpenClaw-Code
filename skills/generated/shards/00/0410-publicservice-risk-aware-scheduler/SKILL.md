---
name: u0410-publicservice-risk-aware-scheduler
description: Run the PublicService Risk-Aware Scheduler capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0410_publicservice-risk-aware-schedul` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate PublicService Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `healthcare-and-public-services:planning-router`.
- **Review posture (score >= 45 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Risk-Aware Scheduler`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.34, execution=0.14, safety=0.27, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 45
  - `review_risk`: risk >= 32
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
