---
name: u0283-federation-evidence-provenance-tracker
description: Run the Federation Evidence Provenance Tracker capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `federation-and-interoperability:provenance-tracker` |
| Feature flag | `skill_0283_federation-evidence-provenance-t` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Federation Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Federation Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `federation-and-interoperability:provenance-tracker`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Evidence Provenance Tracker`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.27, execution=0.31, safety=0.22, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 47
  - `review_risk`: risk >= 41
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for federation-and-interoperability:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `federation-and-interoperability:provenance-tracker` consumers with approval context attached.
