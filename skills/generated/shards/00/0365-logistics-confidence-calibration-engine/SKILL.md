...
description: Build and operate the "Logistics Confidence Calibration Engine" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Confidence Calibration Engine" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-confidence-calibration-engine
...
## Output Contract
- `logistics_confidence_calibration_engine_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_confidence_calibration_engine_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_confidence_calibration_engine_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Confidence Calibration Engine** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics confidence calibration engine normalization/assessment.
