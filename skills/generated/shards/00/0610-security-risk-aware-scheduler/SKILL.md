...
description: Run the Security Risk-Aware Scheduler capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0610_security-risk-aware-scheduler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Security Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `security-and-privacy:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Risk-Aware Scheduler`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.16, execution=0.40, safety=0.20, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 42
  - `review_risk`: risk >= 32
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
