...
## When To Use
Use this skill only when the request explicitly needs `Federation Training Curriculum Composer` in the target domain and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Training Curriculum Composer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in the target domain workflows**
   - Input: noisy upstream payload requiring federation training curriculum composer normalization/assessment.
