...
description: Run the Crisis Knowledge Graph Linker capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0659_crisis-knowledge-graph-linker` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Knowledge Graph Linker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Knowledge Graph Linker.
...
## Operator Use Cases
- Operate Crisis Knowledge Graph Linker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `crisis-and-incident-response:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Knowledge Graph Linker`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.23, execution=0.20, safety=0.30, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 56
  - `review_risk`: risk >= 58
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute entity and relation linking, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Knowledge Graph Linker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
