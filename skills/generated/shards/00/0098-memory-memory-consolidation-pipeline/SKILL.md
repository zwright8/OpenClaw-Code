...
description: Build and operate the "Memory Memory Consolidation Pipeline" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Memory Consolidation Pipeline" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-memory-consolidation-pipeline
...
## Output Contract
- `memory_memory_consolidation_pipeline_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_memory_consolidation_pipeline_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_memory_consolidation_pipeline_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Memory Consolidation Pipeline** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory memory consolidation pipeline normalization/assessment.
