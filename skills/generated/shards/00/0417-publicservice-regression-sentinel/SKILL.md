...
description: Run the PublicService Regression Sentinel capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0417_publicservice-regression-sentine` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Regression Sentinel.
...
## Operator Use Cases
- Operate PublicService Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `healthcare-and-public-services:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Regression Sentinel`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.22, execution=0.39, safety=0.23, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 57
  - `review_risk`: risk >= 50
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
