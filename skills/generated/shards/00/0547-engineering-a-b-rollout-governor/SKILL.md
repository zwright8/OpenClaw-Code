---
name: u0547-engineering-a-b-rollout-governor
description: Run the Engineering A/B Rollout Governor capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering A/B Rollout Governor
...
| Feature flag | `skill_0547_engineering-a-b-rollout-governor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill controls production risk during behavioral changes.

## Trigger Checklist
- [ ] The task explicitly needs Engineering A/B Rollout Governor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering A/B Rollout Governor.
...
## Operator Use Cases
- Operate Engineering A/B Rollout Governor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `software-engineering-automation:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering A/B Rollout Governor`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.30, execution=0.19, safety=0.18, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 62
  - `review_risk`: risk >= 52
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute staged rollout policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering A/B Rollout Governor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
