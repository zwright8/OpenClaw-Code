...
description: Build and operate the "Memory Incident Playbook Synthesizer" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Incident Playbook Synthesizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-incident-playbook-synthesizer
...
## Output Contract
- `memory_incident_playbook_synthesizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_incident_playbook_synthesizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_incident_playbook_synthesizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Incident Playbook Synthesizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory incident playbook synthesizer normalization/assessment.
