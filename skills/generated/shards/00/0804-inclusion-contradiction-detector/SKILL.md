...
description: Run the Inclusion Contradiction Detector capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0804_inclusion-contradiction-detector` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Inclusion Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Contradiction Detector.
...
## Operator Use Cases
- Operate Inclusion Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `accessibility-and-inclusion:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Contradiction Detector`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.16, execution=0.35, safety=0.14, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 55
  - `review_risk`: risk >= 61
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for accessibility-and-inclusion:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `accessibility-and-inclusion:detection-guard` consumers with approval context attached.
