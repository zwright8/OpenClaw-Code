---
name: u0404-publicservice-contradiction-detector
description: Run the PublicService Contradiction Detector capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Contradiction Detector
...
| Routing tag | `healthcare-and-public-services:detection-guard` |
| Feature flag | `skill_0404_publicservice-contradiction-dete` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill catches internal and external claim conflicts early.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Contradiction Detector.
...
## Operator Use Cases
- Operate PublicService Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `healthcare-and-public-services:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Contradiction Detector`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.21, execution=0.30, safety=0.28, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 57
  - `review_risk`: risk >= 54
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `contradiction alerts`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `contradiction alerts`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `contradiction alerts`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `contradiction alerts`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:detection-guard` consumers with approval context attached.
