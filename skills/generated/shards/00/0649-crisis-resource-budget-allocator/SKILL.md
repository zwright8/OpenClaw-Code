...
description: Run the Crisis Resource Budget Allocator capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0649_crisis-resource-budget-allocator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Resource Budget Allocator.
...
## Operator Use Cases
- Operate Crisis Resource Budget Allocator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Resource Budget Allocator`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.21, execution=0.23, safety=0.29, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 60
  - `review_risk`: risk >= 48
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
