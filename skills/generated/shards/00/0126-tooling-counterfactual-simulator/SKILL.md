---
name: u0126-tooling-counterfactual-simulator
description: Run the Tooling Counterfactual Simulator capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Counterfactual Simulator
...
| Feature flag | `skill_0126_tooling-counterfactual-simulator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Counterfactual Simulator.
...
## Operator Use Cases
- Operate Tooling Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Counterfactual Simulator`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.27, execution=0.22, safety=0.29, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 52
  - `review_risk`: risk >= 50
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
