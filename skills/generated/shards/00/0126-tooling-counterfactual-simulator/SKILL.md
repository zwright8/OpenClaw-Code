...
## When To Use
Use this skill when you need "Tooling Counterfactual Simulator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-counterfactual-simulator
...
## Output Contract
- `tooling_counterfactual_simulator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_counterfactual_simulator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_counterfactual_simulator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Counterfactual Simulator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling counterfactual simulator normalization/assessment.
