...
description: Run the Security Context Window Prioritizer capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0602_security-context-window-prioriti` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Context Window Prioritizer.
...
## Operator Use Cases
- Operate Security Context Window Prioritizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Context Window Prioritizer`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.23, execution=0.40, safety=0.16, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 52
  - `review_risk`: risk >= 58
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
