...
description: Build and operate the "Tooling Contradiction Detector" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Contradiction Detector" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-contradiction-detector
...
## Output Contract
- `tooling_contradiction_detector_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_contradiction_detector_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_contradiction_detector_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Contradiction Detector** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling contradiction detector normalization/assessment.
