---
name: u0363-logistics-evidence-provenance-tracker
description: Run the Logistics Evidence Provenance Tracker capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Evidence Provenance Tracker
...
| Routing tag | `resource-allocation-and-logistics:provenance-tracker` |
| Feature flag | `skill_0363_logistics-evidence-provenance-tr` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Logistics Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `resource-allocation-and-logistics:provenance-tracker`.
- **Review posture (score >= 43 or risk >= 37):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 73):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Evidence Provenance Tracker`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.29, execution=0.17, safety=0.35, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 43
  - `review_risk`: risk >= 37
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:provenance-tracker` consumers with approval context attached.
