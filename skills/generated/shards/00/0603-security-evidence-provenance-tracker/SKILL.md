...
description: Run the Security Evidence Provenance Tracker capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `security-and-privacy:provenance-tracker` |
| Feature flag | `skill_0603_security-evidence-provenance-tra` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Security Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `security-and-privacy:provenance-tracker`.
- **Review posture (score >= 42 or risk >= 41):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 63):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Evidence Provenance Tracker`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.19, execution=0.19, safety=0.35, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 42
  - `review_risk`: risk >= 41
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for security-and-privacy:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `security-and-privacy:provenance-tracker` consumers with approval context attached.
