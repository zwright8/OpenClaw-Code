---
name: u0443-education-evidence-provenance-tracker
description: Run the Education Evidence Provenance Tracker capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Evidence Provenance Tracker
...
| Routing tag | `education-and-upskilling:provenance-tracker` |
| Feature flag | `skill_0443_education-evidence-provenance-tr` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Education Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Education Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `education-and-upskilling:provenance-tracker`.
- **Review posture (score >= 45 or risk >= 35):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 71):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Evidence Provenance Tracker`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.42, execution=0.21, safety=0.18, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 45
  - `review_risk`: risk >= 35
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for education-and-upskilling:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `education-and-upskilling:provenance-tracker` consumers with approval context attached.
