---
name: u0742-comms-multi-agent-negotiation-mediator
description: Run the Comms Multi-Agent Negotiation Mediator capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Multi-Agent Negotiation Mediator
...
| Feature flag | `skill_0742_comms-multi-agent-negotiation-me` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill resolves resource and strategy conflicts with explicit tradeoffs.

## Trigger Checklist
- [ ] The task explicitly needs Comms Multi-Agent Negotiation Mediator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Multi-Agent Negotiation Mediator.
...
## Operator Use Cases
- Operate Comms Multi-Agent Negotiation Mediator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `communication-and-explainability:collaboration-mediator`.
- **Review posture (score >= 51 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Multi-Agent Negotiation Mediator`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.24, execution=0.14, safety=0.27, impact=0.36`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 51
  - `review_risk`: risk >= 40
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute structured bargaining protocols, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Multi-Agent Negotiation Mediator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
