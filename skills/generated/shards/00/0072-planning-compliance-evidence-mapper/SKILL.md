...
description: Run the Planning Compliance Evidence Mapper capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `strategic-planning-and-decomposition:normalization-engine` |
| Feature flag | `skill_0072_planning-compliance-evidence-map` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Planning Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Planning Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Compliance Evidence Mapper`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.17, execution=0.29, safety=0.17, impact=0.38`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 54
  - `review_risk`: risk >= 58
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for strategic-planning-and-decomposition:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `strategic-planning-and-decomposition:normalization-engine` consumers with approval context attached.
