---
name: u0718-impact-training-curriculum-composer
description: Run the Impact Training Curriculum Composer capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Training Curriculum Composer
...
| Feature flag | `skill_0718_impact-training-curriculum-compo` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill converts capability gaps into actionable upskilling programs.

## Trigger Checklist
- [ ] The task explicitly needs Impact Training Curriculum Composer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Training Curriculum Composer.
...
## Operator Use Cases
- Operate Impact Training Curriculum Composer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
- **Review posture (score >= 55 or risk >= 58):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 79):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Training Curriculum Composer`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.36, execution=0.23, safety=0.16, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 55
  - `review_risk`: risk >= 58
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute sequenced learning path design, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Training Curriculum Composer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
