---
name: u0527-engineering-goal-decomposer
description: Run the Engineering Goal Decomposer capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Goal Decomposer
...
| Feature flag | `skill_0527_engineering-goal-decomposer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill breaks ambiguous missions into executable units.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Goal Decomposer.
...
## Operator Use Cases
- Operate Engineering Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `software-engineering-automation:planning-router`.
- **Review posture (score >= 49 or risk >= 35):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Goal Decomposer`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.19, execution=0.29, safety=0.24, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 49
  - `review_risk`: risk >= 35
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for software-engineering-automation:planning-router, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `software-engineering-automation:planning-router` consumers with approval context attached.
