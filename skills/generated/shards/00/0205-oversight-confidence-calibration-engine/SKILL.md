---
name: u0205-oversight-confidence-calibration-engine
description: Run the Oversight Confidence Calibration Engine capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Oversight Confidence Calibration Engine
...
| Routing tag | `human-oversight-and-operator-ux:optimization-engine` |
| Feature flag | `skill_0205_oversight-confidence-calibration` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human teams need fast, legible control when stakes are high. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Oversight Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Oversight Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `human-oversight-and-operator-ux:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Confidence Calibration Engine`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.23, execution=0.26, safety=0.38, impact=0.13`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 45
  - `review_risk`: risk >= 35
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for human-oversight-and-operator-ux:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `human-oversight-and-operator-ux:optimization-engine` consumers with approval context attached.
