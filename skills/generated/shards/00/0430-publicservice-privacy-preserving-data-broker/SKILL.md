---
name: u0430-publicservice-privacy-preserving-data-broker
description: Run the PublicService Privacy Preserving Data Broker capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Privacy Preserving Data Broker
...
| Feature flag | `skill_0430_publicservice-privacy-preserving` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill enables collaboration while minimizing raw data exposure.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Privacy Preserving Data Broker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Privacy Preserving Data Broker.
...
## Operator Use Cases
- Operate PublicService Privacy Preserving Data Broker as a reliable, reusable production workflow.
...
- **Review posture (score >= 49 or risk >= 36):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 59):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Privacy Preserving Data Broker`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.29, execution=0.26, safety=0.22, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 49
  - `review_risk`: risk >= 36
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `privacy-scoped exchanges`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute policy-scoped data mediation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Privacy Preserving Data Broker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
