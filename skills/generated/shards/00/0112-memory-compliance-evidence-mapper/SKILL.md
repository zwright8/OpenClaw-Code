---
name: u0112-memory-compliance-evidence-mapper
description: Run the Memory Compliance Evidence Mapper capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory Compliance Evidence Mapper
...
| Routing tag | `memory-and-knowledge-operations:normalization-engine` |
| Feature flag | `skill_0112_memory-compliance-evidence-mappe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents lose performance when lessons are not retained and reused. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Memory Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Memory Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `memory-and-knowledge-operations:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Compliance Evidence Mapper`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.24, execution=0.27, safety=0.24, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 62
  - `review_risk`: risk >= 55
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for memory-and-knowledge-operations:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `memory-and-knowledge-operations:normalization-engine` consumers with approval context attached.
