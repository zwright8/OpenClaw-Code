...
description: Build and operate the "Memory Compliance Evidence Mapper" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Compliance Evidence Mapper" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-compliance-evidence-mapper
...
## Output Contract
- `memory_compliance_evidence_mapper_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_compliance_evidence_mapper_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_compliance_evidence_mapper_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Compliance Evidence Mapper** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory compliance evidence mapper normalization/assessment.
