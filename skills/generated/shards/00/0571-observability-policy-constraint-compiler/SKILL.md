...
description: Run the Observability Policy Constraint Compiler capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0571_observability-policy-constraint-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Observability Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Observability Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `data-quality-and-observability:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Policy Constraint Compiler`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.22, execution=0.30, safety=0.28, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 61
  - `review_risk`: risk >= 53
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
