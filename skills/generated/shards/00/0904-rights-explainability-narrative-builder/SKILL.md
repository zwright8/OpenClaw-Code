---
name: u0904-rights-explainability-narrative-builder
description: Run the Rights Explainability Narrative Builder capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Explainability Narrative Builder
...
| Routing tag | `legal-rights-and-compliance:communication-engine` |
| Feature flag | `skill_0904_rights-explainability-narrative-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Rights Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Rights Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `legal-rights-and-compliance:communication-engine`.
- **Review posture (score >= 49 or risk >= 33):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Explainability Narrative Builder`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.36, execution=0.26, safety=0.14, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 49
  - `review_risk`: risk >= 33
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `decision narratives`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `decision narratives`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `decision narratives`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `decision narratives`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for legal-rights-and-compliance:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `legal-rights-and-compliance:communication-engine` consumers with approval context attached.
