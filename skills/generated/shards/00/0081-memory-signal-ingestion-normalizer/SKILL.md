...
description: Build and operate the "Memory Signal Ingestion Normalizer" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Signal Ingestion Normalizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-signal-ingestion-normalizer
...
## Output Contract
- `memory_signal_ingestion_normalizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_signal_ingestion_normalizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_signal_ingestion_normalizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Signal Ingestion Normalizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory signal ingestion normalizer normalization/assessment.
