---
name: u0081-memory-signal-ingestion-normalizer
description: Run the Memory Signal Ingestion Normalizer capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory Signal Ingestion Normalizer
...
| Feature flag | `skill_0081_memory-signal-ingestion-normaliz` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents lose performance when lessons are not retained and reused. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Memory Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Memory Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `memory-and-knowledge-operations:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Signal Ingestion Normalizer`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.31, execution=0.25, safety=0.32, impact=0.12`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 44
  - `review_risk`: risk >= 31
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
