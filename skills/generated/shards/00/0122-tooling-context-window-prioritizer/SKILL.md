...
description: Build and operate the "Tooling Context Window Prioritizer" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Context Window Prioritizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-context-window-prioritizer
...
## Output Contract
- `tooling_context_window_prioritizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_context_window_prioritizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_context_window_prioritizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Context Window Prioritizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling context window prioritizer normalization/assessment.
