---
name: u0122-tooling-context-window-prioritizer
description: Run the Tooling Context Window Prioritizer capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Context Window Prioritizer
...
| Feature flag | `skill_0122_tooling-context-window-prioritiz` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Tooling Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Context Window Prioritizer.
...
## Operator Use Cases
- Operate Tooling Context Window Prioritizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Context Window Prioritizer`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.16, execution=0.32, safety=0.28, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 52
  - `review_risk`: risk >= 57
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
