...
description: Run the Economic Contradiction Detector capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0324_economic-contradiction-detector` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Economic Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Contradiction Detector.
...
## Operator Use Cases
- Operate Economic Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `economic-optimization:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Contradiction Detector`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.27, execution=0.17, safety=0.26, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 43
  - `review_risk`: risk >= 43
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:detection-guard` consumers with approval context attached.
