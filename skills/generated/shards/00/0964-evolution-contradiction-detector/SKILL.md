---
name: u0964-evolution-contradiction-detector
description: Run the Evolution Contradiction Detector capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Evolution Contradiction Detector
...
| Routing tag | `autonomous-learning-and-evolution:detection-guard` |
| Feature flag | `skill_0964_evolution-contradiction-detector` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill catches internal and external claim conflicts early.

## Trigger Checklist
- [ ] The task explicitly needs Evolution Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Contradiction Detector.
...
## Operator Use Cases
- Operate Evolution Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:detection-guard`.
- **Review posture (score >= 58 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 86):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Contradiction Detector`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.29, execution=0.16, safety=0.30, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 58
  - `review_risk`: risk >= 61
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for autonomous-learning-and-evolution:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `autonomous-learning-and-evolution:detection-guard` consumers with approval context attached.
