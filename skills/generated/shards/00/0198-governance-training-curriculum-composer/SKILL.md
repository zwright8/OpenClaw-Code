---
name: u0198-governance-training-curriculum-composer
description: Run the Governance Training Curriculum Composer capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0198_governance-training-curriculum-c` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Training Curriculum Composer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Training Curriculum Composer.
...
## Operator Use Cases
- Operate Governance Training Curriculum Composer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `safety-and-governance:general-capability`.
- **Review posture (score >= 49 or risk >= 43):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Training Curriculum Composer`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.13, execution=0.30, safety=0.27, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 49
  - `review_risk`: risk >= 43
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute sequenced learning path design, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Training Curriculum Composer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
