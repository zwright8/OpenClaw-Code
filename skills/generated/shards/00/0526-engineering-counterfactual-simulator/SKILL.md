---
name: u0526-engineering-counterfactual-simulator
description: Run the Engineering Counterfactual Simulator capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Counterfactual Simulator
...
| Routing tag | `software-engineering-automation:simulation-lab` |
| Feature flag | `skill_0526_engineering-counterfactual-simul` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Counterfactual Simulator.
...
## Operator Use Cases
- Operate Engineering Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `software-engineering-automation:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Counterfactual Simulator`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.22, execution=0.25, safety=0.23, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 57
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for software-engineering-automation:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `software-engineering-automation:simulation-lab` consumers with approval context attached.
