...
description: Build and operate the "Education Equity Impact Scorer" capability for Education and Upskilling. Trigger when this exact capability is needed in mission execution.
---
...
## When To Use
Use this skill when the request explicitly needs "Education Equity Impact Scorer" outcomes in the Education and Upskilling domain.
...
## Output Contract
- `equity_impact_profiles_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `equity_impact_profiles_scorecard` (scorecard, consumer=operator, guaranteed=true)
...
## Handoff Contract
- Produces: Education Equity Impact Scorer normalized artifacts; execution scorecard; risk posture
- Consumes: skill profiles; learning paths; support resources; claims; evidence; confidence traces
- Downstream routing hint: Route next to education-and-upskilling:general-capability consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Education Equity Impact Scorer** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **Education and Upskilling** with measurable acceptance criteria.
...
- **Daily:** Run when new education and upskilling signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in Education and Upskilling**
   - Input: noisy upstream payload requiring education equity impact scorer normalization/assessment.
