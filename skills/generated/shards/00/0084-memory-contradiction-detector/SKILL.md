...
description: Build and operate the "Memory Contradiction Detector" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Contradiction Detector" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-contradiction-detector
...
## Output Contract
- `memory_contradiction_detector_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_contradiction_detector_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_contradiction_detector_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Contradiction Detector** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory contradiction detector normalization/assessment.
