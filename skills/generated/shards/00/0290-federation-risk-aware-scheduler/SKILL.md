...
## When To Use
Use this skill only when the request explicitly needs `Federation Risk-Aware Scheduler` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Risk-Aware Scheduler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation risk-aware scheduler normalization/assessment.
