...
description: Build and operate the "Memory Failure Root-Cause Miner" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Failure Root-Cause Miner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-failure-root-cause-miner
...
## Output Contract
- `memory_failure_root_cause_miner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_failure_root_cause_miner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_failure_root_cause_miner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Failure Root-Cause Miner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory failure root-cause miner normalization/assessment.
