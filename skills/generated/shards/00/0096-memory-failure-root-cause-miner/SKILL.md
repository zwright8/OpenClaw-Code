...
description: Run the Memory Failure Root-Cause Miner capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0096_memory-failure-root-cause-miner` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Memory Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `memory-and-knowledge-operations:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Failure Root-Cause Miner`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.19, execution=0.32, safety=0.25, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 61
  - `review_risk`: risk >= 56
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
