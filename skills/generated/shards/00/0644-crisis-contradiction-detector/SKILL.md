---
name: u0644-crisis-contradiction-detector
description: Run the Crisis Contradiction Detector capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Contradiction Detector
...
| Routing tag | `crisis-and-incident-response:detection-guard` |
| Feature flag | `skill_0644_crisis-contradiction-detector` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill catches internal and external claim conflicts early.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Contradiction Detector.
...
## Operator Use Cases
- Operate Crisis Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `crisis-and-incident-response:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Contradiction Detector`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.21, execution=0.14, safety=0.32, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 49
  - `review_risk`: risk >= 42
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for crisis-and-incident-response:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `crisis-and-incident-response:detection-guard` consumers with approval context attached.
