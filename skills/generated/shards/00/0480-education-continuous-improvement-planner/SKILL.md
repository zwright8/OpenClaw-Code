---
name: u0480-education-continuous-improvement-planner
description: Run the Education Continuous Improvement Planner capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Continuous Improvement Planner
...
| Feature flag | `skill_0480_education-continuous-improvement` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill turns findings into sustained upgrade cycles.

## Trigger Checklist
- [ ] The task explicitly needs Education Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Education Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `education-and-upskilling:planning-router`.
- **Review posture (score >= 49 or risk >= 31):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Continuous Improvement Planner`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.14, execution=0.31, safety=0.33, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 49
  - `review_risk`: risk >= 31
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `improvement roadmaps`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `improvement roadmaps`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `improvement roadmaps`.
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
