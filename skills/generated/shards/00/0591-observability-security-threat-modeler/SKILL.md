...
description: Run the Observability Security Threat Modeler capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0591_observability-security-threat-mo` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Security Threat Modeler.
...
## Operator Use Cases
- Operate Observability Security Threat Modeler as a reliable, reusable production workflow.
...
- **Review posture (score >= 50 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 63):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Security Threat Modeler`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.36, execution=0.17, safety=0.12, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 50
  - `review_risk`: risk >= 34
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
