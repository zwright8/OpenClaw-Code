...
description: Run the Crisis Security Threat Modeler capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0671_crisis-security-threat-modeler` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill anticipates attack paths before adversaries exploit them.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Security Threat Modeler.
...
## Operator Use Cases
- Operate Crisis Security Threat Modeler as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Security Threat Modeler`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.27, execution=0.22, safety=0.20, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 61
  - `review_risk`: risk >= 56
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `threat models`.
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
