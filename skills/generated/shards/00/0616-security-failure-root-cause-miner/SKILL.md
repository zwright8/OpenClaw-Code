...
description: Run the Security Failure Root-Cause Miner capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0616_security-failure-root-cause-mine` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Security Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `security-and-privacy:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Failure Root-Cause Miner`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
