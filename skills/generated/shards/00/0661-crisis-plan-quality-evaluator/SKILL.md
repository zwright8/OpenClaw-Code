...
description: Run the Crisis Plan Quality Evaluator capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Crisis Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Plan Quality Evaluator`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.34, execution=0.17, safety=0.14, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 55
  - `review_risk`: risk >= 61
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
