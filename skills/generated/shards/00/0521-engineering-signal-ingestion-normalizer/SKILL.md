...
description: Run the Engineering Signal Ingestion Normalizer capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0521_engineering-signal-ingestion-nor` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Engineering Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `software-engineering-automation:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Signal Ingestion Normalizer`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.21, execution=0.38, safety=0.18, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 52
  - `review_risk`: risk >= 36
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
