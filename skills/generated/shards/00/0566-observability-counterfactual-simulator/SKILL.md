...
description: Run the Observability Counterfactual Simulator capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `data-quality-and-observability:simulation-lab` |
| Feature flag | `skill_0566_observability-counterfactual-sim` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Counterfactual Simulator.
...
## Operator Use Cases
- Operate Observability Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `data-quality-and-observability:simulation-lab`.
- **Review posture (score >= 50 or risk >= 39):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 71):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Counterfactual Simulator`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.29, execution=0.32, safety=0.12, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 50
  - `review_risk`: risk >= 39
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for data-quality-and-observability:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `data-quality-and-observability:simulation-lab` consumers with approval context attached.
