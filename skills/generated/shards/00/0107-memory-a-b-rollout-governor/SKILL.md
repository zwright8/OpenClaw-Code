...
description: Build and operate the "Memory A/B Rollout Governor" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory A/B Rollout Governor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-a-b-rollout-governor
...
## Output Contract
- `memory_a_b_rollout_governor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_a_b_rollout_governor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_a_b_rollout_governor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory A/B Rollout Governor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory a/b rollout governor normalization/assessment.
