...
description: Run the Governance Policy Constraint Compiler capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Governance Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `safety-and-governance:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Policy Constraint Compiler`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.34, execution=0.16, safety=0.20, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 60
  - `review_risk`: risk >= 51
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
