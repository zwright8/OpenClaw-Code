---
name: u0049-planning-resource-budget-allocator
description: Run the Planning Resource Budget Allocator capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Planning Resource Budget Allocator
...
| Feature flag | `skill_0049_planning-resource-budget-allocat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because large goals fail when decomposition is inconsistent or incomplete. This specific skill matches mission ambition to finite execution capacity.

## Trigger Checklist
- [ ] The task explicitly needs Planning Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Resource Budget Allocator.
...
## Operator Use Cases
- Operate Planning Resource Budget Allocator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:planning-router`.
- **Review posture (score >= 48 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 61):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Resource Budget Allocator`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.21, execution=0.25, safety=0.31, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 48
  - `review_risk`: risk >= 32
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
