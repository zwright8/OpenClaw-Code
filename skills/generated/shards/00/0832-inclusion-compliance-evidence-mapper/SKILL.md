...
description: Run the Inclusion Compliance Evidence Mapper capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0832_inclusion-compliance-evidence-ma` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Inclusion Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Inclusion Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `accessibility-and-inclusion:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Compliance Evidence Mapper`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.29, execution=0.16, safety=0.35, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 51
  - `review_risk`: risk >= 31
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for accessibility-and-inclusion:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `accessibility-and-inclusion:normalization-engine` consumers with approval context attached.
