...
description: Operate the "Evolution Uncertainty Communicator" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `985`
- **Skill Name:** `u0985-evolution-uncertainty-communicator`
...
|---|---|---|---|
| uncertainty_briefs_report | structured-report | true | orchestrator |
| uncertainty_briefs_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `confidence-bound communication templates` with deterministic settings and trace capture enabled.
3. Produce `uncertainty briefs` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `autonomous-learning-and-evolution:communication-engine` and include approval context.
...
### Handoff Contract
- **Produces:** `Evolution Uncertainty Communicator normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `outcomes`, `error taxonomies`, `adaptation decisions`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to autonomous-learning-and-evolution:communication-engine consumers with approval-gate context
