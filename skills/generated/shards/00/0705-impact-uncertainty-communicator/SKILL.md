...
description: Run the Impact Uncertainty Communicator capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0705_impact-uncertainty-communicator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Uncertainty Communicator.
...
## Operator Use Cases
- Operate Impact Uncertainty Communicator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Uncertainty Communicator`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.26, execution=0.31, safety=0.26, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 46
  - `review_risk`: risk >= 41
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
