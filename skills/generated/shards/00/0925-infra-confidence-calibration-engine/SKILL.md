...
description: Run the Infra Confidence Calibration Engine capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Infra Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Infra Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `infrastructure-and-sustainability:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Confidence Calibration Engine`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.15, execution=0.35, safety=0.35, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 48
  - `review_risk`: risk >= 43
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for infrastructure-and-sustainability:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `infrastructure-and-sustainability:optimization-engine` consumers with approval context attached.
