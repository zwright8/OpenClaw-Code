...
## When To Use
Use this skill when you need "Logistics Plan Quality Evaluator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-plan-quality-evaluator
...
## Output Contract
- `logistics_plan_quality_evaluator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_plan_quality_evaluator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_plan_quality_evaluator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Plan Quality Evaluator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics plan quality evaluator normalization/assessment.
