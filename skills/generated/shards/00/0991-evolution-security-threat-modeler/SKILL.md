...
description: Operate the "Evolution Security Threat Modeler" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `991`
- **Skill Name:** `u0991-evolution-security-threat-modeler`
...
|---|---|---|---|
| threat_models_report | structured-report | true | orchestrator |
| threat_models_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `attack-surface modeling` with deterministic settings and trace capture enabled.
3. Produce `threat models` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `autonomous-learning-and-evolution:detection-guard` and include approval context.
...
### Handoff Contract
- **Produces:** `Evolution Security Threat Modeler normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `outcomes`, `error taxonomies`, `adaptation decisions`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to autonomous-learning-and-evolution:detection-guard consumers with approval-gate context
