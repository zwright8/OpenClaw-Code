---
name: u0493-research-task-handoff-contractor
description: Run the Research Task Handoff Contractor capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Task Handoff Contractor
...
| Routing tag | `scientific-research-systems:contract-compiler` |
| Feature flag | `skill_0493_research-task-handoff-contractor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill standardizes handoffs between agents and humans.

## Trigger Checklist
- [ ] The task explicitly needs Research Task Handoff Contractor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Task Handoff Contractor.
...
## Operator Use Cases
- Operate Research Task Handoff Contractor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `scientific-research-systems:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Task Handoff Contractor`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.26, execution=0.14, safety=0.33, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 53
  - `review_risk`: risk >= 54
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute contracted payload schemas, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Task Handoff Contractor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for scientific-research-systems:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `scientific-research-systems:contract-compiler` consumers with approval context attached.
