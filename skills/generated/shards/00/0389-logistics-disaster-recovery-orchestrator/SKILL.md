---
name: u0389-logistics-disaster-recovery-orchestrator
description: Run the Logistics Disaster Recovery Orchestrator capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Disaster Recovery Orchestrator
...
| Feature flag | `skill_0389_logistics-disaster-recovery-orch` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill improves recovery speed after severe outages.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate Logistics Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.28, execution=0.22, safety=0.29, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 61
  - `review_risk`: risk >= 58
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
