...
## When To Use
Use this skill when you need "Memory Goal Decomposer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-goal-decomposer
...
## Output Contract
- `memory_goal_decomposer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_goal_decomposer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_goal_decomposer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Goal Decomposer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory goal decomposer normalization/assessment.
