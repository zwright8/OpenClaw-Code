---
name: u0103-memory-conflict-resolution-coach
description: Run the Memory Conflict Resolution Coach capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory Conflict Resolution Coach
...
| Feature flag | `skill_0103_memory-conflict-resolution-coach` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents lose performance when lessons are not retained and reused. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Memory Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Memory Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `memory-and-knowledge-operations:collaboration-mediator`.
- **Review posture (score >= 51 or risk >= 41):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Conflict Resolution Coach`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.26, execution=0.12, safety=0.35, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 51
  - `review_risk`: risk >= 41
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
