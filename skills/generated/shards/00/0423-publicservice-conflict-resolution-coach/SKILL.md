...
description: Run the PublicService Conflict Resolution Coach capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0423_publicservice-conflict-resolutio` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Conflict Resolution Coach.
...
## Operator Use Cases
- Operate PublicService Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `healthcare-and-public-services:collaboration-mediator`.
- **Review posture (score >= 58 or risk >= 58):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 88):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Conflict Resolution Coach`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.12, execution=0.27, safety=0.29, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 58
  - `review_risk`: risk >= 58
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:collaboration-mediator` consumers with approval context attached.
