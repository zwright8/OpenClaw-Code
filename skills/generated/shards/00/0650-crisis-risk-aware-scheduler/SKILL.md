---
name: u0650-crisis-risk-aware-scheduler
description: Run the Crisis Risk-Aware Scheduler capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Risk-Aware Scheduler
...
| Feature flag | `skill_0650_crisis-risk-aware-scheduler` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill times execution based on risk posture instead of urgency alone.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Crisis Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `crisis-and-incident-response:planning-router`.
- **Review posture (score >= 46 or risk >= 44):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 64):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Risk-Aware Scheduler`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.25, execution=0.20, safety=0.32, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 46
  - `review_risk`: risk >= 44
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
