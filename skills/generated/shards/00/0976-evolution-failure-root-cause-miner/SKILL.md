...
description: Run the Evolution Failure Root-Cause Miner capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0976_evolution-failure-root-cause-min` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Evolution Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Evolution Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Failure Root-Cause Miner`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.36, execution=0.13, safety=0.14, impact=0.37`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 62
  - `review_risk`: risk >= 52
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
