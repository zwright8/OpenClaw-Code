...
description: Run the Education Security Threat Modeler capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0471_education-security-threat-modele` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Education Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Security Threat Modeler.
...
## Operator Use Cases
- Operate Education Security Threat Modeler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `education-and-upskilling:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Security Threat Modeler`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.26, execution=0.26, safety=0.22, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 53
  - `review_risk`: risk >= 56
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
