---
name: u0061-planning-plan-quality-evaluator
description: Run the Planning Plan Quality Evaluator capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Planning Plan Quality Evaluator
...
| Feature flag | `skill_0061_planning-plan-quality-evaluator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because large goals fail when decomposition is inconsistent or incomplete. This specific skill quantifies whether plans are complete, safe, and feasible.

## Trigger Checklist
- [ ] The task explicitly needs Planning Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Planning Plan Quality Evaluator as a reliable, reusable production workflow.
...
- **Review posture (score >= 54 or risk >= 47):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 80):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Plan Quality Evaluator`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.35, execution=0.20, safety=0.20, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 54
  - `review_risk`: risk >= 47
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
