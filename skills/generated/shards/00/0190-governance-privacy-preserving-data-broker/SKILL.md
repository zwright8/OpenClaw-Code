...
description: Run the Governance Privacy Preserving Data Broker capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0190_governance-privacy-preserving-da` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Privacy Preserving Data Broker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Privacy Preserving Data Broker.
...
## Operator Use Cases
- Operate Governance Privacy Preserving Data Broker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `safety-and-governance:collaboration-mediator`.
- **Review posture (score >= 52 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 65):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Privacy Preserving Data Broker`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.18, execution=0.38, safety=0.17, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 52
  - `review_risk`: risk >= 32
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute policy-scoped data mediation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Privacy Preserving Data Broker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
