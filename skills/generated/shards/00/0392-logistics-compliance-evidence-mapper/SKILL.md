---
name: u0392-logistics-compliance-evidence-mapper
description: Run the Logistics Compliance Evidence Mapper capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Compliance Evidence Mapper
...
| Routing tag | `resource-allocation-and-logistics:normalization-engine` |
| Feature flag | `skill_0392_logistics-compliance-evidence-ma` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Logistics Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `resource-allocation-and-logistics:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Compliance Evidence Mapper`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.19, execution=0.33, safety=0.24, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 52
  - `review_risk`: risk >= 53
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:normalization-engine` consumers with approval context attached.
