---
name: u0313-federation-cost-benefit-forecaster
description: Run the Federation Cost-Benefit Forecaster capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Cost-Benefit Forecaster
...
| Feature flag | `skill_0313_federation-cost-benefit-forecast` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs Federation Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Federation Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `federation-and-interoperability:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Cost-Benefit Forecaster`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.16, execution=0.28, safety=0.25, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 60
  - `review_risk`: risk >= 57
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
