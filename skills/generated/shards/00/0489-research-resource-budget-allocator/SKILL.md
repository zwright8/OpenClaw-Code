...
description: Run the Research Resource Budget Allocator capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0489_research-resource-budget-allocat` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Research Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Resource Budget Allocator.
...
## Operator Use Cases
- Operate Research Resource Budget Allocator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Resource Budget Allocator`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.15, execution=0.18, safety=0.34, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 45
  - `review_risk`: risk >= 45
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
