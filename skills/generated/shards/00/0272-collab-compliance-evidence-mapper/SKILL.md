---
name: u0272-collab-compliance-evidence-mapper
description: Run the Collab Compliance Evidence Mapper capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Compliance Evidence Mapper
...
| Routing tag | `collaboration-and-negotiation:normalization-engine` |
| Feature flag | `skill_0272_collab-compliance-evidence-mappe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Collab Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Collab Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `collaboration-and-negotiation:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Compliance Evidence Mapper`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.35, execution=0.22, safety=0.22, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 55
  - `review_risk`: risk >= 52
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for collaboration-and-negotiation:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `collaboration-and-negotiation:normalization-engine` consumers with approval context attached.
