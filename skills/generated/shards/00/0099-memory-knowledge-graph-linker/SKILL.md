...
description: Build and operate the "Memory Knowledge Graph Linker" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Knowledge Graph Linker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-knowledge-graph-linker
...
## Output Contract
- `memory_knowledge_graph_linker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_knowledge_graph_linker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_knowledge_graph_linker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Knowledge Graph Linker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory knowledge graph linker normalization/assessment.
