---
name: u0905-rights-uncertainty-communicator
description: Run the Rights Uncertainty Communicator capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Uncertainty Communicator
...
| Feature flag | `skill_0905_rights-uncertainty-communicator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Rights Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Uncertainty Communicator.
...
## Operator Use Cases
- Operate Rights Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `legal-rights-and-compliance:communication-engine`.
- **Review posture (score >= 45 or risk >= 42):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 62):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Uncertainty Communicator`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.25, execution=0.30, safety=0.19, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 45
  - `review_risk`: risk >= 42
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
