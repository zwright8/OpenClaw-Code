...
description: Run the Comms Knowledge Graph Linker capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0739_comms-knowledge-graph-linker` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Comms Knowledge Graph Linker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Knowledge Graph Linker.
...
## Operator Use Cases
- Operate Comms Knowledge Graph Linker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `communication-and-explainability:general-capability`.
- **Review posture (score >= 56 or risk >= 52):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 81):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Knowledge Graph Linker`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.13, execution=0.37, safety=0.35, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 56
  - `review_risk`: risk >= 52
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute entity and relation linking, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Knowledge Graph Linker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
