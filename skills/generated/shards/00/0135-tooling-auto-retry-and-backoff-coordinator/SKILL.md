...
description: Build and operate the "Tooling Auto-Retry and Backoff Coordinator" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Auto-Retry and Backoff Coordinator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-auto-retry-and-backoff-coordinator
...
## Output Contract
- `tooling_auto_retry_and_backoff_coordinator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_auto_retry_and_backoff_coordinator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_auto_retry_and_backoff_coordinator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Auto-Retry and Backoff Coordinator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling auto-retry and backoff coordinator normalization/assessment.
