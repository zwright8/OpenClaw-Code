...
description: Build and operate the "Logistics Memory Consolidation Pipeline" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Memory Consolidation Pipeline" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-memory-consolidation-pipeline
...
## Output Contract
- `logistics_memory_consolidation_pipeline_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_memory_consolidation_pipeline_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_memory_consolidation_pipeline_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Memory Consolidation Pipeline** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics memory consolidation pipeline normalization/assessment.
