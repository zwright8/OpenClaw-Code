---
name: u0704-impact-explainability-narrative-builder
description: Run the Impact Explainability Narrative Builder capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Explainability Narrative Builder
...
| Feature flag | `skill_0704_impact-explainability-narrative-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Impact Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Impact Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `social-impact-measurement:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Explainability Narrative Builder`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.27, execution=0.29, safety=0.14, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 59
  - `review_risk`: risk >= 54
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:communication-engine` consumers with approval context attached.
