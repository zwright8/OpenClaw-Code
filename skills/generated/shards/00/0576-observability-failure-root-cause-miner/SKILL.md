---
name: u0576-observability-failure-root-cause-miner
description: Run the Observability Failure Root-Cause Miner capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Failure Root-Cause Miner
...
| Feature flag | `skill_0576_observability-failure-root-cause` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Observability Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Observability Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `data-quality-and-observability:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Failure Root-Cause Miner`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.28, execution=0.28, safety=0.27, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 59
  - `review_risk`: risk >= 53
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
