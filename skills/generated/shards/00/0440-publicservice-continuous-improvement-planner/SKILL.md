...
description: Run the PublicService Continuous Improvement Planner capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0440_publicservice-continuous-improve` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Continuous Improvement Planner.
...
## Operator Use Cases
- Operate PublicService Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `healthcare-and-public-services:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Continuous Improvement Planner`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.37, execution=0.24, safety=0.13, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 53
  - `review_risk`: risk >= 54
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
