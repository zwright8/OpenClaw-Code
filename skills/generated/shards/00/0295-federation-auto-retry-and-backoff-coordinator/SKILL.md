...
## When To Use
Use this skill only when the request explicitly needs `Federation Auto-Retry and Backoff Coordinator` in Federation and Interoperability and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation Auto-Retry and Backoff Coordinator** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **Federation and Interoperability workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new federation and interoperability workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in Federation and Interoperability workflows**
   - Input: noisy upstream payload requiring federation auto-retry and backoff coordinator normalization/assessment.
