...
description: Build and operate the "Logistics Signal Ingestion Normalizer" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Signal Ingestion Normalizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
3. Implement the core capability using schema-first normalization, with deterministic scoring and reproducible artifact generation.
...
- Core method: schema-first normalization
- Execution mode: deterministic-first with explicit tolerances
- Routing tag: resource-allocation-and-logistics:logistics-signal-ingestion-normalizer
...
## Output Contract
- `logistics_signal_ingestion_normalizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_signal_ingestion_normalizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_signal_ingestion_normalizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Signal Ingestion Normalizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics signal ingestion normalizer normalization/assessment.
