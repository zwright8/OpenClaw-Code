---
name: u0361-logistics-signal-ingestion-normalizer
description: Run the Logistics Signal Ingestion Normalizer capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Signal Ingestion Normalizer
...
| Feature flag | `skill_0361_logistics-signal-ingestion-norma` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill stabilizes noisy upstream inputs before they contaminate planning.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Logistics Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Signal Ingestion Normalizer`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.36, execution=0.17, safety=0.31, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 53
  - `review_risk`: risk >= 50
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
