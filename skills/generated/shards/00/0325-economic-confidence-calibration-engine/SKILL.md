---
name: u0325-economic-confidence-calibration-engine
description: Run the Economic Confidence Calibration Engine capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Confidence Calibration Engine
...
| Routing tag | `economic-optimization:optimization-engine` |
| Feature flag | `skill_0325_economic-confidence-calibration-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Economic Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Economic Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `economic-optimization:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Confidence Calibration Engine`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.31, execution=0.28, safety=0.25, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 60
  - `review_risk`: risk >= 48
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:optimization-engine` consumers with approval context attached.
