...
description: Run the Impact Security Threat Modeler capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0711_impact-security-threat-modeler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Security Threat Modeler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Security Threat Modeler.
...
## Operator Use Cases
- Operate Impact Security Threat Modeler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `social-impact-measurement:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Security Threat Modeler`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.26, execution=0.33, safety=0.21, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 60
  - `review_risk`: risk >= 53
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute attack-surface modeling, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Security Threat Modeler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
