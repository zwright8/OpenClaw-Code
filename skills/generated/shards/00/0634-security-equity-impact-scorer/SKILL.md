---
name: u0634-security-equity-impact-scorer
description: Run the Security Equity Impact Scorer capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Equity Impact Scorer
...
| Feature flag | `skill_0634_security-equity-impact-scorer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill exposes uneven benefit/harm distribution before rollout.

## Trigger Checklist
- [ ] The task explicitly needs Security Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Equity Impact Scorer.
...
## Operator Use Cases
- Operate Security Equity Impact Scorer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Equity Impact Scorer`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.15, execution=0.29, safety=0.26, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 45
  - `review_risk`: risk >= 44
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
