---
name: u0562-observability-context-window-prioritizer
description: Run the Observability Context Window Prioritizer capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Context Window Prioritizer
...
| Feature flag | `skill_0562_observability-context-window-pri` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill surfaces the most decision-relevant context under tight token budgets.

## Trigger Checklist
- [ ] The task explicitly needs Observability Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Context Window Prioritizer.
...
## Operator Use Cases
- Operate Observability Context Window Prioritizer as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Context Window Prioritizer`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.14, execution=0.16, safety=0.36, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 42
  - `review_risk`: risk >= 39
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
