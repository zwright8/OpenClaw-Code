---
name: u0785-cultural-uncertainty-communicator
description: Run the Cultural Uncertainty Communicator capability for Multilingual and Cultural Adaptation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Cultural Uncertainty Communicator
...
| Feature flag | `skill_0785_cultural-uncertainty-communicato` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because global utility requires language and cultural context fidelity. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Cultural Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Cultural Uncertainty Communicator.
...
## Operator Use Cases
- Operate Cultural Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `multilingual-and-cultural-adaptation:communication-engine`.
- **Review posture (score >= 50 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Cultural Uncertainty Communicator`, including at least three measurable KPIs tied to context mismatch and exclusion.
...
- Scoring weights: `truth=0.28, execution=0.33, safety=0.14, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 50
  - `review_risk`: risk >= 40
...
1. Incident recovery in Multilingual and Cultural Adaptation: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Cultural Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
