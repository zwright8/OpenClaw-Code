---
name: u0514-research-equity-impact-scorer
description: Run the Research Equity Impact Scorer capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Equity Impact Scorer
...
| Feature flag | `skill_0514_research-equity-impact-scorer` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill exposes uneven benefit/harm distribution before rollout.

## Trigger Checklist
- [ ] The task explicitly needs Research Equity Impact Scorer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Equity Impact Scorer.
...
## Operator Use Cases
- Operate Research Equity Impact Scorer as a reliable, reusable production workflow.
...
- **Review posture (score >= 42 or risk >= 33):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 67):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Equity Impact Scorer`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.26, execution=0.24, safety=0.32, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 42
  - `review_risk`: risk >= 33
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute group-level impact scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Equity Impact Scorer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
