---
name: u0168-governance-dependency-dag-planner
description: Run the Governance Dependency DAG Planner capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0168_governance-dependency-dag-planne` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Dependency DAG Planner.
...
## Operator Use Cases
- Operate Governance Dependency DAG Planner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Dependency DAG Planner`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.28, execution=0.17, safety=0.27, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 50
  - `review_risk`: risk >= 35
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
