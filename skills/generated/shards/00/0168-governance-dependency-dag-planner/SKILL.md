...
description: Build and operate the "Governance Dependency DAG Planner" capability for Safety and Governance. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Governance Dependency DAG Planner** in the **Safety and Governance** capability family.
...
## Handoff Contract
- Produces: Governance Dependency DAG Planner normalized artifacts; execution scorecard; risk posture
...
## Trigger Checklist
- [ ] The request explicitly needs **Governance Dependency DAG Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Safety and Governance**
   - Input: noisy upstream payload requiring governance dependency dag planner normalization/assessment.
...
## When To Use
Use this when a request in **Safety and Governance** depends on **Governance Dependency DAG Planner** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
