---
name: u0898-rights-memory-consolidation-pipeline
description: Run the Rights Memory Consolidation Pipeline capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Memory Consolidation Pipeline
...
| Feature flag | `skill_0898_rights-memory-consolidation-pipe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs Rights Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Rights Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `legal-rights-and-compliance:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Memory Consolidation Pipeline`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.41, execution=0.16, safety=0.22, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 53
  - `review_risk`: risk >= 50
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `consolidated memory snapshots`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `consolidated memory snapshots`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `consolidated memory snapshots`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `consolidated memory snapshots`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
