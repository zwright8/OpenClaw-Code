...
description: Operate the "Observability Task Handoff Contractor" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill when the request explicitly needs "Observability Task Handoff Contractor" outcomes in the Data Quality and Observability domain.
...
## Handoff Contract
- Produces: Observability Task Handoff Contractor normalized artifacts; execution scorecard; risk posture
- Consumes: freshness; drift; schema health; telemetry coverage; claims; evidence; confidence traces
- Downstream routing hint: Route next to data-quality-and-observability:contract-compiler consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Observability Task Handoff Contractor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in workflows**
   - Input: noisy upstream payload requiring observability task handoff contractor normalization/assessment.
