---
name: u0573-observability-task-handoff-contractor
description: Run the Observability Task Handoff Contractor capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Task Handoff Contractor
...
| Routing tag | `data-quality-and-observability:contract-compiler` |
| Feature flag | `skill_0573_observability-task-handoff-contr` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill standardizes handoffs between agents and humans.

## Trigger Checklist
- [ ] The task explicitly needs Observability Task Handoff Contractor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Task Handoff Contractor.
...
## Operator Use Cases
- Operate Observability Task Handoff Contractor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `data-quality-and-observability:contract-compiler`.
- **Review posture (score >= 49 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Task Handoff Contractor`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.28, execution=0.26, safety=0.33, impact=0.13`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 49
  - `review_risk`: risk >= 40
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute contracted payload schemas, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Task Handoff Contractor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for data-quality-and-observability:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `data-quality-and-observability:contract-compiler` consumers with approval context attached.
