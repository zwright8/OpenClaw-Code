...
description: Build and operate the "Logistics Human Approval Router" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Human Approval Router" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-human-approval-router
...
## Output Contract
- `logistics_human_approval_router_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_human_approval_router_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_human_approval_router_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Human Approval Router** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics human approval router normalization/assessment.
