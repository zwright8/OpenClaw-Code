---
name: u0372-logistics-human-approval-router
description: Run the Logistics Human Approval Router capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Human Approval Router
...
| Feature flag | `skill_0372_logistics-human-approval-router` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill directs high-risk decisions to the right humans quickly.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Human Approval Router.
...
## Operator Use Cases
- Operate Logistics Human Approval Router as a reliable, reusable production workflow.
...
- **Review posture (score >= 48 or risk >= 36):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 59):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Human Approval Router`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.32, execution=0.15, safety=0.33, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 48
  - `review_risk`: risk >= 36
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
