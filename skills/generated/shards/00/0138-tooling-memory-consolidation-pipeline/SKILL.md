...
description: Build and operate the "Tooling Memory Consolidation Pipeline" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Memory Consolidation Pipeline" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-memory-consolidation-pipeline
...
## Output Contract
- `tooling_memory_consolidation_pipeline_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_memory_consolidation_pipeline_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_memory_consolidation_pipeline_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Memory Consolidation Pipeline** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling memory consolidation pipeline normalization/assessment.
