---
name: u0584-observability-explainability-narrative-builder
description: Run the Observability Explainability Narrative Builder capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Explainability Narrative Builder
...
| Feature flag | `skill_0584_observability-explainability-nar` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Observability Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Observability Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `data-quality-and-observability:communication-engine`.
- **Review posture (score >= 62 or risk >= 48):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Explainability Narrative Builder`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.34, execution=0.14, safety=0.23, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 62
  - `review_risk`: risk >= 48
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
