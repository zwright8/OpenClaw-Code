---
name: u0192-governance-compliance-evidence-mapper
description: Run the Governance Compliance Evidence Mapper capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Governance Compliance Evidence Mapper
...
| Feature flag | `skill_0192_governance-compliance-evidence-m` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because high-speed autonomy needs enforceable guardrails to stay aligned. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Governance Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Governance Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `safety-and-governance:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Compliance Evidence Mapper`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.15, execution=0.41, safety=0.26, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 43
  - `review_risk`: risk >= 44
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `compliance evidence matrices`.
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for safety-and-governance:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `safety-and-governance:normalization-engine` consumers with approval context attached.
