...
description: Run the Research Dependency DAG Planner capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Research Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Dependency DAG Planner.
...
## Operator Use Cases
- Operate Research Dependency DAG Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `scientific-research-systems:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Dependency DAG Planner`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.28, execution=0.30, safety=0.27, impact=0.14`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 43
  - `review_risk`: risk >= 42
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
