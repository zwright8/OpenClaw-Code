---
name: u0029-epistemic-disaster-recovery-orchestrator
description: Run the Epistemic Disaster Recovery Orchestrator capability for Truth-Seeking and Epistemics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Epistemic Disaster Recovery Orchestrator
...
| Feature flag | `skill_0029_epistemic-disaster-recovery-orch` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions drift when claims are accepted without verification. This specific skill improves recovery speed after severe outages.

## Trigger Checklist
- [ ] The task explicitly needs Epistemic Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Epistemic Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate Epistemic Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `truth-seeking-and-epistemics:general-capability`.
- **Review posture (score >= 58 or risk >= 48):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 89):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Epistemic Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to false certainty and unverified assumptions.
...
- Scoring weights: `truth=0.27, execution=0.40, safety=0.16, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 58
  - `review_risk`: risk >= 48
...
1. Incident recovery in Truth-Seeking and Epistemics: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Epistemic Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
