...
description: Run the Planning Failure Root-Cause Miner capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0056_planning-failure-root-cause-mine` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Planning Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Planning Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Failure Root-Cause Miner`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.19, execution=0.24, safety=0.20, impact=0.37`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 57
  - `review_risk`: risk >= 50
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
