---
name: u0623-security-conflict-resolution-coach
description: Run the Security Conflict Resolution Coach capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Conflict Resolution Coach
...
| Routing tag | `security-and-privacy:collaboration-mediator` |
| Feature flag | `skill_0623_security-conflict-resolution-coa` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Security Conflict Resolution Coach (not generic brainstorming).
...
|---|---|---|---|
| conflict remediation playbooks | structured-artifact | yes | downstream orchestrator |
| conflict remediation playbooks-scorecard | scorecard | yes | operator / reviewer |
| conflict remediation playbooks-handoff | handoff-packet | yes | next owner |
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Conflict Resolution Coach.

### Execution
- Execute guided mediation prompts deterministically and capture reproducible traces.
...
## Operator Use Cases
- Operate Security Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `security-and-privacy:collaboration-mediator`.
- **Review posture (score >= 59 or risk >= 58):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Conflict Resolution Coach`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- **Core:** Implement the core capability using guided mediation prompts, and produce conflict remediation playbooks with deterministic scoring.
...
- Scoring weights: `truth=0.28, execution=0.17, safety=0.27, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 59
  - `review_risk`: risk >= 58
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `conflict remediation playbooks`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `conflict remediation playbooks`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `conflict remediation playbooks`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `conflict remediation playbooks`.
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for security-and-privacy:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
## Handoff Contract
- **Produces:** `conflict remediation playbooks`, scorecard, risk/confidence metadata, remediation backlog.
- **Consumes:** `permissions`, `sensitive data flows`, `threat events`, `claims`, `evidence`, `confidence traces`.
- **Readiness rule:** release only when schema, determinism, policy, and reliability gates all pass.
- **Downstream hint:** route only to `security-and-privacy:collaboration-mediator` consumers with approval context attached.
