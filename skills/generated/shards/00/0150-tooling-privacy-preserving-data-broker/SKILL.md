...
description: Build and operate the "Tooling Privacy Preserving Data Broker" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Privacy Preserving Data Broker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-privacy-preserving-data-broker
...
## Output Contract
- `tooling_privacy_preserving_data_broker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_privacy_preserving_data_broker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_privacy_preserving_data_broker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Privacy Preserving Data Broker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling privacy preserving data broker normalization/assessment.
