...
## When To Use
Use this skill when you need "Memory Security Threat Modeler" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-security-threat-modeler
...
## Output Contract
- `memory_security_threat_modeler_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_security_threat_modeler_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_security_threat_modeler_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Security Threat Modeler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory security threat modeler normalization/assessment.
