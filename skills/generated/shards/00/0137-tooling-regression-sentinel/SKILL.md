---
name: u0137-tooling-regression-sentinel
description: Run the Tooling Regression Sentinel capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Regression Sentinel
...
| Feature flag | `skill_0137_tooling-regression-sentinel` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill prevents unnoticed quality drift after updates.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Regression Sentinel.
...
## Operator Use Cases
- Operate Tooling Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Regression Sentinel`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.23, execution=0.38, safety=0.14, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 47
  - `review_risk`: risk >= 37
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
