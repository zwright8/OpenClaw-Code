...
description: Build and operate the "Tooling Regression Sentinel" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Regression Sentinel" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-regression-sentinel
...
## Output Contract
- `tooling_regression_sentinel_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_regression_sentinel_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_regression_sentinel_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Regression Sentinel** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling regression sentinel normalization/assessment.
