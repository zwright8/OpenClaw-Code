...
description: Build and operate the "PublicService Signal Ingestion Normalizer" capability for Healthcare and Public Services. Trigger when this exact capability is needed in mission execution.
---
...
## When To Use
Use this skill when the request explicitly needs "PublicService Signal Ingestion Normalizer" outcomes in the Healthcare and Public Services domain.
...
## Handoff Contract
- Produces: PublicService Signal Ingestion Normalizer normalized artifacts; execution scorecard; risk posture
- Consumes: protocol checks; service queues; compliance flags; claims; evidence; confidence traces
- Downstream routing hint: Route next to healthcare-and-public-services:normalization-engine consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **PublicService Signal Ingestion Normalizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Healthcare and Public Services**
   - Input: noisy upstream payload requiring publicservice signal ingestion normalizer normalization/assessment.
