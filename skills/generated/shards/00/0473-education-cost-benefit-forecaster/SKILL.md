---
name: u0473-education-cost-benefit-forecaster
description: Run the Education Cost-Benefit Forecaster capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Cost-Benefit Forecaster
...
| Feature flag | `skill_0473_education-cost-benefit-forecaste` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs Education Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Education Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
- **Review posture (score >= 55 or risk >= 57):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 86):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Cost-Benefit Forecaster`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.31, execution=0.22, safety=0.32, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 55
  - `review_risk`: risk >= 57
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
