---
name: u0022-epistemic-multi-agent-negotiation-mediator
description: Run the Epistemic Multi-Agent Negotiation Mediator capability for Truth-Seeking and Epistemics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Epistemic Multi-Agent Negotiation Mediator
...
| Feature flag | `skill_0022_epistemic-multi-agent-negotiatio` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions drift when claims are accepted without verification. This specific skill resolves resource and strategy conflicts with explicit tradeoffs.

## Trigger Checklist
- [ ] The task explicitly needs Epistemic Multi-Agent Negotiation Mediator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Epistemic Multi-Agent Negotiation Mediator.
...
## Operator Use Cases
- Operate Epistemic Multi-Agent Negotiation Mediator as a reliable, reusable production workflow.
...
- **Review posture (score >= 60 or risk >= 55):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Epistemic Multi-Agent Negotiation Mediator`, including at least three measurable KPIs tied to false certainty and unverified assumptions.
...
- Scoring weights: `truth=0.32, execution=0.16, safety=0.34, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 60
  - `review_risk`: risk >= 55
...
1. Incident recovery in Truth-Seeking and Epistemics: ingest noisy signals, execute structured bargaining protocols, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Epistemic Multi-Agent Negotiation Mediator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
