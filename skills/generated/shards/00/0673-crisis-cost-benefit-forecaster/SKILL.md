---
name: u0673-crisis-cost-benefit-forecaster
description: Run the Crisis Cost-Benefit Forecaster capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Cost-Benefit Forecaster
...
| Feature flag | `skill_0673_crisis-cost-benefit-forecaster` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Crisis Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `crisis-and-incident-response:simulation-lab`.
- **Review posture (score >= 59 or risk >= 56):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 89):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Cost-Benefit Forecaster`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.23, execution=0.30, safety=0.30, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 59
  - `review_risk`: risk >= 56
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
