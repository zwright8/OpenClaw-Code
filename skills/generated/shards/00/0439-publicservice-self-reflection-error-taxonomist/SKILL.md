...
description: Run the PublicService Self-Reflection Error Taxonomist capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0439_publicservice-self-reflection-er` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Self-Reflection Error Taxonomist (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Self-Reflection Error Taxonomist.
...
## Operator Use Cases
- Operate PublicService Self-Reflection Error Taxonomist as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `healthcare-and-public-services:general-capability`.
- **Review posture (score >= 59 or risk >= 47):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Self-Reflection Error Taxonomist`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.30, execution=0.25, safety=0.33, impact=0.12`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 59
  - `review_risk`: risk >= 47
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute failure-type clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Self-Reflection Error Taxonomist against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
