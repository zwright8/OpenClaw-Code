---
name: u0900-rights-semantic-retrieval-ranker
description: Run the Rights Semantic Retrieval Ranker capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0900_rights-semantic-retrieval-ranker` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Semantic Retrieval Ranker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Semantic Retrieval Ranker.
...
## Operator Use Cases
- Operate Rights Semantic Retrieval Ranker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `legal-rights-and-compliance:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Semantic Retrieval Ranker`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.32, execution=0.19, safety=0.26, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 56
  - `review_risk`: risk >= 60
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `ranked retrieval results`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute semantic relevance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Semantic Retrieval Ranker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
