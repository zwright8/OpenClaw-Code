---
name: u0485-research-confidence-calibration-engine
description: Run the Research Confidence Calibration Engine capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Confidence Calibration Engine
...
| Routing tag | `scientific-research-systems:optimization-engine` |
| Feature flag | `skill_0485_research-confidence-calibration-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Research Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Research Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `scientific-research-systems:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Confidence Calibration Engine`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.19, execution=0.42, safety=0.13, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 49
  - `review_risk`: risk >= 42
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for scientific-research-systems:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `scientific-research-systems:optimization-engine` consumers with approval context attached.
