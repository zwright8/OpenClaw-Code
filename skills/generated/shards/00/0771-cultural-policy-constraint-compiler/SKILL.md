...
description: Run the Cultural Policy Constraint Compiler capability for Multilingual and Cultural Adaptation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `multilingual-and-cultural-adaptation:contract-compiler` |
| Feature flag | `skill_0771_cultural-policy-constraint-compi` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because global utility requires language and cultural context fidelity. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Cultural Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Cultural Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Cultural Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `multilingual-and-cultural-adaptation:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Cultural Policy Constraint Compiler`, including at least three measurable KPIs tied to context mismatch and exclusion.
...
- Scoring weights: `truth=0.16, execution=0.34, safety=0.18, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 44
  - `review_risk`: risk >= 39
...
1. Incident recovery in Multilingual and Cultural Adaptation: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Cultural Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for multilingual-and-cultural-adaptation:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `multilingual-and-cultural-adaptation:contract-compiler` consumers with approval context attached.
