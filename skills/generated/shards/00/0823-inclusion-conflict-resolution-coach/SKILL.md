---
name: u0823-inclusion-conflict-resolution-coach
description: Run the Inclusion Conflict Resolution Coach capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Inclusion Conflict Resolution Coach
...
| Feature flag | `skill_0823_inclusion-conflict-resolution-co` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Inclusion Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `accessibility-and-inclusion:collaboration-mediator`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Conflict Resolution Coach`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.22, execution=0.28, safety=0.28, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 55
  - `review_risk`: risk >= 61
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for accessibility-and-inclusion:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `accessibility-and-inclusion:collaboration-mediator` consumers with approval context attached.
