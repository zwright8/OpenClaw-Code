...
## When To Use
Use this skill when you need "Tooling Uncertainty Communicator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-uncertainty-communicator
...
## Output Contract
- `tooling_uncertainty_communicator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_uncertainty_communicator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_uncertainty_communicator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Uncertainty Communicator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling uncertainty communicator normalization/assessment.
