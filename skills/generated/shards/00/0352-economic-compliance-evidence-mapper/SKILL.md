---
name: u0352-economic-compliance-evidence-mapper
description: Run the Economic Compliance Evidence Mapper capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Compliance Evidence Mapper
...
| Routing tag | `economic-optimization:normalization-engine` |
| Feature flag | `skill_0352_economic-compliance-evidence-map` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Economic Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Economic Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `economic-optimization:normalization-engine`.
- **Review posture (score >= 47 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 67):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Compliance Evidence Mapper`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.19, execution=0.32, safety=0.31, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 47
  - `review_risk`: risk >= 34
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:normalization-engine` consumers with approval context attached.
