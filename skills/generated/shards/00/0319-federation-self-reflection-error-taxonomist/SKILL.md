---
name: u0319-federation-self-reflection-error-taxonomist
description: Run the Federation Self-Reflection Error Taxonomist capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Self-Reflection Error Taxonomist
...
| Feature flag | `skill_0319_federation-self-reflection-error` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill classifies recurrent reasoning failures for targeted fixes.

## Trigger Checklist
- [ ] The task explicitly needs Federation Self-Reflection Error Taxonomist (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Self-Reflection Error Taxonomist.
...
## Operator Use Cases
- Operate Federation Self-Reflection Error Taxonomist as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `federation-and-interoperability:general-capability`.
- **Review posture (score >= 56 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 88):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Self-Reflection Error Taxonomist`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.17, execution=0.23, safety=0.36, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 56
  - `review_risk`: risk >= 46
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute failure-type clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Self-Reflection Error Taxonomist against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
