...
description: Operate the "Federation Self-Reflection Error Taxonomist" capability in production for the target domain workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `319`
- **Skill Name:** `u0319-federation-self-reflection-error-taxonomist`
...
|---|---|---|---|
| error_taxonomies_report | structured-report | true | orchestrator |
| error_taxonomies_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `failure-type clustering` with deterministic settings and trace capture enabled.
3. Produce `error taxonomies` outputs in machine-readable form for orchestrator/operator use.
4. Keep routing aligned with `federation-and-interoperability:general-capability` and include approval context.
...
### Handoff Contract
- **Produces:** `Federation Self-Reflection Error Taxonomist normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `envelopes`, `tenant boundaries`, `protocol bridges`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to federation-and-interoperability:general-capability consumers with approval-gate context
