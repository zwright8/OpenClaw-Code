...
## When To Use
Use this skill when you need "Tooling Goal Decomposer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-goal-decomposer
...
## Output Contract
- `tooling_goal_decomposer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_goal_decomposer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_goal_decomposer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Goal Decomposer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling goal decomposer normalization/assessment.
