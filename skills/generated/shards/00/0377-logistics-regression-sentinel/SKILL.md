...
description: Build and operate the "Logistics Regression Sentinel" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Regression Sentinel" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-regression-sentinel
...
## Output Contract
- `logistics_regression_sentinel_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_regression_sentinel_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_regression_sentinel_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Regression Sentinel** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics regression sentinel normalization/assessment.
