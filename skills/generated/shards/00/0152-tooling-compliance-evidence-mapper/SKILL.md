...
description: Build and operate the "Tooling Compliance Evidence Mapper" capability for Tool Reliability and Execution Quality. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Tooling Compliance Evidence Mapper** in the **Tool Reliability and Execution Quality** capability family.
...
## Handoff Contract
- Produces: Tooling Compliance Evidence Mapper normalized artifacts; execution scorecard; risk posture
- Consumes: tool runs; error signatures; retry outcomes; claims; evidence; confidence traces
- Downstream routing hint: Route next to tool-reliability-and-execution-quality:normalization-engine consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Compliance Evidence Mapper** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling compliance evidence mapper normalization/assessment.
...
## When To Use
Use this when a request in **Tool Reliability and Execution Quality** depends on **Tooling Compliance Evidence Mapper** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
