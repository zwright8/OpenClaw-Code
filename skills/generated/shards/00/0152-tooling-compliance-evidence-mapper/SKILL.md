---
name: u0152-tooling-compliance-evidence-mapper
description: Run the Tooling Compliance Evidence Mapper capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Compliance Evidence Mapper
...
| Routing tag | `tool-reliability-and-execution-quality:normalization-engine` |
| Feature flag | `skill_0152_tooling-compliance-evidence-mapp` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Tooling Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:normalization-engine`.
- **Review posture (score >= 52 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 63):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Compliance Evidence Mapper`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.27, execution=0.30, safety=0.23, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 52
  - `review_risk`: risk >= 40
...
1. `OPA/Rego` -> auth preflight, execute read+write/orchestrate call(s), normalize output, and attach trace to `compliance evidence matrices`.
2. `Vault/KMS` -> auth preflight, execute read+write/orchestrate call(s), normalize output, and attach trace to `compliance evidence matrices`.
3. `SIEM (Elastic/Splunk)` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `compliance evidence matrices`.
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for tool-reliability-and-execution-quality:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `tool-reliability-and-execution-quality:normalization-engine` consumers with approval context attached.
