---
name: u0501-research-plan-quality-evaluator
description: Run the Research Plan Quality Evaluator capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Plan Quality Evaluator
...
| Feature flag | `skill_0501_research-plan-quality-evaluator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill quantifies whether plans are complete, safe, and feasible.

## Trigger Checklist
- [ ] The task explicitly needs Research Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Research Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `scientific-research-systems:general-capability`.
- **Review posture (score >= 52 or risk >= 52):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 83):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Plan Quality Evaluator`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.17, execution=0.33, safety=0.29, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 52
  - `review_risk`: risk >= 52
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
