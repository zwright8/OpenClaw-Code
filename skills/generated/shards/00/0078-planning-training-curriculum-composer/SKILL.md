---
name: u0078-planning-training-curriculum-composer
description: Run the Planning Training Curriculum Composer capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Planning Training Curriculum Composer
...
| Feature flag | `skill_0078_planning-training-curriculum-com` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because large goals fail when decomposition is inconsistent or incomplete. This specific skill converts capability gaps into actionable upskilling programs.

## Trigger Checklist
- [ ] The task explicitly needs Planning Training Curriculum Composer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Training Curriculum Composer.
...
## Operator Use Cases
- Operate Planning Training Curriculum Composer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:general-capability`.
- **Review posture (score >= 57 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 78):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Training Curriculum Composer`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.16, execution=0.20, safety=0.29, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 57
  - `review_risk`: risk >= 61
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute sequenced learning path design, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Training Curriculum Composer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
