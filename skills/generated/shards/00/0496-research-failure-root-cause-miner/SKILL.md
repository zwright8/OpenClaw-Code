---
name: u0496-research-failure-root-cause-miner
description: Run the Research Failure Root-Cause Miner capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Failure Root-Cause Miner
...
| Feature flag | `skill_0496_research-failure-root-cause-mine` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Research Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Research Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `scientific-research-systems:general-capability`.
- **Review posture (score >= 59 or risk >= 51):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 85):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Failure Root-Cause Miner`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.35, execution=0.12, safety=0.18, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 59
  - `review_risk`: risk >= 51
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
