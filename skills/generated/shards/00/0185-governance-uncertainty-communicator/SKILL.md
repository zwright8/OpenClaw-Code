---
name: u0185-governance-uncertainty-communicator
description: Run the Governance Uncertainty Communicator capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Governance Uncertainty Communicator
...
| Feature flag | `skill_0185_governance-uncertainty-communica` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because high-speed autonomy needs enforceable guardrails to stay aligned. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Governance Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Uncertainty Communicator.
...
## Operator Use Cases
- Operate Governance Uncertainty Communicator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `safety-and-governance:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Uncertainty Communicator`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.18, execution=0.28, safety=0.29, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 56
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
