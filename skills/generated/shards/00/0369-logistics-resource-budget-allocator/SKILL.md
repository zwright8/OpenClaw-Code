...
description: Build and operate the "Logistics Resource Budget Allocator" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Resource Budget Allocator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-resource-budget-allocator
...
## Output Contract
- `logistics_resource_budget_allocator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_resource_budget_allocator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_resource_budget_allocator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Resource Budget Allocator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics resource budget allocator normalization/assessment.
