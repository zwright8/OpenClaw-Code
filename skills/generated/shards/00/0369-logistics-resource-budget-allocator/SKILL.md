---
name: u0369-logistics-resource-budget-allocator
description: Run the Logistics Resource Budget Allocator capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Resource Budget Allocator
...
| Feature flag | `skill_0369_logistics-resource-budget-alloca` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill matches mission ambition to finite execution capacity.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Resource Budget Allocator.
...
## Operator Use Cases
- Operate Logistics Resource Budget Allocator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Resource Budget Allocator`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.28, execution=0.33, safety=0.16, impact=0.22`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 57
  - `review_risk`: risk >= 52
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
