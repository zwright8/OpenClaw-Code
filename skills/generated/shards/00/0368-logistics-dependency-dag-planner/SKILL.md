...
description: Build and operate the "Logistics Dependency DAG Planner" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Dependency DAG Planner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-dependency-dag-planner
...
## Output Contract
- `logistics_dependency_dag_planner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_dependency_dag_planner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_dependency_dag_planner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Dependency DAG Planner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics dependency dag planner normalization/assessment.
