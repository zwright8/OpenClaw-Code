---
name: u0368-logistics-dependency-dag-planner
description: Run the Logistics Dependency DAG Planner capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Dependency DAG Planner
...
| Feature flag | `skill_0368_logistics-dependency-dag-planner` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill prevents sequencing errors and hidden blockers.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Dependency DAG Planner.
...
## Operator Use Cases
- Operate Logistics Dependency DAG Planner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Dependency DAG Planner`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.30, execution=0.19, safety=0.35, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 44
  - `review_risk`: risk >= 32
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
