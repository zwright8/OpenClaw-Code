...
description: Run the Impact Continuous Improvement Planner capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Impact Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `social-impact-measurement:planning-router`.
- **Review posture (score >= 46 or risk >= 42):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Continuous Improvement Planner`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.28, execution=0.30, safety=0.28, impact=0.14`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 46
  - `review_risk`: risk >= 42
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
