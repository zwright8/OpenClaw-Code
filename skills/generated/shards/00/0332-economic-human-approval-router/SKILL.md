...
description: Run the Economic Human Approval Router capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0332_economic-human-approval-router` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Economic Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Human Approval Router.
...
## Operator Use Cases
- Operate Economic Human Approval Router as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Human Approval Router`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.36, execution=0.22, safety=0.16, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 44
  - `review_risk`: risk >= 33
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
