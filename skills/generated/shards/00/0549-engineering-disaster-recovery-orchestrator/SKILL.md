...
description: Run the Engineering Disaster Recovery Orchestrator capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0549_engineering-disaster-recovery-or` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill improves recovery speed after severe outages.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate Engineering Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
- **Review posture (score >= 51 or risk >= 33):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.18, execution=0.31, safety=0.22, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 51
  - `review_risk`: risk >= 33
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
