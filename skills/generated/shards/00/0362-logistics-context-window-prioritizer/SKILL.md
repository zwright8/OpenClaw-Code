...
## When To Use
Use this skill when you need "Logistics Context Window Prioritizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-context-window-prioritizer
...
## Output Contract
- `logistics_context_window_prioritizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_context_window_prioritizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_context_window_prioritizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Context Window Prioritizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics context window prioritizer normalization/assessment.
