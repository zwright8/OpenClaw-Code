---
name: u0362-logistics-context-window-prioritizer
description: Run the Logistics Context Window Prioritizer capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Context Window Prioritizer
...
| Feature flag | `skill_0362_logistics-context-window-priorit` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill surfaces the most decision-relevant context under tight token budgets.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Context Window Prioritizer.
...
## Operator Use Cases
- Operate Logistics Context Window Prioritizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `resource-allocation-and-logistics:planning-router`.
- **Review posture (score >= 52 or risk >= 60):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 81):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Context Window Prioritizer`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.23, execution=0.27, safety=0.23, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 52
  - `review_risk`: risk >= 60
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
