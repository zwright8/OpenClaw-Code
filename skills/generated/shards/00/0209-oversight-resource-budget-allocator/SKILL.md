...
## When To Use
Use this skill only when the request explicitly needs `Oversight Resource Budget Allocator` in Human Oversight and Operator UX and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Oversight Resource Budget Allocator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Human Oversight and Operator UX workflows**
   - Input: noisy upstream payload requiring oversight resource budget allocator normalization/assessment.
