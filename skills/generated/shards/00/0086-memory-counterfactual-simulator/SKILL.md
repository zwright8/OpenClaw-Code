...
## When To Use
Use this skill when you need "Memory Counterfactual Simulator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-counterfactual-simulator
...
## Output Contract
- `memory_counterfactual_simulator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_counterfactual_simulator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_counterfactual_simulator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Counterfactual Simulator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory counterfactual simulator normalization/assessment.
