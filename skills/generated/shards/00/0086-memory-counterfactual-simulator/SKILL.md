...
description: Run the Memory Counterfactual Simulator capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0086_memory-counterfactual-simulator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Counterfactual Simulator.
...
## Operator Use Cases
- Operate Memory Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `memory-and-knowledge-operations:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Counterfactual Simulator`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.22, execution=0.34, safety=0.15, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 51
  - `review_risk`: risk >= 40
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for memory-and-knowledge-operations:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `memory-and-knowledge-operations:simulation-lab` consumers with approval context attached.
