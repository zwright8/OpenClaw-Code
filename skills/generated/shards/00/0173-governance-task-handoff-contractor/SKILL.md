---
name: u0173-governance-task-handoff-contractor
description: Run the Governance Task Handoff Contractor capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Governance Task Handoff Contractor
...
| Feature flag | `skill_0173_governance-task-handoff-contract` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because high-speed autonomy needs enforceable guardrails to stay aligned. This specific skill standardizes handoffs between agents and humans.

## Trigger Checklist
- [ ] The task explicitly needs Governance Task Handoff Contractor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Task Handoff Contractor.
...
## Operator Use Cases
- Operate Governance Task Handoff Contractor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `safety-and-governance:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Task Handoff Contractor`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.29, execution=0.29, safety=0.10, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 53
  - `review_risk`: risk >= 51
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute contracted payload schemas, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Task Handoff Contractor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for safety-and-governance:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `safety-and-governance:contract-compiler` consumers with approval context attached.
