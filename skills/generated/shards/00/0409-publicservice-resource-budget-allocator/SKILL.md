---
name: u0409-publicservice-resource-budget-allocator
description: Run the PublicService Resource Budget Allocator capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Resource Budget Allocator
...
| Feature flag | `skill_0409_publicservice-resource-budget-al` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill matches mission ambition to finite execution capacity.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Resource Budget Allocator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Resource Budget Allocator.
...
## Operator Use Cases
- Operate PublicService Resource Budget Allocator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `healthcare-and-public-services:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Resource Budget Allocator`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.29, execution=0.21, safety=0.17, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 59
  - `review_risk`: risk >= 57
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `budgeted execution plans`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute capacity-aware allocation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Resource Budget Allocator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
