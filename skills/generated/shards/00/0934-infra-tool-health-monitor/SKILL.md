---
name: u0934-infra-tool-health-monitor
description: Run the Infra Tool Health Monitor capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Infra Tool Health Monitor
...
| Feature flag | `skill_0934_infra-tool-health-monitor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill detects tool flakiness before it impacts mission outcomes.

## Trigger Checklist
- [ ] The task explicitly needs Infra Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Tool Health Monitor.
...
## Operator Use Cases
- Operate Infra Tool Health Monitor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `infrastructure-and-sustainability:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Tool Health Monitor`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.22, execution=0.24, safety=0.20, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 47
  - `review_risk`: risk >= 46
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
