---
name: u0123-tooling-evidence-provenance-tracker
description: Run the Tooling Evidence Provenance Tracker capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Evidence Provenance Tracker
...
| Routing tag | `tool-reliability-and-execution-quality:provenance-tracker` |
| Feature flag | `skill_0123_tooling-evidence-provenance-trac` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Tooling Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:provenance-tracker`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Evidence Provenance Tracker`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.14, execution=0.28, safety=0.37, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 59
  - `review_risk`: risk >= 61
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for tool-reliability-and-execution-quality:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `tool-reliability-and-execution-quality:provenance-tracker` consumers with approval context attached.
