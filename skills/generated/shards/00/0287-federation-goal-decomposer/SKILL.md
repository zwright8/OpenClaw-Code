...
description: Run the Federation Goal Decomposer capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0287_federation-goal-decomposer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Federation Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Goal Decomposer.
...
## Operator Use Cases
- Operate Federation Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `federation-and-interoperability:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Goal Decomposer`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.35, execution=0.16, safety=0.32, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 47
  - `review_risk`: risk >= 35
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
