---
name: u0097-memory-regression-sentinel
description: Run the Memory Regression Sentinel capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0097_memory-regression-sentinel` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Regression Sentinel.
...
## Operator Use Cases
- Operate Memory Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `memory-and-knowledge-operations:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Regression Sentinel`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.27, execution=0.26, safety=0.16, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 52
  - `review_risk`: risk >= 56
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
