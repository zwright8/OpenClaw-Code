...
description: Build and operate the "Memory Regression Sentinel" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Regression Sentinel" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-regression-sentinel
...
## Output Contract
- `memory_regression_sentinel_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_regression_sentinel_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_regression_sentinel_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Regression Sentinel** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory regression sentinel normalization/assessment.
