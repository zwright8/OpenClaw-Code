---
name: u0719-impact-self-reflection-error-taxonomist
description: Run the Impact Self-Reflection Error Taxonomist capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Self-Reflection Error Taxonomist
...
| Feature flag | `skill_0719_impact-self-reflection-error-tax` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill classifies recurrent reasoning failures for targeted fixes.

## Trigger Checklist
- [ ] The task explicitly needs Impact Self-Reflection Error Taxonomist (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Self-Reflection Error Taxonomist.
...
## Operator Use Cases
- Operate Impact Self-Reflection Error Taxonomist as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
- **Review posture (score >= 43 or risk >= 33):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 73):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Self-Reflection Error Taxonomist`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.26, execution=0.20, safety=0.24, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 43
  - `review_risk`: risk >= 33
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute failure-type clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Self-Reflection Error Taxonomist against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:general-capability, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:general-capability` consumers with approval context attached.
