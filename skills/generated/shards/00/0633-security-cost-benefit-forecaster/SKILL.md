---
name: u0633-security-cost-benefit-forecaster
description: Run the Security Cost-Benefit Forecaster capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Cost-Benefit Forecaster
...
| Feature flag | `skill_0633_security-cost-benefit-forecaster` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs Security Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Security Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `security-and-privacy:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Cost-Benefit Forecaster`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.17, execution=0.33, safety=0.18, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 56
  - `review_risk`: risk >= 56
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
