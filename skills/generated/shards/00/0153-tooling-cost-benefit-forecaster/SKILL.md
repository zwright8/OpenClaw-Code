...
description: Build and operate the "Tooling Cost-Benefit Forecaster" capability for Tool Reliability and Execution Quality. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Tooling Cost-Benefit Forecaster** in the **Tool Reliability and Execution Quality** capability family.
...
## Handoff Contract
- Produces: Tooling Cost-Benefit Forecaster normalized artifacts; execution scorecard; risk posture
- Consumes: tool runs; error signatures; retry outcomes; claims; evidence; confidence traces
- Downstream routing hint: Route next to tool-reliability-and-execution-quality:simulation-lab consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Cost-Benefit Forecaster** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling cost-benefit forecaster normalization/assessment.
...
## When To Use
Use this when a request in **Tool Reliability and Execution Quality** depends on **Tooling Cost-Benefit Forecaster** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
