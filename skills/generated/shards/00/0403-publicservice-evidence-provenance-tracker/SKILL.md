...
description: Run the PublicService Evidence Provenance Tracker capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `healthcare-and-public-services:provenance-tracker` |
| Feature flag | `skill_0403_publicservice-evidence-provenanc` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate PublicService Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `healthcare-and-public-services:provenance-tracker`.
- **Review posture (score >= 43 or risk >= 41):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 67):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Evidence Provenance Tracker`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.26, execution=0.23, safety=0.25, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 43
  - `review_risk`: risk >= 41
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:provenance-tracker` consumers with approval context attached.
