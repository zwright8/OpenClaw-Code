...
description: Build and operate the "Logistics Auto-Retry and Backoff Coordinator" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Auto-Retry and Backoff Coordinator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-auto-retry-and-backoff-coordinator
...
## Output Contract
- `logistics_auto_retry_and_backoff_coordinator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_auto_retry_and_backoff_coordinator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_auto_retry_and_backoff_coordinator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Auto-Retry and Backoff Coordinator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics auto-retry and backoff coordinator normalization/assessment.
