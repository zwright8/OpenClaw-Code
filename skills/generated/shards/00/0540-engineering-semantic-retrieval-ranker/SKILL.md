...
description: Run the Engineering Semantic Retrieval Ranker capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0540_engineering-semantic-retrieval-r` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Semantic Retrieval Ranker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Semantic Retrieval Ranker.
...
## Operator Use Cases
- Operate Engineering Semantic Retrieval Ranker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `software-engineering-automation:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Semantic Retrieval Ranker`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.22, execution=0.17, safety=0.20, impact=0.41`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 43
  - `review_risk`: risk >= 37
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute semantic relevance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Semantic Retrieval Ranker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
