...
description: Run the Inclusion Regression Sentinel capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0817_inclusion-regression-sentinel` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill prevents unnoticed quality drift after updates.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Regression Sentinel.
...
## Operator Use Cases
- Operate Inclusion Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `accessibility-and-inclusion:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Regression Sentinel`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.24, execution=0.21, safety=0.38, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 58
  - `review_risk`: risk >= 61
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
