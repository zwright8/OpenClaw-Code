---
name: u0657-crisis-regression-sentinel
description: Run the Crisis Regression Sentinel capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Regression Sentinel
...
| Feature flag | `skill_0657_crisis-regression-sentinel` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill prevents unnoticed quality drift after updates.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Regression Sentinel.
...
## Operator Use Cases
- Operate Crisis Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `crisis-and-incident-response:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Regression Sentinel`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.23, execution=0.23, safety=0.27, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 52
  - `review_risk`: risk >= 54
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `regression watchlists`.
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
