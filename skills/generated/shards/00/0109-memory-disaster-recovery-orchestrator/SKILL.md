...
description: Build and operate the "Memory Disaster Recovery Orchestrator" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Disaster Recovery Orchestrator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-disaster-recovery-orchestrator
...
## Output Contract
- `memory_disaster_recovery_orchestrator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_disaster_recovery_orchestrator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_disaster_recovery_orchestrator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Disaster Recovery Orchestrator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory disaster recovery orchestrator normalization/assessment.
