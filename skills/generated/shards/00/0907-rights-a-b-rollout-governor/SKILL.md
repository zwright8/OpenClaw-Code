...
description: Operate the "Rights A/B Rollout Governor" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `907`
- **Skill Name:** `u0907-rights-a-b-rollout-governor`
...
|---|---|---|---|
| safe_rollout_decisions_report | structured-report | true | orchestrator |
| safe_rollout_decisions_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `staged rollout policies` with deterministic settings and trace capture enabled.
3. Produce `safe rollout decisions` outputs in machine-readable form for orchestrator/operator use.
...
### Handoff Contract
- **Produces:** `Rights A/B Rollout Governor normalized artifacts`, `execution scorecard`, `risk posture`
