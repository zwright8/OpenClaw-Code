...
description: Run the Engineering Risk-Aware Scheduler capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0530_engineering-risk-aware-scheduler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Engineering Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `software-engineering-automation:planning-router`.
- **Review posture (score >= 42 or risk >= 44):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 64):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Risk-Aware Scheduler`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.26, execution=0.12, safety=0.30, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 42
  - `review_risk`: risk >= 44
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
