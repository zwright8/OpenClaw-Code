---
name: u0464-education-explainability-narrative-builder
description: Run the Education Explainability Narrative Builder capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Explainability Narrative Builder
...
| Routing tag | `education-and-upskilling:communication-engine` |
| Feature flag | `skill_0464_education-explainability-narrati` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Education Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Education Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `education-and-upskilling:communication-engine`.
- **Review posture (score >= 58 or risk >= 59):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 79):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Explainability Narrative Builder`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.23, execution=0.27, safety=0.33, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 58
  - `review_risk`: risk >= 59
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for education-and-upskilling:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `education-and-upskilling:communication-engine` consumers with approval context attached.
