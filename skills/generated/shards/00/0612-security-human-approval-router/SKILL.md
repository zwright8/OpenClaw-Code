...
description: Run the Security Human Approval Router capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0612_security-human-approval-router` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Security Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Human Approval Router.
...
## Operator Use Cases
- Operate Security Human Approval Router as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Human Approval Router`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.25, execution=0.25, safety=0.25, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 62
  - `review_risk`: risk >= 57
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
