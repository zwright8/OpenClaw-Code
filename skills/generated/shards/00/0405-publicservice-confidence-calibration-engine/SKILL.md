...
description: Run the PublicService Confidence Calibration Engine capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0405_publicservice-confidence-calibra` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Confidence Calibration Engine.
...
## Operator Use Cases
- Operate PublicService Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `healthcare-and-public-services:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Confidence Calibration Engine`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.29, execution=0.18, safety=0.23, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 43
  - `review_risk`: risk >= 32
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for healthcare-and-public-services:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `healthcare-and-public-services:optimization-engine` consumers with approval context attached.
