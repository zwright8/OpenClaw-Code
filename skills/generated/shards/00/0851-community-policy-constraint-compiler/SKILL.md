---
name: u0851-community-policy-constraint-compiler
description: Run the Community Policy Constraint Compiler capability for Community Engagement and Feedback with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Community Policy Constraint Compiler
...
| Feature flag | `skill_0851_community-policy-constraint-comp` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because real-world feedback loops are necessary for continuous alignment. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Community Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Community Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Community Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `community-engagement-and-feedback:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Community Policy Constraint Compiler`, including at least three measurable KPIs tied to community trust loss and unaddressed concerns.
...
- Scoring weights: `truth=0.23, execution=0.31, safety=0.33, impact=0.14`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 50
  - `review_risk`: risk >= 40
...
1. Incident recovery in Community Engagement and Feedback: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Community Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for community-engagement-and-feedback:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `community-engagement-and-feedback:contract-compiler` consumers with approval context attached.
