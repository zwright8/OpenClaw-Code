---
name: u0265-collab-uncertainty-communicator
description: Run the Collab Uncertainty Communicator capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Uncertainty Communicator
...
| Feature flag | `skill_0265_collab-uncertainty-communicator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Collab Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Uncertainty Communicator.
...
## Operator Use Cases
- Operate Collab Uncertainty Communicator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Uncertainty Communicator`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.22, execution=0.18, safety=0.39, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 55
  - `review_risk`: risk >= 58
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
