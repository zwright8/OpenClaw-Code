...
description: Run the Impact Counterfactual Simulator capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `social-impact-measurement:simulation-lab` |
| Feature flag | `skill_0686_impact-counterfactual-simulator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Impact Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Counterfactual Simulator.
...
## Operator Use Cases
- Operate Impact Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `social-impact-measurement:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Counterfactual Simulator`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.29, execution=0.26, safety=0.25, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 55
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:simulation-lab` consumers with approval context attached.
