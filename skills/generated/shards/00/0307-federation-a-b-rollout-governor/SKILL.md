...
## When To Use
Use this skill only when the request explicitly needs `Federation A/B Rollout Governor` in the target domain and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Federation A/B Rollout Governor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in the target domain workflows**
   - Input: noisy upstream payload requiring federation a/b rollout governor normalization/assessment.
