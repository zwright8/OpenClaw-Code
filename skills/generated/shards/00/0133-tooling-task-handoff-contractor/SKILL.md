...
description: Build and operate the "Tooling Task Handoff Contractor" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Task Handoff Contractor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
3. Implement the core capability using contract-driven routing, with deterministic scoring and reproducible artifact generation.
...
- Core method: contract-driven routing
- Execution mode: deterministic-first with explicit tolerances
- Routing tag: tool-reliability-and-execution-quality:tooling-task-handoff-contractor
...
## Output Contract
- `tooling_task_handoff_contractor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_task_handoff_contractor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_task_handoff_contractor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Task Handoff Contractor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling task handoff contractor normalization/assessment.
