...
description: Run the Impact Failure Root-Cause Miner capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0696_impact-failure-root-cause-miner` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Impact Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Failure Root-Cause Miner`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.26, execution=0.14, safety=0.32, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 43
  - `review_risk`: risk >= 31
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
