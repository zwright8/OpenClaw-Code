...
description: Build and operate the "Governance Signal Ingestion Normalizer" capability for Safety and Governance. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Governance Signal Ingestion Normalizer** in the **Safety and Governance** capability family.
...
## Handoff Contract
- Produces: Governance Signal Ingestion Normalizer normalized artifacts; execution scorecard; risk posture
...
## Trigger Checklist
- [ ] The request explicitly needs **Governance Signal Ingestion Normalizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Safety and Governance**
   - Input: noisy upstream payload requiring governance signal ingestion normalizer normalization/assessment.
...
## When To Use
Use this when a request in **Safety and Governance** depends on **Governance Signal Ingestion Normalizer** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
