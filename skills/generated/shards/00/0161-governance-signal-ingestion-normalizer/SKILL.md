...
description: Run the Governance Signal Ingestion Normalizer capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0161_governance-signal-ingestion-norm` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Signal Ingestion Normalizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Signal Ingestion Normalizer.
...
## Operator Use Cases
- Operate Governance Signal Ingestion Normalizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `safety-and-governance:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Signal Ingestion Normalizer`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.23, execution=0.33, safety=0.26, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 45
  - `review_risk`: risk >= 40
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute schema mapping and validation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Signal Ingestion Normalizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
