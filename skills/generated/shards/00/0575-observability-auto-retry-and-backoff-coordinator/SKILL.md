---
name: u0575-observability-auto-retry-and-backoff-coordinator
description: Run the Observability Auto-Retry and Backoff Coordinator capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Auto-Retry and Backoff Coordinator
...
| Feature flag | `skill_0575_observability-auto-retry-and-bac` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill improves success rates while preventing retry storms.

## Trigger Checklist
- [ ] The task explicitly needs Observability Auto-Retry and Backoff Coordinator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Auto-Retry and Backoff Coordinator.
...
## Operator Use Cases
- Operate Observability Auto-Retry and Backoff Coordinator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `data-quality-and-observability:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Auto-Retry and Backoff Coordinator`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.19, execution=0.33, safety=0.22, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 42
  - `review_risk`: risk >= 32
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute adaptive backoff control, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Auto-Retry and Backoff Coordinator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
