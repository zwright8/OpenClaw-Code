---
name: u0605-security-confidence-calibration-engine
description: Run the Security Confidence Calibration Engine capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Confidence Calibration Engine
...
| Routing tag | `security-and-privacy:optimization-engine` |
| Feature flag | `skill_0605_security-confidence-calibration-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Security Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Security Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `security-and-privacy:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Confidence Calibration Engine`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.29, execution=0.24, safety=0.22, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 50
  - `review_risk`: risk >= 45
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `calibrated confidence scores`.
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for security-and-privacy:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `security-and-privacy:optimization-engine` consumers with approval context attached.
