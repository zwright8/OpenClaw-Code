---
name: u0663-crisis-conflict-resolution-coach
description: Run the Crisis Conflict Resolution Coach capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Conflict Resolution Coach
...
| Feature flag | `skill_0663_crisis-conflict-resolution-coach` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Crisis Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `crisis-and-incident-response:collaboration-mediator`.
- **Review posture (score >= 61 or risk >= 58):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 86):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Conflict Resolution Coach`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.31, execution=0.19, safety=0.14, impact=0.36`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 61
  - `review_risk`: risk >= 58
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for crisis-and-incident-response:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `crisis-and-incident-response:collaboration-mediator` consumers with approval context attached.
