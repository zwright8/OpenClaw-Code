---
name: u0170-governance-risk-aware-scheduler
description: Run the Governance Risk-Aware Scheduler capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0170_governance-risk-aware-scheduler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Governance Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `safety-and-governance:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Risk-Aware Scheduler`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.31, execution=0.21, safety=0.27, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 58
  - `review_risk`: risk >= 53
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
