...
description: Run the Observability Privacy Preserving Data Broker capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0590_observability-privacy-preserving` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Privacy Preserving Data Broker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Privacy Preserving Data Broker.
...
## Operator Use Cases
- Operate Observability Privacy Preserving Data Broker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `data-quality-and-observability:collaboration-mediator`.
- **Review posture (score >= 43 or risk >= 39):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 62):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Privacy Preserving Data Broker`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.23, execution=0.21, safety=0.20, impact=0.36`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 43
  - `review_risk`: risk >= 39
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute policy-scoped data mediation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Privacy Preserving Data Broker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
