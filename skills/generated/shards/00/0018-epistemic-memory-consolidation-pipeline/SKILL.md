---
name: u0018-epistemic-memory-consolidation-pipeline
description: Run the Epistemic Memory Consolidation Pipeline capability for Truth-Seeking and Epistemics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Epistemic Memory Consolidation Pipeline
...
| Feature flag | `skill_0018_epistemic-memory-consolidation-p` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions drift when claims are accepted without verification. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs Epistemic Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Epistemic Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Epistemic Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `truth-seeking-and-epistemics:general-capability`.
- **Review posture (score >= 52 or risk >= 48):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 74):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Epistemic Memory Consolidation Pipeline`, including at least three measurable KPIs tied to false certainty and unverified assumptions.
...
- Scoring weights: `truth=0.25, execution=0.38, safety=0.21, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 52
  - `review_risk`: risk >= 48
...
1. Incident recovery in Truth-Seeking and Epistemics: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Epistemic Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for truth-seeking-and-epistemics:general-capability, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `truth-seeking-and-epistemics:general-capability` consumers with approval context attached.
