---
name: u0691-impact-policy-constraint-compiler
description: Run the Impact Policy Constraint Compiler capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Policy Constraint Compiler
...
| Routing tag | `social-impact-measurement:contract-compiler` |
| Feature flag | `skill_0691_impact-policy-constraint-compile` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Impact Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Impact Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `social-impact-measurement:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Policy Constraint Compiler`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.23, execution=0.42, safety=0.15, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 50
  - `review_risk`: risk >= 42
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:contract-compiler` consumers with approval context attached.
