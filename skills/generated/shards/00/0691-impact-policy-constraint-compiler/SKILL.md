...
description: Operate the "Impact Policy Constraint Compiler" capability in production for  workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill when the request explicitly needs "Impact Policy Constraint Compiler" outcomes in the Social Impact Measurement domain.
...
## Handoff Contract
- Produces: Impact Policy Constraint Compiler normalized artifacts; execution scorecard; risk posture
- Consumes: community outcomes; kpi trends; intervention deltas; claims; evidence; confidence traces
- Downstream routing hint: Route next to social-impact-measurement:contract-compiler consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Impact Policy Constraint Compiler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in workflows**
   - Input: noisy upstream payload requiring impact policy constraint compiler normalization/assessment.
