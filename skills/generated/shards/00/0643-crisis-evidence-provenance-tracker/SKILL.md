...
description: Run the Crisis Evidence Provenance Tracker capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `crisis-and-incident-response:provenance-tracker` |
| Feature flag | `skill_0643_crisis-evidence-provenance-track` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Crisis Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `crisis-and-incident-response:provenance-tracker`.
- **Review posture (score >= 51 or risk >= 31):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 64):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Evidence Provenance Tracker`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.33, execution=0.30, safety=0.13, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 51
  - `review_risk`: risk >= 31
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for crisis-and-incident-response:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `crisis-and-incident-response:provenance-tracker` consumers with approval context attached.
