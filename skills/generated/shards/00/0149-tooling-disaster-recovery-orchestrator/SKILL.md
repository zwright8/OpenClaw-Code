...
description: Build and operate the "Tooling Disaster Recovery Orchestrator" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Disaster Recovery Orchestrator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-disaster-recovery-orchestrator
...
## Output Contract
- `tooling_disaster_recovery_orchestrator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_disaster_recovery_orchestrator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_disaster_recovery_orchestrator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Disaster Recovery Orchestrator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling disaster recovery orchestrator normalization/assessment.
