---
name: u0709-impact-disaster-recovery-orchestrator
description: Run the Impact Disaster Recovery Orchestrator capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Disaster Recovery Orchestrator
...
| Feature flag | `skill_0709_impact-disaster-recovery-orchest` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill improves recovery speed after severe outages.

## Trigger Checklist
- [ ] The task explicitly needs Impact Disaster Recovery Orchestrator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Disaster Recovery Orchestrator.
...
## Operator Use Cases
- Operate Impact Disaster Recovery Orchestrator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `social-impact-measurement:general-capability`.
- **Review posture (score >= 46 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Disaster Recovery Orchestrator`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.25, execution=0.31, safety=0.27, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 46
  - `review_risk`: risk >= 40
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute failover and restoration sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Disaster Recovery Orchestrator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
