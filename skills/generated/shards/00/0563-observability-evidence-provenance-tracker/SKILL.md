...
description: Run the Observability Evidence Provenance Tracker capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0563_observability-evidence-provenanc` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Observability Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Observability Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `data-quality-and-observability:provenance-tracker`.
- **Review posture (score >= 42 or risk >= 40):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 70):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Evidence Provenance Tracker`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.20, execution=0.18, safety=0.27, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 42
  - `review_risk`: risk >= 40
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for data-quality-and-observability:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `data-quality-and-observability:provenance-tracker` consumers with approval context attached.
