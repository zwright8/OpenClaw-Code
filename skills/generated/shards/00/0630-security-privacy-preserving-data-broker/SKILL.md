---
name: u0630-security-privacy-preserving-data-broker
description: Run the Security Privacy Preserving Data Broker capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Privacy Preserving Data Broker
...
| Feature flag | `skill_0630_security-privacy-preserving-data` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill enables collaboration while minimizing raw data exposure.

## Trigger Checklist
- [ ] The task explicitly needs Security Privacy Preserving Data Broker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Privacy Preserving Data Broker.
...
## Operator Use Cases
- Operate Security Privacy Preserving Data Broker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `security-and-privacy:collaboration-mediator`.
- **Review posture (score >= 61 or risk >= 56):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 77):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Privacy Preserving Data Broker`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.17, execution=0.24, safety=0.40, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 61
  - `review_risk`: risk >= 56
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute policy-scoped data mediation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Privacy Preserving Data Broker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
