...
description: Build and operate the "Memory Policy Constraint Compiler" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Policy Constraint Compiler" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-policy-constraint-compiler
...
## Output Contract
- `memory_policy_constraint_compiler_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_policy_constraint_compiler_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_policy_constraint_compiler_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Policy Constraint Compiler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory policy constraint compiler normalization/assessment.
