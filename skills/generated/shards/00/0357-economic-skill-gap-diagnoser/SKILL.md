...
## When To Use
Use this skill when you need "Economic Skill Gap Diagnoser" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: economic-optimization:economic-skill-gap-diagnoser
...
## Output Contract
- `economic_skill_gap_diagnoser_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `economic_skill_gap_diagnoser_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `economic_skill_gap_diagnoser_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Skill Gap Diagnoser** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Economic Optimization**
   - Input: noisy upstream payload requiring economic skill gap diagnoser normalization/assessment.
