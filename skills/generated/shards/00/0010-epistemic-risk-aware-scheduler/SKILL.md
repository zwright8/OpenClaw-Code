---
name: u0010-epistemic-risk-aware-scheduler
description: Run the Epistemic Risk-Aware Scheduler capability for Truth-Seeking and Epistemics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0010_epistemic-risk-aware-scheduler` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Epistemic Risk-Aware Scheduler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Epistemic Risk-Aware Scheduler.
...
## Operator Use Cases
- Operate Epistemic Risk-Aware Scheduler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `truth-seeking-and-epistemics:planning-router`.
- **Review posture (score >= 50 or risk >= 38):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 62):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Epistemic Risk-Aware Scheduler`, including at least three measurable KPIs tied to false certainty and unverified assumptions.
...
- Scoring weights: `truth=0.33, execution=0.18, safety=0.33, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 50
  - `review_risk`: risk >= 38
...
1. Incident recovery in Truth-Seeking and Epistemics: ingest noisy signals, execute risk-weighted sequencing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Epistemic Risk-Aware Scheduler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
