---
name: u0301-federation-plan-quality-evaluator
description: Run the Federation Plan Quality Evaluator capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Plan Quality Evaluator
...
| Feature flag | `skill_0301_federation-plan-quality-evaluato` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill quantifies whether plans are complete, safe, and feasible.

## Trigger Checklist
- [ ] The task explicitly needs Federation Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Federation Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `federation-and-interoperability:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Plan Quality Evaluator`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.28, execution=0.34, safety=0.19, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 59
  - `review_risk`: risk >= 46
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
