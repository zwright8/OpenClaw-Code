...
description: Operate the "Federation Plan Quality Evaluator" capability in production for Federation and Interoperability workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `301`
- **Skill Name:** `u0301-federation-plan-quality-evaluator`
...
|---|---|---|---|
| plan_quality_scorecards_report | structured-report | true | orchestrator |
| plan_quality_scorecards_scorecard | scorecard | true | operator |

## Guidelines
1. Validate required inputs before execution and reject non-conforming payloads early.
2. Run `multi-criteria plan scoring` with deterministic settings and trace capture enabled.
3. Produce `plan quality scorecards` outputs in machine-readable form for orchestrator/operator use.
...
### Handoff Contract
- **Produces:** `Federation Plan Quality Evaluator normalized artifacts`, `execution scorecard`, `risk posture`
