---
name: u0239-oversight-self-reflection-error-taxonomist
description: Run the Oversight Self-Reflection Error Taxonomist capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Oversight Self-Reflection Error Taxonomist
...
| Routing tag | `human-oversight-and-operator-ux:general-capability` |
| Feature flag | `skill_0239_oversight-self-reflection-error-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human teams need fast, legible control when stakes are high. This specific skill classifies recurrent reasoning failures for targeted fixes.

## Trigger Checklist
- [ ] The task explicitly needs Oversight Self-Reflection Error Taxonomist (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Self-Reflection Error Taxonomist.
...
## Operator Use Cases
- Operate Oversight Self-Reflection Error Taxonomist as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `human-oversight-and-operator-ux:general-capability`.
- **Review posture (score >= 48 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Self-Reflection Error Taxonomist`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.14, execution=0.29, safety=0.37, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 48
  - `review_risk`: risk >= 34
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `error taxonomies`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `error taxonomies`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `error taxonomies`.
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute failure-type clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Self-Reflection Error Taxonomist against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for human-oversight-and-operator-ux:general-capability, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `human-oversight-and-operator-ux:general-capability` consumers with approval context attached.
