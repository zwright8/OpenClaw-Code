---
name: u0418-publicservice-memory-consolidation-pipeline
description: Run the PublicService Memory Consolidation Pipeline capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Memory Consolidation Pipeline
...
| Feature flag | `skill_0418_publicservice-memory-consolidati` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate PublicService Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `healthcare-and-public-services:general-capability`.
- **Review posture (score >= 61 or risk >= 59):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 79):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Memory Consolidation Pipeline`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.26, execution=0.23, safety=0.22, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 61
  - `review_risk`: risk >= 59
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
