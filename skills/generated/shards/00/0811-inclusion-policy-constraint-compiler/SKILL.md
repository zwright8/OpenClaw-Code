---
name: u0811-inclusion-policy-constraint-compiler
description: Run the Inclusion Policy Constraint Compiler capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Inclusion Policy Constraint Compiler
...
| Routing tag | `accessibility-and-inclusion:contract-compiler` |
| Feature flag | `skill_0811_inclusion-policy-constraint-comp` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Inclusion Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `accessibility-and-inclusion:contract-compiler`.
- **Review posture (score >= 59 or risk >= 60):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 89):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Policy Constraint Compiler`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.21, execution=0.29, safety=0.21, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 59
  - `review_risk`: risk >= 60
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for accessibility-and-inclusion:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `accessibility-and-inclusion:contract-compiler` consumers with approval context attached.
