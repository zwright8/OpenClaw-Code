---
name: u0752-comms-compliance-evidence-mapper
description: Run the Comms Compliance Evidence Mapper capability for Communication and Explainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Comms Compliance Evidence Mapper
...
| Routing tag | `communication-and-explainability:normalization-engine` |
| Feature flag | `skill_0752_comms-compliance-evidence-mapper` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because complex systems require explanations humans can act on quickly. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Comms Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Comms Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Comms Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `communication-and-explainability:normalization-engine`.
- **Review posture (score >= 50 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 72):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Comms Compliance Evidence Mapper`, including at least three measurable KPIs tied to misinterpretation and trust erosion.
...
- Scoring weights: `truth=0.29, execution=0.14, safety=0.36, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 50
  - `review_risk`: risk >= 32
...
1. Incident recovery in Communication and Explainability: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Comms Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for communication-and-explainability:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `communication-and-explainability:normalization-engine` consumers with approval context attached.
