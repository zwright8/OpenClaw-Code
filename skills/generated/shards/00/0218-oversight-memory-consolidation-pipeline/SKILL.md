...
description: Run the Oversight Memory Consolidation Pipeline capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0218_oversight-memory-consolidation-p` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Oversight Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Oversight Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `human-oversight-and-operator-ux:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Memory Consolidation Pipeline`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.22, execution=0.21, safety=0.23, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 47
  - `review_risk`: risk >= 41
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
