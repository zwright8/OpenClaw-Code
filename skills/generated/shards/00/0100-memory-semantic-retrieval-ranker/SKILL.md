...
description: Build and operate the "Memory Semantic Retrieval Ranker" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Semantic Retrieval Ranker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-semantic-retrieval-ranker
...
## Output Contract
- `memory_semantic_retrieval_ranker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_semantic_retrieval_ranker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_semantic_retrieval_ranker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Semantic Retrieval Ranker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory semantic retrieval ranker normalization/assessment.
