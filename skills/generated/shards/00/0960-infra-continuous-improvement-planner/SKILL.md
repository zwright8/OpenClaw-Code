---
name: u0960-infra-continuous-improvement-planner
description: Run the Infra Continuous Improvement Planner capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Infra Continuous Improvement Planner
...
| Feature flag | `skill_0960_infra-continuous-improvement-pla` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill turns findings into sustained upgrade cycles.

## Trigger Checklist
- [ ] The task explicitly needs Infra Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Infra Continuous Improvement Planner as a reliable, reusable production workflow.
...
- **Review posture (score >= 45 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 59):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Continuous Improvement Planner`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.29, execution=0.23, safety=0.19, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 45
  - `review_risk`: risk >= 46
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
