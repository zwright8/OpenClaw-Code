...
description: Run the Federation Experiment Design Generator capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0306_federation-experiment-design-gen` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Federation Experiment Design Generator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Experiment Design Generator.
...
## Operator Use Cases
- Operate Federation Experiment Design Generator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Experiment Design Generator`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.29, execution=0.39, safety=0.19, impact=0.13`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 52
  - `review_risk`: risk >= 40
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute hypothesis-driven design, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Experiment Design Generator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
