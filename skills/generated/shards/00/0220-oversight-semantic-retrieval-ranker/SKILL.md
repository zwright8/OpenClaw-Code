...
description: Operate the "Oversight Semantic Retrieval Ranker" capability in production for Human Oversight and Operator UX workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill only when the request explicitly needs `Oversight Semantic Retrieval Ranker` in Human Oversight and Operator UX and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Oversight Semantic Retrieval Ranker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Human Oversight and Operator UX workflows**
   - Input: noisy upstream payload requiring oversight semantic retrieval ranker normalization/assessment.
