---
name: u0183-governance-conflict-resolution-coach
description: Run the Governance Conflict Resolution Coach capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Governance Conflict Resolution Coach
...
| Feature flag | `skill_0183_governance-conflict-resolution-c` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because high-speed autonomy needs enforceable guardrails to stay aligned. This specific skill reduces deadlocks in human-agent collaboration loops.

## Trigger Checklist
- [ ] The task explicitly needs Governance Conflict Resolution Coach (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Conflict Resolution Coach.
...
## Operator Use Cases
- Operate Governance Conflict Resolution Coach as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `safety-and-governance:collaboration-mediator`.
- **Review posture (score >= 62 or risk >= 61):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 76):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Conflict Resolution Coach`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.32, execution=0.17, safety=0.32, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 61
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute guided mediation prompts, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Conflict Resolution Coach against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for safety-and-governance:collaboration-mediator, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `safety-and-governance:collaboration-mediator` consumers with approval context attached.
