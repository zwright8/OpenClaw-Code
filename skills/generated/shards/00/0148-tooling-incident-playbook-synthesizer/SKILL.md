...
description: Build and operate the "Tooling Incident Playbook Synthesizer" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Incident Playbook Synthesizer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-incident-playbook-synthesizer
...
## Output Contract
- `tooling_incident_playbook_synthesizer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_incident_playbook_synthesizer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_incident_playbook_synthesizer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Incident Playbook Synthesizer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling incident playbook synthesizer normalization/assessment.
