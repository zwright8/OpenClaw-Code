---
name: u0148-tooling-incident-playbook-synthesizer
description: Run the Tooling Incident Playbook Synthesizer capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Incident Playbook Synthesizer
...
| Feature flag | `skill_0148_tooling-incident-playbook-synthe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill creates repeatable response procedures from incident history.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Incident Playbook Synthesizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Incident Playbook Synthesizer.
...
## Operator Use Cases
- Operate Tooling Incident Playbook Synthesizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Incident Playbook Synthesizer`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.27, execution=0.16, safety=0.23, impact=0.34`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 51
  - `review_risk`: risk >= 43
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute pattern-to-playbook synthesis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Incident Playbook Synthesizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
