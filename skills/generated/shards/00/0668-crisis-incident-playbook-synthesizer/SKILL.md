...
description: Run the Crisis Incident Playbook Synthesizer capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0668_crisis-incident-playbook-synthes` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis Incident Playbook Synthesizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Incident Playbook Synthesizer.
...
## Operator Use Cases
- Operate Crisis Incident Playbook Synthesizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `crisis-and-incident-response:communication-engine`.
- **Review posture (score >= 43 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 63):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Incident Playbook Synthesizer`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.21, execution=0.21, safety=0.26, impact=0.32`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 43
  - `review_risk`: risk >= 46
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute pattern-to-playbook synthesis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Incident Playbook Synthesizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
