...
description: Build and operate the "Tooling Human Approval Router" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Human Approval Router" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-human-approval-router
...
## Output Contract
- `tooling_human_approval_router_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_human_approval_router_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_human_approval_router_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Human Approval Router** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling human approval router normalization/assessment.
