---
name: u0132-tooling-human-approval-router
description: Run the Tooling Human Approval Router capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Tooling Human Approval Router
...
| Feature flag | `skill_0132_tooling-human-approval-router` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because automation collapses when tools are flaky and failure modes are opaque. This specific skill directs high-risk decisions to the right humans quickly.

## Trigger Checklist
- [ ] The task explicitly needs Tooling Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Human Approval Router.
...
## Operator Use Cases
- Operate Tooling Human Approval Router as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Human Approval Router`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.21, execution=0.36, safety=0.17, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 52
  - `review_risk`: risk >= 31
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
