---
name: u0046-planning-counterfactual-simulator
description: Run the Planning Counterfactual Simulator capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Planning Counterfactual Simulator
...
| Routing tag | `strategic-planning-and-decomposition:simulation-lab` |
| Feature flag | `skill_0046_planning-counterfactual-simulato` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because large goals fail when decomposition is inconsistent or incomplete. This specific skill tests alternatives before costly commitments.

## Trigger Checklist
- [ ] The task explicitly needs Planning Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Counterfactual Simulator.
...
## Operator Use Cases
- Operate Planning Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Counterfactual Simulator`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.21, execution=0.15, safety=0.33, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 60
  - `review_risk`: risk >= 46
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for strategic-planning-and-decomposition:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `strategic-planning-and-decomposition:simulation-lab` consumers with approval context attached.
