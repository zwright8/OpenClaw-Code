...
description: Run the Tooling Semantic Retrieval Ranker capability for Tool Reliability and Execution Quality with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0140_tooling-semantic-retrieval-ranke` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Tooling Semantic Retrieval Ranker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Tooling Semantic Retrieval Ranker.
...
## Operator Use Cases
- Operate Tooling Semantic Retrieval Ranker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `tool-reliability-and-execution-quality:planning-router`.
- **Review posture (score >= 45 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Tooling Semantic Retrieval Ranker`, including at least three measurable KPIs tied to silent failures and cascading retries.
...
- Scoring weights: `truth=0.15, execution=0.31, safety=0.26, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 45
  - `review_risk`: risk >= 34
...
1. Incident recovery in Tool Reliability and Execution Quality: ingest noisy signals, execute semantic relevance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Tooling Semantic Retrieval Ranker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
