---
name: u0683-impact-evidence-provenance-tracker
description: Run the Impact Evidence Provenance Tracker capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Evidence Provenance Tracker
...
| Routing tag | `social-impact-measurement:provenance-tracker` |
| Feature flag | `skill_0683_impact-evidence-provenance-track` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill preserves source lineage so claims remain auditable.

## Trigger Checklist
- [ ] The task explicitly needs Impact Evidence Provenance Tracker (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Evidence Provenance Tracker.
...
## Operator Use Cases
- Operate Impact Evidence Provenance Tracker as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `social-impact-measurement:provenance-tracker`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Evidence Provenance Tracker`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.39, execution=0.22, safety=0.18, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 60
  - `review_risk`: risk >= 53
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute signed provenance links, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Evidence Provenance Tracker against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:provenance-tracker, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:provenance-tracker` consumers with approval context attached.
