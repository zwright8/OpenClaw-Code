...
## When To Use
Use this skill only when the request explicitly needs `Economic Risk-Aware Scheduler` in the target domain and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic Risk-Aware Scheduler** outcomes (not generic brainstorming).
...
1. **Incident stabilization in the target domain workflows**
   - Input: noisy upstream payload requiring economic risk-aware scheduler normalization/assessment.
