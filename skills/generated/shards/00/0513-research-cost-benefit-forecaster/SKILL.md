...
description: Run the Research Cost-Benefit Forecaster capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0513_research-cost-benefit-forecaster` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Research Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Research Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `scientific-research-systems:simulation-lab`.
- **Review posture (score >= 54 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Cost-Benefit Forecaster`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.25, execution=0.16, safety=0.33, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 54
  - `review_risk`: risk >= 46
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
