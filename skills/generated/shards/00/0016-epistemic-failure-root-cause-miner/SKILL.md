---
name: u0016-epistemic-failure-root-cause-miner
description: Run the Epistemic Failure Root-Cause Miner capability for Truth-Seeking and Epistemics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Epistemic Failure Root-Cause Miner
...
| Feature flag | `skill_0016_epistemic-failure-root-cause-min` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions drift when claims are accepted without verification. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Epistemic Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Epistemic Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Epistemic Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `truth-seeking-and-epistemics:general-capability`.
- **Review posture (score >= 43 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 72):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Epistemic Failure Root-Cause Miner`, including at least three measurable KPIs tied to false certainty and unverified assumptions.
...
- Scoring weights: `truth=0.22, execution=0.20, safety=0.27, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 43
  - `review_risk`: risk >= 34
...
1. Incident recovery in Truth-Seeking and Epistemics: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Epistemic Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
