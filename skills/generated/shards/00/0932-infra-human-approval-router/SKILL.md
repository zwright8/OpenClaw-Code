---
name: u0932-infra-human-approval-router
description: Run the Infra Human Approval Router capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Infra Human Approval Router
...
| Feature flag | `skill_0932_infra-human-approval-router` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill directs high-risk decisions to the right humans quickly.

## Trigger Checklist
- [ ] The task explicitly needs Infra Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Human Approval Router.
...
## Operator Use Cases
- Operate Infra Human Approval Router as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `infrastructure-and-sustainability:planning-router`.
- **Review posture (score >= 47 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 61):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Human Approval Router`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.34, execution=0.21, safety=0.24, impact=0.21`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 47
  - `review_risk`: risk >= 46
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
