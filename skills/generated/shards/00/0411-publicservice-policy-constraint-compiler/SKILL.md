---
name: u0411-publicservice-policy-constraint-compiler
description: Run the PublicService Policy Constraint Compiler capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Policy Constraint Compiler
...
| Feature flag | `skill_0411_publicservice-policy-constraint-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Policy Constraint Compiler.
...
## Operator Use Cases
- Operate PublicService Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `healthcare-and-public-services:contract-compiler`.
- **Review posture (score >= 46 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 65):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Policy Constraint Compiler`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.25, execution=0.20, safety=0.27, impact=0.27`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 46
  - `review_risk`: risk >= 32
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
