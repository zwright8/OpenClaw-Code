---
name: u0645-crisis-confidence-calibration-engine
description: Run the Crisis Confidence Calibration Engine capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Confidence Calibration Engine
...
| Routing tag | `crisis-and-incident-response:optimization-engine` |
| Feature flag | `skill_0645_crisis-confidence-calibration-en` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Crisis Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `crisis-and-incident-response:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Confidence Calibration Engine`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.37, execution=0.33, safety=0.14, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 47
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for crisis-and-incident-response:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `crisis-and-incident-response:optimization-engine` consumers with approval context attached.
