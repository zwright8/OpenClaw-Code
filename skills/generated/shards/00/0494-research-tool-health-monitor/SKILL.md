---
name: u0494-research-tool-health-monitor
description: Run the Research Tool Health Monitor capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Research Tool Health Monitor
...
| Feature flag | `skill_0494_research-tool-health-monitor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because research throughput degrades without reproducibility and prioritization. This specific skill detects tool flakiness before it impacts mission outcomes.

## Trigger Checklist
- [ ] The task explicitly needs Research Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Tool Health Monitor.
...
## Operator Use Cases
- Operate Research Tool Health Monitor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `scientific-research-systems:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Tool Health Monitor`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.23, execution=0.16, safety=0.29, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 57
  - `review_risk`: risk >= 48
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
