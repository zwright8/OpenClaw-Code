...
description: Run the Federation Skill Gap Diagnoser capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0317_federation-skill-gap-diagnoser` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Federation Skill Gap Diagnoser (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Skill Gap Diagnoser.
...
## Operator Use Cases
- Operate Federation Skill Gap Diagnoser as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Skill Gap Diagnoser`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.26, execution=0.23, safety=0.27, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 57
  - `review_risk`: risk >= 46
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute competency coverage analysis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Skill Gap Diagnoser against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
