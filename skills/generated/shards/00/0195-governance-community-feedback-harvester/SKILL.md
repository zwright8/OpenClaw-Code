...
description: Run the Governance Community Feedback Harvester capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0195_governance-community-feedback-ha` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Community Feedback Harvester (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Community Feedback Harvester.
...
## Operator Use Cases
- Operate Governance Community Feedback Harvester as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `safety-and-governance:normalization-engine`.
- **Review posture (score >= 42 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 60):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Community Feedback Harvester`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.14, execution=0.34, safety=0.17, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 42
  - `review_risk`: risk >= 34
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute feedback normalization and clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Community Feedback Harvester against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
