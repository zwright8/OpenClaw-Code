...
## When To Use
Use this skill when you need "Logistics Task Handoff Contractor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-task-handoff-contractor
...
## Output Contract
- `logistics_task_handoff_contractor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_task_handoff_contractor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_task_handoff_contractor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Task Handoff Contractor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics task handoff contractor normalization/assessment.
