...
description: Build and operate the "Logistics Tool Health Monitor" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Tool Health Monitor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-tool-health-monitor
...
## Output Contract
- `logistics_tool_health_monitor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_tool_health_monitor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_tool_health_monitor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Tool Health Monitor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics tool health monitor normalization/assessment.
