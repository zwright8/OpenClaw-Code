---
name: u0374-logistics-tool-health-monitor
description: Run the Logistics Tool Health Monitor capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Tool Health Monitor
...
| Feature flag | `skill_0374_logistics-tool-health-monitor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill detects tool flakiness before it impacts mission outcomes.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Tool Health Monitor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Tool Health Monitor.
...
## Operator Use Cases
- Operate Logistics Tool Health Monitor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `resource-allocation-and-logistics:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Tool Health Monitor`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.33, execution=0.28, safety=0.23, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 47
  - `review_risk`: risk >= 45
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute telemetry aggregation and SLO checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Tool Health Monitor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
