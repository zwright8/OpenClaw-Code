---
name: u0971-evolution-policy-constraint-compiler
description: Run the Evolution Policy Constraint Compiler capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Evolution Policy Constraint Compiler
...
| Routing tag | `autonomous-learning-and-evolution:contract-compiler` |
| Feature flag | `skill_0971_evolution-policy-constraint-comp` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill turns governance language into enforceable runtime checks.

## Trigger Checklist
- [ ] The task explicitly needs Evolution Policy Constraint Compiler (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Policy Constraint Compiler.
...
## Operator Use Cases
- Operate Evolution Policy Constraint Compiler as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:contract-compiler`.
- **Review posture (score >= 46 or risk >= 35):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 62):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Policy Constraint Compiler`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.23, execution=0.20, safety=0.28, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 46
  - `review_risk`: risk >= 35
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute rule compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Policy Constraint Compiler against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for autonomous-learning-and-evolution:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `autonomous-learning-and-evolution:contract-compiler` consumers with approval context attached.
