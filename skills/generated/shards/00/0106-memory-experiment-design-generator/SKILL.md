...
description: Build and operate the "Memory Experiment Design Generator" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Experiment Design Generator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-experiment-design-generator
...
## Output Contract
- `memory_experiment_design_generator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_experiment_design_generator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_experiment_design_generator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Experiment Design Generator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory experiment design generator normalization/assessment.
