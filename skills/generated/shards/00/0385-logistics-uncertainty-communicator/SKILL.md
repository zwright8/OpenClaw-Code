---
name: u0385-logistics-uncertainty-communicator
description: Run the Logistics Uncertainty Communicator capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Uncertainty Communicator
...
| Feature flag | `skill_0385_logistics-uncertainty-communicat` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Uncertainty Communicator.
...
## Operator Use Cases
- Operate Logistics Uncertainty Communicator as a reliable, reusable production workflow.
...
- **Review posture (score >= 45 or risk >= 44):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 71):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Uncertainty Communicator`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.28, execution=0.26, safety=0.22, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 45
  - `review_risk`: risk >= 44
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
