...
## When To Use
Use this skill when you need "Logistics Semantic Retrieval Ranker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-semantic-retrieval-ranker
...
## Output Contract
- `logistics_semantic_retrieval_ranker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_semantic_retrieval_ranker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_semantic_retrieval_ranker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Semantic Retrieval Ranker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics semantic retrieval ranker normalization/assessment.
