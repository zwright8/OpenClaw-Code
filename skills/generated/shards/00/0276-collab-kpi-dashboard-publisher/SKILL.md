---
name: u0276-collab-kpi-dashboard-publisher
description: Run the Collab KPI Dashboard Publisher capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab KPI Dashboard Publisher
...
| Feature flag | `skill_0276_collab-kpi-dashboard-publisher` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill keeps mission status observable in real time.

## Trigger Checklist
- [ ] The task explicitly needs Collab KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Collab KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `collaboration-and-negotiation:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab KPI Dashboard Publisher`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.15, execution=0.35, safety=0.31, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 56
  - `review_risk`: risk >= 56
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
