---
name: u0624-security-explainability-narrative-builder
description: Run the Security Explainability Narrative Builder capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Explainability Narrative Builder
...
| Feature flag | `skill_0624_security-explainability-narrativ` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Security Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Security Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `security-and-privacy:communication-engine`.
- **Review posture (score >= 52 or risk >= 44):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 73):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Explainability Narrative Builder`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.18, execution=0.20, safety=0.33, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 52
  - `review_risk`: risk >= 44
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
