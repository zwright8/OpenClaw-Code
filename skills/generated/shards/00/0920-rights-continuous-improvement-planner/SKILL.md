---
name: u0920-rights-continuous-improvement-planner
description: Run the Rights Continuous Improvement Planner capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Rights Continuous Improvement Planner
...
| Feature flag | `skill_0920_rights-continuous-improvement-pl` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because operations must preserve rights and satisfy jurisdictional obligations. This specific skill turns findings into sustained upgrade cycles.

## Trigger Checklist
- [ ] The task explicitly needs Rights Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Rights Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `legal-rights-and-compliance:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Continuous Improvement Planner`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.29, execution=0.24, safety=0.30, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 53
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `improvement roadmaps`.
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
