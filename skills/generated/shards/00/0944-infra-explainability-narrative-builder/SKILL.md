...
description: Run the Infra Explainability Narrative Builder capability for Infrastructure and Sustainability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0944_infra-explainability-narrative-b` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because long-term scale depends on resilient and sustainable infrastructure choices. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Infra Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Infra Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Infra Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `infrastructure-and-sustainability:communication-engine`.
- **Review posture (score >= 42 or risk >= 45):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 68):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Infra Explainability Narrative Builder`, including at least three measurable KPIs tied to capacity collapse and environmental debt.
...
- Scoring weights: `truth=0.28, execution=0.18, safety=0.25, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 42
  - `review_risk`: risk >= 45
...
1. Incident recovery in Infrastructure and Sustainability: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Infra Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
