---
name: u0608-security-dependency-dag-planner
description: Run the Security Dependency DAG Planner capability for Security and Privacy with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Security Dependency DAG Planner
...
| Feature flag | `skill_0608_security-dependency-dag-planner` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because production autonomy must default to least privilege and strong privacy. This specific skill prevents sequencing errors and hidden blockers.

## Trigger Checklist
- [ ] The task explicitly needs Security Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Security Dependency DAG Planner.
...
## Operator Use Cases
- Operate Security Dependency DAG Planner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Security Dependency DAG Planner`, including at least three measurable KPIs tied to breach, exfiltration, and over-privileged actions.
...
- Scoring weights: `truth=0.27, execution=0.25, safety=0.30, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 42
  - `review_risk`: risk >= 46
...
1. Incident recovery in Security and Privacy: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Security Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
