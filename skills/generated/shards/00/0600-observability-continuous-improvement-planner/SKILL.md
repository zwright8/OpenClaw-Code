...
description: Run the Observability Continuous Improvement Planner capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0600_observability-continuous-improve` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill turns findings into sustained upgrade cycles.

## Trigger Checklist
- [ ] The task explicitly needs Observability Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Observability Continuous Improvement Planner as a reliable, reusable production workflow.
...
- **Review posture (score >= 54 or risk >= 57):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 77):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Continuous Improvement Planner`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.25, execution=0.39, safety=0.20, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 54
  - `review_risk`: risk >= 57
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
