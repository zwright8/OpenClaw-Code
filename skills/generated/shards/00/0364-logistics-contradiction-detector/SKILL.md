...
description: Run the Logistics Contradiction Detector capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `resource-allocation-and-logistics:detection-guard` |
| Feature flag | `skill_0364_logistics-contradiction-detector` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Logistics Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Contradiction Detector.
...
## Operator Use Cases
- Operate Logistics Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `resource-allocation-and-logistics:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Contradiction Detector`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.38, execution=0.14, safety=0.24, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 56
  - `review_risk`: risk >= 50
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:detection-guard, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:detection-guard` consumers with approval context attached.
