---
name: u0376-logistics-failure-root-cause-miner
description: Run the Logistics Failure Root-Cause Miner capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Logistics Failure Root-Cause Miner
...
| Feature flag | `skill_0376_logistics-failure-root-cause-min` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because impact work fails when scarce resources are not routed intelligently. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Logistics Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Logistics Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Failure Root-Cause Miner`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.34, execution=0.20, safety=0.15, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 49
  - `review_risk`: risk >= 44
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
