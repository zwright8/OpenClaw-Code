...
description: Build and operate the "Logistics Failure Root-Cause Miner" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Failure Root-Cause Miner" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-failure-root-cause-miner
...
## Output Contract
- `logistics_failure_root_cause_miner_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_failure_root_cause_miner_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_failure_root_cause_miner_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Failure Root-Cause Miner** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics failure root-cause miner normalization/assessment.
