...
## When To Use
Use this skill when you need "Memory Training Curriculum Composer" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-training-curriculum-composer
...
## Output Contract
- `memory_training_curriculum_composer_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_training_curriculum_composer_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_training_curriculum_composer_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Training Curriculum Composer** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory training curriculum composer normalization/assessment.
