---
name: u0216-oversight-failure-root-cause-miner
description: Run the Oversight Failure Root-Cause Miner capability for Human Oversight and Operator UX with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Oversight Failure Root-Cause Miner
...
| Feature flag | `skill_0216_oversight-failure-root-cause-min` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human teams need fast, legible control when stakes are high. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Oversight Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Oversight Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Oversight Failure Root-Cause Miner as a reliable, reusable production workflow.
...
- **Review posture (score >= 43 or risk >= 42):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 63):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Oversight Failure Root-Cause Miner`, including at least three measurable KPIs tied to slow interventions and approval bottlenecks.
...
- Scoring weights: `truth=0.29, execution=0.30, safety=0.24, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 43
  - `review_risk`: risk >= 42
...
1. Incident recovery in Human Oversight and Operator UX: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Oversight Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
