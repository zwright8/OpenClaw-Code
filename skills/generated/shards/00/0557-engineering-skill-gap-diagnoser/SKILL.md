...
description: Run the Engineering Skill Gap Diagnoser capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0557_engineering-skill-gap-diagnoser` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Skill Gap Diagnoser (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Skill Gap Diagnoser.
...
## Operator Use Cases
- Operate Engineering Skill Gap Diagnoser as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `software-engineering-automation:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Skill Gap Diagnoser`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.35, execution=0.31, safety=0.19, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 62
  - `review_risk`: risk >= 55
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute competency coverage analysis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Skill Gap Diagnoser against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
