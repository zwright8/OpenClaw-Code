...
description: Build and operate the "Economic KPI Dashboard Publisher" capability for Economic Optimization. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Economic KPI Dashboard Publisher" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: economic-optimization:economic-kpi-dashboard-publisher
...
## Output Contract
- `economic_kpi_dashboard_publisher_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `economic_kpi_dashboard_publisher_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `economic_kpi_dashboard_publisher_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Economic KPI Dashboard Publisher** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Economic Optimization**
   - Input: noisy upstream payload requiring economic kpi dashboard publisher normalization/assessment.
