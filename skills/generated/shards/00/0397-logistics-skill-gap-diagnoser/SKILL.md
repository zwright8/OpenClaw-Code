...
description: Run the Logistics Skill Gap Diagnoser capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0397_logistics-skill-gap-diagnoser` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Logistics Skill Gap Diagnoser (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Skill Gap Diagnoser.
...
## Operator Use Cases
- Operate Logistics Skill Gap Diagnoser as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Skill Gap Diagnoser`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.19, execution=0.29, safety=0.36, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 58
  - `review_risk`: risk >= 53
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute competency coverage analysis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Skill Gap Diagnoser against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
