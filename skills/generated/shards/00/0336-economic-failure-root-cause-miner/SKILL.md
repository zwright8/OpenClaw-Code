---
name: u0336-economic-failure-root-cause-miner
description: Run the Economic Failure Root-Cause Miner capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Failure Root-Cause Miner
...
| Feature flag | `skill_0336_economic-failure-root-cause-mine` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill finds recurring break patterns to speed remediation.

## Trigger Checklist
- [ ] The task explicitly needs Economic Failure Root-Cause Miner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Failure Root-Cause Miner.
...
## Operator Use Cases
- Operate Economic Failure Root-Cause Miner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `economic-optimization:general-capability`.
- **Review posture (score >= 52 or risk >= 33):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 65):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Failure Root-Cause Miner`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.26, execution=0.34, safety=0.20, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 52
  - `review_risk`: risk >= 33
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute error pattern mining, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Failure Root-Cause Miner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
