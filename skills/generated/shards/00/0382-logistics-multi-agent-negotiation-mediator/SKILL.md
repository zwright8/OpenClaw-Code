...
description: Build and operate the "Logistics Multi-Agent Negotiation Mediator" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Multi-Agent Negotiation Mediator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-multi-agent-negotiation-mediator
...
## Output Contract
- `logistics_multi_agent_negotiation_mediator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_multi_agent_negotiation_mediator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_multi_agent_negotiation_mediator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Multi-Agent Negotiation Mediator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics multi-agent negotiation mediator normalization/assessment.
