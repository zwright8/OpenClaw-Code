---
name: u0839-inclusion-self-reflection-error-taxonomist
description: Run the Inclusion Self-Reflection Error Taxonomist capability for Accessibility and Inclusion with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Inclusion Self-Reflection Error Taxonomist
...
| Feature flag | `skill_0839_inclusion-self-reflection-error-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because systems must be operable and understandable for diverse users. This specific skill classifies recurrent reasoning failures for targeted fixes.

## Trigger Checklist
- [ ] The task explicitly needs Inclusion Self-Reflection Error Taxonomist (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Inclusion Self-Reflection Error Taxonomist.
...
## Operator Use Cases
- Operate Inclusion Self-Reflection Error Taxonomist as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `accessibility-and-inclusion:general-capability`.
- **Review posture (score >= 60 or risk >= 54):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 78):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Inclusion Self-Reflection Error Taxonomist`, including at least three measurable KPIs tied to barriers for disabled and underserved groups.
...
- Scoring weights: `truth=0.19, execution=0.34, safety=0.21, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 54
...
1. Incident recovery in Accessibility and Inclusion: ingest noisy signals, execute failure-type clustering, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Inclusion Self-Reflection Error Taxonomist against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
