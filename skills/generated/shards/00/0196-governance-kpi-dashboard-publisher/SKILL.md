---
name: u0196-governance-kpi-dashboard-publisher
description: Run the Governance KPI Dashboard Publisher capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0196_governance-kpi-dashboard-publish` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Governance KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `safety-and-governance:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance KPI Dashboard Publisher`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.24, execution=0.27, safety=0.18, impact=0.31`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 59
  - `review_risk`: risk >= 50
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
