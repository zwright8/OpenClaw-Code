...
description: Build and operate the "Governance Contradiction Detector" capability for Safety and Governance. Use only when production execution explicitly requires this exact capability and output contract.
---
...
- Trigger only when the requested outcome explicitly maps to **Governance Contradiction Detector** in the **Safety and Governance** capability family.
...
## Handoff Contract
- Produces: Governance Contradiction Detector normalized artifacts; execution scorecard; risk posture
- Consumes: policies; violations; mitigation actions; claims; evidence; confidence traces
- Downstream routing hint: Route next to safety-and-governance:detection-guard consumers with approval-gate context
...
## Trigger Checklist
- [ ] The request explicitly needs **Governance Contradiction Detector** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Safety and Governance**
   - Input: noisy upstream payload requiring governance contradiction detector normalization/assessment.
...
## When To Use
Use this when a request in **Safety and Governance** depends on **Governance Contradiction Detector** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
