---
name: u0433-publicservice-cost-benefit-forecaster
description: Run the PublicService Cost-Benefit Forecaster capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# PublicService Cost-Benefit Forecaster
...
| Feature flag | `skill_0433_publicservice-cost-benefit-forec` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because public-facing workflows require strict safety and reliability controls. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs PublicService Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate PublicService Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `healthcare-and-public-services:simulation-lab`.
- **Review posture (score >= 51 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 61):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Cost-Benefit Forecaster`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.20, execution=0.25, safety=0.39, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 51
  - `review_risk`: risk >= 32
...
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `forecasted ROI scenarios`.
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
