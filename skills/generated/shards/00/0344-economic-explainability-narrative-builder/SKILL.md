---
name: u0344-economic-explainability-narrative-builder
description: Run the Economic Explainability Narrative Builder capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Economic Explainability Narrative Builder
...
| Feature flag | `skill_0344_economic-explainability-narrativ` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need explicit tradeoff logic for cost, speed, and impact. This specific skill translates technical decisions into operator-usable narratives.

## Trigger Checklist
- [ ] The task explicitly needs Economic Explainability Narrative Builder (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Explainability Narrative Builder.
...
## Operator Use Cases
- Operate Economic Explainability Narrative Builder as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `economic-optimization:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Explainability Narrative Builder`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.37, execution=0.20, safety=0.24, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 58
  - `review_risk`: risk >= 60
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute reason synthesis and abstraction, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Explainability Narrative Builder against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for economic-optimization:communication-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `economic-optimization:communication-engine` consumers with approval context attached.
