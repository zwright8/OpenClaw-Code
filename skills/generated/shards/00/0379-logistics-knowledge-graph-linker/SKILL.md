...
description: Build and operate the "Logistics Knowledge Graph Linker" capability for Resource Allocation and Logistics. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Logistics Knowledge Graph Linker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: resource-allocation-and-logistics:logistics-knowledge-graph-linker
...
## Output Contract
- `logistics_knowledge_graph_linker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `logistics_knowledge_graph_linker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `logistics_knowledge_graph_linker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Logistics Knowledge Graph Linker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Resource Allocation and Logistics**
   - Input: noisy upstream payload requiring logistics knowledge graph linker normalization/assessment.
