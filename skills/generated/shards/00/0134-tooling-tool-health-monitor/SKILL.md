...
description: Build and operate the "Tooling Tool Health Monitor" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Tool Health Monitor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-tool-health-monitor
...
## Output Contract
- `tooling_tool_health_monitor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_tool_health_monitor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_tool_health_monitor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Tool Health Monitor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling tool health monitor normalization/assessment.
