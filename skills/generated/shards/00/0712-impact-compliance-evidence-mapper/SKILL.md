---
name: u0712-impact-compliance-evidence-mapper
description: Run the Impact Compliance Evidence Mapper capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Impact Compliance Evidence Mapper
...
| Routing tag | `social-impact-measurement:normalization-engine` |
| Feature flag | `skill_0712_impact-compliance-evidence-mappe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because missions need measurable outcomes, not just activity volume. This specific skill keeps proof of control coverage continuously current.

## Trigger Checklist
- [ ] The task explicitly needs Impact Compliance Evidence Mapper (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Compliance Evidence Mapper.
...
## Operator Use Cases
- Operate Impact Compliance Evidence Mapper as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `social-impact-measurement:normalization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Compliance Evidence Mapper`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.35, execution=0.19, safety=0.16, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 58
  - `review_risk`: risk >= 58
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute requirement-to-evidence mapping, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Compliance Evidence Mapper against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for social-impact-measurement:normalization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `social-impact-measurement:normalization-engine` consumers with approval context attached.
