...
description: Run the Logistics Goal Decomposer capability for Resource Allocation and Logistics with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0367_logistics-goal-decomposer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Logistics Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Logistics Goal Decomposer.
...
## Operator Use Cases
- Operate Logistics Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `resource-allocation-and-logistics:planning-router`.
- **Review posture (score >= 58 or risk >= 54):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 75):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Logistics Goal Decomposer`, including at least three measurable KPIs tied to supply shortfalls and fairness gaps.
...
- Scoring weights: `truth=0.30, execution=0.20, safety=0.18, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 58
  - `review_risk`: risk >= 54
...
1. Incident recovery in Resource Allocation and Logistics: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Logistics Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for resource-allocation-and-logistics:planning-router, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `resource-allocation-and-logistics:planning-router` consumers with approval context attached.
