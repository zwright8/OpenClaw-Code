...
description: Operate the "Oversight Security Threat Modeler" capability in production for Human Oversight and Operator UX workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `231`
- **Skill Name:** `u0231-oversight-security-threat-modeler`
...
4. Keep routing aligned with `human-oversight-and-operator-ux:detection-guard` and include approval context.
...
### Handoff Contract
- **Produces:** `Oversight Security Threat Modeler normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `approval queues`, `operator workload`, `intervention history`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to human-oversight-and-operator-ux:detection-guard consumers with approval-gate context
