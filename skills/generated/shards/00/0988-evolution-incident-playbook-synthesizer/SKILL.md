---
name: u0988-evolution-incident-playbook-synthesizer
description: Run the Evolution Incident Playbook Synthesizer capability for Autonomous Learning and Evolution with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Evolution Incident Playbook Synthesizer
...
| Feature flag | `skill_0988_evolution-incident-playbook-synt` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents stagnate without structured reflection and continuous improvement loops. This specific skill creates repeatable response procedures from incident history.

## Trigger Checklist
- [ ] The task explicitly needs Evolution Incident Playbook Synthesizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Evolution Incident Playbook Synthesizer.
...
## Operator Use Cases
- Operate Evolution Incident Playbook Synthesizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `autonomous-learning-and-evolution:communication-engine`.
- **Review posture (score >= 58 or risk >= 46):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 88):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Evolution Incident Playbook Synthesizer`, including at least three measurable KPIs tied to capability stagnation and repeated blind spots.
...
- Scoring weights: `truth=0.22, execution=0.33, safety=0.20, impact=0.26`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 58
  - `review_risk`: risk >= 46
...
1. Incident recovery in Autonomous Learning and Evolution: ingest noisy signals, execute pattern-to-playbook synthesis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Evolution Incident Playbook Synthesizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
