...
description: Run the Impact Incident Playbook Synthesizer capability for Social Impact Measurement with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0708_impact-incident-playbook-synthes` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Impact Incident Playbook Synthesizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Impact Incident Playbook Synthesizer.
...
## Operator Use Cases
- Operate Impact Incident Playbook Synthesizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 73):** release artifacts after validation pass and route to `social-impact-measurement:communication-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Impact Incident Playbook Synthesizer`, including at least three measurable KPIs tied to impact theater and unmeasured harm.
...
- Scoring weights: `truth=0.33, execution=0.26, safety=0.24, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 73
  - `review`: score >= 51
  - `review_risk`: risk >= 40
...
1. Incident recovery in Social Impact Measurement: ingest noisy signals, execute pattern-to-playbook synthesis, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Impact Incident Playbook Synthesizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
