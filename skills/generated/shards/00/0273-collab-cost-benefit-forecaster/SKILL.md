---
name: u0273-collab-cost-benefit-forecaster
description: Run the Collab Cost-Benefit Forecaster capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Cost-Benefit Forecaster
...
| Feature flag | `skill_0273_collab-cost-benefit-forecaster` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill prioritizes actions with the strongest net value.

## Trigger Checklist
- [ ] The task explicitly needs Collab Cost-Benefit Forecaster (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Cost-Benefit Forecaster.
...
## Operator Use Cases
- Operate Collab Cost-Benefit Forecaster as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 71):** release artifacts after validation pass and route to `collaboration-and-negotiation:simulation-lab`.
- **Review posture (score >= 44 or risk >= 34):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 59):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Cost-Benefit Forecaster`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.30, execution=0.16, safety=0.31, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 71
  - `review`: score >= 44
  - `review_risk`: risk >= 34
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute cost-impact simulation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Cost-Benefit Forecaster against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
