...
description: Operate the "Oversight Uncertainty Communicator" capability in production for Human Oversight and Operator UX workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Metadata
- **Skill ID:** `225`
- **Skill Name:** `u0225-oversight-uncertainty-communicator`
...
4. Keep routing aligned with `human-oversight-and-operator-ux:communication-engine` and include approval context.
...
### Handoff Contract
- **Produces:** `Oversight Uncertainty Communicator normalized artifacts`, `execution scorecard`, `risk posture`
- **Consumes:** `approval queues`, `operator workload`, `intervention history`, `claims`, `evidence`, `confidence traces`
- **Downstream Hint:** Route next to human-oversight-and-operator-ux:communication-engine consumers with approval-gate context
