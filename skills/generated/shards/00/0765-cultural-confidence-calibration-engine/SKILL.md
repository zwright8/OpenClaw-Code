...
description: Run the Cultural Confidence Calibration Engine capability for Multilingual and Cultural Adaptation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `multilingual-and-cultural-adaptation:optimization-engine` |
| Feature flag | `skill_0765_cultural-confidence-calibration-` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because global utility requires language and cultural context fidelity. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Cultural Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Cultural Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Cultural Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `multilingual-and-cultural-adaptation:optimization-engine`.
- **Review posture (score >= 55 or risk >= 53):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 82):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Cultural Confidence Calibration Engine`, including at least three measurable KPIs tied to context mismatch and exclusion.
...
- Scoring weights: `truth=0.19, execution=0.25, safety=0.27, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 55
  - `review_risk`: risk >= 53
...
1. Incident recovery in Multilingual and Cultural Adaptation: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Cultural Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for multilingual-and-cultural-adaptation:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `multilingual-and-cultural-adaptation:optimization-engine` consumers with approval context attached.
