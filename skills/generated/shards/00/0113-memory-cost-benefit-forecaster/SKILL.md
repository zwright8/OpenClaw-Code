...
description: Build and operate the "Memory Cost-Benefit Forecaster" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Cost-Benefit Forecaster" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-cost-benefit-forecaster
...
## Output Contract
- `memory_cost_benefit_forecaster_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_cost_benefit_forecaster_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_cost_benefit_forecaster_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Cost-Benefit Forecaster** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory cost-benefit forecaster normalization/assessment.
