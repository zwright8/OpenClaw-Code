---
name: u0165-governance-confidence-calibration-engine
description: Run the Governance Confidence Calibration Engine capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `safety-and-governance:optimization-engine` |
| Feature flag | `skill_0165_governance-confidence-calibratio` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Governance Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `safety-and-governance:optimization-engine`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance Confidence Calibration Engine`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.21, execution=0.29, safety=0.26, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 58
  - `review_risk`: risk >= 58
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for safety-and-governance:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `safety-and-governance:optimization-engine` consumers with approval context attached.
