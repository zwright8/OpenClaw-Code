...
description: Run the PublicService Context Window Prioritizer capability for Healthcare and Public Services with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0402_publicservice-context-window-pri` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs PublicService Context Window Prioritizer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for PublicService Context Window Prioritizer.
...
## Operator Use Cases
- Operate PublicService Context Window Prioritizer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `healthcare-and-public-services:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `PublicService Context Window Prioritizer`, including at least three measurable KPIs tied to service harm and procedural violations.
...
- Scoring weights: `truth=0.34, execution=0.24, safety=0.27, impact=0.15`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 48
  - `review_risk`: risk >= 41
...
1. Incident recovery in Healthcare and Public Services: ingest noisy signals, execute importance scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run PublicService Context Window Prioritizer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
