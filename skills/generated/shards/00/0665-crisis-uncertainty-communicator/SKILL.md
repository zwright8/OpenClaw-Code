---
name: u0665-crisis-uncertainty-communicator
description: Run the Crisis Uncertainty Communicator capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Crisis Uncertainty Communicator
...
| Feature flag | `skill_0665_crisis-uncertainty-communicator` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because response quality determines whether incidents are contained or amplified. This specific skill prevents overstatement by explicitly framing uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Crisis Uncertainty Communicator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis Uncertainty Communicator.
...
## Operator Use Cases
- Operate Crisis Uncertainty Communicator as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis Uncertainty Communicator`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.32, execution=0.19, safety=0.16, impact=0.33`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 53
  - `review_risk`: risk >= 59
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute confidence-bound communication templates, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis Uncertainty Communicator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
