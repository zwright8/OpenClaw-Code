...
description: Run the Engineering Human Approval Router capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
## Trigger Checklist
- [ ] The task explicitly needs Engineering Human Approval Router (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Human Approval Router.
...
## Operator Use Cases
- Operate Engineering Human Approval Router as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `software-engineering-automation:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Human Approval Router`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.20, execution=0.41, safety=0.21, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 52
  - `review_risk`: risk >= 39
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute reviewer routing policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Human Approval Router against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
