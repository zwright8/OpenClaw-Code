...
description: Build and operate the "Memory Task Handoff Contractor" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Task Handoff Contractor" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-task-handoff-contractor
...
## Output Contract
- `memory_task_handoff_contractor_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_task_handoff_contractor_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_task_handoff_contractor_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Task Handoff Contractor** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory task handoff contractor normalization/assessment.
