---
name: u0093-memory-task-handoff-contractor
description: Run the Memory Task Handoff Contractor capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Memory Task Handoff Contractor
...
| Routing tag | `memory-and-knowledge-operations:contract-compiler` |
| Feature flag | `skill_0093_memory-task-handoff-contractor` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because agents lose performance when lessons are not retained and reused. This specific skill standardizes handoffs between agents and humans.

## Trigger Checklist
- [ ] The task explicitly needs Memory Task Handoff Contractor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Task Handoff Contractor.
...
## Operator Use Cases
- Operate Memory Task Handoff Contractor as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 77):** release artifacts after validation pass and route to `memory-and-knowledge-operations:contract-compiler`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Task Handoff Contractor`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.19, execution=0.33, safety=0.19, impact=0.29`
- Posture thresholds:
  - `ready`: score >= 77
  - `review`: score >= 42
  - `review_risk`: risk >= 41
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute contracted payload schemas, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Task Handoff Contractor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for memory-and-knowledge-operations:contract-compiler, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `memory-and-knowledge-operations:contract-compiler` consumers with approval context attached.
