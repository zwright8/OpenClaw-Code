...
description: Run the Education Goal Decomposer capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Routing tag | `education-and-upskilling:planning-router` |
| Feature flag | `skill_0447_education-goal-decomposer` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Education Goal Decomposer (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Goal Decomposer.
...
## Operator Use Cases
- Operate Education Goal Decomposer as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 72):** release artifacts after validation pass and route to `education-and-upskilling:planning-router`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Goal Decomposer`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.31, execution=0.32, safety=0.21, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 72
  - `review`: score >= 61
  - `review_risk`: risk >= 48
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute hierarchical decomposition, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Goal Decomposer against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for education-and-upskilling:planning-router, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `education-and-upskilling:planning-router` consumers with approval context attached.
