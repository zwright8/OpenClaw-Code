...
## When To Use
Use this skill when you need "Memory Uncertainty Communicator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-uncertainty-communicator
...
## Output Contract
- `memory_uncertainty_communicator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_uncertainty_communicator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_uncertainty_communicator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Uncertainty Communicator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory uncertainty communicator normalization/assessment.
