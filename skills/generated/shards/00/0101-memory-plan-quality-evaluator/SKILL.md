...
description: Run the Memory Plan Quality Evaluator capability for Memory and Knowledge Operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0101_memory-plan-quality-evaluator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Memory Plan Quality Evaluator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Memory Plan Quality Evaluator.
...
## Operator Use Cases
- Operate Memory Plan Quality Evaluator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 80):** release artifacts after validation pass and route to `memory-and-knowledge-operations:general-capability`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Memory Plan Quality Evaluator`, including at least three measurable KPIs tied to repeated mistakes and context loss.
...
- Scoring weights: `truth=0.11, execution=0.31, safety=0.30, impact=0.28`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 47
  - `review_risk`: risk >= 43
...
1. Incident recovery in Memory and Knowledge Operations: ingest noisy signals, execute multi-criteria plan scoring, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Memory Plan Quality Evaluator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
