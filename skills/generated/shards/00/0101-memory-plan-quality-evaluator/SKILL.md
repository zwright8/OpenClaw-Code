...
description: Build and operate the "Memory Plan Quality Evaluator" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Plan Quality Evaluator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-plan-quality-evaluator
...
## Output Contract
- `memory_plan_quality_evaluator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_plan_quality_evaluator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_plan_quality_evaluator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Plan Quality Evaluator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory plan quality evaluator normalization/assessment.
