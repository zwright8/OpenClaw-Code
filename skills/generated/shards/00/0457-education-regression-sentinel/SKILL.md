---
name: u0457-education-regression-sentinel
description: Run the Education Regression Sentinel capability for Education and Upskilling with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Education Regression Sentinel
...
| Feature flag | `skill_0457_education-regression-sentinel` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because human capability growth requires targeted planning under constraints. This specific skill prevents unnoticed quality drift after updates.

## Trigger Checklist
- [ ] The task explicitly needs Education Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Education Regression Sentinel.
...
## Operator Use Cases
- Operate Education Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `education-and-upskilling:detection-guard`.
- **Review posture (score >= 49 or risk >= 38):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 72):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Education Regression Sentinel`, including at least three measurable KPIs tied to persistent skill gaps and poor learning outcomes.
...
- Scoring weights: `truth=0.32, execution=0.20, safety=0.27, impact=0.20`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 49
  - `review_risk`: risk >= 38
...
1. Incident recovery in Education and Upskilling: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Education Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
