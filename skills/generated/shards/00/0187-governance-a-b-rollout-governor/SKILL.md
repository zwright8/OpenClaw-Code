---
name: u0187-governance-a-b-rollout-governor
description: Run the Governance A/B Rollout Governor capability for Safety and Governance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0187_governance-a-b-rollout-governor` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Governance A/B Rollout Governor (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Governance A/B Rollout Governor.
...
## Operator Use Cases
- Operate Governance A/B Rollout Governor as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Governance A/B Rollout Governor`, including at least three measurable KPIs tied to unsafe actions and policy drift.
...
- Scoring weights: `truth=0.21, execution=0.27, safety=0.22, impact=0.30`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 52
  - `review_risk`: risk >= 47
...
1. Incident recovery in Safety and Governance: ingest noisy signals, execute staged rollout policies, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Governance A/B Rollout Governor against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
