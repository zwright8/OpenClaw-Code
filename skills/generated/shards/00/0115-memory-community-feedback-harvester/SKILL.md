...
description: Build and operate the "Memory Community Feedback Harvester" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Community Feedback Harvester" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-community-feedback-harvester
...
## Output Contract
- `memory_community_feedback_harvester_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_community_feedback_harvester_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_community_feedback_harvester_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Community Feedback Harvester** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory community feedback harvester normalization/assessment.
