...
description: Run the Rights Counterfactual Simulator capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0886_rights-counterfactual-simulator` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Counterfactual Simulator.
...
## Operator Use Cases
- Operate Rights Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `legal-rights-and-compliance:simulation-lab`.
- **Review posture (score >= 57 or risk >= 57):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 84):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Counterfactual Simulator`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.16, execution=0.36, safety=0.13, impact=0.35`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 57
  - `review_risk`: risk >= 57
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for legal-rights-and-compliance:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `legal-rights-and-compliance:simulation-lab` consumers with approval context attached.
