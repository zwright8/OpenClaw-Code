---
name: u0525-engineering-confidence-calibration-engine
description: Run the Engineering Confidence Calibration Engine capability for Software Engineering Automation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Engineering Confidence Calibration Engine
...
| Routing tag | `software-engineering-automation:optimization-engine` |
| Feature flag | `skill_0525_engineering-confidence-calibrati` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because delivery speed must increase without sacrificing correctness. This specific skill aligns reported confidence with actual uncertainty.

## Trigger Checklist
- [ ] The task explicitly needs Engineering Confidence Calibration Engine (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Engineering Confidence Calibration Engine.
...
## Operator Use Cases
- Operate Engineering Confidence Calibration Engine as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `software-engineering-automation:optimization-engine`.
- **Review posture (score >= 45 or risk >= 31):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 66):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Engineering Confidence Calibration Engine`, including at least three measurable KPIs tied to regressions and brittle release pipelines.
...
- Scoring weights: `truth=0.28, execution=0.35, safety=0.12, impact=0.25`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 45
  - `review_risk`: risk >= 31
...
1. Incident recovery in Software Engineering Automation: ingest noisy signals, execute calibration curves and error bins, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Engineering Confidence Calibration Engine against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for software-engineering-automation:optimization-engine, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `software-engineering-automation:optimization-engine` consumers with approval context attached.
