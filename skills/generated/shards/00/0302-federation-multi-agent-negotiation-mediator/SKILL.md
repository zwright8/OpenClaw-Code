---
name: u0302-federation-multi-agent-negotiation-mediator
description: Run the Federation Multi-Agent Negotiation Mediator capability for Federation and Interoperability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Federation Multi-Agent Negotiation Mediator
...
| Feature flag | `skill_0302_federation-multi-agent-negotiati` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because cross-org collaboration fails without shared contracts and trust primitives. This specific skill resolves resource and strategy conflicts with explicit tradeoffs.

## Trigger Checklist
- [ ] The task explicitly needs Federation Multi-Agent Negotiation Mediator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Federation Multi-Agent Negotiation Mediator.
...
## Operator Use Cases
- Operate Federation Multi-Agent Negotiation Mediator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 79):** release artifacts after validation pass and route to `federation-and-interoperability:collaboration-mediator`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Federation Multi-Agent Negotiation Mediator`, including at least three measurable KPIs tied to integration breakage and trust boundary violations.
...
- Scoring weights: `truth=0.28, execution=0.27, safety=0.21, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 79
  - `review`: score >= 60
  - `review_risk`: risk >= 52
...
1. Incident recovery in Federation and Interoperability: ingest noisy signals, execute structured bargaining protocols, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Federation Multi-Agent Negotiation Mediator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
