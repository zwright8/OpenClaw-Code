...
description: Run the Rights Contradiction Detector capability for Legal, Rights, and Compliance with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0884_rights-contradiction-detector` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Rights Contradiction Detector (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Rights Contradiction Detector.
...
## Operator Use Cases
- Operate Rights Contradiction Detector as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 76):** release artifacts after validation pass and route to `legal-rights-and-compliance:detection-guard`.
- **Review posture (score >= 49 or risk >= 32):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 72):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Rights Contradiction Detector`, including at least three measurable KPIs tied to rights violations and compliance penalties.
...
- Scoring weights: `truth=0.27, execution=0.20, safety=0.30, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 49
  - `review_risk`: risk >= 32
...
1. Incident recovery in Legal, Rights, and Compliance: ingest noisy signals, execute cross-claim consistency checks, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Rights Contradiction Detector against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
