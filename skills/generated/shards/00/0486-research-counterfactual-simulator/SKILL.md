...
description: Run the Research Counterfactual Simulator capability for Scientific Research Systems with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0486_research-counterfactual-simulato` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Research Counterfactual Simulator (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Research Counterfactual Simulator.
...
## Operator Use Cases
- Operate Research Counterfactual Simulator as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 75):** release artifacts after validation pass and route to `scientific-research-systems:simulation-lab`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Research Counterfactual Simulator`, including at least three measurable KPIs tied to non-reproducible results and wasted cycles.
...
- Scoring weights: `truth=0.35, execution=0.35, safety=0.13, impact=0.16`
- Posture thresholds:
  - `ready`: score >= 75
  - `review`: score >= 47
  - `review_risk`: risk >= 40
...
1. Incident recovery in Scientific Research Systems: ingest noisy signals, execute counterfactual replay, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Research Counterfactual Simulator against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
3. Pre-deployment gate: validate artifacts for scientific-research-systems:simulation-lab, enforce approvals, then handoff to downstream orchestrator with next actions.
...
- **Downstream hint:** route only to `scientific-research-systems:simulation-lab` consumers with approval context attached.
