...
description: Run the Crisis KPI Dashboard Publisher capability for Crisis and Incident Response with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0676_crisis-kpi-dashboard-publisher` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Crisis KPI Dashboard Publisher (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Crisis KPI Dashboard Publisher.
...
## Operator Use Cases
- Operate Crisis KPI Dashboard Publisher as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Crisis KPI Dashboard Publisher`, including at least three measurable KPIs tied to slow containment and repeated outages.
...
- Scoring weights: `truth=0.30, execution=0.19, safety=0.32, impact=0.19`
- Posture thresholds:
  - `ready`: score >= 76
  - `review`: score >= 62
  - `review_risk`: risk >= 50
...
1. Incident recovery in Crisis and Incident Response: ingest noisy signals, execute metric synthesis and publication, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Crisis KPI Dashboard Publisher against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
