...
description: Build and operate the "Memory Privacy Preserving Data Broker" capability for Memory and Knowledge Operations. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Memory Privacy Preserving Data Broker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: memory-and-knowledge-operations:memory-privacy-preserving-data-broker
...
## Output Contract
- `memory_privacy_preserving_data_broker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `memory_privacy_preserving_data_broker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `memory_privacy_preserving_data_broker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Memory Privacy Preserving Data Broker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Memory and Knowledge Operations**
   - Input: noisy upstream payload requiring memory privacy preserving data broker normalization/assessment.
