---
name: u0568-observability-dependency-dag-planner
description: Run the Observability Dependency DAG Planner capability for Data Quality and Observability with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Observability Dependency DAG Planner
...
| Feature flag | `skill_0568_observability-dependency-dag-pla` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because decisions are only as good as the quality and visibility of data. This specific skill prevents sequencing errors and hidden blockers.

## Trigger Checklist
- [ ] The task explicitly needs Observability Dependency DAG Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Observability Dependency DAG Planner.
...
## Operator Use Cases
- Operate Observability Dependency DAG Planner as a reliable, reusable production workflow.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Observability Dependency DAG Planner`, including at least three measurable KPIs tied to data drift and blind spots.
...
- Scoring weights: `truth=0.14, execution=0.28, safety=0.40, impact=0.18`
- Posture thresholds:
  - `ready`: score >= 80
  - `review`: score >= 53
  - `review_risk`: risk >= 60
...
1. Incident recovery in Data Quality and Observability: ingest noisy signals, execute dependency graph compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Observability Dependency DAG Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
