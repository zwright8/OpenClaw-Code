...
description: Build and operate the "Tooling Knowledge Graph Linker" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Knowledge Graph Linker" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-knowledge-graph-linker
...
## Output Contract
- `tooling_knowledge_graph_linker_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_knowledge_graph_linker_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_knowledge_graph_linker_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Knowledge Graph Linker** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling knowledge graph linker normalization/assessment.
