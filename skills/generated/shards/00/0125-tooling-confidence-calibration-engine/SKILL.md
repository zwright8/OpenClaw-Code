...
description: Build and operate the "Tooling Confidence Calibration Engine" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Confidence Calibration Engine" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-confidence-calibration-engine
...
## Output Contract
- `tooling_confidence_calibration_engine_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_confidence_calibration_engine_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_confidence_calibration_engine_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Confidence Calibration Engine** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling confidence calibration engine normalization/assessment.
