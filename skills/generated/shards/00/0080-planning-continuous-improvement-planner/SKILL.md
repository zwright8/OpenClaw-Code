...
description: Run the Planning Continuous Improvement Planner capability for Strategic Planning and Decomposition with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0080_planning-continuous-improvement-` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Planning Continuous Improvement Planner (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Planning Continuous Improvement Planner.
...
## Operator Use Cases
- Operate Planning Continuous Improvement Planner as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 70):** release artifacts after validation pass and route to `strategic-planning-and-decomposition:planning-router`.
- **Review posture (score >= 58 or risk >= 55):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 76):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Planning Continuous Improvement Planner`, including at least three measurable KPIs tied to execution stalls and hidden dependency failures.
...
- Scoring weights: `truth=0.23, execution=0.21, safety=0.39, impact=0.17`
- Posture thresholds:
  - `ready`: score >= 70
  - `review`: score >= 58
  - `review_risk`: risk >= 55
...
1. Incident recovery in Strategic Planning and Decomposition: ingest noisy signals, execute closed-loop prioritization, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Planning Continuous Improvement Planner against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
