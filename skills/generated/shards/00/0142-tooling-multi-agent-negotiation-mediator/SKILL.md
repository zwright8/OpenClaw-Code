...
description: Build and operate the "Tooling Multi-Agent Negotiation Mediator" capability for Tool Reliability and Execution Quality. Use when outcomes in this capability family are required for production execution.
---
...
## When To Use
Use this skill when you need "Tooling Multi-Agent Negotiation Mediator" outputs that will influence production decisions, automated routing, policy posture, or external-facing actions.
...
- Routing tag: tool-reliability-and-execution-quality:tooling-multi-agent-negotiation-mediator
...
## Output Contract
- `tooling_multi_agent_negotiation_mediator_report` (structured-report, consumer=orchestrator, guaranteed=true)
- `tooling_multi_agent_negotiation_mediator_scorecard` (scorecard, consumer=operator, guaranteed=true)
- `tooling_multi_agent_negotiation_mediator_handoff` (handoff-packet, consumer=downstream-skill, guaranteed=true)
...
## Trigger Checklist
- [ ] The request explicitly needs **Tooling Multi-Agent Negotiation Mediator** outcomes (not generic brainstorming).
...
1. **Incident stabilization in Tool Reliability and Execution Quality**
   - Input: noisy upstream payload requiring tooling multi-agent negotiation mediator normalization/assessment.
