---
name: u0258-collab-memory-consolidation-pipeline
description: Run the Collab Memory Consolidation Pipeline capability for Collaboration and Negotiation with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Collab Memory Consolidation Pipeline
...
| Feature flag | `skill_0258_collab-memory-consolidation-pipe` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because multi-agent systems underperform without explicit conflict resolution. This specific skill turns raw logs into durable reusable memory.

## Trigger Checklist
- [ ] The task explicitly needs Collab Memory Consolidation Pipeline (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Collab Memory Consolidation Pipeline.
...
## Operator Use Cases
- Operate Collab Memory Consolidation Pipeline as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 78):** release artifacts after validation pass and route to `collaboration-and-negotiation:general-capability`.
- **Review posture (score >= 53 or risk >= 55):** require human review before publish, with explicit remediation notes.
- **Critical posture (risk >= 77):** fail closed, execute `rollback-to-last-stable-baseline`, and escalate with incident packet.

## Traceability Map
- **Scope:** Define the scope and success metrics for `Collab Memory Consolidation Pipeline`, including at least three measurable KPIs tied to deadlocks and degraded trust.
...
- Scoring weights: `truth=0.30, execution=0.22, safety=0.24, impact=0.24`
- Posture thresholds:
  - `ready`: score >= 78
  - `review`: score >= 53
  - `review_risk`: risk >= 55
...
1. Incident recovery in Collaboration and Negotiation: ingest noisy signals, execute episodic-to-semantic consolidation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Collab Memory Consolidation Pipeline against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
