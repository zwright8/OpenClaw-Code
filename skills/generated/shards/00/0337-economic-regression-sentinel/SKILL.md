---
name: u0337-economic-regression-sentinel
description: Run the Economic Regression Sentinel capability for Economic Optimization with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_0337_economic-regression-sentinel` |
| Release cycles | `2` |
...
## Trigger Checklist
- [ ] The task explicitly needs Economic Regression Sentinel (not generic brainstorming).
...
## Operational Runbook
### Preflight
- Confirm scope, owner, and success criteria for Economic Regression Sentinel.
...
## Operator Use Cases
- Operate Economic Regression Sentinel as a reliable, reusable production workflow.
...
## Posture Playbook
- **Ready posture (score >= 74):** release artifacts after validation pass and route to `economic-optimization:detection-guard`.
...
## Traceability Map
- **Scope:** Define the scope and success metrics for `Economic Regression Sentinel`, including at least three measurable KPIs tied to overspending and low-impact allocation.
...
- Scoring weights: `truth=0.25, execution=0.27, safety=0.26, impact=0.23`
- Posture thresholds:
  - `ready`: score >= 74
  - `review`: score >= 42
  - `review_risk`: risk >= 38
...
1. Incident recovery in Economic Optimization: ingest noisy signals, execute baseline-delta detection, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Economic Regression Sentinel against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
