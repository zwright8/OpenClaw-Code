...
| Field | Value |
|---|---|
| Skill pack | `b06-filesystem-debug-presentation-ml` |
| Tool count | `26` |
| Inventory rank span | `2691-2730` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for filesystem, debugging, presentation, ml support tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `cargo-sweep` | `clipboard` | no | no |
| bounded mutation / transformation | `clipboard` | `cargo-sweep` | yes | yes |
| validation & artifact checks | `cargo-sweep` | `clipboard` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `cargo-sweep` | Utility for cleaning up unused build files generated by Cargo | local-installable |
| `clipboard` | Cut, copy, and paste anything, anywhere, all from the terminal | local-installable |
| `libxmlb` | Library for querying compressed XML metadata | local-installable |
| `a2ps` | Any-to-PostScript filter | local-installable |
| `kubernetes-cli@1.32` | Kubernetes command-line interface | local-installable |
| `hostess` | Idempotent command-line utility for managing your /etc/hosts file | local-installable |
| `karchive` | Reading, creating, and manipulating file archives | local-installable |
| `phoneinfoga` | Information gathering framework for phone numbers | local-installable |
| `riscv64-elf-gdb` | GNU debugger for riscv64-elf cross development | local-installable |
| `chisel` | Collection of LLDB commands to assist debugging iOS apps | local-installable |
| `lux` | Fast and simple video downloader | local-installable |
| `bastet` | Bastard Tetris | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `cargo-sweep` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clipboard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxmlb` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `a2ps` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kubernetes-cli@1.32` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `hostess` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `karchive` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `phoneinfoga` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `riscv64-elf-gdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chisel` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bastet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pydantic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `check` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jrnl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pympress` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `global` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spirv-llvm-translator` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `mpi4py` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `allegro` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `tinyproxy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pyenv-virtualenvwrapper` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ncnn` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `felinks` | SFTP/FTP/WebDAV, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dylibbundler` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kubernetes-cli@1.32`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `cargo-sweep` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `clipboard` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `libxmlb` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read/query command sequence, capture outputs and exit code in handoff packet.
4. `a2ps` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `kubernetes-cli@1.32` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `hostess` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a filesystem, debugging, presentation, ml support request, run dry-run first, then produce a validated artifact bundle.
