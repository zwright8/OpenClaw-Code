---
name: clickhouse-webhook-listener
description: Use when tasks require clickhouse webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Webhook Listener
...
| Primary artifact | `clickhouse-event-intake-log` |
| Routing tag | `clickhouse:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns ClickHouse Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes clickhouse webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `CLICKHOUSE_CLIENT_ID`, `CLICKHOUSE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `CLICKHOUSE_TOKEN`, `CLICKHOUSE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CLICKHOUSE_WEBHOOK_SECRET`, `CLICKHOUSE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| clickhouse-event-intake-log | structured-artifact | yes | next workflow or operator |
| clickhouse-event-intake-log-scorecard | scorecard | yes | reviewer |
| clickhouse-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for ClickHouse Webhook Listener, then register the signing-secret policy for ClickHouse.
...
3. Implement captures, verifies, and routes clickhouse webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `clickhouse-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:webhook-listener` consumers with approval and credential context attached.
