---
name: b06-language-runtime-linting-cloud
description: Use when maintaining language runtimes, formatter/lint stacks, and foundational cloud/service networking integrations.
---

# Language runtimes, linting, and cloud plumbing

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-language-runtime-linting-cloud` |
| Tool count | `26` |
| Inventory rank span | `2600-2628` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for language runtimes, linting, cloud plumbing tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `perltidy` | `snakefmt` | no | no |
| bounded mutation / transformation | `snakefmt` | `perltidy` | yes | yes |
| validation & artifact checks | `perltidy` | `snakefmt` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `perltidy` | Indents and reformats Perl scripts to make them easier to read | local-installable |
| `snakefmt` | Snakemake code formatter | local-installable |
| `ruby@3.0` | Powerful, clean, object-oriented scripting language | local-installable |
| `uwsgi` | Full stack for building hosting services | local-installable |
| `hexedit` | View and edit files in hexadecimal or ASCII | local-installable |
| `nethack` | Single-player roguelike video game | local-installable |
| `conda-lock` | Lightweight lockfile for conda environments | local-installable |
| `pint` | Prometheus rule linter/validator | local-installable |
| `guetzli` | Perceptual JPEG encoder | local-installable |
| `fancy-cat` | PDF reader for terminal emulators using the Kitty image protocol | local-installable |
| `unxip` | Fast Xcode unarchiver | local-installable |
| `goffice` | Gnumeric spreadsheet program | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `perltidy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `snakefmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruby@3.0` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `uwsgi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hexedit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nethack` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `conda-lock` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `guetzli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fancy-cat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `unxip` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `goffice` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bbot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxau` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scooter` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xsel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `whosthere` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mask` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sdl3_ttf` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `miniserve` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `luacheck` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dalfox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `iredis` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sqlite-utils` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `strace` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudformation-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `perltidy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `snakefmt` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `ruby@3.0` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `uwsgi` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `hexedit` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `nethack` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a language runtimes, linting, cloud plumbing request, run dry-run first, then produce a validated artifact bundle.
