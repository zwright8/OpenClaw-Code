...
description: Use when tasks require clickhouse connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `clickhouse-connection-trace-bundle` |
| Routing tag | `clickhouse:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Connector into a deterministic, auth-checked workflow for unified read/write connector for clickhouse apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| clickhouse-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| clickhouse-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for ClickHouse Connector, then confirm the smallest read-only probe that proves access to ClickHouse.
...
3. Specify the request and response contract for unified read/write connector for clickhouse apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `clickhouse-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:connector` consumers with approval and credential context attached.
