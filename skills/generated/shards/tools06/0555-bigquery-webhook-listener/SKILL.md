---
name: bigquery-webhook-listener
description: Use when tasks require bigquery webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Webhook Listener
...
| Primary artifact | `bigquery-event-intake-log` |
| Routing tag | `bigquery:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns BigQuery Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes bigquery webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `BIGQUERY_WEBHOOK_SECRET`, `BIGQUERY_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `BIGQUERY_TOKEN`, `BIGQUERY_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| bigquery-event-intake-log | structured-artifact | yes | next workflow or operator |
| bigquery-event-intake-log-scorecard | scorecard | yes | reviewer |
| bigquery-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for BigQuery Webhook Listener, then register the signing-secret policy for BigQuery.
...
3. Implement captures, verifies, and routes bigquery webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `bigquery-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:webhook-listener` consumers with approval and credential context attached.
