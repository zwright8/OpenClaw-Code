---
name: b06-editor-agent-network-subtitle-pdf
description: Use when terminal editing, coding-agent orchestration, network/security checks, subtitle/media pipelines, and PDF analysis must run together.
---

# Editor integration, agent tooling, network ops, and PDF workflows

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-editor-agent-network-subtitle-pdf` |
| Tool count | `25` |
| Inventory rank span | `2818-2851` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for editor integration, agent tooling, network ops, pdf workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `krakend` | `kakoune` | no | no |
| bounded mutation / transformation | `kakoune` | `krakend` | yes | yes |
| validation & artifact checks | `krakend` | `kakoune` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `krakend` | Ultra-High performance API Gateway built in Go | local-installable |
| `kakoune` | Selection-based modal text editor | local-installable |
| `vtcode` | CLI Semantic Coding Agent | local-installable |
| `dvr-scan` | Extract scenes with motion from videos | local-installable |
| `osslsigncode` | OpenSSL based Authenticode signing for PE/MSI/Java CAB files | local-installable |
| `superhtml` | HTML Language Server & Templating Language Library | local-installable |
| `powerman` | Control (remotely and in parallel) switched power distribution units | local-installable |
| `c2patool` | CLI for working with C2PA manifests and media assets | local-installable |
| `rye` | Package Management Solution for Python | local-installable |
| `rmlint` | Extremely fast tool to remove dupes and other lint from your filesystem | local-installable |
| `inframap` | Read your tfstate or HCL to generate a graph | local-installable |
| `podofo` | Library to work with the PDF file format | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `krakend` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kakoune` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vtcode` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dvr-scan` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osslsigncode` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `superhtml` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `powerman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c2patool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rye` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rmlint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `inframap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `podofo` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `wasm-pack` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tcpreplay` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chezscheme` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zmap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `telegram-downloader` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `subliminal` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `okta-awscli` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gulp-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nextdns` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gradle-profiler` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `claude-cmd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `khal` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdfalyzer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `okta-awscli`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `krakend` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `kakoune` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `vtcode` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `dvr-scan` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `osslsigncode` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `superhtml` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a editor integration, agent tooling, network ops, pdf workflows request, run dry-run first, then produce a validated artifact bundle.
