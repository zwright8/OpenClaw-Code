---
name: snowflake-webhook-listener
description: Use when tasks require snowflake webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Webhook Listener
...
| Primary artifact | `snowflake-event-intake-log` |
| Routing tag | `snowflake:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Snowflake Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes snowflake webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `SNOWFLAKE_SERVICE_ACCOUNT_JSON`, `SNOWFLAKE_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `SNOWFLAKE_SESSION`, `SNOWFLAKE_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `SNOWFLAKE_WEBHOOK_SECRET`, `SNOWFLAKE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `SNOWFLAKE_TOKEN`, `SNOWFLAKE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| snowflake-event-intake-log | structured-artifact | yes | next workflow or operator |
| snowflake-event-intake-log-scorecard | scorecard | yes | reviewer |
| snowflake-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Snowflake Webhook Listener, then register the signing-secret policy for Snowflake.
...
3. Implement captures, verifies, and routes snowflake webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `snowflake-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:webhook-listener` consumers with approval and credential context attached.
