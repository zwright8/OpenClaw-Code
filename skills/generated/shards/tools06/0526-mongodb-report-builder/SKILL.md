---
name: mongodb-report-builder
description: Use when tasks require mongodb report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-report-bundle` |
| Routing tag | `mongodb:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from mongodb activity..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-report-bundle | structured-artifact | yes | next workflow or operator |
| mongodb-report-bundle-scorecard | scorecard | yes | reviewer |
| mongodb-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for MongoDB Report Builder, then confirm which MongoDB entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from mongodb activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `mongodb-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:report-builder` consumers with approval and credential context attached.
