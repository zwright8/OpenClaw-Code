---
name: mysql-workflow-automator
description: Use when tasks require mysql workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MySQL Workflow Automator
...
| Primary artifact | `mysql-workflow-runbook` |
| Routing tag | `mysql:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using mysql..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-workflow-runbook | structured-artifact | yes | next workflow or operator |
| mysql-workflow-runbook-scorecard | scorecard | yes | reviewer |
| mysql-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for MySQL Workflow Automator, then checkpoint where MySQL state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using mysql. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `mysql-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] mysql-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:workflow-automator` consumers with approval and credential context attached.
