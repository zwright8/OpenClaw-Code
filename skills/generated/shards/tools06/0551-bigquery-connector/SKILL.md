...
description: Use when tasks require bigquery connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bigquery-connection-trace-bundle` |
| Routing tag | `bigquery:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Connector into a deterministic, auth-checked workflow for unified read/write connector for bigquery apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| bigquery-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| bigquery-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for BigQuery Connector, then confirm the smallest read-only probe that proves access to BigQuery.
...
3. Specify the request and response contract for unified read/write connector for bigquery apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `bigquery-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:connector` consumers with approval and credential context attached.
