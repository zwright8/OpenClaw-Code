---
name: mongodb-data-importer
description: Use when tasks require mongodb data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-import-ledger` |
| Routing tag | `mongodb:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when state changes are hard to undo once they begin. This specific skill turns MongoDB Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into mongodb..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-import-ledger | structured-artifact | yes | next workflow or operator |
| mongodb-import-ledger-scorecard | scorecard | yes | reviewer |
| mongodb-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for MongoDB Data Importer before any MongoDB write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into mongodb. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `mongodb-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:data-importer` consumers with approval and credential context attached.
