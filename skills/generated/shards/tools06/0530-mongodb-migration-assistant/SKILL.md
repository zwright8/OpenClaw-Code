---
name: mongodb-migration-assistant
description: Use when tasks require mongodb migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-migration-cutover-plan` |
| Routing tag | `mongodb:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when state changes are hard to undo once they begin. This specific skill turns MongoDB Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from mongodb..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| mongodb-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| mongodb-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for MongoDB Migration Assistant before any MongoDB rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from mongodb. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `mongodb-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:migration-assistant` consumers with approval and credential context attached.
