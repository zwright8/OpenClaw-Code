---
name: snowflake-workflow-automator
description: Use when tasks require snowflake workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Workflow Automator
...
| Primary artifact | `snowflake-workflow-runbook` |
| Routing tag | `snowflake:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using snowflake..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-workflow-runbook | structured-artifact | yes | next workflow or operator |
| snowflake-workflow-runbook-scorecard | scorecard | yes | reviewer |
| snowflake-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Snowflake Workflow Automator, then checkpoint where Snowflake state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using snowflake. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `snowflake-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:workflow-automator` consumers with approval and credential context attached.
