---
name: databricks-approval-router
description: Use when tasks require databricks approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Approval Router
...
| Primary artifact | `databricks-approval-decision-packet` |
| Routing tag | `databricks:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Approval Router into a deterministic, auth-checked workflow for routes databricks-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| databricks-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| databricks-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Databricks Approval Router, then capture the manual fallback path for Databricks.
...
3. Implement routes databricks-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `databricks-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] databricks-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:approval-router` consumers with approval and credential context attached.
