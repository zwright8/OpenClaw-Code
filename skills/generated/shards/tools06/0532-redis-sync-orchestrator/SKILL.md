---
name: redis-sync-orchestrator
description: Use when tasks require redis sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Sync Orchestrator
...
| Primary artifact | `redis-reconciliation-bundle` |
| Routing tag | `redis:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for redis records..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| redis-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| redis-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Redis Sync Orchestrator, then checkpoint the last known good cursor on both sides of Redis.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for redis records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `redis-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redis-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:sync-orchestrator` consumers with approval and credential context attached.
