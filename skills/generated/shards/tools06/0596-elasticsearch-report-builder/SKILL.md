---
name: elasticsearch-report-builder
description: Use when tasks require elasticsearch report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Report Builder
...
| Primary artifact | `elasticsearch-report-bundle` |
| Routing tag | `elasticsearch:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from elasticsearch activity..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-report-bundle | structured-artifact | yes | next workflow or operator |
| elasticsearch-report-bundle-scorecard | scorecard | yes | reviewer |
| elasticsearch-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Elasticsearch Report Builder, then confirm which Elasticsearch entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from elasticsearch activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `elasticsearch-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:report-builder` consumers with approval and credential context attached.
