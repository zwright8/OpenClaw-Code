---
name: postgres-migration-assistant
description: Use when tasks require postgres migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Migration Assistant
...
| Primary artifact | `postgres-migration-cutover-plan` |
| Routing tag | `postgres:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when state changes are hard to undo once they begin. This specific skill turns Postgres Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from postgres..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| postgres-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| postgres-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Postgres Migration Assistant before any Postgres rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from postgres. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `postgres-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] postgres-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:migration-assistant` consumers with approval and credential context attached.
