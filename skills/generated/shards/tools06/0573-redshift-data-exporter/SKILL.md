---
name: redshift-data-exporter
description: Use when tasks require redshift data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Data Exporter
...
| Primary artifact | `redshift-export-package` |
| Routing tag | `redshift:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Data Exporter into a deterministic, auth-checked workflow for exports redshift entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-export-package | structured-artifact | yes | next workflow or operator |
| redshift-export-package-scorecard | scorecard | yes | reviewer |
| redshift-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Redshift Data Exporter, then constrain the export scope for Redshift before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports redshift entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `redshift-export-package` so downstream owners do not need to rediscover context.
...
- [ ] redshift-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:data-exporter` consumers with approval and credential context attached.
