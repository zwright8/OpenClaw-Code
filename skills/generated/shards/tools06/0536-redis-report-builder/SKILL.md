---
name: redis-report-builder
description: Use when tasks require redis report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Report Builder
...
| Primary artifact | `redis-report-bundle` |
| Routing tag | `redis:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from redis activity..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-report-bundle | structured-artifact | yes | next workflow or operator |
| redis-report-bundle-scorecard | scorecard | yes | reviewer |
| redis-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Redis Report Builder, then confirm which Redis entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from redis activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `redis-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redis-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:report-builder` consumers with approval and credential context attached.
