---
name: b06-network-dns-go-testing-storage
description: Use when diagnosing DNS/network posture, running Go test/report tooling, and managing distributed storage or service runtimes.
---

# Network/DNS operations, Go testing, and distributed storage

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-network-dns-go-testing-storage` |
| Tool count | `25` |
| Inventory rank span | `2910-2939` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for network/dns operations, go testing, distributed storage tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `libxrandr` | `icann-rdap` | no | no |
| bounded mutation / transformation | `icann-rdap` | `libxrandr` | yes | yes |
| validation & artifact checks | `libxrandr` | `icann-rdap` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `libxrandr` | X.Org: X Resize, Rotate and Reflection extension library | local-installable |
| `icann-rdap` | Full-rich client for the Registry Data Access Protocol (RDAP) sponsored by ICANN | local-installable |
| `dnstwist` | Test domains for typo squatting, phishing and corporate espionage | local-installable |
| `dwarfs` | Fast high compression read-only file system for Linux, Windows, and macOS | local-installable |
| `notcurses` | Blingful character graphics/TUI library | local-installable |
| `jetty` | Java servlet engine and webserver | local-installable |
| `hwatch` | Modern alternative to the watch command | local-installable |
| `cdncheck` | Utility to detect various technology for a given IP address | local-installable |
| `udunits` | Unidata unit conversion library | local-installable |
| `coredns` | DNS server that chains plugins | local-installable |
| `bore-cli` | Modern, simple TCP tunnel in Rust that exposes local ports to a remote server | local-installable |
| `par2` | Parchive: Parity Archive Volume Set for data recovery | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `libxrandr` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `icann-rdap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dnstwist` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dwarfs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `notcurses` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `jetty` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hwatch` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cdncheck` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `udunits` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `coredns` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bore-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `par2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gotestsum` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jql` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `saxon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ddns-go` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rio-terminal` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pipdeptree` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpostal` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `chrome-devtools-mcp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ballerina` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `artillery` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aravis` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `patchutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `seaweedfs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `libxrandr` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `icann-rdap` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `dnstwist` (DNS, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `dwarfs` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `notcurses` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `jetty` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a network/dns operations, go testing, distributed storage request, run dry-run first, then produce a validated artifact bundle.
