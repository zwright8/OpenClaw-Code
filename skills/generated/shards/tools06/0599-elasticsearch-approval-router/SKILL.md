---
name: elasticsearch-approval-router
description: Use when tasks require elasticsearch approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Approval Router
...
| Primary artifact | `elasticsearch-approval-decision-packet` |
| Routing tag | `elasticsearch:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Approval Router into a deterministic, auth-checked workflow for routes elasticsearch-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| elasticsearch-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| elasticsearch-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Elasticsearch Approval Router, then capture the manual fallback path for Elasticsearch.
...
3. Implement routes elasticsearch-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `elasticsearch-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:approval-router` consumers with approval and credential context attached.
