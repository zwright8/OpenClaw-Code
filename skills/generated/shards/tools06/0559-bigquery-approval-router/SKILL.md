---
name: bigquery-approval-router
description: Use when tasks require bigquery approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Approval Router
...
| Primary artifact | `bigquery-approval-decision-packet` |
| Routing tag | `bigquery:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Approval Router into a deterministic, auth-checked workflow for routes bigquery-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| bigquery-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| bigquery-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for BigQuery Approval Router, then capture the manual fallback path for BigQuery.
...
3. Implement routes bigquery-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `bigquery-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:approval-router` consumers with approval and credential context attached.
