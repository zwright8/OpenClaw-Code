---
name: clickhouse-alert-monitor
description: Use when tasks require clickhouse alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Alert Monitor
...
| Primary artifact | `clickhouse-alert-state-bundle` |
| Routing tag | `clickhouse:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Alert Monitor into a deterministic, auth-checked workflow for monitors key clickhouse signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| clickhouse-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| clickhouse-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for ClickHouse Alert Monitor, then lock the reviewed threshold set for ClickHouse.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key clickhouse signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `clickhouse-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:alert-monitor` consumers with approval and credential context attached.
