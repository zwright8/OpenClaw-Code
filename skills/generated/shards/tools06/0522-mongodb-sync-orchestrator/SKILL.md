---
name: mongodb-sync-orchestrator
description: Use when tasks require mongodb sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-reconciliation-bundle` |
| Routing tag | `mongodb:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for mongodb records..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| mongodb-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| mongodb-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for MongoDB Sync Orchestrator, then checkpoint the last known good cursor on both sides of MongoDB.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for mongodb records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `mongodb-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:sync-orchestrator` consumers with approval and credential context attached.
