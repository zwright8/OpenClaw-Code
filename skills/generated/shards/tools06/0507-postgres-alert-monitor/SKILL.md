---
name: postgres-alert-monitor
description: Use when tasks require postgres alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Alert Monitor
...
| Primary artifact | `postgres-alert-state-bundle` |
| Routing tag | `postgres:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Alert Monitor into a deterministic, auth-checked workflow for monitors key postgres signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| postgres-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| postgres-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Postgres Alert Monitor, then lock the reviewed threshold set for Postgres.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key postgres signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `postgres-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postgres-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:alert-monitor` consumers with approval and credential context attached.
