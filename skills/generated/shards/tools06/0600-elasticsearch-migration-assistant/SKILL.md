---
name: elasticsearch-migration-assistant
description: Use when tasks require elasticsearch migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Migration Assistant
...
| Primary artifact | `elasticsearch-migration-cutover-plan` |
| Routing tag | `elasticsearch:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when state changes are hard to undo once they begin. This specific skill turns Elasticsearch Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from elasticsearch..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| elasticsearch-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| elasticsearch-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Elasticsearch Migration Assistant before any Elasticsearch rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from elasticsearch. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `elasticsearch-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:migration-assistant` consumers with approval and credential context attached.
