---
name: elasticsearch-sync-orchestrator
description: Use when tasks require elasticsearch sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Sync Orchestrator
...
| Primary artifact | `elasticsearch-reconciliation-bundle` |
| Routing tag | `elasticsearch:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for elasticsearch records..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| elasticsearch-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| elasticsearch-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Elasticsearch Sync Orchestrator, then checkpoint the last known good cursor on both sides of Elasticsearch.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for elasticsearch records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `elasticsearch-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:sync-orchestrator` consumers with approval and credential context attached.
