---
name: postgres-report-builder
description: Use when tasks require postgres report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Report Builder
...
| Primary artifact | `postgres-report-bundle` |
| Routing tag | `postgres:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from postgres activity..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-report-bundle | structured-artifact | yes | next workflow or operator |
| postgres-report-bundle-scorecard | scorecard | yes | reviewer |
| postgres-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Postgres Report Builder, then confirm which Postgres entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from postgres activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `postgres-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postgres-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:report-builder` consumers with approval and credential context attached.
