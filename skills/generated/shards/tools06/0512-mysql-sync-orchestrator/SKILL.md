---
name: mysql-sync-orchestrator
description: Use when tasks require mysql sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MySQL Sync Orchestrator
...
| Primary artifact | `mysql-reconciliation-bundle` |
| Routing tag | `mysql:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for mysql records..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| mysql-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| mysql-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for MySQL Sync Orchestrator, then checkpoint the last known good cursor on both sides of MySQL.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for mysql records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `mysql-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mysql-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:sync-orchestrator` consumers with approval and credential context attached.
