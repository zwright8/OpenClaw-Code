---
name: snowflake-migration-assistant
description: Use when tasks require snowflake migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `snowflake-migration-cutover-plan` |
| Routing tag | `snowflake:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when state changes are hard to undo once they begin. This specific skill turns Snowflake Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from snowflake..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| snowflake-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| snowflake-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Snowflake Migration Assistant before any Snowflake rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from snowflake. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `snowflake-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:migration-assistant` consumers with approval and credential context attached.
