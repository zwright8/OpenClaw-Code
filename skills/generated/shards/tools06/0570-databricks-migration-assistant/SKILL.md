---
name: databricks-migration-assistant
description: Use when tasks require databricks migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Migration Assistant
...
| Primary artifact | `databricks-migration-cutover-plan` |
| Routing tag | `databricks:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when state changes are hard to undo once they begin. This specific skill turns Databricks Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from databricks..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| databricks-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| databricks-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Databricks Migration Assistant before any Databricks rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from databricks. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `databricks-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] databricks-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:migration-assistant` consumers with approval and credential context attached.
