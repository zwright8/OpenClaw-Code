---
name: mysql-migration-assistant
description: Use when tasks require mysql migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-migration-cutover-plan` |
| Routing tag | `mysql:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when state changes are hard to undo once they begin. This specific skill turns MySQL Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from mysql..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| mysql-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| mysql-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for MySQL Migration Assistant before any MySQL rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from mysql. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `mysql-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] mysql-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:migration-assistant` consumers with approval and credential context attached.
