---
name: bigquery-data-importer
description: Use when tasks require bigquery data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bigquery-import-ledger` |
| Routing tag | `bigquery:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when state changes are hard to undo once they begin. This specific skill turns BigQuery Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into bigquery..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-import-ledger | structured-artifact | yes | next workflow or operator |
| bigquery-import-ledger-scorecard | scorecard | yes | reviewer |
| bigquery-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for BigQuery Data Importer before any BigQuery write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into bigquery. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `bigquery-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:data-importer` consumers with approval and credential context attached.
