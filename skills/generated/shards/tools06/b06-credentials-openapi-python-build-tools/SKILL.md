---
name: b06-credentials-openapi-python-build-tools
description: Use when rotating credentials, governing API contracts, managing Python environments, and operating build-tool ecosystems.
---

# Credential flows, OpenAPI governance, Python packaging, and build tooling

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-credentials-openapi-python-build-tools` |
| Tool count | `25` |
| Inventory rank span | `2852-2878` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for credential flows, openapi governance, python packaging, build tooling tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `usbredir` | `jump` | no | no |
| bounded mutation / transformation | `jump` | `usbredir` | yes | yes |
| validation & artifact checks | `usbredir` | `jump` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `usbredir` | USB traffic redirection library | local-installable |
| `jump` | Helps you navigate your file system faster by learning your habits | local-installable |
| `mailsy` | Quickly generate a temporary email address | local-installable |
| `knock` | Port-knock server | local-installable |
| `git-credential-libsecret` | Git helper for accessing credentials via libsecret | local-installable |
| `libsmi` | Library to Access SMI MIB Information | local-installable |
| `opencc` | Simplified-traditional Chinese conversion tool | local-installable |
| `mpc` | Command-line music player client for mpd | local-installable |
| `storj-uplink` | Uplink CLI for the Storj network | local-installable |
| `faudio` | Accuracy-focused XAudio reimplementation for open platforms | local-installable |
| `btcli` | Bittensor command-line tool | local-installable |
| `xk6` | Build k6 with extensions | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `usbredir` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `jump` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mailsy` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `knock` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `git-credential-libsecret` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsmi` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `opencc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mpc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `storj-uplink` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `faudio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `btcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xk6` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gpsd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openapi` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lla` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-audit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `linux-headers@5.15` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pgrouting` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bookokrat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mac-cleanup-py` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vlang` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `recode` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `crosstool-ng` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bazel@7` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpst` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `usbredir` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `jump` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `mailsy` (QUIC, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `knock` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `git-credential-libsecret` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libsmi` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a credential flows, openapi governance, python packaging, build tooling request, run dry-run first, then produce a validated artifact bundle.
