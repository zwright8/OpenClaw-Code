---
name: mysql-alert-monitor
description: Use when tasks require mysql alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-alert-state-bundle` |
| Routing tag | `mysql:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Alert Monitor into a deterministic, auth-checked workflow for monitors key mysql signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| mysql-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| mysql-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for MySQL Alert Monitor, then lock the reviewed threshold set for MySQL.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key mysql signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `mysql-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mysql-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:alert-monitor` consumers with approval and credential context attached.
