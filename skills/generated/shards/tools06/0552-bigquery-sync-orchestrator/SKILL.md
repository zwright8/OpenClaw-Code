---
name: bigquery-sync-orchestrator
description: Use when tasks require bigquery sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Sync Orchestrator
...
| Primary artifact | `bigquery-reconciliation-bundle` |
| Routing tag | `bigquery:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for bigquery records..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| bigquery-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| bigquery-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for BigQuery Sync Orchestrator, then checkpoint the last known good cursor on both sides of BigQuery.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for bigquery records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `bigquery-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:sync-orchestrator` consumers with approval and credential context attached.
