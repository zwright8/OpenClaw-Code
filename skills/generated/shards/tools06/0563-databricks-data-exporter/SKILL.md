---
name: databricks-data-exporter
description: Use when tasks require databricks data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Data Exporter
...
| Primary artifact | `databricks-export-package` |
| Routing tag | `databricks:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Data Exporter into a deterministic, auth-checked workflow for exports databricks entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-export-package | structured-artifact | yes | next workflow or operator |
| databricks-export-package-scorecard | scorecard | yes | reviewer |
| databricks-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Databricks Data Exporter, then constrain the export scope for Databricks before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports databricks entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `databricks-export-package` so downstream owners do not need to rediscover context.
...
- [ ] databricks-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:data-exporter` consumers with approval and credential context attached.
