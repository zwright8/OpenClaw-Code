---
name: postgres-webhook-listener
description: Use when tasks require postgres webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Webhook Listener
...
| Primary artifact | `postgres-event-intake-log` |
| Routing tag | `postgres:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Postgres Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes postgres webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `POSTGRES_SERVICE_ACCOUNT_JSON`, `POSTGRES_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `POSTGRES_SESSION`, `POSTGRES_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `POSTGRES_WEBHOOK_SECRET`, `POSTGRES_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `POSTGRES_TOKEN`, `POSTGRES_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| postgres-event-intake-log | structured-artifact | yes | next workflow or operator |
| postgres-event-intake-log-scorecard | scorecard | yes | reviewer |
| postgres-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Postgres Webhook Listener, then register the signing-secret policy for Postgres.
...
3. Implement captures, verifies, and routes postgres webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `postgres-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] postgres-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:webhook-listener` consumers with approval and credential context attached.
