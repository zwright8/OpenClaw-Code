---
name: mongodb-webhook-listener
description: Use when tasks require mongodb webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MongoDB Webhook Listener
...
| Primary artifact | `mongodb-event-intake-log` |
| Routing tag | `mongodb:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns MongoDB Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes mongodb webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `MONGODB_SERVICE_ACCOUNT_JSON`, `MONGODB_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `MONGODB_SESSION`, `MONGODB_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `MONGODB_WEBHOOK_SECRET`, `MONGODB_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `MONGODB_TOKEN`, `MONGODB_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| mongodb-event-intake-log | structured-artifact | yes | next workflow or operator |
| mongodb-event-intake-log-scorecard | scorecard | yes | reviewer |
| mongodb-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for MongoDB Webhook Listener, then register the signing-secret policy for MongoDB.
...
3. Implement captures, verifies, and routes mongodb webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `mongodb-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:webhook-listener` consumers with approval and credential context attached.
