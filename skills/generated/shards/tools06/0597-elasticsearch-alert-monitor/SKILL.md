---
name: elasticsearch-alert-monitor
description: Use when tasks require elasticsearch alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `elasticsearch-alert-state-bundle` |
| Routing tag | `elasticsearch:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Alert Monitor into a deterministic, auth-checked workflow for monitors key elasticsearch signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| elasticsearch-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| elasticsearch-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Elasticsearch Alert Monitor, then lock the reviewed threshold set for Elasticsearch.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key elasticsearch signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `elasticsearch-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:alert-monitor` consumers with approval and credential context attached.
