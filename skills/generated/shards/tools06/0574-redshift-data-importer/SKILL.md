---
name: redshift-data-importer
description: Use when tasks require redshift data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Data Importer
...
| Primary artifact | `redshift-import-ledger` |
| Routing tag | `redshift:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when state changes are hard to undo once they begin. This specific skill turns Redshift Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into redshift..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-import-ledger | structured-artifact | yes | next workflow or operator |
| redshift-import-ledger-scorecard | scorecard | yes | reviewer |
| redshift-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Redshift Data Importer before any Redshift write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into redshift. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `redshift-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] redshift-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:data-importer` consumers with approval and credential context attached.
