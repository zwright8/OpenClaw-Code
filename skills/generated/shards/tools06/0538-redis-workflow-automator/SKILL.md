---
name: redis-workflow-automator
description: Use when tasks require redis workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Workflow Automator
...
| Primary artifact | `redis-workflow-runbook` |
| Routing tag | `redis:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using redis..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-workflow-runbook | structured-artifact | yes | next workflow or operator |
| redis-workflow-runbook-scorecard | scorecard | yes | reviewer |
| redis-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Redis Workflow Automator, then checkpoint where Redis state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using redis. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `redis-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] redis-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:workflow-automator` consumers with approval and credential context attached.
