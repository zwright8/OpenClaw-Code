---
name: snowflake-alert-monitor
description: Use when tasks require snowflake alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Alert Monitor
...
| Primary artifact | `snowflake-alert-state-bundle` |
| Routing tag | `snowflake:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Alert Monitor into a deterministic, auth-checked workflow for monitors key snowflake signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| snowflake-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| snowflake-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Snowflake Alert Monitor, then lock the reviewed threshold set for Snowflake.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key snowflake signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `snowflake-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:alert-monitor` consumers with approval and credential context attached.
