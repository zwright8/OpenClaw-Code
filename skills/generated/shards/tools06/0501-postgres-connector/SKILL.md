...
description: Use when tasks require postgres connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `postgres-connection-trace-bundle` |
| Routing tag | `postgres:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Connector into a deterministic, auth-checked workflow for unified read/write connector for postgres apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| postgres-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| postgres-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Postgres Connector, then confirm the smallest read-only probe that proves access to Postgres.
...
3. Specify the request and response contract for unified read/write connector for postgres apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `postgres-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postgres-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:connector` consumers with approval and credential context attached.
