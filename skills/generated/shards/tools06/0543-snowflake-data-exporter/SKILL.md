...
description: Use when tasks require snowflake data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `snowflake-export-package` |
| Routing tag | `snowflake:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Data Exporter into a deterministic, auth-checked workflow for exports snowflake entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-export-package | structured-artifact | yes | next workflow or operator |
| snowflake-export-package-scorecard | scorecard | yes | reviewer |
| snowflake-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Snowflake Data Exporter, then constrain the export scope for Snowflake before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports snowflake entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `snowflake-export-package` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:data-exporter` consumers with approval and credential context attached.
