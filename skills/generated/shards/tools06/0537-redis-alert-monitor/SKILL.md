---
name: redis-alert-monitor
description: Use when tasks require redis alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `redis-alert-state-bundle` |
| Routing tag | `redis:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Alert Monitor into a deterministic, auth-checked workflow for monitors key redis signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| redis-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| redis-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Redis Alert Monitor, then lock the reviewed threshold set for Redis.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key redis signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `redis-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redis-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:alert-monitor` consumers with approval and credential context attached.
