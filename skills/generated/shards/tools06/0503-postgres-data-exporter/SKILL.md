---
name: postgres-data-exporter
description: Use when tasks require postgres data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `postgres-export-package` |
| Routing tag | `postgres:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Data Exporter into a deterministic, auth-checked workflow for exports postgres entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-export-package | structured-artifact | yes | next workflow or operator |
| postgres-export-package-scorecard | scorecard | yes | reviewer |
| postgres-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Postgres Data Exporter, then constrain the export scope for Postgres before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports postgres entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `postgres-export-package` so downstream owners do not need to rediscover context.
...
- [ ] postgres-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:data-exporter` consumers with approval and credential context attached.
