---
name: snowflake-approval-router
description: Use when tasks require snowflake approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Approval Router
...
| Primary artifact | `snowflake-approval-decision-packet` |
| Routing tag | `snowflake:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Approval Router into a deterministic, auth-checked workflow for routes snowflake-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| snowflake-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| snowflake-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Snowflake Approval Router, then capture the manual fallback path for Snowflake.
...
3. Implement routes snowflake-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `snowflake-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:approval-router` consumers with approval and credential context attached.
