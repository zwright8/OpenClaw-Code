...
description: Use when tasks require redshift connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `redshift-connection-trace-bundle` |
| Routing tag | `redshift:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Connector into a deterministic, auth-checked workflow for unified read/write connector for redshift apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| redshift-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| redshift-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Redshift Connector, then confirm the smallest read-only probe that proves access to Redshift.
...
3. Specify the request and response contract for unified read/write connector for redshift apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `redshift-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redshift-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:connector` consumers with approval and credential context attached.
