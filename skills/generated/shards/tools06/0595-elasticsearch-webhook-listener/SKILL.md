---
name: elasticsearch-webhook-listener
description: Use when tasks require elasticsearch webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Webhook Listener
...
| Primary artifact | `elasticsearch-event-intake-log` |
| Routing tag | `elasticsearch:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Elasticsearch Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes elasticsearch webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `ELASTICSEARCH_SERVICE_ACCOUNT_JSON`, `ELASTICSEARCH_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `ELASTICSEARCH_SESSION`, `ELASTICSEARCH_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `ELASTICSEARCH_WEBHOOK_SECRET`, `ELASTICSEARCH_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `ELASTICSEARCH_TOKEN`, `ELASTICSEARCH_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| elasticsearch-event-intake-log | structured-artifact | yes | next workflow or operator |
| elasticsearch-event-intake-log-scorecard | scorecard | yes | reviewer |
| elasticsearch-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Elasticsearch Webhook Listener, then register the signing-secret policy for Elasticsearch.
...
3. Implement captures, verifies, and routes elasticsearch webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `elasticsearch-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:webhook-listener` consumers with approval and credential context attached.
