---
name: mongodb-data-exporter
description: Use when tasks require mongodb data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-export-package` |
| Routing tag | `mongodb:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Data Exporter into a deterministic, auth-checked workflow for exports mongodb entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-export-package | structured-artifact | yes | next workflow or operator |
| mongodb-export-package-scorecard | scorecard | yes | reviewer |
| mongodb-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for MongoDB Data Exporter, then constrain the export scope for MongoDB before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports mongodb entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `mongodb-export-package` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:data-exporter` consumers with approval and credential context attached.
