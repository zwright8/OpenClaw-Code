---
name: b06-ci-build-firmware-media-ops
description: Use when working on container builds, commit hygiene, firmware/update tooling, and media tagging/conversion operations in one delivery stream.
---

# CI/build, firmware, and media operations

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-ci-build-firmware-media-ops` |
| Tool count | `26` |
| Inventory rank span | `2536-2567` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for ci/build, firmware, media operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `icdiff` | `percona-xtrabackup` | no | no |
| bounded mutation / transformation | `percona-xtrabackup` | `icdiff` | yes | yes |
| validation & artifact checks | `icdiff` | `percona-xtrabackup` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `icdiff` | Improved colored diff | local-installable |
| `percona-xtrabackup` | Open source hot backup tool for InnoDB and XtraDB databases | local-installable |
| `shellspec` | BDD unit testing framework for dash, bash, ksh, zsh and all POSIX shells | local-installable |
| `babel` | Compiler for writing next generation JavaScript | local-installable |
| `mapserver` | Publish spatial data and interactive mapping apps to the web | local-installable |
| `premake` | Write once, build anywhere Lua-based build system | local-installable |
| `boost-build` | C++ build system | local-installable |
| `vsftpd` | Secure FTP server for UNIX | local-installable |
| `commitlint` | Lint commit messages according to a commit convention | local-installable |
| `nixpacks` | App source + Nix packages + Docker = Image | local-installable |
| `monero` | Official Monero wallet and CPU miner | local-installable |
| `dockerfile-language-server` | Language server for Dockerfiles powered by Node, TypeScript, and VSCode | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `icdiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `percona-xtrabackup` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shellspec` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `babel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mapserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `premake` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `boost-build` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vsftpd` | SFTP/FTP/WebDAV, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `commitlint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nixpacks` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `monero` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dockerfile-language-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fwupd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mist-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gonzo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `air` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `id3v2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cpprestsdk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xdotool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tsduck` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `heimdal` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fileicon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aubio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lazyjournal` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `u-boot-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdfpc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `lazyjournal`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `nixpacks`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `icdiff` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `percona-xtrabackup` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `shellspec` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `babel` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `mapserver` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `premake` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a ci/build, firmware, media operations request, run dry-run first, then produce a validated artifact bundle.
