---
name: clickhouse-approval-router
description: Use when tasks require clickhouse approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Approval Router
...
| Primary artifact | `clickhouse-approval-decision-packet` |
| Routing tag | `clickhouse:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Approval Router into a deterministic, auth-checked workflow for routes clickhouse-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| clickhouse-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| clickhouse-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for ClickHouse Approval Router, then capture the manual fallback path for ClickHouse.
...
3. Implement routes clickhouse-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `clickhouse-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:approval-router` consumers with approval and credential context attached.
