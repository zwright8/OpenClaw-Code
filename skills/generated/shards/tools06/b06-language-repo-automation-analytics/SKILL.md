---
name: b06-language-repo-automation-analytics
description: Use when maintaining language toolchains, repository scale checks, background automation, and analytics/visualization pipelines.
---

# Language ecosystems, repo automation, and analytics workflows

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-language-repo-automation-analytics` |
| Tool count | `25` |
| Inventory rank span | `2972-3000` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for language ecosystems, repo automation, analytics workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `haxe` | `libcerf` | no | no |
| bounded mutation / transformation | `libcerf` | `haxe` | yes | yes |
| validation & artifact checks | `haxe` | `libcerf` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `haxe` | Multi-platform programming language | local-installable |
| `libcerf` | Numeric library for complex error functions | local-installable |
| `kotlin-language-server` | Intelligent Kotlin support for any editor/IDE using the Language Server Protocol | local-installable |
| `pscale` | CLI for PlanetScale Database | local-installable |
| `minipro` | Open controller for the MiniPRO TL866xx series of chip programmers | local-installable |
| `dotslash` | Simplified executable deployment | local-installable |
| `man-db` | Unix documentation system | local-installable |
| `astroterm` | Planetarium for your terminal | local-installable |
| `bandcamp-dl` | Simple python script to download Bandcamp albums | local-installable |
| `ruby@3.4` | Powerful, clean, object-oriented scripting language | local-installable |
| `git-sizer` | Compute various size metrics for a Git repository | local-installable |
| `rdfind` | Find duplicate files based on content (NOT file names) | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `haxe` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcerf` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kotlin-language-server` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pscale` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `minipro` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotslash` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `man-db` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `astroterm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bandcamp-dl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ruby@3.4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `git-sizer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rdfind` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chainloop-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pueue` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openfga` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `freeipmi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kaskade` | Kafka protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `igraph` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libvterm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `glade` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `offlineimap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsolv` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `matplotplusplus` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `bbtools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dateutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `haxe` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `libcerf` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `kotlin-language-server` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `pscale` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `minipro` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `dotslash` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a language ecosystems, repo automation, analytics workflows request, run dry-run first, then produce a validated artifact bundle.
