---
name: mongodb-approval-router
description: Use when tasks require mongodb approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MongoDB Approval Router
...
| Primary artifact | `mongodb-approval-decision-packet` |
| Routing tag | `mongodb:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Approval Router into a deterministic, auth-checked workflow for routes mongodb-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| mongodb-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| mongodb-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for MongoDB Approval Router, then capture the manual fallback path for MongoDB.
...
3. Implement routes mongodb-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `mongodb-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:approval-router` consumers with approval and credential context attached.
