---
name: redis-webhook-listener
description: Use when tasks require redis webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Webhook Listener
...
| Primary artifact | `redis-event-intake-log` |
| Routing tag | `redis:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Redis Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes redis webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `REDIS_CLIENT_ID`, `REDIS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `REDIS_TOKEN`, `REDIS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `REDIS_WEBHOOK_SECRET`, `REDIS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| redis-event-intake-log | structured-artifact | yes | next workflow or operator |
| redis-event-intake-log-scorecard | scorecard | yes | reviewer |
| redis-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Redis Webhook Listener, then register the signing-secret policy for Redis.
...
3. Implement captures, verifies, and routes redis webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `redis-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] redis-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:webhook-listener` consumers with approval and credential context attached.
