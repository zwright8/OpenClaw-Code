---
name: clickhouse-data-exporter
description: Use when tasks require clickhouse data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Data Exporter
...
| Primary artifact | `clickhouse-export-package` |
| Routing tag | `clickhouse:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Data Exporter into a deterministic, auth-checked workflow for exports clickhouse entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-export-package | structured-artifact | yes | next workflow or operator |
| clickhouse-export-package-scorecard | scorecard | yes | reviewer |
| clickhouse-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for ClickHouse Data Exporter, then constrain the export scope for ClickHouse before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports clickhouse entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `clickhouse-export-package` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:data-exporter` consumers with approval and credential context attached.
