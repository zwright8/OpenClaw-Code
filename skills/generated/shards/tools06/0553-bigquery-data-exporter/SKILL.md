...
description: Use when tasks require bigquery data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Data Exporter
...
| Primary artifact | `bigquery-export-package` |
| Routing tag | `bigquery:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Data Exporter into a deterministic, auth-checked workflow for exports bigquery entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-export-package | structured-artifact | yes | next workflow or operator |
| bigquery-export-package-scorecard | scorecard | yes | reviewer |
| bigquery-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for BigQuery Data Exporter, then constrain the export scope for BigQuery before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports bigquery entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `bigquery-export-package` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:data-exporter` consumers with approval and credential context attached.
