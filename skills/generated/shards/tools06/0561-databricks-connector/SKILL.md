...
description: Use when tasks require databricks connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `databricks-connection-trace-bundle` |
| Routing tag | `databricks:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Connector into a deterministic, auth-checked workflow for unified read/write connector for databricks apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| databricks-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| databricks-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Databricks Connector, then confirm the smallest read-only probe that proves access to Databricks.
...
3. Specify the request and response contract for unified read/write connector for databricks apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `databricks-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] databricks-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:connector` consumers with approval and credential context attached.
