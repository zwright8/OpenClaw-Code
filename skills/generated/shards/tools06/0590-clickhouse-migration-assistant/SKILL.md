---
name: clickhouse-migration-assistant
description: Use when tasks require clickhouse migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Migration Assistant
...
| Primary artifact | `clickhouse-migration-cutover-plan` |
| Routing tag | `clickhouse:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when state changes are hard to undo once they begin. This specific skill turns ClickHouse Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from clickhouse..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| clickhouse-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| clickhouse-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for ClickHouse Migration Assistant before any ClickHouse rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from clickhouse. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `clickhouse-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:migration-assistant` consumers with approval and credential context attached.
