---
name: b06-partition-radio-testing-data
description: Use when handling low-level storage/radio tasks, test automation harnesses, and structured data or content-processing flows.
---

# Partitioning, radio, test automation, and data ops

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-partition-radio-testing-data` |
| Tool count | `26` |
| Inventory rank span | `2762-2789` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for partitioning, radio, test automation, data ops tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `byacc` | `gptfdisk` | no | no |
| bounded mutation / transformation | `gptfdisk` | `byacc` | yes | yes |
| validation & artifact checks | `byacc` | `gptfdisk` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `byacc` | (Arguably) the best yacc variant | local-installable |
| `gptfdisk` | Text-mode partitioning tools | local-installable |
| `urh` | Universal Radio Hacker | local-installable |
| `gauge` | Test automation tool that supports executable documentation | local-installable |
| `spark` | Sparklines for the shell | local-installable |
| `libatomic_ops` | Implementations for atomic memory update operations | local-installable |
| `speexdsp` | Speex audio processing library | local-installable |
| `mockserver` | Mock HTTP server and proxy | local-installable |
| `neovim-remote` | Control nvim processes using `nvr` command-line tool | local-installable |
| `glibmm@2.66` | C++ interface to glib | local-installable |
| `deck` | Creates slide deck using Markdown and Google Slides | local-installable |
| `enzyme` | High-performance automatic differentiation of LLVM | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `byacc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gptfdisk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `urh` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gauge` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spark` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libatomic_ops` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `speexdsp` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `mockserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `neovim-remote` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `glibmm@2.66` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `deck` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `enzyme` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `choose-rust` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pyupgrade` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `recon-ng` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xsv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xplr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `basex` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fakeroot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xcode-kotlin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-google-auth` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `shadowsocks-libev` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dfmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `couchdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `faircamp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ov` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-google-auth`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `byacc` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `gptfdisk` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `urh` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `gauge` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `spark` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `libatomic_ops` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a partitioning, radio, test automation, data ops request, run dry-run first, then produce a validated artifact bundle.
