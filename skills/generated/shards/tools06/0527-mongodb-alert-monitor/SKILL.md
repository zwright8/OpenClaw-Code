---
name: mongodb-alert-monitor
description: Use when tasks require mongodb alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MongoDB Alert Monitor
...
| Primary artifact | `mongodb-alert-state-bundle` |
| Routing tag | `mongodb:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Alert Monitor into a deterministic, auth-checked workflow for monitors key mongodb signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| mongodb-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| mongodb-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for MongoDB Alert Monitor, then lock the reviewed threshold set for MongoDB.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key mongodb signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `mongodb-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:alert-monitor` consumers with approval and credential context attached.
