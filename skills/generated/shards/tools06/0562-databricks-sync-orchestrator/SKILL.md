---
name: databricks-sync-orchestrator
description: Use when tasks require databricks sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Sync Orchestrator
...
| Primary artifact | `databricks-reconciliation-bundle` |
| Routing tag | `databricks:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for databricks records..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| databricks-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| databricks-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Databricks Sync Orchestrator, then checkpoint the last known good cursor on both sides of Databricks.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for databricks records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `databricks-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] databricks-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:sync-orchestrator` consumers with approval and credential context attached.
