---
name: mongodb-workflow-automator
description: Use when tasks require mongodb workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MongoDB Workflow Automator
...
| Primary artifact | `mongodb-workflow-runbook` |
| Routing tag | `mongodb:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using mongodb..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-workflow-runbook | structured-artifact | yes | next workflow or operator |
| mongodb-workflow-runbook-scorecard | scorecard | yes | reviewer |
| mongodb-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for MongoDB Workflow Automator, then checkpoint where MongoDB state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using mongodb. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `mongodb-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:workflow-automator` consumers with approval and credential context attached.
