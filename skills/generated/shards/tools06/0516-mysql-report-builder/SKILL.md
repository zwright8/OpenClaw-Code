---
name: mysql-report-builder
description: Use when tasks require mysql report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-report-bundle` |
| Routing tag | `mysql:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from mysql activity..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-report-bundle | structured-artifact | yes | next workflow or operator |
| mysql-report-bundle-scorecard | scorecard | yes | reviewer |
| mysql-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for MySQL Report Builder, then confirm which MySQL entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from mysql activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `mysql-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mysql-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:report-builder` consumers with approval and credential context attached.
