...
description: Use when tasks require redis connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `redis-connection-trace-bundle` |
| Routing tag | `redis:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Connector into a deterministic, auth-checked workflow for unified read/write connector for redis apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| redis-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| redis-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Redis Connector, then confirm the smallest read-only probe that proves access to Redis.
...
3. Specify the request and response contract for unified read/write connector for redis apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `redis-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redis-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:connector` consumers with approval and credential context attached.
