---
name: redshift-webhook-listener
description: Use when tasks require redshift webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Webhook Listener
...
| Primary artifact | `redshift-event-intake-log` |
| Routing tag | `redshift:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Redshift Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes redshift webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `REDSHIFT_SERVICE_ACCOUNT_JSON`, `REDSHIFT_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `REDSHIFT_SESSION`, `REDSHIFT_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `REDSHIFT_WEBHOOK_SECRET`, `REDSHIFT_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `REDSHIFT_TOKEN`, `REDSHIFT_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| redshift-event-intake-log | structured-artifact | yes | next workflow or operator |
| redshift-event-intake-log-scorecard | scorecard | yes | reviewer |
| redshift-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Redshift Webhook Listener, then register the signing-secret policy for Redshift.
...
3. Implement captures, verifies, and routes redshift webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `redshift-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] redshift-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:webhook-listener` consumers with approval and credential context attached.
