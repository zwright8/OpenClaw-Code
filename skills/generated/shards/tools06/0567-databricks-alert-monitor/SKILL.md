---
name: databricks-alert-monitor
description: Use when tasks require databricks alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Alert Monitor
...
| Primary artifact | `databricks-alert-state-bundle` |
| Routing tag | `databricks:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Alert Monitor into a deterministic, auth-checked workflow for monitors key databricks signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| databricks-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| databricks-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Databricks Alert Monitor, then lock the reviewed threshold set for Databricks.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key databricks signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `databricks-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] databricks-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:alert-monitor` consumers with approval and credential context attached.
