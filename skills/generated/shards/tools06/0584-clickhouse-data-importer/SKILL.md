---
name: clickhouse-data-importer
description: Use when tasks require clickhouse data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Data Importer
...
| Primary artifact | `clickhouse-import-ledger` |
| Routing tag | `clickhouse:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when state changes are hard to undo once they begin. This specific skill turns ClickHouse Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into clickhouse..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-import-ledger | structured-artifact | yes | next workflow or operator |
| clickhouse-import-ledger-scorecard | scorecard | yes | reviewer |
| clickhouse-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for ClickHouse Data Importer before any ClickHouse write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into clickhouse. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `clickhouse-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:data-importer` consumers with approval and credential context attached.
