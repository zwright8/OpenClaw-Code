---
name: redshift-alert-monitor
description: Use when tasks require redshift alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Alert Monitor
...
| Primary artifact | `redshift-alert-state-bundle` |
| Routing tag | `redshift:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Alert Monitor into a deterministic, auth-checked workflow for monitors key redshift signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| redshift-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| redshift-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Redshift Alert Monitor, then lock the reviewed threshold set for Redshift.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key redshift signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `redshift-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redshift-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:alert-monitor` consumers with approval and credential context attached.
