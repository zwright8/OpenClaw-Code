---
name: clickhouse-sync-orchestrator
description: Use when tasks require clickhouse sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Sync Orchestrator
...
| Primary artifact | `clickhouse-reconciliation-bundle` |
| Routing tag | `clickhouse:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for clickhouse records..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| clickhouse-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| clickhouse-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for ClickHouse Sync Orchestrator, then checkpoint the last known good cursor on both sides of ClickHouse.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for clickhouse records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `clickhouse-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:sync-orchestrator` consumers with approval and credential context attached.
