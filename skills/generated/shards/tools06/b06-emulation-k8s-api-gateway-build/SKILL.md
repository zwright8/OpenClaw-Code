...
| Field | Value |
|---|---|
| Skill pack | `b06-emulation-k8s-api-gateway-build` |
| Tool count | `25` |
| Inventory rank span | `2790-2817` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for emulation, kubernetes hygiene, api gateway, build systems tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `fceux` | `lmdb` | no | no |
| bounded mutation / transformation | `lmdb` | `fceux` | yes | yes |
| validation & artifact checks | `fceux` | `lmdb` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `fceux` | All-in-one NES/Famicom Emulator | local-installable |
| `lmdb` | Lightning memory-mapped database: key-value data store | local-installable |
| `openmotif` | LGPL release of the Motif toolkit | local-installable |
| `synfig` | Command-line renderer | local-installable |
| `kor` | CLI tool to discover unused Kubernetes resources | local-installable |
| `stellar-cli` | Stellar command-line tool for interacting with the Stellar network | local-installable |
| `tex-fmt` | Extremely fast LaTeX formatter written in Rust | local-installable |
| `mago` | Toolchain for PHP to help developers write better code | local-installable |
| `treemd` | TUI and CLI dual pane markdown viewer | local-installable |
| `rabbitmq-c` | C AMQP client library for RabbitMQ | local-installable |
| `latexdiff` | Compare and mark up LaTeX file differences | local-installable |
| `openfst` | Library for weighted finite-state transducers | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `fceux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lmdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openmotif` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `synfig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kor` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `stellar-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tex-fmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mago` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `treemd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rabbitmq-c` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `latexdiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openfst` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `bmon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fheroes2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `go@1.25` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `freeswitch` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dosbox-staging` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libiscsi` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `bash-git-prompt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scalapack` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libtatsu` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pmtiles` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `localtunnel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bazarr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fpc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kor`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `fceux` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `lmdb` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `openmotif` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `synfig` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `kor` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `stellar-cli` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a emulation, kubernetes hygiene, api gateway, build systems request, run dry-run first, then produce a validated artifact bundle.
