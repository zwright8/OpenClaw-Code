...
description: Use when tasks require bigquery migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Migration Assistant
...
| Primary artifact | `bigquery-migration-cutover-plan` |
| Routing tag | `bigquery:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when state changes are hard to undo once they begin. This specific skill turns BigQuery Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from bigquery..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| bigquery-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| bigquery-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for BigQuery Migration Assistant before any BigQuery rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from bigquery. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `bigquery-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:migration-assistant` consumers with approval and credential context attached.
