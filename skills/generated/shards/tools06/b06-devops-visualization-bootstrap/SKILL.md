---
name: b06-devops-visualization-bootstrap
description: Use when bootstrapping mixed development environments across virtualization, image/search tooling, systems diagnostics, and early-stage platform setup tasks.
---

# DevOps, visualization, and platform bootstrap workflows

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-devops-visualization-bootstrap` |
| Tool count | `26` |
| Inventory rank span | `2501-2534` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for devops, visualization, platform bootstrap workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `serie` | `python-freethreading` | no | no |
| bounded mutation / transformation | `python-freethreading` | `serie` | yes | yes |
| validation & artifact checks | `serie` | `python-freethreading` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `serie` | Rich git commit graph in your terminal | local-installable |
| `python-freethreading` | Interpreted, interactive, object-oriented programming language | local-installable |
| `c3c` | Compiler for the C3 language | local-installable |
| `augeas` | Configuration editing tool and API | local-installable |
| `moon` | Task runner and repo management tool for the web ecosystem, written in Rust | local-installable |
| `xmrig` | Monero (XMR) CPU miner | local-installable |
| `simdutf` | Unicode conversion routines, fast | local-installable |
| `sysdig` | System-level exploration and troubleshooting tool | local-installable |
| `prettyping` | Wrapper to colorize and simplify ping's output | local-installable |
| `tag` | Manipulate and query tags on macOS files | local-installable |
| `code2prompt` | CLI tool to convert your codebase into a single LLM prompt | local-installable |
| `solr` | Enterprise search platform from the Apache Lucene project | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `serie` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `python-freethreading` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c3c` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `augeas` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `moon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xmrig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `simdutf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sysdig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `prettyping` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tag` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `code2prompt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `solr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ldc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `f3d` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxcb` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dockerize` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `imagemagick-full` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lume` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `findent` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scikit-image` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spice-gtk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsixel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libewf` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `grafana-alloy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nload` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sratoolkit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `dockerize`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `serie` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `python-freethreading` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `c3c` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `augeas` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `moon` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `xmrig` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a devops, visualization, platform bootstrap workflows request, run dry-run first, then produce a validated artifact bundle.
