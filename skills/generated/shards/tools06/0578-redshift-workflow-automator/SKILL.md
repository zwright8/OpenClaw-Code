---
name: redshift-workflow-automator
description: Use when tasks require redshift workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Workflow Automator
...
| Primary artifact | `redshift-workflow-runbook` |
| Routing tag | `redshift:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using redshift..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-workflow-runbook | structured-artifact | yes | next workflow or operator |
| redshift-workflow-runbook-scorecard | scorecard | yes | reviewer |
| redshift-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Redshift Workflow Automator, then checkpoint where Redshift state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using redshift. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `redshift-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] redshift-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:workflow-automator` consumers with approval and credential context attached.
