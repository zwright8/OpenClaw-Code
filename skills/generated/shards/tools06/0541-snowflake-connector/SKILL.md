...
description: Use when tasks require snowflake connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `snowflake-connection-trace-bundle` |
| Routing tag | `snowflake:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Connector into a deterministic, auth-checked workflow for unified read/write connector for snowflake apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| snowflake-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| snowflake-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Snowflake Connector, then confirm the smallest read-only probe that proves access to Snowflake.
...
3. Specify the request and response contract for unified read/write connector for snowflake apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `snowflake-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:connector` consumers with approval and credential context attached.
