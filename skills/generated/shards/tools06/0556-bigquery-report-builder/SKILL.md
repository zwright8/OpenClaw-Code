---
name: bigquery-report-builder
description: Use when tasks require bigquery report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bigquery-report-bundle` |
| Routing tag | `bigquery:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from bigquery activity..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-report-bundle | structured-artifact | yes | next workflow or operator |
| bigquery-report-bundle-scorecard | scorecard | yes | reviewer |
| bigquery-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for BigQuery Report Builder, then confirm which BigQuery entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from bigquery activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `bigquery-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:report-builder` consumers with approval and credential context attached.
