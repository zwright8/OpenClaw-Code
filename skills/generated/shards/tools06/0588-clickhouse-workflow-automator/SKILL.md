---
name: clickhouse-workflow-automator
description: Use when tasks require clickhouse workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Workflow Automator
...
| Primary artifact | `clickhouse-workflow-runbook` |
| Routing tag | `clickhouse:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using clickhouse..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-workflow-runbook | structured-artifact | yes | next workflow or operator |
| clickhouse-workflow-runbook-scorecard | scorecard | yes | reviewer |
| clickhouse-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for ClickHouse Workflow Automator, then checkpoint where ClickHouse state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using clickhouse. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `clickhouse-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:workflow-automator` consumers with approval and credential context attached.
