---
name: postgres-approval-router
description: Use when tasks require postgres approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Approval Router
...
| Primary artifact | `postgres-approval-decision-packet` |
| Routing tag | `postgres:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Approval Router into a deterministic, auth-checked workflow for routes postgres-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| postgres-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| postgres-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Postgres Approval Router, then capture the manual fallback path for Postgres.
...
3. Implement routes postgres-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `postgres-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] postgres-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:approval-router` consumers with approval and credential context attached.
