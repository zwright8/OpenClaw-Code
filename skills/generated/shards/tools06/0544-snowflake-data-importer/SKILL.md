---
name: snowflake-data-importer
description: Use when tasks require snowflake data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Data Importer
...
| Primary artifact | `snowflake-import-ledger` |
| Routing tag | `snowflake:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when state changes are hard to undo once they begin. This specific skill turns Snowflake Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into snowflake..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-import-ledger | structured-artifact | yes | next workflow or operator |
| snowflake-import-ledger-scorecard | scorecard | yes | reviewer |
| snowflake-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Snowflake Data Importer before any Snowflake write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into snowflake. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `snowflake-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:data-importer` consumers with approval and credential context attached.
