---
name: postgres-sync-orchestrator
description: Use when tasks require postgres sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Sync Orchestrator
...
| Primary artifact | `postgres-reconciliation-bundle` |
| Routing tag | `postgres:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for postgres records..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| postgres-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| postgres-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Postgres Sync Orchestrator, then checkpoint the last known good cursor on both sides of Postgres.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for postgres records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `postgres-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postgres-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:sync-orchestrator` consumers with approval and credential context attached.
