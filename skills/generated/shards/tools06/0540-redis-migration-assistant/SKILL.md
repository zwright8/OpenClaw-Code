---
name: redis-migration-assistant
description: Use when tasks require redis migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Migration Assistant
...
| Primary artifact | `redis-migration-cutover-plan` |
| Routing tag | `redis:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when state changes are hard to undo once they begin. This specific skill turns Redis Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from redis..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| redis-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| redis-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Redis Migration Assistant before any Redis rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from redis. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `redis-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] redis-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:migration-assistant` consumers with approval and credential context attached.
