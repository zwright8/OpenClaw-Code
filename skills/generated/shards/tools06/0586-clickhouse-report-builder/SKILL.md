---
name: clickhouse-report-builder
description: Use when tasks require clickhouse report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# ClickHouse Report Builder
...
| Primary artifact | `clickhouse-report-bundle` |
| Routing tag | `clickhouse:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because ClickHouse workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns ClickHouse Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from clickhouse activity..

## Trigger Checklist
- [ ] The task explicitly requires `ClickHouse Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| clickhouse-report-bundle | structured-artifact | yes | next workflow or operator |
| clickhouse-report-bundle-scorecard | scorecard | yes | reviewer |
| clickhouse-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for ClickHouse Report Builder, then confirm which ClickHouse entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from clickhouse activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `clickhouse-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] clickhouse-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `clickhouse-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `clickhouse:report-builder` consumers with approval and credential context attached.
