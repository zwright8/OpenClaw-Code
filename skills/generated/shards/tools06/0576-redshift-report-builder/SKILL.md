---
name: redshift-report-builder
description: Use when tasks require redshift report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Report Builder
...
| Primary artifact | `redshift-report-bundle` |
| Routing tag | `redshift:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from redshift activity..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-report-bundle | structured-artifact | yes | next workflow or operator |
| redshift-report-bundle-scorecard | scorecard | yes | reviewer |
| redshift-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Redshift Report Builder, then confirm which Redshift entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from redshift activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `redshift-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redshift-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:report-builder` consumers with approval and credential context attached.
