---
name: redis-data-exporter
description: Use when tasks require redis data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `redis-export-package` |
| Routing tag | `redis:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Data Exporter into a deterministic, auth-checked workflow for exports redis entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-export-package | structured-artifact | yes | next workflow or operator |
| redis-export-package-scorecard | scorecard | yes | reviewer |
| redis-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Redis Data Exporter, then constrain the export scope for Redis before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports redis entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `redis-export-package` so downstream owners do not need to rediscover context.
...
- [ ] redis-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:data-exporter` consumers with approval and credential context attached.
