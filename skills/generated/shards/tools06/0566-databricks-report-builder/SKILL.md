---
name: databricks-report-builder
description: Use when tasks require databricks report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Report Builder
...
| Primary artifact | `databricks-report-bundle` |
| Routing tag | `databricks:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from databricks activity..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-report-bundle | structured-artifact | yes | next workflow or operator |
| databricks-report-bundle-scorecard | scorecard | yes | reviewer |
| databricks-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Databricks Report Builder, then confirm which Databricks entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from databricks activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `databricks-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] databricks-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:report-builder` consumers with approval and credential context attached.
