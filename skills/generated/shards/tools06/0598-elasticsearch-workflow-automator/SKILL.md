---
name: elasticsearch-workflow-automator
description: Use when tasks require elasticsearch workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Elasticsearch Workflow Automator
...
| Primary artifact | `elasticsearch-workflow-runbook` |
| Routing tag | `elasticsearch:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using elasticsearch..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-workflow-runbook | structured-artifact | yes | next workflow or operator |
| elasticsearch-workflow-runbook-scorecard | scorecard | yes | reviewer |
| elasticsearch-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Elasticsearch Workflow Automator, then checkpoint where Elasticsearch state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using elasticsearch. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `elasticsearch-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:workflow-automator` consumers with approval and credential context attached.
