---
name: bigquery-alert-monitor
description: Use when tasks require bigquery alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigQuery Alert Monitor
...
| Primary artifact | `bigquery-alert-state-bundle` |
| Routing tag | `bigquery:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigQuery workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigQuery Alert Monitor into a deterministic, auth-checked workflow for monitors key bigquery signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `BigQuery Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigquery-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| bigquery-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| bigquery-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for BigQuery Alert Monitor, then lock the reviewed threshold set for BigQuery.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key bigquery signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `bigquery-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigquery-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigquery-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigquery:alert-monitor` consumers with approval and credential context attached.
