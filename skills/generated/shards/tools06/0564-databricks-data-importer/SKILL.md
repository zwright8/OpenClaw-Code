---
name: databricks-data-importer
description: Use when tasks require databricks data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Data Importer
...
| Primary artifact | `databricks-import-ledger` |
| Routing tag | `databricks:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when state changes are hard to undo once they begin. This specific skill turns Databricks Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into databricks..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-import-ledger | structured-artifact | yes | next workflow or operator |
| databricks-import-ledger-scorecard | scorecard | yes | reviewer |
| databricks-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Databricks Data Importer before any Databricks write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into databricks. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `databricks-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] databricks-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:data-importer` consumers with approval and credential context attached.
