---
name: snowflake-sync-orchestrator
description: Use when tasks require snowflake sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Snowflake Sync Orchestrator
...
| Primary artifact | `snowflake-reconciliation-bundle` |
| Routing tag | `snowflake:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for snowflake records..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| snowflake-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| snowflake-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Snowflake Sync Orchestrator, then checkpoint the last known good cursor on both sides of Snowflake.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for snowflake records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `snowflake-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:sync-orchestrator` consumers with approval and credential context attached.
