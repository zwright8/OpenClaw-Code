---
name: mysql-data-importer
description: Use when tasks require mysql data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-import-ledger` |
| Routing tag | `mysql:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when state changes are hard to undo once they begin. This specific skill turns MySQL Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into mysql..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-import-ledger | structured-artifact | yes | next workflow or operator |
| mysql-import-ledger-scorecard | scorecard | yes | reviewer |
| mysql-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for MySQL Data Importer before any MySQL write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into mysql. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `mysql-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] mysql-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:data-importer` consumers with approval and credential context attached.
