---
name: postgres-workflow-automator
description: Use when tasks require postgres workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Workflow Automator
...
| Primary artifact | `postgres-workflow-runbook` |
| Routing tag | `postgres:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postgres Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using postgres..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-workflow-runbook | structured-artifact | yes | next workflow or operator |
| postgres-workflow-runbook-scorecard | scorecard | yes | reviewer |
| postgres-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Postgres Workflow Automator, then checkpoint where Postgres state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using postgres. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `postgres-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] postgres-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:workflow-automator` consumers with approval and credential context attached.
