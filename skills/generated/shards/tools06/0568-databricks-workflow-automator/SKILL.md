---
name: databricks-workflow-automator
description: Use when tasks require databricks workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Workflow Automator
...
| Primary artifact | `databricks-workflow-runbook` |
| Routing tag | `databricks:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Databricks Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using databricks..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| databricks-workflow-runbook | structured-artifact | yes | next workflow or operator |
| databricks-workflow-runbook-scorecard | scorecard | yes | reviewer |
| databricks-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Databricks Workflow Automator, then checkpoint where Databricks state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using databricks. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `databricks-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] databricks-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:workflow-automator` consumers with approval and credential context attached.
