---
name: mysql-approval-router
description: Use when tasks require mysql approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MySQL Approval Router
...
| Primary artifact | `mysql-approval-decision-packet` |
| Routing tag | `mysql:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Approval Router into a deterministic, auth-checked workflow for routes mysql-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| mysql-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| mysql-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for MySQL Approval Router, then capture the manual fallback path for MySQL.
...
3. Implement routes mysql-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `mysql-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] mysql-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:approval-router` consumers with approval and credential context attached.
