---
name: databricks-webhook-listener
description: Use when tasks require databricks webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Databricks Webhook Listener
...
| Primary artifact | `databricks-event-intake-log` |
| Routing tag | `databricks:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Databricks workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Databricks Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes databricks webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Databricks Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `DATABRICKS_CLIENT_ID`, `DATABRICKS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `DATABRICKS_TOKEN`, `DATABRICKS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DATABRICKS_WEBHOOK_SECRET`, `DATABRICKS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| databricks-event-intake-log | structured-artifact | yes | next workflow or operator |
| databricks-event-intake-log-scorecard | scorecard | yes | reviewer |
| databricks-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Databricks Webhook Listener, then register the signing-secret policy for Databricks.
...
3. Implement captures, verifies, and routes databricks webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `databricks-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] databricks-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `databricks-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `databricks:webhook-listener` consumers with approval and credential context attached.
