---
name: mysql-data-exporter
description: Use when tasks require mysql data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-export-package` |
| Routing tag | `mysql:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Data Exporter into a deterministic, auth-checked workflow for exports mysql entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-export-package | structured-artifact | yes | next workflow or operator |
| mysql-export-package-scorecard | scorecard | yes | reviewer |
| mysql-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for MySQL Data Exporter, then constrain the export scope for MySQL before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports mysql entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `mysql-export-package` so downstream owners do not need to rediscover context.
...
- [ ] mysql-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:data-exporter` consumers with approval and credential context attached.
