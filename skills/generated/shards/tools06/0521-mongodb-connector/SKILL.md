...
description: Use when tasks require mongodb connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mongodb-connection-trace-bundle` |
| Routing tag | `mongodb:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MongoDB workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MongoDB Connector into a deterministic, auth-checked workflow for unified read/write connector for mongodb apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `MongoDB Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mongodb-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| mongodb-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| mongodb-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for MongoDB Connector, then confirm the smallest read-only probe that proves access to MongoDB.
...
3. Specify the request and response contract for unified read/write connector for mongodb apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `mongodb-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mongodb-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mongodb-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mongodb:connector` consumers with approval and credential context attached.
