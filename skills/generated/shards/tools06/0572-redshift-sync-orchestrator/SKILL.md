---
name: redshift-sync-orchestrator
description: Use when tasks require redshift sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Sync Orchestrator
...
| Primary artifact | `redshift-reconciliation-bundle` |
| Routing tag | `redshift:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for redshift records..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| redshift-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| redshift-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Redshift Sync Orchestrator, then checkpoint the last known good cursor on both sides of Redshift.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for redshift records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `redshift-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] redshift-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:sync-orchestrator` consumers with approval and credential context attached.
