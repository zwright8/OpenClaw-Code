...
description: Use when tasks require mysql connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mysql-connection-trace-bundle` |
| Routing tag | `mysql:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns MySQL Connector into a deterministic, auth-checked workflow for unified read/write connector for mysql apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mysql-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| mysql-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| mysql-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for MySQL Connector, then confirm the smallest read-only probe that proves access to MySQL.
...
3. Specify the request and response contract for unified read/write connector for mysql apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `mysql-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mysql-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:connector` consumers with approval and credential context attached.
