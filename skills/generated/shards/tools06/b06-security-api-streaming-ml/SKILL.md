---
name: b06-security-api-streaming-ml
description: Use when validating vulnerabilities, exercising APIs, handling stream/media pipelines, and evaluating ML/data processing tooling in terminal-first workflows.
---

# Security, API, streaming, and ML utility workflows

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b06-security-api-streaming-ml` |
| Tool count | `26` |
| Inventory rank span | `2568-2598` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for security, api, streaming, ml utility workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `govulncheck` | `orocos-kdl` | no | no |
| bounded mutation / transformation | `orocos-kdl` | `govulncheck` | yes | yes |
| validation & artifact checks | `govulncheck` | `orocos-kdl` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `govulncheck` | Database client and tools for the Go vulnerability database | local-installable |
| `orocos-kdl` | Orocos Kinematics and Dynamics C++ library | local-installable |
| `pidgin` | Multi-protocol chat client | local-installable |
| `lldpd` | Implementation of IEEE 802.1ab (LLDP) | local-installable |
| `atac` | Simple API client (Postman-like) in your terminal | local-installable |
| `fselect` | Find files with SQL-like queries | local-installable |
| `megatools` | Command-line client for Mega.co.nz | local-installable |
| `jaq` | JQ clone focussed on correctness, speed, and simplicity | local-installable |
| `mcap` | Serialization-agnostic container file format for pub/sub messages | local-installable |
| `vulkan-validationlayers` | Vulkan layers that enable developers to verify correct use of the Vulkan API | local-installable |
| `tuios` | Terminal UI OS (Terminal Multiplexer) | local-installable |
| `elan-init` | Lean Theorem Prover installer and version manager | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `govulncheck` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `orocos-kdl` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pidgin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lldpd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `atac` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fselect` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `megatools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jaq` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vulkan-validationlayers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tuios` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `elan-init` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mods` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fn` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `csound` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pastel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `golines` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cli53` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gator` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xgboost` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `vnu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mac` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `falco` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mavsdk` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `unoconv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `immich-go` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `govulncheck` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `orocos-kdl` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `pidgin` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `lldpd` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `atac` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `fselect` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a security, api, streaming, ml utility workflows request, run dry-run first, then produce a validated artifact bundle.
