---
name: postgres-data-importer
description: Use when tasks require postgres data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postgres Data Importer
...
| Primary artifact | `postgres-import-ledger` |
| Routing tag | `postgres:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postgres workflows degrade when state changes are hard to undo once they begin. This specific skill turns Postgres Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into postgres..

## Trigger Checklist
- [ ] The task explicitly requires `Postgres Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postgres-import-ledger | structured-artifact | yes | next workflow or operator |
| postgres-import-ledger-scorecard | scorecard | yes | reviewer |
| postgres-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Postgres Data Importer before any Postgres write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into postgres. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `postgres-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] postgres-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postgres-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postgres:data-importer` consumers with approval and credential context attached.
