---
name: redis-data-importer
description: Use when tasks require redis data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `redis-import-ledger` |
| Routing tag | `redis:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when state changes are hard to undo once they begin. This specific skill turns Redis Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into redis..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-import-ledger | structured-artifact | yes | next workflow or operator |
| redis-import-ledger-scorecard | scorecard | yes | reviewer |
| redis-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Redis Data Importer before any Redis write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into redis. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `redis-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] redis-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:data-importer` consumers with approval and credential context attached.
