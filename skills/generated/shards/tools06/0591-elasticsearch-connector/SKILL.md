...
description: Use when tasks require elasticsearch connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `elasticsearch-connection-trace-bundle` |
| Routing tag | `elasticsearch:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Connector into a deterministic, auth-checked workflow for unified read/write connector for elasticsearch apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| elasticsearch-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| elasticsearch-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Elasticsearch Connector, then confirm the smallest read-only probe that proves access to Elasticsearch.
...
3. Specify the request and response contract for unified read/write connector for elasticsearch apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `elasticsearch-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:connector` consumers with approval and credential context attached.
