---
name: redis-approval-router
description: Use when tasks require redis approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redis Approval Router
...
| Primary artifact | `redis-approval-decision-packet` |
| Routing tag | `redis:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redis workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redis Approval Router into a deterministic, auth-checked workflow for routes redis-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Redis Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redis-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| redis-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| redis-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Redis Approval Router, then capture the manual fallback path for Redis.
...
3. Implement routes redis-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `redis-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] redis-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redis-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redis:approval-router` consumers with approval and credential context attached.
