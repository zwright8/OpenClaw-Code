---
name: mysql-webhook-listener
description: Use when tasks require mysql webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# MySQL Webhook Listener
...
| Primary artifact | `mysql-event-intake-log` |
| Routing tag | `mysql:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because MySQL workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns MySQL Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes mysql webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `MySQL Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Service account or machine credential | `service_account` | `MYSQL_SERVICE_ACCOUNT_JSON`, `MYSQL_CREDENTIALS_FILE` | Verify the machine credential or profile against the target environment before any mutating step. |
| Existing session or local profile | `session` | `MYSQL_SESSION`, `MYSQL_PROFILE` | Check the current session/profile and confirm it points at the intended tenant or workspace. |
| Webhook signing secret | `signing_secret` | `MYSQL_WEBHOOK_SECRET`, `MYSQL_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
| Access token or personal access token | `token` | `MYSQL_TOKEN`, `MYSQL_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
...
| --- | --- | --- | --- |
| mysql-event-intake-log | structured-artifact | yes | next workflow or operator |
| mysql-event-intake-log-scorecard | scorecard | yes | reviewer |
| mysql-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for MySQL Webhook Listener, then register the signing-secret policy for MySQL.
...
3. Implement captures, verifies, and routes mysql webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `mysql-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] mysql-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mysql-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mysql:webhook-listener` consumers with approval and credential context attached.
