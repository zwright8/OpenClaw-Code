---
name: elasticsearch-data-importer
description: Use when tasks require elasticsearch data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `elasticsearch-import-ledger` |
| Routing tag | `elasticsearch:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when state changes are hard to undo once they begin. This specific skill turns Elasticsearch Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into elasticsearch..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-import-ledger | structured-artifact | yes | next workflow or operator |
| elasticsearch-import-ledger-scorecard | scorecard | yes | reviewer |
| elasticsearch-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Elasticsearch Data Importer before any Elasticsearch write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into elasticsearch. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `elasticsearch-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:data-importer` consumers with approval and credential context attached.
