---
name: snowflake-report-builder
description: Use when tasks require snowflake report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `snowflake-report-bundle` |
| Routing tag | `snowflake:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Snowflake workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Snowflake Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from snowflake activity..

## Trigger Checklist
- [ ] The task explicitly requires `Snowflake Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| snowflake-report-bundle | structured-artifact | yes | next workflow or operator |
| snowflake-report-bundle-scorecard | scorecard | yes | reviewer |
| snowflake-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Snowflake Report Builder, then confirm which Snowflake entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from snowflake activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `snowflake-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] snowflake-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `snowflake-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `snowflake:report-builder` consumers with approval and credential context attached.
