---
name: elasticsearch-data-exporter
description: Use when tasks require elasticsearch data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `elasticsearch-export-package` |
| Routing tag | `elasticsearch:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Elasticsearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Elasticsearch Data Exporter into a deterministic, auth-checked workflow for exports elasticsearch entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Elasticsearch Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| elasticsearch-export-package | structured-artifact | yes | next workflow or operator |
| elasticsearch-export-package-scorecard | scorecard | yes | reviewer |
| elasticsearch-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Elasticsearch Data Exporter, then constrain the export scope for Elasticsearch before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports elasticsearch entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `elasticsearch-export-package` so downstream owners do not need to rediscover context.
...
- [ ] elasticsearch-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `elasticsearch-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `elasticsearch:data-exporter` consumers with approval and credential context attached.
