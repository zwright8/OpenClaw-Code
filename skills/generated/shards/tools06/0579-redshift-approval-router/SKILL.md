---
name: redshift-approval-router
description: Use when tasks require redshift approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Approval Router
...
| Primary artifact | `redshift-approval-decision-packet` |
| Routing tag | `redshift:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Redshift Approval Router into a deterministic, auth-checked workflow for routes redshift-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| redshift-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| redshift-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Redshift Approval Router, then capture the manual fallback path for Redshift.
...
3. Implement routes redshift-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `redshift-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] redshift-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:approval-router` consumers with approval and credential context attached.
