---
name: redshift-migration-assistant
description: Use when tasks require redshift migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Redshift Migration Assistant
...
| Primary artifact | `redshift-migration-cutover-plan` |
| Routing tag | `redshift:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Redshift workflows degrade when state changes are hard to undo once they begin. This specific skill turns Redshift Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from redshift..

## Trigger Checklist
- [ ] The task explicitly requires `Redshift Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| redshift-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| redshift-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| redshift-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Redshift Migration Assistant before any Redshift rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from redshift. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `redshift-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] redshift-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `redshift-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `redshift:migration-assistant` consumers with approval and credential context attached.
