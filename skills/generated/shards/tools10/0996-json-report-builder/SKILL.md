---
name: json-report-builder
description: Use when tasks require json report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `json-report-bundle` |
| Routing tag | `json:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from json activity..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-report-bundle | structured-artifact | yes | next workflow or operator |
| json-report-bundle-scorecard | scorecard | yes | reviewer |
| json-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for JSON Report Builder, then confirm which JSON entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from json activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `json-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] json-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:report-builder` consumers with approval and credential context attached.
