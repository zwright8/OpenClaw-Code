---
name: wordpress-webhook-listener
description: Use when tasks require wordpress webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `wordpress-event-intake-log` |
| Routing tag | `wordpress:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns WordPress Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes wordpress webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `WORDPRESS_CLIENT_ID`, `WORDPRESS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `WORDPRESS_TOKEN`, `WORDPRESS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `WORDPRESS_WEBHOOK_SECRET`, `WORDPRESS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| wordpress-event-intake-log | structured-artifact | yes | next workflow or operator |
| wordpress-event-intake-log-scorecard | scorecard | yes | reviewer |
| wordpress-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for WordPress Webhook Listener, then register the signing-secret policy for WordPress.
...
3. Implement captures, verifies, and routes wordpress webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `wordpress-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:webhook-listener` consumers with approval and credential context attached.
