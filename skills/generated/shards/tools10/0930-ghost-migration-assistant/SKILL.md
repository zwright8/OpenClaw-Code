---
name: ghost-migration-assistant
description: Use when tasks require ghost migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Migration Assistant
...
| Primary artifact | `ghost-migration-cutover-plan` |
| Routing tag | `ghost:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when state changes are hard to undo once they begin. This specific skill turns Ghost Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from ghost..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| ghost-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| ghost-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Ghost Migration Assistant before any Ghost rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from ghost. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `ghost-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] ghost-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:migration-assistant` consumers with approval and credential context attached.
