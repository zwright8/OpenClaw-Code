...
description: Use when tasks require spreadsheet data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `spreadsheet-import-ledger` |
| Routing tag | `spreadsheet:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when state changes are hard to undo once they begin. This specific skill turns Spreadsheet Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into spreadsheet..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-import-ledger | structured-artifact | yes | next workflow or operator |
| spreadsheet-import-ledger-scorecard | scorecard | yes | reviewer |
| spreadsheet-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Spreadsheet Data Importer before any Spreadsheet write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into spreadsheet. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `spreadsheet-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:data-importer` consumers with approval and credential context attached.
