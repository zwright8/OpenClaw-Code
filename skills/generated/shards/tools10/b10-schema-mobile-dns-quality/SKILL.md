...
| Field | Value |
|---|---|
| Skill pack | `b10-schema-mobile-dns-quality` |
| Tool count | `26` |
| Inventory rank span | `4726-4758` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for schema validation, mobile device tooling, dns automation, quality gates tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `graphql-inspector` | `ddcutil` | no | no |
| bounded mutation / transformation | `ddcutil` | `graphql-inspector` | yes | yes |
| validation & artifact checks | `graphql-inspector` | `ddcutil` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `graphql-inspector` | Validate schema, get schema change notifications, validate operations, and more | local-installable |
| `ddcutil` | Control monitor settings using DDC/CI and USB | local-installable |
| `fblog` | Small command-line JSON log viewer | local-installable |
| `woob` | Web Outside of Browsers | local-installable |
| `ssh-vault` | Encrypt/decrypt using SSH keys | local-installable |
| `vsd` | Download video streams over HTTP, DASH (.mpd), and HLS (.m3u8) | local-installable |
| `gurk` | Signal Messenger client for terminal | local-installable |
| `c2rust` | Migrate C code to Rust | local-installable |
| `oxfmt` | High-performance formatting tool for JavaScript and TypeScript | local-installable |
| `metricbeat` | Collect metrics from your systems and services | local-installable |
| `waybackpy` | Wayback Machine API interface & command-line tool | local-installable |
| `dicebear` | CLI for DiceBear - An avatar library for designers and developers | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `graphql-inspector` | GraphQL over HTTPS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ddcutil` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fblog` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `woob` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ssh-vault` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vsd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gurk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c2rust` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `oxfmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `metricbeat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `waybackpy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dicebear` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `ns-3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ios-sim` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mobiledevice` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cairomm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lexicon` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `efm-langserver` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `precious` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bzip3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `iamb` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `onlykey-agent` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dra` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gplugin` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `otterdog` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `polynote` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `dra`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `otterdog`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `graphql-inspector` (GraphQL over HTTPS, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `ddcutil` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `fblog` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `woob` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `ssh-vault` (SSH, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `vsd` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a schema validation, mobile device tooling, dns automation, quality gates request, run dry-run first, then produce a validated artifact bundle.
