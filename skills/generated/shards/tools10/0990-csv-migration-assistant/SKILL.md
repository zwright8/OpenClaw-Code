---
name: csv-migration-assistant
description: Use when tasks require csv migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-migration-cutover-plan` |
| Routing tag | `csv:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when state changes are hard to undo once they begin. This specific skill turns CSV Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from csv..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| csv-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| csv-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for CSV Migration Assistant before any CSV rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from csv. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `csv-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] csv-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:migration-assistant` consumers with approval and credential context attached.
