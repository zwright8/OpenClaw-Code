...
| Field | Value |
|---|---|
| Skill pack | `b10-quality-media-supplychain` |
| Tool count | `26` |
| Inventory rank span | `4628-4657` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for code quality, media processing, software supply-chain checks tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `gitmux` | `ltex-ls-plus` | no | no |
| bounded mutation / transformation | `ltex-ls-plus` | `gitmux` | yes | yes |
| validation & artifact checks | `gitmux` | `ltex-ls-plus` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `gitmux` | Git status in tmux status bar | local-installable |
| `ltex-ls-plus` | LTeX+ Language Server: maintained fork of LTeX Language Server | local-installable |
| `mdf2iso` | Tool to convert MDF (Alcohol 120% images) images to ISO images | local-installable |
| `htmlhint` | Static code analysis tool you need for your HTML | local-installable |
| `rip2` | Safe and ergonomic alternative to rm | local-installable |
| `cyclonedx-python` | Creates CycloneDX Software Bill of Materials (SBOM) from Python projects | local-installable |
| `nak` | CLI for doing all things nostr | local-installable |
| `rpl` | Text replacement utility | local-installable |
| `audiowaveform` | Generate waveform data and render waveform images from audio files | local-installable |
| `uhubctl` | USB hub per-port power control | local-installable |
| `cargo-deny` | Cargo plugin for linting your dependencies | local-installable |
| `sipsak` | SIP Swiss army knife | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `gitmux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ltex-ls-plus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mdf2iso` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `htmlhint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rip2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cyclonedx-python` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nak` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rpl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `audiowaveform` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `uhubctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-deny` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sipsak` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libzdb` | SQL/DB protocol, Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `soapyhackrf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libupnp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libzim` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `policy_sentry` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `go-blueprint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `papilo` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `abi3audit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rtags` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aspcud` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxml++` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ooniprobe` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `forbidden` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rtmidi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `policy_sentry`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `gitmux` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `ltex-ls-plus` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `mdf2iso` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `htmlhint` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `rip2` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `cyclonedx-python` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a code quality, media processing, software supply-chain checks request, run dry-run first, then produce a validated artifact bundle.
