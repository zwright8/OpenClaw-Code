---
name: wordpress-approval-router
description: Use when tasks require wordpress approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Approval Router
...
| Primary artifact | `wordpress-approval-decision-packet` |
| Routing tag | `wordpress:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Approval Router into a deterministic, auth-checked workflow for routes wordpress-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| wordpress-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| wordpress-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for WordPress Approval Router, then capture the manual fallback path for WordPress.
...
3. Implement routes wordpress-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `wordpress-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:approval-router` consumers with approval and credential context attached.
