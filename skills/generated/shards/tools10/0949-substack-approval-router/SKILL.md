---
name: substack-approval-router
description: Use when tasks require substack approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Approval Router
...
| Primary artifact | `substack-approval-decision-packet` |
| Routing tag | `substack:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Approval Router into a deterministic, auth-checked workflow for routes substack-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| substack-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| substack-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Substack Approval Router, then capture the manual fallback path for Substack.
...
3. Implement routes substack-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `substack-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] substack-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:approval-router` consumers with approval and credential context attached.
