...
| Field | Value |
|---|---|
| Skill pack | `b10-stream-build-deploy` |
| Tool count | `26` |
| Inventory rank span | `4658-4690` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for streaming compute, build automation, deployment workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `rsyncy` | `bento` | no | no |
| bounded mutation / transformation | `bento` | `rsyncy` | yes | yes |
| validation & artifact checks | `rsyncy` | `bento` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `rsyncy` | Status/progress bar for rsync | local-installable |
| `bento` | Fancy stream processing made operationally mundane | local-installable |
| `clickhouse-odbc` | Official ODBC driver implementation for accessing ClickHouse as a data source | local-installable |
| `basti` | Securely connect to RDS, Elasticache, and other AWS resources in VPCs | local-installable |
| `eslint_d` | Speed up eslint to accelerate your development workflow | local-installable |
| `procmail` | Autonomous mail processor | local-installable |
| `storm` | Distributed realtime computation system to process data streams | local-installable |
| `totp-cli` | Authy/Google Authenticator like TOTP CLI tool written in Go | local-installable |
| `aws-c-cal` | AWS Crypto Abstraction Layer | local-installable |
| `chuck` | Concurrent, on-the-fly audio programming language | local-installable |
| `swiftdraw` | Convert SVG into PDF, PNG, JPEG or SF Symbol | local-installable |
| `pjproject` | C library for multimedia protocols such as SIP, SDP, RTP and more | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `rsyncy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bento` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clickhouse-odbc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `basti` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `eslint_d` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `procmail` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `storm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `totp-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-c-cal` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `chuck` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swiftdraw` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pjproject` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `ent` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `di` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tvnamer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ferron` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `proguard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c-blosc2` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `gotests` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kosli-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `usb.ids` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fabric-installer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nextflow` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dar` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ephemeralpg` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-c-cal`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `basti`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `rsyncy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `bento` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `clickhouse-odbc` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `basti` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `eslint_d` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `procmail` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a streaming compute, build automation, deployment workflows request, run dry-run first, then produce a validated artifact bundle.
