---
name: docx-sync-orchestrator
description: Use when tasks require docx sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DOCX Sync Orchestrator
...
| Primary artifact | `docx-reconciliation-bundle` |
| Routing tag | `docx:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for docx records..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| docx-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| docx-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for DOCX Sync Orchestrator, then checkpoint the last known good cursor on both sides of DOCX.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for docx records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `docx-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docx-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:sync-orchestrator` consumers with approval and credential context attached.
