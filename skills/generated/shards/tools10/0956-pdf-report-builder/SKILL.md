...
description: Use when tasks require pdf report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pdf-report-bundle` |
| Routing tag | `pdf:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from pdf activity..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-report-bundle | structured-artifact | yes | next workflow or operator |
| pdf-report-bundle-scorecard | scorecard | yes | reviewer |
| pdf-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for PDF Report Builder, then confirm which PDF entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from pdf activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `pdf-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pdf-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:report-builder` consumers with approval and credential context attached.
