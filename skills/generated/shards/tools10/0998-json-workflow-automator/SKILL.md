---
name: json-workflow-automator
description: Use when tasks require json workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# JSON Workflow Automator
...
| Primary artifact | `json-workflow-runbook` |
| Routing tag | `json:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using json..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-workflow-runbook | structured-artifact | yes | next workflow or operator |
| json-workflow-runbook-scorecard | scorecard | yes | reviewer |
| json-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for JSON Workflow Automator, then checkpoint where JSON state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using json. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `json-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] json-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:workflow-automator` consumers with approval and credential context attached.
