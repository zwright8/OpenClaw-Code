---
name: medium-alert-monitor
description: Use when tasks require medium alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `medium-alert-state-bundle` |
| Routing tag | `medium:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Alert Monitor into a deterministic, auth-checked workflow for monitors key medium signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| medium-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| medium-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Medium Alert Monitor, then lock the reviewed threshold set for Medium.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key medium signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `medium-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] medium-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:alert-monitor` consumers with approval and credential context attached.
