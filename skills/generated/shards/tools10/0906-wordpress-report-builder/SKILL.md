---
name: wordpress-report-builder
description: Use when tasks require wordpress report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Report Builder
...
| Primary artifact | `wordpress-report-bundle` |
| Routing tag | `wordpress:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from wordpress activity..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-report-bundle | structured-artifact | yes | next workflow or operator |
| wordpress-report-bundle-scorecard | scorecard | yes | reviewer |
| wordpress-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for WordPress Report Builder, then confirm which WordPress entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from wordpress activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `wordpress-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:report-builder` consumers with approval and credential context attached.
