---
name: webflow-webhook-listener
description: Use when tasks require webflow webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Webhook Listener
...
| Primary artifact | `webflow-event-intake-log` |
| Routing tag | `webflow:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Webflow Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes webflow webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `WEBFLOW_CLIENT_ID`, `WEBFLOW_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `WEBFLOW_TOKEN`, `WEBFLOW_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `WEBFLOW_WEBHOOK_SECRET`, `WEBFLOW_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| webflow-event-intake-log | structured-artifact | yes | next workflow or operator |
| webflow-event-intake-log-scorecard | scorecard | yes | reviewer |
| webflow-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Webflow Webhook Listener, then register the signing-secret policy for Webflow.
...
3. Implement captures, verifies, and routes webflow webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `webflow-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] webflow-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:webhook-listener` consumers with approval and credential context attached.
