---
name: ghost-alert-monitor
description: Use when tasks require ghost alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `ghost-alert-state-bundle` |
| Routing tag | `ghost:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Alert Monitor into a deterministic, auth-checked workflow for monitors key ghost signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| ghost-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| ghost-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Ghost Alert Monitor, then lock the reviewed threshold set for Ghost.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key ghost signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `ghost-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ghost-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:alert-monitor` consumers with approval and credential context attached.
