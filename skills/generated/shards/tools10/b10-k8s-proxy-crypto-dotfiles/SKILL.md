...
| Field | Value |
|---|---|
| Skill pack | `b10-k8s-proxy-crypto-dotfiles` |
| Tool count | `26` |
| Inventory rank span | `4898-4933` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for kubernetes mcp, proxy tunneling, cryptography, dotfile automation tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `stuntman` | `ox` | no | no |
| bounded mutation / transformation | `ox` | `stuntman` | yes | yes |
| validation & artifact checks | `stuntman` | `ox` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `stuntman` | Implementation of the STUN protocol | local-installable |
| `ox` | Independent Rust text editor that runs in your terminal | local-installable |
| `jimtcl` | Small footprint implementation of Tcl | local-installable |
| `cloudformation-guard` | Checks CloudFormation templates for compliance using a declarative syntax | local-installable |
| `enigma` | Puzzle game inspired by Oxyd and Rock'n'Roll | local-installable |
| `nqp` | Lightweight Raku-like environment for virtual machines | local-installable |
| `lsix` | Shows thumbnails in terminal using sixel graphics | local-installable |
| `lowdown` | Simple markdown translator | local-installable |
| `run` | Easily manage and invoke small scripts and wrappers | local-installable |
| `lhasa` | LHA implementation to decompress .lzh and .lzs archives | local-installable |
| `urlscan` | View/select the URLs in an email message or file | local-installable |
| `filen-cli` | Interface with Filen, an end-to-end encrypted cloud storage service | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `stuntman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jimtcl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cloudformation-guard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `enigma` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nqp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lsix` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lowdown` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `run` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lhasa` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `urlscan` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `filen-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cf-terraforming` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kubernetes-mcp-server` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `proxytunnel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sequoia-sq` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cppi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotdrop` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kyma-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ccrypt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `github-release` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `libgweather` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `megacmd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `freedink` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lacework-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hatari` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `github-release`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `kubernetes-mcp-server`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `stuntman` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `ox` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `jimtcl` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `cloudformation-guard` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read/query command sequence, capture outputs and exit code in handoff packet.
5. `enigma` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `nqp` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a kubernetes mcp, proxy tunneling, cryptography, dotfile automation request, run dry-run first, then produce a validated artifact bundle.
