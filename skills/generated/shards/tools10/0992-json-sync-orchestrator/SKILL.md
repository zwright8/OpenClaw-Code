---
name: json-sync-orchestrator
description: Use when tasks require json sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# JSON Sync Orchestrator
...
| Primary artifact | `json-reconciliation-bundle` |
| Routing tag | `json:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for json records..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| json-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| json-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for JSON Sync Orchestrator, then checkpoint the last known good cursor on both sides of JSON.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for json records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `json-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] json-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:sync-orchestrator` consumers with approval and credential context attached.
