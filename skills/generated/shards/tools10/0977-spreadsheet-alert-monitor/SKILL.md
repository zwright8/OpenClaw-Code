---
name: spreadsheet-alert-monitor
description: Use when tasks require spreadsheet alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Alert Monitor
...
| Primary artifact | `spreadsheet-alert-state-bundle` |
| Routing tag | `spreadsheet:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Alert Monitor into a deterministic, auth-checked workflow for monitors key spreadsheet signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| spreadsheet-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| spreadsheet-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Spreadsheet Alert Monitor, then lock the reviewed threshold set for Spreadsheet.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key spreadsheet signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `spreadsheet-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:alert-monitor` consumers with approval and credential context attached.
