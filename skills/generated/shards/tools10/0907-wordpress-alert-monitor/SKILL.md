---
name: wordpress-alert-monitor
description: Use when tasks require wordpress alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Alert Monitor
...
| Primary artifact | `wordpress-alert-state-bundle` |
| Routing tag | `wordpress:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Alert Monitor into a deterministic, auth-checked workflow for monitors key wordpress signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| wordpress-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| wordpress-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for WordPress Alert Monitor, then lock the reviewed threshold set for WordPress.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key wordpress signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `wordpress-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:alert-monitor` consumers with approval and credential context attached.
