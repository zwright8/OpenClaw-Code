---
name: pdf-sync-orchestrator
description: Use when tasks require pdf sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PDF Sync Orchestrator
...
| Primary artifact | `pdf-reconciliation-bundle` |
| Routing tag | `pdf:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for pdf records..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| pdf-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| pdf-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for PDF Sync Orchestrator, then checkpoint the last known good cursor on both sides of PDF.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for pdf records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `pdf-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pdf-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:sync-orchestrator` consumers with approval and credential context attached.
