---
name: spreadsheet-webhook-listener
description: Use when tasks require spreadsheet webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Webhook Listener
...
| Primary artifact | `spreadsheet-event-intake-log` |
| Routing tag | `spreadsheet:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Spreadsheet Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes spreadsheet webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SPREADSHEET_CLIENT_ID`, `SPREADSHEET_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SPREADSHEET_TOKEN`, `SPREADSHEET_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SPREADSHEET_WEBHOOK_SECRET`, `SPREADSHEET_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| spreadsheet-event-intake-log | structured-artifact | yes | next workflow or operator |
| spreadsheet-event-intake-log-scorecard | scorecard | yes | reviewer |
| spreadsheet-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Spreadsheet Webhook Listener, then register the signing-secret policy for Spreadsheet.
...
3. Implement captures, verifies, and routes spreadsheet webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `spreadsheet-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:webhook-listener` consumers with approval and credential context attached.
