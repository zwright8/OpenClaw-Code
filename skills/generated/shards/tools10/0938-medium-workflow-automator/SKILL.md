---
name: medium-workflow-automator
description: Use when tasks require medium workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Workflow Automator
...
| Primary artifact | `medium-workflow-runbook` |
| Routing tag | `medium:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using medium..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-workflow-runbook | structured-artifact | yes | next workflow or operator |
| medium-workflow-runbook-scorecard | scorecard | yes | reviewer |
| medium-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Medium Workflow Automator, then checkpoint where Medium state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using medium. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `medium-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] medium-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:workflow-automator` consumers with approval and credential context attached.
