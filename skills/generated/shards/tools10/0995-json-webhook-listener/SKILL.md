---
name: json-webhook-listener
description: Use when tasks require json webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# JSON Webhook Listener
...
| Primary artifact | `json-event-intake-log` |
| Routing tag | `json:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns JSON Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes json webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `JSON_CLIENT_ID`, `JSON_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `JSON_TOKEN`, `JSON_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `JSON_WEBHOOK_SECRET`, `JSON_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| json-event-intake-log | structured-artifact | yes | next workflow or operator |
| json-event-intake-log-scorecard | scorecard | yes | reviewer |
| json-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for JSON Webhook Listener, then register the signing-secret policy for JSON.
...
3. Implement captures, verifies, and routes json webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `json-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] json-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:webhook-listener` consumers with approval and credential context attached.
