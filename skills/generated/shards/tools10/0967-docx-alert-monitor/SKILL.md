---
name: docx-alert-monitor
description: Use when tasks require docx alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docx-alert-state-bundle` |
| Routing tag | `docx:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Alert Monitor into a deterministic, auth-checked workflow for monitors key docx signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| docx-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| docx-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for DOCX Alert Monitor, then lock the reviewed threshold set for DOCX.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key docx signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `docx-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docx-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:alert-monitor` consumers with approval and credential context attached.
