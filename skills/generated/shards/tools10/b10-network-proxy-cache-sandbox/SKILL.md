...
| Field | Value |
|---|---|
| Skill pack | `b10-network-proxy-cache-sandbox` |
| Tool count | `26` |
| Inventory rank span | `4935-4966` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for network/proxy services, cache/storage engines, command sandboxing tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `observerward` | `libcdio` | no | no |
| bounded mutation / transformation | `libcdio` | `observerward` | yes | yes |
| validation & artifact checks | `observerward` | `libcdio` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `observerward` | Web application and service fingerprint identification tool | local-installable |
| `libcdio` | Compact Disc Input and Control Library | local-installable |
| `aws-es-proxy` | Small proxy between HTTP client and AWS Elasticsearch | local-installable |
| `ser2net` | Allow network connections to serial ports | local-installable |
| `dmenu` | Dynamic menu for X11 | local-installable |
| `xboard` | Graphical user interface for chess | local-installable |
| `autocannon` | Fast HTTP/1.1 benchmarking tool written in Node.js | local-installable |
| `libserdes` | Schema ser/deserializer lib for Avro + Confluent Schema Registry | local-installable |
| `smartdns` | Rule-based DNS server for fast IP resolution, DoT/DoQ/DoH/DoH3 supported | local-installable |
| `vapoursynth-imwri` | VapourSynth filters - ImageMagick HDRI writer/reader | local-installable |
| `baidupcs-go` | Terminal utility for Baidu Network Disk | local-installable |
| `netcdf-cxx` | C++ libraries and utilities for NetCDF | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `observerward` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libcdio` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `aws-es-proxy` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `ser2net` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dmenu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xboard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `autocannon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libserdes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `smartdns` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vapoursynth-imwri` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `baidupcs-go` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `netcdf-cxx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cadence-workflow` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `torrra` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libgedit-gfls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `autorest` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `plutobook` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `garnet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pioneers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsql` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gssdp` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `geocode-glib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `countdown` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `websocketpp` | JSON-RPC/WebSocket, Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `cidr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fence` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-es-proxy`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `observerward` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `libcdio` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `aws-es-proxy` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `ser2net` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `dmenu` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `xboard` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a network/proxy services, cache/storage engines, command sandboxing request, run dry-run first, then produce a validated artifact bundle.
