...
description: Use when tasks require webflow connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `webflow-connection-trace-bundle` |
| Routing tag | `webflow:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Connector into a deterministic, auth-checked workflow for unified read/write connector for webflow apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| webflow-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| webflow-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Webflow Connector, then confirm the smallest read-only probe that proves access to Webflow.
...
3. Specify the request and response contract for unified read/write connector for webflow apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `webflow-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] webflow-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:connector` consumers with approval and credential context attached.
