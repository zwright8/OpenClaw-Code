...
| Field | Value |
|---|---|
| Skill pack | `b10-site-docs-security-kafka` |
| Tool count | `26` |
| Inventory rank span | `4831-4867` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for static site/docs tooling, security testing, kafka-centric operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `eleventy` | `protoc-gen-doc` | no | no |
| bounded mutation / transformation | `protoc-gen-doc` | `eleventy` | yes | yes |
| validation & artifact checks | `eleventy` | `protoc-gen-doc` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `eleventy` | Simpler static site generator | local-installable |
| `protoc-gen-doc` | Documentation generator plugin for Google Protocol Buffers | local-installable |
| `libtommath` | C library for number theoretic multiple-precision integers | local-installable |
| `lxi-tools` | Open source tools for managing network attached LXI compatible instruments | local-installable |
| `jaguar` | Live reloading for your ESP32 | local-installable |
| `lstr` | Fast, minimalist directory tree viewer | local-installable |
| `swag` | Automatically generate RESTful API documentation with Swagger 2.0 for Go | local-installable |
| `soapyrtlsdr` | SoapySDR RTL-SDR Support Module | local-installable |
| `atkmm@2.28` | Official C++ interface for the ATK accessibility toolkit library | local-installable |
| `git-delete-merged-branches` | Command-line tool to delete merged Git branches | local-installable |
| `imgp` | High-performance CLI batch image resizer & rotator | local-installable |
| `libdrm` | Library for accessing the direct rendering manager | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `eleventy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `protoc-gen-doc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libtommath` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lxi-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jaguar` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lstr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swag` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `soapyrtlsdr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `atkmm@2.28` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `git-delete-merged-branches` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `imgp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libdrm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `jhead` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `varlock` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qhull` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chdig` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `autocode` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `ltex-ls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pvetui` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libopusenc` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pocsuite3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `scummvm-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `urlview` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `votca` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qt-mariadb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yozefu` | Kafka protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `eleventy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `protoc-gen-doc` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `libtommath` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `lxi-tools` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `jaguar` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `lstr` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a static site/docs tooling, security testing, kafka-centric operations request, run dry-run first, then produce a validated artifact bundle.
