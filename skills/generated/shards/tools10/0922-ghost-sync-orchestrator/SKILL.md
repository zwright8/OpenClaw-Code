---
name: ghost-sync-orchestrator
description: Use when tasks require ghost sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Sync Orchestrator
...
| Primary artifact | `ghost-reconciliation-bundle` |
| Routing tag | `ghost:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for ghost records..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| ghost-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| ghost-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Ghost Sync Orchestrator, then checkpoint the last known good cursor on both sides of Ghost.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for ghost records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `ghost-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ghost-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:sync-orchestrator` consumers with approval and credential context attached.
