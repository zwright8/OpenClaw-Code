---
name: json-alert-monitor
description: Use when tasks require json alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `json-alert-state-bundle` |
| Routing tag | `json:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Alert Monitor into a deterministic, auth-checked workflow for monitors key json signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| json-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| json-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for JSON Alert Monitor, then lock the reviewed threshold set for JSON.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key json signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `json-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] json-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:alert-monitor` consumers with approval and credential context attached.
