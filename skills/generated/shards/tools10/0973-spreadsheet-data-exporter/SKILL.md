...
description: Use when tasks require spreadsheet data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Data Exporter
...
| Primary artifact | `spreadsheet-export-package` |
| Routing tag | `spreadsheet:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Data Exporter into a deterministic, auth-checked workflow for exports spreadsheet entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-export-package | structured-artifact | yes | next workflow or operator |
| spreadsheet-export-package-scorecard | scorecard | yes | reviewer |
| spreadsheet-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Spreadsheet Data Exporter, then constrain the export scope for Spreadsheet before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports spreadsheet entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `spreadsheet-export-package` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:data-exporter` consumers with approval and credential context attached.
