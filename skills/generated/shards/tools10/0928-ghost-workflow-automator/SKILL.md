---
name: ghost-workflow-automator
description: Use when tasks require ghost workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Workflow Automator
...
| Primary artifact | `ghost-workflow-runbook` |
| Routing tag | `ghost:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using ghost..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-workflow-runbook | structured-artifact | yes | next workflow or operator |
| ghost-workflow-runbook-scorecard | scorecard | yes | reviewer |
| ghost-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Ghost Workflow Automator, then checkpoint where Ghost state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using ghost. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `ghost-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] ghost-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:workflow-automator` consumers with approval and credential context attached.
