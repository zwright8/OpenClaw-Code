...
description: Use when tasks require medium data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `medium-import-ledger` |
| Routing tag | `medium:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when state changes are hard to undo once they begin. This specific skill turns Medium Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into medium..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-import-ledger | structured-artifact | yes | next workflow or operator |
| medium-import-ledger-scorecard | scorecard | yes | reviewer |
| medium-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Medium Data Importer before any Medium write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into medium. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `medium-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] medium-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:data-importer` consumers with approval and credential context attached.
