---
name: webflow-workflow-automator
description: Use when tasks require webflow workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Workflow Automator
...
| Primary artifact | `webflow-workflow-runbook` |
| Routing tag | `webflow:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using webflow..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-workflow-runbook | structured-artifact | yes | next workflow or operator |
| webflow-workflow-runbook-scorecard | scorecard | yes | reviewer |
| webflow-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Webflow Workflow Automator, then checkpoint where Webflow state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using webflow. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `webflow-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] webflow-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:workflow-automator` consumers with approval and credential context attached.
