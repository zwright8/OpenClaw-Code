---
name: docx-data-exporter
description: Use when tasks require docx data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docx-export-package` |
| Routing tag | `docx:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Data Exporter into a deterministic, auth-checked workflow for exports docx entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-export-package | structured-artifact | yes | next workflow or operator |
| docx-export-package-scorecard | scorecard | yes | reviewer |
| docx-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for DOCX Data Exporter, then constrain the export scope for DOCX before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports docx entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `docx-export-package` so downstream owners do not need to rediscover context.
...
- [ ] docx-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:data-exporter` consumers with approval and credential context attached.
