---
name: csv-sync-orchestrator
description: Use when tasks require csv sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# CSV Sync Orchestrator
...
| Primary artifact | `csv-reconciliation-bundle` |
| Routing tag | `csv:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for csv records..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| csv-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| csv-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for CSV Sync Orchestrator, then checkpoint the last known good cursor on both sides of CSV.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for csv records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `csv-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] csv-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:sync-orchestrator` consumers with approval and credential context attached.
