---
name: pdf-migration-assistant
description: Use when tasks require pdf migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PDF Migration Assistant
...
| Primary artifact | `pdf-migration-cutover-plan` |
| Routing tag | `pdf:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when state changes are hard to undo once they begin. This specific skill turns PDF Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from pdf..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| pdf-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| pdf-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for PDF Migration Assistant before any PDF rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from pdf. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `pdf-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] pdf-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:migration-assistant` consumers with approval and credential context attached.
