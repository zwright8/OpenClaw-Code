---
name: webflow-migration-assistant
description: Use when tasks require webflow migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Migration Assistant
...
| Primary artifact | `webflow-migration-cutover-plan` |
| Routing tag | `webflow:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when state changes are hard to undo once they begin. This specific skill turns Webflow Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from webflow..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| webflow-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| webflow-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Webflow Migration Assistant before any Webflow rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from webflow. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `webflow-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] webflow-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:migration-assistant` consumers with approval and credential context attached.
