...
| Field | Value |
|---|---|
| Skill pack | `b10-media-k8s-encryption-web` |
| Tool count | `26` |
| Inventory rank span | `4760-4796` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for media conversion, kubernetes policy automation, encryption, web serving tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `mkvdts2ac3` | `jsonschema2pojo` | no | no |
| bounded mutation / transformation | `jsonschema2pojo` | `mkvdts2ac3` | yes | yes |
| validation & artifact checks | `mkvdts2ac3` | `jsonschema2pojo` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `mkvdts2ac3` | Convert DTS audio to AC3 within a matroska file | local-installable |
| `jsonschema2pojo` | Generates Java types from JSON Schema (or example JSON) | local-installable |
| `superlu` | Solve large, sparse nonsymmetric systems of equations | local-installable |
| `ocp` | UNIX port of the Open Cubic Player | local-installable |
| `libzzip` | Library providing read access on ZIP-archives | local-installable |
| `mecab-ko-dic` | See mecab | local-installable |
| `iam-policy-json-to-terraform` | Convert a JSON IAM Policy into terraform | local-installable |
| `pdf-diff` | Tool for visualizing differences between two pdf files | local-installable |
| `dillo` | Fast and small graphical web browser | local-installable |
| `minizip-ng` | Zip file manipulation library with minizip 1.x compatibility layer | local-installable |
| `vultr` | Command-line tool for Vultr services | local-installable |
| `age-plugin-yubikey` | Plugin for encrypting files with age and PIV tokens such as YubiKeys | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `mkvdts2ac3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jsonschema2pojo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `superlu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ocp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libzzip` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `mecab-ko-dic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `iam-policy-json-to-terraform` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdf-diff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dillo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `minizip-ng` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `vultr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `age-plugin-yubikey` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `urdfdom` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shapelib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `json5` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kwctl` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `pass-git-helper` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `claude-code-templates` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `prism-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rage` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `phive` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libdazzle` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kubekey` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `scalingo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sysaidmin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `static-web-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kubekey`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `kwctl`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `mkvdts2ac3` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `jsonschema2pojo` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `superlu` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `ocp` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `libzzip` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `mecab-ko-dic` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a media conversion, kubernetes policy automation, encryption, web serving request, run dry-run first, then produce a validated artifact bundle.
