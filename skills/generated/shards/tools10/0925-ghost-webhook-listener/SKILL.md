---
name: ghost-webhook-listener
description: Use when tasks require ghost webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Webhook Listener
...
| Primary artifact | `ghost-event-intake-log` |
| Routing tag | `ghost:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Ghost Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes ghost webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `GHOST_CLIENT_ID`, `GHOST_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `GHOST_TOKEN`, `GHOST_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GHOST_WEBHOOK_SECRET`, `GHOST_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| ghost-event-intake-log | structured-artifact | yes | next workflow or operator |
| ghost-event-intake-log-scorecard | scorecard | yes | reviewer |
| ghost-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Ghost Webhook Listener, then register the signing-secret policy for Ghost.
...
3. Implement captures, verifies, and routes ghost webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `ghost-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] ghost-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:webhook-listener` consumers with approval and credential context attached.
