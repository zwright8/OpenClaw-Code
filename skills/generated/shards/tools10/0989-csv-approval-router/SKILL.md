---
name: csv-approval-router
description: Use when tasks require csv approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-approval-decision-packet` |
| Routing tag | `csv:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Approval Router into a deterministic, auth-checked workflow for routes csv-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| csv-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| csv-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for CSV Approval Router, then capture the manual fallback path for CSV.
...
3. Implement routes csv-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `csv-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] csv-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:approval-router` consumers with approval and credential context attached.
