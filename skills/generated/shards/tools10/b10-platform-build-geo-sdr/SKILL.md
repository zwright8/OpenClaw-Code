...
| Field | Value |
|---|---|
| Skill pack | `b10-platform-build-geo-sdr` |
| Tool count | `26` |
| Inventory rank span | `4501-4532` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for platform operations, release engineering, geospatial, sdr workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `dnsgen` | `nauty` | no | no |
| bounded mutation / transformation | `nauty` | `dnsgen` | yes | yes |
| validation & artifact checks | `dnsgen` | `nauty` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `dnsgen` | Generates DNS names from existing domain names | local-installable |
| `nauty` | Automorphism groups of graphs and digraphs | local-installable |
| `triton` | Joyent Triton CLI | local-installable |
| `libshout` | Data and connectivity library for the Icecast server | local-installable |
| `trafilatura` | Discovery, extraction and processing for Web text | local-installable |
| `erofs-utils` | Utilities for Enhanced Read-Only File System | local-installable |
| `kwok` | Kubernetes WithOut Kubelet - Simulates thousands of Nodes and Clusters | local-installable |
| `packetry` | Fast, intuitive USB 2.0 protocol analysis application for use with Cynthion | local-installable |
| `monit` | Manage and monitor processes, files, directories, and devices | local-installable |
| `h3` | Hexagonal hierarchical geospatial indexing system | local-installable |
| `girara` | Common components for zathura | local-installable |
| `proselint` | Linter for prose | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `dnsgen` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nauty` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `triton` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libshout` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `trafilatura` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `erofs-utils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kwok` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `packetry` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `monit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `h3` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `girara` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `proselint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jreleaser` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cobo-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `distcc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `c2048` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `limesuite` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cuetools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bup` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ponysay` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gabedit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `codeberg-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qtquick3dphysics` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mgis` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `adaptivecpp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kwok`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `dnsgen` (DNS, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `nauty` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `triton` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `libshout` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `trafilatura` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `erofs-utils` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a platform operations, release engineering, geospatial, sdr workflows request, run dry-run first, then produce a validated artifact bundle.
