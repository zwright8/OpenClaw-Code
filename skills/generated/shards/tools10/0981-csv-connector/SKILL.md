---
name: csv-connector
description: Use when tasks require csv connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-connection-trace-bundle` |
| Routing tag | `csv:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Connector into a deterministic, auth-checked workflow for unified read/write connector for csv apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| csv-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| csv-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for CSV Connector, then confirm the smallest read-only probe that proves access to CSV.
...
3. Specify the request and response contract for unified read/write connector for csv apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `csv-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] csv-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:connector` consumers with approval and credential context attached.
