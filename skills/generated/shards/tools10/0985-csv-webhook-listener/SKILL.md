---
name: csv-webhook-listener
description: Use when tasks require csv webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# CSV Webhook Listener
...
| Primary artifact | `csv-event-intake-log` |
| Routing tag | `csv:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns CSV Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes csv webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `CSV_CLIENT_ID`, `CSV_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `CSV_TOKEN`, `CSV_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CSV_WEBHOOK_SECRET`, `CSV_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| csv-event-intake-log | structured-artifact | yes | next workflow or operator |
| csv-event-intake-log-scorecard | scorecard | yes | reviewer |
| csv-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for CSV Webhook Listener, then register the signing-secret policy for CSV.
...
3. Implement captures, verifies, and routes csv webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `csv-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] csv-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:webhook-listener` consumers with approval and credential context attached.
