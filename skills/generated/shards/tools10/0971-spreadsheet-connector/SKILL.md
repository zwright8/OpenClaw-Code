...
description: Use when tasks require spreadsheet connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `spreadsheet-connection-trace-bundle` |
| Routing tag | `spreadsheet:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Connector into a deterministic, auth-checked workflow for unified read/write connector for spreadsheet apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| spreadsheet-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| spreadsheet-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Spreadsheet Connector, then confirm the smallest read-only probe that proves access to Spreadsheet.
...
3. Specify the request and response contract for unified read/write connector for spreadsheet apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `spreadsheet-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:connector` consumers with approval and credential context attached.
