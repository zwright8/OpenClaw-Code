...
description: Use when tasks require ghost connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `ghost-connection-trace-bundle` |
| Routing tag | `ghost:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Connector into a deterministic, auth-checked workflow for unified read/write connector for ghost apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| ghost-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| ghost-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Ghost Connector, then confirm the smallest read-only probe that proves access to Ghost.
...
3. Specify the request and response contract for unified read/write connector for ghost apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `ghost-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ghost-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:connector` consumers with approval and credential context attached.
