...
description: Use when tasks require docx report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docx-report-bundle` |
| Routing tag | `docx:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from docx activity..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-report-bundle | structured-artifact | yes | next workflow or operator |
| docx-report-bundle-scorecard | scorecard | yes | reviewer |
| docx-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for DOCX Report Builder, then confirm which DOCX entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from docx activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `docx-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docx-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:report-builder` consumers with approval and credential context attached.
