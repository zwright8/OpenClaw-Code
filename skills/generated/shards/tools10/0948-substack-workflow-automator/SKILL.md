---
name: substack-workflow-automator
description: Use when tasks require substack workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Workflow Automator
...
| Primary artifact | `substack-workflow-runbook` |
| Routing tag | `substack:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using substack..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-workflow-runbook | structured-artifact | yes | next workflow or operator |
| substack-workflow-runbook-scorecard | scorecard | yes | reviewer |
| substack-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Substack Workflow Automator, then checkpoint where Substack state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using substack. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `substack-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] substack-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:workflow-automator` consumers with approval and credential context attached.
