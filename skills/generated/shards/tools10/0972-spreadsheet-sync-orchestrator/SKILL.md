---
name: spreadsheet-sync-orchestrator
description: Use when tasks require spreadsheet sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Sync Orchestrator
...
| Primary artifact | `spreadsheet-reconciliation-bundle` |
| Routing tag | `spreadsheet:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for spreadsheet records..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| spreadsheet-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| spreadsheet-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Spreadsheet Sync Orchestrator, then checkpoint the last known good cursor on both sides of Spreadsheet.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for spreadsheet records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `spreadsheet-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:sync-orchestrator` consumers with approval and credential context attached.
