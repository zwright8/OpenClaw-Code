...
description: Use when tasks require spreadsheet report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `spreadsheet-report-bundle` |
| Routing tag | `spreadsheet:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from spreadsheet activity..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-report-bundle | structured-artifact | yes | next workflow or operator |
| spreadsheet-report-bundle-scorecard | scorecard | yes | reviewer |
| spreadsheet-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Spreadsheet Report Builder, then confirm which Spreadsheet entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from spreadsheet activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `spreadsheet-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:report-builder` consumers with approval and credential context attached.
