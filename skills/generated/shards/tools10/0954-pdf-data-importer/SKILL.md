---
name: pdf-data-importer
description: Use when tasks require pdf data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pdf-import-ledger` |
| Routing tag | `pdf:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when state changes are hard to undo once they begin. This specific skill turns PDF Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into pdf..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-import-ledger | structured-artifact | yes | next workflow or operator |
| pdf-import-ledger-scorecard | scorecard | yes | reviewer |
| pdf-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for PDF Data Importer before any PDF write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into pdf. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `pdf-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] pdf-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:data-importer` consumers with approval and credential context attached.
