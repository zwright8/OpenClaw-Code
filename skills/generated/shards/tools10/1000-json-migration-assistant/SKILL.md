---
name: json-migration-assistant
description: Use when tasks require json migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# JSON Migration Assistant
...
| Primary artifact | `json-migration-cutover-plan` |
| Routing tag | `json:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when state changes are hard to undo once they begin. This specific skill turns JSON Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from json..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| json-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| json-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for JSON Migration Assistant before any JSON rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from json. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `json-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] json-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:migration-assistant` consumers with approval and credential context attached.
