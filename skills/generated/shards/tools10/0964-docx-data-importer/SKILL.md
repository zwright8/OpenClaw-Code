---
name: docx-data-importer
description: Use when tasks require docx data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docx-import-ledger` |
| Routing tag | `docx:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when state changes are hard to undo once they begin. This specific skill turns DOCX Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into docx..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-import-ledger | structured-artifact | yes | next workflow or operator |
| docx-import-ledger-scorecard | scorecard | yes | reviewer |
| docx-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for DOCX Data Importer before any DOCX write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into docx. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `docx-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] docx-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:data-importer` consumers with approval and credential context attached.
