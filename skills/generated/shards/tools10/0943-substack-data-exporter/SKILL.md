---
name: substack-data-exporter
description: Use when tasks require substack data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Data Exporter
...
| Primary artifact | `substack-export-package` |
| Routing tag | `substack:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Data Exporter into a deterministic, auth-checked workflow for exports substack entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-export-package | structured-artifact | yes | next workflow or operator |
| substack-export-package-scorecard | scorecard | yes | reviewer |
| substack-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Substack Data Exporter, then constrain the export scope for Substack before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports substack entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `substack-export-package` so downstream owners do not need to rediscover context.
...
- [ ] substack-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:data-exporter` consumers with approval and credential context attached.
