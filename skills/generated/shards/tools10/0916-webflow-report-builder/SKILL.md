---
name: webflow-report-builder
description: Use when tasks require webflow report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Report Builder
...
| Primary artifact | `webflow-report-bundle` |
| Routing tag | `webflow:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from webflow activity..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-report-bundle | structured-artifact | yes | next workflow or operator |
| webflow-report-bundle-scorecard | scorecard | yes | reviewer |
| webflow-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Webflow Report Builder, then confirm which Webflow entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from webflow activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `webflow-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] webflow-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:report-builder` consumers with approval and credential context attached.
