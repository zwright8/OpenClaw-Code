...
| Field | Value |
|---|---|
| Skill pack | `b10-download-db-messaging-sec` |
| Tool count | `26` |
| Inventory rank span | `4691-4724` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for download pipelines, database operations, messaging, security scans tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `xurls` | `tz` | no | no |
| bounded mutation / transformation | `tz` | `xurls` | yes | yes |
| validation & artifact checks | `xurls` | `tz` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `xurls` | Extract urls from text | local-installable |
| `tz` | CLI time zone visualizer | local-installable |
| `mysqltuner` | Increase performance and stability of a MySQL installation | local-installable |
| `bsdiff` | Generate and apply patches to binary files | local-installable |
| `feluda` | Detect license usage restrictions in your project | local-installable |
| `cargo-auditable` | Make production Rust binaries auditable | local-installable |
| `ecm` | Prepare CD image files so they compress better | local-installable |
| `gopeed` | Modern download manager that supports all platform | local-installable |
| `libid3tag` | ID3 tag manipulation library | local-installable |
| `crow` | Fast and Easy to use microframework for the web | local-installable |
| `alive2` | Automatic verification of LLVM optimizations | local-installable |
| `webdav` | Simple and standalone WebDAV server | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `xurls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tz` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mysqltuner` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bsdiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `feluda` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-auditable` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ecm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gopeed` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libid3tag` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `crow` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `alive2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `webdav` | SFTP/FTP/WebDAV, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aicommits` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dblab` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cdogs-sdl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `center-im` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qdmr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `airspy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libgsm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `easyeda2kicad` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `png++` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `uni` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fuseki` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jenkins-job-builder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `retire` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cargo-release` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `xurls` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `tz` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `mysqltuner` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `bsdiff` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `feluda` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `cargo-auditable` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a download pipelines, database operations, messaging, security scans request, run dry-run first, then produce a validated artifact bundle.
