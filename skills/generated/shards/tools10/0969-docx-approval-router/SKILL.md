---
name: docx-approval-router
description: Use when tasks require docx approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DOCX Approval Router
...
| Primary artifact | `docx-approval-decision-packet` |
| Routing tag | `docx:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Approval Router into a deterministic, auth-checked workflow for routes docx-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| docx-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| docx-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for DOCX Approval Router, then capture the manual fallback path for DOCX.
...
3. Implement routes docx-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `docx-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] docx-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:approval-router` consumers with approval and credential context attached.
