---
name: pdf-data-exporter
description: Use when tasks require pdf data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pdf-export-package` |
| Routing tag | `pdf:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Data Exporter into a deterministic, auth-checked workflow for exports pdf entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-export-package | structured-artifact | yes | next workflow or operator |
| pdf-export-package-scorecard | scorecard | yes | reviewer |
| pdf-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for PDF Data Exporter, then constrain the export scope for PDF before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports pdf entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `pdf-export-package` so downstream owners do not need to rediscover context.
...
- [ ] pdf-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:data-exporter` consumers with approval and credential context attached.
