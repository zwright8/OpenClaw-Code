---
name: json-connector
description: Use when tasks require json connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `json-connection-trace-bundle` |
| Routing tag | `json:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Connector into a deterministic, auth-checked workflow for unified read/write connector for json apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| json-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| json-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for JSON Connector, then confirm the smallest read-only probe that proves access to JSON.
...
3. Specify the request and response contract for unified read/write connector for json apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `json-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] json-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:connector` consumers with approval and credential context attached.
