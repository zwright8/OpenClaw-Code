---
name: ghost-data-importer
description: Use when tasks require ghost data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `ghost-import-ledger` |
| Routing tag | `ghost:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when state changes are hard to undo once they begin. This specific skill turns Ghost Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into ghost..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-import-ledger | structured-artifact | yes | next workflow or operator |
| ghost-import-ledger-scorecard | scorecard | yes | reviewer |
| ghost-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Ghost Data Importer before any Ghost write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into ghost. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `ghost-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] ghost-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:data-importer` consumers with approval and credential context attached.
