---
name: webflow-approval-router
description: Use when tasks require webflow approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Approval Router
...
| Primary artifact | `webflow-approval-decision-packet` |
| Routing tag | `webflow:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Approval Router into a deterministic, auth-checked workflow for routes webflow-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| webflow-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| webflow-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Webflow Approval Router, then capture the manual fallback path for Webflow.
...
3. Implement routes webflow-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `webflow-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] webflow-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:approval-router` consumers with approval and credential context attached.
