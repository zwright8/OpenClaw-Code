---
name: substack-report-builder
description: Use when tasks require substack report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Report Builder
...
| Primary artifact | `substack-report-bundle` |
| Routing tag | `substack:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from substack activity..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-report-bundle | structured-artifact | yes | next workflow or operator |
| substack-report-bundle-scorecard | scorecard | yes | reviewer |
| substack-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Substack Report Builder, then confirm which Substack entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from substack activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `substack-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] substack-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:report-builder` consumers with approval and credential context attached.
