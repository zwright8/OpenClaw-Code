...
description: Use when tasks require json data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `json-export-package` |
| Routing tag | `json:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Data Exporter into a deterministic, auth-checked workflow for exports json entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-export-package | structured-artifact | yes | next workflow or operator |
| json-export-package-scorecard | scorecard | yes | reviewer |
| json-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for JSON Data Exporter, then constrain the export scope for JSON before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports json entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `json-export-package` so downstream owners do not need to rediscover context.
...
- [ ] json-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:data-exporter` consumers with approval and credential context attached.
