---
name: pdf-workflow-automator
description: Use when tasks require pdf workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PDF Workflow Automator
...
| Primary artifact | `pdf-workflow-runbook` |
| Routing tag | `pdf:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using pdf..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-workflow-runbook | structured-artifact | yes | next workflow or operator |
| pdf-workflow-runbook-scorecard | scorecard | yes | reviewer |
| pdf-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for PDF Workflow Automator, then checkpoint where PDF state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using pdf. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `pdf-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] pdf-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:workflow-automator` consumers with approval and credential context attached.
