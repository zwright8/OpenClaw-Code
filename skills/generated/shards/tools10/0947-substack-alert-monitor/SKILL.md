---
name: substack-alert-monitor
description: Use when tasks require substack alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Alert Monitor
...
| Primary artifact | `substack-alert-state-bundle` |
| Routing tag | `substack:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Alert Monitor into a deterministic, auth-checked workflow for monitors key substack signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| substack-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| substack-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Substack Alert Monitor, then lock the reviewed threshold set for Substack.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key substack signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `substack-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] substack-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:alert-monitor` consumers with approval and credential context attached.
