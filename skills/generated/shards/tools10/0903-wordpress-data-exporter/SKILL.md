---
name: wordpress-data-exporter
description: Use when tasks require wordpress data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Data Exporter
...
| Primary artifact | `wordpress-export-package` |
| Routing tag | `wordpress:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Data Exporter into a deterministic, auth-checked workflow for exports wordpress entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-export-package | structured-artifact | yes | next workflow or operator |
| wordpress-export-package-scorecard | scorecard | yes | reviewer |
| wordpress-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for WordPress Data Exporter, then constrain the export scope for WordPress before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports wordpress entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `wordpress-export-package` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:data-exporter` consumers with approval and credential context attached.
