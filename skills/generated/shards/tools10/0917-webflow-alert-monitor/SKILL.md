---
name: webflow-alert-monitor
description: Use when tasks require webflow alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Alert Monitor
...
| Primary artifact | `webflow-alert-state-bundle` |
| Routing tag | `webflow:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Alert Monitor into a deterministic, auth-checked workflow for monitors key webflow signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| webflow-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| webflow-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Webflow Alert Monitor, then lock the reviewed threshold set for Webflow.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key webflow signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `webflow-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] webflow-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:alert-monitor` consumers with approval and credential context attached.
