---
name: ghost-data-exporter
description: Use when tasks require ghost data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `ghost-export-package` |
| Routing tag | `ghost:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Data Exporter into a deterministic, auth-checked workflow for exports ghost entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-export-package | structured-artifact | yes | next workflow or operator |
| ghost-export-package-scorecard | scorecard | yes | reviewer |
| ghost-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Ghost Data Exporter, then constrain the export scope for Ghost before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports ghost entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `ghost-export-package` so downstream owners do not need to rediscover context.
...
- [ ] ghost-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:data-exporter` consumers with approval and credential context attached.
