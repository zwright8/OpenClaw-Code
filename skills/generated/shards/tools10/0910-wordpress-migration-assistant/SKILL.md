---
name: wordpress-migration-assistant
description: Use when tasks require wordpress migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `wordpress-migration-cutover-plan` |
| Routing tag | `wordpress:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when state changes are hard to undo once they begin. This specific skill turns WordPress Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from wordpress..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| wordpress-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| wordpress-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for WordPress Migration Assistant before any WordPress rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from wordpress. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `wordpress-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:migration-assistant` consumers with approval and credential context attached.
