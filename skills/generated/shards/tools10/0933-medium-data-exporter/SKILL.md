---
name: medium-data-exporter
description: Use when tasks require medium data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Data Exporter
...
| Primary artifact | `medium-export-package` |
| Routing tag | `medium:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Data Exporter into a deterministic, auth-checked workflow for exports medium entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-export-package | structured-artifact | yes | next workflow or operator |
| medium-export-package-scorecard | scorecard | yes | reviewer |
| medium-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Medium Data Exporter, then constrain the export scope for Medium before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports medium entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `medium-export-package` so downstream owners do not need to rediscover context.
...
- [ ] medium-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:data-exporter` consumers with approval and credential context attached.
