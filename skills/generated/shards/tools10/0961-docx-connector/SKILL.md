---
name: docx-connector
description: Use when tasks require docx connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `docx-connection-trace-bundle` |
| Routing tag | `docx:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Connector into a deterministic, auth-checked workflow for unified read/write connector for docx apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| docx-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| docx-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for DOCX Connector, then confirm the smallest read-only probe that proves access to DOCX.
...
3. Specify the request and response contract for unified read/write connector for docx apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `docx-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] docx-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:connector` consumers with approval and credential context attached.
