---
name: wordpress-sync-orchestrator
description: Use when tasks require wordpress sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Sync Orchestrator
...
| Primary artifact | `wordpress-reconciliation-bundle` |
| Routing tag | `wordpress:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for wordpress records..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| wordpress-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| wordpress-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for WordPress Sync Orchestrator, then checkpoint the last known good cursor on both sides of WordPress.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for wordpress records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `wordpress-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:sync-orchestrator` consumers with approval and credential context attached.
