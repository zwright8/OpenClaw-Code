...
description: Use when tasks require csv data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-import-ledger` |
| Routing tag | `csv:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when state changes are hard to undo once they begin. This specific skill turns CSV Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into csv..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-import-ledger | structured-artifact | yes | next workflow or operator |
| csv-import-ledger-scorecard | scorecard | yes | reviewer |
| csv-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for CSV Data Importer before any CSV write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into csv. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `csv-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] csv-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:data-importer` consumers with approval and credential context attached.
