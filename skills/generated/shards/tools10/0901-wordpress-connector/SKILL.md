...
description: Use when tasks require wordpress connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `wordpress-connection-trace-bundle` |
| Routing tag | `wordpress:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Connector into a deterministic, auth-checked workflow for unified read/write connector for wordpress apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| wordpress-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| wordpress-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for WordPress Connector, then confirm the smallest read-only probe that proves access to WordPress.
...
3. Specify the request and response contract for unified read/write connector for wordpress apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `wordpress-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:connector` consumers with approval and credential context attached.
