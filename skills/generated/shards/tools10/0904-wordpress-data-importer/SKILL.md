---
name: wordpress-data-importer
description: Use when tasks require wordpress data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `wordpress-import-ledger` |
| Routing tag | `wordpress:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when state changes are hard to undo once they begin. This specific skill turns WordPress Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into wordpress..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-import-ledger | structured-artifact | yes | next workflow or operator |
| wordpress-import-ledger-scorecard | scorecard | yes | reviewer |
| wordpress-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for WordPress Data Importer before any WordPress write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into wordpress. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `wordpress-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:data-importer` consumers with approval and credential context attached.
