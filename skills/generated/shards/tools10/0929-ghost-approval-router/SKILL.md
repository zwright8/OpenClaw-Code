---
name: ghost-approval-router
description: Use when tasks require ghost approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Approval Router
...
| Primary artifact | `ghost-approval-decision-packet` |
| Routing tag | `ghost:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Approval Router into a deterministic, auth-checked workflow for routes ghost-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| ghost-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| ghost-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Ghost Approval Router, then capture the manual fallback path for Ghost.
...
3. Implement routes ghost-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `ghost-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] ghost-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:approval-router` consumers with approval and credential context attached.
