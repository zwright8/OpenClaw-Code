---
name: webflow-data-importer
description: Use when tasks require webflow data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Data Importer
...
| Primary artifact | `webflow-import-ledger` |
| Routing tag | `webflow:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when state changes are hard to undo once they begin. This specific skill turns Webflow Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into webflow..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-import-ledger | structured-artifact | yes | next workflow or operator |
| webflow-import-ledger-scorecard | scorecard | yes | reviewer |
| webflow-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Webflow Data Importer before any Webflow write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into webflow. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `webflow-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] webflow-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:data-importer` consumers with approval and credential context attached.
