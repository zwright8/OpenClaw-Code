...
description: Use when tasks require csv data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-export-package` |
| Routing tag | `csv:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Data Exporter into a deterministic, auth-checked workflow for exports csv entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-export-package | structured-artifact | yes | next workflow or operator |
| csv-export-package-scorecard | scorecard | yes | reviewer |
| csv-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for CSV Data Exporter, then constrain the export scope for CSV before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports csv entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `csv-export-package` so downstream owners do not need to rediscover context.
...
- [ ] csv-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:data-exporter` consumers with approval and credential context attached.
