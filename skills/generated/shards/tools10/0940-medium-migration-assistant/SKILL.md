---
name: medium-migration-assistant
description: Use when tasks require medium migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Migration Assistant
...
| Primary artifact | `medium-migration-cutover-plan` |
| Routing tag | `medium:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when state changes are hard to undo once they begin. This specific skill turns Medium Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from medium..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| medium-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| medium-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Medium Migration Assistant before any Medium rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from medium. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `medium-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] medium-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:migration-assistant` consumers with approval and credential context attached.
