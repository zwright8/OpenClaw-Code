---
name: ghost-report-builder
description: Use when tasks require ghost report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Ghost Report Builder
...
| Primary artifact | `ghost-report-bundle` |
| Routing tag | `ghost:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Ghost workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Ghost Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from ghost activity..

## Trigger Checklist
- [ ] The task explicitly requires `Ghost Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ghost-report-bundle | structured-artifact | yes | next workflow or operator |
| ghost-report-bundle-scorecard | scorecard | yes | reviewer |
| ghost-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Ghost Report Builder, then confirm which Ghost entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from ghost activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `ghost-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ghost-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ghost-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ghost:report-builder` consumers with approval and credential context attached.
