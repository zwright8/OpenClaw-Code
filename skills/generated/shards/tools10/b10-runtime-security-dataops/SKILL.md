...
| Field | Value |
|---|---|
| Skill pack | `b10-runtime-security-dataops` |
| Tool count | `26` |
| Inventory rank span | `4564-4594` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for runtime toolchains, security controls, data operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `qttranslations` | `garble` | no | no |
| bounded mutation / transformation | `garble` | `qttranslations` | yes | yes |
| validation & artifact checks | `qttranslations` | `garble` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `qttranslations` | Qt translation catalogs | local-installable |
| `garble` | Obfuscate Go builds | local-installable |
| `astgen` | Generate AST in json format for JS/TS | local-installable |
| `orientdb` | Graph database | local-installable |
| `amber` | Crystal web framework. Bare metal performance, productivity and happiness | local-installable |
| `halide` | Language for fast, portable data-parallel computation | local-installable |
| `i686-elf-binutils` | GNU Binutils for i686-elf cross development | local-installable |
| `acl2` | Logic and programming language in which you can model computer systems | local-installable |
| `atkmm` | Official C++ interface for the ATK accessibility toolkit library | local-installable |
| `xbyak` | C++ JIT assembler for x86 (IA32), x64 (AMD64, x86-64) | local-installable |
| `xsimd` | Modern, portable C++ wrappers for SIMD intrinsics | local-installable |
| `s4cmd` | Super S3 command-line tool | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `qttranslations` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `garble` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `astgen` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `orientdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `amber` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `halide` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `i686-elf-binutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `acl2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `atkmm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `xbyak` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xsimd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `s4cmd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `grizzly` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `massdns` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libseccomp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `moodle-dl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fnt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `daq` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shimmy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `g-ls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lando-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `doltgres` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dezoomify-rs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `brogue` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `infat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cql` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `qttranslations` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `garble` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `astgen` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `orientdb` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `amber` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `halide` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a runtime toolchains, security controls, data operations request, run dry-run first, then produce a validated artifact bundle.
