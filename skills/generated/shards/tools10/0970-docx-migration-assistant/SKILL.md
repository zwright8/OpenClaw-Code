---
name: docx-migration-assistant
description: Use when tasks require docx migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DOCX Migration Assistant
...
| Primary artifact | `docx-migration-cutover-plan` |
| Routing tag | `docx:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when state changes are hard to undo once they begin. This specific skill turns DOCX Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from docx..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| docx-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| docx-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for DOCX Migration Assistant before any DOCX rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from docx. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `docx-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] docx-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:migration-assistant` consumers with approval and credential context attached.
