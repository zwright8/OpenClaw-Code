---
name: medium-webhook-listener
description: Use when tasks require medium webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Webhook Listener
...
| Primary artifact | `medium-event-intake-log` |
| Routing tag | `medium:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Medium Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes medium webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `MEDIUM_CLIENT_ID`, `MEDIUM_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `MEDIUM_TOKEN`, `MEDIUM_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `MEDIUM_WEBHOOK_SECRET`, `MEDIUM_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| medium-event-intake-log | structured-artifact | yes | next workflow or operator |
| medium-event-intake-log-scorecard | scorecard | yes | reviewer |
| medium-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Medium Webhook Listener, then register the signing-secret policy for Medium.
...
3. Implement captures, verifies, and routes medium webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `medium-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] medium-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:webhook-listener` consumers with approval and credential context attached.
