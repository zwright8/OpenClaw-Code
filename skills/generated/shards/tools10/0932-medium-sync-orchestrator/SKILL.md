---
name: medium-sync-orchestrator
description: Use when tasks require medium sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Sync Orchestrator
...
| Primary artifact | `medium-reconciliation-bundle` |
| Routing tag | `medium:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for medium records..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| medium-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| medium-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Medium Sync Orchestrator, then checkpoint the last known good cursor on both sides of Medium.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for medium records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `medium-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] medium-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:sync-orchestrator` consumers with approval and credential context attached.
