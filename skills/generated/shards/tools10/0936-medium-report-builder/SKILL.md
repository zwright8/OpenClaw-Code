---
name: medium-report-builder
description: Use when tasks require medium report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Report Builder
...
| Primary artifact | `medium-report-bundle` |
| Routing tag | `medium:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from medium activity..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-report-bundle | structured-artifact | yes | next workflow or operator |
| medium-report-bundle-scorecard | scorecard | yes | reviewer |
| medium-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Medium Report Builder, then confirm which Medium entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from medium activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `medium-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] medium-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:report-builder` consumers with approval and credential context attached.
