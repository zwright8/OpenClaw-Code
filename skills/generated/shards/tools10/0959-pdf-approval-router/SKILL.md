---
name: pdf-approval-router
description: Use when tasks require pdf approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pdf-approval-decision-packet` |
| Routing tag | `pdf:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Approval Router into a deterministic, auth-checked workflow for routes pdf-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| pdf-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| pdf-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for PDF Approval Router, then capture the manual fallback path for PDF.
...
3. Implement routes pdf-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `pdf-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] pdf-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:approval-router` consumers with approval and credential context attached.
