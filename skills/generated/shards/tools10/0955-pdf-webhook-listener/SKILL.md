---
name: pdf-webhook-listener
description: Use when tasks require pdf webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PDF Webhook Listener
...
| Primary artifact | `pdf-event-intake-log` |
| Routing tag | `pdf:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns PDF Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes pdf webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `PDF_CLIENT_ID`, `PDF_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `PDF_TOKEN`, `PDF_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `PDF_WEBHOOK_SECRET`, `PDF_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| pdf-event-intake-log | structured-artifact | yes | next workflow or operator |
| pdf-event-intake-log-scorecard | scorecard | yes | reviewer |
| pdf-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for PDF Webhook Listener, then register the signing-secret policy for PDF.
...
3. Implement captures, verifies, and routes pdf webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `pdf-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] pdf-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:webhook-listener` consumers with approval and credential context attached.
