...
| Field | Value |
|---|---|
| Skill pack | `b10-api-stream-db-container` |
| Tool count | `26` |
| Inventory rank span | `4533-4563` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for api testing, streaming data, container ops, database workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `dict` | `alpine` | no | no |
| bounded mutation / transformation | `alpine` | `dict` | yes | yes |
| validation & artifact checks | `dict` | `alpine` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `dict` | Dictionary Server Protocol (RFC2229) client | local-installable |
| `alpine` | News and email agent | local-installable |
| `bombardier` | Cross-platform HTTP benchmarking tool | local-installable |
| `chmlib` | Library for dealing with Microsoft ITSS/CHM files | local-installable |
| `flarectl` | CLI application for interacting with a Cloudflare account | local-installable |
| `dockly` | Immersive terminal interface for managing docker containers and services | local-installable |
| `slepc` | Scalable Library for Eigenvalue Problem Computations (real) | local-installable |
| `httpyac` | Quickly and easily send REST, SOAP, GraphQL and gRPC requests | local-installable |
| `nats-streaming-server` | Lightweight cloud messaging system | local-installable |
| `libsigc++` | Callback framework for C++ | local-installable |
| `libpanel` | Dock/panel library for GTK 4 | local-installable |
| `osmosis` | Command-line OpenStreetMap data processor | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `dict` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `alpine` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bombardier` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chmlib` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `flarectl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dockly` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `slepc` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `httpyac` | GraphQL over HTTPS, gRPC, QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nats-streaming-server` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libsigc++` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpanel` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `osmosis` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `oxen` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `plutoprint` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bfs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sqliteodbc` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `flix` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rethinkdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `yo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ntl` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `microsocks` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lue-reader` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcp-server-kubernetes` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `jpeginfo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bashunit` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `influxdb@2` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `dockly`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `mcp-server-kubernetes`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `dict` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `alpine` (JSON-RPC/WebSocket, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `bombardier` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `chmlib` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `flarectl` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `dockly` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a api testing, streaming data, container ops, database workflows request, run dry-run first, then produce a validated artifact bundle.
