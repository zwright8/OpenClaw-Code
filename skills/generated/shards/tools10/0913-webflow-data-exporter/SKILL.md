---
name: webflow-data-exporter
description: Use when tasks require webflow data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Webflow Data Exporter
...
| Primary artifact | `webflow-export-package` |
| Routing tag | `webflow:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Webflow workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Webflow Data Exporter into a deterministic, auth-checked workflow for exports webflow entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Webflow Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| webflow-export-package | structured-artifact | yes | next workflow or operator |
| webflow-export-package-scorecard | scorecard | yes | reviewer |
| webflow-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Webflow Data Exporter, then constrain the export scope for Webflow before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports webflow entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `webflow-export-package` so downstream owners do not need to rediscover context.
...
- [ ] webflow-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `webflow-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `webflow:data-exporter` consumers with approval and credential context attached.
