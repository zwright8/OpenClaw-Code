---
name: spreadsheet-approval-router
description: Use when tasks require spreadsheet approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Approval Router
...
| Primary artifact | `spreadsheet-approval-decision-packet` |
| Routing tag | `spreadsheet:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Spreadsheet Approval Router into a deterministic, auth-checked workflow for routes spreadsheet-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| spreadsheet-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| spreadsheet-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Spreadsheet Approval Router, then capture the manual fallback path for Spreadsheet.
...
3. Implement routes spreadsheet-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `spreadsheet-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:approval-router` consumers with approval and credential context attached.
