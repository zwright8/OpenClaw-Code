---
name: wordpress-workflow-automator
description: Use when tasks require wordpress workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WordPress Workflow Automator
...
| Primary artifact | `wordpress-workflow-runbook` |
| Routing tag | `wordpress:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WordPress workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WordPress Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using wordpress..

## Trigger Checklist
- [ ] The task explicitly requires `WordPress Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| wordpress-workflow-runbook | structured-artifact | yes | next workflow or operator |
| wordpress-workflow-runbook-scorecard | scorecard | yes | reviewer |
| wordpress-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for WordPress Workflow Automator, then checkpoint where WordPress state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using wordpress. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `wordpress-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] wordpress-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `wordpress-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `wordpress:workflow-automator` consumers with approval and credential context attached.
