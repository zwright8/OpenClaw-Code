...
description: Use when tasks require json data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `json-import-ledger` |
| Routing tag | `json:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when state changes are hard to undo once they begin. This specific skill turns JSON Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into json..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-import-ledger | structured-artifact | yes | next workflow or operator |
| json-import-ledger-scorecard | scorecard | yes | reviewer |
| json-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for JSON Data Importer before any JSON write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into json. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `json-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] json-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:data-importer` consumers with approval and credential context attached.
