---
name: csv-alert-monitor
description: Use when tasks require csv alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-alert-state-bundle` |
| Routing tag | `csv:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Alert Monitor into a deterministic, auth-checked workflow for monitors key csv signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| csv-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| csv-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for CSV Alert Monitor, then lock the reviewed threshold set for CSV.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key csv signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `csv-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] csv-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:alert-monitor` consumers with approval and credential context attached.
