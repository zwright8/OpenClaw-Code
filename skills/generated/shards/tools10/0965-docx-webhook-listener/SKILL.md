---
name: docx-webhook-listener
description: Use when tasks require docx webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DOCX Webhook Listener
...
| Primary artifact | `docx-event-intake-log` |
| Routing tag | `docx:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns DOCX Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes docx webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `DOCX_CLIENT_ID`, `DOCX_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `DOCX_TOKEN`, `DOCX_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DOCX_WEBHOOK_SECRET`, `DOCX_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| docx-event-intake-log | structured-artifact | yes | next workflow or operator |
| docx-event-intake-log-scorecard | scorecard | yes | reviewer |
| docx-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for DOCX Webhook Listener, then register the signing-secret policy for DOCX.
...
3. Implement captures, verifies, and routes docx webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `docx-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] docx-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:webhook-listener` consumers with approval and credential context attached.
