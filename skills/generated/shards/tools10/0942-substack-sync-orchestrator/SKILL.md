---
name: substack-sync-orchestrator
description: Use when tasks require substack sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Sync Orchestrator
...
| Primary artifact | `substack-reconciliation-bundle` |
| Routing tag | `substack:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for substack records..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| substack-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| substack-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Substack Sync Orchestrator, then checkpoint the last known good cursor on both sides of Substack.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for substack records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `substack-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] substack-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:sync-orchestrator` consumers with approval and credential context attached.
