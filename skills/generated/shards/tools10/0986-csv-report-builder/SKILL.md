...
description: Use when tasks require csv report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-report-bundle` |
| Routing tag | `csv:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from csv activity..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-report-bundle | structured-artifact | yes | next workflow or operator |
| csv-report-bundle-scorecard | scorecard | yes | reviewer |
| csv-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for CSV Report Builder, then confirm which CSV entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from csv activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `csv-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] csv-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:report-builder` consumers with approval and credential context attached.
