---
name: docx-workflow-automator
description: Use when tasks require docx workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# DOCX Workflow Automator
...
| Primary artifact | `docx-workflow-runbook` |
| Routing tag | `docx:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because DOCX workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns DOCX Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using docx..

## Trigger Checklist
- [ ] The task explicitly requires `DOCX Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| docx-workflow-runbook | structured-artifact | yes | next workflow or operator |
| docx-workflow-runbook-scorecard | scorecard | yes | reviewer |
| docx-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for DOCX Workflow Automator, then checkpoint where DOCX state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using docx. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `docx-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] docx-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `docx-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `docx:workflow-automator` consumers with approval and credential context attached.
