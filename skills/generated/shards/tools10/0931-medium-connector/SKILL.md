---
name: medium-connector
description: Use when tasks require medium connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `medium-connection-trace-bundle` |
| Routing tag | `medium:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Connector into a deterministic, auth-checked workflow for unified read/write connector for medium apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| medium-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| medium-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Medium Connector, then confirm the smallest read-only probe that proves access to Medium.
...
3. Specify the request and response contract for unified read/write connector for medium apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `medium-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] medium-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:connector` consumers with approval and credential context attached.
