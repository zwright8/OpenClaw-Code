...
| Field | Value |
|---|---|
| Skill pack | `b10-terminal-java-media-forensics` |
| Tool count | `27` |
| Inventory rank span | `4967-5000` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for terminal system control, java/runtime stacks, media extraction, forensics tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `wmctrl` | `hapi-fhir-cli` | no | no |
| bounded mutation / transformation | `hapi-fhir-cli` | `wmctrl` | yes | yes |
| validation & artifact checks | `wmctrl` | `hapi-fhir-cli` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `wmctrl` | UNIX/Linux command-line tool to interact with an EWMH/NetWM | local-installable |
| `hapi-fhir-cli` | Command-line interface for the HAPI FHIR library | local-installable |
| `openj9` | High performance, scalable, Java virtual machine | local-installable |
| `acronym` | Python-based tool for creating English-ish acronyms from your fancy project | local-installable |
| `superseedr` | BitTorrent Client in your Terminal | local-installable |
| `gearman` | Application framework to farm out work to other machines or processes | local-installable |
| `dash-mpd-cli` | Download media content from a DASH-MPEG or DASH-WebM MPD manifest | local-installable |
| `wifitui` | Fast featureful friendly wifi terminal UI | local-installable |
| `tree-sitter@0.25` | Incremental parsing library | local-installable |
| `cpm` | Fast CPAN module installer | local-installable |
| `khard` | Console carddav client | local-installable |
| `qt-libiodbc` | Qt SQL Database Driver | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `wmctrl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hapi-fhir-cli` | HTTPS/REST, Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `openj9` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `acronym` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `superseedr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gearman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dash-mpd-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `wifitui` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tree-sitter@0.25` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `cpm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `khard` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qt-libiodbc` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `koka` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sandvault` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `animdl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gucharmap` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gifcap` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-c-auth` | JSON-RPC/WebSocket, Local library API/ABI, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `fast_float` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chainsaw` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ccextractor` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ebook-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnmic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cadence` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `miniflux` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sloccount` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-c-auth`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `wmctrl` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `hapi-fhir-cli` (HTTPS/REST, Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `openj9` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `acronym` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `superseedr` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `gearman` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a terminal system control, java/runtime stacks, media extraction, forensics request, run dry-run first, then produce a validated artifact bundle.
