---
name: substack-data-importer
description: Use when tasks require substack data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Data Importer
...
| Primary artifact | `substack-import-ledger` |
| Routing tag | `substack:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when state changes are hard to undo once they begin. This specific skill turns Substack Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into substack..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-import-ledger | structured-artifact | yes | next workflow or operator |
| substack-import-ledger-scorecard | scorecard | yes | reviewer |
| substack-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Substack Data Importer before any Substack write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into substack. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `substack-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] substack-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:data-importer` consumers with approval and credential context attached.
