---
name: json-approval-router
description: Use when tasks require json approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# JSON Approval Router
...
| Primary artifact | `json-approval-decision-packet` |
| Routing tag | `json:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because JSON workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns JSON Approval Router into a deterministic, auth-checked workflow for routes json-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `JSON Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| json-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| json-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| json-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for JSON Approval Router, then capture the manual fallback path for JSON.
...
3. Implement routes json-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `json-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] json-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `json-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `json:approval-router` consumers with approval and credential context attached.
