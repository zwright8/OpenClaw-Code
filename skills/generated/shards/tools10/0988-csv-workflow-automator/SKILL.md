---
name: csv-workflow-automator
description: Use when tasks require csv workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `csv-workflow-runbook` |
| Routing tag | `csv:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because CSV workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns CSV Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using csv..

## Trigger Checklist
- [ ] The task explicitly requires `CSV Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| csv-workflow-runbook | structured-artifact | yes | next workflow or operator |
| csv-workflow-runbook-scorecard | scorecard | yes | reviewer |
| csv-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for CSV Workflow Automator, then checkpoint where CSV state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using csv. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `csv-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] csv-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `csv-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `csv:workflow-automator` consumers with approval and credential context attached.
