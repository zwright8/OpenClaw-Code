---
name: spreadsheet-migration-assistant
description: Use when tasks require spreadsheet migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Spreadsheet Migration Assistant
...
| Primary artifact | `spreadsheet-migration-cutover-plan` |
| Routing tag | `spreadsheet:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Spreadsheet workflows degrade when state changes are hard to undo once they begin. This specific skill turns Spreadsheet Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from spreadsheet..

## Trigger Checklist
- [ ] The task explicitly requires `Spreadsheet Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| spreadsheet-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| spreadsheet-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| spreadsheet-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Spreadsheet Migration Assistant before any Spreadsheet rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from spreadsheet. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `spreadsheet-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] spreadsheet-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `spreadsheet-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `spreadsheet:migration-assistant` consumers with approval and credential context attached.
