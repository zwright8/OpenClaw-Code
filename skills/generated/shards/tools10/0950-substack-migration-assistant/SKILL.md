---
name: substack-migration-assistant
description: Use when tasks require substack migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Migration Assistant
...
| Primary artifact | `substack-migration-cutover-plan` |
| Routing tag | `substack:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when state changes are hard to undo once they begin. This specific skill turns Substack Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from substack..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| substack-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| substack-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Substack Migration Assistant before any Substack rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from substack. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `substack-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] substack-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:migration-assistant` consumers with approval and credential context attached.
