---
name: substack-webhook-listener
description: Use when tasks require substack webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Substack Webhook Listener
...
| Primary artifact | `substack-event-intake-log` |
| Routing tag | `substack:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Substack Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes substack webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SUBSTACK_CLIENT_ID`, `SUBSTACK_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SUBSTACK_TOKEN`, `SUBSTACK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SUBSTACK_WEBHOOK_SECRET`, `SUBSTACK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| substack-event-intake-log | structured-artifact | yes | next workflow or operator |
| substack-event-intake-log-scorecard | scorecard | yes | reviewer |
| substack-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Substack Webhook Listener, then register the signing-secret policy for Substack.
...
3. Implement captures, verifies, and routes substack webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `substack-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] substack-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:webhook-listener` consumers with approval and credential context attached.
