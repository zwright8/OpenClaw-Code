---
name: medium-approval-router
description: Use when tasks require medium approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Medium Approval Router
...
| Primary artifact | `medium-approval-decision-packet` |
| Routing tag | `medium:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Medium workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Medium Approval Router into a deterministic, auth-checked workflow for routes medium-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Medium Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| medium-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| medium-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| medium-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Medium Approval Router, then capture the manual fallback path for Medium.
...
3. Implement routes medium-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `medium-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] medium-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `medium-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `medium:approval-router` consumers with approval and credential context attached.
