...
description: Use when tasks require substack connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `substack-connection-trace-bundle` |
| Routing tag | `substack:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Substack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Substack Connector into a deterministic, auth-checked workflow for unified read/write connector for substack apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Substack Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| substack-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| substack-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| substack-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Substack Connector, then confirm the smallest read-only probe that proves access to Substack.
...
3. Specify the request and response contract for unified read/write connector for substack apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `substack-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] substack-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `substack-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `substack:connector` consumers with approval and credential context attached.
