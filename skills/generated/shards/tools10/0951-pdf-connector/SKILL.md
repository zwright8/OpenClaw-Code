---
name: pdf-connector
description: Use when tasks require pdf connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pdf-connection-trace-bundle` |
| Routing tag | `pdf:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PDF workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PDF Connector into a deterministic, auth-checked workflow for unified read/write connector for pdf apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `PDF Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pdf-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| pdf-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| pdf-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for PDF Connector, then confirm the smallest read-only probe that proves access to PDF.
...
3. Specify the request and response contract for unified read/write connector for pdf apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `pdf-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pdf-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pdf-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pdf:connector` consumers with approval and credential context attached.
