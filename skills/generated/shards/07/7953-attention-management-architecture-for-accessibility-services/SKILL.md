...
description: Operate the "Attention Management Architecture for accessibility services" capability in production for accessibility services workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Attention Management Architecture for accessibility services** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **accessibility services" capability in production for accessibility services workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new accessibility services" capability in production for accessibility services workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in accessibility services" capability in production for accessibility services workflows**
   - Input: noisy upstream payload requiring attention management architecture for accessibility services normalization/assessment.
...
## Handoff Contract
- Produces: `Attention Management Architecture for accessibility services` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **accessibility services" capability in production for accessibility services workflows** depends on **Attention Management Architecture for accessibility services** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
