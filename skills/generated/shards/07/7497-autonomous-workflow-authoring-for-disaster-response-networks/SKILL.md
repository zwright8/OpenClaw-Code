...
## When To Use
Use this skill only when the request explicitly needs `Autonomous Workflow Authoring for disaster response networks` in disaster response networks and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Autonomous Workflow Authoring for disaster response networks** outcomes (not generic brainstorming).
...
1. **Incident stabilization in disaster response networks" capability for disaster response networks**
   - Input: noisy upstream payload requiring autonomous workflow authoring for disaster response networks normalization/assessment.
