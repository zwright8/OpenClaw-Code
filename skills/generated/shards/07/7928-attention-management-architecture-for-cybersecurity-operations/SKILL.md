...
description: Operate the "Attention Management Architecture for cybersecurity operations" capability in production for cybersecurity operations workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Attention Management Architecture for cybersecurity operations** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **cybersecurity operations" capability in production for cybersecurity operations workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new cybersecurity operations" capability in production for cybersecurity operations workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in cybersecurity operations" capability in production for cybersecurity operations workflows**
   - Input: noisy upstream payload requiring attention management architecture for cybersecurity operations normalization/assessment.
...
## Handoff Contract
- Produces: `Attention Management Architecture for cybersecurity operations` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **cybersecurity operations" capability in production for cybersecurity operations workflows** depends on **Attention Management Architecture for cybersecurity operations** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
