...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for mental well-being practices` in mental well-being practices and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for mental well-being practices** outcomes (not generic brainstorming).
...
1. **Incident stabilization in mental well-being practices" capability in production for mental well-being practices workflows**
   - Input: noisy upstream payload requiring resource budget allocation for mental well-being practices normalization/assessment.
