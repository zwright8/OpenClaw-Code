---
name: u01684-human-approval-routing-for-healthcare-operations
description: Run the Human Approval Routing for healthcare operations capability for healthcare operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---

# Human Approval Routing for healthcare operations
...
| Primary artifact | `human-approval-routing-artifact-healthcare-operations` |
| Routing tag | `healthcare-operations:generalist-engine` |
| Feature flag | `skill_01684_human-approval-routing` |
| Release cycles | `2` |

## Why This Skill Exists
Use human approval routing in healthcare operations with emphasis on throughput, reliability, leverage, and execution speed.

## Trigger Checklist
- [ ] The task explicitly needs Human Approval Routing for healthcare operations (not generic brainstorming).
...
|---|---|---|---|
| human-approval-routing-artifact-healthcare-operations | structured-artifact | yes | downstream orchestrator |
| human-approval-routing-artifact-healthcare-operations-scorecard | scorecard | yes | operator / reviewer |
| human-approval-routing-artifact-healthcare-operations-handoff | handoff-packet | yes | next owner |

## Implementation Guide
1. Define measurable outcomes for Human Approval Routing for healthcare operations, including baseline and target metrics for healthcare operations.
...
5. Run unit, integration, simulation, and regression suites for Human Approval Routing for healthcare operations under hyper-productive autonomy conditions.
...
## Operator Use Cases
- Run Human Approval Routing for healthcare operations as a repeatable production workflow for humans and agents.
- Use Human Approval Routing for healthcare operations to accelerate decisions while preserving safety, quality, and auditability.

## Guardrail Policy Matrix
| Guardrail Type | Policy Rule | Automation Hook |
|---|---|---|
| quality | Require unit and integration validations before promoting Human Approval Routing for healthcare operations. | run-validation:unit+integration+simulation+regression-baseline |
...
## Traceability Map
- **Scope:** Define measurable outcomes for Human Approval Routing for healthcare operations, including baseline and target metrics for healthcare operations.
...
- **Validation:** Run unit, integration, simulation, and regression suites for Human Approval Routing for healthcare operations under hyper-productive autonomy conditions.
...
1. `OPA/Rego` -> auth preflight, execute read+write/orchestrate call(s), normalize output, and attach trace to `human-approval-routing-artifact-healthcare-operations`.
2. `Vault/KMS` -> auth preflight, execute read+write/orchestrate call(s), normalize output, and attach trace to `human-approval-routing-artifact-healthcare-operations`.
3. `SIEM (Elastic/Splunk)` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `human-approval-routing-artifact-healthcare-operations`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `human-approval-routing-artifact-healthcare-operations`.
...
1. Incident recovery in healthcare operations: ingest noisy signals, execute human approval routing, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Human Approval Routing for healthcare operations against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
...
## Handoff Contract
- **Produces:** `human-approval-routing-artifact-healthcare-operations`, scorecard, risk/confidence metadata, remediation backlog.
