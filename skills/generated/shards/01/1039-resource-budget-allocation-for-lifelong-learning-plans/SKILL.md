...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for lifelong learning plans` in lifelong learning plans and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for lifelong learning plans** outcomes (not generic brainstorming).
...
1. **Incident stabilization in lifelong learning plans" capability in production for lifelong learning plans workflows**
   - Input: noisy upstream payload requiring resource budget allocation for lifelong learning plans normalization/assessment.
