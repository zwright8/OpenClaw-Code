...
description: Operate the "Risk-Aware Scheduling for disaster response networks" capability in production for disaster response networks workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for disaster response networks** outcomes (not generic brainstorming).
...
1. **Incident stabilization in disaster response networks" capability in production for disaster response networks workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for disaster response networks normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for disaster response networks` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **disaster response networks" capability in production for disaster response networks workflows** depends on **Risk-Aware Scheduling for disaster response networks** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
