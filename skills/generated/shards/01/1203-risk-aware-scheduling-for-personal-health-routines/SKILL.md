...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for personal health routines** outcomes (not generic brainstorming).
...
1. **Incident stabilization in personal health routines" capability in production for personal health routines workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for personal health routines normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for personal health routines` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **personal health routines" capability in production for personal health routines workflows** depends on **Risk-Aware Scheduling for personal health routines** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
