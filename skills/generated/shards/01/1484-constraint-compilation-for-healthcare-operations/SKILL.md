---
name: u01484-constraint-compilation-for-healthcare-operations
description: Run the Constraint Compilation for healthcare operations capability for healthcare operations with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Primary artifact | `constraint-compilation-artifact-healthcare-operations` |
| Routing tag | `healthcare-operations:generalist-engine` |
| Feature flag | `skill_01484_constraint-compilation` |
| Release cycles | `2` |

## Why This Skill Exists
Use constraint compilation in healthcare operations with emphasis on throughput, reliability, leverage, and execution speed.

## Trigger Checklist
- [ ] The task explicitly needs Constraint Compilation for healthcare operations (not generic brainstorming).
...
|---|---|---|---|
| constraint-compilation-artifact-healthcare-operations | structured-artifact | yes | downstream orchestrator |
| constraint-compilation-artifact-healthcare-operations-scorecard | scorecard | yes | operator / reviewer |
| constraint-compilation-artifact-healthcare-operations-handoff | handoff-packet | yes | next owner |
...
5. Run unit, integration, simulation, and regression suites for Constraint Compilation for healthcare operations under hyper-productive autonomy conditions.
...
## Operator Use Cases
- Run Constraint Compilation for healthcare operations as a repeatable production workflow for humans and agents.
- Use Constraint Compilation for healthcare operations to accelerate decisions while preserving safety, quality, and auditability.
...
## Traceability Map
- **Scope:** Define measurable outcomes for Constraint Compilation for healthcare operations, including baseline and target metrics for healthcare operations.
...
- **Validation:** Run unit, integration, simulation, and regression suites for Constraint Compilation for healthcare operations under hyper-productive autonomy conditions.
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `constraint-compilation-artifact-healthcare-operations`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `constraint-compilation-artifact-healthcare-operations`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `constraint-compilation-artifact-healthcare-operations`.
4. `Audit log + immutable storage` -> auth preflight, execute read/query call(s), normalize output, and attach trace to `constraint-compilation-artifact-healthcare-operations`.
...
1. Incident recovery in healthcare operations: ingest noisy signals, execute constraint compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Constraint Compilation for healthcare operations against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
...
## Handoff Contract
- **Produces:** `constraint-compilation-artifact-healthcare-operations`, scorecard, risk/confidence metadata, remediation backlog.
