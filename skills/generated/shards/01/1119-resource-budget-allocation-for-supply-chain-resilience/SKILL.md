...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for supply chain resilience` in supply chain resilience and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for supply chain resilience** outcomes (not generic brainstorming).
...
1. **Incident stabilization in supply chain resilience" capability for supply chain resilience**
   - Input: noisy upstream payload requiring resource budget allocation for supply chain resilience normalization/assessment.
