...
description: Operate the "Resource Budget Allocation for accessibility services" capability in production for accessibility services workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for accessibility services** outcomes (not generic brainstorming).
...
1. **Incident stabilization in accessibility services" capability in production for accessibility services workflows**
   - Input: noisy upstream payload requiring resource budget allocation for accessibility services normalization/assessment.
...
## Handoff Contract
- Produces: `Resource Budget Allocation for accessibility services` execution artifacts + scorecard + risk/confidence metadata.
