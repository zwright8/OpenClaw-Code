...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for nonprofit program delivery** outcomes (not generic brainstorming).
...
1. **Incident stabilization in nonprofit program delivery" capability in production for nonprofit program delivery workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for nonprofit program delivery normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for nonprofit program delivery` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **nonprofit program delivery" capability in production for nonprofit program delivery workflows** depends on **Risk-Aware Scheduling for nonprofit program delivery** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
