...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for fitness and recovery training` in fitness and recovery training and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for fitness and recovery training** outcomes (not generic brainstorming).
...
1. **Incident stabilization in fitness and recovery training" capability in production for fitness and recovery training workflows**
   - Input: noisy upstream payload requiring resource budget allocation for fitness and recovery training normalization/assessment.
