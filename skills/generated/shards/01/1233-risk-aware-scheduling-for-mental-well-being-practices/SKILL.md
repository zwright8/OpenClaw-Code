...
description: Operate the "Risk-Aware Scheduling for mental well-being practices" capability in production for mental well-being practices workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for mental well-being practices** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **mental well-being practices" capability in production for mental well-being practices workflows** with measurable acceptance criteria.
...
- **Daily:** Run when new mental well-being practices" capability in production for mental well-being practices workflows signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in mental well-being practices" capability in production for mental well-being practices workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for mental well-being practices normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for mental well-being practices` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **mental well-being practices" capability in production for mental well-being practices workflows** depends on **Risk-Aware Scheduling for mental well-being practices** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
