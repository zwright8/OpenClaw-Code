...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for cybersecurity operations` in cybersecurity operations and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for cybersecurity operations** outcomes (not generic brainstorming).
...
1. **Incident stabilization in cybersecurity operations" capability for cybersecurity operations**
   - Input: noisy upstream payload requiring resource budget allocation for cybersecurity operations normalization/assessment.
