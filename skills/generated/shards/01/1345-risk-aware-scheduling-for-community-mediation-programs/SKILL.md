...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for community mediation programs** outcomes (not generic brainstorming).
...
1. **Incident stabilization in community mediation programs" capability in production for community mediation programs workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for community mediation programs normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for community mediation programs` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **community mediation programs" capability in production for community mediation programs workflows** depends on **Risk-Aware Scheduling for community mediation programs** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
