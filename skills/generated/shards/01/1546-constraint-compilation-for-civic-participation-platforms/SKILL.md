---
name: u01546-constraint-compilation-for-civic-participation-platforms
description: Run the Constraint Compilation for civic participation platforms capability for civic participation platforms with deterministic outputs, policy-gated release, and handoff-ready operational artifacts. Use when mission execution explicitly requires this capability.
---
...
| Feature flag | `skill_01546_constraint-compilation` |
| Release cycles | `2` |

## Why This Skill Exists
Use constraint compilation in civic participation platforms with emphasis on evidence quality, falsifiability, and calibration.

## Trigger Checklist
- [ ] The task explicitly needs Constraint Compilation for civic participation platforms (not generic brainstorming).
...
|---|---|---|---|
| constraint-compilation-artifact-civic-participation-platforms | structured-artifact | yes | downstream orchestrator |
| constraint-compilation-artifact-civic-participation-platforms-scorecard | scorecard | yes | operator / reviewer |
| constraint-compilation-artifact-civic-participation-platforms-handoff | handoff-packet | yes | next owner |
...
5. Run unit, integration, simulation, and regression suites for Constraint Compilation for civic participation platforms under maximally truth-seeking conditions.
...
## Operator Use Cases
- Run Constraint Compilation for civic participation platforms as a repeatable production workflow for humans and agents.
- Use Constraint Compilation for civic participation platforms to accelerate decisions while preserving safety, quality, and auditability.
...
## Traceability Map
- **Scope:** Define measurable outcomes for Constraint Compilation for civic participation platforms, including baseline and target metrics for civic participation platforms.
...
- **Validation:** Run unit, integration, simulation, and regression suites for Constraint Compilation for civic participation platforms under maximally truth-seeking conditions.
...
1. `Frontier model runtime` -> auth preflight, execute model inference call(s), normalize output, and attach trace to `constraint-compilation-artifact-civic-participation-platforms`.
2. `Task/workflow orchestrator` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `constraint-compilation-artifact-civic-participation-platforms`.
3. `Telemetry store` -> auth preflight, execute read+write call(s), normalize output, and attach trace to `constraint-compilation-artifact-civic-participation-platforms`.
...
1. Incident recovery in civic participation platforms: ingest noisy signals, execute constraint compilation, produce an operator-ready scorecard and remediation queue.
2. Scheduled quality pass: run Constraint Compilation for civic participation platforms against baseline data, compare drift, and publish release/no-release recommendation with evidence links.
...
## Handoff Contract
- **Produces:** `constraint-compilation-artifact-civic-participation-platforms`, scorecard, risk/confidence metadata, remediation backlog.
