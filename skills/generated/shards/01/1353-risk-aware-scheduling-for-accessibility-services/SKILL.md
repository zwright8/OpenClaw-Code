...
## Trigger Checklist
- [ ] The request explicitly needs **Risk-Aware Scheduling for accessibility services** outcomes (not generic brainstorming).
...
1. **Incident stabilization in accessibility services" capability in production for accessibility services workflows**
   - Input: noisy upstream payload requiring risk-aware scheduling for accessibility services normalization/assessment.
...
## Handoff Contract
- Produces: `Risk-Aware Scheduling for accessibility services` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **accessibility services" capability in production for accessibility services workflows** depends on **Risk-Aware Scheduling for accessibility services** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
