...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for community mediation programs` in community mediation programs and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for community mediation programs** outcomes (not generic brainstorming).
...
1. **Incident stabilization in community mediation programs" capability for community mediation programs**
   - Input: noisy upstream payload requiring resource budget allocation for community mediation programs normalization/assessment.
