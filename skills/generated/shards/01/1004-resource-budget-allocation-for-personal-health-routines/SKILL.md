...
## When To Use
Use this skill only when the request explicitly needs `Resource Budget Allocation for personal health routines` in personal health routines and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for personal health routines** outcomes (not generic brainstorming).
...
1. **Incident stabilization in personal health routines" capability in production for personal health routines workflows**
   - Input: noisy upstream payload requiring resource budget allocation for personal health routines normalization/assessment.
