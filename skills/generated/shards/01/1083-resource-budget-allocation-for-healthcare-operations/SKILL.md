...
description: Build and operate the "Resource Budget Allocation for healthcare operations" capability for healthcare operations. Use when this exact capability is required by autonomous or human-guided missions.
---
...
## Trigger Checklist
- [ ] The request explicitly needs **Resource Budget Allocation for healthcare operations** outcomes (not generic brainstorming).
- [ ] Inputs are sufficient to execute in **healthcare operations" capability for healthcare operations** with measurable acceptance criteria.
...
- **Daily:** Run when new healthcare operations" capability for healthcare operations signals arrive or when active decisions depend on this capability.
...
1. **Incident stabilization in healthcare operations" capability for healthcare operations**
   - Input: noisy upstream payload requiring resource budget allocation for healthcare operations normalization/assessment.
...
## Handoff Contract
- Produces: `Resource Budget Allocation for healthcare operations` execution artifacts + scorecard + risk/confidence metadata.
...
## When To Use
Use this when a request in **healthcare operations" capability for healthcare operations** depends on **Resource Budget Allocation for healthcare operations** outcomes with explicit acceptance criteria. Do not use for unconstrained ideation; route discovery work before invoking this execution skill.
