...
description: Use when tasks require bitbucket webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bitbucket-event-intake-log` |
| Routing tag | `bitbucket:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Bitbucket Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes bitbucket webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Access token or personal access token | `token` | `BITBUCKET_TOKEN`, `BITBUCKET_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `BITBUCKET_WEBHOOK_SECRET`, `BITBUCKET_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| bitbucket-event-intake-log | structured-artifact | yes | next workflow or operator |
| bitbucket-event-intake-log-scorecard | scorecard | yes | reviewer |
| bitbucket-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Bitbucket Webhook Listener, then register the signing-secret policy for Bitbucket.
...
3. Implement captures, verifies, and routes bitbucket webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `bitbucket-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:webhook-listener` consumers with approval and credential context attached.
