...
description: Use when tasks require notion workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-workflow-runbook` |
| Routing tag | `notion:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using notion..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-workflow-runbook | structured-artifact | yes | next workflow or operator |
| notion-workflow-runbook-scorecard | scorecard | yes | reviewer |
| notion-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Notion Workflow Automator, then checkpoint where Notion state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using notion. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `notion-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] notion-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:workflow-automator` consumers with approval and credential context attached.
