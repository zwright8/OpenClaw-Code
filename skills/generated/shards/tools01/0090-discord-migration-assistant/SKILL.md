---
name: discord-migration-assistant
description: Use when tasks require discord migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `discord-migration-cutover-plan` |
| Routing tag | `discord:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when state changes are hard to undo once they begin. This specific skill turns Discord Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from discord..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| discord-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| discord-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Discord Migration Assistant before any Discord rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from discord. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `discord-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] discord-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:migration-assistant` consumers with approval and credential context attached.
