---
name: jira-data-importer
description: Use when tasks require jira data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-import-ledger` |
| Routing tag | `jira:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when state changes are hard to undo once they begin. This specific skill turns Jira Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into jira..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-import-ledger | structured-artifact | yes | next workflow or operator |
| jira-import-ledger-scorecard | scorecard | yes | reviewer |
| jira-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Jira Data Importer before any Jira write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into jira. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `jira-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] jira-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:data-importer` consumers with approval and credential context attached.
