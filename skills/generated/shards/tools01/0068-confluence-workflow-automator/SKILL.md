---
name: confluence-workflow-automator
description: Use when tasks require confluence workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Confluence Workflow Automator
...
| Primary artifact | `confluence-workflow-runbook` |
| Routing tag | `confluence:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using confluence..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-workflow-runbook | structured-artifact | yes | next workflow or operator |
| confluence-workflow-runbook-scorecard | scorecard | yes | reviewer |
| confluence-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Confluence Workflow Automator, then checkpoint where Confluence state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using confluence. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `confluence-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] confluence-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:workflow-automator` consumers with approval and credential context attached.
