---
name: discord-data-exporter
description: Use when tasks require discord data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `discord-export-package` |
| Routing tag | `discord:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Data Exporter into a deterministic, auth-checked workflow for exports discord entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-export-package | structured-artifact | yes | next workflow or operator |
| discord-export-package-scorecard | scorecard | yes | reviewer |
| discord-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Discord Data Exporter, then constrain the export scope for Discord before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports discord entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `discord-export-package` so downstream owners do not need to rediscover context.
...
- [ ] discord-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:data-exporter` consumers with approval and credential context attached.
