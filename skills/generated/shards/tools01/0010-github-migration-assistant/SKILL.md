---
name: github-migration-assistant
description: Use when tasks require github migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitHub Migration Assistant
...
| Primary artifact | `github-migration-cutover-plan` |
| Routing tag | `github:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when state changes are hard to undo once they begin. This specific skill turns GitHub Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from github..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| github-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| github-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for GitHub Migration Assistant before any GitHub rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from github. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `github-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] github-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:migration-assistant` consumers with approval and credential context attached.
