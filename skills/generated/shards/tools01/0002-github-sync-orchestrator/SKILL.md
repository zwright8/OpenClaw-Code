...
description: Use when tasks require github sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-reconciliation-bundle` |
| Routing tag | `github:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for github records..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| github-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| github-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for GitHub Sync Orchestrator, then checkpoint the last known good cursor on both sides of GitHub.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for github records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `github-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] github-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:sync-orchestrator` consumers with approval and credential context attached.
