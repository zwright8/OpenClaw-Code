---
name: confluence-alert-monitor
description: Use when tasks require confluence alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `confluence-alert-state-bundle` |
| Routing tag | `confluence:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Alert Monitor into a deterministic, auth-checked workflow for monitors key confluence signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| confluence-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| confluence-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Confluence Alert Monitor, then lock the reviewed threshold set for Confluence.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key confluence signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `confluence-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] confluence-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:alert-monitor` consumers with approval and credential context attached.
