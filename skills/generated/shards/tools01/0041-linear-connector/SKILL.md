...
description: Use when tasks require linear connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `linear-connection-trace-bundle` |
| Routing tag | `linear:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Connector into a deterministic, auth-checked workflow for unified read/write connector for linear apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| linear-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| linear-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Linear Connector, then confirm the smallest read-only probe that proves access to Linear.
...
3. Specify the request and response contract for unified read/write connector for linear apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `linear-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linear-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:connector` consumers with approval and credential context attached.
