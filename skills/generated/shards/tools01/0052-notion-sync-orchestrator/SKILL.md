---
name: notion-sync-orchestrator
description: Use when tasks require notion sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-reconciliation-bundle` |
| Routing tag | `notion:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for notion records..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| notion-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| notion-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Notion Sync Orchestrator, then checkpoint the last known good cursor on both sides of Notion.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for notion records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `notion-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] notion-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:sync-orchestrator` consumers with approval and credential context attached.
