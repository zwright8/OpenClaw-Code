---
name: linear-migration-assistant
description: Use when tasks require linear migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Migration Assistant
...
| Primary artifact | `linear-migration-cutover-plan` |
| Routing tag | `linear:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when state changes are hard to undo once they begin. This specific skill turns Linear Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from linear..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| linear-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| linear-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Linear Migration Assistant before any Linear rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from linear. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `linear-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] linear-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:migration-assistant` consumers with approval and credential context attached.
