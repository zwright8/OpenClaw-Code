---
name: gitlab-approval-router
description: Use when tasks require gitlab approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Approval Router
...
| Primary artifact | `gitlab-approval-decision-packet` |
| Routing tag | `gitlab:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Approval Router into a deterministic, auth-checked workflow for routes gitlab-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| gitlab-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| gitlab-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for GitLab Approval Router, then capture the manual fallback path for GitLab.
...
3. Implement routes gitlab-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `gitlab-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:approval-router` consumers with approval and credential context attached.
