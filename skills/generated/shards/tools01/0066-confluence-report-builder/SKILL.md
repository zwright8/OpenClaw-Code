---
name: confluence-report-builder
description: Use when tasks require confluence report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `confluence-report-bundle` |
| Routing tag | `confluence:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from confluence activity..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-report-bundle | structured-artifact | yes | next workflow or operator |
| confluence-report-bundle-scorecard | scorecard | yes | reviewer |
| confluence-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Confluence Report Builder, then confirm which Confluence entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from confluence activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `confluence-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] confluence-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:report-builder` consumers with approval and credential context attached.
