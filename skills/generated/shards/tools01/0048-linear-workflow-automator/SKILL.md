---
name: linear-workflow-automator
description: Use when tasks require linear workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Workflow Automator
...
| Primary artifact | `linear-workflow-runbook` |
| Routing tag | `linear:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using linear..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-workflow-runbook | structured-artifact | yes | next workflow or operator |
| linear-workflow-runbook-scorecard | scorecard | yes | reviewer |
| linear-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Linear Workflow Automator, then checkpoint where Linear state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using linear. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `linear-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] linear-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:workflow-automator` consumers with approval and credential context attached.
