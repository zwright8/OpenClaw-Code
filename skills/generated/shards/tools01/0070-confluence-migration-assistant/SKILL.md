---
name: confluence-migration-assistant
description: Use when tasks require confluence migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `confluence-migration-cutover-plan` |
| Routing tag | `confluence:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when state changes are hard to undo once they begin. This specific skill turns Confluence Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from confluence..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| confluence-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| confluence-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Confluence Migration Assistant before any Confluence rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from confluence. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `confluence-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] confluence-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:migration-assistant` consumers with approval and credential context attached.
