---
name: microsoft-teams-alert-monitor
description: Use when tasks require microsoft teams alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Alert Monitor
...
| Primary artifact | `microsoft-teams-alert-state-bundle` |
| Routing tag | `microsoft-teams:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Alert Monitor into a deterministic, auth-checked workflow for monitors key microsoft teams signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-teams-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-teams-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Microsoft Teams Alert Monitor, then lock the reviewed threshold set for Microsoft Teams.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key microsoft teams signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `microsoft-teams-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:alert-monitor` consumers with approval and credential context attached.
