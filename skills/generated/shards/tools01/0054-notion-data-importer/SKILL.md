---
name: notion-data-importer
description: Use when tasks require notion data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Notion Data Importer
...
| Primary artifact | `notion-import-ledger` |
| Routing tag | `notion:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when state changes are hard to undo once they begin. This specific skill turns Notion Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into notion..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-import-ledger | structured-artifact | yes | next workflow or operator |
| notion-import-ledger-scorecard | scorecard | yes | reviewer |
| notion-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Notion Data Importer before any Notion write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into notion. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `notion-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] notion-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:data-importer` consumers with approval and credential context attached.
