---
name: notion-migration-assistant
description: Use when tasks require notion migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-migration-cutover-plan` |
| Routing tag | `notion:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when state changes are hard to undo once they begin. This specific skill turns Notion Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from notion..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| notion-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| notion-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Notion Migration Assistant before any Notion rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from notion. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `notion-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] notion-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:migration-assistant` consumers with approval and credential context attached.
