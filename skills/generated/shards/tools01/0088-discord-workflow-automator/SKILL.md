---
name: discord-workflow-automator
description: Use when tasks require discord workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Discord Workflow Automator
...
| Primary artifact | `discord-workflow-runbook` |
| Routing tag | `discord:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using discord..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-workflow-runbook | structured-artifact | yes | next workflow or operator |
| discord-workflow-runbook-scorecard | scorecard | yes | reviewer |
| discord-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Discord Workflow Automator, then checkpoint where Discord state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using discord. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `discord-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] discord-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:workflow-automator` consumers with approval and credential context attached.
