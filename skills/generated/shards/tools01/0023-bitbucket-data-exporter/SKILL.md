---
name: bitbucket-data-exporter
description: Use when tasks require bitbucket data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Data Exporter
...
| Primary artifact | `bitbucket-export-package` |
| Routing tag | `bitbucket:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Data Exporter into a deterministic, auth-checked workflow for exports bitbucket entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-export-package | structured-artifact | yes | next workflow or operator |
| bitbucket-export-package-scorecard | scorecard | yes | reviewer |
| bitbucket-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Bitbucket Data Exporter, then constrain the export scope for Bitbucket before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports bitbucket entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `bitbucket-export-package` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:data-exporter` consumers with approval and credential context attached.
