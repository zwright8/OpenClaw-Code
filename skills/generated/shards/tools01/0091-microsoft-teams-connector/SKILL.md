...
description: Use when tasks require microsoft teams connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `microsoft-teams-connection-trace-bundle` |
| Routing tag | `microsoft-teams:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Connector into a deterministic, auth-checked workflow for unified read/write connector for microsoft teams apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-teams-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-teams-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Microsoft Teams Connector, then confirm the smallest read-only probe that proves access to Microsoft Teams.
...
3. Specify the request and response contract for unified read/write connector for microsoft teams apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `microsoft-teams-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:connector` consumers with approval and credential context attached.
