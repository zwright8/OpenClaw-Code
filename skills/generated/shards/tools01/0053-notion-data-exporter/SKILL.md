---
name: notion-data-exporter
description: Use when tasks require notion data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-export-package` |
| Routing tag | `notion:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Data Exporter into a deterministic, auth-checked workflow for exports notion entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-export-package | structured-artifact | yes | next workflow or operator |
| notion-export-package-scorecard | scorecard | yes | reviewer |
| notion-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Notion Data Exporter, then constrain the export scope for Notion before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports notion entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `notion-export-package` so downstream owners do not need to rediscover context.
...
- [ ] notion-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:data-exporter` consumers with approval and credential context attached.
