...
description: Use when tasks require bitbucket connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bitbucket-connection-trace-bundle` |
| Routing tag | `bitbucket:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Connector into a deterministic, auth-checked workflow for unified read/write connector for bitbucket apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| bitbucket-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| bitbucket-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Bitbucket Connector, then confirm the smallest read-only probe that proves access to Bitbucket.
...
3. Specify the request and response contract for unified read/write connector for bitbucket apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `bitbucket-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:connector` consumers with approval and credential context attached.
