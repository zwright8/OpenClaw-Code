---
name: bitbucket-workflow-automator
description: Use when tasks require bitbucket workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Workflow Automator
...
| Primary artifact | `bitbucket-workflow-runbook` |
| Routing tag | `bitbucket:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using bitbucket..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-workflow-runbook | structured-artifact | yes | next workflow or operator |
| bitbucket-workflow-runbook-scorecard | scorecard | yes | reviewer |
| bitbucket-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Bitbucket Workflow Automator, then checkpoint where Bitbucket state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using bitbucket. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `bitbucket-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:workflow-automator` consumers with approval and credential context attached.
