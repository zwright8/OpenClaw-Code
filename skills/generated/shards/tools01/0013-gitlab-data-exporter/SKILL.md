---
name: gitlab-data-exporter
description: Use when tasks require gitlab data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gitlab-export-package` |
| Routing tag | `gitlab:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Data Exporter into a deterministic, auth-checked workflow for exports gitlab entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-export-package | structured-artifact | yes | next workflow or operator |
| gitlab-export-package-scorecard | scorecard | yes | reviewer |
| gitlab-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for GitLab Data Exporter, then constrain the export scope for GitLab before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports gitlab entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `gitlab-export-package` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:data-exporter` consumers with approval and credential context attached.
