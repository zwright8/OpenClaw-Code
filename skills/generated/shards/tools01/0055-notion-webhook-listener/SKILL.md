---
name: notion-webhook-listener
description: Use when tasks require notion webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-event-intake-log` |
| Routing tag | `notion:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Notion Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes notion webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `NOTION_CLIENT_ID`, `NOTION_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `NOTION_TOKEN`, `NOTION_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `NOTION_WEBHOOK_SECRET`, `NOTION_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| notion-event-intake-log | structured-artifact | yes | next workflow or operator |
| notion-event-intake-log-scorecard | scorecard | yes | reviewer |
| notion-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Notion Webhook Listener, then register the signing-secret policy for Notion.
...
3. Implement captures, verifies, and routes notion webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `notion-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] notion-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:webhook-listener` consumers with approval and credential context attached.
