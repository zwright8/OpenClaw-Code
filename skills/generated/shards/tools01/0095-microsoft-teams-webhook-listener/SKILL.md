---
name: microsoft-teams-webhook-listener
description: Use when tasks require microsoft teams webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `microsoft-teams-event-intake-log` |
| Routing tag | `microsoft-teams:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Microsoft Teams Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes microsoft teams webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `MICROSOFT_TEAMS_CLIENT_ID`, `MICROSOFT_TEAMS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `MICROSOFT_TEAMS_TOKEN`, `MICROSOFT_TEAMS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `MICROSOFT_TEAMS_WEBHOOK_SECRET`, `MICROSOFT_TEAMS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| microsoft-teams-event-intake-log | structured-artifact | yes | next workflow or operator |
| microsoft-teams-event-intake-log-scorecard | scorecard | yes | reviewer |
| microsoft-teams-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Microsoft Teams Webhook Listener, then register the signing-secret policy for Microsoft Teams.
...
3. Implement captures, verifies, and routes microsoft teams webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `microsoft-teams-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:webhook-listener` consumers with approval and credential context attached.
