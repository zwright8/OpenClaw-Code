---
name: bitbucket-alert-monitor
description: Use when tasks require bitbucket alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Alert Monitor
...
| Primary artifact | `bitbucket-alert-state-bundle` |
| Routing tag | `bitbucket:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Alert Monitor into a deterministic, auth-checked workflow for monitors key bitbucket signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| bitbucket-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| bitbucket-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Bitbucket Alert Monitor, then lock the reviewed threshold set for Bitbucket.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key bitbucket signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `bitbucket-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:alert-monitor` consumers with approval and credential context attached.
