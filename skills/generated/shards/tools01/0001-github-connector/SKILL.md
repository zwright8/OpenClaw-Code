...
description: Use when tasks require github connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-connection-trace-bundle` |
| Routing tag | `github:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Connector into a deterministic, auth-checked workflow for unified read/write connector for github apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| github-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| github-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for GitHub Connector, then confirm the smallest read-only probe that proves access to GitHub.
...
3. Specify the request and response contract for unified read/write connector for github apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `github-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] github-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:connector` consumers with approval and credential context attached.
