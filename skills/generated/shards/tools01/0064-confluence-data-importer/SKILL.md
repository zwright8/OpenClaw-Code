---
name: confluence-data-importer
description: Use when tasks require confluence data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Confluence Data Importer
...
| Primary artifact | `confluence-import-ledger` |
| Routing tag | `confluence:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when state changes are hard to undo once they begin. This specific skill turns Confluence Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into confluence..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-import-ledger | structured-artifact | yes | next workflow or operator |
| confluence-import-ledger-scorecard | scorecard | yes | reviewer |
| confluence-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Confluence Data Importer before any Confluence write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into confluence. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `confluence-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] confluence-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:data-importer` consumers with approval and credential context attached.
