---
name: microsoft-teams-sync-orchestrator
description: Use when tasks require microsoft teams sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Sync Orchestrator
...
| Primary artifact | `microsoft-teams-reconciliation-bundle` |
| Routing tag | `microsoft-teams:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for microsoft teams records..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-teams-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-teams-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Microsoft Teams Sync Orchestrator, then checkpoint the last known good cursor on both sides of Microsoft Teams.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for microsoft teams records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `microsoft-teams-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:sync-orchestrator` consumers with approval and credential context attached.
