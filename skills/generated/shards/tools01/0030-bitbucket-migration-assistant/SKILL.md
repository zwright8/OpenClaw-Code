---
name: bitbucket-migration-assistant
description: Use when tasks require bitbucket migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Migration Assistant
...
| Primary artifact | `bitbucket-migration-cutover-plan` |
| Routing tag | `bitbucket:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when state changes are hard to undo once they begin. This specific skill turns Bitbucket Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from bitbucket..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| bitbucket-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| bitbucket-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Bitbucket Migration Assistant before any Bitbucket rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from bitbucket. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `bitbucket-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:migration-assistant` consumers with approval and credential context attached.
