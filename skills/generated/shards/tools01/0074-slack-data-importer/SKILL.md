---
name: slack-data-importer
description: Use when tasks require slack data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `slack-import-ledger` |
| Routing tag | `slack:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when state changes are hard to undo once they begin. This specific skill turns Slack Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into slack..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-import-ledger | structured-artifact | yes | next workflow or operator |
| slack-import-ledger-scorecard | scorecard | yes | reviewer |
| slack-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Slack Data Importer before any Slack write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into slack. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `slack-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] slack-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:data-importer` consumers with approval and credential context attached.
