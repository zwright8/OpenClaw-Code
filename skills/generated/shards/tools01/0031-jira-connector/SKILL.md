---
name: jira-connector
description: Use when tasks require jira connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-connection-trace-bundle` |
| Routing tag | `jira:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Connector into a deterministic, auth-checked workflow for unified read/write connector for jira apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| jira-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| jira-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Jira Connector, then confirm the smallest read-only probe that proves access to Jira.
...
3. Specify the request and response contract for unified read/write connector for jira apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `jira-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] jira-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:connector` consumers with approval and credential context attached.
