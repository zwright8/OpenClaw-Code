---
name: linear-alert-monitor
description: Use when tasks require linear alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Alert Monitor
...
| Primary artifact | `linear-alert-state-bundle` |
| Routing tag | `linear:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Alert Monitor into a deterministic, auth-checked workflow for monitors key linear signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| linear-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| linear-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Linear Alert Monitor, then lock the reviewed threshold set for Linear.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key linear signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `linear-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linear-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:alert-monitor` consumers with approval and credential context attached.
