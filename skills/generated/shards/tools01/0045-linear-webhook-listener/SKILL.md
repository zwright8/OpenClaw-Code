---
name: linear-webhook-listener
description: Use when tasks require linear webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `linear-event-intake-log` |
| Routing tag | `linear:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Linear Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes linear webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `LINEAR_CLIENT_ID`, `LINEAR_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `LINEAR_TOKEN`, `LINEAR_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `LINEAR_WEBHOOK_SECRET`, `LINEAR_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| linear-event-intake-log | structured-artifact | yes | next workflow or operator |
| linear-event-intake-log-scorecard | scorecard | yes | reviewer |
| linear-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Linear Webhook Listener, then register the signing-secret policy for Linear.
...
3. Implement captures, verifies, and routes linear webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `linear-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] linear-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:webhook-listener` consumers with approval and credential context attached.
