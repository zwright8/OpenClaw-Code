---
name: bitbucket-sync-orchestrator
description: Use when tasks require bitbucket sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Sync Orchestrator
...
| Primary artifact | `bitbucket-reconciliation-bundle` |
| Routing tag | `bitbucket:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for bitbucket records..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| bitbucket-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| bitbucket-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Bitbucket Sync Orchestrator, then checkpoint the last known good cursor on both sides of Bitbucket.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for bitbucket records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `bitbucket-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:sync-orchestrator` consumers with approval and credential context attached.
