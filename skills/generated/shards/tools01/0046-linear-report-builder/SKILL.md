---
name: linear-report-builder
description: Use when tasks require linear report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Report Builder
...
| Primary artifact | `linear-report-bundle` |
| Routing tag | `linear:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from linear activity..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-report-bundle | structured-artifact | yes | next workflow or operator |
| linear-report-bundle-scorecard | scorecard | yes | reviewer |
| linear-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Linear Report Builder, then confirm which Linear entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from linear activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `linear-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linear-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:report-builder` consumers with approval and credential context attached.
