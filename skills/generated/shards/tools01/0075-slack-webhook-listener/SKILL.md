...
description: Use when tasks require slack webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `slack-event-intake-log` |
| Routing tag | `slack:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Slack Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes slack webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `SLACK_WEBHOOK_SECRET`, `SLACK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| slack-event-intake-log | structured-artifact | yes | next workflow or operator |
| slack-event-intake-log-scorecard | scorecard | yes | reviewer |
| slack-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Slack Webhook Listener, then register the signing-secret policy for Slack.
...
3. Implement captures, verifies, and routes slack webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `slack-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] slack-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:webhook-listener` consumers with approval and credential context attached.
