...
description: Use when tasks require discord alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Discord Alert Monitor
...
| Primary artifact | `discord-alert-state-bundle` |
| Routing tag | `discord:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Alert Monitor into a deterministic, auth-checked workflow for monitors key discord signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| discord-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| discord-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Discord Alert Monitor, then lock the reviewed threshold set for Discord.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key discord signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `discord-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] discord-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:alert-monitor` consumers with approval and credential context attached.
