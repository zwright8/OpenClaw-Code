---
name: jira-approval-router
description: Use when tasks require jira approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Jira Approval Router
...
| Primary artifact | `jira-approval-decision-packet` |
| Routing tag | `jira:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Approval Router into a deterministic, auth-checked workflow for routes jira-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| jira-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| jira-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Jira Approval Router, then capture the manual fallback path for Jira.
...
3. Implement routes jira-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `jira-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] jira-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:approval-router` consumers with approval and credential context attached.
