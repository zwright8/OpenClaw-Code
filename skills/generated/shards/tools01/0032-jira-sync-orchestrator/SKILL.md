...
description: Use when tasks require jira sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-reconciliation-bundle` |
| Routing tag | `jira:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for jira records..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| jira-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| jira-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Jira Sync Orchestrator, then checkpoint the last known good cursor on both sides of Jira.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for jira records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `jira-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] jira-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:sync-orchestrator` consumers with approval and credential context attached.
