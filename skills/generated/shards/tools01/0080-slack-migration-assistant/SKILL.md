---
name: slack-migration-assistant
description: Use when tasks require slack migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Slack Migration Assistant
...
| Primary artifact | `slack-migration-cutover-plan` |
| Routing tag | `slack:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when state changes are hard to undo once they begin. This specific skill turns Slack Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from slack..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| slack-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| slack-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Slack Migration Assistant before any Slack rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from slack. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `slack-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] slack-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:migration-assistant` consumers with approval and credential context attached.
