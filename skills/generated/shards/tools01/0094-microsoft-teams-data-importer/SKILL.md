---
name: microsoft-teams-data-importer
description: Use when tasks require microsoft teams data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Data Importer
...
| Primary artifact | `microsoft-teams-import-ledger` |
| Routing tag | `microsoft-teams:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when state changes are hard to undo once they begin. This specific skill turns Microsoft Teams Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into microsoft teams..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-import-ledger | structured-artifact | yes | next workflow or operator |
| microsoft-teams-import-ledger-scorecard | scorecard | yes | reviewer |
| microsoft-teams-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Microsoft Teams Data Importer before any Microsoft Teams write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into microsoft teams. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `microsoft-teams-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:data-importer` consumers with approval and credential context attached.
