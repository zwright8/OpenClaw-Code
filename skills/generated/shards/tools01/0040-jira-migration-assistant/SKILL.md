---
name: jira-migration-assistant
description: Use when tasks require jira migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-migration-cutover-plan` |
| Routing tag | `jira:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when state changes are hard to undo once they begin. This specific skill turns Jira Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from jira..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| jira-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| jira-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Jira Migration Assistant before any Jira rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from jira. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `jira-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] jira-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:migration-assistant` consumers with approval and credential context attached.
