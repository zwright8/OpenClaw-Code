---
name: gitlab-migration-assistant
description: Use when tasks require gitlab migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Migration Assistant
...
| Primary artifact | `gitlab-migration-cutover-plan` |
| Routing tag | `gitlab:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when state changes are hard to undo once they begin. This specific skill turns GitLab Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from gitlab..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| gitlab-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| gitlab-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for GitLab Migration Assistant before any GitLab rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from gitlab. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `gitlab-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:migration-assistant` consumers with approval and credential context attached.
