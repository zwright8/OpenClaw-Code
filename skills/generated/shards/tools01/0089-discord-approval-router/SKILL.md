---
name: discord-approval-router
description: Use when tasks require discord approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Discord Approval Router
...
| Primary artifact | `discord-approval-decision-packet` |
| Routing tag | `discord:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Approval Router into a deterministic, auth-checked workflow for routes discord-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| discord-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| discord-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Discord Approval Router, then capture the manual fallback path for Discord.
...
3. Implement routes discord-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `discord-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] discord-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:approval-router` consumers with approval and credential context attached.
