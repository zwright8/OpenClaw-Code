...
description: Use when tasks require github webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-event-intake-log` |
| Routing tag | `github:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns GitHub Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes github webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Access token or personal access token | `token` | `GITHUB_TOKEN`, `GITHUB_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GITHUB_WEBHOOK_SECRET`, `GITHUB_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| github-event-intake-log | structured-artifact | yes | next workflow or operator |
| github-event-intake-log-scorecard | scorecard | yes | reviewer |
| github-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for GitHub Webhook Listener, then register the signing-secret policy for GitHub.
...
3. Implement captures, verifies, and routes github webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `github-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] github-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:webhook-listener` consumers with approval and credential context attached.
