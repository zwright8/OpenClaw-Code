---
name: gitlab-alert-monitor
description: Use when tasks require gitlab alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Alert Monitor
...
| Primary artifact | `gitlab-alert-state-bundle` |
| Routing tag | `gitlab:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Alert Monitor into a deterministic, auth-checked workflow for monitors key gitlab signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| gitlab-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| gitlab-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for GitLab Alert Monitor, then lock the reviewed threshold set for GitLab.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key gitlab signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `gitlab-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:alert-monitor` consumers with approval and credential context attached.
