...
| Field | Value |
|---|---|
| Skill pack | `security-network-operations-toolkit` |
| Tool count | `0` |
...
This pack provides a curated operational toolkit for security & network operations toolkit tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
1. Select a minimal tool from this pack to solve a security & network operations toolkit request, run dry-run first, then produce a validated artifact bundle.
