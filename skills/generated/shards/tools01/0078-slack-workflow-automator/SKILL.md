...
description: Use when tasks require slack workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Slack Workflow Automator
...
| Primary artifact | `slack-workflow-runbook` |
| Routing tag | `slack:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using slack..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-workflow-runbook | structured-artifact | yes | next workflow or operator |
| slack-workflow-runbook-scorecard | scorecard | yes | reviewer |
| slack-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Slack Workflow Automator, then checkpoint where Slack state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using slack. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `slack-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] slack-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:workflow-automator` consumers with approval and credential context attached.
