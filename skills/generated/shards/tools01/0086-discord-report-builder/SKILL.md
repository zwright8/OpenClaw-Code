---
name: discord-report-builder
description: Use when tasks require discord report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `discord-report-bundle` |
| Routing tag | `discord:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from discord activity..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-report-bundle | structured-artifact | yes | next workflow or operator |
| discord-report-bundle-scorecard | scorecard | yes | reviewer |
| discord-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Discord Report Builder, then confirm which Discord entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from discord activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `discord-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] discord-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:report-builder` consumers with approval and credential context attached.
