---
name: gitlab-report-builder
description: Use when tasks require gitlab report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Report Builder
...
| Primary artifact | `gitlab-report-bundle` |
| Routing tag | `gitlab:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from gitlab activity..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-report-bundle | structured-artifact | yes | next workflow or operator |
| gitlab-report-bundle-scorecard | scorecard | yes | reviewer |
| gitlab-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for GitLab Report Builder, then confirm which GitLab entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from gitlab activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `gitlab-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:report-builder` consumers with approval and credential context attached.
