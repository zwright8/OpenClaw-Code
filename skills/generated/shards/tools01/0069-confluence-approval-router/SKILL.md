---
name: confluence-approval-router
description: Use when tasks require confluence approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Confluence Approval Router
...
| Primary artifact | `confluence-approval-decision-packet` |
| Routing tag | `confluence:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Approval Router into a deterministic, auth-checked workflow for routes confluence-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| confluence-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| confluence-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Confluence Approval Router, then capture the manual fallback path for Confluence.
...
3. Implement routes confluence-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `confluence-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] confluence-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:approval-router` consumers with approval and credential context attached.
