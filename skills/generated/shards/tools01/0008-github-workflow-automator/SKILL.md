...
description: Use when tasks require github workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitHub Workflow Automator
...
| Primary artifact | `github-workflow-runbook` |
| Routing tag | `github:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using github..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-workflow-runbook | structured-artifact | yes | next workflow or operator |
| github-workflow-runbook-scorecard | scorecard | yes | reviewer |
| github-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for GitHub Workflow Automator, then checkpoint where GitHub state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using github. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `github-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] github-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:workflow-automator` consumers with approval and credential context attached.
