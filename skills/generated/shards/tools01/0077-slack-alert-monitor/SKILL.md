...
description: Use when tasks require slack alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `slack-alert-state-bundle` |
| Routing tag | `slack:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Alert Monitor into a deterministic, auth-checked workflow for monitors key slack signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| slack-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| slack-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Slack Alert Monitor, then lock the reviewed threshold set for Slack.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key slack signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `slack-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] slack-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:alert-monitor` consumers with approval and credential context attached.
