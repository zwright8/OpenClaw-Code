---
name: bitbucket-data-importer
description: Use when tasks require bitbucket data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Data Importer
...
| Primary artifact | `bitbucket-import-ledger` |
| Routing tag | `bitbucket:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when state changes are hard to undo once they begin. This specific skill turns Bitbucket Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into bitbucket..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-import-ledger | structured-artifact | yes | next workflow or operator |
| bitbucket-import-ledger-scorecard | scorecard | yes | reviewer |
| bitbucket-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Bitbucket Data Importer before any Bitbucket write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into bitbucket. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `bitbucket-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:data-importer` consumers with approval and credential context attached.
