---
name: slack-report-builder
description: Use when tasks require slack report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `slack-report-bundle` |
| Routing tag | `slack:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from slack activity..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-report-bundle | structured-artifact | yes | next workflow or operator |
| slack-report-bundle-scorecard | scorecard | yes | reviewer |
| slack-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Slack Report Builder, then confirm which Slack entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from slack activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `slack-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] slack-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:report-builder` consumers with approval and credential context attached.
