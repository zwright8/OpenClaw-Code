---
name: notion-alert-monitor
description: Use when tasks require notion alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Notion Alert Monitor
...
| Primary artifact | `notion-alert-state-bundle` |
| Routing tag | `notion:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Alert Monitor into a deterministic, auth-checked workflow for monitors key notion signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| notion-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| notion-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Notion Alert Monitor, then lock the reviewed threshold set for Notion.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key notion signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `notion-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] notion-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:alert-monitor` consumers with approval and credential context attached.
