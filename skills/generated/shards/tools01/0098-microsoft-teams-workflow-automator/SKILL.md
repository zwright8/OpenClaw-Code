---
name: microsoft-teams-workflow-automator
description: Use when tasks require microsoft teams workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Workflow Automator
...
| Primary artifact | `microsoft-teams-workflow-runbook` |
| Routing tag | `microsoft-teams:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using microsoft teams..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-workflow-runbook | structured-artifact | yes | next workflow or operator |
| microsoft-teams-workflow-runbook-scorecard | scorecard | yes | reviewer |
| microsoft-teams-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Microsoft Teams Workflow Automator, then checkpoint where Microsoft Teams state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using microsoft teams. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `microsoft-teams-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:workflow-automator` consumers with approval and credential context attached.
