...
description: Use when tasks require github report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-report-bundle` |
| Routing tag | `github:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from github activity..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-report-bundle | structured-artifact | yes | next workflow or operator |
| github-report-bundle-scorecard | scorecard | yes | reviewer |
| github-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for GitHub Report Builder, then confirm which GitHub entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from github activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `github-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] github-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:report-builder` consumers with approval and credential context attached.
