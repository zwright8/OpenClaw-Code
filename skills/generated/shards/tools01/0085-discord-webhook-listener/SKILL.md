---
name: discord-webhook-listener
description: Use when tasks require discord webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `discord-event-intake-log` |
| Routing tag | `discord:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Discord Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes discord webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `DISCORD_TOKEN`, `DISCORD_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DISCORD_WEBHOOK_SECRET`, `DISCORD_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| discord-event-intake-log | structured-artifact | yes | next workflow or operator |
| discord-event-intake-log-scorecard | scorecard | yes | reviewer |
| discord-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Discord Webhook Listener, then register the signing-secret policy for Discord.
...
3. Implement captures, verifies, and routes discord webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `discord-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] discord-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:webhook-listener` consumers with approval and credential context attached.
