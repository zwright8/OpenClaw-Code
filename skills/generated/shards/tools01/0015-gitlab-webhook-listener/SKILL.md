...
description: Use when tasks require gitlab webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gitlab-event-intake-log` |
| Routing tag | `gitlab:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns GitLab Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes gitlab webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| Access token or personal access token | `token` | `GITLAB_TOKEN`, `GITLAB_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GITLAB_WEBHOOK_SECRET`, `GITLAB_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| gitlab-event-intake-log | structured-artifact | yes | next workflow or operator |
| gitlab-event-intake-log-scorecard | scorecard | yes | reviewer |
| gitlab-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for GitLab Webhook Listener, then register the signing-secret policy for GitLab.
...
3. Implement captures, verifies, and routes gitlab webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `gitlab-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:webhook-listener` consumers with approval and credential context attached.
