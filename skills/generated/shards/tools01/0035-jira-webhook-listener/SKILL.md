---
name: jira-webhook-listener
description: Use when tasks require jira webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-event-intake-log` |
| Routing tag | `jira:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Jira Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes jira webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `JIRA_CLIENT_ID`, `JIRA_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `JIRA_TOKEN`, `JIRA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `JIRA_WEBHOOK_SECRET`, `JIRA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| jira-event-intake-log | structured-artifact | yes | next workflow or operator |
| jira-event-intake-log-scorecard | scorecard | yes | reviewer |
| jira-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Jira Webhook Listener, then register the signing-secret policy for Jira.
...
3. Implement captures, verifies, and routes jira webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `jira-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] jira-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:webhook-listener` consumers with approval and credential context attached.
