---
name: slack-data-exporter
description: Use when tasks require slack data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `slack-export-package` |
| Routing tag | `slack:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Data Exporter into a deterministic, auth-checked workflow for exports slack entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-export-package | structured-artifact | yes | next workflow or operator |
| slack-export-package-scorecard | scorecard | yes | reviewer |
| slack-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Slack Data Exporter, then constrain the export scope for Slack before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports slack entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `slack-export-package` so downstream owners do not need to rediscover context.
...
- [ ] slack-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:data-exporter` consumers with approval and credential context attached.
