---
name: github-approval-router
description: Use when tasks require github approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitHub Approval Router
...
| Primary artifact | `github-approval-decision-packet` |
| Routing tag | `github:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Approval Router into a deterministic, auth-checked workflow for routes github-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| github-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| github-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for GitHub Approval Router, then capture the manual fallback path for GitHub.
...
3. Implement routes github-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `github-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] github-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:approval-router` consumers with approval and credential context attached.
