...
| Field | Value |
|---|---|
| Skill pack | `ai-ml-local-inference-toolkit` |
| Tool count | `0` |
...
This pack provides a curated operational toolkit for ai/ml local inference toolkit tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
1. Select a minimal tool from this pack to solve a ai/ml local inference toolkit request, run dry-run first, then produce a validated artifact bundle.
