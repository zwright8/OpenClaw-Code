...
description: Use when tasks require gitlab connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gitlab-connection-trace-bundle` |
| Routing tag | `gitlab:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Connector into a deterministic, auth-checked workflow for unified read/write connector for gitlab apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| gitlab-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| gitlab-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for GitLab Connector, then confirm the smallest read-only probe that proves access to GitLab.
...
3. Specify the request and response contract for unified read/write connector for gitlab apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `gitlab-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:connector` consumers with approval and credential context attached.
