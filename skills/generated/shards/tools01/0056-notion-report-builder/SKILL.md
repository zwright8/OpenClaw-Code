...
description: Use when tasks require notion report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-report-bundle` |
| Routing tag | `notion:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from notion activity..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-report-bundle | structured-artifact | yes | next workflow or operator |
| notion-report-bundle-scorecard | scorecard | yes | reviewer |
| notion-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Notion Report Builder, then confirm which Notion entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from notion activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `notion-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] notion-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:report-builder` consumers with approval and credential context attached.
