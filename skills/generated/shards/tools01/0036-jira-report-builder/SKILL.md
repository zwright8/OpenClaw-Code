---
name: jira-report-builder
description: Use when tasks require jira report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-report-bundle` |
| Routing tag | `jira:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from jira activity..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-report-bundle | structured-artifact | yes | next workflow or operator |
| jira-report-bundle-scorecard | scorecard | yes | reviewer |
| jira-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Jira Report Builder, then confirm which Jira entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from jira activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `jira-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] jira-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:report-builder` consumers with approval and credential context attached.
