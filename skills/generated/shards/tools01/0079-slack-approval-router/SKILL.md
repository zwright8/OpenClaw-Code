---
name: slack-approval-router
description: Use when tasks require slack approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Slack Approval Router
...
| Primary artifact | `slack-approval-decision-packet` |
| Routing tag | `slack:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Approval Router into a deterministic, auth-checked workflow for routes slack-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| slack-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| slack-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Slack Approval Router, then capture the manual fallback path for Slack.
...
3. Implement routes slack-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `slack-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] slack-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:approval-router` consumers with approval and credential context attached.
