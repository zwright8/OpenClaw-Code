---
name: linear-data-importer
description: Use when tasks require linear data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Data Importer
...
| Primary artifact | `linear-import-ledger` |
| Routing tag | `linear:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when state changes are hard to undo once they begin. This specific skill turns Linear Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into linear..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-import-ledger | structured-artifact | yes | next workflow or operator |
| linear-import-ledger-scorecard | scorecard | yes | reviewer |
| linear-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Linear Data Importer before any Linear write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into linear. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `linear-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] linear-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:data-importer` consumers with approval and credential context attached.
