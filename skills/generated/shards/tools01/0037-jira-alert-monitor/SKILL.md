---
name: jira-alert-monitor
description: Use when tasks require jira alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-alert-state-bundle` |
| Routing tag | `jira:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Alert Monitor into a deterministic, auth-checked workflow for monitors key jira signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| jira-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| jira-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Jira Alert Monitor, then lock the reviewed threshold set for Jira.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key jira signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `jira-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] jira-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:alert-monitor` consumers with approval and credential context attached.
