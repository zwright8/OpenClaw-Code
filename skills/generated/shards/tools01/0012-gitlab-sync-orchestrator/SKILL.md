---
name: gitlab-sync-orchestrator
description: Use when tasks require gitlab sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `gitlab-reconciliation-bundle` |
| Routing tag | `gitlab:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for gitlab records..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| gitlab-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| gitlab-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for GitLab Sync Orchestrator, then checkpoint the last known good cursor on both sides of GitLab.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for gitlab records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `gitlab-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:sync-orchestrator` consumers with approval and credential context attached.
