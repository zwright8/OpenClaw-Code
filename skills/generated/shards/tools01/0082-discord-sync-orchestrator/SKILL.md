---
name: discord-sync-orchestrator
description: Use when tasks require discord sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Discord Sync Orchestrator
...
| Primary artifact | `discord-reconciliation-bundle` |
| Routing tag | `discord:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for discord records..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| discord-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| discord-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Discord Sync Orchestrator, then checkpoint the last known good cursor on both sides of Discord.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for discord records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `discord-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] discord-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:sync-orchestrator` consumers with approval and credential context attached.
