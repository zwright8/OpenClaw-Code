---
name: confluence-data-exporter
description: Use when tasks require confluence data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Confluence Data Exporter
...
| Primary artifact | `confluence-export-package` |
| Routing tag | `confluence:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Data Exporter into a deterministic, auth-checked workflow for exports confluence entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-export-package | structured-artifact | yes | next workflow or operator |
| confluence-export-package-scorecard | scorecard | yes | reviewer |
| confluence-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Confluence Data Exporter, then constrain the export scope for Confluence before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports confluence entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `confluence-export-package` so downstream owners do not need to rediscover context.
...
- [ ] confluence-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:data-exporter` consumers with approval and credential context attached.
