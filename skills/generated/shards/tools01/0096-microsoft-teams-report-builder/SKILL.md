---
name: microsoft-teams-report-builder
description: Use when tasks require microsoft teams report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `microsoft-teams-report-bundle` |
| Routing tag | `microsoft-teams:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from microsoft teams activity..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-report-bundle | structured-artifact | yes | next workflow or operator |
| microsoft-teams-report-bundle-scorecard | scorecard | yes | reviewer |
| microsoft-teams-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Microsoft Teams Report Builder, then confirm which Microsoft Teams entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from microsoft teams activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `microsoft-teams-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:report-builder` consumers with approval and credential context attached.
