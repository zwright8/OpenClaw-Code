---
name: gitlab-workflow-automator
description: Use when tasks require gitlab workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Workflow Automator
...
| Primary artifact | `gitlab-workflow-runbook` |
| Routing tag | `gitlab:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitLab Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using gitlab..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-workflow-runbook | structured-artifact | yes | next workflow or operator |
| gitlab-workflow-runbook-scorecard | scorecard | yes | reviewer |
| gitlab-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for GitLab Workflow Automator, then checkpoint where GitLab state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using gitlab. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `gitlab-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:workflow-automator` consumers with approval and credential context attached.
