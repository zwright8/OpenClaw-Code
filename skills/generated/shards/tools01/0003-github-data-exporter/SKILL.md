...
description: Use when tasks require github data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-export-package` |
| Routing tag | `github:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Data Exporter into a deterministic, auth-checked workflow for exports github entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-export-package | structured-artifact | yes | next workflow or operator |
| github-export-package-scorecard | scorecard | yes | reviewer |
| github-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for GitHub Data Exporter, then constrain the export scope for GitHub before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports github entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `github-export-package` so downstream owners do not need to rediscover context.
...
- [ ] github-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:data-exporter` consumers with approval and credential context attached.
