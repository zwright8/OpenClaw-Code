---
name: linear-sync-orchestrator
description: Use when tasks require linear sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Sync Orchestrator
...
| Primary artifact | `linear-reconciliation-bundle` |
| Routing tag | `linear:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for linear records..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| linear-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| linear-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Linear Sync Orchestrator, then checkpoint the last known good cursor on both sides of Linear.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for linear records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `linear-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] linear-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:sync-orchestrator` consumers with approval and credential context attached.
