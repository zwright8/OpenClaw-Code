...
description: Use when tasks require jira workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `jira-workflow-runbook` |
| Routing tag | `jira:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Jira workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Jira Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using jira..

## Trigger Checklist
- [ ] The task explicitly requires `Jira Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| jira-workflow-runbook | structured-artifact | yes | next workflow or operator |
| jira-workflow-runbook-scorecard | scorecard | yes | reviewer |
| jira-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Jira Workflow Automator, then checkpoint where Jira state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using jira. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `jira-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] jira-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `jira-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `jira:workflow-automator` consumers with approval and credential context attached.
