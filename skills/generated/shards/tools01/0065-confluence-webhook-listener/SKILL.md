---
name: confluence-webhook-listener
description: Use when tasks require confluence webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Confluence Webhook Listener
...
| Domain | `Work management and knowledge platforms` |
| Runtime archetype | `signed-event-ingestion-engine` |
| Core method | `signed event intake and routing` |
| Primary artifact | `confluence-event-intake-log` |
| Routing tag | `confluence:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Confluence Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes confluence webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `CONFLUENCE_CLIENT_ID`, `CONFLUENCE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `CONFLUENCE_TOKEN`, `CONFLUENCE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `CONFLUENCE_WEBHOOK_SECRET`, `CONFLUENCE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| confluence-event-intake-log | structured-artifact | yes | next workflow or operator |
| confluence-event-intake-log-scorecard | scorecard | yes | reviewer |
| confluence-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Confluence Webhook Listener, then register the signing-secret policy for Confluence.
...
3. Implement captures, verifies, and routes confluence webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `confluence-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] confluence-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:webhook-listener` consumers with approval and credential context attached.
