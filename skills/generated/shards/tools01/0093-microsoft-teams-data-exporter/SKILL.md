---
name: microsoft-teams-data-exporter
description: Use when tasks require microsoft teams data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Data Exporter
...
| Primary artifact | `microsoft-teams-export-package` |
| Routing tag | `microsoft-teams:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Microsoft Teams Data Exporter into a deterministic, auth-checked workflow for exports microsoft teams entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-export-package | structured-artifact | yes | next workflow or operator |
| microsoft-teams-export-package-scorecard | scorecard | yes | reviewer |
| microsoft-teams-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Microsoft Teams Data Exporter, then constrain the export scope for Microsoft Teams before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports microsoft teams entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `microsoft-teams-export-package` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:data-exporter` consumers with approval and credential context attached.
