...
description: Use when tasks require github alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-alert-state-bundle` |
| Routing tag | `github:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns GitHub Alert Monitor into a deterministic, auth-checked workflow for monitors key github signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| github-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| github-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for GitHub Alert Monitor, then lock the reviewed threshold set for GitHub.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key github signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `github-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] github-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:alert-monitor` consumers with approval and credential context attached.
