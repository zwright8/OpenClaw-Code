---
name: discord-data-importer
description: Use when tasks require discord data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Discord Data Importer
...
| Primary artifact | `discord-import-ledger` |
| Routing tag | `discord:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when state changes are hard to undo once they begin. This specific skill turns Discord Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into discord..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-import-ledger | structured-artifact | yes | next workflow or operator |
| discord-import-ledger-scorecard | scorecard | yes | reviewer |
| discord-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Discord Data Importer before any Discord write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into discord. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `discord-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] discord-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:data-importer` consumers with approval and credential context attached.
