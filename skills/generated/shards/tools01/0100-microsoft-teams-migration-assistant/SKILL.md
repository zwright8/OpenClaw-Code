---
name: microsoft-teams-migration-assistant
description: Use when tasks require microsoft teams migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Microsoft Teams Migration Assistant
...
| Primary artifact | `microsoft-teams-migration-cutover-plan` |
| Routing tag | `microsoft-teams:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Microsoft Teams workflows degrade when state changes are hard to undo once they begin. This specific skill turns Microsoft Teams Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from microsoft teams..

## Trigger Checklist
- [ ] The task explicitly requires `Microsoft Teams Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| microsoft-teams-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| microsoft-teams-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| microsoft-teams-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Microsoft Teams Migration Assistant before any Microsoft Teams rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from microsoft teams. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `microsoft-teams-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] microsoft-teams-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `microsoft-teams-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `microsoft-teams:migration-assistant` consumers with approval and credential context attached.
