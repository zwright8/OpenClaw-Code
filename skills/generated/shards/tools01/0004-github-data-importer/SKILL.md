---
name: github-data-importer
description: Use when tasks require github data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `github-import-ledger` |
| Routing tag | `github:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitHub workflows degrade when state changes are hard to undo once they begin. This specific skill turns GitHub Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into github..

## Trigger Checklist
- [ ] The task explicitly requires `GitHub Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| github-import-ledger | structured-artifact | yes | next workflow or operator |
| github-import-ledger-scorecard | scorecard | yes | reviewer |
| github-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for GitHub Data Importer before any GitHub write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into github. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `github-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] github-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `github-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `github:data-importer` consumers with approval and credential context attached.
