---
name: bitbucket-report-builder
description: Use when tasks require bitbucket report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Report Builder
...
| Primary artifact | `bitbucket-report-bundle` |
| Routing tag | `bitbucket:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from bitbucket activity..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-report-bundle | structured-artifact | yes | next workflow or operator |
| bitbucket-report-bundle-scorecard | scorecard | yes | reviewer |
| bitbucket-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Bitbucket Report Builder, then confirm which Bitbucket entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from bitbucket activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `bitbucket-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:report-builder` consumers with approval and credential context attached.
