---
name: bitbucket-approval-router
description: Use when tasks require bitbucket approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Bitbucket Approval Router
...
| Primary artifact | `bitbucket-approval-decision-packet` |
| Routing tag | `bitbucket:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Bitbucket workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Bitbucket Approval Router into a deterministic, auth-checked workflow for routes bitbucket-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Bitbucket Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bitbucket-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| bitbucket-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| bitbucket-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Bitbucket Approval Router, then capture the manual fallback path for Bitbucket.
...
3. Implement routes bitbucket-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `bitbucket-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] bitbucket-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bitbucket-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bitbucket:approval-router` consumers with approval and credential context attached.
