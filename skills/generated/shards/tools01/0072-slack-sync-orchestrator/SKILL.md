---
name: slack-sync-orchestrator
description: Use when tasks require slack sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Slack Sync Orchestrator
...
| Primary artifact | `slack-reconciliation-bundle` |
| Routing tag | `slack:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Slack workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Slack Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for slack records..

## Trigger Checklist
- [ ] The task explicitly requires `Slack Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| slack-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| slack-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| slack-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Slack Sync Orchestrator, then checkpoint the last known good cursor on both sides of Slack.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for slack records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `slack-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] slack-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `slack-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `slack:sync-orchestrator` consumers with approval and credential context attached.
