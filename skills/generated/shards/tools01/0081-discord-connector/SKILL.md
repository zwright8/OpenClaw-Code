...
description: Use when tasks require discord connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `discord-connection-trace-bundle` |
| Routing tag | `discord:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Discord workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Discord Connector into a deterministic, auth-checked workflow for unified read/write connector for discord apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Discord Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| discord-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| discord-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| discord-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Discord Connector, then confirm the smallest read-only probe that proves access to Discord.
...
3. Specify the request and response contract for unified read/write connector for discord apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `discord-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] discord-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `discord-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `discord:connector` consumers with approval and credential context attached.
