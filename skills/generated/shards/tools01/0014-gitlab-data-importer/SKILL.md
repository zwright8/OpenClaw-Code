---
name: gitlab-data-importer
description: Use when tasks require gitlab data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# GitLab Data Importer
...
| Primary artifact | `gitlab-import-ledger` |
| Routing tag | `gitlab:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because GitLab workflows degrade when state changes are hard to undo once they begin. This specific skill turns GitLab Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into gitlab..

## Trigger Checklist
- [ ] The task explicitly requires `GitLab Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| gitlab-import-ledger | structured-artifact | yes | next workflow or operator |
| gitlab-import-ledger-scorecard | scorecard | yes | reviewer |
| gitlab-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for GitLab Data Importer before any GitLab write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into gitlab. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `gitlab-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] gitlab-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `gitlab-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `gitlab:data-importer` consumers with approval and credential context attached.
