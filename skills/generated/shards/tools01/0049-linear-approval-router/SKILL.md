---
name: linear-approval-router
description: Use when tasks require linear approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Approval Router
...
| Primary artifact | `linear-approval-decision-packet` |
| Routing tag | `linear:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Approval Router into a deterministic, auth-checked workflow for routes linear-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| linear-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| linear-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Linear Approval Router, then capture the manual fallback path for Linear.
...
3. Implement routes linear-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `linear-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] linear-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:approval-router` consumers with approval and credential context attached.
