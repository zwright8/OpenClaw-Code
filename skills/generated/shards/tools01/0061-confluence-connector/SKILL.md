...
description: Use when tasks require confluence connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `confluence-connection-trace-bundle` |
| Routing tag | `confluence:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Connector into a deterministic, auth-checked workflow for unified read/write connector for confluence apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| confluence-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| confluence-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Confluence Connector, then confirm the smallest read-only probe that proves access to Confluence.
...
3. Specify the request and response contract for unified read/write connector for confluence apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `confluence-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] confluence-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:connector` consumers with approval and credential context attached.
