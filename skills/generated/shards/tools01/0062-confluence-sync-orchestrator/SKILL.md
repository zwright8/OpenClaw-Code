---
name: confluence-sync-orchestrator
description: Use when tasks require confluence sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `confluence-reconciliation-bundle` |
| Routing tag | `confluence:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Confluence workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Confluence Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for confluence records..

## Trigger Checklist
- [ ] The task explicitly requires `Confluence Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| confluence-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| confluence-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| confluence-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Confluence Sync Orchestrator, then checkpoint the last known good cursor on both sides of Confluence.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for confluence records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `confluence-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] confluence-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `confluence-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `confluence:sync-orchestrator` consumers with approval and credential context attached.
