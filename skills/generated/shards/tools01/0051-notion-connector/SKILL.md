...
description: Use when tasks require notion connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-connection-trace-bundle` |
| Routing tag | `notion:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Connector into a deterministic, auth-checked workflow for unified read/write connector for notion apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| notion-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| notion-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Notion Connector, then confirm the smallest read-only probe that proves access to Notion.
...
3. Specify the request and response contract for unified read/write connector for notion apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `notion-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] notion-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:connector` consumers with approval and credential context attached.
