---
name: notion-approval-router
description: Use when tasks require notion approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `notion-approval-decision-packet` |
| Routing tag | `notion:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Notion workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Notion Approval Router into a deterministic, auth-checked workflow for routes notion-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Notion Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| notion-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| notion-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| notion-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Notion Approval Router, then capture the manual fallback path for Notion.
...
3. Implement routes notion-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `notion-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] notion-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `notion-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `notion:approval-router` consumers with approval and credential context attached.
