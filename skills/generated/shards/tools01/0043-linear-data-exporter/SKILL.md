---
name: linear-data-exporter
description: Use when tasks require linear data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Linear Data Exporter
...
| Primary artifact | `linear-export-package` |
| Routing tag | `linear:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Linear workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Linear Data Exporter into a deterministic, auth-checked workflow for exports linear entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Linear Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| linear-export-package | structured-artifact | yes | next workflow or operator |
| linear-export-package-scorecard | scorecard | yes | reviewer |
| linear-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Linear Data Exporter, then constrain the export scope for Linear before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports linear entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `linear-export-package` so downstream owners do not need to rediscover context.
...
- [ ] linear-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `linear-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `linear:data-exporter` consumers with approval and credential context attached.
