...
description: Operate the "Beauty Pattern Detection for accessibility services" capability in production for accessibility services workflows. Use when mission execution explicitly requires this capability and outcomes must be reproducible, policy-gated, and handoff-ready.
---
...
## When To Use
Use this skill only when the request explicitly needs `Beauty Pattern Detection for accessibility services` in accessibility services and a downstream consumer requires contract-bound artifacts.
...
## Trigger Checklist
- [ ] The request explicitly needs **Beauty Pattern Detection for accessibility services** outcomes (not generic brainstorming).
...
1. **Incident stabilization in accessibility services" capability in production for accessibility services workflows**
   - Input: noisy upstream payload requiring beauty pattern detection for accessibility services normalization/assessment.
