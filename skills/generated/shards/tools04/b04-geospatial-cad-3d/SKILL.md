---
name: b04-geospatial-cad-3d
description: Use when processing geospatial data, map tiles, CAD/3D assets, or scientific geometry pipelines.
---

# Geospatial, CAD & 3D tooling

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b04-geospatial-cad-3d` |
| Tool count | `9` |
| Inventory rank span | `1506-1997` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for geospatial, cad & 3d tooling tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `tippecanoe` | `gts` | no | no |
| bounded mutation / transformation | `gts` | `tippecanoe` | yes | yes |
| validation & artifact checks | `tippecanoe` | `gts` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `tippecanoe` | Build vector tilesets from collections of GeoJSON features | local-installable |
| `gts` | GNU triangulated surface library | local-installable |
| `osmium-tool` | Libosmium-based command-line tool for processing OpenStreetMap data | local-installable |
| `libgeotiff` | Library and tools for dealing with GeoTIFF | local-installable |
| `lammps` | Molecular Dynamics Simulator | local-installable |
| `gmt` | Tools for manipulating and plotting geographic and Cartesian data | local-installable |
| `open-babel` | Chemical toolbox | local-installable |
| `mapnik` | Toolkit for developing mapping applications | local-installable |
| `opencascade` | 3D modeling and numerical simulation software for CAD/CAM/CAE | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `tippecanoe` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gts` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `osmium-tool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libgeotiff` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `lammps` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gmt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `open-babel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mapnik` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opencascade` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `tippecanoe` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `gts` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `osmium-tool` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `libgeotiff` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `lammps` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `gmt` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a geospatial, cad & 3d tooling request, run dry-run first, then produce a validated artifact bundle.
