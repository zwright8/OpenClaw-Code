---
name: bigcommerce-migration-assistant
description: Use when tasks require bigcommerce migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Migration Assistant
...
| Primary artifact | `bigcommerce-migration-cutover-plan` |
| Routing tag | `bigcommerce:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when state changes are hard to undo once they begin. This specific skill turns BigCommerce Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from bigcommerce..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| bigcommerce-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| bigcommerce-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for BigCommerce Migration Assistant before any BigCommerce rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from bigcommerce. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `bigcommerce-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:migration-assistant` consumers with approval and credential context attached.
