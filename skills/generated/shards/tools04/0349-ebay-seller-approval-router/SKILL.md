---
name: ebay-seller-approval-router
description: Use when tasks require ebay seller approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Approval Router
...
| Primary artifact | `ebay-seller-approval-decision-packet` |
| Routing tag | `ebay-seller:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Approval Router into a deterministic, auth-checked workflow for routes ebay seller-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| ebay-seller-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| ebay-seller-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for eBay Seller Approval Router, then capture the manual fallback path for eBay Seller.
...
3. Implement routes ebay seller-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `ebay-seller-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:approval-router` consumers with approval and credential context attached.
