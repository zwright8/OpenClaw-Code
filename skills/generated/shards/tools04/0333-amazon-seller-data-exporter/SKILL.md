---
name: amazon-seller-data-exporter
description: Use when tasks require amazon seller data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Data Exporter
...
| Primary artifact | `amazon-seller-export-package` |
| Routing tag | `amazon-seller:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Amazon Seller Data Exporter into a deterministic, auth-checked workflow for exports amazon seller entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-export-package | structured-artifact | yes | next workflow or operator |
| amazon-seller-export-package-scorecard | scorecard | yes | reviewer |
| amazon-seller-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Amazon Seller Data Exporter, then constrain the export scope for Amazon Seller before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports amazon seller entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `amazon-seller-export-package` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:data-exporter` consumers with approval and credential context attached.
