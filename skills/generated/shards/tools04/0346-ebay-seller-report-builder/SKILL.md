---
name: ebay-seller-report-builder
description: Use when tasks require ebay seller report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Report Builder
...
| Primary artifact | `ebay-seller-report-bundle` |
| Routing tag | `ebay-seller:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from ebay seller activity..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-report-bundle | structured-artifact | yes | next workflow or operator |
| ebay-seller-report-bundle-scorecard | scorecard | yes | reviewer |
| ebay-seller-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for eBay Seller Report Builder, then confirm which eBay Seller entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from ebay seller activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `ebay-seller-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:report-builder` consumers with approval and credential context attached.
