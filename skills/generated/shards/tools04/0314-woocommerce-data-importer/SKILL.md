---
name: woocommerce-data-importer
description: Use when tasks require woocommerce data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `woocommerce-import-ledger` |
| Routing tag | `woocommerce:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when state changes are hard to undo once they begin. This specific skill turns WooCommerce Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into woocommerce..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-import-ledger | structured-artifact | yes | next workflow or operator |
| woocommerce-import-ledger-scorecard | scorecard | yes | reviewer |
| woocommerce-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for WooCommerce Data Importer before any WooCommerce write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into woocommerce. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `woocommerce-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:data-importer` consumers with approval and credential context attached.
