---
name: shopify-alert-monitor
description: Use when tasks require shopify alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Shopify Alert Monitor
...
| Primary artifact | `shopify-alert-state-bundle` |
| Routing tag | `shopify:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Alert Monitor into a deterministic, auth-checked workflow for monitors key shopify signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| shopify-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| shopify-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Shopify Alert Monitor, then lock the reviewed threshold set for Shopify.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key shopify signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `shopify-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] shopify-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:alert-monitor` consumers with approval and credential context attached.
