---
name: mailchimp-data-importer
description: Use when tasks require mailchimp data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mailchimp-import-ledger` |
| Routing tag | `mailchimp:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when state changes are hard to undo once they begin. This specific skill turns Mailchimp Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into mailchimp..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-import-ledger | structured-artifact | yes | next workflow or operator |
| mailchimp-import-ledger-scorecard | scorecard | yes | reviewer |
| mailchimp-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Mailchimp Data Importer before any Mailchimp write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into mailchimp. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `mailchimp-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:data-importer` consumers with approval and credential context attached.
