---
name: mailchimp-report-builder
description: Use when tasks require mailchimp report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Report Builder
...
| Primary artifact | `mailchimp-report-bundle` |
| Routing tag | `mailchimp:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from mailchimp activity..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-report-bundle | structured-artifact | yes | next workflow or operator |
| mailchimp-report-bundle-scorecard | scorecard | yes | reviewer |
| mailchimp-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Mailchimp Report Builder, then confirm which Mailchimp entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from mailchimp activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `mailchimp-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:report-builder` consumers with approval and credential context attached.
