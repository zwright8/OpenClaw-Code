---
name: bigcommerce-alert-monitor
description: Use when tasks require bigcommerce alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Alert Monitor
...
| Primary artifact | `bigcommerce-alert-state-bundle` |
| Routing tag | `bigcommerce:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Alert Monitor into a deterministic, auth-checked workflow for monitors key bigcommerce signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| bigcommerce-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| bigcommerce-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for BigCommerce Alert Monitor, then lock the reviewed threshold set for BigCommerce.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key bigcommerce signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `bigcommerce-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:alert-monitor` consumers with approval and credential context attached.
