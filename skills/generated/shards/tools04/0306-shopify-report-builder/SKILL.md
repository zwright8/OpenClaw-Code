---
name: shopify-report-builder
description: Use when tasks require shopify report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Shopify Report Builder
...
| Primary artifact | `shopify-report-bundle` |
| Routing tag | `shopify:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from shopify activity..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-report-bundle | structured-artifact | yes | next workflow or operator |
| shopify-report-bundle-scorecard | scorecard | yes | reviewer |
| shopify-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Shopify Report Builder, then confirm which Shopify entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from shopify activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `shopify-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] shopify-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:report-builder` consumers with approval and credential context attached.
