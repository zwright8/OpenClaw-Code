---
name: twilio-webhook-listener
description: Use when tasks require twilio webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `twilio-event-intake-log` |
| Routing tag | `twilio:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Twilio Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes twilio webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `TWILIO_CLIENT_ID`, `TWILIO_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `TWILIO_TOKEN`, `TWILIO_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `TWILIO_WEBHOOK_SECRET`, `TWILIO_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| twilio-event-intake-log | structured-artifact | yes | next workflow or operator |
| twilio-event-intake-log-scorecard | scorecard | yes | reviewer |
| twilio-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Twilio Webhook Listener, then register the signing-secret policy for Twilio.
...
3. Implement captures, verifies, and routes twilio webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `twilio-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] twilio-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:webhook-listener` consumers with approval and credential context attached.
