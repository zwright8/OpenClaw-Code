---
name: woocommerce-sync-orchestrator
description: Use when tasks require woocommerce sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `woocommerce-reconciliation-bundle` |
| Routing tag | `woocommerce:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for woocommerce records..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| woocommerce-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| woocommerce-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for WooCommerce Sync Orchestrator, then checkpoint the last known good cursor on both sides of WooCommerce.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for woocommerce records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `woocommerce-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:sync-orchestrator` consumers with approval and credential context attached.
