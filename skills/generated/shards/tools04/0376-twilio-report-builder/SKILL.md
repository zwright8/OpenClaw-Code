---
name: twilio-report-builder
description: Use when tasks require twilio report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Report Builder
...
| Primary artifact | `twilio-report-bundle` |
| Routing tag | `twilio:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from twilio activity..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-report-bundle | structured-artifact | yes | next workflow or operator |
| twilio-report-bundle-scorecard | scorecard | yes | reviewer |
| twilio-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Twilio Report Builder, then confirm which Twilio entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from twilio activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `twilio-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] twilio-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:report-builder` consumers with approval and credential context attached.
