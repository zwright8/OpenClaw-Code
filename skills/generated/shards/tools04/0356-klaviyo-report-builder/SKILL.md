---
name: klaviyo-report-builder
description: Use when tasks require klaviyo report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `klaviyo-report-bundle` |
| Routing tag | `klaviyo:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from klaviyo activity..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-report-bundle | structured-artifact | yes | next workflow or operator |
| klaviyo-report-bundle-scorecard | scorecard | yes | reviewer |
| klaviyo-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Klaviyo Report Builder, then confirm which Klaviyo entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from klaviyo activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `klaviyo-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:report-builder` consumers with approval and credential context attached.
