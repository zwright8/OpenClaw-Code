---
name: postmark-migration-assistant
description: Use when tasks require postmark migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Migration Assistant
...
| Primary artifact | `postmark-migration-cutover-plan` |
| Routing tag | `postmark:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when state changes are hard to undo once they begin. This specific skill turns Postmark Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from postmark..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| postmark-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| postmark-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Postmark Migration Assistant before any Postmark rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from postmark. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `postmark-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] postmark-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:migration-assistant` consumers with approval and credential context attached.
