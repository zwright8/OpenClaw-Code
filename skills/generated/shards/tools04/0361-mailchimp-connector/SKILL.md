...
description: Use when tasks require mailchimp connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mailchimp-connection-trace-bundle` |
| Routing tag | `mailchimp:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Connector into a deterministic, auth-checked workflow for unified read/write connector for mailchimp apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| mailchimp-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| mailchimp-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Mailchimp Connector, then confirm the smallest read-only probe that proves access to Mailchimp.
...
3. Specify the request and response contract for unified read/write connector for mailchimp apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `mailchimp-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:connector` consumers with approval and credential context attached.
