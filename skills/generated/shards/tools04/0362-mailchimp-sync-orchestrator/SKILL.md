---
name: mailchimp-sync-orchestrator
description: Use when tasks require mailchimp sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Sync Orchestrator
...
| Primary artifact | `mailchimp-reconciliation-bundle` |
| Routing tag | `mailchimp:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for mailchimp records..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| mailchimp-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| mailchimp-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Mailchimp Sync Orchestrator, then checkpoint the last known good cursor on both sides of Mailchimp.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for mailchimp records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `mailchimp-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:sync-orchestrator` consumers with approval and credential context attached.
