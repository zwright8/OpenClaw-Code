---
name: ebay-seller-connector
description: Use when tasks require ebay seller connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Connector
...
| Primary artifact | `ebay-seller-connection-trace-bundle` |
| Routing tag | `ebay-seller:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Connector into a deterministic, auth-checked workflow for unified read/write connector for ebay seller apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| ebay-seller-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| ebay-seller-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for eBay Seller Connector, then confirm the smallest read-only probe that proves access to eBay Seller.
...
3. Specify the request and response contract for unified read/write connector for ebay seller apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `ebay-seller-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:connector` consumers with approval and credential context attached.
