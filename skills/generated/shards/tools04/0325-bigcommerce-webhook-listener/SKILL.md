---
name: bigcommerce-webhook-listener
description: Use when tasks require bigcommerce webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Webhook Listener
...
| Primary artifact | `bigcommerce-event-intake-log` |
| Routing tag | `bigcommerce:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns BigCommerce Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes bigcommerce webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `BIGCOMMERCE_CLIENT_ID`, `BIGCOMMERCE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `BIGCOMMERCE_TOKEN`, `BIGCOMMERCE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `BIGCOMMERCE_WEBHOOK_SECRET`, `BIGCOMMERCE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| bigcommerce-event-intake-log | structured-artifact | yes | next workflow or operator |
| bigcommerce-event-intake-log-scorecard | scorecard | yes | reviewer |
| bigcommerce-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for BigCommerce Webhook Listener, then register the signing-secret policy for BigCommerce.
...
3. Implement captures, verifies, and routes bigcommerce webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `bigcommerce-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:webhook-listener` consumers with approval and credential context attached.
