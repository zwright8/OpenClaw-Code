...
description: Use when tasks require klaviyo connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `klaviyo-connection-trace-bundle` |
| Routing tag | `klaviyo:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Connector into a deterministic, auth-checked workflow for unified read/write connector for klaviyo apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| klaviyo-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| klaviyo-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Klaviyo Connector, then confirm the smallest read-only probe that proves access to Klaviyo.
...
3. Specify the request and response contract for unified read/write connector for klaviyo apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `klaviyo-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:connector` consumers with approval and credential context attached.
