---
name: b04-docs-content-publishing
description: Use when creating, converting, validating, or publishing docs, markdown, PDFs, and developer-facing content artifacts.
---

# Documentation & content publishing

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b04-docs-content-publishing` |
| Tool count | `30` |
| Inventory rank span | `1503-1970` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for documentation & content publishing tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `pdfly` | `mailhog` | no | no |
| bounded mutation / transformation | `mailhog` | `pdfly` | yes | yes |
| validation & artifact checks | `pdfly` | `mailhog` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `pdfly` | CLI tool to extract (meta)data from PDF and manipulate PDF files | local-installable |
| `mailhog` | Web and API based SMTP testing tool | local-installable |
| `swaks` | SMTP command-line test tool | local-installable |
| `navi` | Interactive cheatsheet tool for the command-line | local-installable |
| `pdf2svg` | PDF converter to SVG | local-installable |
| `redocly-cli` | Your all-in-one OpenAPI utility | local-installable |
| `latexindent` | Add indentation to LaTeX files | local-installable |
| `mdbook` | Create modern online books from Markdown files | local-installable |
| `mu` | Tool for searching e-mail messages stored in the maildir-format | local-installable |
| `tinymist` | Services for Typst | local-installable |
| `pdfcpu` | PDF processor written in Go | local-installable |
| `mkdocs` | Project documentation with Markdown | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `pdfly` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mailhog` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `swaks` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `navi` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdf2svg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `redocly-cli` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `latexindent` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mdbook` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tinymist` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pdfcpu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mkdocs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `evince` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `twine` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xpdf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `nb` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `copier` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `help2man` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gcalcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pngpaste` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `usage` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cheat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `toilet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mailutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mkdocs-material` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `presenterm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `antiword` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aha` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ical-buddy` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `biber` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `pdfly` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `mailhog` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `swaks` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `navi` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `pdf2svg` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `redocly-cli` (HTTPS/REST, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a documentation & content publishing request, run dry-run first, then produce a validated artifact bundle.
