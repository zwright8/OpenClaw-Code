---
name: klaviyo-workflow-automator
description: Use when tasks require klaviyo workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Workflow Automator
...
| Primary artifact | `klaviyo-workflow-runbook` |
| Routing tag | `klaviyo:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using klaviyo..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-workflow-runbook | structured-artifact | yes | next workflow or operator |
| klaviyo-workflow-runbook-scorecard | scorecard | yes | reviewer |
| klaviyo-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Klaviyo Workflow Automator, then checkpoint where Klaviyo state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using klaviyo. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `klaviyo-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:workflow-automator` consumers with approval and credential context attached.
