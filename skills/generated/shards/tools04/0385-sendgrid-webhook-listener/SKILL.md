---
name: sendgrid-webhook-listener
description: Use when tasks require sendgrid webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Webhook Listener
...
| Primary artifact | `sendgrid-event-intake-log` |
| Routing tag | `sendgrid:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns SendGrid Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes sendgrid webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SENDGRID_CLIENT_ID`, `SENDGRID_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SENDGRID_TOKEN`, `SENDGRID_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SENDGRID_WEBHOOK_SECRET`, `SENDGRID_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| sendgrid-event-intake-log | structured-artifact | yes | next workflow or operator |
| sendgrid-event-intake-log-scorecard | scorecard | yes | reviewer |
| sendgrid-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for SendGrid Webhook Listener, then register the signing-secret policy for SendGrid.
...
3. Implement captures, verifies, and routes sendgrid webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `sendgrid-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:webhook-listener` consumers with approval and credential context attached.
