---
name: twilio-alert-monitor
description: Use when tasks require twilio alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Alert Monitor
...
| Primary artifact | `twilio-alert-state-bundle` |
| Routing tag | `twilio:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Alert Monitor into a deterministic, auth-checked workflow for monitors key twilio signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| twilio-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| twilio-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Twilio Alert Monitor, then lock the reviewed threshold set for Twilio.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key twilio signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `twilio-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] twilio-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:alert-monitor` consumers with approval and credential context attached.
