...
description: Use when tasks require bigcommerce connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `bigcommerce-connection-trace-bundle` |
| Routing tag | `bigcommerce:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Connector into a deterministic, auth-checked workflow for unified read/write connector for bigcommerce apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| bigcommerce-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| bigcommerce-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for BigCommerce Connector, then confirm the smallest read-only probe that proves access to BigCommerce.
...
3. Specify the request and response contract for unified read/write connector for bigcommerce apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `bigcommerce-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:connector` consumers with approval and credential context attached.
