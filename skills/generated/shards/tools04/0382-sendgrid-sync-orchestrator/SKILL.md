---
name: sendgrid-sync-orchestrator
description: Use when tasks require sendgrid sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Sync Orchestrator
...
| Primary artifact | `sendgrid-reconciliation-bundle` |
| Routing tag | `sendgrid:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for sendgrid records..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| sendgrid-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| sendgrid-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for SendGrid Sync Orchestrator, then checkpoint the last known good cursor on both sides of SendGrid.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for sendgrid records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `sendgrid-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:sync-orchestrator` consumers with approval and credential context attached.
