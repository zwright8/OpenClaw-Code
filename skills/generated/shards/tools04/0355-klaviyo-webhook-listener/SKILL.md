---
name: klaviyo-webhook-listener
description: Use when tasks require klaviyo webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Webhook Listener
...
| Primary artifact | `klaviyo-event-intake-log` |
| Routing tag | `klaviyo:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Klaviyo Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes klaviyo webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `KLAVIYO_CLIENT_ID`, `KLAVIYO_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `KLAVIYO_TOKEN`, `KLAVIYO_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `KLAVIYO_WEBHOOK_SECRET`, `KLAVIYO_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| klaviyo-event-intake-log | structured-artifact | yes | next workflow or operator |
| klaviyo-event-intake-log-scorecard | scorecard | yes | reviewer |
| klaviyo-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Klaviyo Webhook Listener, then register the signing-secret policy for Klaviyo.
...
3. Implement captures, verifies, and routes klaviyo webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `klaviyo-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:webhook-listener` consumers with approval and credential context attached.
