---
name: postmark-approval-router
description: Use when tasks require postmark approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Approval Router
...
| Primary artifact | `postmark-approval-decision-packet` |
| Routing tag | `postmark:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Approval Router into a deterministic, auth-checked workflow for routes postmark-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| postmark-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| postmark-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Postmark Approval Router, then capture the manual fallback path for Postmark.
...
3. Implement routes postmark-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `postmark-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] postmark-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:approval-router` consumers with approval and credential context attached.
