---
name: klaviyo-data-exporter
description: Use when tasks require klaviyo data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Data Exporter
...
| Primary artifact | `klaviyo-export-package` |
| Routing tag | `klaviyo:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Data Exporter into a deterministic, auth-checked workflow for exports klaviyo entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-export-package | structured-artifact | yes | next workflow or operator |
| klaviyo-export-package-scorecard | scorecard | yes | reviewer |
| klaviyo-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Klaviyo Data Exporter, then constrain the export scope for Klaviyo before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports klaviyo entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `klaviyo-export-package` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:data-exporter` consumers with approval and credential context attached.
