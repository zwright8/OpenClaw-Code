...
description: Use when tasks require shopify connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `shopify-connection-trace-bundle` |
| Routing tag | `shopify:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Connector into a deterministic, auth-checked workflow for unified read/write connector for shopify apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| shopify-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| shopify-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Shopify Connector, then confirm the smallest read-only probe that proves access to Shopify.
...
3. Specify the request and response contract for unified read/write connector for shopify apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `shopify-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] shopify-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:connector` consumers with approval and credential context attached.
