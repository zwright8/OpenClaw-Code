...
description: Use when tasks require twilio connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `twilio-connection-trace-bundle` |
| Routing tag | `twilio:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Connector into a deterministic, auth-checked workflow for unified read/write connector for twilio apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| twilio-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| twilio-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Twilio Connector, then confirm the smallest read-only probe that proves access to Twilio.
...
3. Specify the request and response contract for unified read/write connector for twilio apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `twilio-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] twilio-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:connector` consumers with approval and credential context attached.
