---
name: amazon-seller-migration-assistant
description: Use when tasks require amazon seller migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Migration Assistant
...
| Primary artifact | `amazon-seller-migration-cutover-plan` |
| Routing tag | `amazon-seller:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when state changes are hard to undo once they begin. This specific skill turns Amazon Seller Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from amazon seller..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| amazon-seller-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| amazon-seller-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Amazon Seller Migration Assistant before any Amazon Seller rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from amazon seller. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `amazon-seller-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:migration-assistant` consumers with approval and credential context attached.
