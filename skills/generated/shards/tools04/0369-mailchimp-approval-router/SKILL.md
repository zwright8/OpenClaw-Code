---
name: mailchimp-approval-router
description: Use when tasks require mailchimp approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Approval Router
...
| Primary artifact | `mailchimp-approval-decision-packet` |
| Routing tag | `mailchimp:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Approval Router into a deterministic, auth-checked workflow for routes mailchimp-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| mailchimp-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| mailchimp-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Mailchimp Approval Router, then capture the manual fallback path for Mailchimp.
...
3. Implement routes mailchimp-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `mailchimp-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:approval-router` consumers with approval and credential context attached.
