---
name: shopify-migration-assistant
description: Use when tasks require shopify migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `shopify-migration-cutover-plan` |
| Routing tag | `shopify:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when state changes are hard to undo once they begin. This specific skill turns Shopify Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from shopify..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| shopify-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| shopify-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Shopify Migration Assistant before any Shopify rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from shopify. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `shopify-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] shopify-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:migration-assistant` consumers with approval and credential context attached.
