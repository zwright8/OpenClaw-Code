---
name: woocommerce-migration-assistant
description: Use when tasks require woocommerce migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WooCommerce Migration Assistant
...
| Primary artifact | `woocommerce-migration-cutover-plan` |
| Routing tag | `woocommerce:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when state changes are hard to undo once they begin. This specific skill turns WooCommerce Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from woocommerce..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| woocommerce-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| woocommerce-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for WooCommerce Migration Assistant before any WooCommerce rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from woocommerce. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `woocommerce-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:migration-assistant` consumers with approval and credential context attached.
