---
name: shopify-data-exporter
description: Use when tasks require shopify data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Shopify Data Exporter
...
| Primary artifact | `shopify-export-package` |
| Routing tag | `shopify:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Data Exporter into a deterministic, auth-checked workflow for exports shopify entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-export-package | structured-artifact | yes | next workflow or operator |
| shopify-export-package-scorecard | scorecard | yes | reviewer |
| shopify-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Shopify Data Exporter, then constrain the export scope for Shopify before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports shopify entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `shopify-export-package` so downstream owners do not need to rediscover context.
...
- [ ] shopify-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:data-exporter` consumers with approval and credential context attached.
