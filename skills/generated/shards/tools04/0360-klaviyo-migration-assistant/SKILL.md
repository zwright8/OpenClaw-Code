---
name: klaviyo-migration-assistant
description: Use when tasks require klaviyo migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Migration Assistant
...
| Primary artifact | `klaviyo-migration-cutover-plan` |
| Routing tag | `klaviyo:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when state changes are hard to undo once they begin. This specific skill turns Klaviyo Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from klaviyo..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| klaviyo-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| klaviyo-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Klaviyo Migration Assistant before any Klaviyo rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from klaviyo. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `klaviyo-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:migration-assistant` consumers with approval and credential context attached.
