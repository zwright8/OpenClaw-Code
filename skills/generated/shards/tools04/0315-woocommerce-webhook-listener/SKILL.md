...
description: Use when tasks require woocommerce webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `woocommerce-event-intake-log` |
| Routing tag | `woocommerce:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns WooCommerce Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes woocommerce webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `WOOCOMMERCE_CLIENT_ID`, `WOOCOMMERCE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `WOOCOMMERCE_TOKEN`, `WOOCOMMERCE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `WOOCOMMERCE_WEBHOOK_SECRET`, `WOOCOMMERCE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| woocommerce-event-intake-log | structured-artifact | yes | next workflow or operator |
| woocommerce-event-intake-log-scorecard | scorecard | yes | reviewer |
| woocommerce-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for WooCommerce Webhook Listener, then register the signing-secret policy for WooCommerce.
...
3. Implement captures, verifies, and routes woocommerce webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `woocommerce-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:webhook-listener` consumers with approval and credential context attached.
