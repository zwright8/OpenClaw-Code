---
name: bigcommerce-approval-router
description: Use when tasks require bigcommerce approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Approval Router
...
| Primary artifact | `bigcommerce-approval-decision-packet` |
| Routing tag | `bigcommerce:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Approval Router into a deterministic, auth-checked workflow for routes bigcommerce-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| bigcommerce-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| bigcommerce-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for BigCommerce Approval Router, then capture the manual fallback path for BigCommerce.
...
3. Implement routes bigcommerce-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `bigcommerce-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:approval-router` consumers with approval and credential context attached.
