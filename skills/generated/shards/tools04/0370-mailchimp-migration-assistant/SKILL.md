---
name: mailchimp-migration-assistant
description: Use when tasks require mailchimp migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Migration Assistant
...
| Primary artifact | `mailchimp-migration-cutover-plan` |
| Routing tag | `mailchimp:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when state changes are hard to undo once they begin. This specific skill turns Mailchimp Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from mailchimp..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| mailchimp-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| mailchimp-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Mailchimp Migration Assistant before any Mailchimp rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from mailchimp. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `mailchimp-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:migration-assistant` consumers with approval and credential context attached.
