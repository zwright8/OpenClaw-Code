---
name: klaviyo-alert-monitor
description: Use when tasks require klaviyo alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Alert Monitor
...
| Primary artifact | `klaviyo-alert-state-bundle` |
| Routing tag | `klaviyo:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Alert Monitor into a deterministic, auth-checked workflow for monitors key klaviyo signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| klaviyo-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| klaviyo-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Klaviyo Alert Monitor, then lock the reviewed threshold set for Klaviyo.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key klaviyo signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `klaviyo-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:alert-monitor` consumers with approval and credential context attached.
