---
name: amazon-seller-data-importer
description: Use when tasks require amazon seller data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Data Importer
...
| Primary artifact | `amazon-seller-import-ledger` |
| Routing tag | `amazon-seller:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when state changes are hard to undo once they begin. This specific skill turns Amazon Seller Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into amazon seller..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-import-ledger | structured-artifact | yes | next workflow or operator |
| amazon-seller-import-ledger-scorecard | scorecard | yes | reviewer |
| amazon-seller-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Amazon Seller Data Importer before any Amazon Seller write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into amazon seller. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `amazon-seller-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:data-importer` consumers with approval and credential context attached.
