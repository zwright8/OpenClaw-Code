---
name: twilio-migration-assistant
description: Use when tasks require twilio migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Migration Assistant
...
| Primary artifact | `twilio-migration-cutover-plan` |
| Routing tag | `twilio:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when state changes are hard to undo once they begin. This specific skill turns Twilio Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from twilio..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| twilio-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| twilio-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Twilio Migration Assistant before any Twilio rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from twilio. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `twilio-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] twilio-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:migration-assistant` consumers with approval and credential context attached.
