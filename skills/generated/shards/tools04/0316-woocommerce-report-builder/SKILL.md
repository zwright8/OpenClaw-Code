---
name: woocommerce-report-builder
description: Use when tasks require woocommerce report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `woocommerce-report-bundle` |
| Routing tag | `woocommerce:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from woocommerce activity..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-report-bundle | structured-artifact | yes | next workflow or operator |
| woocommerce-report-bundle-scorecard | scorecard | yes | reviewer |
| woocommerce-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for WooCommerce Report Builder, then confirm which WooCommerce entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from woocommerce activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `woocommerce-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:report-builder` consumers with approval and credential context attached.
