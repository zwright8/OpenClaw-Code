---
name: bigcommerce-data-exporter
description: Use when tasks require bigcommerce data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Data Exporter
...
| Primary artifact | `bigcommerce-export-package` |
| Routing tag | `bigcommerce:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Data Exporter into a deterministic, auth-checked workflow for exports bigcommerce entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-export-package | structured-artifact | yes | next workflow or operator |
| bigcommerce-export-package-scorecard | scorecard | yes | reviewer |
| bigcommerce-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for BigCommerce Data Exporter, then constrain the export scope for BigCommerce before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports bigcommerce entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `bigcommerce-export-package` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:data-exporter` consumers with approval and credential context attached.
