---
name: twilio-data-importer
description: Use when tasks require twilio data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Data Importer
...
| Primary artifact | `twilio-import-ledger` |
| Routing tag | `twilio:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when state changes are hard to undo once they begin. This specific skill turns Twilio Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into twilio..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-import-ledger | structured-artifact | yes | next workflow or operator |
| twilio-import-ledger-scorecard | scorecard | yes | reviewer |
| twilio-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Twilio Data Importer before any Twilio write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into twilio. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `twilio-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] twilio-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:data-importer` consumers with approval and credential context attached.
