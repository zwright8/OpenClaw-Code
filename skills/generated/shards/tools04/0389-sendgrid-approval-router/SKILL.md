---
name: sendgrid-approval-router
description: Use when tasks require sendgrid approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Approval Router
...
| Primary artifact | `sendgrid-approval-decision-packet` |
| Routing tag | `sendgrid:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Approval Router into a deterministic, auth-checked workflow for routes sendgrid-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| sendgrid-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| sendgrid-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for SendGrid Approval Router, then capture the manual fallback path for SendGrid.
...
3. Implement routes sendgrid-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `sendgrid-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:approval-router` consumers with approval and credential context attached.
