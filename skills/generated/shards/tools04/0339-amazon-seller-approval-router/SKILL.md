---
name: amazon-seller-approval-router
description: Use when tasks require amazon seller approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Approval Router
...
| Primary artifact | `amazon-seller-approval-decision-packet` |
| Routing tag | `amazon-seller:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Amazon Seller Approval Router into a deterministic, auth-checked workflow for routes amazon seller-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| amazon-seller-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| amazon-seller-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Amazon Seller Approval Router, then capture the manual fallback path for Amazon Seller.
...
3. Implement routes amazon seller-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `amazon-seller-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:approval-router` consumers with approval and credential context attached.
