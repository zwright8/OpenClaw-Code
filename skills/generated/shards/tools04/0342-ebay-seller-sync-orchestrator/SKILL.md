---
name: ebay-seller-sync-orchestrator
description: Use when tasks require ebay seller sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Sync Orchestrator
...
| Primary artifact | `ebay-seller-reconciliation-bundle` |
| Routing tag | `ebay-seller:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for ebay seller records..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| ebay-seller-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| ebay-seller-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for eBay Seller Sync Orchestrator, then checkpoint the last known good cursor on both sides of eBay Seller.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for ebay seller records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `ebay-seller-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:sync-orchestrator` consumers with approval and credential context attached.
