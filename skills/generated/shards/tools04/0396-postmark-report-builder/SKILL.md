---
name: postmark-report-builder
description: Use when tasks require postmark report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Report Builder
...
| Primary artifact | `postmark-report-bundle` |
| Routing tag | `postmark:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from postmark activity..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-report-bundle | structured-artifact | yes | next workflow or operator |
| postmark-report-bundle-scorecard | scorecard | yes | reviewer |
| postmark-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Postmark Report Builder, then confirm which Postmark entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from postmark activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `postmark-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postmark-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:report-builder` consumers with approval and credential context attached.
