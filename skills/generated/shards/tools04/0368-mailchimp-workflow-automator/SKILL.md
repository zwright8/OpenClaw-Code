---
name: mailchimp-workflow-automator
description: Use when tasks require mailchimp workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Workflow Automator
...
| Primary artifact | `mailchimp-workflow-runbook` |
| Routing tag | `mailchimp:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using mailchimp..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-workflow-runbook | structured-artifact | yes | next workflow or operator |
| mailchimp-workflow-runbook-scorecard | scorecard | yes | reviewer |
| mailchimp-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Mailchimp Workflow Automator, then checkpoint where Mailchimp state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using mailchimp. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `mailchimp-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:workflow-automator` consumers with approval and credential context attached.
