---
name: shopify-webhook-listener
description: Use when tasks require shopify webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `shopify-event-intake-log` |
| Routing tag | `shopify:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Shopify Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes shopify webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `SHOPIFY_WEBHOOK_SECRET`, `SHOPIFY_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| shopify-event-intake-log | structured-artifact | yes | next workflow or operator |
| shopify-event-intake-log-scorecard | scorecard | yes | reviewer |
| shopify-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Shopify Webhook Listener, then register the signing-secret policy for Shopify.
...
3. Implement captures, verifies, and routes shopify webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `shopify-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] shopify-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:webhook-listener` consumers with approval and credential context attached.
