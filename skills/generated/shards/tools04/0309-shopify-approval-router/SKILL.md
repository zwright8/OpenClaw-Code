---
name: shopify-approval-router
description: Use when tasks require shopify approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Shopify Approval Router
...
| Primary artifact | `shopify-approval-decision-packet` |
| Routing tag | `shopify:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Approval Router into a deterministic, auth-checked workflow for routes shopify-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| shopify-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| shopify-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Shopify Approval Router, then capture the manual fallback path for Shopify.
...
3. Implement routes shopify-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `shopify-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] shopify-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:approval-router` consumers with approval and credential context attached.
