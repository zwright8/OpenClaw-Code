---
name: ebay-seller-webhook-listener
description: Use when tasks require ebay seller webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Webhook Listener
...
| Primary artifact | `ebay-seller-event-intake-log` |
| Routing tag | `ebay-seller:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns eBay Seller Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes ebay seller webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `EBAY_SELLER_CLIENT_ID`, `EBAY_SELLER_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `EBAY_SELLER_TOKEN`, `EBAY_SELLER_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `EBAY_SELLER_WEBHOOK_SECRET`, `EBAY_SELLER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| ebay-seller-event-intake-log | structured-artifact | yes | next workflow or operator |
| ebay-seller-event-intake-log-scorecard | scorecard | yes | reviewer |
| ebay-seller-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for eBay Seller Webhook Listener, then register the signing-secret policy for eBay Seller.
...
3. Implement captures, verifies, and routes ebay seller webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `ebay-seller-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:webhook-listener` consumers with approval and credential context attached.
