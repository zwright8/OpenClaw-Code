---
name: sendgrid-data-importer
description: Use when tasks require sendgrid data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Data Importer
...
| Primary artifact | `sendgrid-import-ledger` |
| Routing tag | `sendgrid:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when state changes are hard to undo once they begin. This specific skill turns SendGrid Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into sendgrid..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-import-ledger | structured-artifact | yes | next workflow or operator |
| sendgrid-import-ledger-scorecard | scorecard | yes | reviewer |
| sendgrid-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for SendGrid Data Importer before any SendGrid write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into sendgrid. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `sendgrid-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:data-importer` consumers with approval and credential context attached.
