---
name: postmark-workflow-automator
description: Use when tasks require postmark workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Workflow Automator
...
| Primary artifact | `postmark-workflow-runbook` |
| Routing tag | `postmark:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using postmark..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-workflow-runbook | structured-artifact | yes | next workflow or operator |
| postmark-workflow-runbook-scorecard | scorecard | yes | reviewer |
| postmark-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Postmark Workflow Automator, then checkpoint where Postmark state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using postmark. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `postmark-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] postmark-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:workflow-automator` consumers with approval and credential context attached.
