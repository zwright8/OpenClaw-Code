---
name: bigcommerce-report-builder
description: Use when tasks require bigcommerce report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Report Builder
...
| Primary artifact | `bigcommerce-report-bundle` |
| Routing tag | `bigcommerce:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from bigcommerce activity..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-report-bundle | structured-artifact | yes | next workflow or operator |
| bigcommerce-report-bundle-scorecard | scorecard | yes | reviewer |
| bigcommerce-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for BigCommerce Report Builder, then confirm which BigCommerce entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from bigcommerce activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `bigcommerce-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:report-builder` consumers with approval and credential context attached.
