---
name: bigcommerce-workflow-automator
description: Use when tasks require bigcommerce workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Workflow Automator
...
| Primary artifact | `bigcommerce-workflow-runbook` |
| Routing tag | `bigcommerce:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using bigcommerce..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-workflow-runbook | structured-artifact | yes | next workflow or operator |
| bigcommerce-workflow-runbook-scorecard | scorecard | yes | reviewer |
| bigcommerce-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for BigCommerce Workflow Automator, then checkpoint where BigCommerce state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using bigcommerce. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `bigcommerce-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:workflow-automator` consumers with approval and credential context attached.
