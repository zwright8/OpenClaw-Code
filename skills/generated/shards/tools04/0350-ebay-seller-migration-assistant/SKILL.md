---
name: ebay-seller-migration-assistant
description: Use when tasks require ebay seller migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Migration Assistant
...
| Primary artifact | `ebay-seller-migration-cutover-plan` |
| Routing tag | `ebay-seller:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when state changes are hard to undo once they begin. This specific skill turns eBay Seller Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from ebay seller..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| ebay-seller-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| ebay-seller-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for eBay Seller Migration Assistant before any eBay Seller rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from ebay seller. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `ebay-seller-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:migration-assistant` consumers with approval and credential context attached.
