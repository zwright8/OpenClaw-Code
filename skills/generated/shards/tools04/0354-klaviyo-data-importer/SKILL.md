---
name: klaviyo-data-importer
description: Use when tasks require klaviyo data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Data Importer
...
| Primary artifact | `klaviyo-import-ledger` |
| Routing tag | `klaviyo:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when state changes are hard to undo once they begin. This specific skill turns Klaviyo Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into klaviyo..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-import-ledger | structured-artifact | yes | next workflow or operator |
| klaviyo-import-ledger-scorecard | scorecard | yes | reviewer |
| klaviyo-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Klaviyo Data Importer before any Klaviyo write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into klaviyo. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `klaviyo-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:data-importer` consumers with approval and credential context attached.
