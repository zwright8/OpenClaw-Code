---
name: bigcommerce-sync-orchestrator
description: Use when tasks require bigcommerce sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Sync Orchestrator
...
| Primary artifact | `bigcommerce-reconciliation-bundle` |
| Routing tag | `bigcommerce:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns BigCommerce Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for bigcommerce records..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| bigcommerce-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| bigcommerce-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for BigCommerce Sync Orchestrator, then checkpoint the last known good cursor on both sides of BigCommerce.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for bigcommerce records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `bigcommerce-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:sync-orchestrator` consumers with approval and credential context attached.
