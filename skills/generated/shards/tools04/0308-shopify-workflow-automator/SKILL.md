---
name: shopify-workflow-automator
description: Use when tasks require shopify workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Shopify Workflow Automator
...
| Primary artifact | `shopify-workflow-runbook` |
| Routing tag | `shopify:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Shopify Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using shopify..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-workflow-runbook | structured-artifact | yes | next workflow or operator |
| shopify-workflow-runbook-scorecard | scorecard | yes | reviewer |
| shopify-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Shopify Workflow Automator, then checkpoint where Shopify state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using shopify. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `shopify-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] shopify-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:workflow-automator` consumers with approval and credential context attached.
