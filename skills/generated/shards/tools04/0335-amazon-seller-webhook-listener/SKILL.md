---
name: amazon-seller-webhook-listener
description: Use when tasks require amazon seller webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Webhook Listener
...
| Primary artifact | `amazon-seller-event-intake-log` |
| Routing tag | `amazon-seller:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Amazon Seller Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes amazon seller webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Webhook Listener` rather than generic brainstorming.
...
| Webhook signing secret | `signing_secret` | `AMAZON_SELLER_WEBHOOK_SECRET`, `AMAZON_SELLER_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| amazon-seller-event-intake-log | structured-artifact | yes | next workflow or operator |
| amazon-seller-event-intake-log-scorecard | scorecard | yes | reviewer |
| amazon-seller-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Amazon Seller Webhook Listener, then register the signing-secret policy for Amazon Seller.
...
3. Implement captures, verifies, and routes amazon seller webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `amazon-seller-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:webhook-listener` consumers with approval and credential context attached.
