---
name: woocommerce-data-exporter
description: Use when tasks require woocommerce data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WooCommerce Data Exporter
...
| Primary artifact | `woocommerce-export-package` |
| Routing tag | `woocommerce:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Data Exporter into a deterministic, auth-checked workflow for exports woocommerce entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-export-package | structured-artifact | yes | next workflow or operator |
| woocommerce-export-package-scorecard | scorecard | yes | reviewer |
| woocommerce-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for WooCommerce Data Exporter, then constrain the export scope for WooCommerce before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports woocommerce entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `woocommerce-export-package` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:data-exporter` consumers with approval and credential context attached.
