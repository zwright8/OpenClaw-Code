---
name: sendgrid-migration-assistant
description: Use when tasks require sendgrid migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Migration Assistant
...
| Primary artifact | `sendgrid-migration-cutover-plan` |
| Routing tag | `sendgrid:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when state changes are hard to undo once they begin. This specific skill turns SendGrid Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from sendgrid..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| sendgrid-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| sendgrid-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for SendGrid Migration Assistant before any SendGrid rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from sendgrid. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `sendgrid-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:migration-assistant` consumers with approval and credential context attached.
