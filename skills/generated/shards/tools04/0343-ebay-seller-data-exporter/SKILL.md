---
name: ebay-seller-data-exporter
description: Use when tasks require ebay seller data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Data Exporter
...
| Primary artifact | `ebay-seller-export-package` |
| Routing tag | `ebay-seller:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Data Exporter into a deterministic, auth-checked workflow for exports ebay seller entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-export-package | structured-artifact | yes | next workflow or operator |
| ebay-seller-export-package-scorecard | scorecard | yes | reviewer |
| ebay-seller-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for eBay Seller Data Exporter, then constrain the export scope for eBay Seller before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports ebay seller entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `ebay-seller-export-package` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:data-exporter` consumers with approval and credential context attached.
