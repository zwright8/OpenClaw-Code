---
name: amazon-seller-connector
description: Use when tasks require amazon seller connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `amazon-seller-connection-trace-bundle` |
| Routing tag | `amazon-seller:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Amazon Seller Connector into a deterministic, auth-checked workflow for unified read/write connector for amazon seller apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| amazon-seller-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| amazon-seller-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Amazon Seller Connector, then confirm the smallest read-only probe that proves access to Amazon Seller.
...
3. Specify the request and response contract for unified read/write connector for amazon seller apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `amazon-seller-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:connector` consumers with approval and credential context attached.
