---
name: twilio-approval-router
description: Use when tasks require twilio approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Approval Router
...
| Primary artifact | `twilio-approval-decision-packet` |
| Routing tag | `twilio:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Approval Router into a deterministic, auth-checked workflow for routes twilio-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| twilio-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| twilio-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Twilio Approval Router, then capture the manual fallback path for Twilio.
...
3. Implement routes twilio-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `twilio-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] twilio-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:approval-router` consumers with approval and credential context attached.
