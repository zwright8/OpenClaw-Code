---
name: amazon-seller-sync-orchestrator
description: Use when tasks require amazon seller sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Amazon Seller Sync Orchestrator
...
| Primary artifact | `amazon-seller-reconciliation-bundle` |
| Routing tag | `amazon-seller:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Amazon Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Amazon Seller Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for amazon seller records..

## Trigger Checklist
- [ ] The task explicitly requires `Amazon Seller Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| amazon-seller-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| amazon-seller-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| amazon-seller-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Amazon Seller Sync Orchestrator, then checkpoint the last known good cursor on both sides of Amazon Seller.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for amazon seller records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `amazon-seller-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] amazon-seller-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `amazon-seller-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `amazon-seller:sync-orchestrator` consumers with approval and credential context attached.
