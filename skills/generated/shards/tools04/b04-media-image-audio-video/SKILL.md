---
name: b04-media-image-audio-video
description: Use when converting, inspecting, encoding, or extracting image, audio, and video assets.
---
...
| Field | Value |
|---|---|
| Skill pack | `b04-media-image-audio-video` |
| Tool count | `25` |
| Inventory rank span | `1534-1993` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for media processing: image/audio/video tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `gphoto2` | `openimageio` | no | no |
| bounded mutation / transformation | `openimageio` | `gphoto2` | yes | yes |
| validation & artifact checks | `gphoto2` | `openimageio` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `gphoto2` | Command-line interface to libgphoto2 | local-installable |
| `openimageio` | Library for reading, processing and writing images | local-installable |
| `viu` | Simple terminal image viewer written in Rust | local-installable |
| `mplayer` | UNIX movie player | local-installable |
| `cava` | Console-based Audio Visualizer for ALSA | local-installable |
| `pulseaudio` | Sound system for POSIX OSes | local-installable |
| `ffmpeg@7` | Play, record, convert, and stream audio and video | local-installable |
| `gmic` | Full-Featured Open-Source Framework for Image Processing | local-installable |
| `kew` | Command-line music player | local-installable |
| `vhs` | Your CLI home video recorder | local-installable |
| `cmus` | Music player with an ncurses based interface | local-installable |
| `svtplay-dl` | Download videos from https://www.svtplay.se/ | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `gphoto2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openimageio` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `viu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mplayer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cava` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pulseaudio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ffmpeg@7` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gmic` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kew` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vhs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cmus` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `svtplay-dl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `theora` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `audacious` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mediamtx` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ncspot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `immich-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `instaloader` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `timg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bchunk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `feh` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lame` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `agg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `manim` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lilypond` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `gphoto2` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `openimageio` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `viu` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `mplayer` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `cava` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `pulseaudio` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a media processing: image/audio/video request, run dry-run first, then produce a validated artifact bundle.
