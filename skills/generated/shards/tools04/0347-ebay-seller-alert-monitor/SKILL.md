---
name: ebay-seller-alert-monitor
description: Use when tasks require ebay seller alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Alert Monitor
...
| Primary artifact | `ebay-seller-alert-state-bundle` |
| Routing tag | `ebay-seller:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Alert Monitor into a deterministic, auth-checked workflow for monitors key ebay seller signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| ebay-seller-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| ebay-seller-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for eBay Seller Alert Monitor, then lock the reviewed threshold set for eBay Seller.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key ebay seller signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `ebay-seller-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:alert-monitor` consumers with approval and credential context attached.
