---
name: postmark-alert-monitor
description: Use when tasks require postmark alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Alert Monitor
...
| Primary artifact | `postmark-alert-state-bundle` |
| Routing tag | `postmark:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Alert Monitor into a deterministic, auth-checked workflow for monitors key postmark signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| postmark-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| postmark-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Postmark Alert Monitor, then lock the reviewed threshold set for Postmark.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key postmark signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `postmark-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postmark-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:alert-monitor` consumers with approval and credential context attached.
