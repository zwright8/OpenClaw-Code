---
name: ebay-seller-workflow-automator
description: Use when tasks require ebay seller workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Workflow Automator
...
| Primary artifact | `ebay-seller-workflow-runbook` |
| Routing tag | `ebay-seller:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns eBay Seller Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using ebay seller..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-workflow-runbook | structured-artifact | yes | next workflow or operator |
| ebay-seller-workflow-runbook-scorecard | scorecard | yes | reviewer |
| ebay-seller-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for eBay Seller Workflow Automator, then checkpoint where eBay Seller state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using ebay seller. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `ebay-seller-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:workflow-automator` consumers with approval and credential context attached.
