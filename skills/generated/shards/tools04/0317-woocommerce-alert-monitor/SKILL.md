---
name: woocommerce-alert-monitor
description: Use when tasks require woocommerce alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WooCommerce Alert Monitor
...
| Primary artifact | `woocommerce-alert-state-bundle` |
| Routing tag | `woocommerce:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Alert Monitor into a deterministic, auth-checked workflow for monitors key woocommerce signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| woocommerce-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| woocommerce-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for WooCommerce Alert Monitor, then lock the reviewed threshold set for WooCommerce.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key woocommerce signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `woocommerce-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:alert-monitor` consumers with approval and credential context attached.
