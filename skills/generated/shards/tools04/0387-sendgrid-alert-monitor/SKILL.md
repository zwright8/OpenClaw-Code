---
name: sendgrid-alert-monitor
description: Use when tasks require sendgrid alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Alert Monitor
...
| Primary artifact | `sendgrid-alert-state-bundle` |
| Routing tag | `sendgrid:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Alert Monitor into a deterministic, auth-checked workflow for monitors key sendgrid signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| sendgrid-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| sendgrid-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for SendGrid Alert Monitor, then lock the reviewed threshold set for SendGrid.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key sendgrid signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `sendgrid-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:alert-monitor` consumers with approval and credential context attached.
