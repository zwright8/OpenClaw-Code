...
description: Use when tasks require woocommerce connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `woocommerce-connection-trace-bundle` |
| Routing tag | `woocommerce:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Connector into a deterministic, auth-checked workflow for unified read/write connector for woocommerce apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| woocommerce-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| woocommerce-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for WooCommerce Connector, then confirm the smallest read-only probe that proves access to WooCommerce.
...
3. Specify the request and response contract for unified read/write connector for woocommerce apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `woocommerce-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:connector` consumers with approval and credential context attached.
