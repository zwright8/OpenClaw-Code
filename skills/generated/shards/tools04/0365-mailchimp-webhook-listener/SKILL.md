---
name: mailchimp-webhook-listener
description: Use when tasks require mailchimp webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `mailchimp-event-intake-log` |
| Routing tag | `mailchimp:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Mailchimp Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes mailchimp webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `MAILCHIMP_CLIENT_ID`, `MAILCHIMP_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `MAILCHIMP_TOKEN`, `MAILCHIMP_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `MAILCHIMP_WEBHOOK_SECRET`, `MAILCHIMP_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| mailchimp-event-intake-log | structured-artifact | yes | next workflow or operator |
| mailchimp-event-intake-log-scorecard | scorecard | yes | reviewer |
| mailchimp-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Mailchimp Webhook Listener, then register the signing-secret policy for Mailchimp.
...
3. Implement captures, verifies, and routes mailchimp webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `mailchimp-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:webhook-listener` consumers with approval and credential context attached.
