---
name: sendgrid-workflow-automator
description: Use when tasks require sendgrid workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Workflow Automator
...
| Primary artifact | `sendgrid-workflow-runbook` |
| Routing tag | `sendgrid:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using sendgrid..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-workflow-runbook | structured-artifact | yes | next workflow or operator |
| sendgrid-workflow-runbook-scorecard | scorecard | yes | reviewer |
| sendgrid-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for SendGrid Workflow Automator, then checkpoint where SendGrid state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using sendgrid. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `sendgrid-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:workflow-automator` consumers with approval and credential context attached.
