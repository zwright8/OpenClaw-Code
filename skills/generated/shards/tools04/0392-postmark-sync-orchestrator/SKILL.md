---
name: postmark-sync-orchestrator
description: Use when tasks require postmark sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Sync Orchestrator
...
| Primary artifact | `postmark-reconciliation-bundle` |
| Routing tag | `postmark:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for postmark records..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| postmark-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| postmark-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Postmark Sync Orchestrator, then checkpoint the last known good cursor on both sides of Postmark.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for postmark records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `postmark-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] postmark-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:sync-orchestrator` consumers with approval and credential context attached.
