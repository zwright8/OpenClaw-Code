---
name: sendgrid-report-builder
description: Use when tasks require sendgrid report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Report Builder
...
| Primary artifact | `sendgrid-report-bundle` |
| Routing tag | `sendgrid:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from sendgrid activity..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-report-bundle | structured-artifact | yes | next workflow or operator |
| sendgrid-report-bundle-scorecard | scorecard | yes | reviewer |
| sendgrid-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for SendGrid Report Builder, then confirm which SendGrid entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from sendgrid activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `sendgrid-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:report-builder` consumers with approval and credential context attached.
