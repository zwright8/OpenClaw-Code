---
name: twilio-data-exporter
description: Use when tasks require twilio data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Data Exporter
...
| Primary artifact | `twilio-export-package` |
| Routing tag | `twilio:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Data Exporter into a deterministic, auth-checked workflow for exports twilio entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-export-package | structured-artifact | yes | next workflow or operator |
| twilio-export-package-scorecard | scorecard | yes | reviewer |
| twilio-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Twilio Data Exporter, then constrain the export scope for Twilio before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports twilio entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `twilio-export-package` so downstream owners do not need to rediscover context.
...
- [ ] twilio-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:data-exporter` consumers with approval and credential context attached.
