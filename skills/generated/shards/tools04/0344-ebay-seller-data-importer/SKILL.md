---
name: ebay-seller-data-importer
description: Use when tasks require ebay seller data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# eBay Seller Data Importer
...
| Primary artifact | `ebay-seller-import-ledger` |
| Routing tag | `ebay-seller:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because eBay Seller workflows degrade when state changes are hard to undo once they begin. This specific skill turns eBay Seller Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into ebay seller..

## Trigger Checklist
- [ ] The task explicitly requires `eBay Seller Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| ebay-seller-import-ledger | structured-artifact | yes | next workflow or operator |
| ebay-seller-import-ledger-scorecard | scorecard | yes | reviewer |
| ebay-seller-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for eBay Seller Data Importer before any eBay Seller write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into ebay seller. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `ebay-seller-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] ebay-seller-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `ebay-seller-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `ebay-seller:data-importer` consumers with approval and credential context attached.
