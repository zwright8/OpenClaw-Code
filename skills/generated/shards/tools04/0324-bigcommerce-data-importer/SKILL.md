---
name: bigcommerce-data-importer
description: Use when tasks require bigcommerce data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# BigCommerce Data Importer
...
| Primary artifact | `bigcommerce-import-ledger` |
| Routing tag | `bigcommerce:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because BigCommerce workflows degrade when state changes are hard to undo once they begin. This specific skill turns BigCommerce Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into bigcommerce..

## Trigger Checklist
- [ ] The task explicitly requires `BigCommerce Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| bigcommerce-import-ledger | structured-artifact | yes | next workflow or operator |
| bigcommerce-import-ledger-scorecard | scorecard | yes | reviewer |
| bigcommerce-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for BigCommerce Data Importer before any BigCommerce write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into bigcommerce. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `bigcommerce-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] bigcommerce-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `bigcommerce-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `bigcommerce:data-importer` consumers with approval and credential context attached.
