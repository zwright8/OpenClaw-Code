---
name: woocommerce-workflow-automator
description: Use when tasks require woocommerce workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# WooCommerce Workflow Automator
...
| Primary artifact | `woocommerce-workflow-runbook` |
| Routing tag | `woocommerce:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because WooCommerce workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns WooCommerce Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using woocommerce..

## Trigger Checklist
- [ ] The task explicitly requires `WooCommerce Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| woocommerce-workflow-runbook | structured-artifact | yes | next workflow or operator |
| woocommerce-workflow-runbook-scorecard | scorecard | yes | reviewer |
| woocommerce-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for WooCommerce Workflow Automator, then checkpoint where WooCommerce state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using woocommerce. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `woocommerce-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] woocommerce-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `woocommerce-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `woocommerce:workflow-automator` consumers with approval and credential context attached.
