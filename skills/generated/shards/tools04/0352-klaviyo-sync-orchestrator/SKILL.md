---
name: klaviyo-sync-orchestrator
description: Use when tasks require klaviyo sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Klaviyo Sync Orchestrator
...
| Primary artifact | `klaviyo-reconciliation-bundle` |
| Routing tag | `klaviyo:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Klaviyo workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Klaviyo Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for klaviyo records..

## Trigger Checklist
- [ ] The task explicitly requires `Klaviyo Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| klaviyo-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| klaviyo-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| klaviyo-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Klaviyo Sync Orchestrator, then checkpoint the last known good cursor on both sides of Klaviyo.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for klaviyo records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `klaviyo-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] klaviyo-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `klaviyo-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `klaviyo:sync-orchestrator` consumers with approval and credential context attached.
