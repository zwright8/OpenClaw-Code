...
description: Use when tasks require sendgrid connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sendgrid-connection-trace-bundle` |
| Routing tag | `sendgrid:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Connector into a deterministic, auth-checked workflow for unified read/write connector for sendgrid apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| sendgrid-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| sendgrid-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for SendGrid Connector, then confirm the smallest read-only probe that proves access to SendGrid.
...
3. Specify the request and response contract for unified read/write connector for sendgrid apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `sendgrid-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:connector` consumers with approval and credential context attached.
