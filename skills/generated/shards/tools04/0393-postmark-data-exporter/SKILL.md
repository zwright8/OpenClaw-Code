---
name: postmark-data-exporter
description: Use when tasks require postmark data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Data Exporter
...
| Primary artifact | `postmark-export-package` |
| Routing tag | `postmark:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Postmark Data Exporter into a deterministic, auth-checked workflow for exports postmark entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-export-package | structured-artifact | yes | next workflow or operator |
| postmark-export-package-scorecard | scorecard | yes | reviewer |
| postmark-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Postmark Data Exporter, then constrain the export scope for Postmark before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports postmark entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `postmark-export-package` so downstream owners do not need to rediscover context.
...
- [ ] postmark-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:data-exporter` consumers with approval and credential context attached.
