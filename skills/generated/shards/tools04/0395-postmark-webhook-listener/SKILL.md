---
name: postmark-webhook-listener
description: Use when tasks require postmark webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Webhook Listener
...
| Primary artifact | `postmark-event-intake-log` |
| Routing tag | `postmark:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Postmark Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes postmark webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `POSTMARK_CLIENT_ID`, `POSTMARK_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `POSTMARK_TOKEN`, `POSTMARK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `POSTMARK_WEBHOOK_SECRET`, `POSTMARK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| postmark-event-intake-log | structured-artifact | yes | next workflow or operator |
| postmark-event-intake-log-scorecard | scorecard | yes | reviewer |
| postmark-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Postmark Webhook Listener, then register the signing-secret policy for Postmark.
...
3. Implement captures, verifies, and routes postmark webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `postmark-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] postmark-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:webhook-listener` consumers with approval and credential context attached.
