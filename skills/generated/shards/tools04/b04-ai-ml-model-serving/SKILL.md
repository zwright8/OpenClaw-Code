...
| Field | Value |
|---|---|
| Skill pack | `b04-ai-ml-model-serving` |
| Tool count | `14` |
| Inventory rank span | `1546-1994` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for ai/ml model operations & serving tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `stockfish` | `localai` | no | no |
| bounded mutation / transformation | `localai` | `stockfish` | yes | yes |
| validation & artifact checks | `stockfish` | `localai` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `stockfish` | Strong open-source chess engine | local-installable |
| `localai` | OpenAI alternative | local-installable |
| `llmfit` | Find what models run on your hardware | local-installable |
| `gollama` | Go manage your Ollama models | local-installable |
| `echidna` | Ethereum smart contract fuzzer | local-installable |
| `mogenerator` | Generate Objective-C & Swift classes from your Core Data model | local-installable |
| `sentencepiece` | Unsupervised text tokenizer and detokenizer | local-installable |
| `faiss` | Efficient similarity search and clustering of dense vectors | local-installable |
| `vespa-cli` | Command-line tool for Vespa.ai | local-installable |
| `bitcoin` | Decentralized, peer to peer payment network | local-installable |
| `torchvision` | Datasets, transforms, and models for computer vision | local-installable |
| `langgraph-cli` | Command-line interface for deploying apps to the LangGraph platform | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `stockfish` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `localai` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `llmfit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gollama` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `echidna` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mogenerator` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sentencepiece` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `faiss` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vespa-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bitcoin` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `torchvision` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `langgraph-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aptos` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mistral-vibe` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `stockfish` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `localai` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `llmfit` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `gollama` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `echidna` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `mogenerator` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a ai/ml model operations & serving request, run dry-run first, then produce a validated artifact bundle.
