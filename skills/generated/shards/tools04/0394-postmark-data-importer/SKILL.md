---
name: postmark-data-importer
description: Use when tasks require postmark data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Postmark Data Importer
...
| Primary artifact | `postmark-import-ledger` |
| Routing tag | `postmark:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Postmark workflows degrade when state changes are hard to undo once they begin. This specific skill turns Postmark Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into postmark..

## Trigger Checklist
- [ ] The task explicitly requires `Postmark Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| postmark-import-ledger | structured-artifact | yes | next workflow or operator |
| postmark-import-ledger-scorecard | scorecard | yes | reviewer |
| postmark-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Postmark Data Importer before any Postmark write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into postmark. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `postmark-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] postmark-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `postmark-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `postmark:data-importer` consumers with approval and credential context attached.
