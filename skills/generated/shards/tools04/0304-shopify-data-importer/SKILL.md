---
name: shopify-data-importer
description: Use when tasks require shopify data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `shopify-import-ledger` |
| Routing tag | `shopify:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Shopify workflows degrade when state changes are hard to undo once they begin. This specific skill turns Shopify Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into shopify..

## Trigger Checklist
- [ ] The task explicitly requires `Shopify Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| shopify-import-ledger | structured-artifact | yes | next workflow or operator |
| shopify-import-ledger-scorecard | scorecard | yes | reviewer |
| shopify-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Shopify Data Importer before any Shopify write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into shopify. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `shopify-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] shopify-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `shopify-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `shopify:data-importer` consumers with approval and credential context attached.
