---
name: mailchimp-alert-monitor
description: Use when tasks require mailchimp alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Alert Monitor
...
| Primary artifact | `mailchimp-alert-state-bundle` |
| Routing tag | `mailchimp:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Alert Monitor into a deterministic, auth-checked workflow for monitors key mailchimp signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| mailchimp-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| mailchimp-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Mailchimp Alert Monitor, then lock the reviewed threshold set for Mailchimp.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key mailchimp signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `mailchimp-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:alert-monitor` consumers with approval and credential context attached.
