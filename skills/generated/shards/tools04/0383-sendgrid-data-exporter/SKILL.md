---
name: sendgrid-data-exporter
description: Use when tasks require sendgrid data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# SendGrid Data Exporter
...
| Primary artifact | `sendgrid-export-package` |
| Routing tag | `sendgrid:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because SendGrid workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns SendGrid Data Exporter into a deterministic, auth-checked workflow for exports sendgrid entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `SendGrid Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sendgrid-export-package | structured-artifact | yes | next workflow or operator |
| sendgrid-export-package-scorecard | scorecard | yes | reviewer |
| sendgrid-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for SendGrid Data Exporter, then constrain the export scope for SendGrid before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports sendgrid entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `sendgrid-export-package` so downstream owners do not need to rediscover context.
...
- [ ] sendgrid-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sendgrid-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sendgrid:data-exporter` consumers with approval and credential context attached.
