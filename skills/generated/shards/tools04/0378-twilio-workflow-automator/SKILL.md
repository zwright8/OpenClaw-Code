---
name: twilio-workflow-automator
description: Use when tasks require twilio workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Twilio Workflow Automator
...
| Primary artifact | `twilio-workflow-runbook` |
| Routing tag | `twilio:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Twilio workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Twilio Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using twilio..

## Trigger Checklist
- [ ] The task explicitly requires `Twilio Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| twilio-workflow-runbook | structured-artifact | yes | next workflow or operator |
| twilio-workflow-runbook-scorecard | scorecard | yes | reviewer |
| twilio-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Twilio Workflow Automator, then checkpoint where Twilio state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using twilio. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `twilio-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] twilio-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `twilio-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `twilio:workflow-automator` consumers with approval and credential context attached.
