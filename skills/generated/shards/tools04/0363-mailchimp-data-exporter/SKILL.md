---
name: mailchimp-data-exporter
description: Use when tasks require mailchimp data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Mailchimp Data Exporter
...
| Primary artifact | `mailchimp-export-package` |
| Routing tag | `mailchimp:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Mailchimp workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Mailchimp Data Exporter into a deterministic, auth-checked workflow for exports mailchimp entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Mailchimp Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| mailchimp-export-package | structured-artifact | yes | next workflow or operator |
| mailchimp-export-package-scorecard | scorecard | yes | reviewer |
| mailchimp-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Mailchimp Data Exporter, then constrain the export scope for Mailchimp before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports mailchimp entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `mailchimp-export-package` so downstream owners do not need to rediscover context.
...
- [ ] mailchimp-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `mailchimp-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `mailchimp:data-exporter` consumers with approval and credential context attached.
