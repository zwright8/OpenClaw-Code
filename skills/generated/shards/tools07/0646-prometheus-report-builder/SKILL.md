---
name: prometheus-report-builder
description: Use when tasks require prometheus report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Report Builder
...
| Primary artifact | `prometheus-report-bundle` |
| Routing tag | `prometheus:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from prometheus activity..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-report-bundle | structured-artifact | yes | next workflow or operator |
| prometheus-report-bundle-scorecard | scorecard | yes | reviewer |
| prometheus-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Prometheus Report Builder, then confirm which Prometheus entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from prometheus activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `prometheus-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:report-builder` consumers with approval and credential context attached.
