---
name: datadog-migration-assistant
description: Use when tasks require datadog migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `datadog-migration-cutover-plan` |
| Routing tag | `datadog:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when state changes are hard to undo once they begin. This specific skill turns Datadog Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from datadog..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| datadog-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| datadog-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Datadog Migration Assistant before any Datadog rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from datadog. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `datadog-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] datadog-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:migration-assistant` consumers with approval and credential context attached.
