---
name: splunk-report-builder
description: Use when tasks require splunk report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `splunk-report-bundle` |
| Routing tag | `splunk:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from splunk activity..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-report-bundle | structured-artifact | yes | next workflow or operator |
| splunk-report-bundle-scorecard | scorecard | yes | reviewer |
| splunk-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Splunk Report Builder, then confirm which Splunk entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from splunk activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `splunk-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] splunk-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:report-builder` consumers with approval and credential context attached.
