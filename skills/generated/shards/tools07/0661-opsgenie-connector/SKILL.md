...
description: Use when tasks require opsgenie connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `opsgenie-connection-trace-bundle` |
| Routing tag | `opsgenie:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Connector into a deterministic, auth-checked workflow for unified read/write connector for opsgenie apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| opsgenie-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| opsgenie-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Opsgenie Connector, then confirm the smallest read-only probe that proves access to Opsgenie.
...
3. Specify the request and response contract for unified read/write connector for opsgenie apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `opsgenie-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:connector` consumers with approval and credential context attached.
