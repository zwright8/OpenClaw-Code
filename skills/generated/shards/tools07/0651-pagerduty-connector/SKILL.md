...
description: Use when tasks require pagerduty connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pagerduty-connection-trace-bundle` |
| Routing tag | `pagerduty:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Connector into a deterministic, auth-checked workflow for unified read/write connector for pagerduty apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| pagerduty-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| pagerduty-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for PagerDuty Connector, then confirm the smallest read-only probe that proves access to PagerDuty.
...
3. Specify the request and response contract for unified read/write connector for pagerduty apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `pagerduty-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:connector` consumers with approval and credential context attached.
