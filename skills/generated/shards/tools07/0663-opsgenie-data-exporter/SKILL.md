---
name: opsgenie-data-exporter
description: Use when tasks require opsgenie data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Data Exporter
...
| Primary artifact | `opsgenie-export-package` |
| Routing tag | `opsgenie:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Data Exporter into a deterministic, auth-checked workflow for exports opsgenie entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-export-package | structured-artifact | yes | next workflow or operator |
| opsgenie-export-package-scorecard | scorecard | yes | reviewer |
| opsgenie-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Opsgenie Data Exporter, then constrain the export scope for Opsgenie before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports opsgenie entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `opsgenie-export-package` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:data-exporter` consumers with approval and credential context attached.
