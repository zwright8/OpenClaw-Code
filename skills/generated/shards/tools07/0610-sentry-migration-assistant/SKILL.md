---
name: sentry-migration-assistant
description: Use when tasks require sentry migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Migration Assistant
...
| Primary artifact | `sentry-migration-cutover-plan` |
| Routing tag | `sentry:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sentry Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from sentry..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| sentry-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| sentry-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Sentry Migration Assistant before any Sentry rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from sentry. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `sentry-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] sentry-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:migration-assistant` consumers with approval and credential context attached.
