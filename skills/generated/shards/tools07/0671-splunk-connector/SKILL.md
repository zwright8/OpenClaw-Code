...
description: Use when tasks require splunk connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `splunk-connection-trace-bundle` |
| Routing tag | `splunk:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Connector into a deterministic, auth-checked workflow for unified read/write connector for splunk apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| splunk-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| splunk-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Splunk Connector, then confirm the smallest read-only probe that proves access to Splunk.
...
3. Specify the request and response contract for unified read/write connector for splunk apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `splunk-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] splunk-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:connector` consumers with approval and credential context attached.
