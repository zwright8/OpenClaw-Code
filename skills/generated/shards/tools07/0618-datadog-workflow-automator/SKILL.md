---
name: datadog-workflow-automator
description: Use when tasks require datadog workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Workflow Automator
...
| Primary artifact | `datadog-workflow-runbook` |
| Routing tag | `datadog:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using datadog..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-workflow-runbook | structured-artifact | yes | next workflow or operator |
| datadog-workflow-runbook-scorecard | scorecard | yes | reviewer |
| datadog-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Datadog Workflow Automator, then checkpoint where Datadog state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using datadog. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `datadog-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] datadog-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:workflow-automator` consumers with approval and credential context attached.
