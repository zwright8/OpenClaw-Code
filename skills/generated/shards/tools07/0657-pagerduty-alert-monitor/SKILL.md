---
name: pagerduty-alert-monitor
description: Use when tasks require pagerduty alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Alert Monitor
...
| Primary artifact | `pagerduty-alert-state-bundle` |
| Routing tag | `pagerduty:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Alert Monitor into a deterministic, auth-checked workflow for monitors key pagerduty signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| pagerduty-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| pagerduty-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for PagerDuty Alert Monitor, then lock the reviewed threshold set for PagerDuty.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key pagerduty signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `pagerduty-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:alert-monitor` consumers with approval and credential context attached.
