---
name: sentry-alert-monitor
description: Use when tasks require sentry alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Alert Monitor
...
| Primary artifact | `sentry-alert-state-bundle` |
| Routing tag | `sentry:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Alert Monitor into a deterministic, auth-checked workflow for monitors key sentry signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| sentry-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| sentry-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Sentry Alert Monitor, then lock the reviewed threshold set for Sentry.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key sentry signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `sentry-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sentry-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:alert-monitor` consumers with approval and credential context attached.
