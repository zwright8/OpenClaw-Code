---
name: sumo-logic-report-builder
description: Use when tasks require sumo logic report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Report Builder
...
| Primary artifact | `sumo-logic-report-bundle` |
| Routing tag | `sumo-logic:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from sumo logic activity..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-report-bundle | structured-artifact | yes | next workflow or operator |
| sumo-logic-report-bundle-scorecard | scorecard | yes | reviewer |
| sumo-logic-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Sumo Logic Report Builder, then confirm which Sumo Logic entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from sumo logic activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `sumo-logic-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:report-builder` consumers with approval and credential context attached.
