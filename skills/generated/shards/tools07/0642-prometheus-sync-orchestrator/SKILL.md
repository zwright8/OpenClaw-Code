---
name: prometheus-sync-orchestrator
description: Use when tasks require prometheus sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Sync Orchestrator
...
| Primary artifact | `prometheus-reconciliation-bundle` |
| Routing tag | `prometheus:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for prometheus records..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| prometheus-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| prometheus-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Prometheus Sync Orchestrator, then checkpoint the last known good cursor on both sides of Prometheus.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for prometheus records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `prometheus-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:sync-orchestrator` consumers with approval and credential context attached.
