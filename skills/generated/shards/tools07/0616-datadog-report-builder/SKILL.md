---
name: datadog-report-builder
description: Use when tasks require datadog report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Report Builder
...
| Primary artifact | `datadog-report-bundle` |
| Routing tag | `datadog:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from datadog activity..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-report-bundle | structured-artifact | yes | next workflow or operator |
| datadog-report-bundle-scorecard | scorecard | yes | reviewer |
| datadog-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Datadog Report Builder, then confirm which Datadog entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from datadog activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `datadog-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] datadog-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:report-builder` consumers with approval and credential context attached.
