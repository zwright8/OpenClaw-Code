---
name: grafana-webhook-listener
description: Use when tasks require grafana webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `grafana-event-intake-log` |
| Routing tag | `grafana:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Grafana Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes grafana webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `GRAFANA_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `GRAFANA_TOKEN`, `GRAFANA_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `GRAFANA_WEBHOOK_SECRET`, `GRAFANA_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| grafana-event-intake-log | structured-artifact | yes | next workflow or operator |
| grafana-event-intake-log-scorecard | scorecard | yes | reviewer |
| grafana-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Grafana Webhook Listener, then register the signing-secret policy for Grafana.
...
3. Implement captures, verifies, and routes grafana webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `grafana-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] grafana-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:webhook-listener` consumers with approval and credential context attached.
