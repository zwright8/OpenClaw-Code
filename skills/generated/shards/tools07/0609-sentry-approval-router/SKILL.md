---
name: sentry-approval-router
description: Use when tasks require sentry approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Approval Router
...
| Primary artifact | `sentry-approval-decision-packet` |
| Routing tag | `sentry:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Approval Router into a deterministic, auth-checked workflow for routes sentry-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| sentry-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| sentry-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Sentry Approval Router, then capture the manual fallback path for Sentry.
...
3. Implement routes sentry-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `sentry-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] sentry-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:approval-router` consumers with approval and credential context attached.
