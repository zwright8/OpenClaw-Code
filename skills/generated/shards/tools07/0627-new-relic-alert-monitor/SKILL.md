---
name: new-relic-alert-monitor
description: Use when tasks require new relic alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Alert Monitor
...
| Primary artifact | `new-relic-alert-state-bundle` |
| Routing tag | `new-relic:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Alert Monitor into a deterministic, auth-checked workflow for monitors key new relic signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| new-relic-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| new-relic-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for New Relic Alert Monitor, then lock the reviewed threshold set for New Relic.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key new relic signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `new-relic-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:alert-monitor` consumers with approval and credential context attached.
