---
name: datadog-data-exporter
description: Use when tasks require datadog data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Data Exporter
...
| Primary artifact | `datadog-export-package` |
| Routing tag | `datadog:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Data Exporter into a deterministic, auth-checked workflow for exports datadog entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-export-package | structured-artifact | yes | next workflow or operator |
| datadog-export-package-scorecard | scorecard | yes | reviewer |
| datadog-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Datadog Data Exporter, then constrain the export scope for Datadog before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports datadog entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `datadog-export-package` so downstream owners do not need to rediscover context.
...
- [ ] datadog-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:data-exporter` consumers with approval and credential context attached.
