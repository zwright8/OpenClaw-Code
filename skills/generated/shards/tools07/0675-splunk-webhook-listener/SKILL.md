---
name: splunk-webhook-listener
description: Use when tasks require splunk webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Webhook Listener
...
| Primary artifact | `splunk-event-intake-log` |
| Routing tag | `splunk:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Splunk Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes splunk webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `SPLUNK_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `SPLUNK_TOKEN`, `SPLUNK_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SPLUNK_WEBHOOK_SECRET`, `SPLUNK_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| splunk-event-intake-log | structured-artifact | yes | next workflow or operator |
| splunk-event-intake-log-scorecard | scorecard | yes | reviewer |
| splunk-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Splunk Webhook Listener, then register the signing-secret policy for Splunk.
...
3. Implement captures, verifies, and routes splunk webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `splunk-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] splunk-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:webhook-listener` consumers with approval and credential context attached.
