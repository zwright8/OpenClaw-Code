---
name: pagerduty-workflow-automator
description: Use when tasks require pagerduty workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Workflow Automator
...
| Primary artifact | `pagerduty-workflow-runbook` |
| Routing tag | `pagerduty:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using pagerduty..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-workflow-runbook | structured-artifact | yes | next workflow or operator |
| pagerduty-workflow-runbook-scorecard | scorecard | yes | reviewer |
| pagerduty-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for PagerDuty Workflow Automator, then checkpoint where PagerDuty state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using pagerduty. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `pagerduty-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:workflow-automator` consumers with approval and credential context attached.
