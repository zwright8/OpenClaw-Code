---
name: pagerduty-report-builder
description: Use when tasks require pagerduty report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `pagerduty-report-bundle` |
| Routing tag | `pagerduty:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from pagerduty activity..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-report-bundle | structured-artifact | yes | next workflow or operator |
| pagerduty-report-bundle-scorecard | scorecard | yes | reviewer |
| pagerduty-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for PagerDuty Report Builder, then confirm which PagerDuty entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from pagerduty activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `pagerduty-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:report-builder` consumers with approval and credential context attached.
