---
name: grafana-sync-orchestrator
description: Use when tasks require grafana sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `grafana-reconciliation-bundle` |
| Routing tag | `grafana:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for grafana records..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| grafana-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| grafana-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Grafana Sync Orchestrator, then checkpoint the last known good cursor on both sides of Grafana.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for grafana records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `grafana-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] grafana-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:sync-orchestrator` consumers with approval and credential context attached.
