---
name: grafana-workflow-automator
description: Use when tasks require grafana workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Grafana Workflow Automator
...
| Primary artifact | `grafana-workflow-runbook` |
| Routing tag | `grafana:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using grafana..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-workflow-runbook | structured-artifact | yes | next workflow or operator |
| grafana-workflow-runbook-scorecard | scorecard | yes | reviewer |
| grafana-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Grafana Workflow Automator, then checkpoint where Grafana state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using grafana. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `grafana-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] grafana-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:workflow-automator` consumers with approval and credential context attached.
