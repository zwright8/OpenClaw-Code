---
name: prometheus-data-importer
description: Use when tasks require prometheus data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Data Importer
...
| Primary artifact | `prometheus-import-ledger` |
| Routing tag | `prometheus:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when state changes are hard to undo once they begin. This specific skill turns Prometheus Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into prometheus..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-import-ledger | structured-artifact | yes | next workflow or operator |
| prometheus-import-ledger-scorecard | scorecard | yes | reviewer |
| prometheus-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Prometheus Data Importer before any Prometheus write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into prometheus. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `prometheus-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:data-importer` consumers with approval and credential context attached.
