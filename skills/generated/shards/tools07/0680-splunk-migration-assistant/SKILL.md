---
name: splunk-migration-assistant
description: Use when tasks require splunk migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Migration Assistant
...
| Primary artifact | `splunk-migration-cutover-plan` |
| Routing tag | `splunk:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when state changes are hard to undo once they begin. This specific skill turns Splunk Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from splunk..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| splunk-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| splunk-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Splunk Migration Assistant before any Splunk rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from splunk. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `splunk-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] splunk-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:migration-assistant` consumers with approval and credential context attached.
