---
name: opensearch-report-builder
description: Use when tasks require opensearch report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Report Builder
...
| Primary artifact | `opensearch-report-bundle` |
| Routing tag | `opensearch:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from opensearch activity..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-report-bundle | structured-artifact | yes | next workflow or operator |
| opensearch-report-bundle-scorecard | scorecard | yes | reviewer |
| opensearch-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for OpenSearch Report Builder, then confirm which OpenSearch entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from opensearch activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `opensearch-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:report-builder` consumers with approval and credential context attached.
