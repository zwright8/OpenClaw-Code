---
name: opsgenie-data-importer
description: Use when tasks require opsgenie data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Data Importer
...
| Primary artifact | `opsgenie-import-ledger` |
| Routing tag | `opsgenie:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when state changes are hard to undo once they begin. This specific skill turns Opsgenie Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into opsgenie..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-import-ledger | structured-artifact | yes | next workflow or operator |
| opsgenie-import-ledger-scorecard | scorecard | yes | reviewer |
| opsgenie-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Opsgenie Data Importer before any Opsgenie write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into opsgenie. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `opsgenie-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:data-importer` consumers with approval and credential context attached.
