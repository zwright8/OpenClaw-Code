---
name: sumo-logic-sync-orchestrator
description: Use when tasks require sumo logic sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Sync Orchestrator
...
| Primary artifact | `sumo-logic-reconciliation-bundle` |
| Routing tag | `sumo-logic:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for sumo logic records..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| sumo-logic-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| sumo-logic-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Sumo Logic Sync Orchestrator, then checkpoint the last known good cursor on both sides of Sumo Logic.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for sumo logic records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `sumo-logic-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:sync-orchestrator` consumers with approval and credential context attached.
