---
name: prometheus-workflow-automator
description: Use when tasks require prometheus workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Workflow Automator
...
| Primary artifact | `prometheus-workflow-runbook` |
| Routing tag | `prometheus:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using prometheus..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-workflow-runbook | structured-artifact | yes | next workflow or operator |
| prometheus-workflow-runbook-scorecard | scorecard | yes | reviewer |
| prometheus-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Prometheus Workflow Automator, then checkpoint where Prometheus state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using prometheus. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `prometheus-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:workflow-automator` consumers with approval and credential context attached.
