---
name: opsgenie-report-builder
description: Use when tasks require opsgenie report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Report Builder
...
| Primary artifact | `opsgenie-report-bundle` |
| Routing tag | `opsgenie:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from opsgenie activity..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-report-bundle | structured-artifact | yes | next workflow or operator |
| opsgenie-report-bundle-scorecard | scorecard | yes | reviewer |
| opsgenie-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Opsgenie Report Builder, then confirm which Opsgenie entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from opsgenie activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `opsgenie-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:report-builder` consumers with approval and credential context attached.
