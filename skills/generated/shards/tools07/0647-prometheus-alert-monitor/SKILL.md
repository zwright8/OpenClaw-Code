---
name: prometheus-alert-monitor
description: Use when tasks require prometheus alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Alert Monitor
...
| Primary artifact | `prometheus-alert-state-bundle` |
| Routing tag | `prometheus:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Alert Monitor into a deterministic, auth-checked workflow for monitors key prometheus signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| prometheus-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| prometheus-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Prometheus Alert Monitor, then lock the reviewed threshold set for Prometheus.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key prometheus signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `prometheus-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:alert-monitor` consumers with approval and credential context attached.
