---
name: pagerduty-approval-router
description: Use when tasks require pagerduty approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Approval Router
...
| Primary artifact | `pagerduty-approval-decision-packet` |
| Routing tag | `pagerduty:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Approval Router into a deterministic, auth-checked workflow for routes pagerduty-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| pagerduty-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| pagerduty-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for PagerDuty Approval Router, then capture the manual fallback path for PagerDuty.
...
3. Implement routes pagerduty-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `pagerduty-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:approval-router` consumers with approval and credential context attached.
