---
name: splunk-approval-router
description: Use when tasks require splunk approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Approval Router
...
| Primary artifact | `splunk-approval-decision-packet` |
| Routing tag | `splunk:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Approval Router into a deterministic, auth-checked workflow for routes splunk-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| splunk-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| splunk-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Splunk Approval Router, then capture the manual fallback path for Splunk.
...
3. Implement routes splunk-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `splunk-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] splunk-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:approval-router` consumers with approval and credential context attached.
