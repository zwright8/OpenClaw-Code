...
description: Use when tasks require grafana data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `grafana-export-package` |
| Routing tag | `grafana:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Data Exporter into a deterministic, auth-checked workflow for exports grafana entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-export-package | structured-artifact | yes | next workflow or operator |
| grafana-export-package-scorecard | scorecard | yes | reviewer |
| grafana-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Grafana Data Exporter, then constrain the export scope for Grafana before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports grafana entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `grafana-export-package` so downstream owners do not need to rediscover context.
...
- [ ] grafana-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:data-exporter` consumers with approval and credential context attached.
