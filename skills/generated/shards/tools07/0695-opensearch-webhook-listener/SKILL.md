---
name: opensearch-webhook-listener
description: Use when tasks require opensearch webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Webhook Listener
...
| Primary artifact | `opensearch-event-intake-log` |
| Routing tag | `opensearch:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns OpenSearch Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes opensearch webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `OPENSEARCH_CLIENT_ID`, `OPENSEARCH_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `OPENSEARCH_TOKEN`, `OPENSEARCH_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `OPENSEARCH_WEBHOOK_SECRET`, `OPENSEARCH_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| opensearch-event-intake-log | structured-artifact | yes | next workflow or operator |
| opensearch-event-intake-log-scorecard | scorecard | yes | reviewer |
| opensearch-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for OpenSearch Webhook Listener, then register the signing-secret policy for OpenSearch.
...
3. Implement captures, verifies, and routes opensearch webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `opensearch-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:webhook-listener` consumers with approval and credential context attached.
