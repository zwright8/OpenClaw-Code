...
description: Use when tasks require prometheus data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `prometheus-export-package` |
| Routing tag | `prometheus:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Data Exporter into a deterministic, auth-checked workflow for exports prometheus entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-export-package | structured-artifact | yes | next workflow or operator |
| prometheus-export-package-scorecard | scorecard | yes | reviewer |
| prometheus-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Prometheus Data Exporter, then constrain the export scope for Prometheus before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports prometheus entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `prometheus-export-package` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:data-exporter` consumers with approval and credential context attached.
