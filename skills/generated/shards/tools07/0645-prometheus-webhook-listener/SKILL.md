---
name: prometheus-webhook-listener
description: Use when tasks require prometheus webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Webhook Listener
...
| Primary artifact | `prometheus-event-intake-log` |
| Routing tag | `prometheus:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Prometheus Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes prometheus webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `PROMETHEUS_CLIENT_ID`, `PROMETHEUS_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `PROMETHEUS_TOKEN`, `PROMETHEUS_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `PROMETHEUS_WEBHOOK_SECRET`, `PROMETHEUS_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| prometheus-event-intake-log | structured-artifact | yes | next workflow or operator |
| prometheus-event-intake-log-scorecard | scorecard | yes | reviewer |
| prometheus-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Prometheus Webhook Listener, then register the signing-secret policy for Prometheus.
...
3. Implement captures, verifies, and routes prometheus webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `prometheus-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:webhook-listener` consumers with approval and credential context attached.
