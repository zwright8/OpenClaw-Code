...
| Field | Value |
|---|---|
| Skill pack | `b07-db-k8s-firmware-observability` |
| Tool count | `26` |
| Inventory rank span | `3435-3469` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for databases, kubernetes security, firmware, observability tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `algol68g` | `berkeley-db@4` | no | no |
| bounded mutation / transformation | `berkeley-db@4` | `algol68g` | yes | yes |
| validation & artifact checks | `algol68g` | `berkeley-db@4` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `algol68g` | Algol 68 compiler-interpreter | local-installable |
| `berkeley-db@4` | High performance key/value database | local-installable |
| `libgr` | GR framework: a graphics library for visualisation applications | local-installable |
| `pgbouncer` | Lightweight connection pooler for PostgreSQL | local-installable |
| `pqiv` | Powerful image viewer with minimal UI | local-installable |
| `simg2img` | Tool to convert Android sparse images to raw images and back | local-installable |
| `fwup` | Configurable embedded Linux firmware update creator and runner | local-installable |
| `neovim-qt` | Neovim GUI, in Qt | local-installable |
| `verapdf` | Open-source industry-supported PDF/A validation | local-installable |
| `libmtp` | Implementation of Microsoft's Media Transfer Protocol (MTP) | local-installable |
| `etcd-cpp-apiv3` | C++ implementation for etcd's v3 client API, i.e., ETCDCTL_API=3 | local-installable |
| `pkcs11-tools` | Tools to manage objects on PKCS#11 crypotographic tokens | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `algol68g` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `berkeley-db@4` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libgr` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `pgbouncer` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pqiv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `simg2img` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fwup` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `neovim-qt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `verapdf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libmtp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `etcd-cpp-apiv3` | HTTPS/REST, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `pkcs11-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `t-rec` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `commandbox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mesa-glu` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `kube-bench` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `node-red` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `kubernetes-cli@1.30` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `cf2tf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `quilt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fortls` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `imageoptim-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `metview` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `victoriametrics` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `logdy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gptline` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `kube-bench`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `kubernetes-cli@1.30`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `algol68g` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `berkeley-db@4` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `libgr` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `pgbouncer` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `pqiv` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `simg2img` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a databases, kubernetes security, firmware, observability request, run dry-run first, then produce a validated artifact bundle.
