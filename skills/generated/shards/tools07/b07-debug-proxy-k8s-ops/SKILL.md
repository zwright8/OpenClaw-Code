---
name: b07-debug-proxy-k8s-ops
description: Use when doing cross-debugging, syslog/proxy transport work, OpenTofu/Kubernetes platform operations, profiling, package isolation, and binary/toolchain validation.
---

# Debugging, proxying, and Kubernetes operations

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-debug-proxy-k8s-ops` |
| Tool count | `26` |
| Inventory rank span | `3094-3122` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for debugging, proxying, kubernetes operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `aarch64-elf-gdb` | `mcp-proxy` | no | no |
| bounded mutation / transformation | `mcp-proxy` | `aarch64-elf-gdb` | yes | yes |
| validation & artifact checks | `aarch64-elf-gdb` | `mcp-proxy` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `aarch64-elf-gdb` | GNU debugger for aarch64-elf cross development | local-installable |
| `mcp-proxy` | Bridge between Streamable HTTP and stdio MCP transports | local-installable |
| `rsyslog` | Enhanced, multi-threaded syslogd | local-installable |
| `tofuenv` | OpenTofu version manager inspired by tfenv | local-installable |
| `gprof2dot` | Convert the output from many profilers into a Graphviz dot graph | local-installable |
| `msgpack` | Library for a binary-based efficient data interchange format | local-installable |
| `datafusion` | Apache Arrow DataFusion and Ballista query engines | local-installable |
| `boxes` | Draw boxes around text | local-installable |
| `sdl2_net` | Small sample cross-platform networking library | local-installable |
| `backgroundremover` | Remove background from images and video using AI | local-installable |
| `react-native-cli` | Tools for creating native apps for Android and iOS | local-installable |
| `qmmp` | Qt-based Multimedia Player | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `aarch64-elf-gdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `mcp-proxy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rsyslog` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tofuenv` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gprof2dot` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `msgpack` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `datafusion` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `boxes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sdl2_net` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `backgroundremover` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `react-native-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qmmp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `py-spy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `faad2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `sambamba` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `datalad` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `connect` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxft` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `secp256k1` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `nuitka` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `phpbrew` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `egctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `faust` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libosinfo` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libpq@16` | SQL/DB protocol, Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `cargo-udeps` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `aarch64-elf-gdb` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `mcp-proxy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write/orchestrate command sequence, capture outputs and exit code in handoff packet.
3. `rsyslog` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `tofuenv` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `gprof2dot` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `msgpack` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a debugging, proxying, kubernetes operations request, run dry-run first, then produce a validated artifact bundle.
