---
name: sentry-data-exporter
description: Use when tasks require sentry data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Data Exporter
...
| Primary artifact | `sentry-export-package` |
| Routing tag | `sentry:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Data Exporter into a deterministic, auth-checked workflow for exports sentry entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-export-package | structured-artifact | yes | next workflow or operator |
| sentry-export-package-scorecard | scorecard | yes | reviewer |
| sentry-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Sentry Data Exporter, then constrain the export scope for Sentry before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports sentry entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `sentry-export-package` so downstream owners do not need to rediscover context.
...
- [ ] sentry-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:data-exporter` consumers with approval and credential context attached.
