---
name: pagerduty-migration-assistant
description: Use when tasks require pagerduty migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Migration Assistant
...
| Primary artifact | `pagerduty-migration-cutover-plan` |
| Routing tag | `pagerduty:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when state changes are hard to undo once they begin. This specific skill turns PagerDuty Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from pagerduty..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| pagerduty-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| pagerduty-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for PagerDuty Migration Assistant before any PagerDuty rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from pagerduty. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `pagerduty-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:migration-assistant` consumers with approval and credential context attached.
