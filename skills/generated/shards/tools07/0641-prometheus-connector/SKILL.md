...
description: Use when tasks require prometheus connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `prometheus-connection-trace-bundle` |
| Routing tag | `prometheus:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Connector into a deterministic, auth-checked workflow for unified read/write connector for prometheus apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| prometheus-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| prometheus-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Prometheus Connector, then confirm the smallest read-only probe that proves access to Prometheus.
...
3. Specify the request and response contract for unified read/write connector for prometheus apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `prometheus-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:connector` consumers with approval and credential context attached.
