---
name: datadog-sync-orchestrator
description: Use when tasks require datadog sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Sync Orchestrator
...
| Primary artifact | `datadog-reconciliation-bundle` |
| Routing tag | `datadog:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for datadog records..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| datadog-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| datadog-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Datadog Sync Orchestrator, then checkpoint the last known good cursor on both sides of Datadog.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for datadog records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `datadog-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] datadog-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:sync-orchestrator` consumers with approval and credential context attached.
