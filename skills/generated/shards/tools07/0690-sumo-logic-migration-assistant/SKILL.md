---
name: sumo-logic-migration-assistant
description: Use when tasks require sumo logic migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Migration Assistant
...
| Primary artifact | `sumo-logic-migration-cutover-plan` |
| Routing tag | `sumo-logic:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sumo Logic Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from sumo logic..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| sumo-logic-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| sumo-logic-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Sumo Logic Migration Assistant before any Sumo Logic rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from sumo logic. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `sumo-logic-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:migration-assistant` consumers with approval and credential context attached.
