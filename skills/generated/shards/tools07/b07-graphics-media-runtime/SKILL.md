---
name: b07-graphics-media-runtime
description: Use when handling graphics/emulation stacks, DICOM and spatial tooling, shell debugging, crypto/media controls, distributed runtime setup, and LaunchDarkly or jj workflows.
---

# Graphics, media runtimes, and feature-flag operations

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-graphics-media-runtime` |
| Tool count | `26` |
| Inventory rank span | `3367-3399` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for graphics, media runtimes, feature-flag operations tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `aws-lc` | `gupnp-tools` | no | no |
| bounded mutation / transformation | `gupnp-tools` | `aws-lc` | yes | yes |
| validation & artifact checks | `aws-lc` | `gupnp-tools` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `aws-lc` | General-purpose cryptographic library | local-installable |
| `gupnp-tools` | Free replacements of Intel's UPnP tools | local-installable |
| `media-control` | Control and observe media playback from the command-line | local-installable |
| `lazyjj` | TUI for Jujutsu/jj | local-installable |
| `open-scene-graph` | 3D graphics toolkit | local-installable |
| `bochs` | Open source IA-32 (x86) PC emulator written in C++ | local-installable |
| `bashdb` | Bash shell debugger | local-installable |
| `rojo` | Professional grade Roblox development tools | local-installable |
| `gitlogue` | Cinematic Git commit replay tool | local-installable |
| `gdcm` | Grassroots DICOM library and utilities for medical files | local-installable |
| `srecord` | Tools for manipulating EPROM load files | local-installable |
| `spatialite-gui` | GUI tool supporting SpatiaLite | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `aws-lc` | Local library API/ABI, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gupnp-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `media-control` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lazyjj` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `open-scene-graph` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bochs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bashdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rojo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gitlogue` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gdcm` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `srecord` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spatialite-gui` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cookcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hbase` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `most` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vulkan-profiles` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openvdb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `somo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vgmstream` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `gmime` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `zsh-git-prompt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dotnet@9` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dovi_tool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `snapcast` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `erlang@27` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ldcli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `aws-lc`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `aws-lc` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `gupnp-tools` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `media-control` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `lazyjj` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `open-scene-graph` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `bochs` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a graphics, media runtimes, feature-flag operations request, run dry-run first, then produce a validated artifact bundle.
