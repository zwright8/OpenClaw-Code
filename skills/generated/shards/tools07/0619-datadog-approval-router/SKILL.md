---
name: datadog-approval-router
description: Use when tasks require datadog approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Approval Router
...
| Primary artifact | `datadog-approval-decision-packet` |
| Routing tag | `datadog:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Approval Router into a deterministic, auth-checked workflow for routes datadog-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| datadog-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| datadog-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Datadog Approval Router, then capture the manual fallback path for Datadog.
...
3. Implement routes datadog-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `datadog-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] datadog-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:approval-router` consumers with approval and credential context attached.
