---
name: grafana-data-importer
description: Use when tasks require grafana data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Grafana Data Importer
...
| Primary artifact | `grafana-import-ledger` |
| Routing tag | `grafana:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when state changes are hard to undo once they begin. This specific skill turns Grafana Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into grafana..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-import-ledger | structured-artifact | yes | next workflow or operator |
| grafana-import-ledger-scorecard | scorecard | yes | reviewer |
| grafana-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Grafana Data Importer before any Grafana write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into grafana. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `grafana-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] grafana-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:data-importer` consumers with approval and credential context attached.
