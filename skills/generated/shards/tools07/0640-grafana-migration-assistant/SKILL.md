---
name: grafana-migration-assistant
description: Use when tasks require grafana migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Grafana Migration Assistant
...
| Primary artifact | `grafana-migration-cutover-plan` |
| Routing tag | `grafana:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when state changes are hard to undo once they begin. This specific skill turns Grafana Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from grafana..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| grafana-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| grafana-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Grafana Migration Assistant before any Grafana rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from grafana. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `grafana-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] grafana-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:migration-assistant` consumers with approval and credential context attached.
