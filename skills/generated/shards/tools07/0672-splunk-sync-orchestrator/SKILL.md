---
name: splunk-sync-orchestrator
description: Use when tasks require splunk sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Sync Orchestrator
...
| Primary artifact | `splunk-reconciliation-bundle` |
| Routing tag | `splunk:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for splunk records..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| splunk-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| splunk-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Splunk Sync Orchestrator, then checkpoint the last known good cursor on both sides of Splunk.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for splunk records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `splunk-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] splunk-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:sync-orchestrator` consumers with approval and credential context attached.
