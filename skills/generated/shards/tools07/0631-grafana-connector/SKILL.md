...
description: Use when tasks require grafana connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `grafana-connection-trace-bundle` |
| Routing tag | `grafana:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Connector into a deterministic, auth-checked workflow for unified read/write connector for grafana apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| grafana-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| grafana-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Grafana Connector, then confirm the smallest read-only probe that proves access to Grafana.
...
3. Specify the request and response contract for unified read/write connector for grafana apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `grafana-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] grafana-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:connector` consumers with approval and credential context attached.
