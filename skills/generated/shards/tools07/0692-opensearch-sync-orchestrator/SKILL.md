---
name: opensearch-sync-orchestrator
description: Use when tasks require opensearch sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Sync Orchestrator
...
| Primary artifact | `opensearch-reconciliation-bundle` |
| Routing tag | `opensearch:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for opensearch records..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| opensearch-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| opensearch-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for OpenSearch Sync Orchestrator, then checkpoint the last known good cursor on both sides of OpenSearch.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for opensearch records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `opensearch-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:sync-orchestrator` consumers with approval and credential context attached.
