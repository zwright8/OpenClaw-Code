---
name: grafana-approval-router
description: Use when tasks require grafana approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Grafana Approval Router
...
| Primary artifact | `grafana-approval-decision-packet` |
| Routing tag | `grafana:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Approval Router into a deterministic, auth-checked workflow for routes grafana-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| grafana-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| grafana-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Grafana Approval Router, then capture the manual fallback path for Grafana.
...
3. Implement routes grafana-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `grafana-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] grafana-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:approval-router` consumers with approval and credential context attached.
