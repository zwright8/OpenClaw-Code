---
name: pagerduty-data-exporter
description: Use when tasks require pagerduty data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Data Exporter
...
| Primary artifact | `pagerduty-export-package` |
| Routing tag | `pagerduty:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Data Exporter into a deterministic, auth-checked workflow for exports pagerduty entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-export-package | structured-artifact | yes | next workflow or operator |
| pagerduty-export-package-scorecard | scorecard | yes | reviewer |
| pagerduty-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for PagerDuty Data Exporter, then constrain the export scope for PagerDuty before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports pagerduty entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `pagerduty-export-package` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:data-exporter` consumers with approval and credential context attached.
