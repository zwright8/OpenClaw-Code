---
name: prometheus-migration-assistant
description: Use when tasks require prometheus migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Migration Assistant
...
| Primary artifact | `prometheus-migration-cutover-plan` |
| Routing tag | `prometheus:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when state changes are hard to undo once they begin. This specific skill turns Prometheus Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from prometheus..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| prometheus-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| prometheus-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Prometheus Migration Assistant before any Prometheus rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from prometheus. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `prometheus-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:migration-assistant` consumers with approval and credential context attached.
