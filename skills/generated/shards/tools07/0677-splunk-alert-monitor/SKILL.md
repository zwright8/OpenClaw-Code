---
name: splunk-alert-monitor
description: Use when tasks require splunk alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Alert Monitor
...
| Primary artifact | `splunk-alert-state-bundle` |
| Routing tag | `splunk:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Alert Monitor into a deterministic, auth-checked workflow for monitors key splunk signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| splunk-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| splunk-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Splunk Alert Monitor, then lock the reviewed threshold set for Splunk.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key splunk signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `splunk-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] splunk-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:alert-monitor` consumers with approval and credential context attached.
