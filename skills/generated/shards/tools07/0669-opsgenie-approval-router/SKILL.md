---
name: opsgenie-approval-router
description: Use when tasks require opsgenie approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Approval Router
...
| Primary artifact | `opsgenie-approval-decision-packet` |
| Routing tag | `opsgenie:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Approval Router into a deterministic, auth-checked workflow for routes opsgenie-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| opsgenie-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| opsgenie-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Opsgenie Approval Router, then capture the manual fallback path for Opsgenie.
...
3. Implement routes opsgenie-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `opsgenie-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:approval-router` consumers with approval and credential context attached.
