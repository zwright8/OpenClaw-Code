...
description: Use when tasks require datadog webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `datadog-event-intake-log` |
| Routing tag | `datadog:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Datadog Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes datadog webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `DATADOG_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `DATADOG_TOKEN`, `DATADOG_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `DATADOG_WEBHOOK_SECRET`, `DATADOG_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| datadog-event-intake-log | structured-artifact | yes | next workflow or operator |
| datadog-event-intake-log-scorecard | scorecard | yes | reviewer |
| datadog-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Datadog Webhook Listener, then register the signing-secret policy for Datadog.
...
3. Implement captures, verifies, and routes datadog webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `datadog-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] datadog-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:webhook-listener` consumers with approval and credential context attached.
