---
name: sumo-logic-approval-router
description: Use when tasks require sumo logic approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Approval Router
...
| Primary artifact | `sumo-logic-approval-decision-packet` |
| Routing tag | `sumo-logic:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Approval Router into a deterministic, auth-checked workflow for routes sumo logic-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| sumo-logic-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| sumo-logic-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Sumo Logic Approval Router, then capture the manual fallback path for Sumo Logic.
...
3. Implement routes sumo logic-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `sumo-logic-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:approval-router` consumers with approval and credential context attached.
