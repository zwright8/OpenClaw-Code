---
name: new-relic-report-builder
description: Use when tasks require new relic report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Report Builder
...
| Primary artifact | `new-relic-report-bundle` |
| Routing tag | `new-relic:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from new relic activity..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-report-bundle | structured-artifact | yes | next workflow or operator |
| new-relic-report-bundle-scorecard | scorecard | yes | reviewer |
| new-relic-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for New Relic Report Builder, then confirm which New Relic entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from new relic activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `new-relic-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:report-builder` consumers with approval and credential context attached.
