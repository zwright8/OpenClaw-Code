...
description: Use when tasks require opensearch migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `opensearch-migration-cutover-plan` |
| Routing tag | `opensearch:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when state changes are hard to undo once they begin. This specific skill turns OpenSearch Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from opensearch..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| opensearch-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| opensearch-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for OpenSearch Migration Assistant before any OpenSearch rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from opensearch. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `opensearch-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:migration-assistant` consumers with approval and credential context attached.
