...
description: Use when tasks require opensearch connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `opensearch-connection-trace-bundle` |
| Routing tag | `opensearch:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Connector into a deterministic, auth-checked workflow for unified read/write connector for opensearch apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| opensearch-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| opensearch-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for OpenSearch Connector, then confirm the smallest read-only probe that proves access to OpenSearch.
...
3. Specify the request and response contract for unified read/write connector for opensearch apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `opensearch-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:connector` consumers with approval and credential context attached.
