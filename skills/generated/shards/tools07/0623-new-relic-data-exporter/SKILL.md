---
name: new-relic-data-exporter
description: Use when tasks require new relic data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Data Exporter
...
| Primary artifact | `new-relic-export-package` |
| Routing tag | `new-relic:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Data Exporter into a deterministic, auth-checked workflow for exports new relic entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-export-package | structured-artifact | yes | next workflow or operator |
| new-relic-export-package-scorecard | scorecard | yes | reviewer |
| new-relic-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for New Relic Data Exporter, then constrain the export scope for New Relic before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports new relic entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `new-relic-export-package` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:data-exporter` consumers with approval and credential context attached.
