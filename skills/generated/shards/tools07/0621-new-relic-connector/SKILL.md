...
description: Use when tasks require new relic connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `new-relic-connection-trace-bundle` |
| Routing tag | `new-relic:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Connector into a deterministic, auth-checked workflow for unified read/write connector for new relic apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| new-relic-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| new-relic-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for New Relic Connector, then confirm the smallest read-only probe that proves access to New Relic.
...
3. Specify the request and response contract for unified read/write connector for new relic apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `new-relic-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:connector` consumers with approval and credential context attached.
