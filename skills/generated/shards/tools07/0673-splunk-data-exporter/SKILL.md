---
name: splunk-data-exporter
description: Use when tasks require splunk data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `splunk-export-package` |
| Routing tag | `splunk:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Data Exporter into a deterministic, auth-checked workflow for exports splunk entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-export-package | structured-artifact | yes | next workflow or operator |
| splunk-export-package-scorecard | scorecard | yes | reviewer |
| splunk-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Splunk Data Exporter, then constrain the export scope for Splunk before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports splunk entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `splunk-export-package` so downstream owners do not need to rediscover context.
...
- [ ] splunk-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:data-exporter` consumers with approval and credential context attached.
