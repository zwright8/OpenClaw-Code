...
description: Use when tasks require sentry connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sentry-connection-trace-bundle` |
| Routing tag | `sentry:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Connector into a deterministic, auth-checked workflow for unified read/write connector for sentry apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| sentry-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| sentry-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Sentry Connector, then confirm the smallest read-only probe that proves access to Sentry.
...
3. Specify the request and response contract for unified read/write connector for sentry apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `sentry-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sentry-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:connector` consumers with approval and credential context attached.
