---
name: sumo-logic-webhook-listener
description: Use when tasks require sumo logic webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Webhook Listener
...
| Primary artifact | `sumo-logic-event-intake-log` |
| Routing tag | `sumo-logic:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Sumo Logic Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes sumo logic webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `SUMO_LOGIC_CLIENT_ID`, `SUMO_LOGIC_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `SUMO_LOGIC_TOKEN`, `SUMO_LOGIC_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SUMO_LOGIC_WEBHOOK_SECRET`, `SUMO_LOGIC_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| sumo-logic-event-intake-log | structured-artifact | yes | next workflow or operator |
| sumo-logic-event-intake-log-scorecard | scorecard | yes | reviewer |
| sumo-logic-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Sumo Logic Webhook Listener, then register the signing-secret policy for Sumo Logic.
...
3. Implement captures, verifies, and routes sumo logic webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `sumo-logic-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:webhook-listener` consumers with approval and credential context attached.
