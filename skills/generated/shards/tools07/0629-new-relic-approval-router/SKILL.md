---
name: new-relic-approval-router
description: Use when tasks require new relic approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Approval Router
...
| Primary artifact | `new-relic-approval-decision-packet` |
| Routing tag | `new-relic:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Approval Router into a deterministic, auth-checked workflow for routes new relic-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| new-relic-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| new-relic-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for New Relic Approval Router, then capture the manual fallback path for New Relic.
...
3. Implement routes new relic-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `new-relic-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:approval-router` consumers with approval and credential context attached.
