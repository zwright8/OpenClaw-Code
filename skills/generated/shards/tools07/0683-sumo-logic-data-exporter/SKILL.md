---
name: sumo-logic-data-exporter
description: Use when tasks require sumo logic data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Data Exporter
...
| Primary artifact | `sumo-logic-export-package` |
| Routing tag | `sumo-logic:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Data Exporter into a deterministic, auth-checked workflow for exports sumo logic entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-export-package | structured-artifact | yes | next workflow or operator |
| sumo-logic-export-package-scorecard | scorecard | yes | reviewer |
| sumo-logic-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for Sumo Logic Data Exporter, then constrain the export scope for Sumo Logic before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports sumo logic entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `sumo-logic-export-package` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:data-exporter` consumers with approval and credential context attached.
