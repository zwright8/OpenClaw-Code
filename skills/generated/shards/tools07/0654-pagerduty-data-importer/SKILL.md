---
name: pagerduty-data-importer
description: Use when tasks require pagerduty data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Data Importer
...
| Primary artifact | `pagerduty-import-ledger` |
| Routing tag | `pagerduty:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when state changes are hard to undo once they begin. This specific skill turns PagerDuty Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into pagerduty..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-import-ledger | structured-artifact | yes | next workflow or operator |
| pagerduty-import-ledger-scorecard | scorecard | yes | reviewer |
| pagerduty-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for PagerDuty Data Importer before any PagerDuty write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into pagerduty. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `pagerduty-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:data-importer` consumers with approval and credential context attached.
