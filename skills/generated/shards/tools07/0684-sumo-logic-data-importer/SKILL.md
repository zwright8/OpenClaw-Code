---
name: sumo-logic-data-importer
description: Use when tasks require sumo logic data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Data Importer
...
| Primary artifact | `sumo-logic-import-ledger` |
| Routing tag | `sumo-logic:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sumo Logic Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into sumo logic..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-import-ledger | structured-artifact | yes | next workflow or operator |
| sumo-logic-import-ledger-scorecard | scorecard | yes | reviewer |
| sumo-logic-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Sumo Logic Data Importer before any Sumo Logic write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into sumo logic. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `sumo-logic-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:data-importer` consumers with approval and credential context attached.
