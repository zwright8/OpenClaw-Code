---
name: sumo-logic-alert-monitor
description: Use when tasks require sumo logic alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sumo Logic Alert Monitor
...
| Primary artifact | `sumo-logic-alert-state-bundle` |
| Routing tag | `sumo-logic:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Alert Monitor into a deterministic, auth-checked workflow for monitors key sumo logic signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| sumo-logic-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| sumo-logic-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Sumo Logic Alert Monitor, then lock the reviewed threshold set for Sumo Logic.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key sumo logic signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `sumo-logic-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:alert-monitor` consumers with approval and credential context attached.
