---
name: splunk-workflow-automator
description: Use when tasks require splunk workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Workflow Automator
...
| Primary artifact | `splunk-workflow-runbook` |
| Routing tag | `splunk:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Splunk Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using splunk..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-workflow-runbook | structured-artifact | yes | next workflow or operator |
| splunk-workflow-runbook-scorecard | scorecard | yes | reviewer |
| splunk-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Splunk Workflow Automator, then checkpoint where Splunk state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using splunk. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `splunk-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] splunk-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:workflow-automator` consumers with approval and credential context attached.
