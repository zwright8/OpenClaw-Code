---
name: splunk-data-importer
description: Use when tasks require splunk data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Splunk Data Importer
...
| Primary artifact | `splunk-import-ledger` |
| Routing tag | `splunk:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Splunk workflows degrade when state changes are hard to undo once they begin. This specific skill turns Splunk Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into splunk..

## Trigger Checklist
- [ ] The task explicitly requires `Splunk Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| splunk-import-ledger | structured-artifact | yes | next workflow or operator |
| splunk-import-ledger-scorecard | scorecard | yes | reviewer |
| splunk-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Splunk Data Importer before any Splunk write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into splunk. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `splunk-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] splunk-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `splunk-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `splunk:data-importer` consumers with approval and credential context attached.
