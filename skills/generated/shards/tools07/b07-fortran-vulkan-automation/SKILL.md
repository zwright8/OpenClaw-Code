---
name: b07-fortran-vulkan-automation
description: Use when working with Fortran packaging, Vulkan helper libraries, code-wide transformations, FPGA flashing, document/image conversions, SQL shells, reverse proxies, and diff automation.
---

# Fortran/Vulkan toolchains and automation utilities

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-fortran-vulkan-automation` |
| Tool count | `24` |
| Inventory rank span | `3470-3500` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for fortran/vulkan toolchains, automation utilities tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `libtirpc` | `fpm` | no | no |
| bounded mutation / transformation | `fpm` | `libtirpc` | yes | yes |
| validation & artifact checks | `libtirpc` | `fpm` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `libtirpc` | Port of Sun's Transport-Independent RPC library to Linux | local-installable |
| `fpm` | Package manager and build system for Fortran | local-installable |
| `spotbugs` | Tool for Java static analysis (FindBugs's successor) | local-installable |
| `vulkan-volk` | Meta loader for Vulkan API | local-installable |
| `unrtf` | RTF to other formats converter | local-installable |
| `enca` | Charset analyzer and converter | local-installable |
| `anycable-go` | WebSocket server with action cable protocol | local-installable |
| `comby` | Tool for changing code across many languages | local-installable |
| `osc` | Command-line interface to work with an Open Build Service | local-installable |
| `ralph-orchestrator` | Multi-agent orchestration framework for autonomous AI task completion | local-installable |
| `ispc` | Compiler for SIMD programming on the CPU | local-installable |
| `vulkan-utility-libraries` | Utility Libraries for Vulkan | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `libtirpc` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `fpm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spotbugs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vulkan-volk` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `unrtf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `enca` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `anycable-go` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `comby` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ralph-orchestrator` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ispc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `vulkan-utility-libraries` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `openfpgaloader` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xauth` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `erdtree` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jbig2enc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnu-typist` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xvid` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `slides` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `licensefinder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `turso` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `inotify-tools` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `frps` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tkdiff` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `libtirpc` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write/orchestrate command sequence, capture outputs and exit code in handoff packet.
2. `fpm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `spotbugs` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `vulkan-volk` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `unrtf` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `enca` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a fortran/vulkan toolchains, automation utilities request, run dry-run first, then produce a validated artifact bundle.
