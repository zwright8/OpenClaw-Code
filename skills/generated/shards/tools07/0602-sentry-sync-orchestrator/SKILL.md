---
name: sentry-sync-orchestrator
description: Use when tasks require sentry sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Sync Orchestrator
...
| Primary artifact | `sentry-reconciliation-bundle` |
| Routing tag | `sentry:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for sentry records..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| sentry-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| sentry-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for Sentry Sync Orchestrator, then checkpoint the last known good cursor on both sides of Sentry.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for sentry records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `sentry-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sentry-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:sync-orchestrator` consumers with approval and credential context attached.
