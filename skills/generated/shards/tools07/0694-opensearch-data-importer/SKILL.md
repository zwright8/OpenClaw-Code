---
name: opensearch-data-importer
description: Use when tasks require opensearch data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Data Importer
...
| Primary artifact | `opensearch-import-ledger` |
| Routing tag | `opensearch:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when state changes are hard to undo once they begin. This specific skill turns OpenSearch Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into opensearch..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-import-ledger | structured-artifact | yes | next workflow or operator |
| opensearch-import-ledger-scorecard | scorecard | yes | reviewer |
| opensearch-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for OpenSearch Data Importer before any OpenSearch write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into opensearch. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `opensearch-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:data-importer` consumers with approval and credential context attached.
