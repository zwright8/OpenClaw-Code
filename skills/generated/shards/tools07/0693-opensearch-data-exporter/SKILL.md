...
description: Use when tasks require opensearch data exporter with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Data Exporter
...
| Primary artifact | `opensearch-export-package` |
| Routing tag | `opensearch:data-exporter` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Data Exporter into a deterministic, auth-checked workflow for exports opensearch entities into clean, typed datasets..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Data Exporter` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-export-package | structured-artifact | yes | next workflow or operator |
| opensearch-export-package-scorecard | scorecard | yes | reviewer |
| opensearch-export-package-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the approved entities, fields, and destination contract for OpenSearch Data Exporter, then constrain the export scope for OpenSearch before running queries.
2. Validate credential reuse, pagination strategy, and the exact schema version that downstream consumers expect.
3. Implement exports opensearch entities into clean, typed datasets. with deterministic extraction, normalization, and integrity metadata for every page or batch.
...
- Preserve request, response, and approval traces in `opensearch-export-package` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-export-package, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-export-package`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:data-exporter` consumers with approval and credential context attached.
