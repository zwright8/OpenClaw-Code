---
name: prometheus-approval-router
description: Use when tasks require prometheus approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Prometheus Approval Router
...
| Primary artifact | `prometheus-approval-decision-packet` |
| Routing tag | `prometheus:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Prometheus workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Prometheus Approval Router into a deterministic, auth-checked workflow for routes prometheus-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `Prometheus Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| prometheus-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| prometheus-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| prometheus-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for Prometheus Approval Router, then capture the manual fallback path for Prometheus.
...
3. Implement routes prometheus-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `prometheus-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] prometheus-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `prometheus-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `prometheus:approval-router` consumers with approval and credential context attached.
