---
name: b07-net-media-build-cli
description: Use when operating DNS/network telemetry, PDF and binary delta tooling, SDR/media server chains, compiler profiling, and AWS/template-driven engineering workflows.
---

# Network services, media processing, and build CLI workflows

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-net-media-build-cli` |
| Tool count | `26` |
| Inventory rank span | `3123-3155` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for network services, media processing, build cli workflows tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `cppman` | `clippy` | no | no |
| bounded mutation / transformation | `clippy` | `cppman` | yes | yes |
| validation & artifact checks | `cppman` | `clippy` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `cppman` | C++ 98/11/14/17/20 manual pages from cplusplus.com and cppreference.com | local-installable |
| `clippy` | Copy files from your terminal that actually paste into GUI apps | local-installable |
| `minidlna` | Media server software, compliant with DLNA/UPnP-AV clients | local-installable |
| `fastnetmon` | DDoS detection tool with sFlow, Netflow, IPFIX and port mirror support | local-installable |
| `soapysdr` | Vendor and platform neutral SDR support library | local-installable |
| `cruft` | Utility that creates projects from templates and maintains the cruft afterwards | local-installable |
| `dnsx` | DNS query and resolution tool | local-installable |
| `ghorg` | Quickly clone an entire org's or user's repositories into one directory | local-installable |
| `osm2pgrouting` | Import OSM data into pgRouting database | local-installable |
| `cpdf` | PDF Command-line Tools | local-installable |
| `dwarfutils` | Dump and produce DWARF debug information in ELF objects | local-installable |
| `clinfo` | Print information about OpenCL platforms and devices | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `cppman` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clippy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `minidlna` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fastnetmon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `soapysdr` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `cruft` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dnsx` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ghorg` | QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `osm2pgrouting` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cpdf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dwarfutils` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clinfo` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ingress2gateway` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `bagels` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `xdelta` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `aws-c-s3` | Local library API/ABI, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `circumflex` | JSON-RPC/WebSocket, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `undercutf1` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `moto` | JSON-RPC/WebSocket, CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `evil-helix` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ola` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `trzsz` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gcc@9` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `lc0` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clang-build-analyzer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libimobiledevice-glue` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |

## Credential Prompting Rule
- `aws-c-s3`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `ingress2gateway`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
- `moto`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `cppman` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `clippy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `minidlna` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `fastnetmon` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `soapysdr` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `cruft` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a network services, media processing, build cli workflows request, run dry-run first, then produce a validated artifact bundle.
