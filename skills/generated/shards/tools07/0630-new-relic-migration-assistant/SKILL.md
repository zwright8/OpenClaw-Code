---
name: new-relic-migration-assistant
description: Use when tasks require new relic migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `new-relic-migration-cutover-plan` |
| Routing tag | `new-relic:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when state changes are hard to undo once they begin. This specific skill turns New Relic Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from new relic..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| new-relic-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| new-relic-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for New Relic Migration Assistant before any New Relic rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from new relic. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `new-relic-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:migration-assistant` consumers with approval and credential context attached.
