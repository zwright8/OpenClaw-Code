---
name: new-relic-workflow-automator
description: Use when tasks require new relic workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Workflow Automator
...
| Primary artifact | `new-relic-workflow-runbook` |
| Routing tag | `new-relic:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using new relic..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-workflow-runbook | structured-artifact | yes | next workflow or operator |
| new-relic-workflow-runbook-scorecard | scorecard | yes | reviewer |
| new-relic-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for New Relic Workflow Automator, then checkpoint where New Relic state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using new relic. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `new-relic-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:workflow-automator` consumers with approval and credential context attached.
