---
name: new-relic-data-importer
description: Use when tasks require new relic data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Data Importer
...
| Primary artifact | `new-relic-import-ledger` |
| Routing tag | `new-relic:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when state changes are hard to undo once they begin. This specific skill turns New Relic Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into new relic..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-import-ledger | structured-artifact | yes | next workflow or operator |
| new-relic-import-ledger-scorecard | scorecard | yes | reviewer |
| new-relic-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for New Relic Data Importer before any New Relic write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into new relic. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `new-relic-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:data-importer` consumers with approval and credential context attached.
