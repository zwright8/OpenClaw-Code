---
name: pagerduty-webhook-listener
description: Use when tasks require pagerduty webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Webhook Listener
...
| Primary artifact | `pagerduty-event-intake-log` |
| Routing tag | `pagerduty:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns PagerDuty Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes pagerduty webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `PAGERDUTY_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `PAGERDUTY_TOKEN`, `PAGERDUTY_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `PAGERDUTY_WEBHOOK_SECRET`, `PAGERDUTY_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| pagerduty-event-intake-log | structured-artifact | yes | next workflow or operator |
| pagerduty-event-intake-log-scorecard | scorecard | yes | reviewer |
| pagerduty-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for PagerDuty Webhook Listener, then register the signing-secret policy for PagerDuty.
...
3. Implement captures, verifies, and routes pagerduty webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `pagerduty-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:webhook-listener` consumers with approval and credential context attached.
