---
name: opensearch-workflow-automator
description: Use when tasks require opensearch workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Workflow Automator
...
| Primary artifact | `opensearch-workflow-runbook` |
| Routing tag | `opensearch:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using opensearch..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-workflow-runbook | structured-artifact | yes | next workflow or operator |
| opensearch-workflow-runbook-scorecard | scorecard | yes | reviewer |
| opensearch-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for OpenSearch Workflow Automator, then checkpoint where OpenSearch state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using opensearch. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `opensearch-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:workflow-automator` consumers with approval and credential context attached.
