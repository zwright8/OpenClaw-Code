...
| Field | Value |
|---|---|
| Skill pack | `b07-code-workflow-storage` |
| Tool count | `26` |
| Inventory rank span | `3260-3294` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for code analysis, workflow engines, storage/connectivity tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `jython` | `joern` | no | no |
| bounded mutation / transformation | `joern` | `jython` | yes | yes |
| validation & artifact checks | `jython` | `joern` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `jython` | Python implementation written in Java (successor to JPython) | local-installable |
| `joern` | Open-source code analysis platform based on code property graphs | local-installable |
| `utf8cpp` | UTF-8 with C++ in a Portable Way | local-installable |
| `go-jira` | Simple jira command-line client in Go | local-installable |
| `simple-tiles` | Image generation library for spatial data | local-installable |
| `dagu` | Lightweight and powerful workflow engine | local-installable |
| `agda` | Dependently typed functional programming language | local-installable |
| `qca` | Qt Cryptographic Architecture (QCA) | local-installable |
| `libnice-gstreamer` | GStreamer Plugin for libnice | local-installable |
| `dolphie` | Feature-rich top tool for monitoring MySQL | local-installable |
| `fzf-make` | Fuzzy finder with preview window for various command runners including make | local-installable |
| `imap-backup` | Backup GMail (or other IMAP) accounts to disk | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `jython` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `joern` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `utf8cpp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `go-jira` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `simple-tiles` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `dagu` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `agda` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qca` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libnice-gstreamer` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dolphie` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fzf-make` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `imap-backup` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `qbs` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `chkrootkit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `payload-dumper-go` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `regal` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `dosfstools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `spice-protocol` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `otel-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ansible@10` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `px` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `adios2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `silicon` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `typos-lsp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `djvu2pdf` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fox` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
...
1. `jython` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `joern` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `utf8cpp` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `go-jira` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `simple-tiles` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `dagu` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a code analysis, workflow engines, storage/connectivity request, run dry-run first, then produce a validated artifact bundle.
