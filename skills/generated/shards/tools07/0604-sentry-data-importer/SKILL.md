---
name: sentry-data-importer
description: Use when tasks require sentry data importer with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Data Importer
...
| Primary artifact | `sentry-import-ledger` |
| Routing tag | `sentry:data-importer` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when state changes are hard to undo once they begin. This specific skill turns Sentry Data Importer into a deterministic, auth-checked workflow for validates and imports bulk updates into sentry..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Data Importer` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-import-ledger | structured-artifact | yes | next workflow or operator |
| sentry-import-ledger-scorecard | scorecard | yes | reviewer |
| sentry-import-ledger-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the import blast radius, rollback ledger, and required dry-run evidence for Sentry Data Importer before any Sentry write is approved.
2. Validate credential reuse, schema mappings, and the exact dry-run diff that the operator approved for the batch.
3. Implement validates and imports bulk updates into sentry. in staged batches that preserve before/after state and stop immediately on contract drift.
...
- Preserve request, response, and approval traces in `sentry-import-ledger` so downstream owners do not need to rediscover context.
...
- [ ] sentry-import-ledger, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-import-ledger`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:data-importer` consumers with approval and credential context attached.
