---
name: sumo-logic-connector
description: Use when tasks require sumo logic connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sumo-logic-connection-trace-bundle` |
| Routing tag | `sumo-logic:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Connector into a deterministic, auth-checked workflow for unified read/write connector for sumo logic apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| sumo-logic-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| sumo-logic-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Sumo Logic Connector, then confirm the smallest read-only probe that proves access to Sumo Logic.
...
3. Specify the request and response contract for unified read/write connector for sumo logic apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `sumo-logic-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:connector` consumers with approval and credential context attached.
