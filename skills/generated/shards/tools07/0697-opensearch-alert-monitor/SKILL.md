---
name: opensearch-alert-monitor
description: Use when tasks require opensearch alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `opensearch-alert-state-bundle` |
| Routing tag | `opensearch:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Alert Monitor into a deterministic, auth-checked workflow for monitors key opensearch signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| opensearch-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| opensearch-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for OpenSearch Alert Monitor, then lock the reviewed threshold set for OpenSearch.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key opensearch signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `opensearch-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:alert-monitor` consumers with approval and credential context attached.
