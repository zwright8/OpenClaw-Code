---
name: b07-ai-container-geo-cli
description: Use when coordinating SSH/container TUIs, AI chat CLI tools, geospatial raster/image transforms, security hardening, vector DB operations, and terminal productivity helpers.
---

# AI terminal assistants, container control, and geo/data CLIs

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-ai-container-geo-cli` |
| Tool count | `26` |
| Inventory rank span | `3225-3259` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for ai terminal assistants, container control, geo/data clis tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `trzsz-ssh` | `tgpt` | no | no |
| bounded mutation / transformation | `tgpt` | `trzsz-ssh` | yes | yes |
| validation & artifact checks | `trzsz-ssh` | `tgpt` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `trzsz-ssh` | Alternative ssh client with additional features to meet your needs | local-installable |
| `tgpt` | AI Chatbots in terminal without needing API keys | local-installable |
| `oxker` | Terminal User Interface (TUI) to view & control docker containers | local-installable |
| `rasterio` | Reads and writes geospatial raster datasets | local-installable |
| `apngasm` | Next generation of apngasm, the APNG assembler | local-installable |
| `h2` | Java SQL database | local-installable |
| `rbenv-gemset` | KISS yet powerful gem / gemset management for rbenv | local-installable |
| `shtool` | GNU's portable shell tool | local-installable |
| `rrdtool` | Round Robin Database | local-installable |
| `hercules` | System/370, ESA/390 and z/Architecture Emulator | local-installable |
| `libxfixes` | X.Org: Header files for the XFIXES extension | local-installable |
| `clarinet` | Command-line tool and runtime for the Clarity smart contract language | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `trzsz-ssh` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tgpt` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `oxker` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `rasterio` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `apngasm` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `h2` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rbenv-gemset` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `shtool` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rrdtool` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `hercules` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `libxfixes` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `clarinet` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `i2pd` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `imgproxy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `fail2ban` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `context7-mcp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bkmr` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `weaviate` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `cri-tools` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `liquidctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `metis` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ipv6calc` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `jqp` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `catimg` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `beancount` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `quicktype` | GraphQL over HTTPS, QUIC, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `oxker`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `trzsz-ssh` (SSH, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `tgpt` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `oxker` (CLI/stdin-stdout) -> auth preflight (Account/session credentials), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `rasterio` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `apngasm` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `h2` (SQL/DB protocol, CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a ai terminal assistants, container control, geo/data clis request, run dry-run first, then produce a validated artifact bundle.
