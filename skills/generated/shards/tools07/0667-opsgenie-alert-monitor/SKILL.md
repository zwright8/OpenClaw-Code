...
description: Use when tasks require opsgenie alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Alert Monitor
...
| Primary artifact | `opsgenie-alert-state-bundle` |
| Routing tag | `opsgenie:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Alert Monitor into a deterministic, auth-checked workflow for monitors key opsgenie signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| opsgenie-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| opsgenie-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Opsgenie Alert Monitor, then lock the reviewed threshold set for Opsgenie.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key opsgenie signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `opsgenie-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:alert-monitor` consumers with approval and credential context attached.
