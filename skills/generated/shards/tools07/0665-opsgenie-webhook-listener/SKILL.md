---
name: opsgenie-webhook-listener
description: Use when tasks require opsgenie webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Webhook Listener
...
| Primary artifact | `opsgenie-event-intake-log` |
| Routing tag | `opsgenie:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Opsgenie Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes opsgenie webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| OAuth client or delegated session | `oauth2` | `OPSGENIE_CLIENT_ID`, `OPSGENIE_CLIENT_SECRET` | Reuse an active delegated session or validate the client credentials with a lightweight identity call. |
| Access token or personal access token | `token` | `OPSGENIE_TOKEN`, `OPSGENIE_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `OPSGENIE_WEBHOOK_SECRET`, `OPSGENIE_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| opsgenie-event-intake-log | structured-artifact | yes | next workflow or operator |
| opsgenie-event-intake-log-scorecard | scorecard | yes | reviewer |
| opsgenie-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Opsgenie Webhook Listener, then register the signing-secret policy for Opsgenie.
...
3. Implement captures, verifies, and routes opsgenie webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `opsgenie-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:webhook-listener` consumers with approval and credential context attached.
