---
name: opsgenie-workflow-automator
description: Use when tasks require opsgenie workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Workflow Automator
...
| Primary artifact | `opsgenie-workflow-runbook` |
| Routing tag | `opsgenie:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Opsgenie Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using opsgenie..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-workflow-runbook | structured-artifact | yes | next workflow or operator |
| opsgenie-workflow-runbook-scorecard | scorecard | yes | reviewer |
| opsgenie-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Opsgenie Workflow Automator, then checkpoint where Opsgenie state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using opsgenie. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `opsgenie-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:workflow-automator` consumers with approval and credential context attached.
