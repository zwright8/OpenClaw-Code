---
name: new-relic-sync-orchestrator
description: Use when tasks require new relic sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Sync Orchestrator
...
| Primary artifact | `new-relic-reconciliation-bundle` |
| Routing tag | `new-relic:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns New Relic Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for new relic records..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| new-relic-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| new-relic-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| new-relic-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for New Relic Sync Orchestrator, then checkpoint the last known good cursor on both sides of New Relic.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for new relic records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `new-relic-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:sync-orchestrator` consumers with approval and credential context attached.
