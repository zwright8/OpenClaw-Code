...
description: Use when tasks require sumo logic workflow automator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `sumo-logic-workflow-runbook` |
| Routing tag | `sumo-logic:workflow-automator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Sumo Logic workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sumo Logic Workflow Automator into a deterministic, auth-checked workflow for executes repeatable multi-step workflows using sumo logic..

## Trigger Checklist
- [ ] The task explicitly requires `Sumo Logic Workflow Automator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sumo-logic-workflow-runbook | structured-artifact | yes | next workflow or operator |
| sumo-logic-workflow-runbook-scorecard | scorecard | yes | reviewer |
| sumo-logic-workflow-runbook-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the workflow graph, mutating boundaries, and approval requirements for Sumo Logic Workflow Automator, then checkpoint where Sumo Logic state can be replayed safely.
...
3. Implement executes repeatable multi-step workflows using sumo logic. as a deterministic sequence of step contracts, approvals, and checkpoint writes.
...
- Preserve request, response, and approval traces in `sumo-logic-workflow-runbook` so downstream owners do not need to rediscover context.
...
- [ ] sumo-logic-workflow-runbook, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sumo-logic-workflow-runbook`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sumo-logic:workflow-automator` consumers with approval and credential context attached.
