...
description: Use when tasks require sentry webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Webhook Listener
...
| Primary artifact | `sentry-event-intake-log` |
| Routing tag | `sentry:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns Sentry Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes sentry webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `SENTRY_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `SENTRY_TOKEN`, `SENTRY_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `SENTRY_WEBHOOK_SECRET`, `SENTRY_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| sentry-event-intake-log | structured-artifact | yes | next workflow or operator |
| sentry-event-intake-log-scorecard | scorecard | yes | reviewer |
| sentry-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for Sentry Webhook Listener, then register the signing-secret policy for Sentry.
...
3. Implement captures, verifies, and routes sentry webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `sentry-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] sentry-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:webhook-listener` consumers with approval and credential context attached.
