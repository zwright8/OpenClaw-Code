---
name: opensearch-approval-router
description: Use when tasks require opensearch approval router with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# OpenSearch Approval Router
...
| Primary artifact | `opensearch-approval-decision-packet` |
| Routing tag | `opensearch:approval-router` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because OpenSearch workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns OpenSearch Approval Router into a deterministic, auth-checked workflow for routes opensearch-driven decisions to the right approvers..

## Trigger Checklist
- [ ] The task explicitly requires `OpenSearch Approval Router` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opensearch-approval-decision-packet | structured-artifact | yes | next workflow or operator |
| opensearch-approval-decision-packet-scorecard | scorecard | yes | reviewer |
| opensearch-approval-decision-packet-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the decision classes, approver roster, and escalation hierarchy for OpenSearch Approval Router, then capture the manual fallback path for OpenSearch.
...
3. Implement routes opensearch-driven decisions to the right approvers. so the routing decision always includes policy clauses, approver evidence, and escalation context.
...
- Preserve request, response, and approval traces in `opensearch-approval-decision-packet` so downstream owners do not need to rediscover context.
...
- [ ] opensearch-approval-decision-packet, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opensearch-approval-decision-packet`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opensearch:approval-router` consumers with approval and credential context attached.
