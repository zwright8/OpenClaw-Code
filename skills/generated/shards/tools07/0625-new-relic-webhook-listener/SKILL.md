---
name: new-relic-webhook-listener
description: Use when tasks require new relic webhook listener with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# New Relic Webhook Listener
...
| Domain | `Observability and incident platforms` |
| Runtime archetype | `signed-event-ingestion-engine` |
| Core method | `signed event intake and routing` |
| Primary artifact | `new-relic-event-intake-log` |
| Routing tag | `new-relic:webhook-listener` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because New Relic workflows degrade when unverified events can trigger unsafe downstream actions. This specific skill turns New Relic Webhook Listener into a deterministic, auth-checked workflow for captures, verifies, and routes new relic webhook events..

## Trigger Checklist
- [ ] The task explicitly requires `New Relic Webhook Listener` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| API key or account secret | `api_key` | `NEW_RELIC_API_KEY` | Validate the key with a lightweight authenticated request and confirm environment or mode (sandbox vs production). |
| Access token or personal access token | `token` | `NEW_RELIC_TOKEN`, `NEW_RELIC_ACCESS_TOKEN` | Validate the token with the smallest read-only endpoint that proves scope and tenancy. |
| Webhook signing secret | `signing_secret` | `NEW_RELIC_WEBHOOK_SECRET`, `NEW_RELIC_SIGNING_SECRET` | Verify signature generation against a known sample payload before accepting live webhook traffic. |
...
| --- | --- | --- | --- |
| new-relic-event-intake-log | structured-artifact | yes | next workflow or operator |
| new-relic-event-intake-log-scorecard | scorecard | yes | reviewer |
| new-relic-event-intake-log-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the accepted event types, replay window, and downstream side effects for New Relic Webhook Listener, then register the signing-secret policy for New Relic.
...
3. Implement captures, verifies, and routes new relic webhook events. so that every payload is verified, normalized, and either routed or quarantined deterministically.
...
- Preserve request, response, and approval traces in `new-relic-event-intake-log` so downstream owners do not need to rediscover context.
...
- [ ] new-relic-event-intake-log, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `new-relic-event-intake-log`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `new-relic:webhook-listener` consumers with approval and credential context attached.
