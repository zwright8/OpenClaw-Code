---
name: b07-profiling-build-dns
description: Use when running performance profilers, protocol/bioinformatics pipelines, build automation, DNS inspection, code coverage/reporting, packaging, and GUI/toolchain setup.
---

# Profiling, build pipelines, and DNS/network diagnostics

## Quick Reference
| Field | Value |
|---|---|
| Skill pack | `b07-profiling-build-dns` |
| Tool count | `26` |
| Inventory rank span | `3297-3326` |
| References | `references/tools.md`, `references/tools.csv`, `references/workflows.md` |
| Local checker | `scripts/check-tools.sh` |

## Why This Skill Exists
This pack provides a curated operational toolkit for profiling, build pipelines, dns/network diagnostics tasks so operators can select the smallest safe tool, execute with bounded risk, and hand off reproducible outputs.
...
|---|---|---|---|---|
| inventory & read-only diagnosis | `aribb24` | `nanopb` | no | no |
| bounded mutation / transformation | `nanopb` | `aribb24` | yes | yes |
| validation & artifact checks | `aribb24` | `nanopb` | conditional | no |

## Tool Inventory Highlights
| Tool | Typical Use | Access |
|---|---|---|
| `aribb24` | Library for ARIB STD-B24, decoding JIS 8 bit characters and parsing MPEG-TS | local-installable |
| `nanopb` | C library for encoding and decoding Protocol Buffer messages | local-installable |
| `git-trim` | Trim your git remote tracking branches that are merged or gone | local-installable |
| `rsgain` | ReplayGain 2.0 tagging utility | local-installable |
| `tracy` | Real-time, nanosecond resolution frame profiler | local-installable |
| `opencl-headers` | C language header files for the OpenCL API | local-installable |
| `bowtie2` | Fast and sensitive gapped read aligner | local-installable |
| `autopsy` | Graphical interface to Sleuth Kit investigation tools | local-installable |
| `west` | Zephyr meta-tool | local-installable |
| `enex2notion` | Import Evernote ENEX files to Notion | local-installable |
| `progress` | Coreutils progress viewer | local-installable |
| `ifstat` | Tool to report network interface bandwidth | local-installable |

## API Protocols & Credential Requirements
| Tool | Primary protocol(s) | Auth mode | Auth required | API key needed | Operator action |
|---|---|---|---|---|---|
| `aribb24` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `nanopb` | Local library API/ABI, CLI/stdin-stdout | None (local library/runtime) | no | no | No API key required for local library/runtime use. |
| `git-trim` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `rsgain` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `tracy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `opencl-headers` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bowtie2` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `autopsy` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `west` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `enex2notion` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `progress` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ifstat` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `earthly` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gtkmm4` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `csshx` | SSH, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `q` | DNS, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `goctl` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `codecov-cli` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `bundler-completion` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `makeself` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `networkit` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `ragel` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `compiledb` | SQL/DB protocol, CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `eget` | CLI/stdin-stdout | Account/session credentials | yes | no | Confirm account credentials/session context are configured before execution. |
| `gpa` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |
| `gnome-builder` | CLI/stdin-stdout | None or local runtime | no | no | No API key required for local/offline use. |

## Credential Prompting Rule
- `eget`: Reuse existing credentials/session first and validate access. Ask user only if required credentials are missing or invalid.
...
1. `aribb24` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
2. `nanopb` (Local library API/ABI, CLI/stdin-stdout) -> auth preflight (None (local library/runtime)), run read+write command sequence, capture outputs and exit code in handoff packet.
3. `git-trim` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
4. `rsgain` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
5. `tracy` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
6. `opencl-headers` (CLI/stdin-stdout) -> auth preflight (None or local runtime), run read+write command sequence, capture outputs and exit code in handoff packet.
...
1. Select a minimal tool from this pack to solve a profiling, build pipelines, dns/network diagnostics request, run dry-run first, then produce a validated artifact bundle.
