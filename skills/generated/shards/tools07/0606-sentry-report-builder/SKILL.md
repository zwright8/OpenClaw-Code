---
name: sentry-report-builder
description: Use when tasks require sentry report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Sentry Report Builder
...
| Primary artifact | `sentry-report-bundle` |
| Routing tag | `sentry:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Sentry workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Sentry Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from sentry activity..

## Trigger Checklist
- [ ] The task explicitly requires `Sentry Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| sentry-report-bundle | structured-artifact | yes | next workflow or operator |
| sentry-report-bundle-scorecard | scorecard | yes | reviewer |
| sentry-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Sentry Report Builder, then confirm which Sentry entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from sentry activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `sentry-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] sentry-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `sentry-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `sentry:report-builder` consumers with approval and credential context attached.
