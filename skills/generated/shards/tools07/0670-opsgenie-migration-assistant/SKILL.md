---
name: opsgenie-migration-assistant
description: Use when tasks require opsgenie migration assistant with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Opsgenie Migration Assistant
...
| Primary artifact | `opsgenie-migration-cutover-plan` |
| Routing tag | `opsgenie:migration-assistant` |
| Mutating | `yes` |
| Release cycles | `3` |

## Why This Skill Exists
We need this skill because Opsgenie workflows degrade when state changes are hard to undo once they begin. This specific skill turns Opsgenie Migration Assistant into a deterministic, auth-checked workflow for plans and executes safe migrations to or from opsgenie..

## Trigger Checklist
- [ ] The task explicitly requires `Opsgenie Migration Assistant` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| opsgenie-migration-cutover-plan | structured-artifact | yes | next workflow or operator |
| opsgenie-migration-cutover-plan-scorecard | scorecard | yes | reviewer |
| opsgenie-migration-cutover-plan-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the source inventory, target mapping, cutover window, and rollback checkpoint for Opsgenie Migration Assistant before any Opsgenie rehearsal is approved.
2. Validate credential reuse, export baselines, and the human approval chain for every irreversible migration step.
3. Implement plans and executes safe migrations to or from opsgenie. as staged rehearsal, checkpoint, and cutover workflows that preserve a deterministic revert path.
...
- Preserve request, response, and approval traces in `opsgenie-migration-cutover-plan` so downstream owners do not need to rediscover context.
...
- [ ] opsgenie-migration-cutover-plan, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `opsgenie-migration-cutover-plan`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `opsgenie:migration-assistant` consumers with approval and credential context attached.
