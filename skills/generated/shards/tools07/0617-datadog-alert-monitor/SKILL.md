...
description: Use when tasks require datadog alert monitor with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# Datadog Alert Monitor
...
| Primary artifact | `datadog-alert-state-bundle` |
| Routing tag | `datadog:alert-monitor` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Alert Monitor into a deterministic, auth-checked workflow for monitors key datadog signals and raises actionable alerts..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Alert Monitor` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-alert-state-bundle | structured-artifact | yes | next workflow or operator |
| datadog-alert-state-bundle-scorecard | scorecard | yes | reviewer |
| datadog-alert-state-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the monitored signals, freshness expectations, and escalation policy for Datadog Alert Monitor, then lock the reviewed threshold set for Datadog.
2. Validate credential reuse, dedupe windows, and acknowledgement ownership before the monitor starts polling.
3. Implement monitors key datadog signals and raises actionable alerts. with deterministic threshold evaluation, duplicate suppression, and evidence attachment.
...
- Preserve request, response, and approval traces in `datadog-alert-state-bundle` so downstream owners do not need to rediscover context.
...
- [ ] datadog-alert-state-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-alert-state-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:alert-monitor` consumers with approval and credential context attached.
