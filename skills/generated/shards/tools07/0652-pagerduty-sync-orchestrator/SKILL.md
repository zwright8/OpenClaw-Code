---
name: pagerduty-sync-orchestrator
description: Use when tasks require pagerduty sync orchestrator with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---

# PagerDuty Sync Orchestrator
...
| Primary artifact | `pagerduty-reconciliation-bundle` |
| Routing tag | `pagerduty:sync-orchestrator` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because PagerDuty workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns PagerDuty Sync Orchestrator into a deterministic, auth-checked workflow for schedules and reconciles two-way sync for pagerduty records..

## Trigger Checklist
- [ ] The task explicitly requires `PagerDuty Sync Orchestrator` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| pagerduty-reconciliation-bundle | structured-artifact | yes | next workflow or operator |
| pagerduty-reconciliation-bundle-scorecard | scorecard | yes | reviewer |
| pagerduty-reconciliation-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the systems, entities, and conflict policy for PagerDuty Sync Orchestrator, then checkpoint the last known good cursor on both sides of PagerDuty.
2. Validate credential reuse and rate-limit posture for every participating endpoint before the sync window opens.
3. Specify delta-fetch, reconciliation, and idempotency contracts for schedules and reconciles two-way sync for pagerduty records., including replay safety and backfill rules.
...
- Preserve request, response, and approval traces in `pagerduty-reconciliation-bundle` so downstream owners do not need to rediscover context.
...
- [ ] pagerduty-reconciliation-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `pagerduty-reconciliation-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `pagerduty:sync-orchestrator` consumers with approval and credential context attached.
