---
name: grafana-report-builder
description: Use when tasks require grafana report builder with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `grafana-report-bundle` |
| Routing tag | `grafana:report-builder` |
| Mutating | `no` |
| Release cycles | `1` |

## Why This Skill Exists
We need this skill because Grafana workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Grafana Report Builder into a deterministic, auth-checked workflow for generates operational and kpi reports from grafana activity..

## Trigger Checklist
- [ ] The task explicitly requires `Grafana Report Builder` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| grafana-report-bundle | structured-artifact | yes | next workflow or operator |
| grafana-report-bundle-scorecard | scorecard | yes | reviewer |
| grafana-report-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the report audience, metric definitions, and freshness threshold for Grafana Report Builder, then confirm which Grafana entities are in scope.
2. Validate credential reuse and ensure the query window matches the reporting cadence before any render starts.
3. Implement generates operational and kpi reports from grafana activity. with deterministic metric synthesis, template versioning, and publication gating.
...
- Preserve request, response, and approval traces in `grafana-report-bundle` so downstream owners do not need to rediscover context.
...
- [ ] grafana-report-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `grafana-report-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `grafana:report-builder` consumers with approval and credential context attached.
