...
description: Use when tasks require datadog connector with credential-aware preflight, deterministic execution, validation gates, and handoff-ready artifacts.
---
...
| Primary artifact | `datadog-connection-trace-bundle` |
| Routing tag | `datadog:connector` |
| Mutating | `yes` |
| Release cycles | `2` |

## Why This Skill Exists
We need this skill because Datadog workflows degrade when auth, schema, and side-effect handling drift when integrations are run ad hoc. This specific skill turns Datadog Connector into a deterministic, auth-checked workflow for unified read/write connector for datadog apis and data..

## Trigger Checklist
- [ ] The task explicitly requires `Datadog Connector` rather than generic brainstorming.
...
| --- | --- | --- | --- |
| datadog-connection-trace-bundle | structured-artifact | yes | next workflow or operator |
| datadog-connection-trace-bundle-scorecard | scorecard | yes | reviewer |
| datadog-connection-trace-bundle-handoff | handoff-packet | yes | downstream owner |

## Step-by-Step Implementation Guide
1. Define the objective, target resources, and permitted side effects for Datadog Connector, then confirm the smallest read-only probe that proves access to Datadog.
...
3. Specify the request and response contract for unified read/write connector for datadog apis and data., including idempotency rules and provider rate-limit posture.
...
- Preserve request, response, and approval traces in `datadog-connection-trace-bundle` so downstream owners do not need to rediscover context.
...
- [ ] datadog-connection-trace-bundle, scorecard, and handoff packet were produced.
...
## Handoff Contract
- **Produces:** `datadog-connection-trace-bundle`, execution scorecard, approval trace, and next actions.
...
- **Downstream hint:** route to `datadog:connector` consumers with approval and credential context attached.
