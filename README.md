...
Tsallis-INF policies (`tsallis_inf`/`sw_tsallis_inf`/`adwin_tsallis_inf`/`d_tsallis_inf`) provide Tsallis-entropy adversarial weighting using the same propensity-aware implicit-loss telemetry with a dedicated learning-rate scale (`--tsallis-eta-scale`) and optional recency adaptation via sliding-window/adaptive-window/discounted variants.
