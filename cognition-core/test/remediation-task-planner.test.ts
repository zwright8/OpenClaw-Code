...
import { buildRemediationTaskArtifacts } from '../scripts/plan-remediation-tasks.ts';
import { buildRegressionGateSummary, buildRemediationTaskPlanArtifact } from '../scripts/productivity-scorecard.ts';
...
test('buildRemediationTaskArtifacts rounds breach diagnostics for stable reason strings', () => {
    ...
    assert.equal(bundle.artifacts[0].threshold, 70);
    assert.equal(bundle.artifacts[0].actual, 42.13);
    assert.equal(bundle.artifacts[0].gap, 27.87);
});


test('buildRemediationTaskPlanArtifact emits stable remediation artifact shape', () => {
    const remediationPlan = [
        {
            metric: 'automationCoverage',
            priority: 'P1',
            title: 'Increase automation coverage',
            rationale: 'Threshold miss detected.',
            action: 'Automate additional eligible tasks.'
        }
    ];
    ...
    const thresholdBreaches = [
        {
            ...
            priority: 'P1',
            title: 'Increase automation coverage',
            rationale: 'Threshold breach for automationCoverage: expected gte 70 percent, actual 42 percent, miss 28 percent.',
            reasonPayload: {
                code: 'THRESHOLD_BREACH',
                metric: 'automationCoverage',
                comparison: 'gte',
    ...
    const first = buildRemediationTaskPlanArtifact(remediationPlan, thresholdBreaches, 'cognition-core/reports/productivity-scorecard.latest.json');
    const second = buildRemediationTaskPlanArtifact([...remediationPlan].reverse(), [...thresholdBreaches].reverse(), 'cognition-core/reports/productivity-scorecard.latest.json');

    assert.deepEqual(first.tasks, second.tasks);
    assert.deepEqual(first.artifacts, second.artifacts);
    assert.equal(first.artifacts.length, 1);
    assert.equal(first.artifacts[0].sourceReport, 'cognition-core/reports/productivity-scorecard.latest.json');
    assert.equal(first.artifacts[0].comparison, 'gte');
    assert.deepEqual(first.artifacts[0].regressionReasonPayload, {
        code: 'THRESHOLD_BREACH',
        metric: 'automationCoverage',
        comparison: 'gte',
