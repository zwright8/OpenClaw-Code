...
test('OpenClawBot replays cached execution for duplicate task IDs with identical payload', async () => {
    const bot = new OpenClawBot({
        repoRoot: REPO_ROOT,
    ...
    const task = buildTaskRequest({
        ...
        from: 'agent:main',
        target: 'agent:skills-runtime',
        task: 'Run skill runtime for id 1 (replay test)',
        context: {
            skillId: 1
        },
        createdAt: 500_320
    ...
    const second = await bot.executeTask(task);

    assert.equal(first.status, 'success');
    assert.equal(second.status, 'success');
    assert.equal(second.mode, first.mode);
    assert.equal(second.output, first.output);
    assert.equal(second.metrics.replayCacheHit, 1);
});

test('OpenClawBot rejects replay collisions when the same task ID is reused with different payload', async () => {
    const bot = new OpenClawBot({
        repoRoot: REPO_ROOT,
        nowFactory: () => 4_300_000
    });

    const originalTask = buildTaskRequest({
        id: '00000000-0000-4000-8000-000000000910',
        from: 'agent:main',
        target: 'agent:skills-runtime',
        task: 'Run skill runtime for id 1 (collision test)',
        context: {
            skillId: 1
        },
        createdAt: 500_330
    });
    const collidingTask = buildTaskRequest({
        id: '00000000-0000-4000-8000-000000000910',
        from: 'agent:main',
        target: 'agent:skills-runtime',
        task: 'Run skill runtime for id 2 (collision test)',
        context: {
            skillId: 2
        },
        createdAt: 500_331
    ...
    const first = await bot.executeTask(originalTask);
    const second = await bot.executeTask(collidingTask);

    assert.equal(first.status, 'success');
    assert.equal(second.mode, 'generic');
    assert.equal(second.status, 'failure');
    assert.match(second.output, /replay task-id collision/i);
    assert.equal(second.metrics.replayCollision, 1);
});
