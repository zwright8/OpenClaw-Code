...
function formatMarkdown(result) {
    const lines = [
        ...
        '## Observability',
        '',
        `Trace coverage: ${(result.summary.traceCoverage * 100).toFixed(2)}%`,
        `Trace event coverage: ${(result.summary.traceEventCoverage * 100).toFixed(2)}%`,
        `Tool trace coverage: ${(result.summary.toolTraceCoverage * 100).toFixed(2)}%`,
        `Traceparent coverage: ${(Number(result.summary.traceparentCoverage) * 100).toFixed(2)}%`,
        `Linked trace coverage: ${(Number(result.summary.traceLinkCoverage) * 100).toFixed(2)}%`,
        `Trace orphan rate: ${(Number(result.summary.traceOrphanRate) * 100).toFixed(2)}%`,
        `Orphaned trace events: ${Number(result.summary.observability?.orphanedTraceEvents) || 0}`,
        '',
