...
Options:
  ...
  --thompson-top-two-probability <n>  Leader probability for tt_epsilon_ts|tt_bb_ts (0-1, challenger gets 1-p, default: 0.8)
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (token === '--bot-hedged-delay-auto-blend') {
            options.botHedgedDelayAutoBlend = parseFloatInRange(value, '--bot-hedged-delay-auto-blend', 0, 1);
            i++;
            continue;
        }
        ...
        if (token === '--bot-attempt-timeout-auto-blend') {
            options.botAttemptTimeoutAutoBlend = parseFloatInRange(value, '--bot-attempt-timeout-auto-blend', 0, 1);
            i++;
            continue;
        }
...
function printSummary(report) {
    ...
    console.log(`Bot hedged attempts launched: ${report.totals.botHedgedAttemptsLaunched || 0}`);
    console.log(`Bot hedged successful races: ${report.totals.botHedgedSuccesses || 0}`);
    console.log(`Bot hedged race wins: ${report.totals.botHedgedWins || 0}`);
