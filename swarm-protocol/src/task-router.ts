...
function pickWithPowerOfTwoChoices(eligible, options) {
    ...
    const pool = [...eligible].sort((a, b) => a.originalIndex - b.originalIndex);
    const firstIndex = Math.floor(safeRandom(options) * pool.length);
    let secondIndex = Math.floor(safeRandom(options) * pool.length);
